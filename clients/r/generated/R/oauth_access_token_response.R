#' Create a new OauthAccessTokenResponse
#'
#' @description
...
OauthAccessTokenResponse <- R6::R6Class(
  ...
  public = list(
    ...
    `expires_in` = NULL,
    `scope` = NULL,
    `_discriminator_property_name` = 'response_type',
    `_discriminator_mapping_name` = c('authorization_code' = 'OauthAccessTokenResponseCode', 'client_credentials' = 'OauthAccessTokenResponseClientCredentials', 'everlasting_refresh' = 'OauthAccessTokenResponseEverlastingRefresh', 'integration_refresh' = 'OauthAccessTokenResponseIntegrationRefresh', 'refresh_token' = 'OauthAccessTokenResponseRefresh'),

    #' @description
    #' Initialize a new OauthAccessTokenResponse class.
    #'
    ...
    #'
    #' @return OauthAccessTokenResponse as a base R list.
    #' @examples
    #' # convert array of OauthAccessTokenResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert OauthAccessTokenResponse to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      return(OauthAccessTokenResponseObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of OauthAccessTokenResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of OauthAccessTokenResponse
    ...
    #' @return OauthAccessTokenResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to OauthAccessTokenResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`access_token`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OauthAccessTokenResponse: the required field `access_token` is missing."))
      }
      ...
      if (!is.null(input_json$`token_type`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OauthAccessTokenResponse: the required field `token_type` is missing."))
      }
      ...
      if (!is.null(input_json$`expires_in`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OauthAccessTokenResponse: the required field `expires_in` is missing."))
      }
      ...
      if (!is.null(input_json$`scope`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OauthAccessTokenResponse: the required field `scope` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of OauthAccessTokenResponse
    toString = function() {
      self$toJSONString()
    },
...
# OauthAccessTokenResponse$unlock()
#
## Below is an example to define the print function
# OauthAccessTokenResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# OauthAccessTokenResponse$lock()
