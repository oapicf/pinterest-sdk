#' Create a new LocalInventoryItemsBatch
#'
#' @description
#' Unified model for local inventory items batch operation
#'
#' @docType class
#' @title LocalInventoryItemsBatch
#' @description LocalInventoryItemsBatch Class
...
#' @export
LocalInventoryItemsBatch <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new LocalInventoryItemsBatch class.
    #'
    ...
    #'
    #' @return LocalInventoryItemsBatch as a base R list.
    #' @examples
    #' # convert array of LocalInventoryItemsBatch (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert LocalInventoryItemsBatch to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      LocalInventoryItemsBatchObject <- list()
      if (!is.null(self$`batch_id`)) {
        LocalInventoryItemsBatchObject[["batch_id"]] <-
          self$`batch_id`
      }
      if (!is.null(self$`completed_time`)) {
        LocalInventoryItemsBatchObject[["completed_time"]] <-
          self$`completed_time`
      }
      if (!is.null(self$`created_time`)) {
        LocalInventoryItemsBatchObject[["created_time"]] <-
          self$`created_time`
      }
      if (!is.null(self$`operation_results`)) {
        LocalInventoryItemsBatchObject[["operation_results"]] <-
          self$extractSimpleType(self$`operation_results`)
      }
      if (!is.null(self$`status`)) {
        LocalInventoryItemsBatchObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      return(LocalInventoryItemsBatchObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of LocalInventoryItemsBatch
    #'
    #' @param input_json the JSON input
    #' @return the instance of LocalInventoryItemsBatch
    ...
    #' @return LocalInventoryItemsBatch in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to LocalInventoryItemsBatch and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`batch_id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryItemsBatch: the required field `batch_id` is missing."))
      }
      ...
      if (!is.null(input_json$`created_time`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryItemsBatch: the required field `created_time` is missing."))
      }
      ...
      if (!is.null(input_json$`operation_results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryItemsBatch: the required field `operation_results` is missing."))
      }
      # check the required field `status`
      if (!is.null(input_json$`status`)) {
        stopifnot(R6::is.R6(input_json$`status`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryItemsBatch: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of LocalInventoryItemsBatch
    toString = function() {
      self$toJSONString()
    },
...
# LocalInventoryItemsBatch$unlock()
#
## Below is an example to define the print function
# LocalInventoryItemsBatch$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# LocalInventoryItemsBatch$lock()
