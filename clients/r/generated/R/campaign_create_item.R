#' Create a new CampaignCreateItem
#'
#' @description
#' Single campaign create item with create-specific defaults.
#'
#' @docType class
#' @title CampaignCreateItem
#' @description CampaignCreateItem Class
#' @format An \code{R6Class} generator object
#' @field ad_account_id Setting this field does nothing. The ad account ID gets set from the path parameter. character [optional]
#' @field app_id [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)  The store ID of the mobile app for `APP_INSTALL` campaigns. character [optional]
#' @field app_platform [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)  Platform of the mobile app for `APP_INSTALL` campaigns. \link{MobileAppPlatform} [optional]
...
#' @field default_ad_group_budget_in_micro_currency Setting this field does nothing. integer [optional]
...
#' @field name Campaign name - 255 chars max. character
#' @field objective_type  \link{ConversionObjectiveType}
...
CampaignCreateItem <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CampaignCreateItem class.
    #'
    #' @param name Campaign name - 255 chars max.
    #' @param objective_type objective_type
    #' @param ad_account_id Setting this field does nothing. The ad account ID gets set from the path parameter.
    #' @param app_id [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)  The store ID of the mobile app for `APP_INSTALL` campaigns.
    #' @param app_platform [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)  Platform of the mobile app for `APP_INSTALL` campaigns.
    ...
    #' @param default_ad_group_budget_in_micro_currency Setting this field does nothing.
    ...
    #' @param is_automated_campaign Note: This field is immutable unless the campaign is in draft status. Specifies whether the campaign was created in the automated campaign flow. When using Automated Campaigns, a daily spend cap must be set, and Pinterest Performance+ may not be used. The following objective types are supported: - CONSIDERATION - WEB\\_CONVERSION - SALES - LEADS - CATALOG\\_SALES. Default to FALSE.
    #' @param is_campaign_budget_optimization Note: This field is immutable unless the campaign is in draft status. Determines if a campaign automatically generates ad-group level budgets given a campaign budget to maximize campaign outcome. When using campaign budgets with a lifetime spend cap, the end time must be provided.. Default to TRUE.
    #' @param is_flexible_daily_budgets Determine if a campaign has setup for flexible daily budgets, also known as \"Pinterest Performance+ budgets\". Flexible daily budgets are only supported for campaign budget optimization with a daily spend cap, and not a lifetime spend cap.. Default to FALSE.
    ...
    #' @param is_performance_plus Note: This field is immutable unless the campaign is in draft status. You can only use this feature with the following objectives: - CONSIDERATION - WEB\\_CONVERSION - SALES - LEADS - CATALOG\\_SALES - APP\\_INSTALL Enable Pinterest Performance+ for your campaign. To learn more, see [Pinterest Performance+ Setup](https://developers.pinterest.com/docs/api-features/pinterest-performance-plus-setup/). Default to FALSE.
    #' @param is_top_of_search Note: This field is immutable unless the campaign is in draft status.  [Restricted](/docs/getting-started/using-beta-and-restricted-features/)  Have your ads and content appear at the top of search result lists in Pinterest.  You can only use this feature with the following objectives:  - CONSIDERATION  - WEB\\_CONVERSION  - SALES  - LEADS  - CATALOG\\_SALES  You cannot use this feature with Pinterest Performance+ or Premiere Spotlight enabled.. Default to FALSE.
    ...
    initialize = function(`name`, `objective_type`, `ad_account_id` = NULL, `app_id` = NULL, `app_platform` = NULL, `bid_options` = NULL, `daily_spend_cap` = NULL, `default_ad_group_budget_in_micro_currency` = NULL, `end_time` = NULL, `intended_promotion_type` = NULL, `is_automated_campaign` = FALSE, `is_campaign_budget_optimization` = TRUE, `is_flexible_daily_budgets` = FALSE, `is_ltv_optimized` = NULL, `is_performance_plus` = FALSE, `is_top_of_search` = FALSE, `lifetime_spend_cap` = NULL, `order_line_id` = NULL, `start_time` = NULL, `status` = NULL, `tracking_urls` = NULL, ...) {
      if (!missing(`name`)) {
      ...
      if (!is.null(`app_platform`)) {
        if (!(`app_platform` %in% c())) {
          stop(paste("Error! \"", `app_platform`, "\" cannot be assigned to `app_platform`. Must be .", sep = ""))
        }
        stopifnot(R6::is.R6(`app_platform`))
    ...
    #'
    #' @return CampaignCreateItem as a base R list.
    #' @examples
    #' # convert array of CampaignCreateItem (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CampaignCreateItem to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CampaignCreateItemObject <- list()
      if (!is.null(self$`ad_account_id`)) {
        CampaignCreateItemObject[["ad_account_id"]] <-
          self$`ad_account_id`
      }
      if (!is.null(self$`app_id`)) {
        CampaignCreateItemObject[["app_id"]] <-
          self$`app_id`
      }
      if (!is.null(self$`app_platform`)) {
        CampaignCreateItemObject[["app_platform"]] <-
          self$extractSimpleType(self$`app_platform`)
      }
      if (!is.null(self$`bid_options`)) {
        CampaignCreateItemObject[["bid_options"]] <-
          self$extractSimpleType(self$`bid_options`)
      }
      if (!is.null(self$`daily_spend_cap`)) {
        CampaignCreateItemObject[["daily_spend_cap"]] <-
          self$`daily_spend_cap`
      }
      if (!is.null(self$`default_ad_group_budget_in_micro_currency`)) {
        CampaignCreateItemObject[["default_ad_group_budget_in_micro_currency"]] <-
          self$`default_ad_group_budget_in_micro_currency`
      }
      if (!is.null(self$`end_time`)) {
        CampaignCreateItemObject[["end_time"]] <-
          self$`end_time`
      }
      if (!is.null(self$`intended_promotion_type`)) {
        CampaignCreateItemObject[["intended_promotion_type"]] <-
          self$extractSimpleType(self$`intended_promotion_type`)
      }
      if (!is.null(self$`is_automated_campaign`)) {
        CampaignCreateItemObject[["is_automated_campaign"]] <-
          self$`is_automated_campaign`
      }
      if (!is.null(self$`is_campaign_budget_optimization`)) {
        CampaignCreateItemObject[["is_campaign_budget_optimization"]] <-
          self$`is_campaign_budget_optimization`
      }
      if (!is.null(self$`is_flexible_daily_budgets`)) {
        CampaignCreateItemObject[["is_flexible_daily_budgets"]] <-
          self$`is_flexible_daily_budgets`
      }
      if (!is.null(self$`is_ltv_optimized`)) {
        CampaignCreateItemObject[["is_ltv_optimized"]] <-
          self$`is_ltv_optimized`
      }
      if (!is.null(self$`is_performance_plus`)) {
        CampaignCreateItemObject[["is_performance_plus"]] <-
          self$`is_performance_plus`
      }
      if (!is.null(self$`is_top_of_search`)) {
        CampaignCreateItemObject[["is_top_of_search"]] <-
          self$`is_top_of_search`
      }
      if (!is.null(self$`lifetime_spend_cap`)) {
        CampaignCreateItemObject[["lifetime_spend_cap"]] <-
          self$`lifetime_spend_cap`
      }
      if (!is.null(self$`name`)) {
        CampaignCreateItemObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`objective_type`)) {
        CampaignCreateItemObject[["objective_type"]] <-
          self$extractSimpleType(self$`objective_type`)
      }
      if (!is.null(self$`order_line_id`)) {
        CampaignCreateItemObject[["order_line_id"]] <-
          self$`order_line_id`
      }
      if (!is.null(self$`start_time`)) {
        CampaignCreateItemObject[["start_time"]] <-
          self$`start_time`
      }
      if (!is.null(self$`status`)) {
        CampaignCreateItemObject[["status"]] <-
          self$extractSimpleType(self$`status`)
      }
      if (!is.null(self$`tracking_urls`)) {
        CampaignCreateItemObject[["tracking_urls"]] <-
          self$extractSimpleType(self$`tracking_urls`)
      }
      return(CampaignCreateItemObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of CampaignCreateItem
    #'
    #' @param input_json the JSON input
    #' @return the instance of CampaignCreateItem
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`app_platform`)) {
        `app_platform_object` <- MobileAppPlatform$new()
        `app_platform_object`$fromJSON(jsonlite::toJSON(this_object$`app_platform`, auto_unbox = TRUE, digits = NA))
        self$`app_platform` <- `app_platform_object`
      }
      ...
      if (!is.null(this_object$`objective_type`)) {
        `objective_type_object` <- ConversionObjectiveType$new()
    ...
    #' @return CampaignCreateItem in JSON format
    ...
    fromJSONString = function(input_json) {
      ...
      self$`ad_account_id` <- this_object$`ad_account_id`
      self$`app_id` <- this_object$`app_id`
      self$`app_platform` <- MobileAppPlatform$new()$fromJSON(jsonlite::toJSON(this_object$`app_platform`, auto_unbox = TRUE, digits = NA))
      ...
      self$`lifetime_spend_cap` <- this_object$`lifetime_spend_cap`
      self$`name` <- this_object$`name`
      self$`objective_type` <- ConversionObjectiveType$new()$fromJSON(jsonlite::toJSON(this_object$`objective_type`, auto_unbox = TRUE, digits = NA))
      self$`order_line_id` <- this_object$`order_line_id`
      self$`start_time` <- this_object$`start_time`
    ...
    #' @description
    #' Validate JSON input with respect to CampaignCreateItem and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CampaignCreateItem: the required field `name` is missing."))
      }
      # check the required field `objective_type`
      if (!is.null(input_json$`objective_type`)) {
        stopifnot(R6::is.R6(input_json$`objective_type`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CampaignCreateItem: the required field `objective_type` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CampaignCreateItem
    toString = function() {
      self$toJSONString()
    },
...
# CampaignCreateItem$unlock()
#
## Below is an example to define the print function
# CampaignCreateItem$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CampaignCreateItem$lock()
