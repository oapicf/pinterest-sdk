...
IntegrationLogsRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return IntegrationLogsRequest as a base R list.
    #' @examples
    #' # convert array of IntegrationLogsRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert IntegrationLogsRequest to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`logs`)) {
        IntegrationLogsRequestObject[["logs"]] <-
          lapply(self$`logs`, function(x) x$toSimpleType())
      }
      return(IntegrationLogsRequestObject)
    },
