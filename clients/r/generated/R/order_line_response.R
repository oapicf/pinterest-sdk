...
OrderLineResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return OrderLineResponse as a base R list.
    #' @examples
    #' # convert array of OrderLineResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert OrderLineResponse to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`order_line`)) {
        OrderLineResponseObject[["order_line"]] <-
          lapply(self$`order_line`, function(x) x$toSimpleType())
      }
      return(OrderLineResponseObject)
    },
