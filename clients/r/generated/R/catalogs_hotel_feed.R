#' Create a new CatalogsHotelFeed
#'
#' @description
...
CatalogsHotelFeed <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return CatalogsHotelFeed as a base R list.
    #' @examples
    #' # convert array of CatalogsHotelFeed (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CatalogsHotelFeed to a base R type
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`location`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CatalogsHotelFeed: the required field `location` is missing."))
      }
      ...
      if (!is.null(input_json$`default_locale`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CatalogsHotelFeed: the required field `default_locale` is missing."))
      }
...
# CatalogsHotelFeed$unlock()
#
## Below is an example to define the print function
# CatalogsHotelFeed$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CatalogsHotelFeed$lock()
