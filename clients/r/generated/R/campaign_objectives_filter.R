#' Create a new CampaignObjectivesFilter
#'
#' @description
#' CampaignObjectivesFilter Class
#'
#' @docType class
#' @title CampaignObjectivesFilter
#' @description CampaignObjectivesFilter Class
#' @format An \code{R6Class} generator object
#' @field campaign_objective_types List of values for filtering. [\"WEB_SESSIONS\"] in BETA. list(\link{ObjectiveType}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CampaignObjectivesFilter <- R6::R6Class(
  "CampaignObjectivesFilter",
  public = list(
    `campaign_objective_types` = NULL,

    #' @description
    #' Initialize a new CampaignObjectivesFilter class.
    #'
    #' @param campaign_objective_types List of values for filtering. [\"WEB_SESSIONS\"] in BETA.
    #' @param ... Other optional arguments.
    initialize = function(`campaign_objective_types` = NULL, ...) {
      if (!is.null(`campaign_objective_types`)) {
        stopifnot(is.vector(`campaign_objective_types`), length(`campaign_objective_types`) != 0)
        sapply(`campaign_objective_types`, function(x) stopifnot(R6::is.R6(x)))
        self$`campaign_objective_types` <- `campaign_objective_types`
      }
    },

    #' @description
    #' Convert to an R object. This method is deprecated. Use `toSimpleType()` instead.
    toJSON = function() {
      .Deprecated(new = "toSimpleType", msg = "Use the '$toSimpleType()' method instead since that is more clearly named. Use '$toJSONString()' to get a JSON string")
      return(self$toSimpleType())
    },

    #' @description
    #' Convert to a List
    #'
    #' Convert the R6 object to a list to work more easily with other tooling.
    #'
    #' @return CampaignObjectivesFilter as a base R list.
    #' @examples
    #' # convert array of CampaignObjectivesFilter (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    #' df <- x |> map(\(y)y$toList()) |> map(as_tibble) |> list_rbind()
    #' df
    #' }
    toList = function() {
      return(self$toSimpleType())
    },

    #' @description
    #' Convert CampaignObjectivesFilter to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CampaignObjectivesFilterObject <- list()
      if (!is.null(self$`campaign_objective_types`)) {
        CampaignObjectivesFilterObject[["campaign_objective_types"]] <-
          lapply(self$`campaign_objective_types`, function(x) x$toSimpleType())
      }
      return(CampaignObjectivesFilterObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of CampaignObjectivesFilter
    #'
    #' @param input_json the JSON input
    #' @return the instance of CampaignObjectivesFilter
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`campaign_objective_types`)) {
        self$`campaign_objective_types` <- ApiClient$new()$deserializeObj(this_object$`campaign_objective_types`, "array[ObjectiveType]", loadNamespace("openapi"))
      }
      self
    },

    #' @description
    #' To JSON String
    #' 
    #' @param ... Parameters passed to `jsonlite::toJSON`
    #' @return CampaignObjectivesFilter in JSON format
    toJSONString = function(...) {
      simple <- self$toSimpleType()
      json <- jsonlite::toJSON(simple, auto_unbox = TRUE, digits = NA, ...)
      return(as.character(jsonlite::minify(json)))
    },

    #' @description
    #' Deserialize JSON string into an instance of CampaignObjectivesFilter
    #'
    #' @param input_json the JSON input
    #' @return the instance of CampaignObjectivesFilter
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`campaign_objective_types` <- ApiClient$new()$deserializeObj(this_object$`campaign_objective_types`, "array[ObjectiveType]", loadNamespace("openapi"))
      self
    },

    #' @description
    #' Validate JSON input with respect to CampaignObjectivesFilter and throw an exception if invalid
    #'
    #' @param input the JSON input
    validateJSON = function(input) {
      input_json <- jsonlite::fromJSON(input)
    },

    #' @description
    #' To string (JSON format)
    #'
    #' @return String representation of CampaignObjectivesFilter
    toString = function() {
      self$toJSONString()
    },

    #' @description
    #' Return true if the values in all fields are valid.
    #'
    #' @return true if the values in all fields are valid.
    isValid = function() {
      if (length(self$`campaign_objective_types`) > 7) {
        return(FALSE)
      }
      if (length(self$`campaign_objective_types`) < 1) {
        return(FALSE)
      }

      TRUE
    },

    #' @description
    #' Return a list of invalid fields (if any).
    #'
    #' @return A list of invalid fields (if any).
    getInvalidFields = function() {
      invalid_fields <- list()
      if (length(self$`campaign_objective_types`) > 7) {
        invalid_fields["campaign_objective_types"] <- "Invalid length for `campaign_objective_types`, number of items must be less than or equal to 7."
      }
      if (length(self$`campaign_objective_types`) < 1) {
        invalid_fields["campaign_objective_types"] <- "Invalid length for ``, number of items must be greater than or equal to 1."
      }

      invalid_fields
    },

    #' @description
    #' Print the object
    print = function() {
      print(jsonlite::prettify(self$toJSONString()))
      invisible(self)
    }
  ),
  # Lock the class to prevent modifications to the method or field
  lock_class = TRUE
)
## Uncomment below to unlock the class to allow modifications of the method or field
# CampaignObjectivesFilter$unlock()
#
## Below is an example to define the print function
# CampaignObjectivesFilter$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CampaignObjectivesFilter$lock()

