#' Create a new LocalInventoryUpsertOperation
#'
#' @description
#' Upsert operation for local inventory item
#'
#' @docType class
#' @title LocalInventoryUpsertOperation
#' @description LocalInventoryUpsertOperation Class
...
#' @export
LocalInventoryUpsertOperation <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new LocalInventoryUpsertOperation class.
    #'
    ...
    #'
    #' @return LocalInventoryUpsertOperation as a base R list.
    #' @examples
    #' # convert array of LocalInventoryUpsertOperation (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert LocalInventoryUpsertOperation to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      LocalInventoryUpsertOperationObject <- list()
      if (!is.null(self$`attributes`)) {
        LocalInventoryUpsertOperationObject[["attributes"]] <-
          self$extractSimpleType(self$`attributes`)
      }
      if (!is.null(self$`item_id`)) {
        LocalInventoryUpsertOperationObject[["item_id"]] <-
          self$`item_id`
      }
      if (!is.null(self$`operation`)) {
        LocalInventoryUpsertOperationObject[["operation"]] <-
          self$`operation`
      }
      if (!is.null(self$`store_code`)) {
        LocalInventoryUpsertOperationObject[["store_code"]] <-
          self$`store_code`
      }
      return(LocalInventoryUpsertOperationObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of LocalInventoryUpsertOperation
    #'
    #' @param input_json the JSON input
    #' @return the instance of LocalInventoryUpsertOperation
    ...
    #' @return LocalInventoryUpsertOperation in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to LocalInventoryUpsertOperation and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`attributes`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryUpsertOperation: the required field `attributes` is missing."))
      }
      ...
      if (!is.null(input_json$`item_id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryUpsertOperation: the required field `item_id` is missing."))
      }
      ...
      if (!is.null(input_json$`operation`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryUpsertOperation: the required field `operation` is missing."))
      }
      ...
      if (!is.null(input_json$`store_code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocalInventoryUpsertOperation: the required field `store_code` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of LocalInventoryUpsertOperation
    toString = function() {
      self$toJSONString()
    },
...
# LocalInventoryUpsertOperation$unlock()
#
## Below is an example to define the print function
# LocalInventoryUpsertOperation$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# LocalInventoryUpsertOperation$lock()
