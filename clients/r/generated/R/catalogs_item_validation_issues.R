...
CatalogsItemValidationIssues <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`errors`, `item_id`, `item_number`, `warnings`, ...) {
      if (!missing(`errors`)) {
    ...
    #'
    #' @return CatalogsItemValidationIssues as a base R list.
    #' @examples
    #' # convert array of CatalogsItemValidationIssues (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CatalogsItemValidationIssues to a base R type
    #'
