...
MembersToDeleteBody <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return MembersToDeleteBody as a base R list.
    #' @examples
    #' # convert array of MembersToDeleteBody (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MembersToDeleteBody to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`members`)) {
        MembersToDeleteBodyObject[["members"]] <-
          lapply(self$`members`, function(x) x$toSimpleType())
      }
      return(MembersToDeleteBodyObject)
    },
