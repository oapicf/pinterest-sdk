#' Create a new TargetingSpecOperationLocation
#'
#' @description
#' TargetingSpecOperationLocation Class
#'
#' @docType class
#' @title TargetingSpecOperationLocation
#' @description TargetingSpecOperationLocation Class
...
#' @export
TargetingSpecOperationLocation <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new TargetingSpecOperationLocation class.
    #'
    #' @param field field
    ...
    initialize = function(`field`, `operation`, `values`, ...) {
      if (!missing(`field`)) {
        if (!(`field` %in% c("LOCATION"))) {
          stop(paste("Error! \"", `field`, "\" cannot be assigned to `field`. Must be \"LOCATION\".", sep = ""))
        }
    ...
    #'
    #' @return TargetingSpecOperationLocation as a base R list.
    #' @examples
    #' # convert array of TargetingSpecOperationLocation (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert TargetingSpecOperationLocation to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      TargetingSpecOperationLocationObject <- list()
      if (!is.null(self$`field`)) {
        TargetingSpecOperationLocationObject[["field"]] <-
          self$`field`
      }
      if (!is.null(self$`operation`)) {
        TargetingSpecOperationLocationObject[["operation"]] <-
          self$extractSimpleType(self$`operation`)
      }
      if (!is.null(self$`values`)) {
        TargetingSpecOperationLocationObject[["values"]] <-
          self$`values`
      }
      return(TargetingSpecOperationLocationObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of TargetingSpecOperationLocation
    #'
    #' @param input_json the JSON input
    #' @return the instance of TargetingSpecOperationLocation
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`field`)) {
        if (!is.null(this_object$`field`) && !(this_object$`field` %in% c("LOCATION"))) {
          stop(paste("Error! \"", this_object$`field`, "\" cannot be assigned to `field`. Must be \"LOCATION\".", sep = ""))
        }
    ...
    #' @return TargetingSpecOperationLocation in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to TargetingSpecOperationLocation and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`field`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for TargetingSpecOperationLocation: the required field `field` is missing."))
      }
      # check the required field `operation`
      if (!is.null(input_json$`operation`)) {
        stopifnot(R6::is.R6(input_json$`operation`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for TargetingSpecOperationLocation: the required field `operation` is missing."))
      }
      ...
      if (!is.null(input_json$`values`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for TargetingSpecOperationLocation: the required field `values` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of TargetingSpecOperationLocation
    toString = function() {
      self$toJSONString()
    },
...
# TargetingSpecOperationLocation$unlock()
#
## Below is an example to define the print function
# TargetingSpecOperationLocation$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# TargetingSpecOperationLocation$lock()
