...
CatalogsProductGroupFilterKeys <- R6::R6Class(
  ...
  public = list(
    ...
    any_of = list("CatalogsBaseFilterKeys", "ProductGroupReferenceFilter"),

    #' @description
    #' Initialize a new CatalogsProductGroupFilterKeys.
    #'
    #' @param instance an instance of the object defined in the anyOf schemas: "CatalogsBaseFilterKeys", "ProductGroupReferenceFilter"
    initialize = function(instance = NULL) {
      if (is.null(instance)) {
        # do nothing
      } else if (get(class(instance)[[1]], pos = -1)$classname ==  "CatalogsBaseFilterKeys") {
        self$actual_instance <- instance
        self$actual_type <- "CatalogsBaseFilterKeys"
      } else if (get(class(instance)[[1]], pos = -1)$classname ==  "ProductGroupReferenceFilter") {
        self$actual_instance <- instance
        self$actual_type <- "ProductGroupReferenceFilter"
      } else {
        stop(paste("Failed to initialize CatalogsProductGroupFilterKeys with anyOf schemas CatalogsBaseFilterKeys, ProductGroupReferenceFilter. Provided class name: ",
    ...
    fromJSON = function(input) {
      ...
      `CatalogsBaseFilterKeys_result` <- tryCatch({
          `CatalogsBaseFilterKeys`$public_methods$validateJSON(input)
          `CatalogsBaseFilterKeys_instance` <- `CatalogsBaseFilterKeys`$new()
          self$actual_instance <- `CatalogsBaseFilterKeys_instance`$fromJSON(input)
          self$actual_type <- "CatalogsBaseFilterKeys"
          return(self)
        },
      ...
      if (!is.null(`CatalogsBaseFilterKeys_result`["error"])) {
        error_messages <- append(error_messages, `CatalogsBaseFilterKeys_result`["message"])
      }
      ...
      # no match
      stop(paste("No match found when deserializing the input into CatalogsProductGroupFilterKeys with anyOf schemas CatalogsBaseFilterKeys, ProductGroupReferenceFilter. Details: >>",
    ...
    #' @description
    #' Convert CatalogsProductGroupFilterKeys to a base R type
    #'
