...
Pin <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return Pin as a base R list.
    #' @examples
    #' # convert array of Pin (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert Pin to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PinObject <- list()
      if (!is.null(self$`ai_disclosures`)) {
        PinObject[["ai_disclosures"]] <-
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Pin: the required field `id` is missing."))
      }
    },
