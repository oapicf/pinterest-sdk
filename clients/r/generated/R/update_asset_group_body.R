...
UpdateAssetGroupBody <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return UpdateAssetGroupBody as a base R list.
    #' @examples
    #' # convert array of UpdateAssetGroupBody (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert UpdateAssetGroupBody to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`asset_groups_to_update`)) {
        UpdateAssetGroupBodyObject[["asset_groups_to_update"]] <-
          lapply(self$`asset_groups_to_update`, function(x) x$toSimpleType())
      }
      return(UpdateAssetGroupBodyObject)
    },
