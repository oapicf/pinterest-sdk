  ...
  public = list(
    ...
    initialize = function(`country`, `items`, `language`, `operation`, ...) {
      ...
      if (!missing(`operation`)) {
        if (!(`operation` %in% c("DELETE_DISCONTINUED"))) {
          stop(paste("Error! \"", `operation`, "\" cannot be assigned to `operation`. Must be \"DELETE_DISCONTINUED\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`operation`)) {
        if (!is.null(this_object$`operation`) && !(this_object$`operation` %in% c("DELETE_DISCONTINUED"))) {
          stop(paste("Error! \"", this_object$`operation`, "\" cannot be assigned to `operation`. Must be \"DELETE_DISCONTINUED\".", sep = ""))
        }
