#' Create a new AdvancedAuctionItem
#'
#' @description
#' AdvancedAuctionItem Class
#'
#' @docType class
...
AdvancedAuctionItem <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`item_id`, `country`, `language`, `bid_options`, ...) {
      if (!missing(`item_id`)) {
    ...
    #'
    #' @return AdvancedAuctionItem as a base R list.
    #' @examples
    #' # convert array of AdvancedAuctionItem (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert AdvancedAuctionItem to a base R type
    #'
    ...
    #' @description
    #' Deserialize JSON string into an instance of AdvancedAuctionItem
    #'
    ...
    #' @return AdvancedAuctionItem in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to AdvancedAuctionItem and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`item_id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AdvancedAuctionItem: the required field `item_id` is missing."))
      }
      # check the required field `country`
      if (!is.null(input_json$`country`)) {
        stopifnot(R6::is.R6(input_json$`country`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for AdvancedAuctionItem: the required field `country` is missing."))
      }
    ...
    #'
    #' @return String representation of AdvancedAuctionItem
    toString = function() {
      self$toJSONString()
    },
...
# AdvancedAuctionItem$unlock()
#
## Below is an example to define the print function
# AdvancedAuctionItem$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AdvancedAuctionItem$lock()
