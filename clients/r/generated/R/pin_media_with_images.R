  ...
  public = list(
    ...
    initialize = function(`media_type`, `items` = NULL, ...) {
      if (!missing(`media_type`)) {
        if (!(`media_type` %in% c("multiple_images"))) {
          stop(paste("Error! \"", `media_type`, "\" cannot be assigned to `media_type`. Must be \"multiple_images\".", sep = ""))
        }
    ...
    #'
    #' @return PinMediaWithImages as a base R list.
    #' @examples
    #' # convert array of PinMediaWithImages (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PinMediaWithImages to a base R type
    #'
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`media_type`)) {
        if (!is.null(this_object$`media_type`) && !(this_object$`media_type` %in% c("multiple_images"))) {
          stop(paste("Error! \"", this_object$`media_type`, "\" cannot be assigned to `media_type`. Must be \"multiple_images\".", sep = ""))
        }
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`media_type`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PinMediaWithImages: the required field `media_type` is missing."))
      }
    },
