#' Create a new ItemIdStoreCodePair
#'
#' @description
#' A pair of item_id and store_code that uniquely identifies a local inventory item
#'
#' @docType class
#' @title ItemIdStoreCodePair
#' @description ItemIdStoreCodePair Class
...
#' @export
ItemIdStoreCodePair <- R6::R6Class(
  "ItemIdStoreCodePair",
  public = list(
    `item_id` = NULL,
    `store_code` = NULL,
    ...
    #'
    #' @return ItemIdStoreCodePair as a base R list.
    #' @examples
    #' # convert array of ItemIdStoreCodePair (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ItemIdStoreCodePair to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ItemIdStoreCodePairObject <- list()
      if (!is.null(self$`item_id`)) {
        ItemIdStoreCodePairObject[["item_id"]] <-
          self$`item_id`
      }
      if (!is.null(self$`store_code`)) {
        ItemIdStoreCodePairObject[["store_code"]] <-
          self$`store_code`
      }
      return(ItemIdStoreCodePairObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ItemIdStoreCodePair
    #'
    #' @param input_json the JSON input
    #' @return the instance of ItemIdStoreCodePair
    ...
    #' @return ItemIdStoreCodePair in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ItemIdStoreCodePair and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`item_id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ItemIdStoreCodePair: the required field `item_id` is missing."))
      }
      ...
      if (!is.null(input_json$`store_code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ItemIdStoreCodePair: the required field `store_code` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ItemIdStoreCodePair
    toString = function() {
      self$toJSONString()
    },
...
# ItemIdStoreCodePair$unlock()
#
## Below is an example to define the print function
# ItemIdStoreCodePair$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ItemIdStoreCodePair$lock()
