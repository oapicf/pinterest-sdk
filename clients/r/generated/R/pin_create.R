...
#' @field sponsor_id The sponsor account id to request paid partnership from.  Currently the field is only available to a list of users in a closed beta. character [optional]
...
PinCreate <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`ai_disclosures` = NULL, `alt_text` = NULL, `board_id` = NULL, `board_section_id` = NULL, `description` = NULL, `dominant_color` = NULL, `link` = NULL, `media_source` = NULL, `parent_pin_id` = NULL, `sponsor_id` = NULL, `title` = NULL, ...) {
      ...
      if (!is.null(`sponsor_id`)) {
        if (!(is.character(`sponsor_id`) && length(`sponsor_id`) == 1)) {
          stop(paste("Error! Invalid data for `sponsor_id`. Must be a string:", `sponsor_id`))
        }
    ...
    #'
    #' @return PinCreate as a base R list.
    #' @examples
    #' # convert array of PinCreate (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PinCreate to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`sponsor_id`)) {
        PinCreateObject[["sponsor_id"]] <-
          self$`sponsor_id`
      }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`sponsor_id`)) {
        self$`sponsor_id` <- this_object$`sponsor_id`
      }
    ...
    isValid = function() {
      ...
      if (!str_detect(self$`sponsor_id`, "^\\d+$")) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      ...
      if (!str_detect(self$`sponsor_id`, "^\\d+$")) {
        invalid_fields["sponsor_id"] <- "Invalid value for `sponsor_id`, must conform to the pattern ^\\d+$."
      }
