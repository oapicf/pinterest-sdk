...
ItemIdFilter <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return ItemIdFilter as a base R list.
    #' @examples
    #' # convert array of ItemIdFilter (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ItemIdFilter to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`ITEM_ID`)) {
        ItemIdFilterObject[["ITEM_ID"]] <-
          self$extractSimpleType(self$`ITEM_ID`)
      }
      return(ItemIdFilterObject)
    },
