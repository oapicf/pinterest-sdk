  ...
  public = list(
    ...
    initialize = function(...) {
      ...
      if (!val %in% enumvec) {
        ...
        warning("Initializing CatalogsFeedProcessingStatus with DUMMY_ENUM. Use one of the valid values: ",
          paste0(enumvec, collapse = ", "),
          ". If you did not manually initialize CatalogsFeedProcessingStatus, this may already be overwritten by an enum loaded from a JSON config.")
      }
      private$value <- val
    },
    ...
    #' @description
    #' Convert CatalogsFeedProcessingStatus to a base R type
    #'
...
.parse_CatalogsFeedProcessingStatus <- function(vals) {
  res <- gsub("^\\[|\\]$", "", "[COMPLETED, FAILED, PROCESSING]")
  unlist(strsplit(res, ", "))
}
