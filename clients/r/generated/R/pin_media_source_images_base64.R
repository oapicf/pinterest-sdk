#' Create a new PinMediaSourceImagesBase64
#'
#' @description
#' Multiple Base64-based images media source
#'
#' @docType class
#' @title PinMediaSourceImagesBase64
#' @description PinMediaSourceImagesBase64 Class
#' @format An \code{R6Class} generator object
#' @field index  integer [optional]
#' @field items Array with image objects. list(\link{PinMediaSourceImagesBase64Item})
#' @field source_type The source type of the media. character
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PinMediaSourceImagesBase64 <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PinMediaSourceImagesBase64 class.
    #'
    ...
    initialize = function(`items`, `source_type`, `index` = NULL, ...) {
      ...
      if (!missing(`source_type`)) {
        if (!(`source_type` %in% c("multiple_image_base64"))) {
          stop(paste("Error! \"", `source_type`, "\" cannot be assigned to `source_type`. Must be \"multiple_image_base64\".", sep = ""))
        }
    ...
    #' @description
    #' Deserialize JSON string into an instance of PinMediaSourceImagesBase64
    #'
    #' @param input_json the JSON input
    #' @return the instance of PinMediaSourceImagesBase64
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[PinMediaSourceImagesBase64Item]", loadNamespace("openapi"))
      }
      if (!is.null(this_object$`source_type`)) {
        if (!is.null(this_object$`source_type`) && !(this_object$`source_type` %in% c("multiple_image_base64"))) {
          stop(paste("Error! \"", this_object$`source_type`, "\" cannot be assigned to `source_type`. Must be \"multiple_image_base64\".", sep = ""))
        }
    ...
    #' @description
    #' Validate JSON input with respect to PinMediaSourceImagesBase64 and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`items`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PinMediaSourceImagesBase64: the required field `items` is missing."))
      }
      ...
      if (!is.null(input_json$`source_type`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PinMediaSourceImagesBase64: the required field `source_type` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PinMediaSourceImagesBase64
    toString = function() {
      self$toJSONString()
    },
...
# PinMediaSourceImagesBase64$unlock()
#
## Below is an example to define the print function
# PinMediaSourceImagesBase64$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PinMediaSourceImagesBase64$lock()
