#' Create a new CatalogsCreativeAssetsProduct
#'
#' @description
#' CatalogsCreativeAssetsProduct Class
#'
#' @docType class
#' @title CatalogsCreativeAssetsProduct
#' @description CatalogsCreativeAssetsProduct Class
#' @format An \code{R6Class} generator object
#' @field catalog_type  character
#' @field metadata  \link{CatalogsCreativeAssetsProductMetadata}
#' @field pin  \link{Pin}
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CatalogsCreativeAssetsProduct <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CatalogsCreativeAssetsProduct class.
    #'
    ...
    #'
    #' @return CatalogsCreativeAssetsProduct as a base R list.
    #' @examples
    #' # convert array of CatalogsCreativeAssetsProduct (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CatalogsCreativeAssetsProduct to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CatalogsCreativeAssetsProductObject <- list()
      if (!is.null(self$`catalog_type`)) {
        CatalogsCreativeAssetsProductObject[["catalog_type"]] <-
          self$`catalog_type`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of CatalogsCreativeAssetsProduct
    #'
    #' @param input_json the JSON input
    #' @return the instance of CatalogsCreativeAssetsProduct
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`metadata`)) {
        `metadata_object` <- CatalogsCreativeAssetsProductMetadata$new()
    ...
    #' @return CatalogsCreativeAssetsProduct in JSON format
    ...
    fromJSONString = function(input_json) {
      ...
      self$`catalog_type` <- this_object$`catalog_type`
      self$`metadata` <- CatalogsCreativeAssetsProductMetadata$new()$fromJSON(jsonlite::toJSON(this_object$`metadata`, auto_unbox = TRUE, digits = NA))
      self$`pin` <- Pin$new()$fromJSON(jsonlite::toJSON(this_object$`pin`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to CatalogsCreativeAssetsProduct and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`catalog_type`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CatalogsCreativeAssetsProduct: the required field `catalog_type` is missing."))
      }
      # check the required field `metadata`
      if (!is.null(input_json$`metadata`)) {
        stopifnot(R6::is.R6(input_json$`metadata`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CatalogsCreativeAssetsProduct: the required field `metadata` is missing."))
      }
      # check the required field `pin`
      if (!is.null(input_json$`pin`)) {
        stopifnot(R6::is.R6(input_json$`pin`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CatalogsCreativeAssetsProduct: the required field `pin` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CatalogsCreativeAssetsProduct
    toString = function() {
      self$toJSONString()
    },
...
# CatalogsCreativeAssetsProduct$unlock()
#
## Below is an example to define the print function
# CatalogsCreativeAssetsProduct$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CatalogsCreativeAssetsProduct$lock()
