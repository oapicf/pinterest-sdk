...
LeadsExportResponseData <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return LeadsExportResponseData as a base R list.
    #' @examples
    #' # convert array of LeadsExportResponseData (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert LeadsExportResponseData to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`export_status`)) {
        LeadsExportResponseDataObject[["export_status"]] <-
          self$extractSimpleType(self$`export_status`)
      }
