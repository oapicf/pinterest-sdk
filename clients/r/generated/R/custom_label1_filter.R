...
CustomLabel1Filter <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return CustomLabel1Filter as a base R list.
    #' @examples
    #' # convert array of CustomLabel1Filter (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CustomLabel1Filter to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`CUSTOM_LABEL_1`)) {
        CustomLabel1FilterObject[["CUSTOM_LABEL_1"]] <-
          self$extractSimpleType(self$`CUSTOM_LABEL_1`)
      }
      return(CustomLabel1FilterObject)
    },
