# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate

context("Test CatalogsUpdateCreativeAssetsItem")

model_instance <- CatalogsUpdateCreativeAssetsItem$new()

test_that("creative_assets_id", {
  # tests for the property `creative_assets_id` (character)
  # The catalog creative assets item id in the merchant namespace

  # uncomment below to test the property
  #expect_equal(model.instance$`creative_assets_id`, "EXPECTED_RESULT")
})

test_that("operation", {
  # tests for the property `operation` (character)

  # uncomment below to test the property
  #expect_equal(model.instance$`operation`, "EXPECTED_RESULT")
})

test_that("attributes", {
  # tests for the property `attributes` (CatalogsUpdatableCreativeAssetsAttributes)

  # uncomment below to test the property
  #expect_equal(model.instance$`attributes`, "EXPECTED_RESULT")
})
