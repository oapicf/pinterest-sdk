

# QuerypinanalyticsmetrictypesItems

## Enum

The class is defined in **[QuerypinanalyticsmetrictypesItems.java](../../src/main/java/org/openapitools/model/QuerypinanalyticsmetrictypesItems.java)**


* `IMPRESSION` (value: `"IMPRESSION"`)

* `OUTBOUND_CLICK` (value: `"OUTBOUND_CLICK"`)

* `PIN_CLICK` (value: `"PIN_CLICK"`)

* `SAVE` (value: `"SAVE"`)

* `SAVE_RATE` (value: `"SAVE_RATE"`)

* `TOTAL_COMMENTS` (value: `"TOTAL_COMMENTS"`)

* `TOTAL_REACTIONS` (value: `"TOTAL_REACTIONS"`)

* `USER_FOLLOW` (value: `"USER_FOLLOW"`)

* `PROFILE_VISIT` (value: `"PROFILE_VISIT"`)

* `VIDEO_MRC_VIEW` (value: `"VIDEO_MRC_VIEW"`)

* `VIDEO_10_S_VIEW` (value: `"VIDEO_10S_VIEW"`)

* `QUARTILE_95_PERCENT_VIEW` (value: `"QUARTILE_95_PERCENT_VIEW"`)

* `VIDEO_V50_WATCH_TIME` (value: `"VIDEO_V50_WATCH_TIME"`)

* `VIDEO_START` (value: `"VIDEO_START"`)

* `VIDEO_AVG_WATCH_TIME` (value: `"VIDEO_AVG_WATCH_TIME"`)



