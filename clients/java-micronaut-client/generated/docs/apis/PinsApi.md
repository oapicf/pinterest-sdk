...
* **[client_credentials](auth.md#client_credentials)**, scopes: `boards:read`, `boards:write`, `pins:read`, `pins:write`
...
# **pinsDelete**
```java
Mono<Pin> PinsApi.pinsDelete(pinIdadAccountId)
```
...
# **pinsGet**
```java
Mono<Pin> PinsApi.pinsGet(pinIdadAccountIdpinMetrics)
```
...
# **pinsList**
```java
Mono<PinsList200Response> PinsApi.pinsList(pinFilterpinMetricsincludeProtectedPinspinTypecreativeTypesadAccountIddomaindomainsincludeProductTagObjbookmarkpageSize)
```
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **pinFilter** | [**PinFilter**](.md)| The filter to apply to the pins | [optional parameter] [enum: `exclude_native`, `exclude_repins`, `has_been_promoted`] |
| **pinMetrics** | `Boolean`| Specify whether to return 90d and lifetime Pin metrics. Total comments and total reactions are only available with lifetime Pin metrics. If Pin was created before &#x60;2023-03-20&#x60; lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then. | [optional parameter] [default to `false`] |
| **includeProtectedPins** | `Boolean`| Whether to include protected pins in the results | [optional parameter] [default to `false`] |
| **pinType** | [**PinType**](.md)| The type of pins to return, currently only enabled for private pins | [optional parameter] [enum: `PRIVATE`] |
...
| **domains** | [**List&lt;String&gt;**](String.md)| Only return pins with links whose domain matches any value in the list. Values are joined comma-separated on the wire (e.g. &#x60;?domains&#x3D;instagram.com,jcpenney.com&#x60;). | [optional parameter] |
...
# **pinsSave**
```java
Mono<Pin> PinsApi.pinsSave(pinIdpinsSaveRequestCreateadAccountId)
```
