// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * API version: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 */

package openapi




type ItemAttributesRequest struct {

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage0Link *string `json:"ad_image_0_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage0Tag *string `json:"ad_image_0_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage10Link *string `json:"ad_image_10_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage10Tag *string `json:"ad_image_10_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage11Link *string `json:"ad_image_11_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage11Tag *string `json:"ad_image_11_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage12Link *string `json:"ad_image_12_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage12Tag *string `json:"ad_image_12_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage13Link *string `json:"ad_image_13_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage13Tag *string `json:"ad_image_13_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage14Link *string `json:"ad_image_14_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage14Tag *string `json:"ad_image_14_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage15Link *string `json:"ad_image_15_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage15Tag *string `json:"ad_image_15_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage16Link *string `json:"ad_image_16_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage16Tag *string `json:"ad_image_16_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage17Link *string `json:"ad_image_17_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage17Tag *string `json:"ad_image_17_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage18Link *string `json:"ad_image_18_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage18Tag *string `json:"ad_image_18_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage19Link *string `json:"ad_image_19_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage19Tag *string `json:"ad_image_19_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage1Link *string `json:"ad_image_1_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage1Tag *string `json:"ad_image_1_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage2Link *string `json:"ad_image_2_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage2Tag *string `json:"ad_image_2_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage3Link *string `json:"ad_image_3_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage3Tag *string `json:"ad_image_3_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage4Link *string `json:"ad_image_4_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage4Tag *string `json:"ad_image_4_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage5Link *string `json:"ad_image_5_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage5Tag *string `json:"ad_image_5_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage6Link *string `json:"ad_image_6_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage6Tag *string `json:"ad_image_6_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage7Link *string `json:"ad_image_7_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage7Tag *string `json:"ad_image_7_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage8Link *string `json:"ad_image_8_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage8Tag *string `json:"ad_image_8_tag,omitempty"`

	// Ad image link that supplements main image for shopping campaigns. Image format:- Pixel size at least 75 x 75 Link guidelines:- Include extension in file name - Do not include template or placeholder images in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdImage9Link *string `json:"ad_image_9_link,omitempty"`

	// Ad image tag. If you provide an ad_image_x_link, include the image tag with the corresponding ad_image_x_tag attribute.
	AdImage9Tag *string `json:"ad_image_9_tag,omitempty"`

	// Allows advertisers to specify a separate URL that can be used to track traffic coming from Pinterest shopping ads. Must send full URL including tracking. Must begin with http:// or https://.
	AdLink *string `json:"ad_link,omitempty"`

	// Restricted. Ad video link that supplements main video for shopping campaigns. Video format:- Pixel size between 75 x 75 and 9450 x 9450 - File size smaller than 2 GB - Time span between 4 seconds and 15 minutes - Accepted formats: .MP4, .MOV, .M4V Link guidelines:- Include extension in file name - Do not include template or placeholder videos in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdVideo0Link *string `json:"ad_video_0_link,omitempty"`

	// Restricted. Ad video tag. If you provide an ad_video_x_link, include the video tag with the corresponding ad_video_x_tag attribute.
	AdVideo0Tag *string `json:"ad_video_0_tag,omitempty"`

	// Restricted. Ad video link that supplements main video for shopping campaigns. Video format:- Pixel size between 75 x 75 and 9450 x 9450 - File size smaller than 2 GB - Time span between 4 seconds and 15 minutes - Accepted formats: .MP4, .MOV, .M4V Link guidelines:- Include extension in file name - Do not include template or placeholder videos in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdVideo1Link *string `json:"ad_video_1_link,omitempty"`

	// Restricted. Ad video tag. If you provide an ad_video_x_link, include the video tag with the corresponding ad_video_x_tag attribute.
	AdVideo1Tag *string `json:"ad_video_1_tag,omitempty"`

	// Restricted. Ad video link that supplements main video for shopping campaigns. Video format:- Pixel size between 75 x 75 and 9450 x 9450 - File size smaller than 2 GB - Time span between 4 seconds and 15 minutes - Accepted formats: .MP4, .MOV, .M4V Link guidelines:- Include extension in file name - Do not include template or placeholder videos in link - Make URL accessible to Pinterest user-agent - Must start with http:// or https://
	AdVideo2Link *string `json:"ad_video_2_link,omitempty"`

	// Restricted. Ad video tag. If you provide an ad_video_x_link, include the video tag with the corresponding ad_video_x_tag attribute.
	AdVideo2Tag *string `json:"ad_video_2_tag,omitempty"`

	// <= 2000 characters The links to additional images for your product. Up to ten additional images can be used to show a product from different angles or to show different stages. Must begin with http:// or https://.
	AdditionalImageLink *[]string `json:"additional_image_link,omitempty"`

	// Set this attribute to TRUE if you're submitting items that are considered \"adult\". These will not be shown on Pinterest.
	Adult *bool `json:"adult,omitempty"`

	// The age group to apply a demographic range to the product. Must be one of the following values (upper or lowercased): 'newborn', 'infant', 'toddler', 'kids', or 'adult'.
	AgeGroup *string `json:"age_group,omitempty"`

	// AI content disclosures for individual assets (image_link, additional_image_link, or video_link) on this item. Each entry declares which disclosure types apply to a single asset URL.
	AiDisclosures []CatalogsAiContentDisclosure `json:"ai_disclosures,omitempty"`

	// The deep link to the product on the Android app.
	AndroidDeepLink string `json:"android_deep_link,omitempty"`

	// The availability of the product. Must be one of the following values (upper or lowercased): 'in stock', 'out of stock', 'preorder'.
	Availability string `json:"availability,omitempty"`

	// Average reviews for the item. Can be a number from 1-5.
	AverageReviewRating *float32 `json:"average_review_rating,omitempty"`

	// The brand of the product.
	Brand *string `json:"brand,omitempty"`

	// This attribute is not supported anymore.
	// Deprecated
	CheckoutEnabled *bool `json:"checkout_enabled,omitempty"`

	// The primary color of the product.
	Color *string `json:"color,omitempty"`

	// The condition of the product. Must be one of the following values (upper or lowercased): 'new', 'used', or 'refurbished'.
	Condition *string `json:"condition,omitempty"`

	// <= 511 characters for retail and creative asset catalogs, <= 127 characters for hotel catalogs. Custom grouping of products.
	CustomLabel0 *string `json:"custom_label_0,omitempty"`

	// <= 511 characters for retail and creative asset catalogs, <= 127 characters for hotel catalogs. Custom grouping of products.
	CustomLabel1 *string `json:"custom_label_1,omitempty"`

	// <= 511 characters for retail and creative asset catalogs, <= 127 characters for hotel catalogs. Custom grouping of products.
	CustomLabel2 *string `json:"custom_label_2,omitempty"`

	// <= 511 characters for retail and creative asset catalogs, <= 127 characters for hotel catalogs. Custom grouping of products.
	CustomLabel3 *string `json:"custom_label_3,omitempty"`

	// <= 511 characters for retail and creative asset catalogs, <= 127 characters for hotel catalogs. Custom grouping of products.
	CustomLabel4 *string `json:"custom_label_4,omitempty"`

	// An attribute for any integer information ranging from 0 to 4,294,967,295, which can be used to group items.
	CustomNumber0 *int32 `json:"custom_number_0,omitempty"`

	// An attribute for any integer information ranging from 0 to 4,294,967,295, which can be used to group items.
	CustomNumber1 *int32 `json:"custom_number_1,omitempty"`

	// An attribute for any integer information ranging from 0 to 4,294,967,295, which can be used to group items.
	CustomNumber2 *int32 `json:"custom_number_2,omitempty"`

	// An attribute for any integer information ranging from 0 to 4,294,967,295, which can be used to group items.
	CustomNumber3 *int32 `json:"custom_number_3,omitempty"`

	// An attribute for any integer information ranging from 0 to 4,294,967,295, which can be used to group items.
	CustomNumber4 *int32 `json:"custom_number_4,omitempty"`

	// <= 10000 characters. The description of the product.
	Description string `json:"description,omitempty"`

	// The item is free to ship.
	FreeShippingLabel *bool `json:"free_shipping_label,omitempty"`

	// The minimum order purchase necessary for the customer to get free shipping. Only relevant if free shipping is offered.
	FreeShippingLimit *string `json:"free_shipping_limit,omitempty"`

	// The gender associated with the product. Must be one of the following values (upper or lowercased): 'male', 'female', or 'unisex'.
	Gender *string `json:"gender,omitempty"`

	// The categorization of the product based on the standardized Google Product Taxonomy. This is a set taxonomy. Both the text values and numeric codes are accepted.
	GoogleProductCategory *string `json:"google_product_category,omitempty"`

	Gtin *UpdatableItemAttributesGtin `json:"gtin,omitempty"`

	// <= 127 characters. The user-created unique ID that represents the product. Only Unicode characters are accepted.
	// Deprecated
	Id string `json:"id,omitempty"`

	ImageLink ItemAttributesRequestImageLink `json:"image_link,omitempty"`

	// Installment price of the product. Expected format: <payment_count>:<payment_amount> <currency>
	InstallmentPrice *string `json:"installment_price,omitempty"`

	// The deep link to the product on the iOS app.
	IosDeepLink string `json:"ios_deep_link,omitempty"`

	// <= 127 characters. The parent ID of the product.
	ItemGroupId *string `json:"item_group_id,omitempty"`

	// The millisecond timestamp when the item was lastly modified by the merchant.
	LastUpdatedTime *int64 `json:"last_updated_time,omitempty"`

	// <= 511 characters. The landing page for the product.
	Link string `json:"link,omitempty"`

	// The material used to make the product.
	Material *string `json:"material,omitempty"`

	// The minimum advertised price of the product. It supports the following formats: \"19.99 USD\", \"19.99USD\" and \"19.99\". If the currency is not included, we default to US dollars.
	MinAdPrice *string `json:"min_ad_price,omitempty"`

	// The mobile-optimized version of your landing page. Must begin with http:// or https://.
	MobileLink *string `json:"mobile_link,omitempty"`

	// Manufacturer Part Number are alpha-numeric codes created by the manufacturer of a product to uniquely identify it among all products from the same manufacturer.
	Mpn *string `json:"mpn,omitempty"`

	// The number of ratings for the item.
	NumberOfRatings *int32 `json:"number_of_ratings,omitempty"`

	// The number of reviews available for the item.
	NumberOfReviews *int32 `json:"number_of_reviews,omitempty"`

	// The description of the pattern used for the product.
	Pattern *string `json:"pattern,omitempty"`

	// The price of the product. It supports the following formats: \"24.99 USD\", \"24.99USD\" and \"24.99\". If the currency is not included, we default to US dollars.
	Price string `json:"price,omitempty"`

	// <= 1000 characters. The categorization of your product based on your custom product taxonomy. Subcategories must be sent separated by \" > \".
	ProductType *string `json:"product_type,omitempty"`

	// A unique identifier referencing the promotion associated with this catalog item.
	PromotionId *string `json:"promotion_id,omitempty"`

	// The discounted price of the product. The sale_price must be lower than the price. It supports the following formats: \"14.99 USD\", \"14.99USD\" and \"14.99\".
	SalePrice *string `json:"sale_price,omitempty"`

	// Sale price effective date. Expected format: <start_date>/<end_date> (ISO 8601 format)
	SalePriceEffectiveDate *string `json:"sale_price_effective_date,omitempty"`

	// By default, product pins created from a catalog are able to be saved by Pinners. If you want to disable the save pin feature, set this attribute to true. This feature is only available for allowlisted merchants. Please contact your account manager to enable this feature.
	SavePinDisabled bool `json:"save_pin_disabled,omitempty"`

	// Shipping consists of one group of up to four elements, country, region, service (all optional) and price (required). All colons, even for blank values, are required.
	Shipping *string `json:"shipping,omitempty"`

	// The height of the package needed to ship the product. Ensure there is a space between the numeric string and the metric.
	ShippingHeight *string `json:"shipping_height,omitempty"`

	// The weight of the product. Ensure there is a space between the numeric string and the metric.
	ShippingWeight *string `json:"shipping_weight,omitempty"`

	// The width of the package needed to ship the product. Ensure there is a space between the numeric string and the metric.
	ShippingWidth *string `json:"shipping_width,omitempty"`

	// The size of the product.
	Size *string `json:"size,omitempty"`

	// Indicates the country's sizing system in which you are submitting your product. Must be one of the following values (upper or lowercased): 'US', 'UK', 'EU', 'DE', 'FR', 'JP', 'CN', 'IT', 'BR', 'MEX', or 'AU'.
	SizeSystem *string `json:"size_system,omitempty"`

	// Additional description for the size. Must be one of the following values (upper or lowercased): 'regular', 'petite', 'plus', 'big_and_tall', or 'maternity'.
	SizeType *string `json:"size_type,omitempty"`

	// Tax consists of one group of up to four elements, country, region, rate (all required) and tax_ship (optional). All colons, even for blank values, are required.
	Tax *string `json:"tax,omitempty"`

	// <= 500 characters. The name of the product.
	Title string `json:"title,omitempty"`

	// Unit pricing base measure of the product. Expected format: <base_measure> <unit_type>
	UnitPricingBaseMeasure *string `json:"unit_pricing_base_measure,omitempty"`

	// Unit pricing total measure of the product. Expected format: <total_units> <unit_type>
	UnitPricingMeasure *string `json:"unit_pricing_measure,omitempty"`

	// Options for this variant. People will see these options next to your Pin and can select the one they want. List them in the order you want them displayed.
	VariantNames *[]string `json:"variant_names,omitempty"`

	// Option values for this variant. People will see these options next to your Pin and can select the one they want. List them in the order you want them displayed. The order of the variant values must be consistent with the order of the variant names.
	VariantValues *[]string `json:"variant_values,omitempty"`

	// <= 2,000 characters Hosted link to the product video. File types for linked videos must be .mp4, .mov or .m4v. File size cannot exceed 2GB.
	VideoLink *string `json:"video_link,omitempty"`
}

// AssertItemAttributesRequestRequired checks complex required fields (models, arrays, maps) and embedded parents.
// Primitive required fields are validated for JSON request bodies in UnmarshalJSON so zero values remain valid.
func AssertItemAttributesRequestRequired(obj ItemAttributesRequest) error {
	for _, el := range obj.AiDisclosures {
		if err := AssertCatalogsAiContentDisclosureRequired(el); err != nil {
			return err
		}
	}
	if obj.Gtin != nil {
		if err := AssertUpdatableItemAttributesGtinRequired(*obj.Gtin); err != nil {
			return err
		}
	}
	if err := AssertItemAttributesRequestImageLinkRequired(obj.ImageLink); err != nil {
		return err
	}
	return nil
}

// AssertItemAttributesRequestConstraints checks if the values respects the defined constraints
func AssertItemAttributesRequestConstraints(obj ItemAttributesRequest) error {
	for _, el := range obj.AiDisclosures {
		if err := AssertCatalogsAiContentDisclosureConstraints(el); err != nil {
			return err
		}
	}
    if obj.Gtin != nil {
     	if err := AssertUpdatableItemAttributesGtinConstraints(*obj.Gtin); err != nil {
     		return err
     	}
    }
	if err := AssertItemAttributesRequestImageLinkConstraints(obj.ImageLink); err != nil {
		return err
	}
	return nil
}
