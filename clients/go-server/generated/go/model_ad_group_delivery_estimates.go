...
// AssertAdGroupDeliveryEstimatesRequired checks complex required fields (models, arrays, maps) and embedded parents.
// Primitive required fields are validated for JSON request bodies in UnmarshalJSON so zero values remain valid.
func AssertAdGroupDeliveryEstimatesRequired(obj AdGroupDeliveryEstimates) error {
	if obj.Keywords != nil {
		for _, el := range *obj.Keywords {
			if err := AssertAdGroupDeliveryEstimatesKeywordsItemsRequired(el); err != nil {
				return err
			}
		}
	}
...
// AssertAdGroupDeliveryEstimatesConstraints checks if the values respects the defined constraints
func AssertAdGroupDeliveryEstimatesConstraints(obj AdGroupDeliveryEstimates) error {
    if obj.Keywords != nil {
     	for _, el := range *obj.Keywords {
     		if err := AssertAdGroupDeliveryEstimatesKeywordsItemsConstraints(el); err != nil {
     			return err
     		}
     	}
    }
	if obj.MonthlyFrequencyCap < 0 {
		return &ParsingError{Param: "MonthlyFrequencyCap", Err: errors.New(errMsgMinValueConstraint)}
	}
