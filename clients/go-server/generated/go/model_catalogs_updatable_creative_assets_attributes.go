// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * API version: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 */

package openapi




type CatalogsUpdatableCreativeAssetsAttributes struct {

	// Link to the creative assets page.
	AndroidDeepLink *string `json:"android_deep_link,omitempty"`

	// Custom grouping of creative assets.
	CustomLabel0 *string `json:"custom_label_0,omitempty"`

	// Custom grouping of creative assets.
	CustomLabel1 *string `json:"custom_label_1,omitempty"`

	// Custom grouping of creative assets.
	CustomLabel2 *string `json:"custom_label_2,omitempty"`

	// Custom grouping of creative assets.
	CustomLabel3 *string `json:"custom_label_3,omitempty"`

	// Custom grouping of creative assets.
	CustomLabel4 *string `json:"custom_label_4,omitempty"`

	// Brief description of the creative assets.
	Description string `json:"description,omitempty"`

	// The categorization of the product based on the standardized Google Product Taxonomy. This is a set taxonomy. Both the text values and numeric codes are accepted.
	GoogleProductCategory *string `json:"google_product_category,omitempty"`

	// IOS deep link to the creative assets page.
	IosDeepLink *string `json:"ios_deep_link,omitempty"`

	// Link to the creative assets page.
	Link string `json:"link,omitempty"`

	// The name of the creative assets.
	Title string `json:"title,omitempty"`

	// Visibility of the creative assets. Must be one of the following values (upper or lowercase): 'visible', 'hidden'.
	Visibility *string `json:"visibility,omitempty"`
}

// AssertCatalogsUpdatableCreativeAssetsAttributesRequired checks complex required fields (models, arrays, maps) and embedded parents.
// Primitive required fields are validated for JSON request bodies in UnmarshalJSON so zero values remain valid.
func AssertCatalogsUpdatableCreativeAssetsAttributesRequired(obj CatalogsUpdatableCreativeAssetsAttributes) error {
	return nil
}

// AssertCatalogsUpdatableCreativeAssetsAttributesConstraints checks if the values respects the defined constraints
func AssertCatalogsUpdatableCreativeAssetsAttributesConstraints(obj CatalogsUpdatableCreativeAssetsAttributes) error {
	return nil
}
