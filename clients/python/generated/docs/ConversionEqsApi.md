...
with pinterestsdk.ApiClient(configuration) as api_client:
    ...
    api_instance = pinterestsdk.ConversionEqsApi(api_client)
    lookback_period = pinterestsdk.LookbackPeriodOptions() # LookbackPeriodOptions | Lookback window (number of days).
    ad_account_id = 'ad_account_id_example' # str | Unique identifier of an ad account.
    source_platform = pinterestsdk.SourcePlatformOptions() # SourcePlatformOptions | Source platform of event. (optional)
    ingestion_source = pinterestsdk.IngestionSourceOptions() # IngestionSourceOptions | Ingestion source of event. (optional)
