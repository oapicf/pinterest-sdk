...
module PinterestSdkClient
  ...
  class CatalogsAiContentDisclosure < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PinterestSdkClient::CatalogsAiContentDisclosure` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PinterestSdkClient::CatalogsAiContentDisclosure`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'disclosure')
        if (value = attributes[:'disclosure']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if @disclosure.length < 1
        invalid_properties.push('invalid value for "disclosure", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @disclosure.length < 1
      return false if @url.nil?
      return false if @url.to_s.length > 2000
      true
    end
    ...
    def disclosure=(disclosure)
      ...
      if disclosure.length < 1
        fail ArgumentError, 'invalid value for "disclosure", number of items must be greater than or equal to 1.'
      end
