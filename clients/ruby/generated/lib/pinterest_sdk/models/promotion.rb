  ...
  class Promotion < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PinterestSdkClient::Promotion` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PinterestSdkClient::Promotion`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] promotion_title Value to be assigned
    def promotion_title=(promotion_title)
      if promotion_title.nil?
        fail ArgumentError, 'promotion_title cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] promotion_type Value to be assigned
    def promotion_type=(promotion_type)
      if promotion_type.nil?
        fail ArgumentError, 'promotion_type cannot be nil'
      end
    ...
    def hash
      [ad_account_id, discount_status, end_time, external_id, id, platform_type, promotion_code, promotion_custom_id, promotion_title, promotion_type, start_time, status, template_values].hash
    end
