...
module PinterestSdkClient
  class CustomerSegment < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PinterestSdkClient::CustomerSegment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PinterestSdkClient::CustomerSegment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'audience_ids')
        if (value = attributes[:'audience_ids']).is_a?(Array)
          self.audience_ids = value
        end
      else
        self.audience_ids = nil
      end
    ...
    def list_invalid_properties
      ...
      if @audience_ids.nil?
        invalid_properties.push('invalid value for "audience_ids", audience_ids cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @audience_ids.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] audience_ids Value to be assigned
    def audience_ids=(audience_ids)
      if audience_ids.nil?
        fail ArgumentError, 'audience_ids cannot be nil'
      end
    ...
    def hash
      [ad_account_id, audience_ids, created_time, id, name, status, updated_time].hash
    end
