...
module PinterestSdkClient
  class AdGroupUpdate < ApiModelBase
    # Advertiser ID.
    ...
    attr_accessor :customer_segment_id
    ...
    attr_accessor :is_local_inventory
    ...
    attr_accessor :local_inventory_radius_in_miles
    ...
    attr_accessor :placement_traffic_type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PinterestSdkClient::AdGroupUpdate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PinterestSdkClient::AdGroupUpdate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] customer_segment_id Value to be assigned
    def customer_segment_id=(customer_segment_id)
      if customer_segment_id.nil?
        fail ArgumentError, 'customer_segment_id cannot be nil'
      end

      pattern = Regexp.new(/^\d+$/)
      if customer_segment_id !~ pattern
        fail ArgumentError, "invalid value for \"customer_segment_id\", must conform to the pattern #{pattern}."
      end
