...
module PinterestSdkClient
  class PinsApi
    ...
    def pins_list_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'pin_filter'] && !allowable_values.include?(opts[:'pin_filter'])
        fail ArgumentError, "invalid value for \"pin_filter\", must be one of #{allowable_values}"
      end
      allowable_values = ["PRIVATE"]
      if @api_client.config.client_side_validation && opts[:'pin_type'] && !allowable_values.include?(opts[:'pin_type'])
        fail ArgumentError, "invalid value for \"pin_type\", must be one of #{allowable_values}"
      end
      if @api_client.config.client_side_validation && !opts[:'ad_account_id'].nil? && opts[:'ad_account_id'].to_s.length > 18
    ...
    # @option opts [String] :ad_account_id Unique identifier of an ad account.
    # @return [Pin]
    def pins_save(pin_id, pins_save_request, opts = {})
      data, _status_code, _headers = pins_save_with_http_info(pin_id, pins_save_request, opts)
      data
    end
    ...
    # @option opts [String] :ad_account_id Unique identifier of an ad account.
    # @return [Array<(Pin, Integer, Hash)>] Pin data, response status code and response headers
    def pins_save_with_http_info(pin_id, pins_save_request, opts = {})
      if @api_client.config.debugging
      ...
      if @api_client.config.client_side_validation && pins_save_request.nil?
        fail ArgumentError, "Missing the required parameter 'pins_save_request' when calling PinsApi.pins_save"
      end
      if @api_client.config.client_side_validation && !opts[:'ad_account_id'].nil? && opts[:'ad_account_id'].to_s.length > 18
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(pins_save_request)
