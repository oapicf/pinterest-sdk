/*
Pinterest REST API

Pinterest's REST API

API version: 5.28.0
Contact: blah+oapicf@cliffano.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the PinAnalyticsDailyMetrics type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PinAnalyticsDailyMetrics{}

// PinAnalyticsDailyMetrics struct for PinAnalyticsDailyMetrics
type PinAnalyticsDailyMetrics struct {
	DataStatus *DataStatus `json:"data_status,omitempty"`
	// Metrics date (UTC): YYYY-MM-DD.
	Date *string `json:"date,omitempty"`
	Metrics *map[string]float32 `json:"metrics,omitempty"`
}

// NewPinAnalyticsDailyMetrics instantiates a new PinAnalyticsDailyMetrics object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPinAnalyticsDailyMetrics() *PinAnalyticsDailyMetrics {
	this := PinAnalyticsDailyMetrics{}
	return &this
}

// NewPinAnalyticsDailyMetricsWithDefaults instantiates a new PinAnalyticsDailyMetrics object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPinAnalyticsDailyMetricsWithDefaults() *PinAnalyticsDailyMetrics {
	this := PinAnalyticsDailyMetrics{}
	return &this
}

// GetDataStatus returns the DataStatus field value if set, zero value otherwise.
func (o *PinAnalyticsDailyMetrics) GetDataStatus() DataStatus {
	if o == nil || IsNil(o.DataStatus) {
		var ret DataStatus
		return ret
	}
	return *o.DataStatus
}

// GetDataStatusOk returns a tuple with the DataStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PinAnalyticsDailyMetrics) GetDataStatusOk() (*DataStatus, bool) {
	if o == nil || IsNil(o.DataStatus) {
		return nil, false
	}
	return o.DataStatus, true
}

// HasDataStatus returns a boolean if a field has been set.
func (o *PinAnalyticsDailyMetrics) HasDataStatus() bool {
	if o != nil && !IsNil(o.DataStatus) {
		return true
	}

	return false
}

// SetDataStatus gets a reference to the given DataStatus and assigns it to the DataStatus field.
func (o *PinAnalyticsDailyMetrics) SetDataStatus(v DataStatus) {
	o.DataStatus = &v
}

// GetDate returns the Date field value if set, zero value otherwise.
func (o *PinAnalyticsDailyMetrics) GetDate() string {
	if o == nil || IsNil(o.Date) {
		var ret string
		return ret
	}
	return *o.Date
}

// GetDateOk returns a tuple with the Date field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PinAnalyticsDailyMetrics) GetDateOk() (*string, bool) {
	if o == nil || IsNil(o.Date) {
		return nil, false
	}
	return o.Date, true
}

// HasDate returns a boolean if a field has been set.
func (o *PinAnalyticsDailyMetrics) HasDate() bool {
	if o != nil && !IsNil(o.Date) {
		return true
	}

	return false
}

// SetDate gets a reference to the given string and assigns it to the Date field.
func (o *PinAnalyticsDailyMetrics) SetDate(v string) {
	o.Date = &v
}

// GetMetrics returns the Metrics field value if set, zero value otherwise.
func (o *PinAnalyticsDailyMetrics) GetMetrics() map[string]float32 {
	if o == nil || IsNil(o.Metrics) {
		var ret map[string]float32
		return ret
	}
	return *o.Metrics
}

// GetMetricsOk returns a tuple with the Metrics field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PinAnalyticsDailyMetrics) GetMetricsOk() (*map[string]float32, bool) {
	if o == nil || IsNil(o.Metrics) {
		return nil, false
	}
	return o.Metrics, true
}

// HasMetrics returns a boolean if a field has been set.
func (o *PinAnalyticsDailyMetrics) HasMetrics() bool {
	if o != nil && !IsNil(o.Metrics) {
		return true
	}

	return false
}

// SetMetrics gets a reference to the given map[string]float32 and assigns it to the Metrics field.
func (o *PinAnalyticsDailyMetrics) SetMetrics(v map[string]float32) {
	o.Metrics = &v
}

func (o PinAnalyticsDailyMetrics) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PinAnalyticsDailyMetrics) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.DataStatus) {
		toSerialize["data_status"] = o.DataStatus
	}
	if !IsNil(o.Date) {
		toSerialize["date"] = o.Date
	}
	if !IsNil(o.Metrics) {
		toSerialize["metrics"] = o.Metrics
	}
	return toSerialize, nil
}

type NullablePinAnalyticsDailyMetrics struct {
	value *PinAnalyticsDailyMetrics
	isSet bool
}

func (v NullablePinAnalyticsDailyMetrics) Get() *PinAnalyticsDailyMetrics {
	return v.value
}

func (v *NullablePinAnalyticsDailyMetrics) Set(val *PinAnalyticsDailyMetrics) {
	v.value = val
	v.isSet = true
}

func (v NullablePinAnalyticsDailyMetrics) IsSet() bool {
	return v.isSet
}

func (v *NullablePinAnalyticsDailyMetrics) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePinAnalyticsDailyMetrics(val *PinAnalyticsDailyMetrics) *NullablePinAnalyticsDailyMetrics {
	return &NullablePinAnalyticsDailyMetrics{value: val, isSet: true}
}

func (v NullablePinAnalyticsDailyMetrics) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePinAnalyticsDailyMetrics) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


