/*
Pinterest REST API

Pinterest's REST API

API version: 5.28.0
Contact: blah+oapicf@cliffano.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the AssetGroupDeleteError type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AssetGroupDeleteError{}

// AssetGroupDeleteError struct for AssetGroupDeleteError
type AssetGroupDeleteError struct {
	// Asset group id of the exception.
	AssetGroupId *string `json:"asset_group_id,omitempty"`
	// Error code associated with the error deleting asset group.
	Code *int32 `json:"code,omitempty"`
	// Error message associated with the error deleting asset group.
	Message *string `json:"message,omitempty"`
}

// NewAssetGroupDeleteError instantiates a new AssetGroupDeleteError object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAssetGroupDeleteError() *AssetGroupDeleteError {
	this := AssetGroupDeleteError{}
	return &this
}

// NewAssetGroupDeleteErrorWithDefaults instantiates a new AssetGroupDeleteError object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAssetGroupDeleteErrorWithDefaults() *AssetGroupDeleteError {
	this := AssetGroupDeleteError{}
	return &this
}

// GetAssetGroupId returns the AssetGroupId field value if set, zero value otherwise.
func (o *AssetGroupDeleteError) GetAssetGroupId() string {
	if o == nil || IsNil(o.AssetGroupId) {
		var ret string
		return ret
	}
	return *o.AssetGroupId
}

// GetAssetGroupIdOk returns a tuple with the AssetGroupId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AssetGroupDeleteError) GetAssetGroupIdOk() (*string, bool) {
	if o == nil || IsNil(o.AssetGroupId) {
		return nil, false
	}
	return o.AssetGroupId, true
}

// HasAssetGroupId returns a boolean if a field has been set.
func (o *AssetGroupDeleteError) HasAssetGroupId() bool {
	if o != nil && !IsNil(o.AssetGroupId) {
		return true
	}

	return false
}

// SetAssetGroupId gets a reference to the given string and assigns it to the AssetGroupId field.
func (o *AssetGroupDeleteError) SetAssetGroupId(v string) {
	o.AssetGroupId = &v
}

// GetCode returns the Code field value if set, zero value otherwise.
func (o *AssetGroupDeleteError) GetCode() int32 {
	if o == nil || IsNil(o.Code) {
		var ret int32
		return ret
	}
	return *o.Code
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AssetGroupDeleteError) GetCodeOk() (*int32, bool) {
	if o == nil || IsNil(o.Code) {
		return nil, false
	}
	return o.Code, true
}

// HasCode returns a boolean if a field has been set.
func (o *AssetGroupDeleteError) HasCode() bool {
	if o != nil && !IsNil(o.Code) {
		return true
	}

	return false
}

// SetCode gets a reference to the given int32 and assigns it to the Code field.
func (o *AssetGroupDeleteError) SetCode(v int32) {
	o.Code = &v
}

// GetMessage returns the Message field value if set, zero value otherwise.
func (o *AssetGroupDeleteError) GetMessage() string {
	if o == nil || IsNil(o.Message) {
		var ret string
		return ret
	}
	return *o.Message
}

// GetMessageOk returns a tuple with the Message field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AssetGroupDeleteError) GetMessageOk() (*string, bool) {
	if o == nil || IsNil(o.Message) {
		return nil, false
	}
	return o.Message, true
}

// HasMessage returns a boolean if a field has been set.
func (o *AssetGroupDeleteError) HasMessage() bool {
	if o != nil && !IsNil(o.Message) {
		return true
	}

	return false
}

// SetMessage gets a reference to the given string and assigns it to the Message field.
func (o *AssetGroupDeleteError) SetMessage(v string) {
	o.Message = &v
}

func (o AssetGroupDeleteError) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AssetGroupDeleteError) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AssetGroupId) {
		toSerialize["asset_group_id"] = o.AssetGroupId
	}
	if !IsNil(o.Code) {
		toSerialize["code"] = o.Code
	}
	if !IsNil(o.Message) {
		toSerialize["message"] = o.Message
	}
	return toSerialize, nil
}

type NullableAssetGroupDeleteError struct {
	value *AssetGroupDeleteError
	isSet bool
}

func (v NullableAssetGroupDeleteError) Get() *AssetGroupDeleteError {
	return v.value
}

func (v *NullableAssetGroupDeleteError) Set(val *AssetGroupDeleteError) {
	v.value = val
	v.isSet = true
}

func (v NullableAssetGroupDeleteError) IsSet() bool {
	return v.isSet
}

func (v *NullableAssetGroupDeleteError) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAssetGroupDeleteError(val *AssetGroupDeleteError) *NullableAssetGroupDeleteError {
	return &NullableAssetGroupDeleteError{value: val, isSet: true}
}

func (v NullableAssetGroupDeleteError) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAssetGroupDeleteError) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


