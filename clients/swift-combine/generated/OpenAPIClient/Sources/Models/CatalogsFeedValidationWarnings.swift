//
// CatalogsFeedValidationWarnings.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct CatalogsFeedValidationWarnings: Codable {
    /// ad_image_0_link is duplicated with another ad image link.
    public var aDIMAGE0LINKDUPLICATED: Int?
    /// Ad image link 0 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE0LINKLENGTHTOOLONG: Int?
    /// Ad image link 0 is required because an image tag was provided.
    public var aDIMAGE0LINKREQUIRED: Int?
    /// Ad image link 0 format is unsupported.
    public var aDIMAGE0LINKWARNING: Int?
    /// ad_image_0_tag is duplicated with another ad image tag.
    public var aDIMAGE0TAGDUPLICATED: Int?
    /// Ad image tag 0 length is too long. The maximum length is 511 characters.
    public var aDIMAGE0TAGLENGTHTOOLONG: Int?
    /// Ad image tag 0 is required because an image link was provided.
    public var aDIMAGE0TAGREQUIRED: Int?
    /// ad_image_10_link is duplicated with another ad image link.
    public var aDIMAGE10LINKDUPLICATED: Int?
    /// Ad image link 10 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE10LINKLENGTHTOOLONG: Int?
    /// Ad image link 10 is required because an image tag was provided.
    public var aDIMAGE10LINKREQUIRED: Int?
    /// Ad image link 10 format is unsupported.
    public var aDIMAGE10LINKWARNING: Int?
    /// ad_image_10_tag is duplicated with another ad image tag.
    public var aDIMAGE10TAGDUPLICATED: Int?
    /// Ad image tag 10 length is too long. The maximum length is 511 characters.
    public var aDIMAGE10TAGLENGTHTOOLONG: Int?
    /// Ad image tag 10 is required because an image link was provided.
    public var aDIMAGE10TAGREQUIRED: Int?
    /// ad_image_11_link is duplicated with another ad image link.
    public var aDIMAGE11LINKDUPLICATED: Int?
    /// Ad image link 11 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE11LINKLENGTHTOOLONG: Int?
    /// Ad image link 11 is required because an image tag was provided.
    public var aDIMAGE11LINKREQUIRED: Int?
    /// Ad image link 11 format is unsupported.
    public var aDIMAGE11LINKWARNING: Int?
    /// ad_image_11_tag is duplicated with another ad image tag.
    public var aDIMAGE11TAGDUPLICATED: Int?
    /// Ad image tag 11 length is too long. The maximum length is 511 characters.
    public var aDIMAGE11TAGLENGTHTOOLONG: Int?
    /// Ad image tag 11 is required because an image link was provided.
    public var aDIMAGE11TAGREQUIRED: Int?
    /// ad_image_12_link is duplicated with another ad image link.
    public var aDIMAGE12LINKDUPLICATED: Int?
    /// Ad image link 12 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE12LINKLENGTHTOOLONG: Int?
    /// Ad image link 12 is required because an image tag was provided.
    public var aDIMAGE12LINKREQUIRED: Int?
    /// Ad image link 12 format is unsupported.
    public var aDIMAGE12LINKWARNING: Int?
    /// ad_image_12_tag is duplicated with another ad image tag.
    public var aDIMAGE12TAGDUPLICATED: Int?
    /// Ad image tag 12 length is too long. The maximum length is 511 characters.
    public var aDIMAGE12TAGLENGTHTOOLONG: Int?
    /// Ad image tag 12 is required because an image link was provided.
    public var aDIMAGE12TAGREQUIRED: Int?
    /// ad_image_13_link is duplicated with another ad image link.
    public var aDIMAGE13LINKDUPLICATED: Int?
    /// Ad image link 13 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE13LINKLENGTHTOOLONG: Int?
    /// Ad image link 13 is required because an image tag was provided.
    public var aDIMAGE13LINKREQUIRED: Int?
    /// Ad image link 13 format is unsupported.
    public var aDIMAGE13LINKWARNING: Int?
    /// ad_image_13_tag is duplicated with another ad image tag.
    public var aDIMAGE13TAGDUPLICATED: Int?
    /// Ad image tag 13 length is too long. The maximum length is 511 characters.
    public var aDIMAGE13TAGLENGTHTOOLONG: Int?
    /// Ad image tag 13 is required because an image link was provided.
    public var aDIMAGE13TAGREQUIRED: Int?
    /// ad_image_14_link is duplicated with another ad image link.
    public var aDIMAGE14LINKDUPLICATED: Int?
    /// Ad image link 14 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE14LINKLENGTHTOOLONG: Int?
    /// Ad image link 14 is required because an image tag was provided.
    public var aDIMAGE14LINKREQUIRED: Int?
    /// Ad image link 14 format is unsupported.
    public var aDIMAGE14LINKWARNING: Int?
    /// ad_image_14_tag is duplicated with another ad image tag.
    public var aDIMAGE14TAGDUPLICATED: Int?
    /// Ad image tag 14 length is too long. The maximum length is 511 characters.
    public var aDIMAGE14TAGLENGTHTOOLONG: Int?
    /// Ad image tag 14 is required because an image link was provided.
    public var aDIMAGE14TAGREQUIRED: Int?
    /// ad_image_15_link is duplicated with another ad image link.
    public var aDIMAGE15LINKDUPLICATED: Int?
    /// Ad image link 15 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE15LINKLENGTHTOOLONG: Int?
    /// Ad image link 15 is required because an image tag was provided.
    public var aDIMAGE15LINKREQUIRED: Int?
    /// Ad image link 15 format is unsupported.
    public var aDIMAGE15LINKWARNING: Int?
    /// ad_image_15_tag is duplicated with another ad image tag.
    public var aDIMAGE15TAGDUPLICATED: Int?
    /// Ad image tag 15 length is too long. The maximum length is 511 characters.
    public var aDIMAGE15TAGLENGTHTOOLONG: Int?
    /// Ad image tag 15 is required because an image link was provided.
    public var aDIMAGE15TAGREQUIRED: Int?
    /// ad_image_16_link is duplicated with another ad image link.
    public var aDIMAGE16LINKDUPLICATED: Int?
    /// Ad image link 16 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE16LINKLENGTHTOOLONG: Int?
    /// Ad image link 16 is required because an image tag was provided.
    public var aDIMAGE16LINKREQUIRED: Int?
    /// Ad image link 16 format is unsupported.
    public var aDIMAGE16LINKWARNING: Int?
    /// ad_image_16_tag is duplicated with another ad image tag.
    public var aDIMAGE16TAGDUPLICATED: Int?
    /// Ad image tag 16 length is too long. The maximum length is 511 characters.
    public var aDIMAGE16TAGLENGTHTOOLONG: Int?
    /// Ad image tag 16 is required because an image link was provided.
    public var aDIMAGE16TAGREQUIRED: Int?
    /// ad_image_17_link is duplicated with another ad image link.
    public var aDIMAGE17LINKDUPLICATED: Int?
    /// Ad image link 17 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE17LINKLENGTHTOOLONG: Int?
    /// Ad image link 17 is required because an image tag was provided.
    public var aDIMAGE17LINKREQUIRED: Int?
    /// Ad image link 17 format is unsupported.
    public var aDIMAGE17LINKWARNING: Int?
    /// ad_image_17_tag is duplicated with another ad image tag.
    public var aDIMAGE17TAGDUPLICATED: Int?
    /// Ad image tag 17 length is too long. The maximum length is 511 characters.
    public var aDIMAGE17TAGLENGTHTOOLONG: Int?
    /// Ad image tag 17 is required because an image link was provided.
    public var aDIMAGE17TAGREQUIRED: Int?
    /// ad_image_18_link is duplicated with another ad image link.
    public var aDIMAGE18LINKDUPLICATED: Int?
    /// Ad image link 18 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE18LINKLENGTHTOOLONG: Int?
    /// Ad image link 18 is required because an image tag was provided.
    public var aDIMAGE18LINKREQUIRED: Int?
    /// Ad image link 18 format is unsupported.
    public var aDIMAGE18LINKWARNING: Int?
    /// ad_image_18_tag is duplicated with another ad image tag.
    public var aDIMAGE18TAGDUPLICATED: Int?
    /// Ad image tag 18 length is too long. The maximum length is 511 characters.
    public var aDIMAGE18TAGLENGTHTOOLONG: Int?
    /// Ad image tag 18 is required because an image link was provided.
    public var aDIMAGE18TAGREQUIRED: Int?
    /// ad_image_19_link is duplicated with another ad image link.
    public var aDIMAGE19LINKDUPLICATED: Int?
    /// Ad image link 19 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE19LINKLENGTHTOOLONG: Int?
    /// Ad image link 19 is required because an image tag was provided.
    public var aDIMAGE19LINKREQUIRED: Int?
    /// Ad image link 19 format is unsupported.
    public var aDIMAGE19LINKWARNING: Int?
    /// ad_image_19_tag is duplicated with another ad image tag.
    public var aDIMAGE19TAGDUPLICATED: Int?
    /// Ad image tag 19 length is too long. The maximum length is 511 characters.
    public var aDIMAGE19TAGLENGTHTOOLONG: Int?
    /// Ad image tag 19 is required because an image link was provided.
    public var aDIMAGE19TAGREQUIRED: Int?
    /// ad_image_1_link is duplicated with another ad image link.
    public var aDIMAGE1LINKDUPLICATED: Int?
    /// Ad image link 1 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE1LINKLENGTHTOOLONG: Int?
    /// Ad image link 1 is required because an image tag was provided.
    public var aDIMAGE1LINKREQUIRED: Int?
    /// Ad image link 1 format is unsupported.
    public var aDIMAGE1LINKWARNING: Int?
    /// ad_image_1_tag is duplicated with another ad image tag.
    public var aDIMAGE1TAGDUPLICATED: Int?
    /// Ad image tag 1 length is too long. The maximum length is 511 characters.
    public var aDIMAGE1TAGLENGTHTOOLONG: Int?
    /// Ad image tag 1 is required because an image link was provided.
    public var aDIMAGE1TAGREQUIRED: Int?
    /// ad_image_2_link is duplicated with another ad image link.
    public var aDIMAGE2LINKDUPLICATED: Int?
    /// Ad image link 2 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE2LINKLENGTHTOOLONG: Int?
    /// Ad image link 2 is required because an image tag was provided.
    public var aDIMAGE2LINKREQUIRED: Int?
    /// Ad image link 2 format is unsupported.
    public var aDIMAGE2LINKWARNING: Int?
    /// ad_image_2_tag is duplicated with another ad image tag.
    public var aDIMAGE2TAGDUPLICATED: Int?
    /// Ad image tag 2 length is too long. The maximum length is 511 characters.
    public var aDIMAGE2TAGLENGTHTOOLONG: Int?
    /// Ad image tag 2 is required because an image link was provided.
    public var aDIMAGE2TAGREQUIRED: Int?
    /// ad_image_3_link is duplicated with another ad image link.
    public var aDIMAGE3LINKDUPLICATED: Int?
    /// Ad image link 3 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE3LINKLENGTHTOOLONG: Int?
    /// Ad image link 3 is required because an image tag was provided.
    public var aDIMAGE3LINKREQUIRED: Int?
    /// Ad image link 3 format is unsupported.
    public var aDIMAGE3LINKWARNING: Int?
    /// ad_image_3_tag is duplicated with another ad image tag.
    public var aDIMAGE3TAGDUPLICATED: Int?
    /// Ad image tag 3 length is too long. The maximum length is 511 characters.
    public var aDIMAGE3TAGLENGTHTOOLONG: Int?
    /// Ad image tag 3 is required because an image link was provided.
    public var aDIMAGE3TAGREQUIRED: Int?
    /// ad_image_4_link is duplicated with another ad image link.
    public var aDIMAGE4LINKDUPLICATED: Int?
    /// Ad image link 4 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE4LINKLENGTHTOOLONG: Int?
    /// Ad image link 4 is required because an image tag was provided.
    public var aDIMAGE4LINKREQUIRED: Int?
    /// Ad image link 4 format is unsupported.
    public var aDIMAGE4LINKWARNING: Int?
    /// ad_image_4_tag is duplicated with another ad image tag.
    public var aDIMAGE4TAGDUPLICATED: Int?
    /// Ad image tag 4 length is too long. The maximum length is 511 characters.
    public var aDIMAGE4TAGLENGTHTOOLONG: Int?
    /// Ad image tag 4 is required because an image link was provided.
    public var aDIMAGE4TAGREQUIRED: Int?
    /// ad_image_5_link is duplicated with another ad image link.
    public var aDIMAGE5LINKDUPLICATED: Int?
    /// Ad image link 5 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE5LINKLENGTHTOOLONG: Int?
    /// Ad image link 5 is required because an image tag was provided.
    public var aDIMAGE5LINKREQUIRED: Int?
    /// Ad image link 5 format is unsupported.
    public var aDIMAGE5LINKWARNING: Int?
    /// ad_image_5_tag is duplicated with another ad image tag.
    public var aDIMAGE5TAGDUPLICATED: Int?
    /// Ad image tag 5 length is too long. The maximum length is 511 characters.
    public var aDIMAGE5TAGLENGTHTOOLONG: Int?
    /// Ad image tag 5 is required because an image link was provided.
    public var aDIMAGE5TAGREQUIRED: Int?
    /// ad_image_6_link is duplicated with another ad image link.
    public var aDIMAGE6LINKDUPLICATED: Int?
    /// Ad image link 6 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE6LINKLENGTHTOOLONG: Int?
    /// Ad image link 6 is required because an image tag was provided.
    public var aDIMAGE6LINKREQUIRED: Int?
    /// Ad image link 6 format is unsupported.
    public var aDIMAGE6LINKWARNING: Int?
    /// ad_image_6_tag is duplicated with another ad image tag.
    public var aDIMAGE6TAGDUPLICATED: Int?
    /// Ad image tag 6 length is too long. The maximum length is 511 characters.
    public var aDIMAGE6TAGLENGTHTOOLONG: Int?
    /// Ad image tag 6 is required because an image link was provided.
    public var aDIMAGE6TAGREQUIRED: Int?
    /// ad_image_7_link is duplicated with another ad image link.
    public var aDIMAGE7LINKDUPLICATED: Int?
    /// Ad image link 7 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE7LINKLENGTHTOOLONG: Int?
    /// Ad image link 7 is required because an image tag was provided.
    public var aDIMAGE7LINKREQUIRED: Int?
    /// Ad image link 7 format is unsupported.
    public var aDIMAGE7LINKWARNING: Int?
    /// ad_image_7_tag is duplicated with another ad image tag.
    public var aDIMAGE7TAGDUPLICATED: Int?
    /// Ad image tag 7 length is too long. The maximum length is 511 characters.
    public var aDIMAGE7TAGLENGTHTOOLONG: Int?
    /// Ad image tag 7 is required because an image link was provided.
    public var aDIMAGE7TAGREQUIRED: Int?
    /// ad_image_8_link is duplicated with another ad image link.
    public var aDIMAGE8LINKDUPLICATED: Int?
    /// Ad image link 8 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE8LINKLENGTHTOOLONG: Int?
    /// Ad image link 8 is required because an image tag was provided.
    public var aDIMAGE8LINKREQUIRED: Int?
    /// Ad image link 8 format is unsupported.
    public var aDIMAGE8LINKWARNING: Int?
    /// ad_image_8_tag is duplicated with another ad image tag.
    public var aDIMAGE8TAGDUPLICATED: Int?
    /// Ad image tag 8 length is too long. The maximum length is 511 characters.
    public var aDIMAGE8TAGLENGTHTOOLONG: Int?
    /// Ad image tag 8 is required because an image link was provided.
    public var aDIMAGE8TAGREQUIRED: Int?
    /// ad_image_9_link is duplicated with another ad image link.
    public var aDIMAGE9LINKDUPLICATED: Int?
    /// Ad image link 9 length is too long. The maximum length is 2047 characters.
    public var aDIMAGE9LINKLENGTHTOOLONG: Int?
    /// Ad image link 9 is required because an image tag was provided.
    public var aDIMAGE9LINKREQUIRED: Int?
    /// Ad image link 9 format is unsupported.
    public var aDIMAGE9LINKWARNING: Int?
    /// ad_image_9_tag is duplicated with another ad image tag.
    public var aDIMAGE9TAGDUPLICATED: Int?
    /// Ad image tag 9 length is too long. The maximum length is 511 characters.
    public var aDIMAGE9TAGLENGTHTOOLONG: Int?
    /// Ad image tag 9 is required because an image link was provided.
    public var aDIMAGE9TAGREQUIRED: Int?
    /// Some items have ad links that are formatted incorrectly.
    public var AD_LINK_FORMAT_WARNING: Int?
    /// Some items have ad link URLs that are duplicates of the link URLs for those items.
    public var AD_LINK_SAME_AS_LINK: Int?
    /// ad_video_0_link is duplicated with another ad video link.
    public var aDVIDEO0LINKDUPLICATED: Int?
    /// ad_video_0_link length is too long. The maximum length is 511 characters.
    public var aDVIDEO0LINKLENGTHTOOLONG: Int?
    /// ad_video_0_link is required for this item because ad_video_0_tag was provided.
    public var aDVIDEO0LINKREQUIRED: Int?
    /// ad_video_0_link is formatted incorrectly and will not be published with your items.
    public var aDVIDEO0LINKWARNING: Int?
    /// ad_video_0_tag is duplicated with another ad video tag.
    public var aDVIDEO0TAGDUPLICATED: Int?
    /// ad_video_0_tag length is too long. The maximum length is 511 characters.
    public var aDVIDEO0TAGLENGTHTOOLONG: Int?
    /// ad_video_0_tag is required because ad_video_0_link was provided.
    public var aDVIDEO0TAGREQUIRED: Int?
    /// ad_video_1_link is duplicated with another ad video link.
    public var aDVIDEO1LINKDUPLICATED: Int?
    /// ad_video_1_link length is too long. The maximum length is 511 characters.
    public var aDVIDEO1LINKLENGTHTOOLONG: Int?
    /// ad_video_1_link is required for this item because ad_video_1_tag was provided.
    public var aDVIDEO1LINKREQUIRED: Int?
    /// ad_video_1_link is formatted incorrectly and will not be published with your items.
    public var aDVIDEO1LINKWARNING: Int?
    /// ad_video_1_tag is duplicated with another ad video tag.
    public var aDVIDEO1TAGDUPLICATED: Int?
    /// ad_video_1_tag length is too long. The maximum length is 511 characters.
    public var aDVIDEO1TAGLENGTHTOOLONG: Int?
    /// ad_video_1_tag is required because ad_video_1_link was provided.
    public var aDVIDEO1TAGREQUIRED: Int?
    /// ad_video_2_link is duplicated with another ad video link.
    public var aDVIDEO2LINKDUPLICATED: Int?
    /// ad_video_2_link length is too long. The maximum length is 511 characters.
    public var aDVIDEO2LINKLENGTHTOOLONG: Int?
    /// ad_video_2_link is required for this item because ad_video_2_tag was provided.
    public var aDVIDEO2LINKREQUIRED: Int?
    /// ad_video_2_link is formatted incorrectly and will not be published with your items.
    public var aDVIDEO2LINKWARNING: Int?
    /// ad_video_2_tag is duplicated with another ad video tag.
    public var aDVIDEO2TAGDUPLICATED: Int?
    /// ad_video_2_tag length is too long. The maximum length is 511 characters.
    public var aDVIDEO2TAGLENGTHTOOLONG: Int?
    /// ad_video_2_tag is required because ad_video_2_link was provided.
    public var aDVIDEO2TAGREQUIRED: Int?
    /// Some items have additional_image_link URLs that contain too many characters, so those items will not be published.
    public var ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG: Int?
    /// Some items have additional_image_link URLs that are formatted incorrectly and will not be published with your items.
    public var ADDITIONAL_IMAGE_LINK_WARNING: Int?
    /// Some items have adwords_redirect links that are formatted incorrectly.
    public var ADWORDS_FORMAT_WARNING: Int?
    /// Some items have adwords_redirect URLs that are duplicates of the link URLs for those items.
    public var ADWORDS_SAME_AS_LINK: Int?
    /// Some items have age group values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    public var AGE_GROUP_INVALID: Int?
    /// Some items include invalid android_deep_link.
    public var ANDROID_DEEP_LINK_INVALID: Int?
    /// Some items have availability_date values that are formatted incorrectly, those items will be published without an availability date.
    public var AVAILABILITY_DATE_INVALID: Int?
    /// Some items include a currency that doesn't match the usual currency for the location where that product is sold or shipped.
    public var COUNTRY_DOES_NOT_MAP_TO_CURRENCY: Int?
    /// Some items have custom_label values that are too long, those items will be published without that custom label.
    public var CUSTOM_LABEL_LENGTH_TOO_LONG: Int?
    /// The description for some items were truncated because they contain too many characters.
    public var DESCRIPTION_LENGTH_TOO_LONG: Int?
    /// Your feed contains duplicate headers.
    public var DUPLICATE_HEADERS: Int?
    /// Some items have expiration_date values that are formatted incorrectly, those items will be published without an expiration date.
    public var EXPIRATION_DATE_INVALID: Int?
    /// Ingestion completed early because there are no changes to your feed since the last successful update.
    public var FETCH_SAME_SIGNATURE: Int?
    /// Some items have gender values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    public var GENDER_INVALID: Int?
    /// Some items include incorrectly formatted GTINs.
    public var GTIN_INVALID: Int?
    /// Some items have image_link URLs that are formatted incorrectly and will not be published with those items.
    public var IMAGE_LINK_WARNING: Int?
    /// Some items include inconsistent currencies in price fields.
    public var INCONSISTENT_CURRENCY_VALUES: Int?
    /// The product count has increased or decreased significantly compared to the last successful ingestion.
    public var INDEXED_PRODUCT_COUNT_LARGE_DELTA: Int?
    /// Some items include invalid ios_deep_link values.
    public var IOS_DEEP_LINK_INVALID: Int?
    /// Some items have is_bundle values that are formatted incorrectly, those items will be published without being bundled with other products.
    public var IS_BUNDLE_INVALID: Int?
    /// Some items include additional_image_links that can't be found.
    public var ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE: Int?
    /// Some items have an invalid product link which contains invalid UTM tracking paramaters.
    public var LINK_FORMAT_WARNING: Int?
    /// Some items include min_ad_price values that are formatted incorrectly.
    public var MIN_AD_PRICE_INVALID: Int?
    /// Some items include incorrectly formatted MPNs.
    public var MPN_INVALID: Int?
    /// Some items have invalid multipack values.
    public var MULTIPACK_INVALID: Int?
    /// Some items include condition values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    public var OPTIONAL_CONDITION_INVALID: Int?
    /// Some items are missing a condition value, which may limit visibility in recommendations, search results and shopping experiences.
    public var OPTIONAL_CONDITION_MISSING: Int?
    /// Some items include google_product_category values that are not formatted correctly according to the GPC taxonomy.
    public var OPTIONAL_PRODUCT_CATEGORY_INVALID: Int?
    /// Some items are missing a google_product_category.
    public var OPTIONAL_PRODUCT_CATEGORY_MISSING: Int?
    /// Some items only have 1 or 2 levels of google_product_category values, which may limit visibility in recommendations, search results and shopping experiences.
    public var PRODUCT_CATEGORY_DEPTH_WARNING: Int?
    /// Some items have product_type values that are too long, those items will be published without that product type.
    public var PRODUCT_TYPE_LENGTH_TOO_LONG: Int?
    /// Some items have sale_price_effective_date values that are formatted incorrectly, those items will be published without a sale date.
    public var SALE_DATE_INVALID: Int?
    /// Some items have sale price values that are higher than the original price of the item.
    public var SALES_PRICE_INVALID: Int?
    /// Some items include a sales price that is higher than the list price. The sales price has been defaulted to the list price.
    public var SALES_PRICE_TOO_HIGH: Int?
    /// Some items include sales price that is much lower than the list price.
    public var SALES_PRICE_TOO_LOW: Int?
    /// Some items include incorrectly formatted shipping_height.
    public var SHIPPING_HEIGHT_INVALID: Int?
    /// Some items have shipping values that are formatted incorrectly.
    public var SHIPPING_INVALID: Int?
    /// Some items have invalid shipping_weight values.
    public var SHIPPING_WEIGHT_INVALID: Int?
    /// Some items include incorrectly formatted shipping_width.
    public var SHIPPING_WIDTH_INVALID: Int?
    /// Some items have size system values which are not one of the supported size systems.
    public var SIZE_SYSTEM_INVALID: Int?
    /// Some items have size type values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    public var SIZE_TYPE_INVALID: Int?
    /// Some items have tax values that are formatted incorrectly.
    public var TAX_INVALID: Int?
    /// The title for some items were truncated because they contain too many characters.
    public var TITLE_LENGTH_TOO_LONG: Int?
    /// Some items have additional_image_link values that exceed the limit for additional images, those items will be published without some of your images.
    public var TOO_MANY_ADDITIONAL_IMAGE_LINKS: Int?
    /// Some items have updated_time values thate are formatted incorrectly, those items will be published without an updated time.
    public var UPDATED_TIME_INVALID: Int?
    /// Some items include utm_source values that are formatted incorrectly and have been automatically corrected.
    public var UTM_SOURCE_AUTO_CORRECTED: Int?
    /// A video is required in the item when ad_video fields are provided.
    public var VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED: Int?
    /// Some items have weight_unit values that are formatted incorrectly, those items will be published without a weight unit.
    public var WEIGHT_UNIT_INVALID: Int?

    public init(aDIMAGE0LINKDUPLICATED: Int? = nil, aDIMAGE0LINKLENGTHTOOLONG: Int? = nil, aDIMAGE0LINKREQUIRED: Int? = nil, aDIMAGE0LINKWARNING: Int? = nil, aDIMAGE0TAGDUPLICATED: Int? = nil, aDIMAGE0TAGLENGTHTOOLONG: Int? = nil, aDIMAGE0TAGREQUIRED: Int? = nil, aDIMAGE10LINKDUPLICATED: Int? = nil, aDIMAGE10LINKLENGTHTOOLONG: Int? = nil, aDIMAGE10LINKREQUIRED: Int? = nil, aDIMAGE10LINKWARNING: Int? = nil, aDIMAGE10TAGDUPLICATED: Int? = nil, aDIMAGE10TAGLENGTHTOOLONG: Int? = nil, aDIMAGE10TAGREQUIRED: Int? = nil, aDIMAGE11LINKDUPLICATED: Int? = nil, aDIMAGE11LINKLENGTHTOOLONG: Int? = nil, aDIMAGE11LINKREQUIRED: Int? = nil, aDIMAGE11LINKWARNING: Int? = nil, aDIMAGE11TAGDUPLICATED: Int? = nil, aDIMAGE11TAGLENGTHTOOLONG: Int? = nil, aDIMAGE11TAGREQUIRED: Int? = nil, aDIMAGE12LINKDUPLICATED: Int? = nil, aDIMAGE12LINKLENGTHTOOLONG: Int? = nil, aDIMAGE12LINKREQUIRED: Int? = nil, aDIMAGE12LINKWARNING: Int? = nil, aDIMAGE12TAGDUPLICATED: Int? = nil, aDIMAGE12TAGLENGTHTOOLONG: Int? = nil, aDIMAGE12TAGREQUIRED: Int? = nil, aDIMAGE13LINKDUPLICATED: Int? = nil, aDIMAGE13LINKLENGTHTOOLONG: Int? = nil, aDIMAGE13LINKREQUIRED: Int? = nil, aDIMAGE13LINKWARNING: Int? = nil, aDIMAGE13TAGDUPLICATED: Int? = nil, aDIMAGE13TAGLENGTHTOOLONG: Int? = nil, aDIMAGE13TAGREQUIRED: Int? = nil, aDIMAGE14LINKDUPLICATED: Int? = nil, aDIMAGE14LINKLENGTHTOOLONG: Int? = nil, aDIMAGE14LINKREQUIRED: Int? = nil, aDIMAGE14LINKWARNING: Int? = nil, aDIMAGE14TAGDUPLICATED: Int? = nil, aDIMAGE14TAGLENGTHTOOLONG: Int? = nil, aDIMAGE14TAGREQUIRED: Int? = nil, aDIMAGE15LINKDUPLICATED: Int? = nil, aDIMAGE15LINKLENGTHTOOLONG: Int? = nil, aDIMAGE15LINKREQUIRED: Int? = nil, aDIMAGE15LINKWARNING: Int? = nil, aDIMAGE15TAGDUPLICATED: Int? = nil, aDIMAGE15TAGLENGTHTOOLONG: Int? = nil, aDIMAGE15TAGREQUIRED: Int? = nil, aDIMAGE16LINKDUPLICATED: Int? = nil, aDIMAGE16LINKLENGTHTOOLONG: Int? = nil, aDIMAGE16LINKREQUIRED: Int? = nil, aDIMAGE16LINKWARNING: Int? = nil, aDIMAGE16TAGDUPLICATED: Int? = nil, aDIMAGE16TAGLENGTHTOOLONG: Int? = nil, aDIMAGE16TAGREQUIRED: Int? = nil, aDIMAGE17LINKDUPLICATED: Int? = nil, aDIMAGE17LINKLENGTHTOOLONG: Int? = nil, aDIMAGE17LINKREQUIRED: Int? = nil, aDIMAGE17LINKWARNING: Int? = nil, aDIMAGE17TAGDUPLICATED: Int? = nil, aDIMAGE17TAGLENGTHTOOLONG: Int? = nil, aDIMAGE17TAGREQUIRED: Int? = nil, aDIMAGE18LINKDUPLICATED: Int? = nil, aDIMAGE18LINKLENGTHTOOLONG: Int? = nil, aDIMAGE18LINKREQUIRED: Int? = nil, aDIMAGE18LINKWARNING: Int? = nil, aDIMAGE18TAGDUPLICATED: Int? = nil, aDIMAGE18TAGLENGTHTOOLONG: Int? = nil, aDIMAGE18TAGREQUIRED: Int? = nil, aDIMAGE19LINKDUPLICATED: Int? = nil, aDIMAGE19LINKLENGTHTOOLONG: Int? = nil, aDIMAGE19LINKREQUIRED: Int? = nil, aDIMAGE19LINKWARNING: Int? = nil, aDIMAGE19TAGDUPLICATED: Int? = nil, aDIMAGE19TAGLENGTHTOOLONG: Int? = nil, aDIMAGE19TAGREQUIRED: Int? = nil, aDIMAGE1LINKDUPLICATED: Int? = nil, aDIMAGE1LINKLENGTHTOOLONG: Int? = nil, aDIMAGE1LINKREQUIRED: Int? = nil, aDIMAGE1LINKWARNING: Int? = nil, aDIMAGE1TAGDUPLICATED: Int? = nil, aDIMAGE1TAGLENGTHTOOLONG: Int? = nil, aDIMAGE1TAGREQUIRED: Int? = nil, aDIMAGE2LINKDUPLICATED: Int? = nil, aDIMAGE2LINKLENGTHTOOLONG: Int? = nil, aDIMAGE2LINKREQUIRED: Int? = nil, aDIMAGE2LINKWARNING: Int? = nil, aDIMAGE2TAGDUPLICATED: Int? = nil, aDIMAGE2TAGLENGTHTOOLONG: Int? = nil, aDIMAGE2TAGREQUIRED: Int? = nil, aDIMAGE3LINKDUPLICATED: Int? = nil, aDIMAGE3LINKLENGTHTOOLONG: Int? = nil, aDIMAGE3LINKREQUIRED: Int? = nil, aDIMAGE3LINKWARNING: Int? = nil, aDIMAGE3TAGDUPLICATED: Int? = nil, aDIMAGE3TAGLENGTHTOOLONG: Int? = nil, aDIMAGE3TAGREQUIRED: Int? = nil, aDIMAGE4LINKDUPLICATED: Int? = nil, aDIMAGE4LINKLENGTHTOOLONG: Int? = nil, aDIMAGE4LINKREQUIRED: Int? = nil, aDIMAGE4LINKWARNING: Int? = nil, aDIMAGE4TAGDUPLICATED: Int? = nil, aDIMAGE4TAGLENGTHTOOLONG: Int? = nil, aDIMAGE4TAGREQUIRED: Int? = nil, aDIMAGE5LINKDUPLICATED: Int? = nil, aDIMAGE5LINKLENGTHTOOLONG: Int? = nil, aDIMAGE5LINKREQUIRED: Int? = nil, aDIMAGE5LINKWARNING: Int? = nil, aDIMAGE5TAGDUPLICATED: Int? = nil, aDIMAGE5TAGLENGTHTOOLONG: Int? = nil, aDIMAGE5TAGREQUIRED: Int? = nil, aDIMAGE6LINKDUPLICATED: Int? = nil, aDIMAGE6LINKLENGTHTOOLONG: Int? = nil, aDIMAGE6LINKREQUIRED: Int? = nil, aDIMAGE6LINKWARNING: Int? = nil, aDIMAGE6TAGDUPLICATED: Int? = nil, aDIMAGE6TAGLENGTHTOOLONG: Int? = nil, aDIMAGE6TAGREQUIRED: Int? = nil, aDIMAGE7LINKDUPLICATED: Int? = nil, aDIMAGE7LINKLENGTHTOOLONG: Int? = nil, aDIMAGE7LINKREQUIRED: Int? = nil, aDIMAGE7LINKWARNING: Int? = nil, aDIMAGE7TAGDUPLICATED: Int? = nil, aDIMAGE7TAGLENGTHTOOLONG: Int? = nil, aDIMAGE7TAGREQUIRED: Int? = nil, aDIMAGE8LINKDUPLICATED: Int? = nil, aDIMAGE8LINKLENGTHTOOLONG: Int? = nil, aDIMAGE8LINKREQUIRED: Int? = nil, aDIMAGE8LINKWARNING: Int? = nil, aDIMAGE8TAGDUPLICATED: Int? = nil, aDIMAGE8TAGLENGTHTOOLONG: Int? = nil, aDIMAGE8TAGREQUIRED: Int? = nil, aDIMAGE9LINKDUPLICATED: Int? = nil, aDIMAGE9LINKLENGTHTOOLONG: Int? = nil, aDIMAGE9LINKREQUIRED: Int? = nil, aDIMAGE9LINKWARNING: Int? = nil, aDIMAGE9TAGDUPLICATED: Int? = nil, aDIMAGE9TAGLENGTHTOOLONG: Int? = nil, aDIMAGE9TAGREQUIRED: Int? = nil, AD_LINK_FORMAT_WARNING: Int? = nil, AD_LINK_SAME_AS_LINK: Int? = nil, aDVIDEO0LINKDUPLICATED: Int? = nil, aDVIDEO0LINKLENGTHTOOLONG: Int? = nil, aDVIDEO0LINKREQUIRED: Int? = nil, aDVIDEO0LINKWARNING: Int? = nil, aDVIDEO0TAGDUPLICATED: Int? = nil, aDVIDEO0TAGLENGTHTOOLONG: Int? = nil, aDVIDEO0TAGREQUIRED: Int? = nil, aDVIDEO1LINKDUPLICATED: Int? = nil, aDVIDEO1LINKLENGTHTOOLONG: Int? = nil, aDVIDEO1LINKREQUIRED: Int? = nil, aDVIDEO1LINKWARNING: Int? = nil, aDVIDEO1TAGDUPLICATED: Int? = nil, aDVIDEO1TAGLENGTHTOOLONG: Int? = nil, aDVIDEO1TAGREQUIRED: Int? = nil, aDVIDEO2LINKDUPLICATED: Int? = nil, aDVIDEO2LINKLENGTHTOOLONG: Int? = nil, aDVIDEO2LINKREQUIRED: Int? = nil, aDVIDEO2LINKWARNING: Int? = nil, aDVIDEO2TAGDUPLICATED: Int? = nil, aDVIDEO2TAGLENGTHTOOLONG: Int? = nil, aDVIDEO2TAGREQUIRED: Int? = nil, ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG: Int? = nil, ADDITIONAL_IMAGE_LINK_WARNING: Int? = nil, ADWORDS_FORMAT_WARNING: Int? = nil, ADWORDS_SAME_AS_LINK: Int? = nil, AGE_GROUP_INVALID: Int? = nil, ANDROID_DEEP_LINK_INVALID: Int? = nil, AVAILABILITY_DATE_INVALID: Int? = nil, COUNTRY_DOES_NOT_MAP_TO_CURRENCY: Int? = nil, CUSTOM_LABEL_LENGTH_TOO_LONG: Int? = nil, DESCRIPTION_LENGTH_TOO_LONG: Int? = nil, DUPLICATE_HEADERS: Int? = nil, EXPIRATION_DATE_INVALID: Int? = nil, FETCH_SAME_SIGNATURE: Int? = nil, GENDER_INVALID: Int? = nil, GTIN_INVALID: Int? = nil, IMAGE_LINK_WARNING: Int? = nil, INCONSISTENT_CURRENCY_VALUES: Int? = nil, INDEXED_PRODUCT_COUNT_LARGE_DELTA: Int? = nil, IOS_DEEP_LINK_INVALID: Int? = nil, IS_BUNDLE_INVALID: Int? = nil, ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE: Int? = nil, LINK_FORMAT_WARNING: Int? = nil, MIN_AD_PRICE_INVALID: Int? = nil, MPN_INVALID: Int? = nil, MULTIPACK_INVALID: Int? = nil, OPTIONAL_CONDITION_INVALID: Int? = nil, OPTIONAL_CONDITION_MISSING: Int? = nil, OPTIONAL_PRODUCT_CATEGORY_INVALID: Int? = nil, OPTIONAL_PRODUCT_CATEGORY_MISSING: Int? = nil, PRODUCT_CATEGORY_DEPTH_WARNING: Int? = nil, PRODUCT_TYPE_LENGTH_TOO_LONG: Int? = nil, SALE_DATE_INVALID: Int? = nil, SALES_PRICE_INVALID: Int? = nil, SALES_PRICE_TOO_HIGH: Int? = nil, SALES_PRICE_TOO_LOW: Int? = nil, SHIPPING_HEIGHT_INVALID: Int? = nil, SHIPPING_INVALID: Int? = nil, SHIPPING_WEIGHT_INVALID: Int? = nil, SHIPPING_WIDTH_INVALID: Int? = nil, SIZE_SYSTEM_INVALID: Int? = nil, SIZE_TYPE_INVALID: Int? = nil, TAX_INVALID: Int? = nil, TITLE_LENGTH_TOO_LONG: Int? = nil, TOO_MANY_ADDITIONAL_IMAGE_LINKS: Int? = nil, UPDATED_TIME_INVALID: Int? = nil, UTM_SOURCE_AUTO_CORRECTED: Int? = nil, VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED: Int? = nil, WEIGHT_UNIT_INVALID: Int? = nil) {
        self.aDIMAGE0LINKDUPLICATED = aDIMAGE0LINKDUPLICATED
        self.aDIMAGE0LINKLENGTHTOOLONG = aDIMAGE0LINKLENGTHTOOLONG
        self.aDIMAGE0LINKREQUIRED = aDIMAGE0LINKREQUIRED
        self.aDIMAGE0LINKWARNING = aDIMAGE0LINKWARNING
        self.aDIMAGE0TAGDUPLICATED = aDIMAGE0TAGDUPLICATED
        self.aDIMAGE0TAGLENGTHTOOLONG = aDIMAGE0TAGLENGTHTOOLONG
        self.aDIMAGE0TAGREQUIRED = aDIMAGE0TAGREQUIRED
        self.aDIMAGE10LINKDUPLICATED = aDIMAGE10LINKDUPLICATED
        self.aDIMAGE10LINKLENGTHTOOLONG = aDIMAGE10LINKLENGTHTOOLONG
        self.aDIMAGE10LINKREQUIRED = aDIMAGE10LINKREQUIRED
        self.aDIMAGE10LINKWARNING = aDIMAGE10LINKWARNING
        self.aDIMAGE10TAGDUPLICATED = aDIMAGE10TAGDUPLICATED
        self.aDIMAGE10TAGLENGTHTOOLONG = aDIMAGE10TAGLENGTHTOOLONG
        self.aDIMAGE10TAGREQUIRED = aDIMAGE10TAGREQUIRED
        self.aDIMAGE11LINKDUPLICATED = aDIMAGE11LINKDUPLICATED
        self.aDIMAGE11LINKLENGTHTOOLONG = aDIMAGE11LINKLENGTHTOOLONG
        self.aDIMAGE11LINKREQUIRED = aDIMAGE11LINKREQUIRED
        self.aDIMAGE11LINKWARNING = aDIMAGE11LINKWARNING
        self.aDIMAGE11TAGDUPLICATED = aDIMAGE11TAGDUPLICATED
        self.aDIMAGE11TAGLENGTHTOOLONG = aDIMAGE11TAGLENGTHTOOLONG
        self.aDIMAGE11TAGREQUIRED = aDIMAGE11TAGREQUIRED
        self.aDIMAGE12LINKDUPLICATED = aDIMAGE12LINKDUPLICATED
        self.aDIMAGE12LINKLENGTHTOOLONG = aDIMAGE12LINKLENGTHTOOLONG
        self.aDIMAGE12LINKREQUIRED = aDIMAGE12LINKREQUIRED
        self.aDIMAGE12LINKWARNING = aDIMAGE12LINKWARNING
        self.aDIMAGE12TAGDUPLICATED = aDIMAGE12TAGDUPLICATED
        self.aDIMAGE12TAGLENGTHTOOLONG = aDIMAGE12TAGLENGTHTOOLONG
        self.aDIMAGE12TAGREQUIRED = aDIMAGE12TAGREQUIRED
        self.aDIMAGE13LINKDUPLICATED = aDIMAGE13LINKDUPLICATED
        self.aDIMAGE13LINKLENGTHTOOLONG = aDIMAGE13LINKLENGTHTOOLONG
        self.aDIMAGE13LINKREQUIRED = aDIMAGE13LINKREQUIRED
        self.aDIMAGE13LINKWARNING = aDIMAGE13LINKWARNING
        self.aDIMAGE13TAGDUPLICATED = aDIMAGE13TAGDUPLICATED
        self.aDIMAGE13TAGLENGTHTOOLONG = aDIMAGE13TAGLENGTHTOOLONG
        self.aDIMAGE13TAGREQUIRED = aDIMAGE13TAGREQUIRED
        self.aDIMAGE14LINKDUPLICATED = aDIMAGE14LINKDUPLICATED
        self.aDIMAGE14LINKLENGTHTOOLONG = aDIMAGE14LINKLENGTHTOOLONG
        self.aDIMAGE14LINKREQUIRED = aDIMAGE14LINKREQUIRED
        self.aDIMAGE14LINKWARNING = aDIMAGE14LINKWARNING
        self.aDIMAGE14TAGDUPLICATED = aDIMAGE14TAGDUPLICATED
        self.aDIMAGE14TAGLENGTHTOOLONG = aDIMAGE14TAGLENGTHTOOLONG
        self.aDIMAGE14TAGREQUIRED = aDIMAGE14TAGREQUIRED
        self.aDIMAGE15LINKDUPLICATED = aDIMAGE15LINKDUPLICATED
        self.aDIMAGE15LINKLENGTHTOOLONG = aDIMAGE15LINKLENGTHTOOLONG
        self.aDIMAGE15LINKREQUIRED = aDIMAGE15LINKREQUIRED
        self.aDIMAGE15LINKWARNING = aDIMAGE15LINKWARNING
        self.aDIMAGE15TAGDUPLICATED = aDIMAGE15TAGDUPLICATED
        self.aDIMAGE15TAGLENGTHTOOLONG = aDIMAGE15TAGLENGTHTOOLONG
        self.aDIMAGE15TAGREQUIRED = aDIMAGE15TAGREQUIRED
        self.aDIMAGE16LINKDUPLICATED = aDIMAGE16LINKDUPLICATED
        self.aDIMAGE16LINKLENGTHTOOLONG = aDIMAGE16LINKLENGTHTOOLONG
        self.aDIMAGE16LINKREQUIRED = aDIMAGE16LINKREQUIRED
        self.aDIMAGE16LINKWARNING = aDIMAGE16LINKWARNING
        self.aDIMAGE16TAGDUPLICATED = aDIMAGE16TAGDUPLICATED
        self.aDIMAGE16TAGLENGTHTOOLONG = aDIMAGE16TAGLENGTHTOOLONG
        self.aDIMAGE16TAGREQUIRED = aDIMAGE16TAGREQUIRED
        self.aDIMAGE17LINKDUPLICATED = aDIMAGE17LINKDUPLICATED
        self.aDIMAGE17LINKLENGTHTOOLONG = aDIMAGE17LINKLENGTHTOOLONG
        self.aDIMAGE17LINKREQUIRED = aDIMAGE17LINKREQUIRED
        self.aDIMAGE17LINKWARNING = aDIMAGE17LINKWARNING
        self.aDIMAGE17TAGDUPLICATED = aDIMAGE17TAGDUPLICATED
        self.aDIMAGE17TAGLENGTHTOOLONG = aDIMAGE17TAGLENGTHTOOLONG
        self.aDIMAGE17TAGREQUIRED = aDIMAGE17TAGREQUIRED
        self.aDIMAGE18LINKDUPLICATED = aDIMAGE18LINKDUPLICATED
        self.aDIMAGE18LINKLENGTHTOOLONG = aDIMAGE18LINKLENGTHTOOLONG
        self.aDIMAGE18LINKREQUIRED = aDIMAGE18LINKREQUIRED
        self.aDIMAGE18LINKWARNING = aDIMAGE18LINKWARNING
        self.aDIMAGE18TAGDUPLICATED = aDIMAGE18TAGDUPLICATED
        self.aDIMAGE18TAGLENGTHTOOLONG = aDIMAGE18TAGLENGTHTOOLONG
        self.aDIMAGE18TAGREQUIRED = aDIMAGE18TAGREQUIRED
        self.aDIMAGE19LINKDUPLICATED = aDIMAGE19LINKDUPLICATED
        self.aDIMAGE19LINKLENGTHTOOLONG = aDIMAGE19LINKLENGTHTOOLONG
        self.aDIMAGE19LINKREQUIRED = aDIMAGE19LINKREQUIRED
        self.aDIMAGE19LINKWARNING = aDIMAGE19LINKWARNING
        self.aDIMAGE19TAGDUPLICATED = aDIMAGE19TAGDUPLICATED
        self.aDIMAGE19TAGLENGTHTOOLONG = aDIMAGE19TAGLENGTHTOOLONG
        self.aDIMAGE19TAGREQUIRED = aDIMAGE19TAGREQUIRED
        self.aDIMAGE1LINKDUPLICATED = aDIMAGE1LINKDUPLICATED
        self.aDIMAGE1LINKLENGTHTOOLONG = aDIMAGE1LINKLENGTHTOOLONG
        self.aDIMAGE1LINKREQUIRED = aDIMAGE1LINKREQUIRED
        self.aDIMAGE1LINKWARNING = aDIMAGE1LINKWARNING
        self.aDIMAGE1TAGDUPLICATED = aDIMAGE1TAGDUPLICATED
        self.aDIMAGE1TAGLENGTHTOOLONG = aDIMAGE1TAGLENGTHTOOLONG
        self.aDIMAGE1TAGREQUIRED = aDIMAGE1TAGREQUIRED
        self.aDIMAGE2LINKDUPLICATED = aDIMAGE2LINKDUPLICATED
        self.aDIMAGE2LINKLENGTHTOOLONG = aDIMAGE2LINKLENGTHTOOLONG
        self.aDIMAGE2LINKREQUIRED = aDIMAGE2LINKREQUIRED
        self.aDIMAGE2LINKWARNING = aDIMAGE2LINKWARNING
        self.aDIMAGE2TAGDUPLICATED = aDIMAGE2TAGDUPLICATED
        self.aDIMAGE2TAGLENGTHTOOLONG = aDIMAGE2TAGLENGTHTOOLONG
        self.aDIMAGE2TAGREQUIRED = aDIMAGE2TAGREQUIRED
        self.aDIMAGE3LINKDUPLICATED = aDIMAGE3LINKDUPLICATED
        self.aDIMAGE3LINKLENGTHTOOLONG = aDIMAGE3LINKLENGTHTOOLONG
        self.aDIMAGE3LINKREQUIRED = aDIMAGE3LINKREQUIRED
        self.aDIMAGE3LINKWARNING = aDIMAGE3LINKWARNING
        self.aDIMAGE3TAGDUPLICATED = aDIMAGE3TAGDUPLICATED
        self.aDIMAGE3TAGLENGTHTOOLONG = aDIMAGE3TAGLENGTHTOOLONG
        self.aDIMAGE3TAGREQUIRED = aDIMAGE3TAGREQUIRED
        self.aDIMAGE4LINKDUPLICATED = aDIMAGE4LINKDUPLICATED
        self.aDIMAGE4LINKLENGTHTOOLONG = aDIMAGE4LINKLENGTHTOOLONG
        self.aDIMAGE4LINKREQUIRED = aDIMAGE4LINKREQUIRED
        self.aDIMAGE4LINKWARNING = aDIMAGE4LINKWARNING
        self.aDIMAGE4TAGDUPLICATED = aDIMAGE4TAGDUPLICATED
        self.aDIMAGE4TAGLENGTHTOOLONG = aDIMAGE4TAGLENGTHTOOLONG
        self.aDIMAGE4TAGREQUIRED = aDIMAGE4TAGREQUIRED
        self.aDIMAGE5LINKDUPLICATED = aDIMAGE5LINKDUPLICATED
        self.aDIMAGE5LINKLENGTHTOOLONG = aDIMAGE5LINKLENGTHTOOLONG
        self.aDIMAGE5LINKREQUIRED = aDIMAGE5LINKREQUIRED
        self.aDIMAGE5LINKWARNING = aDIMAGE5LINKWARNING
        self.aDIMAGE5TAGDUPLICATED = aDIMAGE5TAGDUPLICATED
        self.aDIMAGE5TAGLENGTHTOOLONG = aDIMAGE5TAGLENGTHTOOLONG
        self.aDIMAGE5TAGREQUIRED = aDIMAGE5TAGREQUIRED
        self.aDIMAGE6LINKDUPLICATED = aDIMAGE6LINKDUPLICATED
        self.aDIMAGE6LINKLENGTHTOOLONG = aDIMAGE6LINKLENGTHTOOLONG
        self.aDIMAGE6LINKREQUIRED = aDIMAGE6LINKREQUIRED
        self.aDIMAGE6LINKWARNING = aDIMAGE6LINKWARNING
        self.aDIMAGE6TAGDUPLICATED = aDIMAGE6TAGDUPLICATED
        self.aDIMAGE6TAGLENGTHTOOLONG = aDIMAGE6TAGLENGTHTOOLONG
        self.aDIMAGE6TAGREQUIRED = aDIMAGE6TAGREQUIRED
        self.aDIMAGE7LINKDUPLICATED = aDIMAGE7LINKDUPLICATED
        self.aDIMAGE7LINKLENGTHTOOLONG = aDIMAGE7LINKLENGTHTOOLONG
        self.aDIMAGE7LINKREQUIRED = aDIMAGE7LINKREQUIRED
        self.aDIMAGE7LINKWARNING = aDIMAGE7LINKWARNING
        self.aDIMAGE7TAGDUPLICATED = aDIMAGE7TAGDUPLICATED
        self.aDIMAGE7TAGLENGTHTOOLONG = aDIMAGE7TAGLENGTHTOOLONG
        self.aDIMAGE7TAGREQUIRED = aDIMAGE7TAGREQUIRED
        self.aDIMAGE8LINKDUPLICATED = aDIMAGE8LINKDUPLICATED
        self.aDIMAGE8LINKLENGTHTOOLONG = aDIMAGE8LINKLENGTHTOOLONG
        self.aDIMAGE8LINKREQUIRED = aDIMAGE8LINKREQUIRED
        self.aDIMAGE8LINKWARNING = aDIMAGE8LINKWARNING
        self.aDIMAGE8TAGDUPLICATED = aDIMAGE8TAGDUPLICATED
        self.aDIMAGE8TAGLENGTHTOOLONG = aDIMAGE8TAGLENGTHTOOLONG
        self.aDIMAGE8TAGREQUIRED = aDIMAGE8TAGREQUIRED
        self.aDIMAGE9LINKDUPLICATED = aDIMAGE9LINKDUPLICATED
        self.aDIMAGE9LINKLENGTHTOOLONG = aDIMAGE9LINKLENGTHTOOLONG
        self.aDIMAGE9LINKREQUIRED = aDIMAGE9LINKREQUIRED
        self.aDIMAGE9LINKWARNING = aDIMAGE9LINKWARNING
        self.aDIMAGE9TAGDUPLICATED = aDIMAGE9TAGDUPLICATED
        self.aDIMAGE9TAGLENGTHTOOLONG = aDIMAGE9TAGLENGTHTOOLONG
        self.aDIMAGE9TAGREQUIRED = aDIMAGE9TAGREQUIRED
        self.AD_LINK_FORMAT_WARNING = AD_LINK_FORMAT_WARNING
        self.AD_LINK_SAME_AS_LINK = AD_LINK_SAME_AS_LINK
        self.aDVIDEO0LINKDUPLICATED = aDVIDEO0LINKDUPLICATED
        self.aDVIDEO0LINKLENGTHTOOLONG = aDVIDEO0LINKLENGTHTOOLONG
        self.aDVIDEO0LINKREQUIRED = aDVIDEO0LINKREQUIRED
        self.aDVIDEO0LINKWARNING = aDVIDEO0LINKWARNING
        self.aDVIDEO0TAGDUPLICATED = aDVIDEO0TAGDUPLICATED
        self.aDVIDEO0TAGLENGTHTOOLONG = aDVIDEO0TAGLENGTHTOOLONG
        self.aDVIDEO0TAGREQUIRED = aDVIDEO0TAGREQUIRED
        self.aDVIDEO1LINKDUPLICATED = aDVIDEO1LINKDUPLICATED
        self.aDVIDEO1LINKLENGTHTOOLONG = aDVIDEO1LINKLENGTHTOOLONG
        self.aDVIDEO1LINKREQUIRED = aDVIDEO1LINKREQUIRED
        self.aDVIDEO1LINKWARNING = aDVIDEO1LINKWARNING
        self.aDVIDEO1TAGDUPLICATED = aDVIDEO1TAGDUPLICATED
        self.aDVIDEO1TAGLENGTHTOOLONG = aDVIDEO1TAGLENGTHTOOLONG
        self.aDVIDEO1TAGREQUIRED = aDVIDEO1TAGREQUIRED
        self.aDVIDEO2LINKDUPLICATED = aDVIDEO2LINKDUPLICATED
        self.aDVIDEO2LINKLENGTHTOOLONG = aDVIDEO2LINKLENGTHTOOLONG
        self.aDVIDEO2LINKREQUIRED = aDVIDEO2LINKREQUIRED
        self.aDVIDEO2LINKWARNING = aDVIDEO2LINKWARNING
        self.aDVIDEO2TAGDUPLICATED = aDVIDEO2TAGDUPLICATED
        self.aDVIDEO2TAGLENGTHTOOLONG = aDVIDEO2TAGLENGTHTOOLONG
        self.aDVIDEO2TAGREQUIRED = aDVIDEO2TAGREQUIRED
        self.ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG = ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG
        self.ADDITIONAL_IMAGE_LINK_WARNING = ADDITIONAL_IMAGE_LINK_WARNING
        self.ADWORDS_FORMAT_WARNING = ADWORDS_FORMAT_WARNING
        self.ADWORDS_SAME_AS_LINK = ADWORDS_SAME_AS_LINK
        self.AGE_GROUP_INVALID = AGE_GROUP_INVALID
        self.ANDROID_DEEP_LINK_INVALID = ANDROID_DEEP_LINK_INVALID
        self.AVAILABILITY_DATE_INVALID = AVAILABILITY_DATE_INVALID
        self.COUNTRY_DOES_NOT_MAP_TO_CURRENCY = COUNTRY_DOES_NOT_MAP_TO_CURRENCY
        self.CUSTOM_LABEL_LENGTH_TOO_LONG = CUSTOM_LABEL_LENGTH_TOO_LONG
        self.DESCRIPTION_LENGTH_TOO_LONG = DESCRIPTION_LENGTH_TOO_LONG
        self.DUPLICATE_HEADERS = DUPLICATE_HEADERS
        self.EXPIRATION_DATE_INVALID = EXPIRATION_DATE_INVALID
        self.FETCH_SAME_SIGNATURE = FETCH_SAME_SIGNATURE
        self.GENDER_INVALID = GENDER_INVALID
        self.GTIN_INVALID = GTIN_INVALID
        self.IMAGE_LINK_WARNING = IMAGE_LINK_WARNING
        self.INCONSISTENT_CURRENCY_VALUES = INCONSISTENT_CURRENCY_VALUES
        self.INDEXED_PRODUCT_COUNT_LARGE_DELTA = INDEXED_PRODUCT_COUNT_LARGE_DELTA
        self.IOS_DEEP_LINK_INVALID = IOS_DEEP_LINK_INVALID
        self.IS_BUNDLE_INVALID = IS_BUNDLE_INVALID
        self.ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE = ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE
        self.LINK_FORMAT_WARNING = LINK_FORMAT_WARNING
        self.MIN_AD_PRICE_INVALID = MIN_AD_PRICE_INVALID
        self.MPN_INVALID = MPN_INVALID
        self.MULTIPACK_INVALID = MULTIPACK_INVALID
        self.OPTIONAL_CONDITION_INVALID = OPTIONAL_CONDITION_INVALID
        self.OPTIONAL_CONDITION_MISSING = OPTIONAL_CONDITION_MISSING
        self.OPTIONAL_PRODUCT_CATEGORY_INVALID = OPTIONAL_PRODUCT_CATEGORY_INVALID
        self.OPTIONAL_PRODUCT_CATEGORY_MISSING = OPTIONAL_PRODUCT_CATEGORY_MISSING
        self.PRODUCT_CATEGORY_DEPTH_WARNING = PRODUCT_CATEGORY_DEPTH_WARNING
        self.PRODUCT_TYPE_LENGTH_TOO_LONG = PRODUCT_TYPE_LENGTH_TOO_LONG
        self.SALE_DATE_INVALID = SALE_DATE_INVALID
        self.SALES_PRICE_INVALID = SALES_PRICE_INVALID
        self.SALES_PRICE_TOO_HIGH = SALES_PRICE_TOO_HIGH
        self.SALES_PRICE_TOO_LOW = SALES_PRICE_TOO_LOW
        self.SHIPPING_HEIGHT_INVALID = SHIPPING_HEIGHT_INVALID
        self.SHIPPING_INVALID = SHIPPING_INVALID
        self.SHIPPING_WEIGHT_INVALID = SHIPPING_WEIGHT_INVALID
        self.SHIPPING_WIDTH_INVALID = SHIPPING_WIDTH_INVALID
        self.SIZE_SYSTEM_INVALID = SIZE_SYSTEM_INVALID
        self.SIZE_TYPE_INVALID = SIZE_TYPE_INVALID
        self.TAX_INVALID = TAX_INVALID
        self.TITLE_LENGTH_TOO_LONG = TITLE_LENGTH_TOO_LONG
        self.TOO_MANY_ADDITIONAL_IMAGE_LINKS = TOO_MANY_ADDITIONAL_IMAGE_LINKS
        self.UPDATED_TIME_INVALID = UPDATED_TIME_INVALID
        self.UTM_SOURCE_AUTO_CORRECTED = UTM_SOURCE_AUTO_CORRECTED
        self.VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED = VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED
        self.WEIGHT_UNIT_INVALID = WEIGHT_UNIT_INVALID
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case aDIMAGE0LINKDUPLICATED = "AD_IMAGE_0_LINK_DUPLICATED"
        case aDIMAGE0LINKLENGTHTOOLONG = "AD_IMAGE_0_LINK_LENGTH_TOO_LONG"
        case aDIMAGE0LINKREQUIRED = "AD_IMAGE_0_LINK_REQUIRED"
        case aDIMAGE0LINKWARNING = "AD_IMAGE_0_LINK_WARNING"
        case aDIMAGE0TAGDUPLICATED = "AD_IMAGE_0_TAG_DUPLICATED"
        case aDIMAGE0TAGLENGTHTOOLONG = "AD_IMAGE_0_TAG_LENGTH_TOO_LONG"
        case aDIMAGE0TAGREQUIRED = "AD_IMAGE_0_TAG_REQUIRED"
        case aDIMAGE10LINKDUPLICATED = "AD_IMAGE_10_LINK_DUPLICATED"
        case aDIMAGE10LINKLENGTHTOOLONG = "AD_IMAGE_10_LINK_LENGTH_TOO_LONG"
        case aDIMAGE10LINKREQUIRED = "AD_IMAGE_10_LINK_REQUIRED"
        case aDIMAGE10LINKWARNING = "AD_IMAGE_10_LINK_WARNING"
        case aDIMAGE10TAGDUPLICATED = "AD_IMAGE_10_TAG_DUPLICATED"
        case aDIMAGE10TAGLENGTHTOOLONG = "AD_IMAGE_10_TAG_LENGTH_TOO_LONG"
        case aDIMAGE10TAGREQUIRED = "AD_IMAGE_10_TAG_REQUIRED"
        case aDIMAGE11LINKDUPLICATED = "AD_IMAGE_11_LINK_DUPLICATED"
        case aDIMAGE11LINKLENGTHTOOLONG = "AD_IMAGE_11_LINK_LENGTH_TOO_LONG"
        case aDIMAGE11LINKREQUIRED = "AD_IMAGE_11_LINK_REQUIRED"
        case aDIMAGE11LINKWARNING = "AD_IMAGE_11_LINK_WARNING"
        case aDIMAGE11TAGDUPLICATED = "AD_IMAGE_11_TAG_DUPLICATED"
        case aDIMAGE11TAGLENGTHTOOLONG = "AD_IMAGE_11_TAG_LENGTH_TOO_LONG"
        case aDIMAGE11TAGREQUIRED = "AD_IMAGE_11_TAG_REQUIRED"
        case aDIMAGE12LINKDUPLICATED = "AD_IMAGE_12_LINK_DUPLICATED"
        case aDIMAGE12LINKLENGTHTOOLONG = "AD_IMAGE_12_LINK_LENGTH_TOO_LONG"
        case aDIMAGE12LINKREQUIRED = "AD_IMAGE_12_LINK_REQUIRED"
        case aDIMAGE12LINKWARNING = "AD_IMAGE_12_LINK_WARNING"
        case aDIMAGE12TAGDUPLICATED = "AD_IMAGE_12_TAG_DUPLICATED"
        case aDIMAGE12TAGLENGTHTOOLONG = "AD_IMAGE_12_TAG_LENGTH_TOO_LONG"
        case aDIMAGE12TAGREQUIRED = "AD_IMAGE_12_TAG_REQUIRED"
        case aDIMAGE13LINKDUPLICATED = "AD_IMAGE_13_LINK_DUPLICATED"
        case aDIMAGE13LINKLENGTHTOOLONG = "AD_IMAGE_13_LINK_LENGTH_TOO_LONG"
        case aDIMAGE13LINKREQUIRED = "AD_IMAGE_13_LINK_REQUIRED"
        case aDIMAGE13LINKWARNING = "AD_IMAGE_13_LINK_WARNING"
        case aDIMAGE13TAGDUPLICATED = "AD_IMAGE_13_TAG_DUPLICATED"
        case aDIMAGE13TAGLENGTHTOOLONG = "AD_IMAGE_13_TAG_LENGTH_TOO_LONG"
        case aDIMAGE13TAGREQUIRED = "AD_IMAGE_13_TAG_REQUIRED"
        case aDIMAGE14LINKDUPLICATED = "AD_IMAGE_14_LINK_DUPLICATED"
        case aDIMAGE14LINKLENGTHTOOLONG = "AD_IMAGE_14_LINK_LENGTH_TOO_LONG"
        case aDIMAGE14LINKREQUIRED = "AD_IMAGE_14_LINK_REQUIRED"
        case aDIMAGE14LINKWARNING = "AD_IMAGE_14_LINK_WARNING"
        case aDIMAGE14TAGDUPLICATED = "AD_IMAGE_14_TAG_DUPLICATED"
        case aDIMAGE14TAGLENGTHTOOLONG = "AD_IMAGE_14_TAG_LENGTH_TOO_LONG"
        case aDIMAGE14TAGREQUIRED = "AD_IMAGE_14_TAG_REQUIRED"
        case aDIMAGE15LINKDUPLICATED = "AD_IMAGE_15_LINK_DUPLICATED"
        case aDIMAGE15LINKLENGTHTOOLONG = "AD_IMAGE_15_LINK_LENGTH_TOO_LONG"
        case aDIMAGE15LINKREQUIRED = "AD_IMAGE_15_LINK_REQUIRED"
        case aDIMAGE15LINKWARNING = "AD_IMAGE_15_LINK_WARNING"
        case aDIMAGE15TAGDUPLICATED = "AD_IMAGE_15_TAG_DUPLICATED"
        case aDIMAGE15TAGLENGTHTOOLONG = "AD_IMAGE_15_TAG_LENGTH_TOO_LONG"
        case aDIMAGE15TAGREQUIRED = "AD_IMAGE_15_TAG_REQUIRED"
        case aDIMAGE16LINKDUPLICATED = "AD_IMAGE_16_LINK_DUPLICATED"
        case aDIMAGE16LINKLENGTHTOOLONG = "AD_IMAGE_16_LINK_LENGTH_TOO_LONG"
        case aDIMAGE16LINKREQUIRED = "AD_IMAGE_16_LINK_REQUIRED"
        case aDIMAGE16LINKWARNING = "AD_IMAGE_16_LINK_WARNING"
        case aDIMAGE16TAGDUPLICATED = "AD_IMAGE_16_TAG_DUPLICATED"
        case aDIMAGE16TAGLENGTHTOOLONG = "AD_IMAGE_16_TAG_LENGTH_TOO_LONG"
        case aDIMAGE16TAGREQUIRED = "AD_IMAGE_16_TAG_REQUIRED"
        case aDIMAGE17LINKDUPLICATED = "AD_IMAGE_17_LINK_DUPLICATED"
        case aDIMAGE17LINKLENGTHTOOLONG = "AD_IMAGE_17_LINK_LENGTH_TOO_LONG"
        case aDIMAGE17LINKREQUIRED = "AD_IMAGE_17_LINK_REQUIRED"
        case aDIMAGE17LINKWARNING = "AD_IMAGE_17_LINK_WARNING"
        case aDIMAGE17TAGDUPLICATED = "AD_IMAGE_17_TAG_DUPLICATED"
        case aDIMAGE17TAGLENGTHTOOLONG = "AD_IMAGE_17_TAG_LENGTH_TOO_LONG"
        case aDIMAGE17TAGREQUIRED = "AD_IMAGE_17_TAG_REQUIRED"
        case aDIMAGE18LINKDUPLICATED = "AD_IMAGE_18_LINK_DUPLICATED"
        case aDIMAGE18LINKLENGTHTOOLONG = "AD_IMAGE_18_LINK_LENGTH_TOO_LONG"
        case aDIMAGE18LINKREQUIRED = "AD_IMAGE_18_LINK_REQUIRED"
        case aDIMAGE18LINKWARNING = "AD_IMAGE_18_LINK_WARNING"
        case aDIMAGE18TAGDUPLICATED = "AD_IMAGE_18_TAG_DUPLICATED"
        case aDIMAGE18TAGLENGTHTOOLONG = "AD_IMAGE_18_TAG_LENGTH_TOO_LONG"
        case aDIMAGE18TAGREQUIRED = "AD_IMAGE_18_TAG_REQUIRED"
        case aDIMAGE19LINKDUPLICATED = "AD_IMAGE_19_LINK_DUPLICATED"
        case aDIMAGE19LINKLENGTHTOOLONG = "AD_IMAGE_19_LINK_LENGTH_TOO_LONG"
        case aDIMAGE19LINKREQUIRED = "AD_IMAGE_19_LINK_REQUIRED"
        case aDIMAGE19LINKWARNING = "AD_IMAGE_19_LINK_WARNING"
        case aDIMAGE19TAGDUPLICATED = "AD_IMAGE_19_TAG_DUPLICATED"
        case aDIMAGE19TAGLENGTHTOOLONG = "AD_IMAGE_19_TAG_LENGTH_TOO_LONG"
        case aDIMAGE19TAGREQUIRED = "AD_IMAGE_19_TAG_REQUIRED"
        case aDIMAGE1LINKDUPLICATED = "AD_IMAGE_1_LINK_DUPLICATED"
        case aDIMAGE1LINKLENGTHTOOLONG = "AD_IMAGE_1_LINK_LENGTH_TOO_LONG"
        case aDIMAGE1LINKREQUIRED = "AD_IMAGE_1_LINK_REQUIRED"
        case aDIMAGE1LINKWARNING = "AD_IMAGE_1_LINK_WARNING"
        case aDIMAGE1TAGDUPLICATED = "AD_IMAGE_1_TAG_DUPLICATED"
        case aDIMAGE1TAGLENGTHTOOLONG = "AD_IMAGE_1_TAG_LENGTH_TOO_LONG"
        case aDIMAGE1TAGREQUIRED = "AD_IMAGE_1_TAG_REQUIRED"
        case aDIMAGE2LINKDUPLICATED = "AD_IMAGE_2_LINK_DUPLICATED"
        case aDIMAGE2LINKLENGTHTOOLONG = "AD_IMAGE_2_LINK_LENGTH_TOO_LONG"
        case aDIMAGE2LINKREQUIRED = "AD_IMAGE_2_LINK_REQUIRED"
        case aDIMAGE2LINKWARNING = "AD_IMAGE_2_LINK_WARNING"
        case aDIMAGE2TAGDUPLICATED = "AD_IMAGE_2_TAG_DUPLICATED"
        case aDIMAGE2TAGLENGTHTOOLONG = "AD_IMAGE_2_TAG_LENGTH_TOO_LONG"
        case aDIMAGE2TAGREQUIRED = "AD_IMAGE_2_TAG_REQUIRED"
        case aDIMAGE3LINKDUPLICATED = "AD_IMAGE_3_LINK_DUPLICATED"
        case aDIMAGE3LINKLENGTHTOOLONG = "AD_IMAGE_3_LINK_LENGTH_TOO_LONG"
        case aDIMAGE3LINKREQUIRED = "AD_IMAGE_3_LINK_REQUIRED"
        case aDIMAGE3LINKWARNING = "AD_IMAGE_3_LINK_WARNING"
        case aDIMAGE3TAGDUPLICATED = "AD_IMAGE_3_TAG_DUPLICATED"
        case aDIMAGE3TAGLENGTHTOOLONG = "AD_IMAGE_3_TAG_LENGTH_TOO_LONG"
        case aDIMAGE3TAGREQUIRED = "AD_IMAGE_3_TAG_REQUIRED"
        case aDIMAGE4LINKDUPLICATED = "AD_IMAGE_4_LINK_DUPLICATED"
        case aDIMAGE4LINKLENGTHTOOLONG = "AD_IMAGE_4_LINK_LENGTH_TOO_LONG"
        case aDIMAGE4LINKREQUIRED = "AD_IMAGE_4_LINK_REQUIRED"
        case aDIMAGE4LINKWARNING = "AD_IMAGE_4_LINK_WARNING"
        case aDIMAGE4TAGDUPLICATED = "AD_IMAGE_4_TAG_DUPLICATED"
        case aDIMAGE4TAGLENGTHTOOLONG = "AD_IMAGE_4_TAG_LENGTH_TOO_LONG"
        case aDIMAGE4TAGREQUIRED = "AD_IMAGE_4_TAG_REQUIRED"
        case aDIMAGE5LINKDUPLICATED = "AD_IMAGE_5_LINK_DUPLICATED"
        case aDIMAGE5LINKLENGTHTOOLONG = "AD_IMAGE_5_LINK_LENGTH_TOO_LONG"
        case aDIMAGE5LINKREQUIRED = "AD_IMAGE_5_LINK_REQUIRED"
        case aDIMAGE5LINKWARNING = "AD_IMAGE_5_LINK_WARNING"
        case aDIMAGE5TAGDUPLICATED = "AD_IMAGE_5_TAG_DUPLICATED"
        case aDIMAGE5TAGLENGTHTOOLONG = "AD_IMAGE_5_TAG_LENGTH_TOO_LONG"
        case aDIMAGE5TAGREQUIRED = "AD_IMAGE_5_TAG_REQUIRED"
        case aDIMAGE6LINKDUPLICATED = "AD_IMAGE_6_LINK_DUPLICATED"
        case aDIMAGE6LINKLENGTHTOOLONG = "AD_IMAGE_6_LINK_LENGTH_TOO_LONG"
        case aDIMAGE6LINKREQUIRED = "AD_IMAGE_6_LINK_REQUIRED"
        case aDIMAGE6LINKWARNING = "AD_IMAGE_6_LINK_WARNING"
        case aDIMAGE6TAGDUPLICATED = "AD_IMAGE_6_TAG_DUPLICATED"
        case aDIMAGE6TAGLENGTHTOOLONG = "AD_IMAGE_6_TAG_LENGTH_TOO_LONG"
        case aDIMAGE6TAGREQUIRED = "AD_IMAGE_6_TAG_REQUIRED"
        case aDIMAGE7LINKDUPLICATED = "AD_IMAGE_7_LINK_DUPLICATED"
        case aDIMAGE7LINKLENGTHTOOLONG = "AD_IMAGE_7_LINK_LENGTH_TOO_LONG"
        case aDIMAGE7LINKREQUIRED = "AD_IMAGE_7_LINK_REQUIRED"
        case aDIMAGE7LINKWARNING = "AD_IMAGE_7_LINK_WARNING"
        case aDIMAGE7TAGDUPLICATED = "AD_IMAGE_7_TAG_DUPLICATED"
        case aDIMAGE7TAGLENGTHTOOLONG = "AD_IMAGE_7_TAG_LENGTH_TOO_LONG"
        case aDIMAGE7TAGREQUIRED = "AD_IMAGE_7_TAG_REQUIRED"
        case aDIMAGE8LINKDUPLICATED = "AD_IMAGE_8_LINK_DUPLICATED"
        case aDIMAGE8LINKLENGTHTOOLONG = "AD_IMAGE_8_LINK_LENGTH_TOO_LONG"
        case aDIMAGE8LINKREQUIRED = "AD_IMAGE_8_LINK_REQUIRED"
        case aDIMAGE8LINKWARNING = "AD_IMAGE_8_LINK_WARNING"
        case aDIMAGE8TAGDUPLICATED = "AD_IMAGE_8_TAG_DUPLICATED"
        case aDIMAGE8TAGLENGTHTOOLONG = "AD_IMAGE_8_TAG_LENGTH_TOO_LONG"
        case aDIMAGE8TAGREQUIRED = "AD_IMAGE_8_TAG_REQUIRED"
        case aDIMAGE9LINKDUPLICATED = "AD_IMAGE_9_LINK_DUPLICATED"
        case aDIMAGE9LINKLENGTHTOOLONG = "AD_IMAGE_9_LINK_LENGTH_TOO_LONG"
        case aDIMAGE9LINKREQUIRED = "AD_IMAGE_9_LINK_REQUIRED"
        case aDIMAGE9LINKWARNING = "AD_IMAGE_9_LINK_WARNING"
        case aDIMAGE9TAGDUPLICATED = "AD_IMAGE_9_TAG_DUPLICATED"
        case aDIMAGE9TAGLENGTHTOOLONG = "AD_IMAGE_9_TAG_LENGTH_TOO_LONG"
        case aDIMAGE9TAGREQUIRED = "AD_IMAGE_9_TAG_REQUIRED"
        case AD_LINK_FORMAT_WARNING
        case AD_LINK_SAME_AS_LINK
        case aDVIDEO0LINKDUPLICATED = "AD_VIDEO_0_LINK_DUPLICATED"
        case aDVIDEO0LINKLENGTHTOOLONG = "AD_VIDEO_0_LINK_LENGTH_TOO_LONG"
        case aDVIDEO0LINKREQUIRED = "AD_VIDEO_0_LINK_REQUIRED"
        case aDVIDEO0LINKWARNING = "AD_VIDEO_0_LINK_WARNING"
        case aDVIDEO0TAGDUPLICATED = "AD_VIDEO_0_TAG_DUPLICATED"
        case aDVIDEO0TAGLENGTHTOOLONG = "AD_VIDEO_0_TAG_LENGTH_TOO_LONG"
        case aDVIDEO0TAGREQUIRED = "AD_VIDEO_0_TAG_REQUIRED"
        case aDVIDEO1LINKDUPLICATED = "AD_VIDEO_1_LINK_DUPLICATED"
        case aDVIDEO1LINKLENGTHTOOLONG = "AD_VIDEO_1_LINK_LENGTH_TOO_LONG"
        case aDVIDEO1LINKREQUIRED = "AD_VIDEO_1_LINK_REQUIRED"
        case aDVIDEO1LINKWARNING = "AD_VIDEO_1_LINK_WARNING"
        case aDVIDEO1TAGDUPLICATED = "AD_VIDEO_1_TAG_DUPLICATED"
        case aDVIDEO1TAGLENGTHTOOLONG = "AD_VIDEO_1_TAG_LENGTH_TOO_LONG"
        case aDVIDEO1TAGREQUIRED = "AD_VIDEO_1_TAG_REQUIRED"
        case aDVIDEO2LINKDUPLICATED = "AD_VIDEO_2_LINK_DUPLICATED"
        case aDVIDEO2LINKLENGTHTOOLONG = "AD_VIDEO_2_LINK_LENGTH_TOO_LONG"
        case aDVIDEO2LINKREQUIRED = "AD_VIDEO_2_LINK_REQUIRED"
        case aDVIDEO2LINKWARNING = "AD_VIDEO_2_LINK_WARNING"
        case aDVIDEO2TAGDUPLICATED = "AD_VIDEO_2_TAG_DUPLICATED"
        case aDVIDEO2TAGLENGTHTOOLONG = "AD_VIDEO_2_TAG_LENGTH_TOO_LONG"
        case aDVIDEO2TAGREQUIRED = "AD_VIDEO_2_TAG_REQUIRED"
        case ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG
        case ADDITIONAL_IMAGE_LINK_WARNING
        case ADWORDS_FORMAT_WARNING
        case ADWORDS_SAME_AS_LINK
        case AGE_GROUP_INVALID
        case ANDROID_DEEP_LINK_INVALID
        case AVAILABILITY_DATE_INVALID
        case COUNTRY_DOES_NOT_MAP_TO_CURRENCY
        case CUSTOM_LABEL_LENGTH_TOO_LONG
        case DESCRIPTION_LENGTH_TOO_LONG
        case DUPLICATE_HEADERS
        case EXPIRATION_DATE_INVALID
        case FETCH_SAME_SIGNATURE
        case GENDER_INVALID
        case GTIN_INVALID
        case IMAGE_LINK_WARNING
        case INCONSISTENT_CURRENCY_VALUES
        case INDEXED_PRODUCT_COUNT_LARGE_DELTA
        case IOS_DEEP_LINK_INVALID
        case IS_BUNDLE_INVALID
        case ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE
        case LINK_FORMAT_WARNING
        case MIN_AD_PRICE_INVALID
        case MPN_INVALID
        case MULTIPACK_INVALID
        case OPTIONAL_CONDITION_INVALID
        case OPTIONAL_CONDITION_MISSING
        case OPTIONAL_PRODUCT_CATEGORY_INVALID
        case OPTIONAL_PRODUCT_CATEGORY_MISSING
        case PRODUCT_CATEGORY_DEPTH_WARNING
        case PRODUCT_TYPE_LENGTH_TOO_LONG
        case SALE_DATE_INVALID
        case SALES_PRICE_INVALID
        case SALES_PRICE_TOO_HIGH
        case SALES_PRICE_TOO_LOW
        case SHIPPING_HEIGHT_INVALID
        case SHIPPING_INVALID
        case SHIPPING_WEIGHT_INVALID
        case SHIPPING_WIDTH_INVALID
        case SIZE_SYSTEM_INVALID
        case SIZE_TYPE_INVALID
        case TAX_INVALID
        case TITLE_LENGTH_TOO_LONG
        case TOO_MANY_ADDITIONAL_IMAGE_LINKS
        case UPDATED_TIME_INVALID
        case UTM_SOURCE_AUTO_CORRECTED
        case VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED
        case WEIGHT_UNIT_INVALID
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        aDIMAGE0LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0LINKDUPLICATED)
        aDIMAGE0LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0LINKLENGTHTOOLONG)
        aDIMAGE0LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0LINKREQUIRED)
        aDIMAGE0LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0LINKWARNING)
        aDIMAGE0TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0TAGDUPLICATED)
        aDIMAGE0TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0TAGLENGTHTOOLONG)
        aDIMAGE0TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE0TAGREQUIRED)
        aDIMAGE10LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10LINKDUPLICATED)
        aDIMAGE10LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10LINKLENGTHTOOLONG)
        aDIMAGE10LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10LINKREQUIRED)
        aDIMAGE10LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10LINKWARNING)
        aDIMAGE10TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10TAGDUPLICATED)
        aDIMAGE10TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10TAGLENGTHTOOLONG)
        aDIMAGE10TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE10TAGREQUIRED)
        aDIMAGE11LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11LINKDUPLICATED)
        aDIMAGE11LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11LINKLENGTHTOOLONG)
        aDIMAGE11LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11LINKREQUIRED)
        aDIMAGE11LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11LINKWARNING)
        aDIMAGE11TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11TAGDUPLICATED)
        aDIMAGE11TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11TAGLENGTHTOOLONG)
        aDIMAGE11TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE11TAGREQUIRED)
        aDIMAGE12LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12LINKDUPLICATED)
        aDIMAGE12LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12LINKLENGTHTOOLONG)
        aDIMAGE12LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12LINKREQUIRED)
        aDIMAGE12LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12LINKWARNING)
        aDIMAGE12TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12TAGDUPLICATED)
        aDIMAGE12TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12TAGLENGTHTOOLONG)
        aDIMAGE12TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE12TAGREQUIRED)
        aDIMAGE13LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13LINKDUPLICATED)
        aDIMAGE13LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13LINKLENGTHTOOLONG)
        aDIMAGE13LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13LINKREQUIRED)
        aDIMAGE13LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13LINKWARNING)
        aDIMAGE13TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13TAGDUPLICATED)
        aDIMAGE13TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13TAGLENGTHTOOLONG)
        aDIMAGE13TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE13TAGREQUIRED)
        aDIMAGE14LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14LINKDUPLICATED)
        aDIMAGE14LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14LINKLENGTHTOOLONG)
        aDIMAGE14LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14LINKREQUIRED)
        aDIMAGE14LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14LINKWARNING)
        aDIMAGE14TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14TAGDUPLICATED)
        aDIMAGE14TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14TAGLENGTHTOOLONG)
        aDIMAGE14TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE14TAGREQUIRED)
        aDIMAGE15LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15LINKDUPLICATED)
        aDIMAGE15LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15LINKLENGTHTOOLONG)
        aDIMAGE15LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15LINKREQUIRED)
        aDIMAGE15LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15LINKWARNING)
        aDIMAGE15TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15TAGDUPLICATED)
        aDIMAGE15TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15TAGLENGTHTOOLONG)
        aDIMAGE15TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE15TAGREQUIRED)
        aDIMAGE16LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16LINKDUPLICATED)
        aDIMAGE16LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16LINKLENGTHTOOLONG)
        aDIMAGE16LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16LINKREQUIRED)
        aDIMAGE16LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16LINKWARNING)
        aDIMAGE16TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16TAGDUPLICATED)
        aDIMAGE16TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16TAGLENGTHTOOLONG)
        aDIMAGE16TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE16TAGREQUIRED)
        aDIMAGE17LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17LINKDUPLICATED)
        aDIMAGE17LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17LINKLENGTHTOOLONG)
        aDIMAGE17LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17LINKREQUIRED)
        aDIMAGE17LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17LINKWARNING)
        aDIMAGE17TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17TAGDUPLICATED)
        aDIMAGE17TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17TAGLENGTHTOOLONG)
        aDIMAGE17TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE17TAGREQUIRED)
        aDIMAGE18LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18LINKDUPLICATED)
        aDIMAGE18LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18LINKLENGTHTOOLONG)
        aDIMAGE18LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18LINKREQUIRED)
        aDIMAGE18LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18LINKWARNING)
        aDIMAGE18TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18TAGDUPLICATED)
        aDIMAGE18TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18TAGLENGTHTOOLONG)
        aDIMAGE18TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE18TAGREQUIRED)
        aDIMAGE19LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19LINKDUPLICATED)
        aDIMAGE19LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19LINKLENGTHTOOLONG)
        aDIMAGE19LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19LINKREQUIRED)
        aDIMAGE19LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19LINKWARNING)
        aDIMAGE19TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19TAGDUPLICATED)
        aDIMAGE19TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19TAGLENGTHTOOLONG)
        aDIMAGE19TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE19TAGREQUIRED)
        aDIMAGE1LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1LINKDUPLICATED)
        aDIMAGE1LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1LINKLENGTHTOOLONG)
        aDIMAGE1LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1LINKREQUIRED)
        aDIMAGE1LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1LINKWARNING)
        aDIMAGE1TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1TAGDUPLICATED)
        aDIMAGE1TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1TAGLENGTHTOOLONG)
        aDIMAGE1TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE1TAGREQUIRED)
        aDIMAGE2LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2LINKDUPLICATED)
        aDIMAGE2LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2LINKLENGTHTOOLONG)
        aDIMAGE2LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2LINKREQUIRED)
        aDIMAGE2LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2LINKWARNING)
        aDIMAGE2TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2TAGDUPLICATED)
        aDIMAGE2TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2TAGLENGTHTOOLONG)
        aDIMAGE2TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE2TAGREQUIRED)
        aDIMAGE3LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3LINKDUPLICATED)
        aDIMAGE3LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3LINKLENGTHTOOLONG)
        aDIMAGE3LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3LINKREQUIRED)
        aDIMAGE3LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3LINKWARNING)
        aDIMAGE3TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3TAGDUPLICATED)
        aDIMAGE3TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3TAGLENGTHTOOLONG)
        aDIMAGE3TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE3TAGREQUIRED)
        aDIMAGE4LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4LINKDUPLICATED)
        aDIMAGE4LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4LINKLENGTHTOOLONG)
        aDIMAGE4LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4LINKREQUIRED)
        aDIMAGE4LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4LINKWARNING)
        aDIMAGE4TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4TAGDUPLICATED)
        aDIMAGE4TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4TAGLENGTHTOOLONG)
        aDIMAGE4TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE4TAGREQUIRED)
        aDIMAGE5LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5LINKDUPLICATED)
        aDIMAGE5LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5LINKLENGTHTOOLONG)
        aDIMAGE5LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5LINKREQUIRED)
        aDIMAGE5LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5LINKWARNING)
        aDIMAGE5TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5TAGDUPLICATED)
        aDIMAGE5TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5TAGLENGTHTOOLONG)
        aDIMAGE5TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE5TAGREQUIRED)
        aDIMAGE6LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6LINKDUPLICATED)
        aDIMAGE6LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6LINKLENGTHTOOLONG)
        aDIMAGE6LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6LINKREQUIRED)
        aDIMAGE6LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6LINKWARNING)
        aDIMAGE6TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6TAGDUPLICATED)
        aDIMAGE6TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6TAGLENGTHTOOLONG)
        aDIMAGE6TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE6TAGREQUIRED)
        aDIMAGE7LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7LINKDUPLICATED)
        aDIMAGE7LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7LINKLENGTHTOOLONG)
        aDIMAGE7LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7LINKREQUIRED)
        aDIMAGE7LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7LINKWARNING)
        aDIMAGE7TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7TAGDUPLICATED)
        aDIMAGE7TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7TAGLENGTHTOOLONG)
        aDIMAGE7TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE7TAGREQUIRED)
        aDIMAGE8LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8LINKDUPLICATED)
        aDIMAGE8LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8LINKLENGTHTOOLONG)
        aDIMAGE8LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8LINKREQUIRED)
        aDIMAGE8LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8LINKWARNING)
        aDIMAGE8TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8TAGDUPLICATED)
        aDIMAGE8TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8TAGLENGTHTOOLONG)
        aDIMAGE8TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE8TAGREQUIRED)
        aDIMAGE9LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9LINKDUPLICATED)
        aDIMAGE9LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9LINKLENGTHTOOLONG)
        aDIMAGE9LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9LINKREQUIRED)
        aDIMAGE9LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9LINKWARNING)
        aDIMAGE9TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9TAGDUPLICATED)
        aDIMAGE9TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9TAGLENGTHTOOLONG)
        aDIMAGE9TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDIMAGE9TAGREQUIRED)
        AD_LINK_FORMAT_WARNING = try container.decodeIfPresent(Int.self, forKey: .AD_LINK_FORMAT_WARNING)
        AD_LINK_SAME_AS_LINK = try container.decodeIfPresent(Int.self, forKey: .AD_LINK_SAME_AS_LINK)
        aDVIDEO0LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0LINKDUPLICATED)
        aDVIDEO0LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0LINKLENGTHTOOLONG)
        aDVIDEO0LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0LINKREQUIRED)
        aDVIDEO0LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0LINKWARNING)
        aDVIDEO0TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0TAGDUPLICATED)
        aDVIDEO0TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0TAGLENGTHTOOLONG)
        aDVIDEO0TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO0TAGREQUIRED)
        aDVIDEO1LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1LINKDUPLICATED)
        aDVIDEO1LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1LINKLENGTHTOOLONG)
        aDVIDEO1LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1LINKREQUIRED)
        aDVIDEO1LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1LINKWARNING)
        aDVIDEO1TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1TAGDUPLICATED)
        aDVIDEO1TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1TAGLENGTHTOOLONG)
        aDVIDEO1TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO1TAGREQUIRED)
        aDVIDEO2LINKDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2LINKDUPLICATED)
        aDVIDEO2LINKLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2LINKLENGTHTOOLONG)
        aDVIDEO2LINKREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2LINKREQUIRED)
        aDVIDEO2LINKWARNING = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2LINKWARNING)
        aDVIDEO2TAGDUPLICATED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2TAGDUPLICATED)
        aDVIDEO2TAGLENGTHTOOLONG = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2TAGLENGTHTOOLONG)
        aDVIDEO2TAGREQUIRED = try container.decodeIfPresent(Int.self, forKey: .aDVIDEO2TAGREQUIRED)
        ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG = try container.decodeIfPresent(Int.self, forKey: .ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG)
        ADDITIONAL_IMAGE_LINK_WARNING = try container.decodeIfPresent(Int.self, forKey: .ADDITIONAL_IMAGE_LINK_WARNING)
        ADWORDS_FORMAT_WARNING = try container.decodeIfPresent(Int.self, forKey: .ADWORDS_FORMAT_WARNING)
        ADWORDS_SAME_AS_LINK = try container.decodeIfPresent(Int.self, forKey: .ADWORDS_SAME_AS_LINK)
        AGE_GROUP_INVALID = try container.decodeIfPresent(Int.self, forKey: .AGE_GROUP_INVALID)
        ANDROID_DEEP_LINK_INVALID = try container.decodeIfPresent(Int.self, forKey: .ANDROID_DEEP_LINK_INVALID)
        AVAILABILITY_DATE_INVALID = try container.decodeIfPresent(Int.self, forKey: .AVAILABILITY_DATE_INVALID)
        COUNTRY_DOES_NOT_MAP_TO_CURRENCY = try container.decodeIfPresent(Int.self, forKey: .COUNTRY_DOES_NOT_MAP_TO_CURRENCY)
        CUSTOM_LABEL_LENGTH_TOO_LONG = try container.decodeIfPresent(Int.self, forKey: .CUSTOM_LABEL_LENGTH_TOO_LONG)
        DESCRIPTION_LENGTH_TOO_LONG = try container.decodeIfPresent(Int.self, forKey: .DESCRIPTION_LENGTH_TOO_LONG)
        DUPLICATE_HEADERS = try container.decodeIfPresent(Int.self, forKey: .DUPLICATE_HEADERS)
        EXPIRATION_DATE_INVALID = try container.decodeIfPresent(Int.self, forKey: .EXPIRATION_DATE_INVALID)
        FETCH_SAME_SIGNATURE = try container.decodeIfPresent(Int.self, forKey: .FETCH_SAME_SIGNATURE)
        GENDER_INVALID = try container.decodeIfPresent(Int.self, forKey: .GENDER_INVALID)
        GTIN_INVALID = try container.decodeIfPresent(Int.self, forKey: .GTIN_INVALID)
        IMAGE_LINK_WARNING = try container.decodeIfPresent(Int.self, forKey: .IMAGE_LINK_WARNING)
        INCONSISTENT_CURRENCY_VALUES = try container.decodeIfPresent(Int.self, forKey: .INCONSISTENT_CURRENCY_VALUES)
        INDEXED_PRODUCT_COUNT_LARGE_DELTA = try container.decodeIfPresent(Int.self, forKey: .INDEXED_PRODUCT_COUNT_LARGE_DELTA)
        IOS_DEEP_LINK_INVALID = try container.decodeIfPresent(Int.self, forKey: .IOS_DEEP_LINK_INVALID)
        IS_BUNDLE_INVALID = try container.decodeIfPresent(Int.self, forKey: .IS_BUNDLE_INVALID)
        ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE = try container.decodeIfPresent(Int.self, forKey: .ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE)
        LINK_FORMAT_WARNING = try container.decodeIfPresent(Int.self, forKey: .LINK_FORMAT_WARNING)
        MIN_AD_PRICE_INVALID = try container.decodeIfPresent(Int.self, forKey: .MIN_AD_PRICE_INVALID)
        MPN_INVALID = try container.decodeIfPresent(Int.self, forKey: .MPN_INVALID)
        MULTIPACK_INVALID = try container.decodeIfPresent(Int.self, forKey: .MULTIPACK_INVALID)
        OPTIONAL_CONDITION_INVALID = try container.decodeIfPresent(Int.self, forKey: .OPTIONAL_CONDITION_INVALID)
        OPTIONAL_CONDITION_MISSING = try container.decodeIfPresent(Int.self, forKey: .OPTIONAL_CONDITION_MISSING)
        OPTIONAL_PRODUCT_CATEGORY_INVALID = try container.decodeIfPresent(Int.self, forKey: .OPTIONAL_PRODUCT_CATEGORY_INVALID)
        OPTIONAL_PRODUCT_CATEGORY_MISSING = try container.decodeIfPresent(Int.self, forKey: .OPTIONAL_PRODUCT_CATEGORY_MISSING)
        PRODUCT_CATEGORY_DEPTH_WARNING = try container.decodeIfPresent(Int.self, forKey: .PRODUCT_CATEGORY_DEPTH_WARNING)
        PRODUCT_TYPE_LENGTH_TOO_LONG = try container.decodeIfPresent(Int.self, forKey: .PRODUCT_TYPE_LENGTH_TOO_LONG)
        SALE_DATE_INVALID = try container.decodeIfPresent(Int.self, forKey: .SALE_DATE_INVALID)
        SALES_PRICE_INVALID = try container.decodeIfPresent(Int.self, forKey: .SALES_PRICE_INVALID)
        SALES_PRICE_TOO_HIGH = try container.decodeIfPresent(Int.self, forKey: .SALES_PRICE_TOO_HIGH)
        SALES_PRICE_TOO_LOW = try container.decodeIfPresent(Int.self, forKey: .SALES_PRICE_TOO_LOW)
        SHIPPING_HEIGHT_INVALID = try container.decodeIfPresent(Int.self, forKey: .SHIPPING_HEIGHT_INVALID)
        SHIPPING_INVALID = try container.decodeIfPresent(Int.self, forKey: .SHIPPING_INVALID)
        SHIPPING_WEIGHT_INVALID = try container.decodeIfPresent(Int.self, forKey: .SHIPPING_WEIGHT_INVALID)
        SHIPPING_WIDTH_INVALID = try container.decodeIfPresent(Int.self, forKey: .SHIPPING_WIDTH_INVALID)
        SIZE_SYSTEM_INVALID = try container.decodeIfPresent(Int.self, forKey: .SIZE_SYSTEM_INVALID)
        SIZE_TYPE_INVALID = try container.decodeIfPresent(Int.self, forKey: .SIZE_TYPE_INVALID)
        TAX_INVALID = try container.decodeIfPresent(Int.self, forKey: .TAX_INVALID)
        TITLE_LENGTH_TOO_LONG = try container.decodeIfPresent(Int.self, forKey: .TITLE_LENGTH_TOO_LONG)
        TOO_MANY_ADDITIONAL_IMAGE_LINKS = try container.decodeIfPresent(Int.self, forKey: .TOO_MANY_ADDITIONAL_IMAGE_LINKS)
        UPDATED_TIME_INVALID = try container.decodeIfPresent(Int.self, forKey: .UPDATED_TIME_INVALID)
        UTM_SOURCE_AUTO_CORRECTED = try container.decodeIfPresent(Int.self, forKey: .UTM_SOURCE_AUTO_CORRECTED)
        VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED = try container.decodeIfPresent(Int.self, forKey: .VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED)
        WEIGHT_UNIT_INVALID = try container.decodeIfPresent(Int.self, forKey: .WEIGHT_UNIT_INVALID)
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(aDIMAGE0LINKDUPLICATED, forKey: .aDIMAGE0LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE0LINKLENGTHTOOLONG, forKey: .aDIMAGE0LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE0LINKREQUIRED, forKey: .aDIMAGE0LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE0LINKWARNING, forKey: .aDIMAGE0LINKWARNING)
        try container.encodeIfPresent(aDIMAGE0TAGDUPLICATED, forKey: .aDIMAGE0TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE0TAGLENGTHTOOLONG, forKey: .aDIMAGE0TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE0TAGREQUIRED, forKey: .aDIMAGE0TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE10LINKDUPLICATED, forKey: .aDIMAGE10LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE10LINKLENGTHTOOLONG, forKey: .aDIMAGE10LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE10LINKREQUIRED, forKey: .aDIMAGE10LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE10LINKWARNING, forKey: .aDIMAGE10LINKWARNING)
        try container.encodeIfPresent(aDIMAGE10TAGDUPLICATED, forKey: .aDIMAGE10TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE10TAGLENGTHTOOLONG, forKey: .aDIMAGE10TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE10TAGREQUIRED, forKey: .aDIMAGE10TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE11LINKDUPLICATED, forKey: .aDIMAGE11LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE11LINKLENGTHTOOLONG, forKey: .aDIMAGE11LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE11LINKREQUIRED, forKey: .aDIMAGE11LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE11LINKWARNING, forKey: .aDIMAGE11LINKWARNING)
        try container.encodeIfPresent(aDIMAGE11TAGDUPLICATED, forKey: .aDIMAGE11TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE11TAGLENGTHTOOLONG, forKey: .aDIMAGE11TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE11TAGREQUIRED, forKey: .aDIMAGE11TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE12LINKDUPLICATED, forKey: .aDIMAGE12LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE12LINKLENGTHTOOLONG, forKey: .aDIMAGE12LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE12LINKREQUIRED, forKey: .aDIMAGE12LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE12LINKWARNING, forKey: .aDIMAGE12LINKWARNING)
        try container.encodeIfPresent(aDIMAGE12TAGDUPLICATED, forKey: .aDIMAGE12TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE12TAGLENGTHTOOLONG, forKey: .aDIMAGE12TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE12TAGREQUIRED, forKey: .aDIMAGE12TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE13LINKDUPLICATED, forKey: .aDIMAGE13LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE13LINKLENGTHTOOLONG, forKey: .aDIMAGE13LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE13LINKREQUIRED, forKey: .aDIMAGE13LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE13LINKWARNING, forKey: .aDIMAGE13LINKWARNING)
        try container.encodeIfPresent(aDIMAGE13TAGDUPLICATED, forKey: .aDIMAGE13TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE13TAGLENGTHTOOLONG, forKey: .aDIMAGE13TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE13TAGREQUIRED, forKey: .aDIMAGE13TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE14LINKDUPLICATED, forKey: .aDIMAGE14LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE14LINKLENGTHTOOLONG, forKey: .aDIMAGE14LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE14LINKREQUIRED, forKey: .aDIMAGE14LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE14LINKWARNING, forKey: .aDIMAGE14LINKWARNING)
        try container.encodeIfPresent(aDIMAGE14TAGDUPLICATED, forKey: .aDIMAGE14TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE14TAGLENGTHTOOLONG, forKey: .aDIMAGE14TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE14TAGREQUIRED, forKey: .aDIMAGE14TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE15LINKDUPLICATED, forKey: .aDIMAGE15LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE15LINKLENGTHTOOLONG, forKey: .aDIMAGE15LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE15LINKREQUIRED, forKey: .aDIMAGE15LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE15LINKWARNING, forKey: .aDIMAGE15LINKWARNING)
        try container.encodeIfPresent(aDIMAGE15TAGDUPLICATED, forKey: .aDIMAGE15TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE15TAGLENGTHTOOLONG, forKey: .aDIMAGE15TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE15TAGREQUIRED, forKey: .aDIMAGE15TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE16LINKDUPLICATED, forKey: .aDIMAGE16LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE16LINKLENGTHTOOLONG, forKey: .aDIMAGE16LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE16LINKREQUIRED, forKey: .aDIMAGE16LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE16LINKWARNING, forKey: .aDIMAGE16LINKWARNING)
        try container.encodeIfPresent(aDIMAGE16TAGDUPLICATED, forKey: .aDIMAGE16TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE16TAGLENGTHTOOLONG, forKey: .aDIMAGE16TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE16TAGREQUIRED, forKey: .aDIMAGE16TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE17LINKDUPLICATED, forKey: .aDIMAGE17LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE17LINKLENGTHTOOLONG, forKey: .aDIMAGE17LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE17LINKREQUIRED, forKey: .aDIMAGE17LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE17LINKWARNING, forKey: .aDIMAGE17LINKWARNING)
        try container.encodeIfPresent(aDIMAGE17TAGDUPLICATED, forKey: .aDIMAGE17TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE17TAGLENGTHTOOLONG, forKey: .aDIMAGE17TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE17TAGREQUIRED, forKey: .aDIMAGE17TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE18LINKDUPLICATED, forKey: .aDIMAGE18LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE18LINKLENGTHTOOLONG, forKey: .aDIMAGE18LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE18LINKREQUIRED, forKey: .aDIMAGE18LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE18LINKWARNING, forKey: .aDIMAGE18LINKWARNING)
        try container.encodeIfPresent(aDIMAGE18TAGDUPLICATED, forKey: .aDIMAGE18TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE18TAGLENGTHTOOLONG, forKey: .aDIMAGE18TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE18TAGREQUIRED, forKey: .aDIMAGE18TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE19LINKDUPLICATED, forKey: .aDIMAGE19LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE19LINKLENGTHTOOLONG, forKey: .aDIMAGE19LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE19LINKREQUIRED, forKey: .aDIMAGE19LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE19LINKWARNING, forKey: .aDIMAGE19LINKWARNING)
        try container.encodeIfPresent(aDIMAGE19TAGDUPLICATED, forKey: .aDIMAGE19TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE19TAGLENGTHTOOLONG, forKey: .aDIMAGE19TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE19TAGREQUIRED, forKey: .aDIMAGE19TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE1LINKDUPLICATED, forKey: .aDIMAGE1LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE1LINKLENGTHTOOLONG, forKey: .aDIMAGE1LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE1LINKREQUIRED, forKey: .aDIMAGE1LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE1LINKWARNING, forKey: .aDIMAGE1LINKWARNING)
        try container.encodeIfPresent(aDIMAGE1TAGDUPLICATED, forKey: .aDIMAGE1TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE1TAGLENGTHTOOLONG, forKey: .aDIMAGE1TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE1TAGREQUIRED, forKey: .aDIMAGE1TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE2LINKDUPLICATED, forKey: .aDIMAGE2LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE2LINKLENGTHTOOLONG, forKey: .aDIMAGE2LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE2LINKREQUIRED, forKey: .aDIMAGE2LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE2LINKWARNING, forKey: .aDIMAGE2LINKWARNING)
        try container.encodeIfPresent(aDIMAGE2TAGDUPLICATED, forKey: .aDIMAGE2TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE2TAGLENGTHTOOLONG, forKey: .aDIMAGE2TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE2TAGREQUIRED, forKey: .aDIMAGE2TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE3LINKDUPLICATED, forKey: .aDIMAGE3LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE3LINKLENGTHTOOLONG, forKey: .aDIMAGE3LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE3LINKREQUIRED, forKey: .aDIMAGE3LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE3LINKWARNING, forKey: .aDIMAGE3LINKWARNING)
        try container.encodeIfPresent(aDIMAGE3TAGDUPLICATED, forKey: .aDIMAGE3TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE3TAGLENGTHTOOLONG, forKey: .aDIMAGE3TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE3TAGREQUIRED, forKey: .aDIMAGE3TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE4LINKDUPLICATED, forKey: .aDIMAGE4LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE4LINKLENGTHTOOLONG, forKey: .aDIMAGE4LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE4LINKREQUIRED, forKey: .aDIMAGE4LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE4LINKWARNING, forKey: .aDIMAGE4LINKWARNING)
        try container.encodeIfPresent(aDIMAGE4TAGDUPLICATED, forKey: .aDIMAGE4TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE4TAGLENGTHTOOLONG, forKey: .aDIMAGE4TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE4TAGREQUIRED, forKey: .aDIMAGE4TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE5LINKDUPLICATED, forKey: .aDIMAGE5LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE5LINKLENGTHTOOLONG, forKey: .aDIMAGE5LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE5LINKREQUIRED, forKey: .aDIMAGE5LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE5LINKWARNING, forKey: .aDIMAGE5LINKWARNING)
        try container.encodeIfPresent(aDIMAGE5TAGDUPLICATED, forKey: .aDIMAGE5TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE5TAGLENGTHTOOLONG, forKey: .aDIMAGE5TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE5TAGREQUIRED, forKey: .aDIMAGE5TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE6LINKDUPLICATED, forKey: .aDIMAGE6LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE6LINKLENGTHTOOLONG, forKey: .aDIMAGE6LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE6LINKREQUIRED, forKey: .aDIMAGE6LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE6LINKWARNING, forKey: .aDIMAGE6LINKWARNING)
        try container.encodeIfPresent(aDIMAGE6TAGDUPLICATED, forKey: .aDIMAGE6TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE6TAGLENGTHTOOLONG, forKey: .aDIMAGE6TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE6TAGREQUIRED, forKey: .aDIMAGE6TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE7LINKDUPLICATED, forKey: .aDIMAGE7LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE7LINKLENGTHTOOLONG, forKey: .aDIMAGE7LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE7LINKREQUIRED, forKey: .aDIMAGE7LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE7LINKWARNING, forKey: .aDIMAGE7LINKWARNING)
        try container.encodeIfPresent(aDIMAGE7TAGDUPLICATED, forKey: .aDIMAGE7TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE7TAGLENGTHTOOLONG, forKey: .aDIMAGE7TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE7TAGREQUIRED, forKey: .aDIMAGE7TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE8LINKDUPLICATED, forKey: .aDIMAGE8LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE8LINKLENGTHTOOLONG, forKey: .aDIMAGE8LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE8LINKREQUIRED, forKey: .aDIMAGE8LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE8LINKWARNING, forKey: .aDIMAGE8LINKWARNING)
        try container.encodeIfPresent(aDIMAGE8TAGDUPLICATED, forKey: .aDIMAGE8TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE8TAGLENGTHTOOLONG, forKey: .aDIMAGE8TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE8TAGREQUIRED, forKey: .aDIMAGE8TAGREQUIRED)
        try container.encodeIfPresent(aDIMAGE9LINKDUPLICATED, forKey: .aDIMAGE9LINKDUPLICATED)
        try container.encodeIfPresent(aDIMAGE9LINKLENGTHTOOLONG, forKey: .aDIMAGE9LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE9LINKREQUIRED, forKey: .aDIMAGE9LINKREQUIRED)
        try container.encodeIfPresent(aDIMAGE9LINKWARNING, forKey: .aDIMAGE9LINKWARNING)
        try container.encodeIfPresent(aDIMAGE9TAGDUPLICATED, forKey: .aDIMAGE9TAGDUPLICATED)
        try container.encodeIfPresent(aDIMAGE9TAGLENGTHTOOLONG, forKey: .aDIMAGE9TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDIMAGE9TAGREQUIRED, forKey: .aDIMAGE9TAGREQUIRED)
        try container.encodeIfPresent(AD_LINK_FORMAT_WARNING, forKey: .AD_LINK_FORMAT_WARNING)
        try container.encodeIfPresent(AD_LINK_SAME_AS_LINK, forKey: .AD_LINK_SAME_AS_LINK)
        try container.encodeIfPresent(aDVIDEO0LINKDUPLICATED, forKey: .aDVIDEO0LINKDUPLICATED)
        try container.encodeIfPresent(aDVIDEO0LINKLENGTHTOOLONG, forKey: .aDVIDEO0LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO0LINKREQUIRED, forKey: .aDVIDEO0LINKREQUIRED)
        try container.encodeIfPresent(aDVIDEO0LINKWARNING, forKey: .aDVIDEO0LINKWARNING)
        try container.encodeIfPresent(aDVIDEO0TAGDUPLICATED, forKey: .aDVIDEO0TAGDUPLICATED)
        try container.encodeIfPresent(aDVIDEO0TAGLENGTHTOOLONG, forKey: .aDVIDEO0TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO0TAGREQUIRED, forKey: .aDVIDEO0TAGREQUIRED)
        try container.encodeIfPresent(aDVIDEO1LINKDUPLICATED, forKey: .aDVIDEO1LINKDUPLICATED)
        try container.encodeIfPresent(aDVIDEO1LINKLENGTHTOOLONG, forKey: .aDVIDEO1LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO1LINKREQUIRED, forKey: .aDVIDEO1LINKREQUIRED)
        try container.encodeIfPresent(aDVIDEO1LINKWARNING, forKey: .aDVIDEO1LINKWARNING)
        try container.encodeIfPresent(aDVIDEO1TAGDUPLICATED, forKey: .aDVIDEO1TAGDUPLICATED)
        try container.encodeIfPresent(aDVIDEO1TAGLENGTHTOOLONG, forKey: .aDVIDEO1TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO1TAGREQUIRED, forKey: .aDVIDEO1TAGREQUIRED)
        try container.encodeIfPresent(aDVIDEO2LINKDUPLICATED, forKey: .aDVIDEO2LINKDUPLICATED)
        try container.encodeIfPresent(aDVIDEO2LINKLENGTHTOOLONG, forKey: .aDVIDEO2LINKLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO2LINKREQUIRED, forKey: .aDVIDEO2LINKREQUIRED)
        try container.encodeIfPresent(aDVIDEO2LINKWARNING, forKey: .aDVIDEO2LINKWARNING)
        try container.encodeIfPresent(aDVIDEO2TAGDUPLICATED, forKey: .aDVIDEO2TAGDUPLICATED)
        try container.encodeIfPresent(aDVIDEO2TAGLENGTHTOOLONG, forKey: .aDVIDEO2TAGLENGTHTOOLONG)
        try container.encodeIfPresent(aDVIDEO2TAGREQUIRED, forKey: .aDVIDEO2TAGREQUIRED)
        try container.encodeIfPresent(ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG, forKey: .ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG)
        try container.encodeIfPresent(ADDITIONAL_IMAGE_LINK_WARNING, forKey: .ADDITIONAL_IMAGE_LINK_WARNING)
        try container.encodeIfPresent(ADWORDS_FORMAT_WARNING, forKey: .ADWORDS_FORMAT_WARNING)
        try container.encodeIfPresent(ADWORDS_SAME_AS_LINK, forKey: .ADWORDS_SAME_AS_LINK)
        try container.encodeIfPresent(AGE_GROUP_INVALID, forKey: .AGE_GROUP_INVALID)
        try container.encodeIfPresent(ANDROID_DEEP_LINK_INVALID, forKey: .ANDROID_DEEP_LINK_INVALID)
        try container.encodeIfPresent(AVAILABILITY_DATE_INVALID, forKey: .AVAILABILITY_DATE_INVALID)
        try container.encodeIfPresent(COUNTRY_DOES_NOT_MAP_TO_CURRENCY, forKey: .COUNTRY_DOES_NOT_MAP_TO_CURRENCY)
        try container.encodeIfPresent(CUSTOM_LABEL_LENGTH_TOO_LONG, forKey: .CUSTOM_LABEL_LENGTH_TOO_LONG)
        try container.encodeIfPresent(DESCRIPTION_LENGTH_TOO_LONG, forKey: .DESCRIPTION_LENGTH_TOO_LONG)
        try container.encodeIfPresent(DUPLICATE_HEADERS, forKey: .DUPLICATE_HEADERS)
        try container.encodeIfPresent(EXPIRATION_DATE_INVALID, forKey: .EXPIRATION_DATE_INVALID)
        try container.encodeIfPresent(FETCH_SAME_SIGNATURE, forKey: .FETCH_SAME_SIGNATURE)
        try container.encodeIfPresent(GENDER_INVALID, forKey: .GENDER_INVALID)
        try container.encodeIfPresent(GTIN_INVALID, forKey: .GTIN_INVALID)
        try container.encodeIfPresent(IMAGE_LINK_WARNING, forKey: .IMAGE_LINK_WARNING)
        try container.encodeIfPresent(INCONSISTENT_CURRENCY_VALUES, forKey: .INCONSISTENT_CURRENCY_VALUES)
        try container.encodeIfPresent(INDEXED_PRODUCT_COUNT_LARGE_DELTA, forKey: .INDEXED_PRODUCT_COUNT_LARGE_DELTA)
        try container.encodeIfPresent(IOS_DEEP_LINK_INVALID, forKey: .IOS_DEEP_LINK_INVALID)
        try container.encodeIfPresent(IS_BUNDLE_INVALID, forKey: .IS_BUNDLE_INVALID)
        try container.encodeIfPresent(ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE, forKey: .ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE)
        try container.encodeIfPresent(LINK_FORMAT_WARNING, forKey: .LINK_FORMAT_WARNING)
        try container.encodeIfPresent(MIN_AD_PRICE_INVALID, forKey: .MIN_AD_PRICE_INVALID)
        try container.encodeIfPresent(MPN_INVALID, forKey: .MPN_INVALID)
        try container.encodeIfPresent(MULTIPACK_INVALID, forKey: .MULTIPACK_INVALID)
        try container.encodeIfPresent(OPTIONAL_CONDITION_INVALID, forKey: .OPTIONAL_CONDITION_INVALID)
        try container.encodeIfPresent(OPTIONAL_CONDITION_MISSING, forKey: .OPTIONAL_CONDITION_MISSING)
        try container.encodeIfPresent(OPTIONAL_PRODUCT_CATEGORY_INVALID, forKey: .OPTIONAL_PRODUCT_CATEGORY_INVALID)
        try container.encodeIfPresent(OPTIONAL_PRODUCT_CATEGORY_MISSING, forKey: .OPTIONAL_PRODUCT_CATEGORY_MISSING)
        try container.encodeIfPresent(PRODUCT_CATEGORY_DEPTH_WARNING, forKey: .PRODUCT_CATEGORY_DEPTH_WARNING)
        try container.encodeIfPresent(PRODUCT_TYPE_LENGTH_TOO_LONG, forKey: .PRODUCT_TYPE_LENGTH_TOO_LONG)
        try container.encodeIfPresent(SALE_DATE_INVALID, forKey: .SALE_DATE_INVALID)
        try container.encodeIfPresent(SALES_PRICE_INVALID, forKey: .SALES_PRICE_INVALID)
        try container.encodeIfPresent(SALES_PRICE_TOO_HIGH, forKey: .SALES_PRICE_TOO_HIGH)
        try container.encodeIfPresent(SALES_PRICE_TOO_LOW, forKey: .SALES_PRICE_TOO_LOW)
        try container.encodeIfPresent(SHIPPING_HEIGHT_INVALID, forKey: .SHIPPING_HEIGHT_INVALID)
        try container.encodeIfPresent(SHIPPING_INVALID, forKey: .SHIPPING_INVALID)
        try container.encodeIfPresent(SHIPPING_WEIGHT_INVALID, forKey: .SHIPPING_WEIGHT_INVALID)
        try container.encodeIfPresent(SHIPPING_WIDTH_INVALID, forKey: .SHIPPING_WIDTH_INVALID)
        try container.encodeIfPresent(SIZE_SYSTEM_INVALID, forKey: .SIZE_SYSTEM_INVALID)
        try container.encodeIfPresent(SIZE_TYPE_INVALID, forKey: .SIZE_TYPE_INVALID)
        try container.encodeIfPresent(TAX_INVALID, forKey: .TAX_INVALID)
        try container.encodeIfPresent(TITLE_LENGTH_TOO_LONG, forKey: .TITLE_LENGTH_TOO_LONG)
        try container.encodeIfPresent(TOO_MANY_ADDITIONAL_IMAGE_LINKS, forKey: .TOO_MANY_ADDITIONAL_IMAGE_LINKS)
        try container.encodeIfPresent(UPDATED_TIME_INVALID, forKey: .UPDATED_TIME_INVALID)
        try container.encodeIfPresent(UTM_SOURCE_AUTO_CORRECTED, forKey: .UTM_SOURCE_AUTO_CORRECTED)
        try container.encodeIfPresent(VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED, forKey: .VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED)
        try container.encodeIfPresent(WEIGHT_UNIT_INVALID, forKey: .WEIGHT_UNIT_INVALID)
    }
}
