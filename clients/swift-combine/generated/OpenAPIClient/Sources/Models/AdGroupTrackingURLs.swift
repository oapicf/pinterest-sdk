//
// AdGroupTrackingURLs.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

///   Third-party tracking URLs. Up to three tracking URLs - with a max length of 2,000 - are supported for   each event type. Tracking URLs set at the ad group or ad level can override   those set at the campaign level. For more information, see [Third-party and dynamic tracking](https://help.pinterest.com/en/business/article/third-party-and-dynamic-tracking). 
public struct AdGroupTrackingURLs: Codable {
    public var audienceVerification: [String]?
    public var buyableButton: [String]?
    public var click: [String]?
    public var engagement: [String]?
    public var impression: [String]?

    public init(audienceVerification: [String]? = nil, buyableButton: [String]? = nil, click: [String]? = nil, engagement: [String]? = nil, impression: [String]? = nil) {
        self.audienceVerification = audienceVerification
        self.buyableButton = buyableButton
        self.click = click
        self.engagement = engagement
        self.impression = impression
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case audienceVerification = "audience_verification"
        case buyableButton = "buyable_button"
        case click
        case engagement
        case impression
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        audienceVerification = try container.decodeIfPresent([String].self, forKey: .audienceVerification)
        buyableButton = try container.decodeIfPresent([String].self, forKey: .buyableButton)
        click = try container.decodeIfPresent([String].self, forKey: .click)
        engagement = try container.decodeIfPresent([String].self, forKey: .engagement)
        impression = try container.decodeIfPresent([String].self, forKey: .impression)
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(audienceVerification, forKey: .audienceVerification)
        try container.encodeIfPresent(buyableButton, forKey: .buyableButton)
        try container.encodeIfPresent(click, forKey: .click)
        try container.encodeIfPresent(engagement, forKey: .engagement)
        try container.encodeIfPresent(impression, forKey: .impression)
    }
}
