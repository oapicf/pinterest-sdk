//
// TopVideoPinsAnalyticsResponsePinsItems.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/// Array with metrics, status, and pin id for the requested metric 
public struct TopVideoPinsAnalyticsResponsePinsItems: Codable {
    public var dataStatus: [String: DataStatus]?
    public var metrics: [String: Double]?
    /// The pin id
    public var pinId: String?

    public init(dataStatus: [String: DataStatus]? = nil, metrics: [String: Double]? = nil, pinId: String? = nil) {
        self.dataStatus = dataStatus
        self.metrics = metrics
        self.pinId = pinId
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case dataStatus = "data_status"
        case metrics
        case pinId = "pin_id"
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        dataStatus = try container.decodeIfPresent([String: DataStatus].self, forKey: .dataStatus)
        metrics = try container.decodeIfPresent([String: Double].self, forKey: .metrics)
        pinId = try container.decodeIfPresent(String.self, forKey: .pinId)
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(dataStatus, forKey: .dataStatus)
        try container.encodeIfPresent(metrics, forKey: .metrics)
        try container.encodeIfPresent(pinId, forKey: .pinId)
    }
}
