...
> `trends_editorial_articles_list`(req::`HTTP.Request`, `region`::`ProductCategoryRegion`;) -> `Vector{TrendsEditorial}`
...
> `trends_featured_topics_list`(req::`HTTP.Request`, `region`::`ProductCategoryRegion`; `interest`=nothing,) -> `Vector{FeaturedTrend}`
...
> `trends_product_categories_details_list`(req::`HTTP.Request`, `product_categories`::`Vector{ProductCategoryEnum}`, `region`::`ProductCategoryRegion`; `lookback_window`=nothing, `engagement_type`=nothing,) -> `Vector{ProductCategoryDetails}`
...
> `trends_product_categories_trending_list`(req::`HTTP.Request`, `region`::`ProductCategoryRegion`; `verticals`=nothing, `ages`=nothing, `genders`=nothing, `engagement_type`=nothing,) -> `Vector{TrendingProductCategory}`
