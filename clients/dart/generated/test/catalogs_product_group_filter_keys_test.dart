//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CatalogsProductGroupFilterKeys
void main() {
  // final instance = CatalogsProductGroupFilterKeys();

  group('test CatalogsProductGroupFilterKeys', () {
    // CatalogsProductGroupPricingCriteria MIN_PRICE
    test('to test the property `MIN_PRICE`', () async {
      // TODO
    });

    // CatalogsProductGroupPricingCriteria MAX_PRICE
    test('to test the property `MAX_PRICE`', () async {
      // TODO
    });

    // CatalogsProductGroupCurrencyCriteria CURRENCY
    test('to test the property `CURRENCY`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria ITEM_ID
    test('to test the property `ITEM_ID`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria AVAILABILITY
    test('to test the property `AVAILABILITY`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria BRAND
    test('to test the property `BRAND`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria CONDITION
    test('to test the property `CONDITION`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria cUSTOMLABEL0
    test('to test the property `cUSTOMLABEL0`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria cUSTOMLABEL1
    test('to test the property `cUSTOMLABEL1`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria cUSTOMLABEL2
    test('to test the property `cUSTOMLABEL2`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria cUSTOMLABEL3
    test('to test the property `cUSTOMLABEL3`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria cUSTOMLABEL4
    test('to test the property `cUSTOMLABEL4`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria ITEM_GROUP_ID
    test('to test the property `ITEM_GROUP_ID`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleGenderCriteria GENDER
    test('to test the property `GENDER`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleMediaTypesCriteria MEDIA_TYPE
    test('to test the property `MEDIA_TYPE`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria pRODUCTTYPE4
    test('to test the property `pRODUCTTYPE4`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria pRODUCTTYPE3
    test('to test the property `pRODUCTTYPE3`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria pRODUCTTYPE2
    test('to test the property `pRODUCTTYPE2`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria pRODUCTTYPE1
    test('to test the property `pRODUCTTYPE1`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria pRODUCTTYPE0
    test('to test the property `pRODUCTTYPE0`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY6
    test('to test the property `gOOGLEPRODUCTCATEGORY6`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY5
    test('to test the property `gOOGLEPRODUCTCATEGORY5`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY4
    test('to test the property `gOOGLEPRODUCTCATEGORY4`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY3
    test('to test the property `gOOGLEPRODUCTCATEGORY3`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY2
    test('to test the property `gOOGLEPRODUCTCATEGORY2`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY1
    test('to test the property `gOOGLEPRODUCTCATEGORY1`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringListCriteria gOOGLEPRODUCTCATEGORY0
    test('to test the property `gOOGLEPRODUCTCATEGORY0`', () async {
      // TODO
    });

    // CatalogsProductGroupMultipleStringCriteria PRODUCT_GROUP
    test('to test the property `PRODUCT_GROUP`', () async {
      // TODO
    });


  });

}
