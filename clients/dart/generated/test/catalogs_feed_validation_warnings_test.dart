//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CatalogsFeedValidationWarnings
void main() {
  // final instance = CatalogsFeedValidationWarnings();

  group('test CatalogsFeedValidationWarnings', () {
    // ad_image_0_link is duplicated with another ad image link.
    // int aDIMAGE0LINKDUPLICATED
    test('to test the property `aDIMAGE0LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 0 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE0LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE0LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 0 is required because an image tag was provided.
    // int aDIMAGE0LINKREQUIRED
    test('to test the property `aDIMAGE0LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 0 format is unsupported.
    // int aDIMAGE0LINKWARNING
    test('to test the property `aDIMAGE0LINKWARNING`', () async {
      // TODO
    });

    // ad_image_0_tag is duplicated with another ad image tag.
    // int aDIMAGE0TAGDUPLICATED
    test('to test the property `aDIMAGE0TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 0 length is too long. The maximum length is 511 characters.
    // int aDIMAGE0TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE0TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 0 is required because an image link was provided.
    // int aDIMAGE0TAGREQUIRED
    test('to test the property `aDIMAGE0TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_10_link is duplicated with another ad image link.
    // int aDIMAGE10LINKDUPLICATED
    test('to test the property `aDIMAGE10LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 10 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE10LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE10LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 10 is required because an image tag was provided.
    // int aDIMAGE10LINKREQUIRED
    test('to test the property `aDIMAGE10LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 10 format is unsupported.
    // int aDIMAGE10LINKWARNING
    test('to test the property `aDIMAGE10LINKWARNING`', () async {
      // TODO
    });

    // ad_image_10_tag is duplicated with another ad image tag.
    // int aDIMAGE10TAGDUPLICATED
    test('to test the property `aDIMAGE10TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 10 length is too long. The maximum length is 511 characters.
    // int aDIMAGE10TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE10TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 10 is required because an image link was provided.
    // int aDIMAGE10TAGREQUIRED
    test('to test the property `aDIMAGE10TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_11_link is duplicated with another ad image link.
    // int aDIMAGE11LINKDUPLICATED
    test('to test the property `aDIMAGE11LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 11 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE11LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE11LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 11 is required because an image tag was provided.
    // int aDIMAGE11LINKREQUIRED
    test('to test the property `aDIMAGE11LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 11 format is unsupported.
    // int aDIMAGE11LINKWARNING
    test('to test the property `aDIMAGE11LINKWARNING`', () async {
      // TODO
    });

    // ad_image_11_tag is duplicated with another ad image tag.
    // int aDIMAGE11TAGDUPLICATED
    test('to test the property `aDIMAGE11TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 11 length is too long. The maximum length is 511 characters.
    // int aDIMAGE11TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE11TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 11 is required because an image link was provided.
    // int aDIMAGE11TAGREQUIRED
    test('to test the property `aDIMAGE11TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_12_link is duplicated with another ad image link.
    // int aDIMAGE12LINKDUPLICATED
    test('to test the property `aDIMAGE12LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 12 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE12LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE12LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 12 is required because an image tag was provided.
    // int aDIMAGE12LINKREQUIRED
    test('to test the property `aDIMAGE12LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 12 format is unsupported.
    // int aDIMAGE12LINKWARNING
    test('to test the property `aDIMAGE12LINKWARNING`', () async {
      // TODO
    });

    // ad_image_12_tag is duplicated with another ad image tag.
    // int aDIMAGE12TAGDUPLICATED
    test('to test the property `aDIMAGE12TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 12 length is too long. The maximum length is 511 characters.
    // int aDIMAGE12TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE12TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 12 is required because an image link was provided.
    // int aDIMAGE12TAGREQUIRED
    test('to test the property `aDIMAGE12TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_13_link is duplicated with another ad image link.
    // int aDIMAGE13LINKDUPLICATED
    test('to test the property `aDIMAGE13LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 13 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE13LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE13LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 13 is required because an image tag was provided.
    // int aDIMAGE13LINKREQUIRED
    test('to test the property `aDIMAGE13LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 13 format is unsupported.
    // int aDIMAGE13LINKWARNING
    test('to test the property `aDIMAGE13LINKWARNING`', () async {
      // TODO
    });

    // ad_image_13_tag is duplicated with another ad image tag.
    // int aDIMAGE13TAGDUPLICATED
    test('to test the property `aDIMAGE13TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 13 length is too long. The maximum length is 511 characters.
    // int aDIMAGE13TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE13TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 13 is required because an image link was provided.
    // int aDIMAGE13TAGREQUIRED
    test('to test the property `aDIMAGE13TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_14_link is duplicated with another ad image link.
    // int aDIMAGE14LINKDUPLICATED
    test('to test the property `aDIMAGE14LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 14 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE14LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE14LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 14 is required because an image tag was provided.
    // int aDIMAGE14LINKREQUIRED
    test('to test the property `aDIMAGE14LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 14 format is unsupported.
    // int aDIMAGE14LINKWARNING
    test('to test the property `aDIMAGE14LINKWARNING`', () async {
      // TODO
    });

    // ad_image_14_tag is duplicated with another ad image tag.
    // int aDIMAGE14TAGDUPLICATED
    test('to test the property `aDIMAGE14TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 14 length is too long. The maximum length is 511 characters.
    // int aDIMAGE14TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE14TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 14 is required because an image link was provided.
    // int aDIMAGE14TAGREQUIRED
    test('to test the property `aDIMAGE14TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_15_link is duplicated with another ad image link.
    // int aDIMAGE15LINKDUPLICATED
    test('to test the property `aDIMAGE15LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 15 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE15LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE15LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 15 is required because an image tag was provided.
    // int aDIMAGE15LINKREQUIRED
    test('to test the property `aDIMAGE15LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 15 format is unsupported.
    // int aDIMAGE15LINKWARNING
    test('to test the property `aDIMAGE15LINKWARNING`', () async {
      // TODO
    });

    // ad_image_15_tag is duplicated with another ad image tag.
    // int aDIMAGE15TAGDUPLICATED
    test('to test the property `aDIMAGE15TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 15 length is too long. The maximum length is 511 characters.
    // int aDIMAGE15TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE15TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 15 is required because an image link was provided.
    // int aDIMAGE15TAGREQUIRED
    test('to test the property `aDIMAGE15TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_16_link is duplicated with another ad image link.
    // int aDIMAGE16LINKDUPLICATED
    test('to test the property `aDIMAGE16LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 16 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE16LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE16LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 16 is required because an image tag was provided.
    // int aDIMAGE16LINKREQUIRED
    test('to test the property `aDIMAGE16LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 16 format is unsupported.
    // int aDIMAGE16LINKWARNING
    test('to test the property `aDIMAGE16LINKWARNING`', () async {
      // TODO
    });

    // ad_image_16_tag is duplicated with another ad image tag.
    // int aDIMAGE16TAGDUPLICATED
    test('to test the property `aDIMAGE16TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 16 length is too long. The maximum length is 511 characters.
    // int aDIMAGE16TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE16TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 16 is required because an image link was provided.
    // int aDIMAGE16TAGREQUIRED
    test('to test the property `aDIMAGE16TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_17_link is duplicated with another ad image link.
    // int aDIMAGE17LINKDUPLICATED
    test('to test the property `aDIMAGE17LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 17 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE17LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE17LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 17 is required because an image tag was provided.
    // int aDIMAGE17LINKREQUIRED
    test('to test the property `aDIMAGE17LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 17 format is unsupported.
    // int aDIMAGE17LINKWARNING
    test('to test the property `aDIMAGE17LINKWARNING`', () async {
      // TODO
    });

    // ad_image_17_tag is duplicated with another ad image tag.
    // int aDIMAGE17TAGDUPLICATED
    test('to test the property `aDIMAGE17TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 17 length is too long. The maximum length is 511 characters.
    // int aDIMAGE17TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE17TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 17 is required because an image link was provided.
    // int aDIMAGE17TAGREQUIRED
    test('to test the property `aDIMAGE17TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_18_link is duplicated with another ad image link.
    // int aDIMAGE18LINKDUPLICATED
    test('to test the property `aDIMAGE18LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 18 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE18LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE18LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 18 is required because an image tag was provided.
    // int aDIMAGE18LINKREQUIRED
    test('to test the property `aDIMAGE18LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 18 format is unsupported.
    // int aDIMAGE18LINKWARNING
    test('to test the property `aDIMAGE18LINKWARNING`', () async {
      // TODO
    });

    // ad_image_18_tag is duplicated with another ad image tag.
    // int aDIMAGE18TAGDUPLICATED
    test('to test the property `aDIMAGE18TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 18 length is too long. The maximum length is 511 characters.
    // int aDIMAGE18TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE18TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 18 is required because an image link was provided.
    // int aDIMAGE18TAGREQUIRED
    test('to test the property `aDIMAGE18TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_19_link is duplicated with another ad image link.
    // int aDIMAGE19LINKDUPLICATED
    test('to test the property `aDIMAGE19LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 19 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE19LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE19LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 19 is required because an image tag was provided.
    // int aDIMAGE19LINKREQUIRED
    test('to test the property `aDIMAGE19LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 19 format is unsupported.
    // int aDIMAGE19LINKWARNING
    test('to test the property `aDIMAGE19LINKWARNING`', () async {
      // TODO
    });

    // ad_image_19_tag is duplicated with another ad image tag.
    // int aDIMAGE19TAGDUPLICATED
    test('to test the property `aDIMAGE19TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 19 length is too long. The maximum length is 511 characters.
    // int aDIMAGE19TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE19TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 19 is required because an image link was provided.
    // int aDIMAGE19TAGREQUIRED
    test('to test the property `aDIMAGE19TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_1_link is duplicated with another ad image link.
    // int aDIMAGE1LINKDUPLICATED
    test('to test the property `aDIMAGE1LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 1 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE1LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE1LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 1 is required because an image tag was provided.
    // int aDIMAGE1LINKREQUIRED
    test('to test the property `aDIMAGE1LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 1 format is unsupported.
    // int aDIMAGE1LINKWARNING
    test('to test the property `aDIMAGE1LINKWARNING`', () async {
      // TODO
    });

    // ad_image_1_tag is duplicated with another ad image tag.
    // int aDIMAGE1TAGDUPLICATED
    test('to test the property `aDIMAGE1TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 1 length is too long. The maximum length is 511 characters.
    // int aDIMAGE1TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE1TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 1 is required because an image link was provided.
    // int aDIMAGE1TAGREQUIRED
    test('to test the property `aDIMAGE1TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_2_link is duplicated with another ad image link.
    // int aDIMAGE2LINKDUPLICATED
    test('to test the property `aDIMAGE2LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 2 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE2LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE2LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 2 is required because an image tag was provided.
    // int aDIMAGE2LINKREQUIRED
    test('to test the property `aDIMAGE2LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 2 format is unsupported.
    // int aDIMAGE2LINKWARNING
    test('to test the property `aDIMAGE2LINKWARNING`', () async {
      // TODO
    });

    // ad_image_2_tag is duplicated with another ad image tag.
    // int aDIMAGE2TAGDUPLICATED
    test('to test the property `aDIMAGE2TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 2 length is too long. The maximum length is 511 characters.
    // int aDIMAGE2TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE2TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 2 is required because an image link was provided.
    // int aDIMAGE2TAGREQUIRED
    test('to test the property `aDIMAGE2TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_3_link is duplicated with another ad image link.
    // int aDIMAGE3LINKDUPLICATED
    test('to test the property `aDIMAGE3LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 3 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE3LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE3LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 3 is required because an image tag was provided.
    // int aDIMAGE3LINKREQUIRED
    test('to test the property `aDIMAGE3LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 3 format is unsupported.
    // int aDIMAGE3LINKWARNING
    test('to test the property `aDIMAGE3LINKWARNING`', () async {
      // TODO
    });

    // ad_image_3_tag is duplicated with another ad image tag.
    // int aDIMAGE3TAGDUPLICATED
    test('to test the property `aDIMAGE3TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 3 length is too long. The maximum length is 511 characters.
    // int aDIMAGE3TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE3TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 3 is required because an image link was provided.
    // int aDIMAGE3TAGREQUIRED
    test('to test the property `aDIMAGE3TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_4_link is duplicated with another ad image link.
    // int aDIMAGE4LINKDUPLICATED
    test('to test the property `aDIMAGE4LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 4 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE4LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE4LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 4 is required because an image tag was provided.
    // int aDIMAGE4LINKREQUIRED
    test('to test the property `aDIMAGE4LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 4 format is unsupported.
    // int aDIMAGE4LINKWARNING
    test('to test the property `aDIMAGE4LINKWARNING`', () async {
      // TODO
    });

    // ad_image_4_tag is duplicated with another ad image tag.
    // int aDIMAGE4TAGDUPLICATED
    test('to test the property `aDIMAGE4TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 4 length is too long. The maximum length is 511 characters.
    // int aDIMAGE4TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE4TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 4 is required because an image link was provided.
    // int aDIMAGE4TAGREQUIRED
    test('to test the property `aDIMAGE4TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_5_link is duplicated with another ad image link.
    // int aDIMAGE5LINKDUPLICATED
    test('to test the property `aDIMAGE5LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 5 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE5LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE5LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 5 is required because an image tag was provided.
    // int aDIMAGE5LINKREQUIRED
    test('to test the property `aDIMAGE5LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 5 format is unsupported.
    // int aDIMAGE5LINKWARNING
    test('to test the property `aDIMAGE5LINKWARNING`', () async {
      // TODO
    });

    // ad_image_5_tag is duplicated with another ad image tag.
    // int aDIMAGE5TAGDUPLICATED
    test('to test the property `aDIMAGE5TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 5 length is too long. The maximum length is 511 characters.
    // int aDIMAGE5TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE5TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 5 is required because an image link was provided.
    // int aDIMAGE5TAGREQUIRED
    test('to test the property `aDIMAGE5TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_6_link is duplicated with another ad image link.
    // int aDIMAGE6LINKDUPLICATED
    test('to test the property `aDIMAGE6LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 6 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE6LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE6LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 6 is required because an image tag was provided.
    // int aDIMAGE6LINKREQUIRED
    test('to test the property `aDIMAGE6LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 6 format is unsupported.
    // int aDIMAGE6LINKWARNING
    test('to test the property `aDIMAGE6LINKWARNING`', () async {
      // TODO
    });

    // ad_image_6_tag is duplicated with another ad image tag.
    // int aDIMAGE6TAGDUPLICATED
    test('to test the property `aDIMAGE6TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 6 length is too long. The maximum length is 511 characters.
    // int aDIMAGE6TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE6TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 6 is required because an image link was provided.
    // int aDIMAGE6TAGREQUIRED
    test('to test the property `aDIMAGE6TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_7_link is duplicated with another ad image link.
    // int aDIMAGE7LINKDUPLICATED
    test('to test the property `aDIMAGE7LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 7 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE7LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE7LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 7 is required because an image tag was provided.
    // int aDIMAGE7LINKREQUIRED
    test('to test the property `aDIMAGE7LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 7 format is unsupported.
    // int aDIMAGE7LINKWARNING
    test('to test the property `aDIMAGE7LINKWARNING`', () async {
      // TODO
    });

    // ad_image_7_tag is duplicated with another ad image tag.
    // int aDIMAGE7TAGDUPLICATED
    test('to test the property `aDIMAGE7TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 7 length is too long. The maximum length is 511 characters.
    // int aDIMAGE7TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE7TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 7 is required because an image link was provided.
    // int aDIMAGE7TAGREQUIRED
    test('to test the property `aDIMAGE7TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_8_link is duplicated with another ad image link.
    // int aDIMAGE8LINKDUPLICATED
    test('to test the property `aDIMAGE8LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 8 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE8LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE8LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 8 is required because an image tag was provided.
    // int aDIMAGE8LINKREQUIRED
    test('to test the property `aDIMAGE8LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 8 format is unsupported.
    // int aDIMAGE8LINKWARNING
    test('to test the property `aDIMAGE8LINKWARNING`', () async {
      // TODO
    });

    // ad_image_8_tag is duplicated with another ad image tag.
    // int aDIMAGE8TAGDUPLICATED
    test('to test the property `aDIMAGE8TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 8 length is too long. The maximum length is 511 characters.
    // int aDIMAGE8TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE8TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 8 is required because an image link was provided.
    // int aDIMAGE8TAGREQUIRED
    test('to test the property `aDIMAGE8TAGREQUIRED`', () async {
      // TODO
    });

    // ad_image_9_link is duplicated with another ad image link.
    // int aDIMAGE9LINKDUPLICATED
    test('to test the property `aDIMAGE9LINKDUPLICATED`', () async {
      // TODO
    });

    // Ad image link 9 length is too long. The maximum length is 2047 characters.
    // int aDIMAGE9LINKLENGTHTOOLONG
    test('to test the property `aDIMAGE9LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image link 9 is required because an image tag was provided.
    // int aDIMAGE9LINKREQUIRED
    test('to test the property `aDIMAGE9LINKREQUIRED`', () async {
      // TODO
    });

    // Ad image link 9 format is unsupported.
    // int aDIMAGE9LINKWARNING
    test('to test the property `aDIMAGE9LINKWARNING`', () async {
      // TODO
    });

    // ad_image_9_tag is duplicated with another ad image tag.
    // int aDIMAGE9TAGDUPLICATED
    test('to test the property `aDIMAGE9TAGDUPLICATED`', () async {
      // TODO
    });

    // Ad image tag 9 length is too long. The maximum length is 511 characters.
    // int aDIMAGE9TAGLENGTHTOOLONG
    test('to test the property `aDIMAGE9TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // Ad image tag 9 is required because an image link was provided.
    // int aDIMAGE9TAGREQUIRED
    test('to test the property `aDIMAGE9TAGREQUIRED`', () async {
      // TODO
    });

    // Some items have ad links that are formatted incorrectly.
    // int AD_LINK_FORMAT_WARNING
    test('to test the property `AD_LINK_FORMAT_WARNING`', () async {
      // TODO
    });

    // Some items have ad link URLs that are duplicates of the link URLs for those items.
    // int AD_LINK_SAME_AS_LINK
    test('to test the property `AD_LINK_SAME_AS_LINK`', () async {
      // TODO
    });

    // ad_video_0_link is duplicated with another ad video link.
    // int aDVIDEO0LINKDUPLICATED
    test('to test the property `aDVIDEO0LINKDUPLICATED`', () async {
      // TODO
    });

    // ad_video_0_link length is too long. The maximum length is 511 characters.
    // int aDVIDEO0LINKLENGTHTOOLONG
    test('to test the property `aDVIDEO0LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_0_link is required for this item because ad_video_0_tag was provided.
    // int aDVIDEO0LINKREQUIRED
    test('to test the property `aDVIDEO0LINKREQUIRED`', () async {
      // TODO
    });

    // ad_video_0_link is formatted incorrectly and will not be published with your items.
    // int aDVIDEO0LINKWARNING
    test('to test the property `aDVIDEO0LINKWARNING`', () async {
      // TODO
    });

    // ad_video_0_tag is duplicated with another ad video tag.
    // int aDVIDEO0TAGDUPLICATED
    test('to test the property `aDVIDEO0TAGDUPLICATED`', () async {
      // TODO
    });

    // ad_video_0_tag length is too long. The maximum length is 511 characters.
    // int aDVIDEO0TAGLENGTHTOOLONG
    test('to test the property `aDVIDEO0TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_0_tag is required because ad_video_0_link was provided.
    // int aDVIDEO0TAGREQUIRED
    test('to test the property `aDVIDEO0TAGREQUIRED`', () async {
      // TODO
    });

    // ad_video_1_link is duplicated with another ad video link.
    // int aDVIDEO1LINKDUPLICATED
    test('to test the property `aDVIDEO1LINKDUPLICATED`', () async {
      // TODO
    });

    // ad_video_1_link length is too long. The maximum length is 511 characters.
    // int aDVIDEO1LINKLENGTHTOOLONG
    test('to test the property `aDVIDEO1LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_1_link is required for this item because ad_video_1_tag was provided.
    // int aDVIDEO1LINKREQUIRED
    test('to test the property `aDVIDEO1LINKREQUIRED`', () async {
      // TODO
    });

    // ad_video_1_link is formatted incorrectly and will not be published with your items.
    // int aDVIDEO1LINKWARNING
    test('to test the property `aDVIDEO1LINKWARNING`', () async {
      // TODO
    });

    // ad_video_1_tag is duplicated with another ad video tag.
    // int aDVIDEO1TAGDUPLICATED
    test('to test the property `aDVIDEO1TAGDUPLICATED`', () async {
      // TODO
    });

    // ad_video_1_tag length is too long. The maximum length is 511 characters.
    // int aDVIDEO1TAGLENGTHTOOLONG
    test('to test the property `aDVIDEO1TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_1_tag is required because ad_video_1_link was provided.
    // int aDVIDEO1TAGREQUIRED
    test('to test the property `aDVIDEO1TAGREQUIRED`', () async {
      // TODO
    });

    // ad_video_2_link is duplicated with another ad video link.
    // int aDVIDEO2LINKDUPLICATED
    test('to test the property `aDVIDEO2LINKDUPLICATED`', () async {
      // TODO
    });

    // ad_video_2_link length is too long. The maximum length is 511 characters.
    // int aDVIDEO2LINKLENGTHTOOLONG
    test('to test the property `aDVIDEO2LINKLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_2_link is required for this item because ad_video_2_tag was provided.
    // int aDVIDEO2LINKREQUIRED
    test('to test the property `aDVIDEO2LINKREQUIRED`', () async {
      // TODO
    });

    // ad_video_2_link is formatted incorrectly and will not be published with your items.
    // int aDVIDEO2LINKWARNING
    test('to test the property `aDVIDEO2LINKWARNING`', () async {
      // TODO
    });

    // ad_video_2_tag is duplicated with another ad video tag.
    // int aDVIDEO2TAGDUPLICATED
    test('to test the property `aDVIDEO2TAGDUPLICATED`', () async {
      // TODO
    });

    // ad_video_2_tag length is too long. The maximum length is 511 characters.
    // int aDVIDEO2TAGLENGTHTOOLONG
    test('to test the property `aDVIDEO2TAGLENGTHTOOLONG`', () async {
      // TODO
    });

    // ad_video_2_tag is required because ad_video_2_link was provided.
    // int aDVIDEO2TAGREQUIRED
    test('to test the property `aDVIDEO2TAGREQUIRED`', () async {
      // TODO
    });

    // Some items have additional_image_link URLs that contain too many characters, so those items will not be published.
    // int ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG
    test('to test the property `ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG`', () async {
      // TODO
    });

    // Some items have additional_image_link URLs that are formatted incorrectly and will not be published with your items.
    // int ADDITIONAL_IMAGE_LINK_WARNING
    test('to test the property `ADDITIONAL_IMAGE_LINK_WARNING`', () async {
      // TODO
    });

    // Some items have adwords_redirect links that are formatted incorrectly.
    // int ADWORDS_FORMAT_WARNING
    test('to test the property `ADWORDS_FORMAT_WARNING`', () async {
      // TODO
    });

    // Some items have adwords_redirect URLs that are duplicates of the link URLs for those items.
    // int ADWORDS_SAME_AS_LINK
    test('to test the property `ADWORDS_SAME_AS_LINK`', () async {
      // TODO
    });

    // Some items have age group values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    // int AGE_GROUP_INVALID
    test('to test the property `AGE_GROUP_INVALID`', () async {
      // TODO
    });

    // Some items include invalid android_deep_link.
    // int ANDROID_DEEP_LINK_INVALID
    test('to test the property `ANDROID_DEEP_LINK_INVALID`', () async {
      // TODO
    });

    // Some items have availability_date values that are formatted incorrectly, those items will be published without an availability date.
    // int AVAILABILITY_DATE_INVALID
    test('to test the property `AVAILABILITY_DATE_INVALID`', () async {
      // TODO
    });

    // Some items include a currency that doesn't match the usual currency for the location where that product is sold or shipped.
    // int COUNTRY_DOES_NOT_MAP_TO_CURRENCY
    test('to test the property `COUNTRY_DOES_NOT_MAP_TO_CURRENCY`', () async {
      // TODO
    });

    // Some items have custom_label values that are too long, those items will be published without that custom label.
    // int CUSTOM_LABEL_LENGTH_TOO_LONG
    test('to test the property `CUSTOM_LABEL_LENGTH_TOO_LONG`', () async {
      // TODO
    });

    // The description for some items were truncated because they contain too many characters.
    // int DESCRIPTION_LENGTH_TOO_LONG
    test('to test the property `DESCRIPTION_LENGTH_TOO_LONG`', () async {
      // TODO
    });

    // Your feed contains duplicate headers.
    // int DUPLICATE_HEADERS
    test('to test the property `DUPLICATE_HEADERS`', () async {
      // TODO
    });

    // Some items have expiration_date values that are formatted incorrectly, those items will be published without an expiration date.
    // int EXPIRATION_DATE_INVALID
    test('to test the property `EXPIRATION_DATE_INVALID`', () async {
      // TODO
    });

    // Ingestion completed early because there are no changes to your feed since the last successful update.
    // int FETCH_SAME_SIGNATURE
    test('to test the property `FETCH_SAME_SIGNATURE`', () async {
      // TODO
    });

    // Some items have gender values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    // int GENDER_INVALID
    test('to test the property `GENDER_INVALID`', () async {
      // TODO
    });

    // Some items include incorrectly formatted GTINs.
    // int GTIN_INVALID
    test('to test the property `GTIN_INVALID`', () async {
      // TODO
    });

    // Some items have image_link URLs that are formatted incorrectly and will not be published with those items.
    // int IMAGE_LINK_WARNING
    test('to test the property `IMAGE_LINK_WARNING`', () async {
      // TODO
    });

    // Some items include inconsistent currencies in price fields.
    // int INCONSISTENT_CURRENCY_VALUES
    test('to test the property `INCONSISTENT_CURRENCY_VALUES`', () async {
      // TODO
    });

    // The product count has increased or decreased significantly compared to the last successful ingestion.
    // int INDEXED_PRODUCT_COUNT_LARGE_DELTA
    test('to test the property `INDEXED_PRODUCT_COUNT_LARGE_DELTA`', () async {
      // TODO
    });

    // Some items include invalid ios_deep_link values.
    // int IOS_DEEP_LINK_INVALID
    test('to test the property `IOS_DEEP_LINK_INVALID`', () async {
      // TODO
    });

    // Some items have is_bundle values that are formatted incorrectly, those items will be published without being bundled with other products.
    // int IS_BUNDLE_INVALID
    test('to test the property `IS_BUNDLE_INVALID`', () async {
      // TODO
    });

    // Some items include additional_image_links that can't be found.
    // int ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE
    test('to test the property `ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE`', () async {
      // TODO
    });

    // Some items have an invalid product link which contains invalid UTM tracking paramaters.
    // int LINK_FORMAT_WARNING
    test('to test the property `LINK_FORMAT_WARNING`', () async {
      // TODO
    });

    // Some items include min_ad_price values that are formatted incorrectly.
    // int MIN_AD_PRICE_INVALID
    test('to test the property `MIN_AD_PRICE_INVALID`', () async {
      // TODO
    });

    // Some items include incorrectly formatted MPNs.
    // int MPN_INVALID
    test('to test the property `MPN_INVALID`', () async {
      // TODO
    });

    // Some items have invalid multipack values.
    // int MULTIPACK_INVALID
    test('to test the property `MULTIPACK_INVALID`', () async {
      // TODO
    });

    // Some items include condition values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    // int OPTIONAL_CONDITION_INVALID
    test('to test the property `OPTIONAL_CONDITION_INVALID`', () async {
      // TODO
    });

    // Some items are missing a condition value, which may limit visibility in recommendations, search results and shopping experiences.
    // int OPTIONAL_CONDITION_MISSING
    test('to test the property `OPTIONAL_CONDITION_MISSING`', () async {
      // TODO
    });

    // Some items include google_product_category values that are not formatted correctly according to the GPC taxonomy.
    // int OPTIONAL_PRODUCT_CATEGORY_INVALID
    test('to test the property `OPTIONAL_PRODUCT_CATEGORY_INVALID`', () async {
      // TODO
    });

    // Some items are missing a google_product_category.
    // int OPTIONAL_PRODUCT_CATEGORY_MISSING
    test('to test the property `OPTIONAL_PRODUCT_CATEGORY_MISSING`', () async {
      // TODO
    });

    // Some items only have 1 or 2 levels of google_product_category values, which may limit visibility in recommendations, search results and shopping experiences.
    // int PRODUCT_CATEGORY_DEPTH_WARNING
    test('to test the property `PRODUCT_CATEGORY_DEPTH_WARNING`', () async {
      // TODO
    });

    // Some items have product_type values that are too long, those items will be published without that product type.
    // int PRODUCT_TYPE_LENGTH_TOO_LONG
    test('to test the property `PRODUCT_TYPE_LENGTH_TOO_LONG`', () async {
      // TODO
    });

    // Some items have sale_price_effective_date values that are formatted incorrectly, those items will be published without a sale date.
    // int SALE_DATE_INVALID
    test('to test the property `SALE_DATE_INVALID`', () async {
      // TODO
    });

    // Some items have sale price values that are higher than the original price of the item.
    // int SALES_PRICE_INVALID
    test('to test the property `SALES_PRICE_INVALID`', () async {
      // TODO
    });

    // Some items include a sales price that is higher than the list price. The sales price has been defaulted to the list price.
    // int SALES_PRICE_TOO_HIGH
    test('to test the property `SALES_PRICE_TOO_HIGH`', () async {
      // TODO
    });

    // Some items include sales price that is much lower than the list price.
    // int SALES_PRICE_TOO_LOW
    test('to test the property `SALES_PRICE_TOO_LOW`', () async {
      // TODO
    });

    // Some items include incorrectly formatted shipping_height.
    // int SHIPPING_HEIGHT_INVALID
    test('to test the property `SHIPPING_HEIGHT_INVALID`', () async {
      // TODO
    });

    // Some items have shipping values that are formatted incorrectly.
    // int SHIPPING_INVALID
    test('to test the property `SHIPPING_INVALID`', () async {
      // TODO
    });

    // Some items have invalid shipping_weight values.
    // int SHIPPING_WEIGHT_INVALID
    test('to test the property `SHIPPING_WEIGHT_INVALID`', () async {
      // TODO
    });

    // Some items include incorrectly formatted shipping_width.
    // int SHIPPING_WIDTH_INVALID
    test('to test the property `SHIPPING_WIDTH_INVALID`', () async {
      // TODO
    });

    // Some items have size system values which are not one of the supported size systems.
    // int SIZE_SYSTEM_INVALID
    test('to test the property `SIZE_SYSTEM_INVALID`', () async {
      // TODO
    });

    // Some items have size type values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.
    // int SIZE_TYPE_INVALID
    test('to test the property `SIZE_TYPE_INVALID`', () async {
      // TODO
    });

    // Some items have tax values that are formatted incorrectly.
    // int TAX_INVALID
    test('to test the property `TAX_INVALID`', () async {
      // TODO
    });

    // The title for some items were truncated because they contain too many characters.
    // int TITLE_LENGTH_TOO_LONG
    test('to test the property `TITLE_LENGTH_TOO_LONG`', () async {
      // TODO
    });

    // Some items have additional_image_link values that exceed the limit for additional images, those items will be published without some of your images.
    // int TOO_MANY_ADDITIONAL_IMAGE_LINKS
    test('to test the property `TOO_MANY_ADDITIONAL_IMAGE_LINKS`', () async {
      // TODO
    });

    // Some items have updated_time values thate are formatted incorrectly, those items will be published without an updated time.
    // int UPDATED_TIME_INVALID
    test('to test the property `UPDATED_TIME_INVALID`', () async {
      // TODO
    });

    // Some items include utm_source values that are formatted incorrectly and have been automatically corrected.
    // int UTM_SOURCE_AUTO_CORRECTED
    test('to test the property `UTM_SOURCE_AUTO_CORRECTED`', () async {
      // TODO
    });

    // A video is required in the item when ad_video fields are provided.
    // int VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED
    test('to test the property `VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED`', () async {
      // TODO
    });

    // Some items have weight_unit values that are formatted incorrectly, those items will be published without a weight unit.
    // int WEIGHT_UNIT_INVALID
    test('to test the property `WEIGHT_UNIT_INVALID`', () async {
      // TODO
    });


  });

}
