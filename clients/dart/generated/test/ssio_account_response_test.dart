//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for SSIOAccountResponse
void main() {
  // final instance = SSIOAccountResponse();

  group('test SSIOAccountResponse', () {
    // An array of Salesforce account information that includes address, io terms, etc.
    // List<SSIOAccountItem> billtoInfos (default value: const [])
    test('to test the property `billtoInfos`', () async {
      // TODO
    });

    // Advertiser eligible to update order lines
    // bool canEdit
    test('to test the property `canEdit`', () async {
      // TODO
    });

    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // Advertiser eligible to create order lines
    // bool eligible
    test('to test the property `eligible`', () async {
      // TODO
    });

    // Error indicator from Salesforce which could be \"No Error\"
    // String error
    test('to test the property `error`', () async {
      // TODO
    });

    // List<SSIOAccountPMPName> pmpNames (default value: const [])
    test('to test the property `pmpNames`', () async {
      // TODO
    });


  });

}
