//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CatalogsRetailItemsBatch
void main() {
  // final instance = CatalogsRetailItemsBatch();

  group('test CatalogsRetailItemsBatch', () {
    // Id of the catalogs items batch
    // String batchId
    test('to test the property `batchId`', () async {
      // TODO
    });

    // Time of the batch creation: YYYY-MM-DD'T'hh:mm:ssTZD
    // DateTime createdTime
    test('to test the property `createdTime`', () async {
      // TODO
    });

    // Time of the batch completion: YYYY-MM-DD'T'hh:mm:ssTZD
    // DateTime completedTime
    test('to test the property `completedTime`', () async {
      // TODO
    });

    // BatchOperationStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // CatalogsType catalogType
    test('to test the property `catalogType`', () async {
      // TODO
    });

    // Array with the catalogs items processing records part of the catalogs items batch
    // List<ItemProcessingRecord> items (default value: const [])
    test('to test the property `items`', () async {
      // TODO
    });


  });

}
