//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CustomerListUploadResponse
void main() {
  // final instance = CustomerListUploadResponse();

  group('test CustomerListUploadResponse', () {
    // Advertiser ID.
    // String adAccountId
    test('to test the property `adAccountId`', () async {
      // TODO
    });

    // Customer List Upload creation_time. Epoch (seconds).
    // int creationTime
    test('to test the property `creationTime`', () async {
      // TODO
    });

    // ID of the customer list associated with this upload.
    // String customerListId
    test('to test the property `customerListId`', () async {
      // TODO
    });

    // Error counts by error code
    // List<ErrorDetail> errorCounts (default value: const [])
    test('to test the property `errorCounts`', () async {
      // TODO
    });

    // Customer List Upload ID.
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // UserListOperationType operation
    test('to test the property `operation`', () async {
      // TODO
    });

    // RecordCounts recordCounts
    test('to test the property `recordCounts`', () async {
      // TODO
    });

    // Workload processing state
    // String state
    test('to test the property `state`', () async {
      // TODO
    });

    // Customer List Upload updated_time. Epoch (seconds).
    // int updatedTime
    test('to test the property `updatedTime`', () async {
      // TODO
    });


  });

}
