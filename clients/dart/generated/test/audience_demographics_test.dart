//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for AudienceDemographics
void main() {
  // final instance = AudienceDemographics();

  group('test AudienceDemographics', () {
    // Ages distribution.
    // List<AudienceDemographicValue> ages (default value: const [])
    test('to test the property `ages`', () async {
      // TODO
    });

    // Country area distribution.
    // List<AudienceDemographicValue> countries (default value: const [])
    test('to test the property `countries`', () async {
      // TODO
    });

    // Device usage distribution.
    // List<AudienceDemographicValue> devices (default value: const [])
    test('to test the property `devices`', () async {
      // TODO
    });

    // Gender distribution.
    // List<AudienceDemographicValue> genders (default value: const [])
    test('to test the property `genders`', () async {
      // TODO
    });

    // Geographic metro area distribution.
    // List<AudienceDemographicValue> metros (default value: const [])
    test('to test the property `metros`', () async {
      // TODO
    });


  });

}
