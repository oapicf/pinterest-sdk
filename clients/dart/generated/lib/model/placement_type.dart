...
enum PlacementType {
  SEARCH._(r'SEARCH'),
  BROWSE._(r'BROWSE'),
  RELATED_PINS._(r'RELATED_PINS'),
  ;

  /// Instantiate a new enum with the provided value.
  const PlacementType._(this._value);
  ...
  /// Returns the instance of [PlacementType] that was successfully decoded
  /// from the passed [value] on success, null otherwise.
  static PlacementType? fromJson(dynamic value) => PlacementTypeTypeTransformer().decode(value);

  /// Returns a [List] containing instances of [PlacementType]
...
class PlacementTypeTypeTransformer {
  ...
  /// Encodes this enum as a value suitable for JSON.
  String encode(PlacementType data) => data._value;
  ...
  PlacementType? decode(dynamic data, {bool allowNull = true}) {
    if (data is PlacementType) {
      return data;
    }
    if (data != null) {
      switch (data) {
        case r'SEARCH': return PlacementType.SEARCH;
        case r'BROWSE': return PlacementType.BROWSE;
        case r'RELATED_PINS': return PlacementType.RELATED_PINS;
        default:
          if (!allowNull) {
