...
class AdCreate {
  /// Returns a new [AdCreate] instance.
  AdCreate({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AdCreate &&
  ...
  @override
  String toString() => 'AdCreate[adGroupId=$adGroupId, androidDeepLink=$androidDeepLink, carouselAndroidDeepLinks=$carouselAndroidDeepLinks, carouselDestinationUrls=$carouselDestinationUrls, carouselIosDeepLinks=$carouselIosDeepLinks, clickTrackingUrl=$clickTrackingUrl, collectionItemsDestinationUrlTemplate=$collectionItemsDestinationUrlTemplate, collectionsHeaderType=$collectionsHeaderType, creativeType=$creativeType, customizableCtaType=$customizableCtaType, destinationUrl=$destinationUrl, disclosureType=$disclosureType, disclosureUrl=$disclosureUrl, gridClickType=$gridClickType, iosDeepLink=$iosDeepLink, isCarting=$isCarting, isCollageAcceptedTerms=$isCollageAcceptedTerms, isCollageSingleDestination=$isCollageSingleDestination, isPinDeleted=$isPinDeleted, isRemovable=$isRemovable, leadFormId=$leadFormId, name=$name, pinId=$pinId, quizPinData=$quizPinData, status=$status, trackingUrls=$trackingUrls, viewTrackingUrl=$viewTrackingUrl]';
  ...
  /// Returns a new [AdCreate] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AdCreate? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'ad_group_id'), 'Required key "AdCreate[ad_group_id]" is missing from JSON.');
        assert(json[r'ad_group_id'] != null, 'Required key "AdCreate[ad_group_id]" has a null value in JSON.');
        assert(json.containsKey(r'creative_type'), 'Required key "AdCreate[creative_type]" is missing from JSON.');
        assert(json[r'creative_type'] != null, 'Required key "AdCreate[creative_type]" has a null value in JSON.');
        assert(json.containsKey(r'pin_id'), 'Required key "AdCreate[pin_id]" is missing from JSON.');
        assert(json[r'pin_id'] != null, 'Required key "AdCreate[pin_id]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<AdCreate> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AdCreate>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AdCreate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AdCreate> mapFromJson(dynamic json) {
    final map = <String, AdCreate>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AdCreate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AdCreate-objects as value to a dart map
  static Map<String, List<AdCreate>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AdCreate>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AdCreate.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
