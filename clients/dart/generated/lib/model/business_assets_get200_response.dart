...
class BusinessAssetsGet200Response {
  ...
  static BusinessAssetsGet200Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'items'), 'Required key "BusinessAssetsGet200Response[items]" is missing from JSON.');
        assert(json[r'items'] != null, 'Required key "BusinessAssetsGet200Response[items]" has a null value in JSON.');
        return true;
      }());

      return BusinessAssetsGet200Response(
        bookmark: mapValueOfType<String>(json, r'bookmark'),
        items: BusinessAssets.listFromJson(json[r'items']),
      );
    }
    return null;
  }
