...
class CatalogsHotelItemResponse {
  ...
  @override
  String toString() => 'CatalogsHotelItemResponse[attributes=$attributes, catalogType=$catalogType, hotelId=$hotelId, pins=$pins]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CatalogsHotelItemResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CatalogsHotelItemResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
