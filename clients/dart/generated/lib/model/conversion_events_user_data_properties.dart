...
class ConversionEventsUserDataProperties {
  /// Returns a new [ConversionEventsUserDataProperties] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ConversionEventsUserDataProperties &&
  ...
  @override
  String toString() => 'ConversionEventsUserDataProperties[clickId=$clickId, clientIpAddress=$clientIpAddress, clientUserAgent=$clientUserAgent, country=$country, ct=$ct, db=$db, em=$em, externalId=$externalId, fn=$fn, ge=$ge, hashedMaids=$hashedMaids, ln=$ln, partnerId=$partnerId, ph=$ph, st=$st, zp=$zp]';
  ...
  /// Returns a new [ConversionEventsUserDataProperties] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ConversionEventsUserDataProperties? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ConversionEventsUserDataProperties[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ConversionEventsUserDataProperties[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ConversionEventsUserDataProperties> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ConversionEventsUserDataProperties>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ConversionEventsUserDataProperties.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ConversionEventsUserDataProperties> mapFromJson(dynamic json) {
    final map = <String, ConversionEventsUserDataProperties>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ConversionEventsUserDataProperties.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ConversionEventsUserDataProperties-objects as value to a dart map
  static Map<String, List<ConversionEventsUserDataProperties>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ConversionEventsUserDataProperties>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ConversionEventsUserDataProperties.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
