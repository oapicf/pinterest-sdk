...
class OrderLines {
  /// Returns a new [OrderLines] instance.
  OrderLines({
    this.adAccountId,
    this.budget,
    this.endTime,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OrderLines &&
  ...
  @override
  String toString() => 'OrderLines[adAccountId=$adAccountId, budget=$budget, endTime=$endTime, id=$id, name=$name, paidBudget=$paidBudget, paidType=$paidType, purchaseOrderId=$purchaseOrderId, startTime=$startTime, status=$status, type=$type]';
  ...
  /// Returns a new [OrderLines] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OrderLines? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OrderLines[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OrderLines[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OrderLines> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OrderLines>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OrderLines.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OrderLines> mapFromJson(dynamic json) {
    final map = <String, OrderLines>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OrderLines.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OrderLines-objects as value to a dart map
  static Map<String, List<OrderLines>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OrderLines>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = OrderLines.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
