...
class ConversionTagCommon {
  ...
  static ConversionTagCommon? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "ConversionTagCommon[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "ConversionTagCommon[name]" has a null value in JSON.');
        return true;
      }());
