...
class AggregatedPinComment {
  /// Returns a new [AggregatedPinComment] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AggregatedPinComment &&
  ...
  @override
  String toString() => 'AggregatedPinComment[aiDisclosures=$aiDisclosures, altText=$altText, boardId=$boardId, boardOwner=$boardOwner, boardSectionId=$boardSectionId, createdAt=$createdAt, creativeType=$creativeType, description=$description, dominantColor=$dominantColor, hasBeenPromoted=$hasBeenPromoted, id=$id, isOwner=$isOwner, isProduct=$isProduct, isStandard=$isStandard, link=$link, media=$media, parentPinId=$parentPinId, pinMetrics=$pinMetrics, title=$title]';
  ...
  /// Returns a new [AggregatedPinComment] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AggregatedPinComment? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'), 'Required key "AggregatedPinComment[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "AggregatedPinComment[id]" has a null value in JSON.');
        return true;
      }());

      return AggregatedPinComment(
  ...
  static List<AggregatedPinComment> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AggregatedPinComment>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AggregatedPinComment.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AggregatedPinComment> mapFromJson(dynamic json) {
    final map = <String, AggregatedPinComment>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AggregatedPinComment.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AggregatedPinComment-objects as value to a dart map
  static Map<String, List<AggregatedPinComment>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AggregatedPinComment>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AggregatedPinComment.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
