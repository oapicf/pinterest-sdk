...
class PinterestLibError {
  ...
  static PinterestLibError? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'code'), 'Required key "PinterestLibError[code]" is missing from JSON.');
        assert(json[r'code'] != null, 'Required key "PinterestLibError[code]" has a null value in JSON.');
        assert(json.containsKey(r'message'), 'Required key "PinterestLibError[message]" is missing from JSON.');
        assert(json[r'message'] != null, 'Required key "PinterestLibError[message]" has a null value in JSON.');
        return true;
      }());
