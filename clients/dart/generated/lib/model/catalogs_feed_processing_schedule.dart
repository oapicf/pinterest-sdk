...
class CatalogsFeedProcessingSchedule {
  ...
  static CatalogsFeedProcessingSchedule? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'time'), 'Required key "CatalogsFeedProcessingSchedule[time]" is missing from JSON.');
        assert(json[r'time'] != null, 'Required key "CatalogsFeedProcessingSchedule[time]" has a null value in JSON.');
        assert(json.containsKey(r'timezone'), 'Required key "CatalogsFeedProcessingSchedule[timezone]" is missing from JSON.');
        return true;
      }());

      return CatalogsFeedProcessingSchedule(
        time: mapValueOfType<String>(json, r'time')!,
        timezone: CatalogsFeedProcessingScheduleTimezone.fromJson(json[r'timezone']),
      );
    }
    return null;
  }
