...
class LeadFormUpdateRequest {
  /// Returns a new [LeadFormUpdateRequest] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LeadFormUpdateRequest &&
  ...
  @override
  String toString() => 'LeadFormUpdateRequest[completionMessage=$completionMessage, disclosureLanguage=$disclosureLanguage, hasAcceptedTerms=$hasAcceptedTerms, name=$name, policyLinks=$policyLinks, privacyPolicyLink=$privacyPolicyLink, questions=$questions, status=$status, id=$id]';
  ...
  /// Returns a new [LeadFormUpdateRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LeadFormUpdateRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LeadFormUpdateRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LeadFormUpdateRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return LeadFormUpdateRequest(
  ...
  static List<LeadFormUpdateRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LeadFormUpdateRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LeadFormUpdateRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LeadFormUpdateRequest> mapFromJson(dynamic json) {
    final map = <String, LeadFormUpdateRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LeadFormUpdateRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LeadFormUpdateRequest-objects as value to a dart map
  static Map<String, List<LeadFormUpdateRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LeadFormUpdateRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LeadFormUpdateRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
