//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class CatalogsFeedIngestion {
  /// Returns a new [CatalogsFeedIngestion] instance.
  CatalogsFeedIngestion({
    required this.createdAt,
    required this.feedId,
    required this.id,
    required this.status,
  });

  /// Timestamp of the feed ingestion.
  DateTime createdAt;

  /// Catalog Feed id pertaining to the feed ingestion.
  String feedId;

  /// Unique identifier of a feed ingestion.
  String id;

  /// Status of the feed ingestion.
  CatalogsFeedProcessingStatus status;

  @override
  bool operator ==(Object other) => identical(this, other) || other is CatalogsFeedIngestion &&
    other.createdAt == createdAt &&
    other.feedId == feedId &&
    other.id == id &&
    other.status == status;

  @override
  int get hashCode =>
    // ignore: unnecessary_parenthesis
    (createdAt.hashCode) +
    (feedId.hashCode) +
    (id.hashCode) +
    (status.hashCode);

  @override
  String toString() => 'CatalogsFeedIngestion[createdAt=$createdAt, feedId=$feedId, id=$id, status=$status]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
      json[r'created_at'] = this.createdAt.toUtc().toIso8601String();
      json[r'feed_id'] = this.feedId;
      json[r'id'] = this.id;
      json[r'status'] = this.status;
    return json;
  }

  /// Returns a new [CatalogsFeedIngestion] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CatalogsFeedIngestion? fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();

      // Ensure that the map contains the required keys.
      // Note 1: the values aren't checked for validity beyond being non-null.
      // Note 2: this code is stripped in release mode!
      assert(() {
        assert(json.containsKey(r'created_at'), 'Required key "CatalogsFeedIngestion[created_at]" is missing from JSON.');
        assert(json[r'created_at'] != null, 'Required key "CatalogsFeedIngestion[created_at]" has a null value in JSON.');
        assert(json.containsKey(r'feed_id'), 'Required key "CatalogsFeedIngestion[feed_id]" is missing from JSON.');
        assert(json[r'feed_id'] != null, 'Required key "CatalogsFeedIngestion[feed_id]" has a null value in JSON.');
        assert(json.containsKey(r'id'), 'Required key "CatalogsFeedIngestion[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "CatalogsFeedIngestion[id]" has a null value in JSON.');
        assert(json.containsKey(r'status'), 'Required key "CatalogsFeedIngestion[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "CatalogsFeedIngestion[status]" has a null value in JSON.');
        return true;
      }());

      return CatalogsFeedIngestion(
        createdAt: mapDateTime(json, r'created_at', r'')!,
        feedId: mapValueOfType<String>(json, r'feed_id')!,
        id: mapValueOfType<String>(json, r'id')!,
        status: CatalogsFeedProcessingStatus.fromJson(json[r'status'])!,
      );
    }
    return null;
  }

  static List<CatalogsFeedIngestion> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CatalogsFeedIngestion>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CatalogsFeedIngestion.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
    }
    return result.toList(growable: growable);
  }

  static Map<String, CatalogsFeedIngestion> mapFromJson(dynamic json) {
    final map = <String, CatalogsFeedIngestion>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CatalogsFeedIngestion.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
    }
    return map;
  }

  // maps a json object with a list of CatalogsFeedIngestion-objects as value to a dart map
  static Map<String, List<CatalogsFeedIngestion>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CatalogsFeedIngestion>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CatalogsFeedIngestion.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

  /// The list of required keys that must be present in a JSON.
  static const requiredKeys = <String>{
    'created_at',
    'feed_id',
    'id',
    'status',
  };
}

