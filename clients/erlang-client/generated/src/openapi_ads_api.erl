...
-spec ads/analytics(ctx:ctx(), binary(), calendar:date(), calendar:date(), list(), openapi_granularity, maps:map()) -> {ok, [openapi_ads_analytics_response_inner:openapi_ads_analytics_response_inner()], openapi_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), openapi_utils:response_info()}.
ads/analytics(Ctx, AdAccountId, StartDate, EndDate, Columns, Granularity, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(openapi_api, config, #{})),
