...
void OauthApi::setupRoutes() {
    ...
    Routes::Post(*router, base + "/oauth/conversion_token", Routes::bind(&OauthApi::oauth_conversion_token_handler, this));
    Routes::Post(*router, base + "/oauth/token", Routes::bind(&OauthApi::oauth_token_handler, this));
    Routes::Post(*router, base + "/oauth/token/revoke", Routes::bind(&OauthApi::token_revoke_handler, this));
...
void OauthApi::oauth_conversion_token_handler(const Pistache::Rest::Request& request, Pistache::Http::ResponseWriter response) {
    try {
...
#define HTTP_BEARER_AUTH_DEFINED 0
            ...
            this->oauth_conversion_token(response);
...
#define HTTP_BEARER_AUTH_DEFINED 0
#endif
#define REST_PATH "/oauth/conversion_token" 
...
void OauthApi::token_revoke_handler(const Pistache::Rest::Request& request, Pistache::Http::ResponseWriter response) {
    try {
            
            try {
                this->token_revoke(request, response);
            } catch (Pistache::Http::HttpError &e) {
                response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
                return;
            } catch (std::exception &e) {
...
#define HTTP_BEARER_AUTH_DEFINED 0
#endif
#define REST_PATH "/oauth/token/revoke" 
    static_assert(HTTP_BASIC_AUTH_DEFINED + HTTP_BEARER_AUTH_DEFINED < 2, "Path '" REST_PATH "' has more than one security scheme specified, and the Pistache server generator does not support that.");
#undef REST_PATH
#ifdef HTTP_BEARER_AUTH_DEFINED
#undef HTTP_BEARER_AUTH_DEFINED
