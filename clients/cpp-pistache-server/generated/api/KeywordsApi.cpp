...
#define HTTP_BEARER_AUTH_DEFINED 0
            ...
            this->keywords_get(adAccountId, campaignId, adGroupId, adGroupIds, matchTypes, pageSize, bookmark, response);
            } catch (Pistache::Http::HttpError &e) {
                response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
                return;
            } catch (std::exception &e) {
...
void KeywordsApi::trending_keywords_list_handler(const Pistache::Rest::Request& request, Pistache::Http::ResponseWriter response) {
    try {
        ...
        auto includePredictionQuery = request.query().get("include_prediction");
        std::optional<bool> includePrediction;
        if (includePredictionQuery.has_value()) {
            bool valueQuery_instance;
            if (fromStringValue(includePredictionQuery.value(), valueQuery_instance)) {
                includePrediction = valueQuery_instance;
            }
        }
...
#define HTTP_BEARER_AUTH_DEFINED 0
            ...
            this->trending_keywords_list(region, trendType, interests, genders, ages, includeKeywords, normalizeAgainstGroup, limit, includePrediction, includeDemographics, response);
            } catch (Pistache::Http::HttpError &e) {
                response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
                return;
            } catch (std::exception &e) {
