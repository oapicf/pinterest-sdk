...
private:
    ...
    virtual void analytics_create_conversion_product_report( const std::string &adAccountId, const org::openapitools::server::model::ConversionProductReportRequest &conversionProductReportRequest, Pistache::Http::ResponseWriter &response) = 0;
    /// <summary>
    ...
    /// <param name="granularity">   TOTAL - metrics are aggregated over the specified date range.    DAY - metrics are broken down daily.    HOUR - metrics are broken down hourly.    WEEKLY - metrics are broken down weekly.    MONTHLY - metrics are broken down monthly (optional, default to std::make_shared&lt;Granularity&gt;())</param>
