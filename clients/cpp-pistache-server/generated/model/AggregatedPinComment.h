/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * AggregatedPinComment.h
 *
 * 
 */

#ifndef AggregatedPinComment_H_
#define AggregatedPinComment_H_


#include "AiDisclosures.h"
#include <nlohmann/json.hpp>
#include "CreativeType.h"
#include "PinMedia.h"
#include <string>
#include "BoardOwner.h"
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// 
/// </summary>
class  AggregatedPinComment
{
public:
    AggregatedPinComment();
    virtual ~AggregatedPinComment();


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const AggregatedPinComment& rhs) const;
    bool operator!=(const AggregatedPinComment& rhs) const;

    /////////////////////////////////////////////
    /// AggregatedPinComment members

    /// <summary>
    /// AI disclosure declarations the creator has made about this Pin.
    /// </summary>
    org::openapitools::server::model::AiDisclosures getAiDisclosures() const;
    void setAiDisclosures(org::openapitools::server::model::AiDisclosures const& value);
    bool aiDisclosuresIsSet() const;
    void unsetAi_disclosures();
    /// <summary>
    /// 
    /// </summary>
    std::string getAltText() const;
    void setAltText(std::string const& value);
    bool altTextIsSet() const;
    void unsetAlt_text();
    /// <summary>
    /// The board to which this Pin belongs.
    /// </summary>
    std::string getBoardId() const;
    void setBoardId(std::string const& value);
    bool boardIdIsSet() const;
    void unsetBoard_id();
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::BoardOwner getBoardOwner() const;
    void setBoardOwner(org::openapitools::server::model::BoardOwner const& value);
    bool boardOwnerIsSet() const;
    void unsetBoard_owner();
    /// <summary>
    /// The board section to which this Pin belongs.
    /// </summary>
    std::string getBoardSectionId() const;
    void setBoardSectionId(std::string const& value);
    bool boardSectionIdIsSet() const;
    void unsetBoard_section_id();
    /// <summary>
    /// 
    /// </summary>
    std::string getCreatedAt() const;
    void setCreatedAt(std::string const& value);
    bool createdAtIsSet() const;
    void unsetCreated_at();
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::CreativeType getCreativeType() const;
    void setCreativeType(org::openapitools::server::model::CreativeType const& value);
    bool creativeTypeIsSet() const;
    void unsetCreative_type();
    /// <summary>
    /// 
    /// </summary>
    std::string getDescription() const;
    void setDescription(std::string const& value);
    bool descriptionIsSet() const;
    void unsetDescription();
    /// <summary>
    /// Dominant pin color. Hex number, e.g. &#x60;#6E7874&#x60;.
    /// </summary>
    std::string getDominantColor() const;
    void setDominantColor(std::string const& value);
    bool dominantColorIsSet() const;
    void unsetDominant_color();
    /// <summary>
    /// Whether the Pin has been promoted or not.
    /// </summary>
    bool isHasBeenPromoted() const;
    void setHasBeenPromoted(bool const value);
    bool hasBeenPromotedIsSet() const;
    void unsetHas_been_promoted();
    /// <summary>
    /// 
    /// </summary>
    std::string getId() const;
    void setId(std::string const& value);
    /// <summary>
    /// Whether the \&quot;operation user_account\&quot; is the Pin owner.
    /// </summary>
    bool isIsOwner() const;
    void setIsOwner(bool const value);
    bool isOwnerIsSet() const;
    void unsetIs_owner();
    /// <summary>
    /// Whether the Pin is a product Pin.
    /// </summary>
    bool isIsProduct() const;
    void setIsProduct(bool const value);
    bool isProductIsSet() const;
    void unsetIs_product();
    /// <summary>
    /// Whether the Pin is standard or not. See documentation on [Changes to Pin creation](/docs/api-features/content-overview/) for more information.
    /// </summary>
    bool isIsStandard() const;
    void setIsStandard(bool const value);
    bool isStandardIsSet() const;
    void unsetIs_standard();
    /// <summary>
    /// 
    /// </summary>
    std::string getLink() const;
    void setLink(std::string const& value);
    bool linkIsSet() const;
    void unsetLink();
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::PinMedia getMedia() const;
    void setMedia(org::openapitools::server::model::PinMedia const& value);
    bool mediaIsSet() const;
    void unsetMedia();
    /// <summary>
    /// The source pin id if this pin was saved from another pin. [Learn more](https://help.pinterest.com/article/save-pins-on-pinterest).
    /// </summary>
    std::string getParentPinId() const;
    void setParentPinId(std::string const& value);
    bool parentPinIdIsSet() const;
    void unsetParent_pin_id();
    /// <summary>
    /// Pin metrics with associated time intervals if any.
    /// </summary>
    nlohmann::json getPinMetrics() const;
    void setPinMetrics(nlohmann::json const& value);
    bool pinMetricsIsSet() const;
    void unsetPin_metrics();
    /// <summary>
    /// 
    /// </summary>
    std::string getTitle() const;
    void setTitle(std::string const& value);
    bool titleIsSet() const;
    void unsetTitle();

    friend  void to_json(nlohmann::json& j, const AggregatedPinComment& o);
    friend  void from_json(const nlohmann::json& j, AggregatedPinComment& o);
protected:
    org::openapitools::server::model::AiDisclosures m_Ai_disclosures;
    bool m_Ai_disclosuresIsSet;
    std::string m_Alt_text;
    bool m_Alt_textIsSet;
    std::string m_Board_id;
    bool m_Board_idIsSet;
    org::openapitools::server::model::BoardOwner m_Board_owner;
    bool m_Board_ownerIsSet;
    std::string m_Board_section_id;
    bool m_Board_section_idIsSet;
    std::string m_Created_at;
    bool m_Created_atIsSet;
    org::openapitools::server::model::CreativeType m_Creative_type;
    bool m_Creative_typeIsSet;
    std::string m_Description;
    bool m_DescriptionIsSet;
    std::string m_Dominant_color;
    bool m_Dominant_colorIsSet;
    bool m_Has_been_promoted;
    bool m_Has_been_promotedIsSet;
    std::string m_Id;

    bool m_Is_owner;
    bool m_Is_ownerIsSet;
    bool m_Is_product;
    bool m_Is_productIsSet;
    bool m_Is_standard;
    bool m_Is_standardIsSet;
    std::string m_Link;
    bool m_LinkIsSet;
    org::openapitools::server::model::PinMedia m_Media;
    bool m_MediaIsSet;
    std::string m_Parent_pin_id;
    bool m_Parent_pin_idIsSet;
    nlohmann::json m_Pin_metrics;
    bool m_Pin_metricsIsSet;
    std::string m_Title;
    bool m_TitleIsSet;
    
};

} // namespace org::openapitools::server::model

#endif /* AggregatedPinComment_H_ */
