/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "LeadFormBatchUpdate.h"
#include "Helpers.h"

#include <sstream>

namespace org::openapitools::server::model
{

LeadFormBatchUpdate::LeadFormBatchUpdate()
{
    m_Completion_message = "";
    m_Completion_messageIsSet = false;
    m_Disclosure_language = "";
    m_Disclosure_languageIsSet = false;
    m_Has_accepted_terms = false;
    m_Has_accepted_termsIsSet = false;
    m_Id = "";
    m_Name = "";
    m_NameIsSet = false;
    m_Policy_linksIsSet = false;
    m_Privacy_policy_link = "";
    m_Privacy_policy_linkIsSet = false;
    m_QuestionsIsSet = false;
    m_StatusIsSet = false;
    
}

LeadFormBatchUpdate::~LeadFormBatchUpdate() = default;

void LeadFormBatchUpdate::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw org::openapitools::server::helpers::ValidationException(msg.str());
    }
}

bool LeadFormBatchUpdate::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool LeadFormBatchUpdate::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "LeadFormBatchUpdate" : pathPrefix;

                     
    
    /* Id */ {
        const std::string& value = m_Id;
        const std::string currentValuePath = _pathPrefix + ".id";
                
        

    }
             
    if (policyLinksIsSet())
    {
        const std::vector<org::openapitools::server::model::LeadFormPolicyLink>& value = m_Policy_links;
        const std::string currentValuePath = _pathPrefix + ".policyLinks";
                
        
        if (value.size() < 0)
        {
            success = false;
            msg << currentValuePath << ": must have at least 0 elements;";
        }
        if (value.size() > 3)
        {
            success = false;
            msg << currentValuePath << ": must have at most 3 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const org::openapitools::server::model::LeadFormPolicyLink& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".policyLinks") && success;
 
                i++;
            }
        }

    }
             
    if (questionsIsSet())
    {
        const std::vector<org::openapitools::server::model::LeadFormQuestion>& value = m_Questions;
        const std::string currentValuePath = _pathPrefix + ".questions";
                
        
        if (value.size() < 0)
        {
            success = false;
            msg << currentValuePath << ": must have at least 0 elements;";
        }
        if (value.size() > 10)
        {
            success = false;
            msg << currentValuePath << ": must have at most 10 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const org::openapitools::server::model::LeadFormQuestion& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".questions") && success;
 
                i++;
            }
        }

    }
        
    return success;
}

bool LeadFormBatchUpdate::operator==(const LeadFormBatchUpdate& rhs) const
{
    return
    
    
    
    ((!completionMessageIsSet() && !rhs.completionMessageIsSet()) || (completionMessageIsSet() && rhs.completionMessageIsSet() && getCompletionMessage() == rhs.getCompletionMessage())) &&
    
    
    ((!disclosureLanguageIsSet() && !rhs.disclosureLanguageIsSet()) || (disclosureLanguageIsSet() && rhs.disclosureLanguageIsSet() && getDisclosureLanguage() == rhs.getDisclosureLanguage())) &&
    
    
    ((!hasAcceptedTermsIsSet() && !rhs.hasAcceptedTermsIsSet()) || (hasAcceptedTermsIsSet() && rhs.hasAcceptedTermsIsSet() && isHasAcceptedTerms() == rhs.isHasAcceptedTerms())) &&
    
    (getId() == rhs.getId())
     &&
    
    
    ((!nameIsSet() && !rhs.nameIsSet()) || (nameIsSet() && rhs.nameIsSet() && getName() == rhs.getName())) &&
    
    
    ((!policyLinksIsSet() && !rhs.policyLinksIsSet()) || (policyLinksIsSet() && rhs.policyLinksIsSet() && getPolicyLinks() == rhs.getPolicyLinks())) &&
    
    
    ((!privacyPolicyLinkIsSet() && !rhs.privacyPolicyLinkIsSet()) || (privacyPolicyLinkIsSet() && rhs.privacyPolicyLinkIsSet() && getPrivacyPolicyLink() == rhs.getPrivacyPolicyLink())) &&
    
    
    ((!questionsIsSet() && !rhs.questionsIsSet()) || (questionsIsSet() && rhs.questionsIsSet() && getQuestions() == rhs.getQuestions())) &&
    
    
    ((!statusIsSet() && !rhs.statusIsSet()) || (statusIsSet() && rhs.statusIsSet() && getStatus() == rhs.getStatus()))
    
    ;
}

bool LeadFormBatchUpdate::operator!=(const LeadFormBatchUpdate& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const LeadFormBatchUpdate& o)
{
    j = nlohmann::json::object();
    if(o.completionMessageIsSet())
        j["completion_message"] = o.m_Completion_message;
    if(o.disclosureLanguageIsSet())
        j["disclosure_language"] = o.m_Disclosure_language;
    if(o.hasAcceptedTermsIsSet())
        j["has_accepted_terms"] = o.m_Has_accepted_terms;
    j["id"] = o.m_Id;
    if(o.nameIsSet())
        j["name"] = o.m_Name;
    if(o.policyLinksIsSet() || !o.m_Policy_links.empty())
        j["policy_links"] = o.m_Policy_links;
    if(o.privacyPolicyLinkIsSet())
        j["privacy_policy_link"] = o.m_Privacy_policy_link;
    if(o.questionsIsSet() || !o.m_Questions.empty())
        j["questions"] = o.m_Questions;
    if(o.statusIsSet())
        j["status"] = o.m_Status;
    
}

void from_json(const nlohmann::json& j, LeadFormBatchUpdate& o)
{
    if(j.find("completion_message") != j.end())
    {
        j.at("completion_message").get_to(o.m_Completion_message);
        o.m_Completion_messageIsSet = true;
    } 
    if(j.find("disclosure_language") != j.end())
    {
        j.at("disclosure_language").get_to(o.m_Disclosure_language);
        o.m_Disclosure_languageIsSet = true;
    } 
    if(j.find("has_accepted_terms") != j.end())
    {
        j.at("has_accepted_terms").get_to(o.m_Has_accepted_terms);
        o.m_Has_accepted_termsIsSet = true;
    } 
    j.at("id").get_to(o.m_Id);
    if(j.find("name") != j.end())
    {
        j.at("name").get_to(o.m_Name);
        o.m_NameIsSet = true;
    } 
    if(j.find("policy_links") != j.end())
    {
        j.at("policy_links").get_to(o.m_Policy_links);
        o.m_Policy_linksIsSet = true;
    } 
    if(j.find("privacy_policy_link") != j.end())
    {
        j.at("privacy_policy_link").get_to(o.m_Privacy_policy_link);
        o.m_Privacy_policy_linkIsSet = true;
    } 
    if(j.find("questions") != j.end())
    {
        j.at("questions").get_to(o.m_Questions);
        o.m_QuestionsIsSet = true;
    } 
    if(j.find("status") != j.end())
    {
        j.at("status").get_to(o.m_Status);
        o.m_StatusIsSet = true;
    } 
    
}

std::string LeadFormBatchUpdate::getCompletionMessage() const
{
    return m_Completion_message;
}
void LeadFormBatchUpdate::setCompletionMessage(std::string const& value)
{
    m_Completion_message = value;
    m_Completion_messageIsSet = true;
}
bool LeadFormBatchUpdate::completionMessageIsSet() const
{
    return m_Completion_messageIsSet;
}
void LeadFormBatchUpdate::unsetCompletion_message()
{
    m_Completion_messageIsSet = false;
}
std::string LeadFormBatchUpdate::getDisclosureLanguage() const
{
    return m_Disclosure_language;
}
void LeadFormBatchUpdate::setDisclosureLanguage(std::string const& value)
{
    m_Disclosure_language = value;
    m_Disclosure_languageIsSet = true;
}
bool LeadFormBatchUpdate::disclosureLanguageIsSet() const
{
    return m_Disclosure_languageIsSet;
}
void LeadFormBatchUpdate::unsetDisclosure_language()
{
    m_Disclosure_languageIsSet = false;
}
bool LeadFormBatchUpdate::isHasAcceptedTerms() const
{
    return m_Has_accepted_terms;
}
void LeadFormBatchUpdate::setHasAcceptedTerms(bool const value)
{
    m_Has_accepted_terms = value;
    m_Has_accepted_termsIsSet = true;
}
bool LeadFormBatchUpdate::hasAcceptedTermsIsSet() const
{
    return m_Has_accepted_termsIsSet;
}
void LeadFormBatchUpdate::unsetHas_accepted_terms()
{
    m_Has_accepted_termsIsSet = false;
}
std::string LeadFormBatchUpdate::getId() const
{
    return m_Id;
}
void LeadFormBatchUpdate::setId(std::string const& value)
{
    m_Id = value;
}
std::string LeadFormBatchUpdate::getName() const
{
    return m_Name;
}
void LeadFormBatchUpdate::setName(std::string const& value)
{
    m_Name = value;
    m_NameIsSet = true;
}
bool LeadFormBatchUpdate::nameIsSet() const
{
    return m_NameIsSet;
}
void LeadFormBatchUpdate::unsetName()
{
    m_NameIsSet = false;
}
std::vector<org::openapitools::server::model::LeadFormPolicyLink> LeadFormBatchUpdate::getPolicyLinks() const
{
    return m_Policy_links;
}
void LeadFormBatchUpdate::setPolicyLinks(std::vector<org::openapitools::server::model::LeadFormPolicyLink> const& value)
{
    m_Policy_links = value;
    m_Policy_linksIsSet = true;
}
bool LeadFormBatchUpdate::policyLinksIsSet() const
{
    return m_Policy_linksIsSet;
}
void LeadFormBatchUpdate::unsetPolicy_links()
{
    m_Policy_linksIsSet = false;
}
std::string LeadFormBatchUpdate::getPrivacyPolicyLink() const
{
    return m_Privacy_policy_link;
}
void LeadFormBatchUpdate::setPrivacyPolicyLink(std::string const& value)
{
    m_Privacy_policy_link = value;
    m_Privacy_policy_linkIsSet = true;
}
bool LeadFormBatchUpdate::privacyPolicyLinkIsSet() const
{
    return m_Privacy_policy_linkIsSet;
}
void LeadFormBatchUpdate::unsetPrivacy_policy_link()
{
    m_Privacy_policy_linkIsSet = false;
}
std::vector<org::openapitools::server::model::LeadFormQuestion> LeadFormBatchUpdate::getQuestions() const
{
    return m_Questions;
}
void LeadFormBatchUpdate::setQuestions(std::vector<org::openapitools::server::model::LeadFormQuestion> const& value)
{
    m_Questions = value;
    m_QuestionsIsSet = true;
}
bool LeadFormBatchUpdate::questionsIsSet() const
{
    return m_QuestionsIsSet;
}
void LeadFormBatchUpdate::unsetQuestions()
{
    m_QuestionsIsSet = false;
}
org::openapitools::server::model::LeadFormStatus LeadFormBatchUpdate::getStatus() const
{
    return m_Status;
}
void LeadFormBatchUpdate::setStatus(org::openapitools::server::model::LeadFormStatus const& value)
{
    m_Status = value;
    m_StatusIsSet = true;
}
bool LeadFormBatchUpdate::statusIsSet() const
{
    return m_StatusIsSet;
}
void LeadFormBatchUpdate::unsetStatus()
{
    m_StatusIsSet = false;
}


} // namespace org::openapitools::server::model

