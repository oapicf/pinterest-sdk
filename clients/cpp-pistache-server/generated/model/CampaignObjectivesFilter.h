/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.23.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * CampaignObjectivesFilter.h
 *
 * 
 */

#ifndef CampaignObjectivesFilter_H_
#define CampaignObjectivesFilter_H_


#include "ObjectiveType.h"
#include <vector>
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// 
/// </summary>
class  CampaignObjectivesFilter
{
public:
    CampaignObjectivesFilter();
    virtual ~CampaignObjectivesFilter() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const CampaignObjectivesFilter& rhs) const;
    bool operator!=(const CampaignObjectivesFilter& rhs) const;

    /////////////////////////////////////////////
    /// CampaignObjectivesFilter members

    /// <summary>
    /// List of values for filtering. [\&quot;WEB_SESSIONS\&quot;] in BETA.
    /// </summary>
    std::vector<org::openapitools::server::model::ObjectiveType> getCampaignObjectiveTypes() const;
    void setCampaignObjectiveTypes(std::vector<org::openapitools::server::model::ObjectiveType> const& value);
    bool campaignObjectiveTypesIsSet() const;
    void unsetCampaign_objective_types();

    friend  void to_json(nlohmann::json& j, const CampaignObjectivesFilter& o);
    friend  void from_json(const nlohmann::json& j, CampaignObjectivesFilter& o);
protected:
    std::vector<org::openapitools::server::model::ObjectiveType> m_Campaign_objective_types;
    bool m_Campaign_objective_typesIsSet;
    
};

} // namespace org::openapitools::server::model

#endif /* CampaignObjectivesFilter_H_ */
