/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * PlacementMultipliers.h
 *
 * This represents a mapping from placement to a bid price adjustment.  Multiplier values must be between 0 and 10. A value of 10 represents a 900% increase in bid price (from $1 to $10 for example). A value of 0 will stop distribution for this item on the specified placement in &#x60;MAX_BID&#x60; ad groups in &#x60;CATALOG_SALES&#x60; campaigns. All placement multipliers must be set at the same time. If a multiplier is not provided it is assumed to be 1 (no bid adjustment).
 */

#ifndef PlacementMultipliers_H_
#define PlacementMultipliers_H_


#include "PlacementType.h"
#include <map>
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// This represents a mapping from placement to a bid price adjustment.  Multiplier values must be between 0 and 10. A value of 10 represents a 900% increase in bid price (from $1 to $10 for example). A value of 0 will stop distribution for this item on the specified placement in &#x60;MAX_BID&#x60; ad groups in &#x60;CATALOG_SALES&#x60; campaigns. All placement multipliers must be set at the same time. If a multiplier is not provided it is assumed to be 1 (no bid adjustment).
/// </summary>
class  PlacementMultipliers
{
public:
    PlacementMultipliers();
    virtual ~PlacementMultipliers();


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const PlacementMultipliers& rhs) const;
    bool operator!=(const PlacementMultipliers& rhs) const;

    /////////////////////////////////////////////
    /// PlacementMultipliers members

    /// <summary>
    /// Placement type identifier.
    /// </summary>
    org::openapitools::server::model::PlacementType getPLACEMENT() const;
    void setPLACEMENT(org::openapitools::server::model::PlacementType const& value);
    bool pLACEMENTIsSet() const;
    void unsetPLACEMENT();

    friend  void to_json(nlohmann::json& j, const PlacementMultipliers& o);
    friend  void from_json(const nlohmann::json& j, PlacementMultipliers& o);
protected:
    org::openapitools::server::model::PlacementType m_PLACEMENT;
    bool m_PLACEMENTIsSet;
    
};

} // namespace org::openapitools::server::model

#endif /* PlacementMultipliers_H_ */
