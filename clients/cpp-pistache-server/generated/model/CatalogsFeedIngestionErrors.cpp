/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "CatalogsFeedIngestionErrors.h"
#include "Helpers.h"

#include <sstream>

namespace org::openapitools::server::model
{

CatalogsFeedIngestionErrors::CatalogsFeedIngestionErrors()
{
    m_ACCOUNT_FLAGGED = 0;
    m_ACCOUNT_FLAGGEDIsSet = false;
    m_FETCH_GOOGLE_SHEET_NOT_SHARED = 0;
    m_FETCH_GOOGLE_SHEET_NOT_SHAREDIsSet = false;
    m_IMAGE_FILE_NOT_ACCESSIBLE = 0;
    m_IMAGE_FILE_NOT_ACCESSIBLEIsSet = false;
    m_IMAGE_FILE_NOT_FOUND = 0;
    m_IMAGE_FILE_NOT_FOUNDIsSet = false;
    m_IMAGE_INVALID_FILE = 0;
    m_IMAGE_INVALID_FILEIsSet = false;
    m_IMAGE_LEVEL_INTERNAL_ERROR = 0;
    m_IMAGE_LEVEL_INTERNAL_ERRORIsSet = false;
    m_IMAGE_MALFORMED_URL = 0;
    m_IMAGE_MALFORMED_URLIsSet = false;
    m_LARGE_PRODUCT_COUNT_DECREASE = 0;
    m_LARGE_PRODUCT_COUNT_DECREASEIsSet = false;
    m_LINE_LEVEL_INTERNAL_ERROR = 0;
    m_LINE_LEVEL_INTERNAL_ERRORIsSet = false;
    
}

CatalogsFeedIngestionErrors::~CatalogsFeedIngestionErrors() = default;

void CatalogsFeedIngestionErrors::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw org::openapitools::server::helpers::ValidationException(msg.str());
    }
}

bool CatalogsFeedIngestionErrors::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool CatalogsFeedIngestionErrors::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "CatalogsFeedIngestionErrors" : pathPrefix;

                                        
    return success;
}

bool CatalogsFeedIngestionErrors::operator==(const CatalogsFeedIngestionErrors& rhs) const
{
    return
    
    
    
    ((!aCCOUNTFLAGGEDIsSet() && !rhs.aCCOUNTFLAGGEDIsSet()) || (aCCOUNTFLAGGEDIsSet() && rhs.aCCOUNTFLAGGEDIsSet() && getACCOUNTFLAGGED() == rhs.getACCOUNTFLAGGED())) &&
    
    
    ((!fETCHGOOGLESHEETNOTSHAREDIsSet() && !rhs.fETCHGOOGLESHEETNOTSHAREDIsSet()) || (fETCHGOOGLESHEETNOTSHAREDIsSet() && rhs.fETCHGOOGLESHEETNOTSHAREDIsSet() && getFETCHGOOGLESHEETNOTSHARED() == rhs.getFETCHGOOGLESHEETNOTSHARED())) &&
    
    
    ((!iMAGEFILENOTACCESSIBLEIsSet() && !rhs.iMAGEFILENOTACCESSIBLEIsSet()) || (iMAGEFILENOTACCESSIBLEIsSet() && rhs.iMAGEFILENOTACCESSIBLEIsSet() && getIMAGEFILENOTACCESSIBLE() == rhs.getIMAGEFILENOTACCESSIBLE())) &&
    
    
    ((!iMAGEFILENOTFOUNDIsSet() && !rhs.iMAGEFILENOTFOUNDIsSet()) || (iMAGEFILENOTFOUNDIsSet() && rhs.iMAGEFILENOTFOUNDIsSet() && getIMAGEFILENOTFOUND() == rhs.getIMAGEFILENOTFOUND())) &&
    
    
    ((!iMAGEINVALIDFILEIsSet() && !rhs.iMAGEINVALIDFILEIsSet()) || (iMAGEINVALIDFILEIsSet() && rhs.iMAGEINVALIDFILEIsSet() && getIMAGEINVALIDFILE() == rhs.getIMAGEINVALIDFILE())) &&
    
    
    ((!iMAGELEVELINTERNALERRORIsSet() && !rhs.iMAGELEVELINTERNALERRORIsSet()) || (iMAGELEVELINTERNALERRORIsSet() && rhs.iMAGELEVELINTERNALERRORIsSet() && getIMAGELEVELINTERNALERROR() == rhs.getIMAGELEVELINTERNALERROR())) &&
    
    
    ((!iMAGEMALFORMEDURLIsSet() && !rhs.iMAGEMALFORMEDURLIsSet()) || (iMAGEMALFORMEDURLIsSet() && rhs.iMAGEMALFORMEDURLIsSet() && getIMAGEMALFORMEDURL() == rhs.getIMAGEMALFORMEDURL())) &&
    
    
    ((!lARGEPRODUCTCOUNTDECREASEIsSet() && !rhs.lARGEPRODUCTCOUNTDECREASEIsSet()) || (lARGEPRODUCTCOUNTDECREASEIsSet() && rhs.lARGEPRODUCTCOUNTDECREASEIsSet() && getLARGEPRODUCTCOUNTDECREASE() == rhs.getLARGEPRODUCTCOUNTDECREASE())) &&
    
    
    ((!lINELEVELINTERNALERRORIsSet() && !rhs.lINELEVELINTERNALERRORIsSet()) || (lINELEVELINTERNALERRORIsSet() && rhs.lINELEVELINTERNALERRORIsSet() && getLINELEVELINTERNALERROR() == rhs.getLINELEVELINTERNALERROR()))
    
    ;
}

bool CatalogsFeedIngestionErrors::operator!=(const CatalogsFeedIngestionErrors& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const CatalogsFeedIngestionErrors& o)
{
    j = nlohmann::json::object();
    if(o.aCCOUNTFLAGGEDIsSet())
        j["ACCOUNT_FLAGGED"] = o.m_ACCOUNT_FLAGGED;
    if(o.fETCHGOOGLESHEETNOTSHAREDIsSet())
        j["FETCH_GOOGLE_SHEET_NOT_SHARED"] = o.m_FETCH_GOOGLE_SHEET_NOT_SHARED;
    if(o.iMAGEFILENOTACCESSIBLEIsSet())
        j["IMAGE_FILE_NOT_ACCESSIBLE"] = o.m_IMAGE_FILE_NOT_ACCESSIBLE;
    if(o.iMAGEFILENOTFOUNDIsSet())
        j["IMAGE_FILE_NOT_FOUND"] = o.m_IMAGE_FILE_NOT_FOUND;
    if(o.iMAGEINVALIDFILEIsSet())
        j["IMAGE_INVALID_FILE"] = o.m_IMAGE_INVALID_FILE;
    if(o.iMAGELEVELINTERNALERRORIsSet())
        j["IMAGE_LEVEL_INTERNAL_ERROR"] = o.m_IMAGE_LEVEL_INTERNAL_ERROR;
    if(o.iMAGEMALFORMEDURLIsSet())
        j["IMAGE_MALFORMED_URL"] = o.m_IMAGE_MALFORMED_URL;
    if(o.lARGEPRODUCTCOUNTDECREASEIsSet())
        j["LARGE_PRODUCT_COUNT_DECREASE"] = o.m_LARGE_PRODUCT_COUNT_DECREASE;
    if(o.lINELEVELINTERNALERRORIsSet())
        j["LINE_LEVEL_INTERNAL_ERROR"] = o.m_LINE_LEVEL_INTERNAL_ERROR;
    
}

void from_json(const nlohmann::json& j, CatalogsFeedIngestionErrors& o)
{
    if(j.find("ACCOUNT_FLAGGED") != j.end())
    {
        j.at("ACCOUNT_FLAGGED").get_to(o.m_ACCOUNT_FLAGGED);
        o.m_ACCOUNT_FLAGGEDIsSet = true;
    } 
    if(j.find("FETCH_GOOGLE_SHEET_NOT_SHARED") != j.end())
    {
        j.at("FETCH_GOOGLE_SHEET_NOT_SHARED").get_to(o.m_FETCH_GOOGLE_SHEET_NOT_SHARED);
        o.m_FETCH_GOOGLE_SHEET_NOT_SHAREDIsSet = true;
    } 
    if(j.find("IMAGE_FILE_NOT_ACCESSIBLE") != j.end())
    {
        j.at("IMAGE_FILE_NOT_ACCESSIBLE").get_to(o.m_IMAGE_FILE_NOT_ACCESSIBLE);
        o.m_IMAGE_FILE_NOT_ACCESSIBLEIsSet = true;
    } 
    if(j.find("IMAGE_FILE_NOT_FOUND") != j.end())
    {
        j.at("IMAGE_FILE_NOT_FOUND").get_to(o.m_IMAGE_FILE_NOT_FOUND);
        o.m_IMAGE_FILE_NOT_FOUNDIsSet = true;
    } 
    if(j.find("IMAGE_INVALID_FILE") != j.end())
    {
        j.at("IMAGE_INVALID_FILE").get_to(o.m_IMAGE_INVALID_FILE);
        o.m_IMAGE_INVALID_FILEIsSet = true;
    } 
    if(j.find("IMAGE_LEVEL_INTERNAL_ERROR") != j.end())
    {
        j.at("IMAGE_LEVEL_INTERNAL_ERROR").get_to(o.m_IMAGE_LEVEL_INTERNAL_ERROR);
        o.m_IMAGE_LEVEL_INTERNAL_ERRORIsSet = true;
    } 
    if(j.find("IMAGE_MALFORMED_URL") != j.end())
    {
        j.at("IMAGE_MALFORMED_URL").get_to(o.m_IMAGE_MALFORMED_URL);
        o.m_IMAGE_MALFORMED_URLIsSet = true;
    } 
    if(j.find("LARGE_PRODUCT_COUNT_DECREASE") != j.end())
    {
        j.at("LARGE_PRODUCT_COUNT_DECREASE").get_to(o.m_LARGE_PRODUCT_COUNT_DECREASE);
        o.m_LARGE_PRODUCT_COUNT_DECREASEIsSet = true;
    } 
    if(j.find("LINE_LEVEL_INTERNAL_ERROR") != j.end())
    {
        j.at("LINE_LEVEL_INTERNAL_ERROR").get_to(o.m_LINE_LEVEL_INTERNAL_ERROR);
        o.m_LINE_LEVEL_INTERNAL_ERRORIsSet = true;
    } 
    
}

int32_t CatalogsFeedIngestionErrors::getACCOUNTFLAGGED() const
{
    return m_ACCOUNT_FLAGGED;
}
void CatalogsFeedIngestionErrors::setACCOUNTFLAGGED(int32_t const value)
{
    m_ACCOUNT_FLAGGED = value;
    m_ACCOUNT_FLAGGEDIsSet = true;
}
bool CatalogsFeedIngestionErrors::aCCOUNTFLAGGEDIsSet() const
{
    return m_ACCOUNT_FLAGGEDIsSet;
}
void CatalogsFeedIngestionErrors::unsetACCOUNT_FLAGGED()
{
    m_ACCOUNT_FLAGGEDIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getFETCHGOOGLESHEETNOTSHARED() const
{
    return m_FETCH_GOOGLE_SHEET_NOT_SHARED;
}
void CatalogsFeedIngestionErrors::setFETCHGOOGLESHEETNOTSHARED(int32_t const value)
{
    m_FETCH_GOOGLE_SHEET_NOT_SHARED = value;
    m_FETCH_GOOGLE_SHEET_NOT_SHAREDIsSet = true;
}
bool CatalogsFeedIngestionErrors::fETCHGOOGLESHEETNOTSHAREDIsSet() const
{
    return m_FETCH_GOOGLE_SHEET_NOT_SHAREDIsSet;
}
void CatalogsFeedIngestionErrors::unsetFETCH_GOOGLE_SHEET_NOT_SHARED()
{
    m_FETCH_GOOGLE_SHEET_NOT_SHAREDIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getIMAGEFILENOTACCESSIBLE() const
{
    return m_IMAGE_FILE_NOT_ACCESSIBLE;
}
void CatalogsFeedIngestionErrors::setIMAGEFILENOTACCESSIBLE(int32_t const value)
{
    m_IMAGE_FILE_NOT_ACCESSIBLE = value;
    m_IMAGE_FILE_NOT_ACCESSIBLEIsSet = true;
}
bool CatalogsFeedIngestionErrors::iMAGEFILENOTACCESSIBLEIsSet() const
{
    return m_IMAGE_FILE_NOT_ACCESSIBLEIsSet;
}
void CatalogsFeedIngestionErrors::unsetIMAGE_FILE_NOT_ACCESSIBLE()
{
    m_IMAGE_FILE_NOT_ACCESSIBLEIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getIMAGEFILENOTFOUND() const
{
    return m_IMAGE_FILE_NOT_FOUND;
}
void CatalogsFeedIngestionErrors::setIMAGEFILENOTFOUND(int32_t const value)
{
    m_IMAGE_FILE_NOT_FOUND = value;
    m_IMAGE_FILE_NOT_FOUNDIsSet = true;
}
bool CatalogsFeedIngestionErrors::iMAGEFILENOTFOUNDIsSet() const
{
    return m_IMAGE_FILE_NOT_FOUNDIsSet;
}
void CatalogsFeedIngestionErrors::unsetIMAGE_FILE_NOT_FOUND()
{
    m_IMAGE_FILE_NOT_FOUNDIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getIMAGEINVALIDFILE() const
{
    return m_IMAGE_INVALID_FILE;
}
void CatalogsFeedIngestionErrors::setIMAGEINVALIDFILE(int32_t const value)
{
    m_IMAGE_INVALID_FILE = value;
    m_IMAGE_INVALID_FILEIsSet = true;
}
bool CatalogsFeedIngestionErrors::iMAGEINVALIDFILEIsSet() const
{
    return m_IMAGE_INVALID_FILEIsSet;
}
void CatalogsFeedIngestionErrors::unsetIMAGE_INVALID_FILE()
{
    m_IMAGE_INVALID_FILEIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getIMAGELEVELINTERNALERROR() const
{
    return m_IMAGE_LEVEL_INTERNAL_ERROR;
}
void CatalogsFeedIngestionErrors::setIMAGELEVELINTERNALERROR(int32_t const value)
{
    m_IMAGE_LEVEL_INTERNAL_ERROR = value;
    m_IMAGE_LEVEL_INTERNAL_ERRORIsSet = true;
}
bool CatalogsFeedIngestionErrors::iMAGELEVELINTERNALERRORIsSet() const
{
    return m_IMAGE_LEVEL_INTERNAL_ERRORIsSet;
}
void CatalogsFeedIngestionErrors::unsetIMAGE_LEVEL_INTERNAL_ERROR()
{
    m_IMAGE_LEVEL_INTERNAL_ERRORIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getIMAGEMALFORMEDURL() const
{
    return m_IMAGE_MALFORMED_URL;
}
void CatalogsFeedIngestionErrors::setIMAGEMALFORMEDURL(int32_t const value)
{
    m_IMAGE_MALFORMED_URL = value;
    m_IMAGE_MALFORMED_URLIsSet = true;
}
bool CatalogsFeedIngestionErrors::iMAGEMALFORMEDURLIsSet() const
{
    return m_IMAGE_MALFORMED_URLIsSet;
}
void CatalogsFeedIngestionErrors::unsetIMAGE_MALFORMED_URL()
{
    m_IMAGE_MALFORMED_URLIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getLARGEPRODUCTCOUNTDECREASE() const
{
    return m_LARGE_PRODUCT_COUNT_DECREASE;
}
void CatalogsFeedIngestionErrors::setLARGEPRODUCTCOUNTDECREASE(int32_t const value)
{
    m_LARGE_PRODUCT_COUNT_DECREASE = value;
    m_LARGE_PRODUCT_COUNT_DECREASEIsSet = true;
}
bool CatalogsFeedIngestionErrors::lARGEPRODUCTCOUNTDECREASEIsSet() const
{
    return m_LARGE_PRODUCT_COUNT_DECREASEIsSet;
}
void CatalogsFeedIngestionErrors::unsetLARGE_PRODUCT_COUNT_DECREASE()
{
    m_LARGE_PRODUCT_COUNT_DECREASEIsSet = false;
}
int32_t CatalogsFeedIngestionErrors::getLINELEVELINTERNALERROR() const
{
    return m_LINE_LEVEL_INTERNAL_ERROR;
}
void CatalogsFeedIngestionErrors::setLINELEVELINTERNALERROR(int32_t const value)
{
    m_LINE_LEVEL_INTERNAL_ERROR = value;
    m_LINE_LEVEL_INTERNAL_ERRORIsSet = true;
}
bool CatalogsFeedIngestionErrors::lINELEVELINTERNALERRORIsSet() const
{
    return m_LINE_LEVEL_INTERNAL_ERRORIsSet;
}
void CatalogsFeedIngestionErrors::unsetLINE_LEVEL_INTERNAL_ERROR()
{
    m_LINE_LEVEL_INTERNAL_ERRORIsSet = false;
}


} // namespace org::openapitools::server::model

