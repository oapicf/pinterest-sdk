/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * CatalogsHotelItemsBatch.h
 *
 * Object describing the catalogs hotel items batch. If specified, you must provide all properties.
 */

#ifndef CatalogsHotelItemsBatch_H_
#define CatalogsHotelItemsBatch_H_


#include <string>
#include "HotelProcessingRecord.h"
#include <vector>
#include "BatchOperationStatus.h"
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// Object describing the catalogs hotel items batch. If specified, you must provide all properties.
/// </summary>
class  CatalogsHotelItemsBatch
{
public:
    CatalogsHotelItemsBatch();
    virtual ~CatalogsHotelItemsBatch();


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const CatalogsHotelItemsBatch& rhs) const;
    bool operator!=(const CatalogsHotelItemsBatch& rhs) const;

    /////////////////////////////////////////////
    /// CatalogsHotelItemsBatch members

    /// <summary>
    /// Id of the catalogs items batch
    /// </summary>
    std::string getBatchId() const;
    void setBatchId(std::string const& value);
    bool batchIdIsSet() const;
    void unsetBatch_id();
    /// <summary>
    /// 
    /// </summary>
    std::string getCatalogType() const;
    void setCatalogType(std::string const& value);
    /// <summary>
    /// Date and time (UTC) of the batch completion: YYYY-MM-DD&#39;T&#39;hh:mm:ss
    /// </summary>
    std::string getCompletedTime() const;
    void setCompletedTime(std::string const& value);
    bool completedTimeIsSet() const;
    void unsetCompleted_time();
    /// <summary>
    /// Date and time (UTC) of the batch creation: YYYY-MM-DD&#39;T&#39;hh:mm:ss
    /// </summary>
    std::string getCreatedTime() const;
    void setCreatedTime(std::string const& value);
    bool createdTimeIsSet() const;
    void unsetCreated_time();
    /// <summary>
    /// Array with the catalogs items processing records part of the catalogs items batch
    /// </summary>
    std::vector<org::openapitools::server::model::HotelProcessingRecord> getItems() const;
    void setItems(std::vector<org::openapitools::server::model::HotelProcessingRecord> const& value);
    bool itemsIsSet() const;
    void unsetItems();
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::BatchOperationStatus getStatus() const;
    void setStatus(org::openapitools::server::model::BatchOperationStatus const& value);
    bool statusIsSet() const;
    void unsetStatus();

    friend  void to_json(nlohmann::json& j, const CatalogsHotelItemsBatch& o);
    friend  void from_json(const nlohmann::json& j, CatalogsHotelItemsBatch& o);
protected:
    std::string m_Batch_id;
    bool m_Batch_idIsSet;
    std::string m_Catalog_type;

    std::string m_Completed_time;
    bool m_Completed_timeIsSet;
    std::string m_Created_time;
    bool m_Created_timeIsSet;
    std::vector<org::openapitools::server::model::HotelProcessingRecord> m_Items;
    bool m_ItemsIsSet;
    org::openapitools::server::model::BatchOperationStatus m_Status;
    bool m_StatusIsSet;
    
};

} // namespace org::openapitools::server::model

#endif /* CatalogsHotelItemsBatch_H_ */
