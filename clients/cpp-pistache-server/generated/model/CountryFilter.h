/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * CountryFilter.h
 *
 * 
 */

#ifndef CountryFilter_H_
#define CountryFilter_H_


#include "CatalogsProductGroupMultipleCountriesCriteria.h"
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// 
/// </summary>
class  CountryFilter
{
public:
    CountryFilter();
    virtual ~CountryFilter();


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const CountryFilter& rhs) const;
    bool operator!=(const CountryFilter& rhs) const;

    /////////////////////////////////////////////
    /// CountryFilter members

    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::CatalogsProductGroupMultipleCountriesCriteria getCOUNTRY() const;
    void setCOUNTRY(org::openapitools::server::model::CatalogsProductGroupMultipleCountriesCriteria const& value);

    friend  void to_json(nlohmann::json& j, const CountryFilter& o);
    friend  void from_json(const nlohmann::json& j, CountryFilter& o);
protected:
    org::openapitools::server::model::CatalogsProductGroupMultipleCountriesCriteria m_COUNTRY;

    
};

} // namespace org::openapitools::server::model

#endif /* CountryFilter_H_ */
