...
class TrendsApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val trendsEditorialArticlesListPerSecond = config.getDouble("performance.operationsPerSecond.trendsEditorialArticlesList") * rateMultiplier * instanceMultiplier
    ...
    // Set up CSV feeders
    val trends_editorial_articles/listQUERYFeeder = csv(userDataDirectory + File.separator + "trendsEditorialArticlesList-queryParams.csv").random
    ...
    val scntrendsEditorialArticlesList = scenario("trendsEditorialArticlesListSimulation")
        .feed(trends_editorial_articles/listQUERYFeeder)
        .exec(http("trendsEditorialArticlesList")
        .httpRequest("GET","/trends/editorial_articles")
        .queryParam("region","${region}")
)

    // Run scntrendsEditorialArticlesList with warm up and reach a constant rate for entire duration
    scenarioBuilders += scntrendsEditorialArticlesList.inject(
        rampUsersPerSec(1) to(trendsEditorialArticlesListPerSecond) during(rampUpSeconds),
        constantUsersPerSec(trendsEditorialArticlesListPerSecond) during(durationSeconds),
        rampUsersPerSec(trendsEditorialArticlesListPerSecond) to(1) during(rampDownSeconds)
    )
