...
class LabelsApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val scnlabelsCreate = scenario("labelsCreateSimulation")
        .feed(labels/createPATHFeeder)
        .exec(http("labelsCreate")
        .httpRequest("POST","/ad_accounts/${ad_account_id}/labels")
)

    // Run scnlabelsCreate with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnlabelsCreate.inject(
        rampUsersPerSec(1) to(labelsCreatePerSecond) during(rampUpSeconds),
        constantUsersPerSec(labelsCreatePerSecond) during(durationSeconds),
        rampUsersPerSec(labelsCreatePerSecond) to(1) during(rampDownSeconds)
    )

    
    val scnlabelsList = scenario("labelsListSimulation")
        .feed(labels/listQUERYFeeder)
    ...
    scenarioBuilders += scnlabelsList.inject(
    ...
    val scnlabelsUpdate = scenario("labelsUpdateSimulation")
        .feed(labels/updatePATHFeeder)
        .exec(http("labelsUpdate")
        .httpRequest("PATCH","/ad_accounts/${ad_account_id}/labels")
)

    // Run scnlabelsUpdate with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnlabelsUpdate.inject(
        rampUsersPerSec(1) to(labelsUpdatePerSecond) during(rampUpSeconds),
        constantUsersPerSec(labelsUpdatePerSecond) during(durationSeconds),
        rampUsersPerSec(labelsUpdatePerSecond) to(1) during(rampDownSeconds)
    )
