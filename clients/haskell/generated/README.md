...
main = do
  ...
  let PinterestRESTBackend{..} = API.createPinterestRESTClient
...
Additionally, you have to provide value for the `PinterestRESTAuth` type provided by the
`PinterestREST.API` module:

```
auth :: PinterestRESTAuth
auth =
  PinterestRESTAuth
...
```
runPinterestRESTMiddlewareServer config requestMiddlewares auth server
```
