# TargetingSpec

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**AGEUnderscoreBUCKET** | [**array[TargetingSpecAgeBucket]**](TargetingSpecAgeBucket.md) |  | [optional] [default to null]
**APPTYPE** | [**array[TargetingSpecAppType]**](TargetingSpecAppType.md) |  | [optional] [default to null]
**AUDIENCEUnderscoreEXCLUDE** | **array[string]** |  | [optional] [default to null]
**AUDIENCEUnderscoreINCLUDE** | **array[string]** |  | [optional] [default to null]
**GENDER** | [**array[TargetingSpecGender]**](TargetingSpecGender.md) |  | [optional] [default to null]
**GEO** | **array[string]** |  | [optional] [default to null]
**GEOUnderscoreEXCLUDE** | **array[string]** |  | [optional] [default to null]
**INTEREST** | **array[string]** |  | [optional] [default to null]
**LOCALE** | **array[string]** |  | [optional] [default to null]
**LOCATION** | **array[string]** |  | [optional] [default to null]
**LOCATIONUnderscoreEXCLUDE** | **array[string]** |  | [optional] [default to null]
**MAXIMUMUnderscoreAGE** | **string** |  | [optional] [default to null]
**MINIMUMUnderscoreAGE** | **string** |  | [optional] [default to null]
**SHOPPINGUnderscoreRETARGETING** | [**array[TargetingSpecShoppingRetargeting]**](TargetingSpecShoppingRetargeting.md) |  | [optional] [default to null]
**TARGETINGUnderscoreSTRATEGY** | **array[string]** |  | [optional] [default to null]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


