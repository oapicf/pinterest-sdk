...
void PinsResource::handler_GET_internal(const std::shared_ptr<restbed::Session> session) {
    ...
    try {
        std::tie(status_code, resultObject) =
            handler_GET(pinFilter, pinMetrics, includeProtectedPins, pinType, creativeTypes, adAccountId, bookmark, pageSize);
    }
    catch(const PinsApiException& e) {
        std::tie(status_code, result) = handlePinsApiException(e);
    }
...
{
    return handler_GET_func(pinFilter, pinMetrics, includeProtectedPins, pinType, creativeTypes, adAccountId, bookmark, pageSize);
}
