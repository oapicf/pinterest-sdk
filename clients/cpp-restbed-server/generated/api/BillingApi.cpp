...
void Ad_accountsAd_account_idBilling_invoicesResource::handler_GET_internal(const std::shared_ptr<restbed::Session> session)
{
    ...
    std::string bookmark = request->get_query_parameter("bookmark", "");
    int32_t pageSize = request->get_query_parameter("pageSize", 25);
    std::string sort = request->get_query_parameter("sort", "DUE_DATE");
    std::string order = request->get_query_parameter("order", "");
    std::string status = request->get_query_parameter("status", "");
    std::string documentType = request->get_query_parameter("documentType", "");
    ...
    try {
        std::tie(status_code, resultObject) =
            handler_GET(adAccountId, bookmark, pageSize, sort, order, status, documentType, startDueDate, endDueDate);
    }
    catch(const BillingApiException& e) {
        std::tie(status_code, result) = handleBillingApiException(e);
    }
...
std::pair<int, Billing_invoices_get_200_response> Ad_accountsAd_account_idBilling_invoicesResource::handler_GET(
        std::string & adAccountId, std::string & bookmark, int32_t & pageSize, std::string & sort, std::string & order, std::string & status, std::string & documentType, std::string & startDueDate, std::string & endDueDate)
{
    return handler_GET_func(adAccountId, bookmark, pageSize, sort, order, status, documentType, startDueDate, endDueDate);
}
