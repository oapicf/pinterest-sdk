/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * CatalogsFeedUpdateRequestSchema.h
 *
 * 
 */

#ifndef CatalogsFeedUpdateRequestSchema_H_
#define CatalogsFeedUpdateRequestSchema_H_



#include "ProductAvailabilityType.h"
#include "CatalogsVerticalFeedsUpdateRequest.h"
#include "CatalogsFeedProcessingSchedule.h"
#include "CatalogsFeedCredentials.h"
#include <string>
#include "CatalogsStatus.h"
#include "CatalogsFeedsUpdateRequest.h"
#include "CatalogsFormat.h"
#include "NullableCurrency.h"
#include <memory>
#include <vector>
#include <array>
#include <boost/property_tree/ptree.hpp>
#include "CatalogsVerticalFeedsUpdateRequest.h"
#include "CatalogsFeedsUpdateRequest.h"
#include "helpers.h"

namespace org {
namespace openapitools {
namespace server {
namespace model {

/// <summary>
/// 
/// </summary>
class  CatalogsFeedUpdateRequestSchema : public CatalogsVerticalFeedsUpdateRequest, public CatalogsFeedsUpdateRequest
{
public:
    CatalogsFeedUpdateRequestSchema() = default;
    explicit CatalogsFeedUpdateRequestSchema(boost::property_tree::ptree const& pt);
    virtual ~CatalogsFeedUpdateRequestSchema();

    CatalogsFeedUpdateRequestSchema(const CatalogsFeedUpdateRequestSchema& other) = default; // copy constructor
    CatalogsFeedUpdateRequestSchema(CatalogsFeedUpdateRequestSchema&& other) noexcept = default; // move constructor

    CatalogsFeedUpdateRequestSchema& operator=(const CatalogsFeedUpdateRequestSchema& other) = default; // copy assignment
    CatalogsFeedUpdateRequestSchema& operator=(CatalogsFeedUpdateRequestSchema&& other) noexcept = default; // move assignment

    std::string toJsonString(bool prettyJson = false) const;
    void fromJsonString(std::string const& jsonString);
    boost::property_tree::ptree toPropertyTree() const;
    void fromPropertyTree(boost::property_tree::ptree const& pt);


    /////////////////////////////////////////////
    /// CatalogsFeedUpdateRequestSchema members

    /// <summary>
    /// 
    /// </summary>
    std::string getCatalogType() const;
    void setCatalogType(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFeedCredentials getCredentials() const;
    void setCredentials(CatalogsFeedCredentials value);

    /// <summary>
    /// 
    /// </summary>
    ProductAvailabilityType getDefaultAvailability() const;
    void setDefaultAvailability(ProductAvailabilityType value);

    /// <summary>
    /// 
    /// </summary>
    NullableCurrency getDefaultCurrency() const;
    void setDefaultCurrency(NullableCurrency value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFormat getFormat() const;
    void setFormat(CatalogsFormat value);

    /// <summary>
    /// The URL where a feed is available for download. This URL is what Pinterest will use to download a feed for processing.
    /// </summary>
    std::string getLocation() const;
    void setLocation(std::string value);

    /// <summary>
    /// A human-friendly name associated to a given feed.
    /// </summary>
    std::string getName() const;
    void setName(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFeedProcessingSchedule getPreferredProcessingSchedule() const;
    void setPreferredProcessingSchedule(CatalogsFeedProcessingSchedule value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsStatus getStatus() const;
    void setStatus(CatalogsStatus value);

protected:
    std::string m_Catalog_type = "";
    CatalogsFeedCredentials m_Credentials;
    ProductAvailabilityType m_Default_availability = ProductAvailabilityType{};
    NullableCurrency m_Default_currency = NullableCurrency{};
    CatalogsFormat m_Format = CatalogsFormat{};
    std::string m_Location = "";
    std::string m_Name = "";
    CatalogsFeedProcessingSchedule m_Preferred_processing_schedule;
    CatalogsStatus m_Status = CatalogsStatus{};
};

std::vector<CatalogsFeedUpdateRequestSchema> createCatalogsFeedUpdateRequestSchemaVectorFromJsonString(const std::string& json);

template<>
inline boost::property_tree::ptree toPt<CatalogsFeedUpdateRequestSchema>(const CatalogsFeedUpdateRequestSchema& val) {
    return val.toPropertyTree();
}

template<>
inline CatalogsFeedUpdateRequestSchema fromPt<CatalogsFeedUpdateRequestSchema>(const boost::property_tree::ptree& pt) {
    CatalogsFeedUpdateRequestSchema ret;
    ret.fromPropertyTree(pt);
    return ret;
}

}
}
}
}

#endif /* CatalogsFeedUpdateRequestSchema_H_ */
