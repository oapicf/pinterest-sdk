...
ReportingColumnSync::ReportingColumnSync(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}
...
ptree ReportingColumnSync::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void ReportingColumnSync::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string ReportingColumnSync::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void ReportingColumnSync::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string ReportingColumnSync::getEnumValue() const {
    return m_ReportingColumnSyncEnumValue;
}

void ReportingColumnSync::setEnumValue(const std::string& val) {
    static const std::array<std::string, 185> allowedValues = {
    ...
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_ReportingColumnSyncEnumValue = val;
    } else {
...
std::vector<ReportingColumnSync> createReportingColumnSyncVectorFromJsonString(const std::string& json)
{
    ...
    auto vec = std::vector<ReportingColumnSync>();
    for (const auto& child: pt) {
        vec.emplace_back(ReportingColumnSync(child.second));
    }
