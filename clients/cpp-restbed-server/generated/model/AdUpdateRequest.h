/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * AdUpdateRequest.h
 *
 * 
 */

#ifndef AdUpdateRequest_H_
#define AdUpdateRequest_H_



#include "CustomizableCTAType.h"
#include "CreativeType.h"
#include "EntityStatus.h"
#include <string>
#include "GridClickType.h"
#include "DisclosureType.h"
#include <vector>
#include <memory>
#include <vector>
#include <array>
#include <boost/property_tree/ptree.hpp>
#include "helpers.h"

namespace org {
namespace openapitools {
namespace server {
namespace model {

/// <summary>
/// 
/// </summary>
class  AdUpdateRequest 
{
public:
    AdUpdateRequest() = default;
    explicit AdUpdateRequest(boost::property_tree::ptree const& pt);
    virtual ~AdUpdateRequest();

    AdUpdateRequest(const AdUpdateRequest& other) = default; // copy constructor
    AdUpdateRequest(AdUpdateRequest&& other) noexcept = default; // move constructor

    AdUpdateRequest& operator=(const AdUpdateRequest& other) = default; // copy assignment
    AdUpdateRequest& operator=(AdUpdateRequest&& other) noexcept = default; // move assignment

    std::string toJsonString(bool prettyJson = false) const;
    void fromJsonString(std::string const& jsonString);
    boost::property_tree::ptree toPropertyTree() const;
    void fromPropertyTree(boost::property_tree::ptree const& pt);


    /////////////////////////////////////////////
    /// AdUpdateRequest members

    /// <summary>
    /// The ID of this ad.
    /// </summary>
    std::string getId() const;
    void setId(std::string value);

    /// <summary>
    /// Pin ID. This field may only be updated for draft ads.
    /// </summary>
    std::string getPinId() const;
    void setPinId(std::string value);

    /// <summary>
    /// ID of the ad group that contains the ad.
    /// </summary>
    std::string getAdGroupId() const;
    void setAdGroupId(std::string value);

    /// <summary>
    /// Deep link URL for Android devices.
    /// </summary>
    std::string getAndroidDeepLink() const;
    void setAndroidDeepLink(std::string value);

    /// <summary>
    /// Comma-separated deep links for the carousel pin on Android.
    /// </summary>
    std::vector<std::string> getCarouselAndroidDeepLinks() const;
    void setCarouselAndroidDeepLinks(std::vector<std::string> value);

    /// <summary>
    /// Comma-separated destination URLs for the carousel pin to promote.
    /// </summary>
    std::vector<std::string> getCarouselDestinationUrls() const;
    void setCarouselDestinationUrls(std::vector<std::string> value);

    /// <summary>
    /// Comma-separated deep links for the carousel pin on iOS.
    /// </summary>
    std::vector<std::string> getCarouselIosDeepLinks() const;
    void setCarouselIosDeepLinks(std::vector<std::string> value);

    /// <summary>
    /// Tracking url for the ad clicks.
    /// </summary>
    std::string getClickTrackingUrl() const;
    void setClickTrackingUrl(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CreativeType getCreativeType() const;
    void setCreativeType(CreativeType value);

    /// <summary>
    /// 
    /// </summary>
    CustomizableCTAType getCustomizableCtaType() const;
    void setCustomizableCtaType(CustomizableCTAType value);

    /// <summary>
    /// Destination URL.
    /// </summary>
    std::string getDestinationUrl() const;
    void setDestinationUrl(std::string value);

    /// <summary>
    /// 
    /// </summary>
    DisclosureType getDisclosureType() const;
    void setDisclosureType(DisclosureType value);

    /// <summary>
    /// URL for a page that provides disclosures about a pharmaceutical product, such as potential side effects. Make sure the URL takes the user directly to the disclosure content and the referenced site is secure.
    /// </summary>
    std::string getDisclosureUrl() const;
    void setDisclosureUrl(std::string value);

    /// <summary>
    /// 
    /// </summary>
    GridClickType getGridClickType() const;
    void setGridClickType(GridClickType value);

    /// <summary>
    /// Deep link URL for iOS devices.
    /// </summary>
    std::string getIosDeepLink() const;
    void setIosDeepLink(std::string value);

    /// <summary>
    /// Is the ad a carting/WTB ad?
    /// </summary>
    bool isIsCarting() const;
    void setIsCarting(bool value);

    /// <summary>
    /// Is original pin deleted?
    /// </summary>
    bool isIsPinDeleted() const;
    void setIsPinDeleted(bool value);

    /// <summary>
    /// Is pin repinnable?
    /// </summary>
    bool isIsRemovable() const;
    void setIsRemovable(bool value);

    /// <summary>
    /// Lead form ID for lead ad generation.
    /// </summary>
    std::string getLeadFormId() const;
    void setLeadFormId(std::string value);

    /// <summary>
    /// Name of the ad - 255 chars max.
    /// </summary>
    std::string getName() const;
    void setName(std::string value);

    /// <summary>
    /// Before creating a quiz ad, you must create an organic Pin using POST/Create Pin for each result in the quiz. Quiz ads cannot be saved by a Pinner. Quiz ad results can be saved.
    /// </summary>
    std::string getQuizPinData() const;
    void setQuizPinData(std::string value);

    /// <summary>
    /// 
    /// </summary>
    EntityStatus getStatus() const;
    void setStatus(EntityStatus value);

    /// <summary>
    /// 
    /// </summary>
    std::string getTrackingUrls() const;
    void setTrackingUrls(std::string value);

    /// <summary>
    /// Tracking URL for ad impressions.
    /// </summary>
    std::string getViewTrackingUrl() const;
    void setViewTrackingUrl(std::string value);

protected:
    std::string m_Id = "";
    std::string m_Pin_id = "";
    std::string m_Ad_group_id = "";
    std::string m_Android_deep_link = "";
    std::vector<std::string> m_Carousel_android_deep_links;
    std::vector<std::string> m_Carousel_destination_urls;
    std::vector<std::string> m_Carousel_ios_deep_links;
    std::string m_Click_tracking_url = "";
    CreativeType m_Creative_type = CreativeType{};
    CustomizableCTAType m_Customizable_cta_type = CustomizableCTAType{};
    std::string m_Destination_url = "";
    DisclosureType m_Disclosure_type = DisclosureType{};
    std::string m_Disclosure_url = "";
    GridClickType m_Grid_click_type = GridClickType{};
    std::string m_Ios_deep_link = "";
    bool m_Is_carting = false;
    bool m_Is_pin_deleted = false;
    bool m_Is_removable = false;
    std::string m_Lead_form_id = "";
    std::string m_Name = "";
    std::string m_Quiz_pin_data = nullptr;
    EntityStatus m_Status = EntityStatus{};
    std::string m_Tracking_urls = nullptr;
    std::string m_View_tracking_url = "";
};

std::vector<AdUpdateRequest> createAdUpdateRequestVectorFromJsonString(const std::string& json);

template<>
inline boost::property_tree::ptree toPt<AdUpdateRequest>(const AdUpdateRequest& val) {
    return val.toPropertyTree();
}

template<>
inline AdUpdateRequest fromPt<AdUpdateRequest>(const boost::property_tree::ptree& pt) {
    AdUpdateRequest ret;
    ret.fromPropertyTree(pt);
    return ret;
}

}
}
}
}

#endif /* AdUpdateRequest_H_ */
