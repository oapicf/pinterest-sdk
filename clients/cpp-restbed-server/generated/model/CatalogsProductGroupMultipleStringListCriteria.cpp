...
ptree CatalogsProductGroupMultipleStringListCriteria::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
...
void CatalogsProductGroupMultipleStringListCriteria::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	m_Negated = pt.get("negated", false);
	// push all items of Values into member
	if (pt.get_child_optional("values")) {
        m_Values = fromPt<std::vector<std::vector>>(pt.get_child("values"));
	}
}
...
void CatalogsProductGroupMultipleStringListCriteria::setValues(std::vector<std::vector> value)
{
    m_Values = value;
}
