...
ProductCategoryEnum::ProductCategoryEnum(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}
...
ptree ProductCategoryEnum::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void ProductCategoryEnum::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string ProductCategoryEnum::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void ProductCategoryEnum::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string ProductCategoryEnum::getEnumValue() const {
    return m_ProductCategoryEnumEnumValue;
}

void ProductCategoryEnum::setEnumValue(const std::string& val) {
    static const std::array<std::string, 383> allowedValues = {
    ...
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_ProductCategoryEnumEnumValue = val;
    } else {
...
std::vector<ProductCategoryEnum> createProductCategoryEnumVectorFromJsonString(const std::string& json)
{
    ...
    auto vec = std::vector<ProductCategoryEnum>();
    for (const auto& child: pt) {
        vec.emplace_back(ProductCategoryEnum(child.second));
    }
