/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * CatalogsHotelFeed.h
 *
 * Catalogs Hotel Feed object
 */

#ifndef CatalogsHotelFeed_H_
#define CatalogsHotelFeed_H_



#include "CatalogsFeedProcessingSchedule.h"
#include "CatalogsFeedCredentials.h"
#include <string>
#include "CatalogsStatus.h"
#include "CatalogsFormat.h"
#include "NullableCurrency.h"
#include <memory>
#include <vector>
#include <array>
#include <boost/property_tree/ptree.hpp>
#include "helpers.h"

namespace org {
namespace openapitools {
namespace server {
namespace model {

/// <summary>
/// Catalogs Hotel Feed object
/// </summary>
class  CatalogsHotelFeed 
{
public:
    CatalogsHotelFeed() = default;
    explicit CatalogsHotelFeed(boost::property_tree::ptree const& pt);
    virtual ~CatalogsHotelFeed();

    CatalogsHotelFeed(const CatalogsHotelFeed& other) = default; // copy constructor
    CatalogsHotelFeed(CatalogsHotelFeed&& other) noexcept = default; // move constructor

    CatalogsHotelFeed& operator=(const CatalogsHotelFeed& other) = default; // copy assignment
    CatalogsHotelFeed& operator=(CatalogsHotelFeed&& other) noexcept = default; // move assignment

    std::string toJsonString(bool prettyJson = false) const;
    void fromJsonString(std::string const& jsonString);
    boost::property_tree::ptree toPropertyTree() const;
    void fromPropertyTree(boost::property_tree::ptree const& pt);


    /////////////////////////////////////////////
    /// CatalogsHotelFeed members

    /// <summary>
    /// Catalog id pertaining to the feed. If not provided, feed will use a default catalog based on type.
    /// </summary>
    std::string getCatalogId() const;
    void setCatalogId(std::string value);

    /// <summary>
    /// 
    /// </summary>
    std::string getCatalogType() const;
    void setCatalogType(std::string value);

    /// <summary>
    /// 
    /// </summary>
    std::string getCreatedAt() const;
    void setCreatedAt(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFeedCredentials getCredentials() const;
    void setCredentials(CatalogsFeedCredentials value);

    /// <summary>
    /// 
    /// </summary>
    NullableCurrency getDefaultCurrency() const;
    void setDefaultCurrency(NullableCurrency value);

    /// <summary>
    /// The locale used within a feed for product descriptions.
    /// </summary>
    std::string getDefaultLocale() const;
    void setDefaultLocale(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFormat getFormat() const;
    void setFormat(CatalogsFormat value);

    /// <summary>
    /// ID of the feed entity.
    /// </summary>
    std::string getId() const;
    void setId(std::string value);

    /// <summary>
    /// The URL where a feed is available for download. This URL is what Pinterest will use to download a feed for processing.
    /// </summary>
    std::string getLocation() const;
    void setLocation(std::string value);

    /// <summary>
    /// A human-friendly name associated to a given feed. This value is currently nullable due to historical reasons. It is expected to become non-nullable in the future.
    /// </summary>
    std::string getName() const;
    void setName(std::string value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsFeedProcessingSchedule getPreferredProcessingSchedule() const;
    void setPreferredProcessingSchedule(CatalogsFeedProcessingSchedule value);

    /// <summary>
    /// 
    /// </summary>
    CatalogsStatus getStatus() const;
    void setStatus(CatalogsStatus value);

    /// <summary>
    /// 
    /// </summary>
    std::string getUpdatedAt() const;
    void setUpdatedAt(std::string value);

protected:
    std::string m_Catalog_id = "";
    std::string m_Catalog_type = "";
    std::string m_Created_at = "";
    CatalogsFeedCredentials m_Credentials;
    NullableCurrency m_Default_currency = NullableCurrency{};
    std::string m_Default_locale = "";
    CatalogsFormat m_Format = CatalogsFormat{};
    std::string m_Id = "";
    std::string m_Location = "";
    std::string m_Name = "";
    CatalogsFeedProcessingSchedule m_Preferred_processing_schedule;
    CatalogsStatus m_Status = CatalogsStatus{};
    std::string m_Updated_at = "";
};

std::vector<CatalogsHotelFeed> createCatalogsHotelFeedVectorFromJsonString(const std::string& json);

template<>
inline boost::property_tree::ptree toPt<CatalogsHotelFeed>(const CatalogsHotelFeed& val) {
    return val.toPropertyTree();
}

template<>
inline CatalogsHotelFeed fromPt<CatalogsHotelFeed>(const boost::property_tree::ptree& pt) {
    CatalogsHotelFeed ret;
    ret.fromPropertyTree(pt);
    return ret;
}

}
}
}
}

#endif /* CatalogsHotelFeed_H_ */
