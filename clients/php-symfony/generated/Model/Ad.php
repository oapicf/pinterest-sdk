<?php
/**
 * Ad
 *
 * PHP version 8.1.1
 *
 * @category Class
 * @package  OpenAPI\Server\Model
 * @author   OpenAPI Generator team
 * @link     https://github.com/openapitools/openapi-generator
 */

/**
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 *
 */

/**
 * NOTE: This class is auto generated by the openapi generator program.
 * https://github.com/openapitools/openapi-generator
 * Do not edit the class manually.
 */

namespace OpenAPI\Server\Model;

use Symfony\Component\Validator\Constraints as Assert;
use JMS\Serializer\Annotation\Type;
use JMS\Serializer\Annotation\Accessor;
use JMS\Serializer\Annotation\SerializedName;

/**
 * Class representing the Ad model.
 *
 * @package OpenAPI\Server\Model
 * @author  OpenAPI Generator team
 */

class Ad 
{
        /**
     * The ID of the advertiser that this ad belongs to.
     *
     * @var string|null
     * @SerializedName("ad_account_id")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    #[Assert\Regex("/^\\d+$/")]
    protected ?string $adAccountId = null;

    /**
     * ID of the ad group that contains the ad.
     *
     * @var string|null
     * @SerializedName("ad_group_id")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    #[Assert\Regex("/^(AG)?\\d+$/")]
    protected ?string $adGroupId = null;

    /**
     * Deep link URL for Android devices.
     *
     * @var string|null
     * @SerializedName("android_deep_link")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $androidDeepLink = null;

    /**
     * ID of the ad campaign that contains this ad.
     *
     * @var string|null
     * @SerializedName("campaign_id")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    #[Assert\Regex("/^\\d+$/")]
    protected ?string $campaignId = null;

    /**
     * Comma-separated deep links for the carousel pin on Android.
     *
     * @var string[]|null
     * @SerializedName("carousel_android_deep_links")
     * @Type("array<string>")
     */
    #[Assert\All([
        new Assert\Type("string"),
    ])]
    protected ?array $carouselAndroidDeepLinks = null;

    /**
     * Comma-separated destination URLs for the carousel pin to promote.
     *
     * @var string[]|null
     * @SerializedName("carousel_destination_urls")
     * @Type("array<string>")
     */
    #[Assert\All([
        new Assert\Type("string"),
    ])]
    protected ?array $carouselDestinationUrls = null;

    /**
     * Comma-separated deep links for the carousel pin on iOS.
     *
     * @var string[]|null
     * @SerializedName("carousel_ios_deep_links")
     * @Type("array<string>")
     */
    #[Assert\All([
        new Assert\Type("string"),
    ])]
    protected ?array $carouselIosDeepLinks = null;

    /**
     * The vendor platform type of the carting/WTB ad.
     *
     * @var int|null
     * @SerializedName("carting_platform_type")
     * @Type("int")
     */
    #[Assert\Type("int")]
    #[Assert\GreaterThanOrEqual(1)]
    #[Assert\LessThanOrEqual(2)]
    protected ?int $cartingPlatformType = null;

    /**
     * Array of carting/WTB products for the ad.
     *
     * @var CartingProduct[]|null
     * @SerializedName("carting_products")
     * @Type("array<OpenAPI\Server\Model\CartingProduct>")
     */
    #[Assert\All([
        new Assert\Type("OpenAPI\Server\Model\CartingProduct"),
    ])]
    protected ?array $cartingProducts = null;

    /**
     * Tracking url for the ad clicks.
     *
     * @var string|null
     * @SerializedName("click_tracking_url")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $clickTrackingUrl = null;

    /**
     * Destination URL template for all items within a collections drawer.
     *
     * @var string|null
     * @SerializedName("collection_items_destination_url_template")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $collectionItemsDestinationUrlTemplate = null;

    /**
     * @var AdCollectionsHeaderType|null
     * @SerializedName("collections_header_type")
     * @Accessor(getter="getSerializedCollectionsHeaderType", setter="setDeserializedCollectionsHeaderType")
     * @Type("string")
     */
    protected ?AdCollectionsHeaderType $collectionsHeaderType = null;

    /**
     * Pin creation time. Unix timestamp in seconds.
     *
     * @var int|null
     * @SerializedName("created_time")
     * @Type("int")
     */
    #[Assert\Type("int")]
    protected ?int $createdTime = null;

    /**
     * @var CreativeType|null
     * @SerializedName("creative_type")
     * @Accessor(getter="getSerializedCreativeType", setter="setDeserializedCreativeType")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Valid]
    protected ?CreativeType $creativeType = null;

    /**
     * @var CustomizableCTAType|null
     * @SerializedName("customizable_cta_type")
     * @Accessor(getter="getSerializedCustomizableCtaType", setter="setDeserializedCustomizableCtaType")
     * @Type("string")
     */
    protected ?CustomizableCTAType $customizableCtaType = null;

    /**
     * Destination URL.
     *
     * @var string|null
     * @SerializedName("destination_url")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $destinationUrl = null;

    /**
     * @var DisclosureType|null
     * @SerializedName("disclosure_type")
     * @Accessor(getter="getSerializedDisclosureType", setter="setDeserializedDisclosureType")
     * @Type("string")
     */
    protected ?DisclosureType $disclosureType = null;

    /**
     * URL for a page that provides disclosures about a pharmaceutical product, such as potential side effects. Make sure the URL takes the user directly to the disclosure content and the referenced site is secure.
     *
     * @var string|null
     * @SerializedName("disclosure_url")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $disclosureUrl = null;

    /**
     * @var GridClickType|null
     * @SerializedName("grid_click_type")
     * @Accessor(getter="getSerializedGridClickType", setter="setDeserializedGridClickType")
     * @Type("string")
     */
    protected ?GridClickType $gridClickType = null;

    /**
     * The ID of this ad.
     *
     * @var string|null
     * @SerializedName("id")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    #[Assert\Length(max: 18)]
    #[Assert\Regex("/^\\d+$/")]
    protected ?string $id = null;

    /**
     * Deep link URL for iOS devices.
     *
     * @var string|null
     * @SerializedName("ios_deep_link")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $iosDeepLink = null;

    /**
     * Is the ad a carting/WTB ad?
     *
     * @var bool|null
     * @SerializedName("is_carting")
     * @Type("bool")
     */
    #[Assert\Type("bool")]
    protected ?bool $isCarting = null;

    /**
     * Whether the advertiser has accepted the terms and conditions for collage ad.
     *
     * @var bool|null
     * @SerializedName("is_collage_accepted_terms")
     * @Type("bool")
     */
    #[Assert\Type("bool")]
    protected ?bool $isCollageAcceptedTerms = null;

    /**
     * Whether the collage ad has a single destination url override.
     *
     * @var bool|null
     * @SerializedName("is_collage_single_destination")
     * @Type("bool")
     */
    #[Assert\Type("bool")]
    protected ?bool $isCollageSingleDestination = null;

    /**
     * Is original pin deleted?
     *
     * @var bool|null
     * @SerializedName("is_pin_deleted")
     * @Type("bool")
     */
    #[Assert\Type("bool")]
    protected ?bool $isPinDeleted = null;

    /**
     * Is pin repinnable?
     *
     * @var bool|null
     * @SerializedName("is_removable")
     * @Type("bool")
     */
    #[Assert\Type("bool")]
    protected ?bool $isRemovable = null;

    /**
     * Lead form ID for lead ad generation.
     *
     * @var string|null
     * @SerializedName("lead_form_id")
     * @Type("string")
     */
    #[Assert\Type("string")]
    #[Assert\Regex("/^(AG)?\\d+$/")]
    protected ?string $leadFormId = null;

    /**
     * Name of the ad - 255 chars max.
     *
     * @var string|null
     * @SerializedName("name")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $name = null;

    /**
     * Pin ID. This field may only be updated for draft ads.
     *
     * @var string|null
     * @SerializedName("pin_id")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    #[Assert\Regex("/^\\d+$/")]
    protected ?string $pinId = null;

    /**
     * Before creating a quiz ad, you must create an organic Pin using POST/Create Pin for each result in the quiz. Quiz ads cannot be saved by a Pinner. Quiz ad results can be saved.
     *
     * @var QuizPinData|null
     * @SerializedName("quiz_pin_data")
     * @Type("OpenAPI\Server\Model\QuizPinData")
     */
    #[Assert\Type("OpenAPI\Server\Model\QuizPinData")]
    protected ?QuizPinData $quizPinData = null;

    /**
     * Enum reason why the pin was rejected. Returned if &#x60;review_status&#x60; is \&quot;REJECTED\&quot;.
     *
     * @var AdDisapprovalReasons[]|null
     * @SerializedName("rejected_reasons")
     * @Accessor(getter="getSerializedRejectedReasons", setter="setDeserializedRejectedReasons")
     * @Type("array<string>")
     */
    #[Assert\NotNull]
    #[Assert\Valid]
    #[Assert\All([
        new Assert\Type("OpenAPI\Server\Model\AdDisapprovalReasons"),
    ])]
    protected ?array $rejectedReasons = null;

    /**
     * Text reason why the pin was rejected. Returned if &#x60;review_status&#x60; is \&quot;REJECTED\&quot;.
     *
     * @var string[]|null
     * @SerializedName("rejection_labels")
     * @Type("array<string>")
     */
    #[Assert\NotNull]
    #[Assert\All([
        new Assert\Type("string"),
    ])]
    protected ?array $rejectionLabels = null;

    /**
     * Ad review status
     *
     * @var AdReviewStatus|null
     * @SerializedName("review_status")
     * @Accessor(getter="getSerializedReviewStatus", setter="setDeserializedReviewStatus")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Valid]
    protected ?AdReviewStatus $reviewStatus = null;

    /**
     * @var EntityStatus|null
     * @SerializedName("status")
     * @Accessor(getter="getSerializedStatus", setter="setDeserializedStatus")
     * @Type("string")
     */
    protected ?EntityStatus $status = null;

    /**
     * Ad summary status
     *
     * @var PinPromotionSummaryStatus|null
     * @SerializedName("summary_status")
     * @Accessor(getter="getSerializedSummaryStatus", setter="setDeserializedSummaryStatus")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Valid]
    protected ?PinPromotionSummaryStatus $summaryStatus = null;

    /**
     * @var TrackingUrls|null
     * @SerializedName("tracking_urls")
     * @Type("OpenAPI\Server\Model\TrackingUrls")
     */
    #[Assert\Type("OpenAPI\Server\Model\TrackingUrls")]
    protected ?TrackingUrls $trackingUrls = null;

    /**
     * Always \&quot;ad\&quot;.
     *
     * @var string|null
     * @SerializedName("type")
     * @Type("string")
     */
    #[Assert\NotNull]
    #[Assert\Type("string")]
    protected ?string $type = null;

    /**
     * Last update time. Unix timestamp in seconds.
     *
     * @var int|null
     * @SerializedName("updated_time")
     * @Type("int")
     */
    #[Assert\Type("int")]
    protected ?int $updatedTime = null;

    /**
     * Tracking URL for ad impressions.
     *
     * @var string|null
     * @SerializedName("view_tracking_url")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $viewTrackingUrl = null;

    /**
     * Constructor
     * @param array|null $data Associated array of property values initializing the model
     */
    public function __construct(?array $data = null)
    {
        if (is_array($data)) {
            $this->adAccountId = array_key_exists('adAccountId', $data) ? $data['adAccountId'] : $this->adAccountId;
            $this->adGroupId = array_key_exists('adGroupId', $data) ? $data['adGroupId'] : $this->adGroupId;
            $this->androidDeepLink = array_key_exists('androidDeepLink', $data) ? $data['androidDeepLink'] : $this->androidDeepLink;
            $this->campaignId = array_key_exists('campaignId', $data) ? $data['campaignId'] : $this->campaignId;
            $this->carouselAndroidDeepLinks = array_key_exists('carouselAndroidDeepLinks', $data) ? $data['carouselAndroidDeepLinks'] : $this->carouselAndroidDeepLinks;
            $this->carouselDestinationUrls = array_key_exists('carouselDestinationUrls', $data) ? $data['carouselDestinationUrls'] : $this->carouselDestinationUrls;
            $this->carouselIosDeepLinks = array_key_exists('carouselIosDeepLinks', $data) ? $data['carouselIosDeepLinks'] : $this->carouselIosDeepLinks;
            $this->cartingPlatformType = array_key_exists('cartingPlatformType', $data) ? $data['cartingPlatformType'] : $this->cartingPlatformType;
            $this->cartingProducts = array_key_exists('cartingProducts', $data) ? $data['cartingProducts'] : $this->cartingProducts;
            $this->clickTrackingUrl = array_key_exists('clickTrackingUrl', $data) ? $data['clickTrackingUrl'] : $this->clickTrackingUrl;
            $this->collectionItemsDestinationUrlTemplate = array_key_exists('collectionItemsDestinationUrlTemplate', $data) ? $data['collectionItemsDestinationUrlTemplate'] : $this->collectionItemsDestinationUrlTemplate;
            $this->collectionsHeaderType = array_key_exists('collectionsHeaderType', $data) ? $data['collectionsHeaderType'] : $this->collectionsHeaderType;
            $this->createdTime = array_key_exists('createdTime', $data) ? $data['createdTime'] : $this->createdTime;
            $this->creativeType = array_key_exists('creativeType', $data) ? $data['creativeType'] : $this->creativeType;
            $this->customizableCtaType = array_key_exists('customizableCtaType', $data) ? $data['customizableCtaType'] : $this->customizableCtaType;
            $this->destinationUrl = array_key_exists('destinationUrl', $data) ? $data['destinationUrl'] : $this->destinationUrl;
            $this->disclosureType = array_key_exists('disclosureType', $data) ? $data['disclosureType'] : $this->disclosureType;
            $this->disclosureUrl = array_key_exists('disclosureUrl', $data) ? $data['disclosureUrl'] : $this->disclosureUrl;
            $this->gridClickType = array_key_exists('gridClickType', $data) ? $data['gridClickType'] : $this->gridClickType;
            $this->id = array_key_exists('id', $data) ? $data['id'] : $this->id;
            $this->iosDeepLink = array_key_exists('iosDeepLink', $data) ? $data['iosDeepLink'] : $this->iosDeepLink;
            $this->isCarting = array_key_exists('isCarting', $data) ? $data['isCarting'] : $this->isCarting;
            $this->isCollageAcceptedTerms = array_key_exists('isCollageAcceptedTerms', $data) ? $data['isCollageAcceptedTerms'] : $this->isCollageAcceptedTerms;
            $this->isCollageSingleDestination = array_key_exists('isCollageSingleDestination', $data) ? $data['isCollageSingleDestination'] : $this->isCollageSingleDestination;
            $this->isPinDeleted = array_key_exists('isPinDeleted', $data) ? $data['isPinDeleted'] : $this->isPinDeleted;
            $this->isRemovable = array_key_exists('isRemovable', $data) ? $data['isRemovable'] : $this->isRemovable;
            $this->leadFormId = array_key_exists('leadFormId', $data) ? $data['leadFormId'] : $this->leadFormId;
            $this->name = array_key_exists('name', $data) ? $data['name'] : $this->name;
            $this->pinId = array_key_exists('pinId', $data) ? $data['pinId'] : $this->pinId;
            $this->quizPinData = array_key_exists('quizPinData', $data) ? $data['quizPinData'] : $this->quizPinData;
            $this->rejectedReasons = array_key_exists('rejectedReasons', $data) ? $data['rejectedReasons'] : $this->rejectedReasons;
            $this->rejectionLabels = array_key_exists('rejectionLabels', $data) ? $data['rejectionLabels'] : $this->rejectionLabels;
            $this->reviewStatus = array_key_exists('reviewStatus', $data) ? $data['reviewStatus'] : $this->reviewStatus;
            $this->status = array_key_exists('status', $data) ? $data['status'] : $this->status;
            $this->summaryStatus = array_key_exists('summaryStatus', $data) ? $data['summaryStatus'] : $this->summaryStatus;
            $this->trackingUrls = array_key_exists('trackingUrls', $data) ? $data['trackingUrls'] : $this->trackingUrls;
            $this->type = array_key_exists('type', $data) ? $data['type'] : $this->type;
            $this->updatedTime = array_key_exists('updatedTime', $data) ? $data['updatedTime'] : $this->updatedTime;
            $this->viewTrackingUrl = array_key_exists('viewTrackingUrl', $data) ? $data['viewTrackingUrl'] : $this->viewTrackingUrl;
        }
    }

    /**
     * Gets adAccountId.
     *
     * @return string|null
     */
    public function getAdAccountId(): ?string
    {
        return $this->adAccountId;
    }

    /**
     * Sets adAccountId.
     *
     * @param string|null $adAccountId  The ID of the advertiser that this ad belongs to.
     *
     * @return $this
     */
    public function setAdAccountId(?string $adAccountId): self
    {
        $this->adAccountId = $adAccountId;

        return $this;
    }




    /**
     * Gets adGroupId.
     *
     * @return string|null
     */
    public function getAdGroupId(): ?string
    {
        return $this->adGroupId;
    }

    /**
     * Sets adGroupId.
     *
     * @param string|null $adGroupId  ID of the ad group that contains the ad.
     *
     * @return $this
     */
    public function setAdGroupId(?string $adGroupId): self
    {
        $this->adGroupId = $adGroupId;

        return $this;
    }




    /**
     * Gets androidDeepLink.
     *
     * @return string|null
     */
    public function getAndroidDeepLink(): ?string
    {
        return $this->androidDeepLink;
    }

    /**
     * Sets androidDeepLink.
     *
     * @param string|null $androidDeepLink  Deep link URL for Android devices.
     *
     * @return $this
     */
    public function setAndroidDeepLink(?string $androidDeepLink = null): self
    {
        $this->androidDeepLink = $androidDeepLink;

        return $this;
    }




    /**
     * Gets campaignId.
     *
     * @return string|null
     */
    public function getCampaignId(): ?string
    {
        return $this->campaignId;
    }

    /**
     * Sets campaignId.
     *
     * @param string|null $campaignId  ID of the ad campaign that contains this ad.
     *
     * @return $this
     */
    public function setCampaignId(?string $campaignId): self
    {
        $this->campaignId = $campaignId;

        return $this;
    }




    /**
     * Gets carouselAndroidDeepLinks.
     *
     * @return string[]|null
     */
    public function getCarouselAndroidDeepLinks(): ?array
    {
        return $this->carouselAndroidDeepLinks;
    }

    /**
     * Sets carouselAndroidDeepLinks.
     *
     * @param string[]|null $carouselAndroidDeepLinks  Comma-separated deep links for the carousel pin on Android.
     *
     * @return $this
     */
    public function setCarouselAndroidDeepLinks(?array $carouselAndroidDeepLinks = null): self
    {
        $this->carouselAndroidDeepLinks = $carouselAndroidDeepLinks;

        return $this;
    }




    /**
     * Gets carouselDestinationUrls.
     *
     * @return string[]|null
     */
    public function getCarouselDestinationUrls(): ?array
    {
        return $this->carouselDestinationUrls;
    }

    /**
     * Sets carouselDestinationUrls.
     *
     * @param string[]|null $carouselDestinationUrls  Comma-separated destination URLs for the carousel pin to promote.
     *
     * @return $this
     */
    public function setCarouselDestinationUrls(?array $carouselDestinationUrls = null): self
    {
        $this->carouselDestinationUrls = $carouselDestinationUrls;

        return $this;
    }




    /**
     * Gets carouselIosDeepLinks.
     *
     * @return string[]|null
     */
    public function getCarouselIosDeepLinks(): ?array
    {
        return $this->carouselIosDeepLinks;
    }

    /**
     * Sets carouselIosDeepLinks.
     *
     * @param string[]|null $carouselIosDeepLinks  Comma-separated deep links for the carousel pin on iOS.
     *
     * @return $this
     */
    public function setCarouselIosDeepLinks(?array $carouselIosDeepLinks = null): self
    {
        $this->carouselIosDeepLinks = $carouselIosDeepLinks;

        return $this;
    }




    /**
     * Gets cartingPlatformType.
     *
     * @return int|null
     */
    public function getCartingPlatformType(): ?int
    {
        return $this->cartingPlatformType;
    }

    /**
     * Sets cartingPlatformType.
     *
     * @param int|null $cartingPlatformType  The vendor platform type of the carting/WTB ad.
     *
     * @return $this
     */
    public function setCartingPlatformType(?int $cartingPlatformType = null): self
    {
        $this->cartingPlatformType = $cartingPlatformType;

        return $this;
    }




    /**
     * Gets cartingProducts.
     *
     * @return CartingProduct[]|null
     */
    public function getCartingProducts(): ?array
    {
        return $this->cartingProducts;
    }

    /**
     * Sets cartingProducts.
     *
     * @param CartingProduct[]|null $cartingProducts  Array of carting/WTB products for the ad.
     *
     * @return $this
     */
    public function setCartingProducts(?array $cartingProducts = null): self
    {
        $this->cartingProducts = $cartingProducts;

        return $this;
    }




    /**
     * Gets clickTrackingUrl.
     *
     * @return string|null
     */
    public function getClickTrackingUrl(): ?string
    {
        return $this->clickTrackingUrl;
    }

    /**
     * Sets clickTrackingUrl.
     *
     * @param string|null $clickTrackingUrl  Tracking url for the ad clicks.
     *
     * @return $this
     */
    public function setClickTrackingUrl(?string $clickTrackingUrl = null): self
    {
        $this->clickTrackingUrl = $clickTrackingUrl;

        return $this;
    }




    /**
     * Gets collectionItemsDestinationUrlTemplate.
     *
     * @return string|null
     */
    public function getCollectionItemsDestinationUrlTemplate(): ?string
    {
        return $this->collectionItemsDestinationUrlTemplate;
    }

    /**
     * Sets collectionItemsDestinationUrlTemplate.
     *
     * @param string|null $collectionItemsDestinationUrlTemplate  Destination URL template for all items within a collections drawer.
     *
     * @return $this
     */
    public function setCollectionItemsDestinationUrlTemplate(?string $collectionItemsDestinationUrlTemplate = null): self
    {
        $this->collectionItemsDestinationUrlTemplate = $collectionItemsDestinationUrlTemplate;

        return $this;
    }




    /**
     * Gets collectionsHeaderType.
     *
     * @return AdCollectionsHeaderType|null
     */
    public function getCollectionsHeaderType(): ?AdCollectionsHeaderType
    {
        return $this->collectionsHeaderType;
    }

    /**
     * Sets collectionsHeaderType.
     *
     * @param AdCollectionsHeaderType|null $collectionsHeaderType
     *
     * @return $this
     */
    public function setCollectionsHeaderType(?AdCollectionsHeaderType $collectionsHeaderType = null): self
    {
        $this->collectionsHeaderType = $collectionsHeaderType;

        return $this;
    }

    /**
     * Gets collectionsHeaderType for serialization.
     *
     * @return string|null
     */
    public function getSerializedCollectionsHeaderType(): string|null
    {
        return !is_null($this->collectionsHeaderType?->value) ? (string) $this->collectionsHeaderType->value : null;
    }

    /**
     * Sets collectionsHeaderType.
     *
     * @param string|AdCollectionsHeaderType|null $collectionsHeaderType
     *
     * @return $this
     */
    public function setDeserializedCollectionsHeaderType(string|AdCollectionsHeaderType|null $collectionsHeaderType = null): self
    {
        if (is_string($collectionsHeaderType)) {
            $collectionsHeaderType = AdCollectionsHeaderType::tryFrom($collectionsHeaderType);
        }

        $this->collectionsHeaderType = $collectionsHeaderType;

        return $this;
    }



    /**
     * Gets createdTime.
     *
     * @return int|null
     */
    public function getCreatedTime(): ?int
    {
        return $this->createdTime;
    }

    /**
     * Sets createdTime.
     *
     * @param int|null $createdTime  Pin creation time. Unix timestamp in seconds.
     *
     * @return $this
     */
    public function setCreatedTime(?int $createdTime = null): self
    {
        $this->createdTime = $createdTime;

        return $this;
    }




    /**
     * Gets creativeType.
     *
     * @return CreativeType|null
     */
    public function getCreativeType(): ?CreativeType
    {
        return $this->creativeType;
    }

    /**
     * Sets creativeType.
     *
     * @param CreativeType|null $creativeType
     *
     * @return $this
     */
    public function setCreativeType(?CreativeType $creativeType): self
    {
        $this->creativeType = $creativeType;

        return $this;
    }

    /**
     * Gets creativeType for serialization.
     *
     * @return string|null
     */
    public function getSerializedCreativeType(): string|null
    {
        return !is_null($this->creativeType?->value) ? (string) $this->creativeType->value : null;
    }

    /**
     * Sets creativeType.
     *
     * @param string|CreativeType|null $creativeType
     *
     * @return $this
     */
    public function setDeserializedCreativeType(string|CreativeType|null $creativeType): self
    {
        if (is_string($creativeType)) {
            $creativeType = CreativeType::tryFrom($creativeType);
        }

        $this->creativeType = $creativeType;

        return $this;
    }



    /**
     * Gets customizableCtaType.
     *
     * @return CustomizableCTAType|null
     */
    public function getCustomizableCtaType(): ?CustomizableCTAType
    {
        return $this->customizableCtaType;
    }

    /**
     * Sets customizableCtaType.
     *
     * @param CustomizableCTAType|null $customizableCtaType
     *
     * @return $this
     */
    public function setCustomizableCtaType(?CustomizableCTAType $customizableCtaType = null): self
    {
        $this->customizableCtaType = $customizableCtaType;

        return $this;
    }

    /**
     * Gets customizableCtaType for serialization.
     *
     * @return string|null
     */
    public function getSerializedCustomizableCtaType(): string|null
    {
        return !is_null($this->customizableCtaType?->value) ? (string) $this->customizableCtaType->value : null;
    }

    /**
     * Sets customizableCtaType.
     *
     * @param string|CustomizableCTAType|null $customizableCtaType
     *
     * @return $this
     */
    public function setDeserializedCustomizableCtaType(string|CustomizableCTAType|null $customizableCtaType = null): self
    {
        if (is_string($customizableCtaType)) {
            $customizableCtaType = CustomizableCTAType::tryFrom($customizableCtaType);
        }

        $this->customizableCtaType = $customizableCtaType;

        return $this;
    }



    /**
     * Gets destinationUrl.
     *
     * @return string|null
     */
    public function getDestinationUrl(): ?string
    {
        return $this->destinationUrl;
    }

    /**
     * Sets destinationUrl.
     *
     * @param string|null $destinationUrl  Destination URL.
     *
     * @return $this
     */
    public function setDestinationUrl(?string $destinationUrl = null): self
    {
        $this->destinationUrl = $destinationUrl;

        return $this;
    }




    /**
     * Gets disclosureType.
     *
     * @return DisclosureType|null
     */
    public function getDisclosureType(): ?DisclosureType
    {
        return $this->disclosureType;
    }

    /**
     * Sets disclosureType.
     *
     * @param DisclosureType|null $disclosureType
     *
     * @return $this
     */
    public function setDisclosureType(?DisclosureType $disclosureType = null): self
    {
        $this->disclosureType = $disclosureType;

        return $this;
    }

    /**
     * Gets disclosureType for serialization.
     *
     * @return string|null
     */
    public function getSerializedDisclosureType(): string|null
    {
        return !is_null($this->disclosureType?->value) ? (string) $this->disclosureType->value : null;
    }

    /**
     * Sets disclosureType.
     *
     * @param string|DisclosureType|null $disclosureType
     *
     * @return $this
     */
    public function setDeserializedDisclosureType(string|DisclosureType|null $disclosureType = null): self
    {
        if (is_string($disclosureType)) {
            $disclosureType = DisclosureType::tryFrom($disclosureType);
        }

        $this->disclosureType = $disclosureType;

        return $this;
    }



    /**
     * Gets disclosureUrl.
     *
     * @return string|null
     */
    public function getDisclosureUrl(): ?string
    {
        return $this->disclosureUrl;
    }

    /**
     * Sets disclosureUrl.
     *
     * @param string|null $disclosureUrl  URL for a page that provides disclosures about a pharmaceutical product, such as potential side effects. Make sure the URL takes the user directly to the disclosure content and the referenced site is secure.
     *
     * @return $this
     */
    public function setDisclosureUrl(?string $disclosureUrl = null): self
    {
        $this->disclosureUrl = $disclosureUrl;

        return $this;
    }




    /**
     * Gets gridClickType.
     *
     * @return GridClickType|null
     */
    public function getGridClickType(): ?GridClickType
    {
        return $this->gridClickType;
    }

    /**
     * Sets gridClickType.
     *
     * @param GridClickType|null $gridClickType
     *
     * @return $this
     */
    public function setGridClickType(?GridClickType $gridClickType = null): self
    {
        $this->gridClickType = $gridClickType;

        return $this;
    }

    /**
     * Gets gridClickType for serialization.
     *
     * @return string|null
     */
    public function getSerializedGridClickType(): string|null
    {
        return !is_null($this->gridClickType?->value) ? (string) $this->gridClickType->value : null;
    }

    /**
     * Sets gridClickType.
     *
     * @param string|GridClickType|null $gridClickType
     *
     * @return $this
     */
    public function setDeserializedGridClickType(string|GridClickType|null $gridClickType = null): self
    {
        if (is_string($gridClickType)) {
            $gridClickType = GridClickType::tryFrom($gridClickType);
        }

        $this->gridClickType = $gridClickType;

        return $this;
    }



    /**
     * Gets id.
     *
     * @return string|null
     */
    public function getId(): ?string
    {
        return $this->id;
    }

    /**
     * Sets id.
     *
     * @param string|null $id  The ID of this ad.
     *
     * @return $this
     */
    public function setId(?string $id): self
    {
        $this->id = $id;

        return $this;
    }




    /**
     * Gets iosDeepLink.
     *
     * @return string|null
     */
    public function getIosDeepLink(): ?string
    {
        return $this->iosDeepLink;
    }

    /**
     * Sets iosDeepLink.
     *
     * @param string|null $iosDeepLink  Deep link URL for iOS devices.
     *
     * @return $this
     */
    public function setIosDeepLink(?string $iosDeepLink = null): self
    {
        $this->iosDeepLink = $iosDeepLink;

        return $this;
    }




    /**
     * Gets isCarting.
     *
     * @return bool|null
     */
    public function isIsCarting(): ?bool
    {
        return $this->isCarting;
    }

    /**
     * Sets isCarting.
     *
     * @param bool|null $isCarting  Is the ad a carting/WTB ad?
     *
     * @return $this
     */
    public function setIsCarting(?bool $isCarting = null): self
    {
        $this->isCarting = $isCarting;

        return $this;
    }




    /**
     * Gets isCollageAcceptedTerms.
     *
     * @return bool|null
     */
    public function isIsCollageAcceptedTerms(): ?bool
    {
        return $this->isCollageAcceptedTerms;
    }

    /**
     * Sets isCollageAcceptedTerms.
     *
     * @param bool|null $isCollageAcceptedTerms  Whether the advertiser has accepted the terms and conditions for collage ad.
     *
     * @return $this
     */
    public function setIsCollageAcceptedTerms(?bool $isCollageAcceptedTerms = null): self
    {
        $this->isCollageAcceptedTerms = $isCollageAcceptedTerms;

        return $this;
    }




    /**
     * Gets isCollageSingleDestination.
     *
     * @return bool|null
     */
    public function isIsCollageSingleDestination(): ?bool
    {
        return $this->isCollageSingleDestination;
    }

    /**
     * Sets isCollageSingleDestination.
     *
     * @param bool|null $isCollageSingleDestination  Whether the collage ad has a single destination url override.
     *
     * @return $this
     */
    public function setIsCollageSingleDestination(?bool $isCollageSingleDestination = null): self
    {
        $this->isCollageSingleDestination = $isCollageSingleDestination;

        return $this;
    }




    /**
     * Gets isPinDeleted.
     *
     * @return bool|null
     */
    public function isIsPinDeleted(): ?bool
    {
        return $this->isPinDeleted;
    }

    /**
     * Sets isPinDeleted.
     *
     * @param bool|null $isPinDeleted  Is original pin deleted?
     *
     * @return $this
     */
    public function setIsPinDeleted(?bool $isPinDeleted = null): self
    {
        $this->isPinDeleted = $isPinDeleted;

        return $this;
    }




    /**
     * Gets isRemovable.
     *
     * @return bool|null
     */
    public function isIsRemovable(): ?bool
    {
        return $this->isRemovable;
    }

    /**
     * Sets isRemovable.
     *
     * @param bool|null $isRemovable  Is pin repinnable?
     *
     * @return $this
     */
    public function setIsRemovable(?bool $isRemovable = null): self
    {
        $this->isRemovable = $isRemovable;

        return $this;
    }




    /**
     * Gets leadFormId.
     *
     * @return string|null
     */
    public function getLeadFormId(): ?string
    {
        return $this->leadFormId;
    }

    /**
     * Sets leadFormId.
     *
     * @param string|null $leadFormId  Lead form ID for lead ad generation.
     *
     * @return $this
     */
    public function setLeadFormId(?string $leadFormId = null): self
    {
        $this->leadFormId = $leadFormId;

        return $this;
    }




    /**
     * Gets name.
     *
     * @return string|null
     */
    public function getName(): ?string
    {
        return $this->name;
    }

    /**
     * Sets name.
     *
     * @param string|null $name  Name of the ad - 255 chars max.
     *
     * @return $this
     */
    public function setName(?string $name = null): self
    {
        $this->name = $name;

        return $this;
    }




    /**
     * Gets pinId.
     *
     * @return string|null
     */
    public function getPinId(): ?string
    {
        return $this->pinId;
    }

    /**
     * Sets pinId.
     *
     * @param string|null $pinId  Pin ID. This field may only be updated for draft ads.
     *
     * @return $this
     */
    public function setPinId(?string $pinId): self
    {
        $this->pinId = $pinId;

        return $this;
    }




    /**
     * Gets quizPinData.
     *
     * @return QuizPinData|null
     */
    public function getQuizPinData(): ?QuizPinData
    {
        return $this->quizPinData;
    }

    /**
     * Sets quizPinData.
     *
     * @param QuizPinData|null $quizPinData  Before creating a quiz ad, you must create an organic Pin using POST/Create Pin for each result in the quiz. Quiz ads cannot be saved by a Pinner. Quiz ad results can be saved.
     *
     * @return $this
     */
    public function setQuizPinData(?QuizPinData $quizPinData = null): self
    {
        $this->quizPinData = $quizPinData;

        return $this;
    }




    /**
     * Gets rejectedReasons.
     *
     * @return AdDisapprovalReasons[]|null
     */
    public function getRejectedReasons(): ?array
    {
        return $this->rejectedReasons;
    }

    /**
     * Sets rejectedReasons.
     *
     * @param AdDisapprovalReasons[]|null $rejectedReasons  Enum reason why the pin was rejected. Returned if `review_status` is \"REJECTED\".
     *
     * @return $this
     */
    public function setRejectedReasons(?array $rejectedReasons): self
    {
        $this->rejectedReasons = $rejectedReasons;

        return $this;
    }


    /**
     * Gets rejectedReasons for serialization.
     *
     * @return array
     */
    public function getSerializedRejectedReasons(): array
    {
        return array_map(
            static fn ($value) => $value?->value ? (string) $value->value : null,
            $this->rejectedReasons ?? []
        );
    }

    /**
     * Sets rejectedReasons.
     *
     * @param ?array $rejectedReasons
     *
     * @return $this
     */
    public function setDeserializedRejectedReasons(?array $rejectedReasons = []): self
    {
        $this->rejectedReasons = array_map(
            static function ($value) {
                if (is_string($value)) {
                    $value = AdDisapprovalReasons::tryFrom($value);
                }

                return $value;
            },
            $rejectedReasons ?? []
        );

        return $this;
    }


    /**
     * Gets rejectionLabels.
     *
     * @return string[]|null
     */
    public function getRejectionLabels(): ?array
    {
        return $this->rejectionLabels;
    }

    /**
     * Sets rejectionLabels.
     *
     * @param string[]|null $rejectionLabels  Text reason why the pin was rejected. Returned if `review_status` is \"REJECTED\".
     *
     * @return $this
     */
    public function setRejectionLabels(?array $rejectionLabels): self
    {
        $this->rejectionLabels = $rejectionLabels;

        return $this;
    }




    /**
     * Gets reviewStatus.
     *
     * @return AdReviewStatus|null
     */
    public function getReviewStatus(): ?AdReviewStatus
    {
        return $this->reviewStatus;
    }

    /**
     * Sets reviewStatus.
     *
     * @param AdReviewStatus|null $reviewStatus  Ad review status
     *
     * @return $this
     */
    public function setReviewStatus(?AdReviewStatus $reviewStatus): self
    {
        $this->reviewStatus = $reviewStatus;

        return $this;
    }

    /**
     * Gets reviewStatus for serialization.
     *
     * @return string|null
     */
    public function getSerializedReviewStatus(): string|null
    {
        return !is_null($this->reviewStatus?->value) ? (string) $this->reviewStatus->value : null;
    }

    /**
     * Sets reviewStatus.
     *
     * @param string|AdReviewStatus|null $reviewStatus  Ad review status
     *
     * @return $this
     */
    public function setDeserializedReviewStatus(string|AdReviewStatus|null $reviewStatus): self
    {
        if (is_string($reviewStatus)) {
            $reviewStatus = AdReviewStatus::tryFrom($reviewStatus);
        }

        $this->reviewStatus = $reviewStatus;

        return $this;
    }



    /**
     * Gets status.
     *
     * @return EntityStatus|null
     */
    public function getStatus(): ?EntityStatus
    {
        return $this->status;
    }

    /**
     * Sets status.
     *
     * @param EntityStatus|null $status
     *
     * @return $this
     */
    public function setStatus(?EntityStatus $status = null): self
    {
        $this->status = $status;

        return $this;
    }

    /**
     * Gets status for serialization.
     *
     * @return string|null
     */
    public function getSerializedStatus(): string|null
    {
        return !is_null($this->status?->value) ? (string) $this->status->value : null;
    }

    /**
     * Sets status.
     *
     * @param string|EntityStatus|null $status
     *
     * @return $this
     */
    public function setDeserializedStatus(string|EntityStatus|null $status = null): self
    {
        if (is_string($status)) {
            $status = EntityStatus::tryFrom($status);
        }

        $this->status = $status;

        return $this;
    }



    /**
     * Gets summaryStatus.
     *
     * @return PinPromotionSummaryStatus|null
     */
    public function getSummaryStatus(): ?PinPromotionSummaryStatus
    {
        return $this->summaryStatus;
    }

    /**
     * Sets summaryStatus.
     *
     * @param PinPromotionSummaryStatus|null $summaryStatus  Ad summary status
     *
     * @return $this
     */
    public function setSummaryStatus(?PinPromotionSummaryStatus $summaryStatus): self
    {
        $this->summaryStatus = $summaryStatus;

        return $this;
    }

    /**
     * Gets summaryStatus for serialization.
     *
     * @return string|null
     */
    public function getSerializedSummaryStatus(): string|null
    {
        return !is_null($this->summaryStatus?->value) ? (string) $this->summaryStatus->value : null;
    }

    /**
     * Sets summaryStatus.
     *
     * @param string|PinPromotionSummaryStatus|null $summaryStatus  Ad summary status
     *
     * @return $this
     */
    public function setDeserializedSummaryStatus(string|PinPromotionSummaryStatus|null $summaryStatus): self
    {
        if (is_string($summaryStatus)) {
            $summaryStatus = PinPromotionSummaryStatus::tryFrom($summaryStatus);
        }

        $this->summaryStatus = $summaryStatus;

        return $this;
    }



    /**
     * Gets trackingUrls.
     *
     * @return TrackingUrls|null
     */
    public function getTrackingUrls(): ?TrackingUrls
    {
        return $this->trackingUrls;
    }

    /**
     * Sets trackingUrls.
     *
     * @param TrackingUrls|null $trackingUrls
     *
     * @return $this
     */
    public function setTrackingUrls(?TrackingUrls $trackingUrls = null): self
    {
        $this->trackingUrls = $trackingUrls;

        return $this;
    }




    /**
     * Gets type.
     *
     * @return string|null
     */
    public function getType(): ?string
    {
        return $this->type;
    }

    /**
     * Sets type.
     *
     * @param string|null $type  Always \"ad\".
     *
     * @return $this
     */
    public function setType(?string $type): self
    {
        $this->type = $type;

        return $this;
    }




    /**
     * Gets updatedTime.
     *
     * @return int|null
     */
    public function getUpdatedTime(): ?int
    {
        return $this->updatedTime;
    }

    /**
     * Sets updatedTime.
     *
     * @param int|null $updatedTime  Last update time. Unix timestamp in seconds.
     *
     * @return $this
     */
    public function setUpdatedTime(?int $updatedTime = null): self
    {
        $this->updatedTime = $updatedTime;

        return $this;
    }




    /**
     * Gets viewTrackingUrl.
     *
     * @return string|null
     */
    public function getViewTrackingUrl(): ?string
    {
        return $this->viewTrackingUrl;
    }

    /**
     * Sets viewTrackingUrl.
     *
     * @param string|null $viewTrackingUrl  Tracking URL for ad impressions.
     *
     * @return $this
     */
    public function setViewTrackingUrl(?string $viewTrackingUrl = null): self
    {
        $this->viewTrackingUrl = $viewTrackingUrl;

        return $this;
    }



}


