...
class CatalogsListProductsByFilterRequest {
     ...
     * @param {(module:model/CatalogsListProductsByFilterRequestOneOf)} instance The actual instance to initialize CatalogsListProductsByFilterRequest.
     */
    constructor(instance = null) {
        ...
        var errorMessages = [];
        try {
            if (typeof instance === "CatalogsListProductsByFilterRequestOneOf") {
                this.actualInstance = instance;
            } else {
                // plain JS object
                // validate the object
                CatalogsListProductsByFilterRequestOneOf.validateJSON(instance); // throw an exception if no match
                // create CatalogsListProductsByFilterRequestOneOf from JS object
                this.actualInstance = CatalogsListProductsByFilterRequestOneOf.constructFromObject(instance);
            }
            match++;
        } catch(err) {
            // json data failed to deserialize into CatalogsListProductsByFilterRequestOneOf
            errorMessages.push("Failed to construct CatalogsListProductsByFilterRequestOneOf: " + err)
        }

        if (match > 1) {
            throw new Error("Multiple matches found constructing `CatalogsListProductsByFilterRequest` with oneOf schemas CatalogsListProductsByFilterRequestOneOf. Input: " + JSON.stringify(instance));
        } else if (match === 0) {
            this.actualInstance = null; // clear the actual instance in case there are multiple matches
            throw new Error("No match found constructing `CatalogsListProductsByFilterRequest` with oneOf schemas CatalogsListProductsByFilterRequestOneOf. Details: " +
                            errorMessages.join(", "));
        } else { // only 1 match
            // the input is valid
        }
    ...
    /**
     * Gets the actual instance, which can be <code>CatalogsListProductsByFilterRequestOneOf</code>.
     * @return {(module:model/CatalogsListProductsByFilterRequestOneOf)} The actual instance.
     */
    getActualInstance() {
    ...
    /**
     * Sets the actual instance, which can be <code>CatalogsListProductsByFilterRequestOneOf</code>.
     * @param {(module:model/CatalogsListProductsByFilterRequestOneOf)} obj The actual instance.
     */
...
CatalogsListProductsByFilterRequest.OneOf = ["CatalogsListProductsByFilterRequestOneOf"];
