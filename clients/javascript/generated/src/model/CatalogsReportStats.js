        ...
        if (instance && typeof instance === 'object' && instance['report_type'] !== undefined) {
            ...
            switch (discriminatorValue) {
                case 'DISTRIBUTION_ISSUES':
                    try {
                        CatalogsReportDistributionStats.validateJSON(instance);
                        this.actualInstance = CatalogsReportDistributionStats.constructFromObject(instance);
                        return;
                    } catch(err) {
                        throw new Error("Failed to construct CatalogsReportDistributionStats using discriminator value 'DISTRIBUTION_ISSUES': " + err);
                    }
                case 'FEED_INGESTION_ISSUES':
                    try {
                        CatalogsReportFeedIngestionStats.validateJSON(instance);
                        this.actualInstance = CatalogsReportFeedIngestionStats.constructFromObject(instance);
                        return;
                    } catch(err) {
                        throw new Error("Failed to construct CatalogsReportFeedIngestionStats using discriminator value 'FEED_INGESTION_ISSUES': " + err);
                    }
                default:
                    throw new Error("Unrecognized discriminator value '" + discriminatorValue + "' constructing `CatalogsReportStats`. Expected one of: DISTRIBUTION_ISSUES, FEED_INGESTION_ISSUES.");
            }
        }
