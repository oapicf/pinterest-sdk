/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * BulkDownloadCreate.h
 *
 * Resource create operation model.
 */

#ifndef ORG_OPENAPITOOLS_CLIENT_MODEL_BulkDownloadCreate_H_
#define ORG_OPENAPITOOLS_CLIENT_MODEL_BulkDownloadCreate_H_

#include <stdexcept>
#include <boost/optional.hpp>

#include "CppRestOpenAPIClient/ModelBase.h"

#include "CppRestOpenAPIClient/model/BulkDownloadCampaignFilter.h"
#include "CppRestOpenAPIClient/model/BulkOutputFormat.h"
#include <cpprest/details/basic_types.h>
#include <vector>
#include "CppRestOpenAPIClient/model/BulkEntityType.h"

namespace org {
namespace openapitools {
namespace client {
namespace model {

class BulkDownloadCampaignFilter;


/// <summary>
/// Resource create operation model.
/// </summary>
class  BulkDownloadCreate
    : public ModelBase
{
public:
    BulkDownloadCreate();
    virtual ~BulkDownloadCreate();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// BulkDownloadCreate members


    std::shared_ptr<BulkDownloadCampaignFilter> getCampaignFilter() const;
    bool campaignFilterIsSet() const;
    void unsetCampaign_filter();
    void setCampaignFilter(const std::shared_ptr<BulkDownloadCampaignFilter>& value);

    /// <summary>
    /// All entities specified by these IDs as well as their children and grandchildren will be downloaded if the entity type is one of the types requested to be downloaded.
    /// </summary>
    std::vector<utility::string_t> getEntityIds() const;
    bool entityIdsIsSet() const;
    void unsetEntity_ids();
    void setEntityIds(const std::vector<utility::string_t>& value);

    /// <summary>
    /// All entity types specified will be downloaded. Fewer types result in faster downloads.
    /// </summary>
    std::vector<std::shared_ptr<BulkEntityType>> getEntityTypes() const;
    bool entityTypesIsSet() const;
    void unsetEntity_types();
    void setEntityTypes(const std::vector<std::shared_ptr<BulkEntityType>>& value);

    std::shared_ptr<BulkOutputFormat> getOutputFormat() const;
    bool outputFormatIsSet() const;
    void unsetOutput_format();
    void setOutputFormat(const std::shared_ptr<BulkOutputFormat>& value);

    /// <summary>
    /// Unix UTC timestamp to retrieve all entities that have changed since this time.
    /// </summary>
    utility::string_t getUpdatedSince() const;
    bool updatedSinceIsSet() const;
    void unsetUpdated_since();
    void setUpdatedSince(const utility::string_t& value);


protected:
    std::shared_ptr<BulkDownloadCampaignFilter> m_Campaign_filter;
    bool m_Campaign_filterIsSet;

    std::vector<utility::string_t> m_Entity_ids;
    bool m_Entity_idsIsSet;

    std::vector<std::shared_ptr<BulkEntityType>> m_Entity_types;
    bool m_Entity_typesIsSet;

    std::shared_ptr<BulkOutputFormat> m_Output_format;
    bool m_Output_formatIsSet;

    utility::string_t m_Updated_since;
    bool m_Updated_sinceIsSet;

};


}
}
}
}

#endif /* ORG_OPENAPITOOLS_CLIENT_MODEL_BulkDownloadCreate_H_ */
