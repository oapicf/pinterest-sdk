/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * PacingDeliveryType.h
 *
 * Ad group pacing delivery type. With ACCELERATED, an ad group budget is spent as fast as possible. With STANDARD, an ad group budget is spent smoothly over a day. When using CBO, only the STANDARD pacing delivery type is allowed.
 */

#ifndef ORG_OPENAPITOOLS_CLIENT_MODEL_PacingDeliveryType_H_
#define ORG_OPENAPITOOLS_CLIENT_MODEL_PacingDeliveryType_H_

#include <boost/optional.hpp>

#include "CppRestOpenAPIClient/ModelBase.h"


namespace org {
namespace openapitools {
namespace client {
namespace model {


class  PacingDeliveryType
    : public ModelBase
{
public:
    PacingDeliveryType();
    PacingDeliveryType(utility::string_t str);
    operator utility::string_t() const {
        return enumToStrMap.at(getValue());
    }


    virtual ~PacingDeliveryType();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;

    enum class ePacingDeliveryType
    {
        STANDARD,
        ACCELERATED,
    };

    ePacingDeliveryType getValue() const;
    void setValue(ePacingDeliveryType const value);

    protected:
        ePacingDeliveryType m_value;
        std::map<ePacingDeliveryType,utility::string_t> enumToStrMap = {
        { ePacingDeliveryType::STANDARD, _XPLATSTR("STANDARD") },
        { ePacingDeliveryType::ACCELERATED, _XPLATSTR("ACCELERATED") }
};
        std::map<utility::string_t,ePacingDeliveryType> strToEnumMap = {
        { _XPLATSTR("STANDARD"), ePacingDeliveryType::STANDARD },
        { _XPLATSTR("ACCELERATED"), ePacingDeliveryType::ACCELERATED }
};

};

}
}
}
}

#endif /* ORG_OPENAPITOOLS_CLIENT_MODEL_PacingDeliveryType_H_ */
