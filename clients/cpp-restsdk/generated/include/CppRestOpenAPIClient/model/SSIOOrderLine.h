/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * SSIOOrderLine.h
 *
 * A Salesforce SSIO order line.
 */

#ifndef ORG_OPENAPITOOLS_CLIENT_MODEL_SSIOOrderLine_H_
#define ORG_OPENAPITOOLS_CLIENT_MODEL_SSIOOrderLine_H_

#include <stdexcept>
#include <boost/optional.hpp>

#include "CppRestOpenAPIClient/ModelBase.h"

#include "CppRestOpenAPIClient/model/Currency.h"
#include <cpprest/details/basic_types.h>

namespace org {
namespace openapitools {
namespace client {
namespace model {



/// <summary>
/// A Salesforce SSIO order line.
/// </summary>
class  SSIOOrderLine
    : public ModelBase
{
public:
    SSIOOrderLine();
    virtual ~SSIOOrderLine();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// SSIOOrderLine members


    /// <summary>
    /// The SFDC id for the terms
    /// </summary>
    utility::string_t getAcceptedTermsId() const;
    bool acceptedTermsIdIsSet() const;
    void unsetAccepted_terms_id();
    void setAcceptedTermsId(const utility::string_t& value);

    /// <summary>
    /// The UTC timestamp (to the nearest second) when terms were accepted.
    /// </summary>
    utility::string_t getAcceptedTermsTime() const;
    bool acceptedTermsTimeIsSet() const;
    void unsetAccepted_terms_time();
    void setAcceptedTermsTime(const utility::string_t& value);

    /// <summary>
    /// Ads manager order line id
    /// </summary>
    utility::string_t getAdsManagerOrderLineId() const;
    bool adsManagerOrderLineIdIsSet() const;
    void unsetAds_manager_order_line_id();
    void setAdsManagerOrderLineId(const utility::string_t& value);

    /// <summary>
    /// Agency link
    /// </summary>
    utility::string_t getAgencyLink() const;
    bool agencyLinkIsSet() const;
    void unsetAgency_link();
    void setAgencyLink(const utility::string_t& value);

    /// <summary>
    /// Bill-to company name
    /// </summary>
    utility::string_t getBillToCompanyName() const;
    bool billToCompanyNameIsSet() const;
    void unsetBill_to_company_name();
    void setBillToCompanyName(const utility::string_t& value);

    /// <summary>
    /// Billing contact email
    /// </summary>
    utility::string_t getBillingContactEmail() const;
    bool billingContactEmailIsSet() const;
    void unsetBilling_contact_email();
    void setBillingContactEmail(const utility::string_t& value);

    /// <summary>
    /// Billing contact first name
    /// </summary>
    utility::string_t getBillingContactFirstname() const;
    bool billingContactFirstnameIsSet() const;
    void unsetBilling_contact_firstname();
    void setBillingContactFirstname(const utility::string_t& value);

    /// <summary>
    /// Billing contact last name
    /// </summary>
    utility::string_t getBillingContactLastname() const;
    bool billingContactLastnameIsSet() const;
    void unsetBilling_contact_lastname();
    void setBillingContactLastname(const utility::string_t& value);

    /// <summary>
    /// If budget order line, the budget amount.
    /// </summary>
    double getBudgetAmount() const;
    bool budgetAmountIsSet() const;
    void unsetBudget_amount();
    void setBudgetAmount(double value);

    std::shared_ptr<Currency> getCurrencyInfo() const;
    bool currencyInfoIsSet() const;
    void unsetCurrency_info();
    void setCurrencyInfo(const std::shared_ptr<Currency>& value);

    /// <summary>
    /// End date of the order line.
    /// </summary>
    utility::datetime getEndDate() const;
    bool endDateIsSet() const;
    void unsetEnd_date();
    void setEndDate(const utility::datetime& value);

    /// <summary>
    /// If ongoing (perpetual) order line, the estimated monthly spend.
    /// </summary>
    double getEstimatedMonthlySpend() const;
    bool estimatedMonthlySpendIsSet() const;
    void unsetEstimated_monthly_spend();
    void setEstimatedMonthlySpend(double value);

    /// <summary>
    /// Last modified date.
    /// </summary>
    utility::string_t getLastModifiedDateTime() const;
    bool lastModifiedDateTimeIsSet() const;
    void unsetLast_modified_date_time();
    void setLastModifiedDateTime(const utility::string_t& value);

    /// <summary>
    /// Billing media email
    /// </summary>
    utility::string_t getMediaContactEmail() const;
    bool mediaContactEmailIsSet() const;
    void unsetMedia_contact_email();
    void setMediaContactEmail(const utility::string_t& value);

    /// <summary>
    /// Billing media contact first name
    /// </summary>
    utility::string_t getMediaContactFirstname() const;
    bool mediaContactFirstnameIsSet() const;
    void unsetMedia_contact_firstname();
    void setMediaContactFirstname(const utility::string_t& value);

    /// <summary>
    /// Billing media contact last name
    /// </summary>
    utility::string_t getMediaContactLastname() const;
    bool mediaContactLastnameIsSet() const;
    void unsetMedia_contact_lastname();
    void setMediaContactLastname(const utility::string_t& value);

    /// <summary>
    /// The order name
    /// </summary>
    utility::string_t getOrderName() const;
    bool orderNameIsSet() const;
    void unsetOrder_name();
    void setOrderName(const utility::string_t& value);

    /// <summary>
    /// The pin order id associated with the order line in SFDC
    /// </summary>
    utility::string_t getPinOrderId() const;
    bool pinOrderIdIsSet() const;
    void unsetPin_order_id();
    void setPinOrderId(const utility::string_t& value);

    /// <summary>
    /// The Pinterest marketing partner name
    /// </summary>
    utility::string_t getPmpName() const;
    bool pmpNameIsSet() const;
    void unsetPmp_name();
    void setPmpName(const utility::string_t& value);

    /// <summary>
    /// The PO number
    /// </summary>
    utility::string_t getPoNumber() const;
    bool poNumberIsSet() const;
    void unsetPo_number();
    void setPoNumber(const utility::string_t& value);

    /// <summary>
    /// Order line id in SFDC
    /// </summary>
    utility::string_t getSalesforceOrderLineId() const;
    bool salesforceOrderLineIdIsSet() const;
    void unsetSalesforce_order_line_id();
    void setSalesforceOrderLineId(const utility::string_t& value);

    /// <summary>
    /// Start date of the order line.
    /// </summary>
    utility::datetime getStartDate() const;
    bool startDateIsSet() const;
    void unsetStart_date();
    void setStartDate(const utility::datetime& value);


protected:
    boost::optional<utility::string_t> m_Accepted_terms_id;

    boost::optional<utility::string_t> m_Accepted_terms_time;

    boost::optional<utility::string_t> m_Ads_manager_order_line_id;

    boost::optional<utility::string_t> m_Agency_link;

    boost::optional<utility::string_t> m_Bill_to_company_name;

    boost::optional<utility::string_t> m_Billing_contact_email;

    boost::optional<utility::string_t> m_Billing_contact_firstname;

    boost::optional<utility::string_t> m_Billing_contact_lastname;

    boost::optional<double> m_Budget_amount;

    std::shared_ptr<Currency> m_Currency_info;
    bool m_Currency_infoIsSet;

    boost::optional<utility::datetime> m_End_date;

    boost::optional<double> m_Estimated_monthly_spend;

    boost::optional<utility::string_t> m_Last_modified_date_time;

    boost::optional<utility::string_t> m_Media_contact_email;

    boost::optional<utility::string_t> m_Media_contact_firstname;

    boost::optional<utility::string_t> m_Media_contact_lastname;

    boost::optional<utility::string_t> m_Order_name;

    boost::optional<utility::string_t> m_Pin_order_id;

    boost::optional<utility::string_t> m_Pmp_name;

    boost::optional<utility::string_t> m_Po_number;

    boost::optional<utility::string_t> m_Salesforce_order_line_id;

    boost::optional<utility::datetime> m_Start_date;

};


}
}
}
}

#endif /* ORG_OPENAPITOOLS_CLIENT_MODEL_SSIOOrderLine_H_ */
