/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.23.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.18.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * LeadSubscriptionPostParamsCreate.h
 *
 * 
 */

#ifndef ORG_OPENAPITOOLS_CLIENT_MODEL_LeadSubscriptionPostParamsCreate_H_
#define ORG_OPENAPITOOLS_CLIENT_MODEL_LeadSubscriptionPostParamsCreate_H_

#include <boost/optional.hpp>

#include "CppRestOpenAPIClient/ModelBase.h"

#include <cpprest/details/basic_types.h>
#include "CppRestOpenAPIClient/model/LeadSubscriptionPostParamsCreate_allOf_partner_metadata.h"

namespace org {
namespace openapitools {
namespace client {
namespace model {

class LeadSubscriptionPostParamsCreate_allOf_partner_metadata;


class  LeadSubscriptionPostParamsCreate
    : public ModelBase
{
public:
    LeadSubscriptionPostParamsCreate();
    virtual ~LeadSubscriptionPostParamsCreate();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// LeadSubscriptionPostParamsCreate members


    /// <summary>
    /// Lead form ID.
    /// </summary>
    utility::string_t getLeadFormId() const;
    bool leadFormIdIsSet() const;
    void unsetLead_form_id();
    void setLeadFormId(const utility::string_t& value);

    /// <summary>
    /// Standard HTTPS webhook URL.
    /// </summary>
    utility::string_t getWebhookUrl() const;
    bool webhookUrlIsSet() const;
    void unsetWebhook_url();
    void setWebhookUrl(const utility::string_t& value);

    /// <summary>
    /// Partner access token. Only for clients that requires authentication. We recommend to avoid this param.
    /// </summary>
    utility::string_t getPartnerAccessToken() const;
    bool partnerAccessTokenIsSet() const;
    void unsetPartner_access_token();
    void setPartnerAccessToken(const utility::string_t& value);

    std::shared_ptr<LeadSubscriptionPostParamsCreate_allOf_partner_metadata> getPartnerMetadata() const;
    bool partnerMetadataIsSet() const;
    void unsetPartner_metadata();
    void setPartnerMetadata(const std::shared_ptr<LeadSubscriptionPostParamsCreate_allOf_partner_metadata>& value);

    /// <summary>
    /// Partner refresh token. Only for clients that requires authentication. We recommend to avoid this param.
    /// </summary>
    utility::string_t getPartnerRefreshToken() const;
    bool partnerRefreshTokenIsSet() const;
    void unsetPartner_refresh_token();
    void setPartnerRefreshToken(const utility::string_t& value);


protected:
    utility::string_t m_Lead_form_id;
    bool m_Lead_form_idIsSet;

    utility::string_t m_Webhook_url;
    bool m_Webhook_urlIsSet;

    utility::string_t m_Partner_access_token;
    bool m_Partner_access_tokenIsSet;

    std::shared_ptr<LeadSubscriptionPostParamsCreate_allOf_partner_metadata> m_Partner_metadata;
    bool m_Partner_metadataIsSet;

    utility::string_t m_Partner_refresh_token;
    bool m_Partner_refresh_tokenIsSet;

};


}
}
}
}

#endif /* ORG_OPENAPITOOLS_CLIENT_MODEL_LeadSubscriptionPostParamsCreate_H_ */
