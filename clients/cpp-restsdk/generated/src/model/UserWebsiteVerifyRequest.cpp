/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.14.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.18.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "CppRestOpenAPIClient/model/UserWebsiteVerifyRequest.h"

namespace org {
namespace openapitools {
namespace client {
namespace model {

UserWebsiteVerifyRequest::UserWebsiteVerifyRequest()
{
    m_Website = utility::conversions::to_string_t("");
    m_WebsiteIsSet = false;
    m_Verification_methodIsSet = false;
}

UserWebsiteVerifyRequest::~UserWebsiteVerifyRequest()
{
}

void UserWebsiteVerifyRequest::validate()
{
    // TODO: implement validation
}

web::json::value UserWebsiteVerifyRequest::toJson() const
{
    web::json::value val = web::json::value::object();
    if(m_WebsiteIsSet)
    {
        
        val[utility::conversions::to_string_t(_XPLATSTR("website"))] = ModelBase::toJson(m_Website);
    }
    if(m_Verification_methodIsSet)
    {
        
        utility::string_t refVal = fromVerification_methodEnum(m_Verification_method);
        val[utility::conversions::to_string_t(_XPLATSTR("verification_method"))] = ModelBase::toJson(refVal);
        
    }

    return val;
}

bool UserWebsiteVerifyRequest::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("website"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("website")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setWebsite;
            ok &= ModelBase::fromJson(fieldValue, refVal_setWebsite);
            setWebsite(refVal_setWebsite);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("verification_method"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("verification_method")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setVerificationMethod;
            ok &= ModelBase::fromJson(fieldValue, refVal_setVerificationMethod);
            
            setVerificationMethod(toVerification_methodEnum(refVal_setVerificationMethod));
            
        }
    }
    return ok;
}

void UserWebsiteVerifyRequest::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }
    if(m_WebsiteIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("website")), m_Website));
    }
    if(m_Verification_methodIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("verification_method")), fromVerification_methodEnum(m_Verification_method)));
    }
}

bool UserWebsiteVerifyRequest::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }

    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("website"))))
    {
        utility::string_t refVal_setWebsite;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("website"))), refVal_setWebsite );
        setWebsite(refVal_setWebsite);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("verification_method"))))
    {
        utility::string_t refVal_setVerificationMethod;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("verification_method"))), refVal_setVerificationMethod );
        setVerificationMethod(toVerification_methodEnum(refVal_setVerificationMethod));
    }
    return ok;
}

UserWebsiteVerifyRequest::Verification_methodEnum UserWebsiteVerifyRequest::toVerification_methodEnum(const utility::string_t& value) const
{
    
    if (value == utility::conversions::to_string_t("FILENAME")) {
        return Verification_methodEnum::FILENAME;
    }
    
    if (value == utility::conversions::to_string_t("METATAG")) {
        return Verification_methodEnum::METATAG;
    }
    
    if (value == utility::conversions::to_string_t("DNSTXT")) {
        return Verification_methodEnum::DNSTXT;
    }
    
    throw std::invalid_argument("Invalid value for conversion to Verification_methodEnum");
}


const utility::string_t UserWebsiteVerifyRequest::fromVerification_methodEnum(const Verification_methodEnum value) const
{
    switch(value)
    {
        
        case Verification_methodEnum::FILENAME: return utility::conversions::to_string_t("FILENAME");
        
        case Verification_methodEnum::METATAG: return utility::conversions::to_string_t("METATAG");
        
        case Verification_methodEnum::DNSTXT: return utility::conversions::to_string_t("DNSTXT");
        
    }
}


utility::string_t UserWebsiteVerifyRequest::getWebsite() const
{
    return m_Website;
}


void UserWebsiteVerifyRequest::setWebsite(const utility::string_t& value)
{
    m_Website = value;
    m_WebsiteIsSet = true;
}

bool UserWebsiteVerifyRequest::websiteIsSet() const
{
    return m_WebsiteIsSet;
}

void UserWebsiteVerifyRequest::unsetWebsite()
{
    m_WebsiteIsSet = false;
}
UserWebsiteVerifyRequest::Verification_methodEnum UserWebsiteVerifyRequest::getVerificationMethod() const
{
    return m_Verification_method;
}


void UserWebsiteVerifyRequest::setVerificationMethod(const Verification_methodEnum value)
{
    m_Verification_method = value;
    m_Verification_methodIsSet = true;
}

bool UserWebsiteVerifyRequest::verificationMethodIsSet() const
{
    return m_Verification_methodIsSet;
}

void UserWebsiteVerifyRequest::unsetVerification_method()
{
    m_Verification_methodIsSet = false;
}

}
}
}
}


