/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator
 * https://github.com/OpenAPITools/openapi-generator
 * Do not edit the class manually.
 */

#pragma once

#include "OpenAPIBaseModel.h"
#include "OpenAPICatalogsAiContentDisclosure.h"

namespace OpenAPI
{

/*
 * OpenAPICatalogsCreativeAssetsAttributes
 *
 * 
 */
class OPENAPI_API OpenAPICatalogsCreativeAssetsAttributes : public Model
{
public:
    virtual ~OpenAPICatalogsCreativeAssetsAttributes() {}
	bool FromJson(const TSharedPtr<FJsonValue>& JsonValue) final;
	void WriteJson(JsonWriter& Writer) const final;

	/* Link to the creative assets page. */
	TOptional<FString> AndroidDeepLink;
	/* Custom grouping of creative assets. */
	TOptional<FString> CustomLabel0;
	/* Custom grouping of creative assets. */
	TOptional<FString> CustomLabel1;
	/* Custom grouping of creative assets. */
	TOptional<FString> CustomLabel2;
	/* Custom grouping of creative assets. */
	TOptional<FString> CustomLabel3;
	/* Custom grouping of creative assets. */
	TOptional<FString> CustomLabel4;
	/* Brief description of the creative assets. */
	TOptional<FString> Description;
	/* The categorization of the product based on the standardized Google Product Taxonomy. This is a set taxonomy. Both the text values and numeric codes are accepted. */
	TOptional<FString> GoogleProductCategory;
	/* IOS deep link to the creative assets page. */
	TOptional<FString> IosDeepLink;
	/* Link to the creative assets page. */
	TOptional<FString> Link;
	/* The name of the creative assets. */
	TOptional<FString> Title;
	/* Visibility of the creative assets. Must be one of the following values (upper or lowercase): 'visible', 'hidden'. */
	TOptional<FString> Visibility;
	/* AI content disclosures for individual assets (image_link or video_link) on this creative assets item. Each entry declares which disclosure types apply to a single asset URL. */
	TOptional<TArray<OpenAPICatalogsAiContentDisclosure>> AiDisclosures;
	/* The creative assets image. */
	TOptional<FString> ImageLink;
	/* The creative assets video. */
	TOptional<FString> VideoLink;
};

}
