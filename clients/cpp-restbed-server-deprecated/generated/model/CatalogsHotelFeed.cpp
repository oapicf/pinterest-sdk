/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "CatalogsHotelFeed.h"

#include <string>
#include <vector>
#include <sstream>
#include <stdexcept>
#include <algorithm>
#include <boost/property_tree/ptree.hpp>
#include <boost/property_tree/json_parser.hpp>

using boost::property_tree::ptree;
using boost::property_tree::read_json;
using boost::property_tree::write_json;

namespace org {
namespace openapitools {
namespace server {
namespace model {

CatalogsHotelFeed::CatalogsHotelFeed(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}

CatalogsHotelFeed::~CatalogsHotelFeed() = default;

std::string CatalogsHotelFeed::toJsonString(bool prettyJson /* = false */)
{
    return toJsonString_internal(prettyJson);
}

void CatalogsHotelFeed::fromJsonString(std::string const& jsonString)
{
    fromJsonString_internal(jsonString);
}

boost::property_tree::ptree CatalogsHotelFeed::toPropertyTree()
{
    return toPropertyTree_internal();
}

void CatalogsHotelFeed::fromPropertyTree(boost::property_tree::ptree const& pt)
{
    fromPropertyTree_internal(pt);
}

std::string CatalogsHotelFeed::toJsonString_internal(bool prettyJson)
{
	std::stringstream ss;
	write_json(ss, this->toPropertyTree(), prettyJson);
	return ss.str();
}

void CatalogsHotelFeed::fromJsonString_internal(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	this->fromPropertyTree(pt);
}

ptree CatalogsHotelFeed::toPropertyTree_internal()
{
	ptree pt;
	ptree tmp_node;
	pt.put("catalog_id", m_Catalog_id);
	pt.put("catalog_type", m_Catalog_type);
	pt.put("created_at", m_Created_at);
	if (m_Credentials != nullptr) {
		pt.add_child("credentials", m_Credentials->toPropertyTree());
	}
	pt.put("default_locale", m_Default_locale);
	pt.put("id", m_Id);
	pt.put("location", m_Location);
	pt.put("name", m_Name);
	if (m_Preferred_processing_schedule != nullptr) {
		pt.add_child("preferred_processing_schedule", m_Preferred_processing_schedule->toPropertyTree());
	}
	pt.put("updated_at", m_Updated_at);
	return pt;
}

void CatalogsHotelFeed::fromPropertyTree_internal(ptree const &pt)
{
	ptree tmp_node;
	m_Catalog_id = pt.get("catalog_id", "");
	setCatalogType(pt.get("catalog_type", ""));
	m_Created_at = pt.get("created_at", "");
	if (pt.get_child_optional("credentials")) {
		m_Credentials = std::make_shared<CatalogsFeedCredentials>();
		m_Credentials->fromPropertyTree(pt.get_child("credentials"));
	}
	m_Default_locale = pt.get("default_locale", "");
	m_Id = pt.get("id", "");
	m_Location = pt.get("location", "");
	m_Name = pt.get("name", "");
	if (pt.get_child_optional("preferred_processing_schedule")) {
		m_Preferred_processing_schedule = std::make_shared<CatalogsFeedProcessingSchedule>();
		m_Preferred_processing_schedule->fromPropertyTree(pt.get_child("preferred_processing_schedule"));
	}
	m_Updated_at = pt.get("updated_at", "");
}

std::string CatalogsHotelFeed::getCatalogId() const
{
    return m_Catalog_id;
}

void CatalogsHotelFeed::setCatalogId(std::string value)
{
	m_Catalog_id = value;
}
std::string CatalogsHotelFeed::getCatalogType() const
{
    return m_Catalog_type;
}

void CatalogsHotelFeed::setCatalogType(std::string value)
{
	if (std::find(m_Catalog_typeEnum.begin(), m_Catalog_typeEnum.end(), value) != m_Catalog_typeEnum.end()) {
		m_Catalog_type = value;
	} else {
		throw std::runtime_error("Value " + value + " not allowed");
	}
}
std::string CatalogsHotelFeed::getCreatedAt() const
{
    return m_Created_at;
}

void CatalogsHotelFeed::setCreatedAt(std::string value)
{
	m_Created_at = value;
}
std::shared_ptr<CatalogsFeedCredentials> CatalogsHotelFeed::getCredentials() const
{
    return m_Credentials;
}

void CatalogsHotelFeed::setCredentials(std::shared_ptr<CatalogsFeedCredentials> value)
{
	m_Credentials = value;
}
std::shared_ptr<NullableCurrency> CatalogsHotelFeed::getDefaultCurrency() const
{
    return m_Default_currency;
}

void CatalogsHotelFeed::setDefaultCurrency(std::shared_ptr<NullableCurrency> value)
{
	m_Default_currency = value;
}
std::string CatalogsHotelFeed::getDefaultLocale() const
{
    return m_Default_locale;
}

void CatalogsHotelFeed::setDefaultLocale(std::string value)
{
	m_Default_locale = value;
}
std::shared_ptr<CatalogsFormat> CatalogsHotelFeed::getFormat() const
{
    return m_Format;
}

void CatalogsHotelFeed::setFormat(std::shared_ptr<CatalogsFormat> value)
{
	m_Format = value;
}
std::string CatalogsHotelFeed::getId() const
{
    return m_Id;
}

void CatalogsHotelFeed::setId(std::string value)
{
	m_Id = value;
}
std::string CatalogsHotelFeed::getLocation() const
{
    return m_Location;
}

void CatalogsHotelFeed::setLocation(std::string value)
{
	m_Location = value;
}
std::string CatalogsHotelFeed::getName() const
{
    return m_Name;
}

void CatalogsHotelFeed::setName(std::string value)
{
	m_Name = value;
}
std::shared_ptr<CatalogsFeedProcessingSchedule> CatalogsHotelFeed::getPreferredProcessingSchedule() const
{
    return m_Preferred_processing_schedule;
}

void CatalogsHotelFeed::setPreferredProcessingSchedule(std::shared_ptr<CatalogsFeedProcessingSchedule> value)
{
	m_Preferred_processing_schedule = value;
}
std::shared_ptr<CatalogsStatus> CatalogsHotelFeed::getStatus() const
{
    return m_Status;
}

void CatalogsHotelFeed::setStatus(std::shared_ptr<CatalogsStatus> value)
{
	m_Status = value;
}
std::string CatalogsHotelFeed::getUpdatedAt() const
{
    return m_Updated_at;
}

void CatalogsHotelFeed::setUpdatedAt(std::string value)
{
	m_Updated_at = value;
}

std::vector<CatalogsHotelFeed> createCatalogsHotelFeedVectorFromJsonString(const std::string& json)
{
    std::stringstream sstream(json);
    boost::property_tree::ptree pt;
    boost::property_tree::json_parser::read_json(sstream,pt);

    auto vec = std::vector<CatalogsHotelFeed>();
    for (const auto& child: pt) {
        vec.emplace_back(CatalogsHotelFeed(child.second));
    }

    return vec;
}

}
}
}
}

