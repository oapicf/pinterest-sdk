/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "PinCreate.h"

#include <string>
#include <vector>
#include <sstream>
#include <stdexcept>
#include <boost/property_tree/ptree.hpp>
#include <boost/property_tree/json_parser.hpp>

using boost::property_tree::ptree;
using boost::property_tree::read_json;
using boost::property_tree::write_json;

namespace org {
namespace openapitools {
namespace server {
namespace model {

PinCreate::PinCreate(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}

PinCreate::~PinCreate() = default;

std::string PinCreate::toJsonString(bool prettyJson /* = false */)
{
    return toJsonString_internal(prettyJson);
}

void PinCreate::fromJsonString(std::string const& jsonString)
{
    fromJsonString_internal(jsonString);
}

boost::property_tree::ptree PinCreate::toPropertyTree()
{
    return toPropertyTree_internal();
}

void PinCreate::fromPropertyTree(boost::property_tree::ptree const& pt)
{
    fromPropertyTree_internal(pt);
}

std::string PinCreate::toJsonString_internal(bool prettyJson)
{
	std::stringstream ss;
	write_json(ss, this->toPropertyTree(), prettyJson);
	return ss.str();
}

void PinCreate::fromJsonString_internal(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	this->fromPropertyTree(pt);
}

ptree PinCreate::toPropertyTree_internal()
{
	ptree pt;
	ptree tmp_node;
	if (m_Ai_disclosures != nullptr) {
		pt.add_child("ai_disclosures", m_Ai_disclosures->toPropertyTree());
	}
	pt.put("alt_text", m_Alt_text);
	pt.put("board_id", m_Board_id);
	pt.put("board_section_id", m_Board_section_id);
	pt.put("description", m_Description);
	pt.put("dominant_color", m_Dominant_color);
	pt.put("link", m_Link);
	if (m_Media_source != nullptr) {
		pt.add_child("media_source", m_Media_source->toPropertyTree());
	}
	pt.put("parent_pin_id", m_Parent_pin_id);
	pt.put("sponsor_id", m_Sponsor_id);
	pt.put("title", m_Title);
	return pt;
}

void PinCreate::fromPropertyTree_internal(ptree const &pt)
{
	ptree tmp_node;
	if (pt.get_child_optional("ai_disclosures")) {
		m_Ai_disclosures = nullptr;
		m_Ai_disclosures->fromPropertyTree(pt.get_child("ai_disclosures"));
	}
	m_Alt_text = pt.get("alt_text", "");
	m_Board_id = pt.get("board_id", "");
	m_Board_section_id = pt.get("board_section_id", "");
	m_Description = pt.get("description", "");
	m_Dominant_color = pt.get("dominant_color", "");
	m_Link = pt.get("link", "");
	if (pt.get_child_optional("media_source")) {
		m_Media_source = std::make_shared<PinMediaSource>();
		m_Media_source->fromPropertyTree(pt.get_child("media_source"));
	}
	m_Parent_pin_id = pt.get("parent_pin_id", "");
	m_Sponsor_id = pt.get("sponsor_id", "");
	m_Title = pt.get("title", "");
}

std::shared_ptr<AiDisclosures> PinCreate::getAiDisclosures() const
{
    return m_Ai_disclosures;
}

void PinCreate::setAiDisclosures(std::shared_ptr<AiDisclosures> value)
{
	m_Ai_disclosures = value;
}
std::string PinCreate::getAltText() const
{
    return m_Alt_text;
}

void PinCreate::setAltText(std::string value)
{
	m_Alt_text = value;
}
std::string PinCreate::getBoardId() const
{
    return m_Board_id;
}

void PinCreate::setBoardId(std::string value)
{
	m_Board_id = value;
}
std::string PinCreate::getBoardSectionId() const
{
    return m_Board_section_id;
}

void PinCreate::setBoardSectionId(std::string value)
{
	m_Board_section_id = value;
}
std::string PinCreate::getDescription() const
{
    return m_Description;
}

void PinCreate::setDescription(std::string value)
{
	m_Description = value;
}
std::string PinCreate::getDominantColor() const
{
    return m_Dominant_color;
}

void PinCreate::setDominantColor(std::string value)
{
	m_Dominant_color = value;
}
std::string PinCreate::getLink() const
{
    return m_Link;
}

void PinCreate::setLink(std::string value)
{
	m_Link = value;
}
std::shared_ptr<PinMediaSource> PinCreate::getMediaSource() const
{
    return m_Media_source;
}

void PinCreate::setMediaSource(std::shared_ptr<PinMediaSource> value)
{
	m_Media_source = value;
}
std::string PinCreate::getParentPinId() const
{
    return m_Parent_pin_id;
}

void PinCreate::setParentPinId(std::string value)
{
	m_Parent_pin_id = value;
}
std::string PinCreate::getSponsorId() const
{
    return m_Sponsor_id;
}

void PinCreate::setSponsorId(std::string value)
{
	m_Sponsor_id = value;
}
std::string PinCreate::getTitle() const
{
    return m_Title;
}

void PinCreate::setTitle(std::string value)
{
	m_Title = value;
}

std::vector<PinCreate> createPinCreateVectorFromJsonString(const std::string& json)
{
    std::stringstream sstream(json);
    boost::property_tree::ptree pt;
    boost::property_tree::json_parser::read_json(sstream,pt);

    auto vec = std::vector<PinCreate>();
    for (const auto& child: pt) {
        vec.emplace_back(PinCreate(child.second));
    }

    return vec;
}

}
}
}
}

