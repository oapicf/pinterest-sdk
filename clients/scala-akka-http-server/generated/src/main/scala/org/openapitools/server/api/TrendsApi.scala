...
class TrendsApi(
    trendsService: TrendsApiService,
    trendsMarshaller: TrendsApiMarshaller
) {

  
  import trendsMarshaller._

  lazy val route: Route =
    path("trends" / "editorial_articles") { 
      get { 
        parameters("region".as[String]) { (region) => 
            trendsService.trendsEditorialArticlesList(region = region)
        }
      }
    } ~
    path("trends" / "topics" / "featured") { 
      get { 
        parameters("interest".as[String].?, "region".as[String]) { (interest, region) => 
            trendsService.trendsFeaturedTopicsList(region = region, interest = interest)
        }
      }
    } ~
    path("trends" / "product_categories" / "details") { 
      get { 
        parameters("product_categories".as[String], "region".as[String], "lookback_window".as[String].?, "engagement_type".as[String].?) { (productCategories, region, lookbackWindow, engagementType) => 
            trendsService.trendsProductCategoriesDetailsList(productCategories = productCategories, region = region, lookbackWindow = lookbackWindow, engagementType = engagementType)
        }
      }
    } ~
    path("trends" / "product_categories" / "trending") { 
      get { 
        parameters("region".as[String], "verticals".as[String].?, "ages".as[String].?, "genders".as[String].?, "engagement_type".as[String].?) { (region, verticals, ages, genders, engagementType) => 
            trendsService.trendsProductCategoriesTrendingList(region = region, verticals = verticals, ages = ages, genders = genders, engagementType = engagementType)
        }
      }
    }
}
...
trait TrendsApiService {

  def trendsEditorialArticlesList200(responseTrendsEditorialarray: Seq[TrendsEditorial])(implicit toEntityMarshallerTrendsEditorialarray: ToEntityMarshaller[Seq[TrendsEditorial]]): Route =
    complete((200, responseTrendsEditorialarray))
  def trendsEditorialArticlesList400(responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((400, responseError))
  def trendsEditorialArticlesList401(responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((401, responseError))
  def trendsEditorialArticlesList403(responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((403, responseError))
  def trendsEditorialArticlesList404(responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((404, responseError))
  def trendsEditorialArticlesList429(responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((429, responseError))
  def trendsEditorialArticlesListDefault(statusCode: Int, responseError: Error)(implicit toEntityMarshallerError: ToEntityMarshaller[Error]): Route =
    complete((statusCode, responseError))
  /**
   * Code: 200, Message: The request has succeeded., DataType: Seq[TrendsEditorial]
   ...
   */
  def trendsEditorialArticlesList(region: String)
      (implicit toEntityMarshallerTrendsEditorialarray: ToEntityMarshaller[Seq[TrendsEditorial]], toEntityMarshallerError: ToEntityMarshaller[Error]): Route
...
trait TrendsApiMarshaller {


  implicit def toEntityMarshallerTrendsEditorialarray: ToEntityMarshaller[Seq[TrendsEditorial]]
