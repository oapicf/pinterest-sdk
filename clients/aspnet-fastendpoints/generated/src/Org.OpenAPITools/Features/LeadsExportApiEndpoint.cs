...
public class LeadsExportCreateEndpoint : FastEndpoints.Endpoint<LeadsExportCreateRequest, LeadsExports>
{
    public override void Configure()
    {
        ...
        Summary(s => {
            s.Summary = "Create a request to export leads collected from a lead ad";
            s.RequestParam(r => r.AdAccountId, "Unique identifier of an ad account.");
            s.RequestParam(r => r.leadsExportsCreate, "");
    ...
    public override async Task HandleAsync(LeadsExportCreateRequest req, CancellationToken ct)
    {
        //Response = new()
        //{
            //...
...
public class LeadsExportGetEndpoint : FastEndpoints.Endpoint<LeadsExportGetRequest, LeadsExportResponseData>
{
    public override void Configure()
    {
        ...
        Summary(s => {
            s.Summary = "Get the lead export from the lead export create call";
            s.RequestParam(r => r.AdAccountId, "Unique identifier of an ad account.");
            s.RequestParam(r => r.LeadsExportId, "lead_export_id token returned from the create a lead export endpoint");
            s.Responses[200] = "The request has succeeded.";
            s.Responses[400] = "The request could not be understood by the server due to unexpected data.";
            s.Responses[401] = "Authentication is required and has either failed or not been provided.";
            s.Responses[403] = "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.";
    ...
    public override async Task HandleAsync(LeadsExportGetRequest req, CancellationToken ct)
    {
        //Response = new()
        //{
            //...
