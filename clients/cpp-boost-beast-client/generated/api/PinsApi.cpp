...
PinsApiException::PinsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status PinsApiException::getStatus() const
{
    return m_status;
}
...
std::map<std::string, std::shared_ptr<std::map>>
PinsApi::multiPins_analytics(
    ...
    std::string path = m_context + "/pins/analytics";
    ...
    std::map<std::string, std::shared_ptr<std::map>> deserializedResponse = std::map<std::string, std::shared_ptr<std::map>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::map<std::string, std::shared_ptr<std::map>>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw PinsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw PinsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw PinsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw PinsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw PinsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw PinsApiException(statusCode, "An unexpected error response.");
}
std::map<std::string, std::shared_ptr<PinAnalyticsMetricsResponse>>
PinsApi::pins_analytics(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/pins/{pin_id}/analytics";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "pin_id", pinId);
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
    ...
    std::map<std::string, std::shared_ptr<PinAnalyticsMetricsResponse>> deserializedResponse = std::map<std::string, std::shared_ptr<PinAnalyticsMetricsResponse>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::map<std::string, std::shared_ptr<PinAnalyticsMetricsResponse>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            true);
...
PinsApi::pins_create(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/pins";
    std::map<std::string, std::string> headers;
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(pinCreate));
    } else {
    ...
    std::shared_ptr<Pin> deserializedResponse = std::make_shared<Pin>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Pin>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Pins_list_200_response>
PinsApi::pins_list(
    ...
    if (pinFilter) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "pin_filter",
            serializeQueryParameterValue(*pinFilter));
    }
    if (pinMetrics) {
        appendQueryParameter(
            queryParameterStream,
    ...
    if (includeProtectedPins) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*includeProtectedPins));
    }
    if (pinType) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "pin_type",
            serializeQueryParameterValue(*pinType));
    }
    if (creativeTypes) {
        appendMultiQueryParameters(
            queryParameterStream,
    ...
    if (domain) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "domain",
            serializeQueryParameterValue(*domain));
    }
    if (domains) {
        appendMultiQueryParameters(
            queryParameterStream,
    ...
    if (includeProductTagObj) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*includeProductTagObj));
    }
    if (bookmark) {
        appendQueryParameter(
            queryParameterStream,
    ...
    std::shared_ptr<Pins_list_200_response> deserializedResponse = std::make_shared<Pins_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Pins_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
PinsApi::pins_delete(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/pins/{pin_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "pin_id", pinId);
    // query params
...
PinsApi::pins_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(pinUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
PinsApi::pins_save(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/pins/{pin_id}/save";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(pinsSaveRequestCreate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string PinsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void PinsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void PinsApi::handleStdException(const std::exception&) {
    throw;
}

void PinsApi::handleUncaughtException() {
    throw;
}
