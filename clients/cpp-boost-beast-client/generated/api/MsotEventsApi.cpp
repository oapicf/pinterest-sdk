...
MsotEventsApiException::MsotEventsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status MsotEventsApiException::getStatus() const
{
    return m_status;
}
...
void
MsotEventsApi::msotEvents_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/msot/events";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(conversionMSOTEventsCreate));
    } else {
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw MsotEventsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw MsotEventsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw MsotEventsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw MsotEventsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw MsotEventsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw MsotEventsApiException(statusCode, "An unexpected error response.");
}


std::string MsotEventsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void MsotEventsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void MsotEventsApi::handleStdException(const std::exception&) {
    throw;
}

void MsotEventsApi::handleUncaughtException() {
    throw;
}
