...
CatalogFeedsApiException::CatalogFeedsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CatalogFeedsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<Feed_processing_results_list_200_response>
CatalogFeedsApi::feedProcessingResults_list(
    ...
    std::string path = m_context + "/catalogs/feeds/{feed_id}/processing_results";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "feed_id", feedId);
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (adAccountId) {
    ...
    std::shared_ptr<Feed_processing_results_list_200_response> deserializedResponse = std::make_shared<Feed_processing_results_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Feed_processing_results_list_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CatalogFeedsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CatalogFeedsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CatalogFeedsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CatalogFeedsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CatalogFeedsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CatalogFeedsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<CatalogsFeed>
CatalogFeedsApi::feeds_create(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/feeds";
    std::map<std::string, std::string> headers;
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogsFeedCreateRequestSchema));
    } else {
    ...
    std::shared_ptr<CatalogsFeed> deserializedResponse = std::make_shared<CatalogsFeed>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsFeed>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Feeds_list_200_response>
CatalogFeedsApi::feeds_list(
    ...
    std::shared_ptr<Feeds_list_200_response> deserializedResponse = std::make_shared<Feeds_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Feeds_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
CatalogFeedsApi::feeds_delete(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/feeds/{feed_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "feed_id", feedId);
    // query params
...
CatalogFeedsApi::feeds_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogsFeedUpdateRequestSchema));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::shared_ptr<CatalogsFeedIngestion>
CatalogFeedsApi::feeds_ingest(
    const std::string& feedId, const boost::optional<std::string>& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/feeds/{feed_id}/ingest";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "feed_id", feedId);
    // query params
    ...
    std::shared_ptr<CatalogsFeedIngestion> deserializedResponse = std::make_shared<CatalogsFeedIngestion>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsFeedIngestion>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Items_issues_list_200_response>
CatalogFeedsApi::itemsIssues_list(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/processing_results/{processing_result_id}/item_issues";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "processing_result_id", processingResultId);
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (itemNumbers) {
    ...
    if (itemValidationIssue) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*itemValidationIssue));
    }
    if (adAccountId) {
        appendQueryParameter(
            queryParameterStream,
    ...
    std::shared_ptr<Items_issues_list_200_response> deserializedResponse = std::make_shared<Items_issues_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Items_issues_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string CatalogFeedsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CatalogFeedsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CatalogFeedsApi::handleStdException(const std::exception&) {
    throw;
}

void CatalogFeedsApi::handleUncaughtException() {
    throw;
}
