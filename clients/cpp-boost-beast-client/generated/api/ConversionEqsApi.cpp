...
ConversionEqsApiException::ConversionEqsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ConversionEqsApiException::getStatus() const
{
    return m_status;
}
...
std::vector<std::shared_ptr<EventQualityScore>>
ConversionEqsApi::conversionEqs_list(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/conversion_eqs";
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "lookback_period",
            serializeQueryParameterValue(lookbackPeriod));
    if (sourcePlatform) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "source_platform",
            serializeQueryParameterValue(*sourcePlatform));
    }
    if (ingestionSource) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "ingestion_source",
            serializeQueryParameterValue(*ingestionSource));
    }
    ...
    std::vector<std::shared_ptr<EventQualityScore>> deserializedResponse = std::vector<std::shared_ptr<EventQualityScore>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<EventQualityScore>>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ConversionEqsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ConversionEqsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ConversionEqsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw ConversionEqsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ConversionEqsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw ConversionEqsApiException(statusCode, "An unexpected error response.");
}


std::string ConversionEqsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ConversionEqsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ConversionEqsApi::handleStdException(const std::exception&) {
    throw;
}

void ConversionEqsApi::handleUncaughtException() {
    throw;
}
