...
NotificationApiException::NotificationApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status NotificationApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<NotificationResponse>
NotificationApi::notification_post(
    ...
    std::string path = m_context + "/notifications";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(notificationPostRequest));
    } else {
    ...
    std::shared_ptr<NotificationResponse> deserializedResponse = std::make_shared<NotificationResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<NotificationResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw NotificationApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    throw NotificationApiException(statusCode, "An unexpected error response.");
}


std::string NotificationApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void NotificationApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void NotificationApi::handleStdException(const std::exception&) {
    throw;
}

void NotificationApi::handleUncaughtException() {
    throw;
}
