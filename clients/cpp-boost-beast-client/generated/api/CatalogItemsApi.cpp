...
CatalogItemsApiException::CatalogItemsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CatalogItemsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CatalogsItemsBatch>
CatalogItemsApi::itemsBatch_get(
    const std::string& batchId, const boost::optional<std::string>& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/items/batch/{batch_id}";
    ...
    std::shared_ptr<CatalogsItemsBatch> deserializedResponse = std::make_shared<CatalogsItemsBatch>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsItemsBatch>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CatalogItemsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CatalogItemsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CatalogItemsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CatalogItemsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CatalogItemsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CatalogItemsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<CatalogsItemsBatch>
CatalogItemsApi::itemsBatch_post(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/items/batch";
    std::map<std::string, std::string> headers;
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogsItemsBatchPostRequest));
    } else {
...
std::shared_ptr<Items_post_200_response>
CatalogItemsApi::items_post(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/items";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogsItemsRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // query params
    ...
    std::shared_ptr<Items_post_200_response> deserializedResponse = std::make_shared<Items_post_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Items_post_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string CatalogItemsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CatalogItemsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CatalogItemsApi::handleStdException(const std::exception&) {
    throw;
}

void CatalogItemsApi::handleUncaughtException() {
    throw;
}
