...
PromotionsApiException::PromotionsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status PromotionsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<PromotionsResponse>
PromotionsApi::promotions_create(
    const std::string& adAccountId, const std::vector<std::shared_ptr<PromotionCreate>>& promotionCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/promotions";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(promotionCreate));
    } else {
    ...
    std::shared_ptr<PromotionsResponse> deserializedResponse = std::make_shared<PromotionsResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<PromotionsResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw PromotionsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw PromotionsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw PromotionsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw PromotionsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw PromotionsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw PromotionsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Promotions_list_200_response>
PromotionsApi::promotions_list(
    ...
    std::shared_ptr<Promotions_list_200_response> deserializedResponse = std::make_shared<Promotions_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Promotions_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
PromotionsApi::promotions_update(
    const std::string& adAccountId, const std::vector<std::shared_ptr<PromotionBatchUpdate>>& promotionBatchUpdate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/promotions";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(promotionBatchUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::shared_ptr<Promotion>
PromotionsApi::promotions_delete(
    const std::string& promotionId, const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/promotions/{promotion_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "promotion_id", promotionId);
    replacePathParameter(path, "ad_account_id", adAccountId);
    ...
    std::shared_ptr<Promotion> deserializedResponse = std::make_shared<Promotion>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Promotion>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string PromotionsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void PromotionsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void PromotionsApi::handleStdException(const std::exception&) {
    throw;
}

void PromotionsApi::handleUncaughtException() {
    throw;
}
