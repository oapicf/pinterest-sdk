...
KeywordsApiException::KeywordsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status KeywordsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<KeywordsMetricsArrayResponse>
KeywordsApi::countryKeywordsMetrics_get(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/keywords/metrics";
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "country_code",
            serializeQueryParameterValue(countryCode));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "keywords",
    ...
    std::shared_ptr<KeywordsMetricsArrayResponse> deserializedResponse = std::make_shared<KeywordsMetricsArrayResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<KeywordsMetricsArrayResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw KeywordsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw KeywordsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw KeywordsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw KeywordsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw KeywordsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw KeywordsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Keywords>
KeywordsApi::keywords_create(
    const std::string& adAccountId, const std::shared_ptr<KeywordsCreate>& keywordsCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/keywords";
    std::map<std::string, std::string> headers;
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(keywordsCreate));
    } else {
    ...
    std::shared_ptr<Keywords> deserializedResponse = std::make_shared<Keywords>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Keywords>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Keywords_get_200_response>
KeywordsApi::keywords_get(
    ...
    std::shared_ptr<Keywords_get_200_response> deserializedResponse = std::make_shared<Keywords_get_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Keywords_get_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
KeywordsApi::keywords_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(keywordsUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::shared_ptr<TrendingKeywordsResponse>
KeywordsApi::trendingKeywords_list(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/trends/keywords/{region}/top/{trend_type}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "region", region);
    replacePathParameter(path, "trend_type", trendType);
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (interests) {
    ...
    if (normalizeAgainstGroup) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*normalizeAgainstGroup));
    }
    if (limit) {
        appendQueryParameter(
            queryParameterStream,
    ...
    if (includeDemographics) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*includeDemographics));
    }
    path += queryParameterStream.str();
    ...
    std::shared_ptr<TrendingKeywordsResponse> deserializedResponse = std::make_shared<TrendingKeywordsResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<TrendingKeywordsResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string KeywordsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void KeywordsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void KeywordsApi::handleStdException(const std::exception&) {
    throw;
}

void KeywordsApi::handleUncaughtException() {
    throw;
}
