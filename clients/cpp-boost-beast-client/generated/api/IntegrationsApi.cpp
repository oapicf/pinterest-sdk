...
IntegrationsApiException::IntegrationsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status IntegrationsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<IntegrationMetadata>
IntegrationsApi::integrationsCommerce_del(
    const std::string& externalBusinessId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations/commerce/{external_business_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "external_business_id", externalBusinessId);
    ...
    std::shared_ptr<IntegrationMetadata> deserializedResponse = std::make_shared<IntegrationMetadata>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<IntegrationMetadata>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw IntegrationsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw IntegrationsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw IntegrationsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw IntegrationsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw IntegrationsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw IntegrationsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<IntegrationMetadata>
IntegrationsApi::integrationsCommerce_get(
    const std::string& externalBusinessId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations/commerce/{external_business_id}";
    std::map<std::string, std::string> headers;
...
std::shared_ptr<IntegrationMetadata>
IntegrationsApi::integrationsCommerce_patch(
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(integrationMetadataUpdate));
    } else {
...
std::shared_ptr<IntegrationMetadata>
IntegrationsApi::integrationsCommerce_post(
    const std::shared_ptr<IntegrationMetadataCreate>& integrationMetadataCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations/commerce";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(integrationMetadataCreate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
...
std::shared_ptr<IntegrationLogsSuccessResponse>
IntegrationsApi::integrationsLogs_post(
    const std::shared_ptr<IntegrationLogsRequestCreate>& integrationLogsRequestCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations/logs";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(integrationLogsRequestCreate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    ...
    std::shared_ptr<IntegrationLogsSuccessResponse> deserializedResponse = std::make_shared<IntegrationLogsSuccessResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<IntegrationLogsSuccessResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw IntegrationsApiException(statusCode, "The server could not understand the request due to invalid syntax.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw IntegrationsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
...
std::shared_ptr<IntegrationRecord>
IntegrationsApi::integrations_getById(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations/{id}";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<IntegrationRecord> deserializedResponse = std::make_shared<IntegrationRecord>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<IntegrationRecord>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Integrations_get_list_200_response>
IntegrationsApi::integrations_getList(
    const boost::optional<std::string>& bookmark, const boost::optional<int32_t>& pageSize) {
    std::string serializedRequestBody;
    std::string path = m_context + "/integrations";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    std::shared_ptr<Integrations_get_list_200_response> deserializedResponse = std::make_shared<Integrations_get_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Integrations_get_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string IntegrationsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void IntegrationsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void IntegrationsApi::handleStdException(const std::exception&) {
    throw;
}

void IntegrationsApi::handleUncaughtException() {
    throw;
}
