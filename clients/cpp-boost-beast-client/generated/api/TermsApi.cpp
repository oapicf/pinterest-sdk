...
TermsApiException::TermsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status TermsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<RelatedTerms>
TermsApi::termsRelated_list(
    const std::vector<std::string>& terms) {
    std::string serializedRequestBody;
    std::string path = m_context + "/terms/related";
    ...
    std::shared_ptr<RelatedTerms> deserializedResponse = std::make_shared<RelatedTerms>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<RelatedTerms>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw TermsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw TermsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw TermsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw TermsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw TermsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw TermsApiException(statusCode, "An unexpected error response.");
}
std::vector<std::string>
TermsApi::termsSuggested_list(
    const std::string& term, const boost::optional<int32_t>& limit) {
    std::string serializedRequestBody;
    std::string path = m_context + "/terms/suggested";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
...
std::string TermsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void TermsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void TermsApi::handleStdException(const std::exception&) {
    throw;
}

void TermsApi::handleUncaughtException() {
    throw;
}
