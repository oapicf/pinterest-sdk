...
LabelsApiException::LabelsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status LabelsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<LabeledEntities>
LabelsApi::labels_apply(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/labels/{label_id}/apply";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(labeledEntitiesCreate));
    } else {
    ...
    replacePathParameter(path, "label_id", labelId);
    ...
    std::shared_ptr<LabeledEntities> deserializedResponse = std::make_shared<LabeledEntities>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LabeledEntities>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw LabelsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw LabelsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw LabelsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw LabelsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw LabelsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw LabelsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<LabelsResponse>
LabelsApi::labels_create(
    const std::string& adAccountId, const std::shared_ptr<LabelCreateRequest>& labelCreateRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/labels";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(labelCreateRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
    ...
    std::shared_ptr<LabelsResponse> deserializedResponse = std::make_shared<LabelsResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LabelsResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Labels_list_200_response>
LabelsApi::labels_list(
    ...
    std::shared_ptr<Labels_list_200_response> deserializedResponse = std::make_shared<Labels_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Labels_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
LabelsApi::labels_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(labelUpdateRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::shared_ptr<LabeledEntities>
LabelsApi::labels_remove(
    const std::string& adAccountId, const std::string& labelId, const std::shared_ptr<LabeledEntitiesCreate>& labeledEntitiesCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/labels/{label_id}/remove";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
...
std::string LabelsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void LabelsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void LabelsApi::handleStdException(const std::exception&) {
    throw;
}

void LabelsApi::handleUncaughtException() {
    throw;
}
