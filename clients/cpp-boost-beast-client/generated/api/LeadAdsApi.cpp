...
LeadAdsApiException::LeadAdsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status LeadAdsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<LeadSubscription>
LeadAdsApi::adAccountsSubscriptions_delById(
    const std::string& adAccountId, const std::string& subscriptionId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/leads/subscriptions/{subscription_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "subscription_id", subscriptionId);
    ...
    std::shared_ptr<LeadSubscription> deserializedResponse = std::make_shared<LeadSubscription>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LeadSubscription>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw LeadAdsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw LeadAdsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw LeadAdsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw LeadAdsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw LeadAdsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw LeadAdsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<LeadSubscription>
LeadAdsApi::adAccountsSubscriptions_getById(
    const std::string& adAccountId, const std::string& subscriptionId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/leads/subscriptions/{subscription_id}";
    std::map<std::string, std::string> headers;
...
std::shared_ptr<Ad_accounts_subscriptions_get_list_200_response>
LeadAdsApi::adAccountsSubscriptions_getList(
    const std::string& adAccountId, const boost::optional<std::string>& bookmark, const boost::optional<int32_t>& pageSize) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/leads/subscriptions";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    // query params
    ...
    std::shared_ptr<Ad_accounts_subscriptions_get_list_200_response> deserializedResponse = std::make_shared<Ad_accounts_subscriptions_get_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Ad_accounts_subscriptions_get_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<LeadSubscription>
LeadAdsApi::adAccountsSubscriptions_post(
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(leadSubscriptionPostParamsCreate));
    } else {
...
std::string LeadAdsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void LeadAdsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void LeadAdsApi::handleStdException(const std::exception&) {
    throw;
}

void LeadAdsApi::handleUncaughtException() {
    throw;
}
