...
ConversionDeletionRequestsApiException::ConversionDeletionRequestsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ConversionDeletionRequestsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<ConversionDeletionRequest>
ConversionDeletionRequestsApi::conversionDeletionRequest_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/conversion_deletion_requests";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(conversionDeletionRequestCreate));
    } else {
    ...
    std::shared_ptr<ConversionDeletionRequest> deserializedResponse = std::make_shared<ConversionDeletionRequest>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ConversionDeletionRequest>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ConversionDeletionRequestsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ConversionDeletionRequestsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ConversionDeletionRequestsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw ConversionDeletionRequestsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ConversionDeletionRequestsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw ConversionDeletionRequestsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Conversion_deletion_request_list_200_response>
ConversionDeletionRequestsApi::conversionDeletionRequest_list(
    ...
    std::shared_ptr<Conversion_deletion_request_list_200_response> deserializedResponse = std::make_shared<Conversion_deletion_request_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Conversion_deletion_request_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ConversionDeletionRequest>
ConversionDeletionRequestsApi::conversionDeletionRequest_delete(
    const std::string& requestId, const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/conversion_deletion_requests/{request_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "request_id", requestId);
    replacePathParameter(path, "ad_account_id", adAccountId);
...
std::shared_ptr<ConversionDeletionRequest>
ConversionDeletionRequestsApi::conversionDeletionRequest_get(
    const std::string& requestId, const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/conversion_deletion_requests/{request_id}";
    std::map<std::string, std::string> headers;
...
std::string ConversionDeletionRequestsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ConversionDeletionRequestsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ConversionDeletionRequestsApi::handleStdException(const std::exception&) {
    throw;
}

void ConversionDeletionRequestsApi::handleUncaughtException() {
    throw;
}
