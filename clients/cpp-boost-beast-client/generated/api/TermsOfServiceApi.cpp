...
TermsOfServiceApiException::TermsOfServiceApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status TermsOfServiceApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<TermsOfService>
TermsOfServiceApi::termsOfService_get(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/terms_of_service";
    ...
    if (includeHtml) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "include_html",
            serializeQueryParameterValue(*includeHtml));
    }
    if (tosType) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "tos_type",
            serializeQueryParameterValue(*tosType));
    }
    ...
    std::shared_ptr<TermsOfService> deserializedResponse = std::make_shared<TermsOfService>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<TermsOfService>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw TermsOfServiceApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw TermsOfServiceApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw TermsOfServiceApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw TermsOfServiceApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw TermsOfServiceApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw TermsOfServiceApiException(statusCode, "An unexpected error response.");
}


std::string TermsOfServiceApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void TermsOfServiceApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void TermsOfServiceApi::handleStdException(const std::exception&) {
    throw;
}

void TermsOfServiceApi::handleUncaughtException() {
    throw;
}
