...
CustomerListsApiException::CustomerListsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CustomerListsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CustomerList>
CustomerListsApi::customerLists_create(
    const std::string& adAccountId, const std::shared_ptr<CustomerListCreate>& customerListCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_lists";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(customerListCreate));
    } else {
    ...
    std::shared_ptr<CustomerList> deserializedResponse = std::make_shared<CustomerList>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CustomerList>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CustomerListsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CustomerListsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CustomerListsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CustomerListsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CustomerListsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CustomerListsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Customer_lists_list_200_response>
CustomerListsApi::customerLists_list(
    ...
    if (excludeNca) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "exclude_nca",
            serializeQueryParameterValue(*excludeNca));
    }
    ...
    std::shared_ptr<Customer_lists_list_200_response> deserializedResponse = std::make_shared<Customer_lists_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Customer_lists_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<CustomerList>
CustomerListsApi::customerLists_get(
    const std::string& adAccountId, const std::string& customerListId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "customer_list_id", customerListId);
...
std::shared_ptr<CustomerList>
CustomerListsApi::customerLists_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(customerListUpdateWithRequiredBody));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string CustomerListsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CustomerListsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CustomerListsApi::handleStdException(const std::exception&) {
    throw;
}

void CustomerListsApi::handleUncaughtException() {
    throw;
}
