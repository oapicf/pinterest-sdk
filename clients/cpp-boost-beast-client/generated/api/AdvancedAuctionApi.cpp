...
AdvancedAuctionApiException::AdvancedAuctionApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status AdvancedAuctionApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<AdvancedAuctionItems>
AdvancedAuctionApi::advancedAuctionItemsGet_post(
    ...
    std::string path = m_context + "/advanced_auction/items/get";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(advancedAuctionItemsGetRequest));
    } else {
    ...
    std::shared_ptr<AdvancedAuctionItems> deserializedResponse = std::make_shared<AdvancedAuctionItems>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<AdvancedAuctionItems>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw AdvancedAuctionApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw AdvancedAuctionApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw AdvancedAuctionApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw AdvancedAuctionApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw AdvancedAuctionApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    if (statusCode == boost::beast::http::status(500)) {
        throw AdvancedAuctionApiException(statusCode, "The server encountered an unexpected condition that prevented it from fulfilling the request.");
    }
    throw AdvancedAuctionApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<AdvancedAuctionProcessedItems>
AdvancedAuctionApi::advancedAuctionItemsSubmit_post(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/advanced_auction/items/submit";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(advancedAuctionItemsSubmitRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // query params
    ...
    std::shared_ptr<AdvancedAuctionProcessedItems> deserializedResponse = std::make_shared<AdvancedAuctionProcessedItems>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<AdvancedAuctionProcessedItems>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(206)) {
        ResponseBodyDeserializer<std::shared_ptr<AdvancedAuctionProcessedItems>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
...
std::string AdvancedAuctionApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void AdvancedAuctionApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void AdvancedAuctionApi::handleStdException(const std::exception&) {
    throw;
}

void AdvancedAuctionApi::handleUncaughtException() {
    throw;
}
