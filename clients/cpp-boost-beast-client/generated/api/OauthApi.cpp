...
OauthApiException::OauthApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status OauthApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<ConversionAccessToken>
OauthApi::oauth_conversionToken(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/oauth/conversion_token";
    ...
    std::shared_ptr<ConversionAccessToken> deserializedResponse = std::make_shared<ConversionAccessToken>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ConversionAccessToken>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw OauthApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw OauthApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw OauthApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw OauthApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw OauthApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw OauthApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<OauthAccessToken>
OauthApi::oauth_token(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/oauth/token";
    std::map<std::string, std::string> headers;
    ...
    formParameters.emplace_back(
        "grant_type",
        toFormParameterValue(grantType),
        false);
    formParameters.emplace_back(
        "code",
        toFormParameterValue(code),
        false);
    formParameters.emplace_back(
        "continuous_refresh",
        toFormParameterValue(continuousRefresh),
        false);
    formParameters.emplace_back(
        "redirect_uri",
        toFormParameterValue(redirectUri),
        false);
    formParameters.emplace_back(
        "refresh_token",
        toFormParameterValue(refreshToken),
        false);
    formParameters.emplace_back(
        "scope",
        toFormParameterValue(scope),
        false);
    ...
    std::shared_ptr<OauthAccessToken> deserializedResponse = std::make_shared<OauthAccessToken>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<OauthAccessToken>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
OauthApi::token_revoke(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/oauth/token/revoke";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/x-www-form-urlencoded", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    ...
    formParameters.emplace_back(
        "token",
        toFormParameterValue(token),
        false);
    formParameters.emplace_back(
        "token_type_hint",
        toFormParameterValue(tokenTypeHint),
        false);
    if (normalizeMediaType(requestContentType) == "multipart/form-data") {
        const std::string multipartBoundary = selectMultipartBoundary(formParameters);
        headers["Content-Type"] = requestContentType + "; boundary=" + multipartBoundary;
...
std::string OauthApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void OauthApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void OauthApi::handleStdException(const std::exception&) {
    throw;
}

void OauthApi::handleUncaughtException() {
    throw;
}
