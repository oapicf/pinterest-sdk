...
LeadFormsApiException::LeadFormsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status LeadFormsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<LeadFormTest>
LeadFormsApi::leadFormTest_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/lead_forms/{lead_form_id}/test";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(leadFormTestCreate));
    } else {
    ...
    replacePathParameter(path, "lead_form_id", leadFormId);
    ...
    std::shared_ptr<LeadFormTest> deserializedResponse = std::make_shared<LeadFormTest>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LeadFormTest>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw LeadFormsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw LeadFormsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    throw LeadFormsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<LeadForm>
LeadFormsApi::leadForm_get(
    const std::string& leadFormId, const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/lead_forms/{lead_form_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "lead_form_id", leadFormId);
    replacePathParameter(path, "ad_account_id", adAccountId);
    ...
    std::shared_ptr<LeadForm> deserializedResponse = std::make_shared<LeadForm>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LeadForm>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw LeadFormsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw LeadFormsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw LeadFormsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw LeadFormsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw LeadFormsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Lead_forms_create_200_response>
LeadFormsApi::leadForms_create(
    const std::string& adAccountId, const std::vector<std::shared_ptr<LeadFormCreate>>& leadFormCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/lead_forms";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(leadFormCreate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
    ...
    std::shared_ptr<Lead_forms_create_200_response> deserializedResponse = std::make_shared<Lead_forms_create_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Lead_forms_create_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Lead_forms_list_200_response>
LeadFormsApi::leadForms_list(
    ...
    std::shared_ptr<Lead_forms_list_200_response> deserializedResponse = std::make_shared<Lead_forms_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Lead_forms_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
LeadFormsApi::leadForms_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(leadFormBatchUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string LeadFormsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void LeadFormsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void LeadFormsApi::handleStdException(const std::exception&) {
    throw;
}

void LeadFormsApi::handleUncaughtException() {
    throw;
}
