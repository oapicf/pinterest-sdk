...
MediaApiException::MediaApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status MediaApiException::getStatus() const
{
    return m_status;
}
...
MediaApi::media_create(
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(mediaUploadCreate));
    } else {
    ...
    std::shared_ptr<MediaUpload> deserializedResponse = std::make_shared<MediaUpload>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<MediaUpload>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw MediaApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw MediaApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw MediaApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw MediaApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw MediaApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw MediaApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Media_list_200_response>
MediaApi::media_list(
    ...
    std::shared_ptr<Media_list_200_response> deserializedResponse = std::make_shared<Media_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Media_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
MediaApi::media_get(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/media/{media_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "media_id", mediaId);
    ...
    std::shared_ptr<Media> deserializedResponse = std::make_shared<Media>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Media>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string MediaApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void MediaApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void MediaApi::handleStdException(const std::exception&) {
    throw;
}

void MediaApi::handleUncaughtException() {
    throw;
}
