...
ProductTagsApiException::ProductTagsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ProductTagsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<ProductTagsResponse>
ProductTagsApi::productTags_bulkAdd(
    ...
    std::string path = m_context + "/pins/{pin_id}/product_tags";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(productTagsBulkAddRequest));
    } else {
    ...
    std::shared_ptr<ProductTagsResponse> deserializedResponse = std::make_shared<ProductTagsResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ProductTagsResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ProductTagsApiException(statusCode, "The request contains ineligible product tags.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ProductTagsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ProductTagsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ProductTagsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw ProductTagsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<ProductTagsResponse>
ProductTagsApi::productTags_list(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ProductTagsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ProductTagsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    ...
    if (statusCode == boost::beast::http::status(404)) {
        throw ProductTagsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ProductTagsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
...
ProductTagsApi::productTags_bulkDelete(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(productTagsBulkDeleteRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string ProductTagsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ProductTagsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ProductTagsApi::handleStdException(const std::exception&) {
    throw;
}

void ProductTagsApi::handleUncaughtException() {
    throw;
}
