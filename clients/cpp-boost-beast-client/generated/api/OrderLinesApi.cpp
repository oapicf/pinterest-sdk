...
OrderLinesApiException::OrderLinesApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status OrderLinesApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<OrderLine>
OrderLinesApi::orderLines_get(
    const std::string& orderLineId, const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/order_lines/{order_line_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "order_line_id", orderLineId);
    ...
    std::shared_ptr<OrderLine> deserializedResponse = std::make_shared<OrderLine>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<OrderLine>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw OrderLinesApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw OrderLinesApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw OrderLinesApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw OrderLinesApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw OrderLinesApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw OrderLinesApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Order_lines_list_200_response>
OrderLinesApi::orderLines_list(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/order_lines";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    // query params
    ...
    std::shared_ptr<Order_lines_list_200_response> deserializedResponse = std::make_shared<Order_lines_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Order_lines_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string OrderLinesApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void OrderLinesApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void OrderLinesApi::handleStdException(const std::exception&) {
    throw;
}

void OrderLinesApi::handleUncaughtException() {
    throw;
}
