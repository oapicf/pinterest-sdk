...
TargetingTemplateApiException::TargetingTemplateApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status TargetingTemplateApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<TargetingTemplate>
TargetingTemplateApi::targetingTemplate_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/targeting_templates";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(targetingTemplateCreate));
    } else {
    ...
    std::shared_ptr<TargetingTemplate> deserializedResponse = std::make_shared<TargetingTemplate>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<TargetingTemplate>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw TargetingTemplateApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw TargetingTemplateApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw TargetingTemplateApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw TargetingTemplateApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw TargetingTemplateApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw TargetingTemplateApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Targeting_template_list_200_response>
TargetingTemplateApi::targetingTemplate_list(
    ...
    if (includeSizing) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "include_sizing",
            serializeQueryParameterValue(*includeSizing));
    }
    if (searchQuery) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "search_query",
            serializeQueryParameterValue(*searchQuery));
    }
    ...
    std::shared_ptr<Targeting_template_list_200_response> deserializedResponse = std::make_shared<Targeting_template_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Targeting_template_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
void
TargetingTemplateApi::targetingTemplate_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(targetingTemplateUpdateRequestReadOrUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string TargetingTemplateApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void TargetingTemplateApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void TargetingTemplateApi::handleStdException(const std::exception&) {
    throw;
}

void TargetingTemplateApi::handleUncaughtException() {
    throw;
}
