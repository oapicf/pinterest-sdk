...
BulkApiException::BulkApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status BulkApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<BulkDownload>
BulkApi::bulkDownload_create(
    const std::string& adAccountId, const std::shared_ptr<BulkDownloadCreate>& bulkDownloadCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/bulk/download";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(bulkDownloadCreate));
    } else {
    ...
    std::shared_ptr<BulkDownload> deserializedResponse = std::make_shared<BulkDownload>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<BulkDownload>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw BulkApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw BulkApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw BulkApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw BulkApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw BulkApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw BulkApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<BulkJobData>
BulkApi::bulkRequest_get(
    const std::string& adAccountId, const std::string& bulkRequestId, const boost::optional<bool>& includeDetails) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/bulk/{bulk_request_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "bulk_request_id", bulkRequestId);
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (includeDetails) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "include_details",
            serializeQueryParameterValue(*includeDetails));
    }
    ...
    std::shared_ptr<BulkJobData> deserializedResponse = std::make_shared<BulkJobData>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<BulkJobData>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<BulkUpsertResponse>
BulkApi::bulkUpsert_create(
    const std::string& adAccountId, const std::shared_ptr<BulkUpsertRequest>& bulkUpsertRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/bulk/upsert";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(bulkUpsertRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
    ...
    std::shared_ptr<BulkUpsertResponse> deserializedResponse = std::make_shared<BulkUpsertResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<BulkUpsertResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    throw BulkApiException(statusCode, "Unexpected error");
}


std::string BulkApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void BulkApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void BulkApi::handleStdException(const std::exception&) {
    throw;
}

void BulkApi::handleUncaughtException() {
    throw;
}
