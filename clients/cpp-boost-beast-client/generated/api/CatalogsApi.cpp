...
CatalogsApiException::CatalogsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CatalogsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CatalogsAvailableFilterValues>
CatalogsApi::catalogs_availableFilterValues(
    ...
    std::string path = m_context + "/catalogs/available_filter_values";
    ...
    if (country) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "country",
            serializeQueryParameterValue(*country));
    }
    if (language) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "language",
            serializeQueryParameterValue(*language));
    }
    if (adAccountId) {
        appendQueryParameter(
            queryParameterStream,
    ...
    std::shared_ptr<CatalogsAvailableFilterValues> deserializedResponse = std::make_shared<CatalogsAvailableFilterValues>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsAvailableFilterValues>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CatalogsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CatalogsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CatalogsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CatalogsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CatalogsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CatalogsApiException(statusCode, "An unexpected error response.");
}
...
CatalogsApi::catalogs_create(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs";
    std::map<std::string, std::string> headers;
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogCreate));
    } else {
    ...
    std::shared_ptr<Catalog> deserializedResponse = std::make_shared<Catalog>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Catalog>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Catalogs_list_200_response>
CatalogsApi::catalogs_list(
    ...
    std::shared_ptr<Catalogs_list_200_response> deserializedResponse = std::make_shared<Catalogs_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Catalogs_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string CatalogsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CatalogsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CatalogsApi::handleStdException(const std::exception&) {
    throw;
}

void CatalogsApi::handleUncaughtException() {
    throw;
}
