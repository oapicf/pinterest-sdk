...
ConversionsApiException::ConversionsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ConversionsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<Advertiser_defined_events_create_200_response>
ConversionsApi::advertiserDefinedEvents_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/advertiser_defined_events";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(advertiserDefinedEventsCreateRequest));
    } else {
    ...
    std::shared_ptr<Advertiser_defined_events_create_200_response> deserializedResponse = std::make_shared<Advertiser_defined_events_create_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Advertiser_defined_events_create_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ConversionsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ConversionsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ConversionsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw ConversionsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ConversionsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw ConversionsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Advertiser_defined_events_create_200_response>
ConversionsApi::advertiserDefinedEvents_delete(
...
std::shared_ptr<Advertiser_defined_events_get_200_response>
ConversionsApi::advertiserDefinedEvents_get(
    ...
    std::shared_ptr<Advertiser_defined_events_get_200_response> deserializedResponse = std::make_shared<Advertiser_defined_events_get_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Advertiser_defined_events_get_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Advertiser_defined_events_create_200_response>
ConversionsApi::advertiserDefinedEvents_update(
    const std::string& adAccountId, const std::shared_ptr<AdvertiserDefinedEventsCreateRequest>& advertiserDefinedEventsCreateRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/advertiser_defined_events";
    std::map<std::string, std::string> headers;
...
std::string ConversionsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ConversionsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ConversionsApi::handleStdException(const std::exception&) {
    throw;
}

void ConversionsApi::handleUncaughtException() {
    throw;
}
