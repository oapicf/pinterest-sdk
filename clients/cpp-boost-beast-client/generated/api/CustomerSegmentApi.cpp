...
CustomerSegmentApiException::CustomerSegmentApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CustomerSegmentApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CustomerSegment>
CustomerSegmentApi::customerSegment_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_segments";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(customerSegmentCreate));
    } else {
    ...
    std::shared_ptr<CustomerSegment> deserializedResponse = std::make_shared<CustomerSegment>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CustomerSegment>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CustomerSegmentApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CustomerSegmentApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CustomerSegmentApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CustomerSegmentApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CustomerSegmentApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CustomerSegmentApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Customer_segment_list_200_response>
CustomerSegmentApi::customerSegment_list(
    ...
    std::shared_ptr<Customer_segment_list_200_response> deserializedResponse = std::make_shared<Customer_segment_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Customer_segment_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
void
CustomerSegmentApi::customerSegment_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(customerSegmentUpdateRequestUpdateWithRequiredBody));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string CustomerSegmentApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CustomerSegmentApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CustomerSegmentApi::handleStdException(const std::exception&) {
    throw;
}

void CustomerSegmentApi::handleUncaughtException() {
    throw;
}
