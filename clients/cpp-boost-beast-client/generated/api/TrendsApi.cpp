...
TrendsApiException::TrendsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status TrendsApiException::getStatus() const
{
    return m_status;
}
...
std::vector<std::shared_ptr<TrendsEditorial>>
TrendsApi::trendsEditorialArticles_list(
    const std::shared_ptr<ProductCategoryRegion>& region) {
    std::string serializedRequestBody;
    std::string path = m_context + "/trends/editorial_articles";
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "region",
            serializeQueryParameterValue(region));
    ...
    std::vector<std::shared_ptr<TrendsEditorial>> deserializedResponse = std::vector<std::shared_ptr<TrendsEditorial>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<TrendsEditorial>>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw TrendsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw TrendsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw TrendsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw TrendsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw TrendsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw TrendsApiException(statusCode, "An unexpected error response.");
}
std::vector<std::shared_ptr<FeaturedTrend>>
TrendsApi::trendsFeaturedTopics_list(
    const std::shared_ptr<ProductCategoryRegion>& region, const boost::optional<std::shared_ptr<InterestsEnum>>& interest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/trends/topics/featured";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (interest) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "interest",
            serializeQueryParameterValue(*interest));
    }
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
    ...
    std::vector<std::shared_ptr<FeaturedTrend>> deserializedResponse = std::vector<std::shared_ptr<FeaturedTrend>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<FeaturedTrend>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::vector<std::shared_ptr<ProductCategoryDetails>>
TrendsApi::trendsProductCategoriesDetails_list(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/trends/product_categories/details";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    if (lookbackWindow) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "lookback_window",
            serializeQueryParameterValue(*lookbackWindow));
    }
    if (engagementType) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "engagement_type",
            serializeQueryParameterValue(*engagementType));
    }
    path += queryParameterStream.str();
    ...
    std::vector<std::shared_ptr<ProductCategoryDetails>> deserializedResponse = std::vector<std::shared_ptr<ProductCategoryDetails>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<ProductCategoryDetails>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::vector<std::shared_ptr<TrendingProductCategory>>
TrendsApi::trendsProductCategoriesTrending_list(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/trends/product_categories/trending";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    std::vector<std::shared_ptr<TrendingProductCategory>> deserializedResponse = std::vector<std::shared_ptr<TrendingProductCategory>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<TrendingProductCategory>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string TrendsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void TrendsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void TrendsApi::handleStdException(const std::exception&) {
    throw;
}

void TrendsApi::handleUncaughtException() {
    throw;
}
