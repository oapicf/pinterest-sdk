...
SchedulesApiException::SchedulesApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status SchedulesApiException::getStatus() const
{
    return m_status;
}
...
std::vector<std::shared_ptr<Schedules_create_200_response_inner>>
SchedulesApi::schedules_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/schedules";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(scheduleCreate));
    } else {
    ...
    std::vector<std::shared_ptr<Schedules_create_200_response_inner>> deserializedResponse = std::vector<std::shared_ptr<Schedules_create_200_response_inner>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<Schedules_create_200_response_inner>>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(201)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<Schedule>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw SchedulesApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw SchedulesApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw SchedulesApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw SchedulesApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw SchedulesApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw SchedulesApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Schedules_list_200_response>
SchedulesApi::schedules_list(
    ...
    if (scheduleType) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "schedule_type",
            serializeQueryParameterValue(*scheduleType));
    }
        appendMultiQueryParameters(
            queryParameterStream,
            queryParameterSeparator,
    ...
    std::shared_ptr<Schedules_list_200_response> deserializedResponse = std::make_shared<Schedules_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Schedules_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
SchedulesApi::schedules_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(scheduleBatchUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string SchedulesApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void SchedulesApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void SchedulesApi::handleStdException(const std::exception&) {
    throw;
}

void SchedulesApi::handleUncaughtException() {
    throw;
}
