...
CatalogReportsApiException::CatalogReportsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CatalogReportsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CatalogsCreateReportResponse>
CatalogReportsApi::reports_create(
    ...
    std::string path = m_context + "/catalogs/reports";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(catalogsReportParameters));
    } else {
    ...
    std::shared_ptr<CatalogsCreateReportResponse> deserializedResponse = std::make_shared<CatalogsCreateReportResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsCreateReportResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CatalogReportsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CatalogReportsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CatalogReportsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CatalogReportsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CatalogReportsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CatalogReportsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<CatalogsReport>
CatalogReportsApi::reports_get(
    ...
    std::shared_ptr<CatalogsReport> deserializedResponse = std::make_shared<CatalogsReport>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CatalogsReport>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Reports_stats_200_response>
CatalogReportsApi::reports_stats(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/catalogs/reports/stats";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    std::shared_ptr<Reports_stats_200_response> deserializedResponse = std::make_shared<Reports_stats_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Reports_stats_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string CatalogReportsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CatalogReportsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CatalogReportsApi::handleStdException(const std::exception&) {
    throw;
}

void CatalogReportsApi::handleUncaughtException() {
    throw;
}
