...
LeadsExportApiException::LeadsExportApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status LeadsExportApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<LeadsExports>
LeadsExportApi::leadsExport_create(
    const std::string& adAccountId, const std::shared_ptr<LeadsExportsCreate>& leadsExportsCreate) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/leads_export";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(leadsExportsCreate));
    } else {
    ...
    std::shared_ptr<LeadsExports> deserializedResponse = std::make_shared<LeadsExports>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LeadsExports>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw LeadsExportApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw LeadsExportApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw LeadsExportApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw LeadsExportApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw LeadsExportApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw LeadsExportApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<LeadsExportResponseData>
LeadsExportApi::leadsExport_get(
    const std::string& adAccountId, const std::string& leadsExportId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/leads_export/{leads_export_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "leads_export_id", leadsExportId);
    ...
    std::shared_ptr<LeadsExportResponseData> deserializedResponse = std::make_shared<LeadsExportResponseData>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LeadsExportResponseData>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string LeadsExportApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void LeadsExportApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void LeadsExportApi::handleStdException(const std::exception&) {
    throw;
}

void LeadsExportApi::handleUncaughtException() {
    throw;
}
