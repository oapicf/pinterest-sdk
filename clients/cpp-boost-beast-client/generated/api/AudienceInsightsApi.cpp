...
AudienceInsightsApiException::AudienceInsightsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status AudienceInsightsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<Audience_insights_scope_and_type_get_200_response>
AudienceInsightsApi::audienceInsightsScopeAndType_get(
    const std::string& adAccountId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/insights/audiences";
    ...
    std::shared_ptr<Audience_insights_scope_and_type_get_200_response> deserializedResponse = std::make_shared<Audience_insights_scope_and_type_get_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Audience_insights_scope_and_type_get_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw AudienceInsightsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw AudienceInsightsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw AudienceInsightsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw AudienceInsightsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw AudienceInsightsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw AudienceInsightsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<AudienceInsights>
AudienceInsightsApi::audienceInsights_get(
    const std::string& adAccountId, const std::shared_ptr<AudienceInsightType>& audienceInsightType) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/audience_insights";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    // query params
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            serializeQueryParameterValue(audienceInsightType));
    ...
    std::shared_ptr<AudienceInsights> deserializedResponse = std::make_shared<AudienceInsights>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<AudienceInsights>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string AudienceInsightsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void AudienceInsightsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void AudienceInsightsApi::handleStdException(const std::exception&) {
    throw;
}

void AudienceInsightsApi::handleUncaughtException() {
    throw;
}
