...
SearchApiException::SearchApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status SearchApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<Search_partner_pins_200_response>
SearchApi::searchPartnerPins(
    ...
    std::string path = m_context + "/search/partner/pins";
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "term",
            serializeQueryParameterValue(term));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "country_code",
    ...
    if (locale) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "locale",
            serializeQueryParameterValue(*locale));
    }
    if (limit) {
        appendQueryParameter(
            queryParameterStream,
    ...
    std::shared_ptr<Search_partner_pins_200_response> deserializedResponse = std::make_shared<Search_partner_pins_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Search_partner_pins_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw SearchApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw SearchApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw SearchApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw SearchApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw SearchApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw SearchApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Boards_list_200_response>
SearchApi::searchUserBoards_get(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/search/boards";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    if (query) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "query",
            serializeQueryParameterValue(*query));
    }
    if (bookmark) {
        appendQueryParameter(
            queryParameterStream,
...
std::string SearchApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void SearchApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void SearchApi::handleStdException(const std::exception&) {
    throw;
}

void SearchApi::handleUncaughtException() {
    throw;
}
