...
CustomerListUploadsApiException::CustomerListUploadsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CustomerListUploadsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<CustomerListUploadCreateResponse>
CustomerListUploadsApi::customerListUploads_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}/uploads";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(customerListUploadCreateRequest));
    } else {
    ...
    std::shared_ptr<CustomerListUploadCreateResponse> deserializedResponse = std::make_shared<CustomerListUploadCreateResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CustomerListUploadCreateResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw CustomerListUploadsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw CustomerListUploadsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw CustomerListUploadsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw CustomerListUploadsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw CustomerListUploadsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw CustomerListUploadsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<CustomerListUpload>
CustomerListUploadsApi::customerListUploads_get(
    const std::string& adAccountId, const std::string& customerListId, const std::string& customerListUploadId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}/uploads/{customer_list_upload_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "customer_list_id", customerListId);
    replacePathParameter(path, "customer_list_upload_id", customerListUploadId);
    ...
    std::shared_ptr<CustomerListUpload> deserializedResponse = std::make_shared<CustomerListUpload>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CustomerListUpload>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<CustomerListUpload>
CustomerListUploadsApi::customerListUploads_run(
    const std::string& adAccountId, const std::string& customerListId, const std::string& customerListUploadId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}/uploads/{customer_list_upload_id}/run";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "customer_list_id", customerListId);
...
std::string CustomerListUploadsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CustomerListUploadsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CustomerListUploadsApi::handleStdException(const std::exception&) {
    throw;
}

void CustomerListUploadsApi::handleUncaughtException() {
    throw;
}
