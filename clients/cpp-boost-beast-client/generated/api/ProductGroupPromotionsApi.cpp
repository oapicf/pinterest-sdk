...
ProductGroupPromotionsApiException::ProductGroupPromotionsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ProductGroupPromotionsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<ProductGroupPromotions>
ProductGroupPromotionsApi::productGroupPromotions_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/product_group_promotions";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(productGroupPromotionsCreate));
    } else {
    ...
    std::shared_ptr<ProductGroupPromotions> deserializedResponse = std::make_shared<ProductGroupPromotions>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ProductGroupPromotions>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ProductGroupPromotionsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ProductGroupPromotionsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ProductGroupPromotionsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw ProductGroupPromotionsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ProductGroupPromotionsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw ProductGroupPromotionsApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Product_group_promotions_list_200_response>
ProductGroupPromotionsApi::productGroupPromotions_list(
    ...
    if (adGroupId) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "ad_group_id",
            serializeQueryParameterValue(*adGroupId));
    }
    ...
    std::shared_ptr<Product_group_promotions_list_200_response> deserializedResponse = std::make_shared<Product_group_promotions_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Product_group_promotions_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ProductGroupPromotions>
ProductGroupPromotionsApi::productGroupPromotions_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(productGroupPromotionsUpdateWithRequiredBody));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::shared_ptr<ProductGroupPromotion>
ProductGroupPromotionsApi::productGroupPromotions_get(
    const std::string& adAccountId, const std::string& productGroupPromotionId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/product_group_promotions/{product_group_promotion_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    replacePathParameter(path, "product_group_promotion_id", productGroupPromotionId);
    ...
    std::shared_ptr<ProductGroupPromotion> deserializedResponse = std::make_shared<ProductGroupPromotion>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ProductGroupPromotion>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::vector<std::shared_ptr<ProductGroupAnalyticsItems>>
ProductGroupPromotionsApi::productGroups_analytics(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/product_groups/analytics";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "ad_account_id", adAccountId);
    // query params
    ...
    std::vector<std::shared_ptr<ProductGroupAnalyticsItems>> deserializedResponse = std::vector<std::shared_ptr<ProductGroupAnalyticsItems>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<ProductGroupAnalyticsItems>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string ProductGroupPromotionsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ProductGroupPromotionsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ProductGroupPromotionsApi::handleStdException(const std::exception&) {
    throw;
}

void ProductGroupPromotionsApi::handleUncaughtException() {
    throw;
}
