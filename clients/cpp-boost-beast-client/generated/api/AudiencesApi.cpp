...
AudiencesApiException::AudiencesApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status AudiencesApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<AdAccountsAudience>
AudiencesApi::audiences_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/audiences";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(adAccountsAudienceCreate));
    } else {
    ...
    std::shared_ptr<AdAccountsAudience> deserializedResponse = std::make_shared<AdAccountsAudience>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<AdAccountsAudience>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw AudiencesApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw AudiencesApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw AudiencesApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw AudiencesApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw AudiencesApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    throw AudiencesApiException(statusCode, "An unexpected error response.");
}
std::shared_ptr<Audiences_list_200_response>
AudiencesApi::audiences_list(
    ...
    if (ownershipType) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "ownership_type",
            serializeQueryParameterValue(*ownershipType));
    }
    if (excludeNca) {
        appendQueryParameter(
            queryParameterStream,
    ...
    std::shared_ptr<Audiences_list_200_response> deserializedResponse = std::make_shared<Audiences_list_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Audiences_list_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
AudiencesApi::audiences_get(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/ad_accounts/{ad_account_id}/audiences/{audience_id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "audience_id", audienceId);
    replacePathParameter(path, "ad_account_id", adAccountId);
...
AudiencesApi::audiences_update(
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(adAccountsAudienceUpdate));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
...
std::string AudiencesApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void AudiencesApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void AudiencesApi::handleStdException(const std::exception&) {
    throw;
}

void AudiencesApi::handleUncaughtException() {
    throw;
}
