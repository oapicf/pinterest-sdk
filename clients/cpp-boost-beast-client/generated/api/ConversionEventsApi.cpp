...
ConversionEventsApiException::ConversionEventsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ConversionEventsApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<ConversionEvents>
ConversionEventsApi::events_create(
    ...
    std::string path = m_context + "/ad_accounts/{ad_account_id}/events";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(conversionEventsCreate));
    } else {
    ...
    if (test) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "test",
            serializeQueryParameterValue(*test));
    }
    ...
    std::shared_ptr<ConversionEvents> deserializedResponse = std::make_shared<ConversionEvents>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ConversionEvents>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ConversionEventsApiException(statusCode, "The request could not be understood by the server due to unexpected data.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ConversionEventsApiException(statusCode, "Authentication is required and has either failed or not been provided.");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw ConversionEventsApiException(statusCode, "The request was valid, but the server is refusing action. The user might not have the necessary permissions for a resource.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw ConversionEventsApiException(statusCode, "The requested resource could not be found on this server.");
    }
    if (statusCode == boost::beast::http::status(422)) {
        throw ConversionEventsApiException(statusCode, "The request was well-formed but was unable to be followed due to semantic errors.");
    }
    if (statusCode == boost::beast::http::status(429)) {
        throw ConversionEventsApiException(statusCode, "The user has sent too many requests in a given amount of time and is being rate limited.");
    }
    if (statusCode == boost::beast::http::status(503)) {
        throw ConversionEventsApiException(statusCode, "The server is currently unable to handle the request due to a temporary overload or scheduled maintenance.");
    }
    throw ConversionEventsApiException(statusCode, "An unexpected error response.");
}


std::string ConversionEventsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ConversionEventsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ConversionEventsApi::handleStdException(const std::exception&) {
    throw;
}

void ConversionEventsApi::handleUncaughtException() {
    throw;
}
