...
AiDisclosuresUpdate::AiDisclosuresUpdate(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string AiDisclosuresUpdate::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value AiDisclosuresUpdate::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void AiDisclosuresUpdate::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for AiDisclosuresUpdate");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object AiDisclosuresUpdate::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ValuesIsSet) {
            object["values"] = JsonValueConverter<std::vector<std::shared_ptr<AiDisclosureItem>>>::toJsonValue(getValues());
        }
    return object;
}

void AiDisclosuresUpdate::fromJsonObject_internal(boost::json::object const& object)
{
    m_ValuesIsSet = false;
    {
        const auto ValuesIt = object.find("values");
        if (ValuesIt != object.end()) {
            setValues(JsonValueConverter<std::vector<std::shared_ptr<AiDisclosureItem>>>::fromJsonValue(ValuesIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<AiDisclosuresUpdate>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<AiDisclosuresUpdate>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<AiDisclosuresUpdate>>>::fromJsonValue(boost::json::parse(json));
}
