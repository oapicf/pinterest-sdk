...
ConversionEventsCreate::ConversionEventsCreate(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string ConversionEventsCreate::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value ConversionEventsCreate::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ConversionEventsCreate::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ConversionEventsCreate");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ConversionEventsCreate::toJsonObject_internal() const
{
    boost::json::object object;
        object["data"] = JsonValueConverter<std::vector<std::shared_ptr<ConversionEventsDataItems>>>::toJsonValue(getData());
    return object;
}

void ConversionEventsCreate::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto DataIt = object.find("data");
        if (DataIt != object.end()) {
            setData(JsonValueConverter<std::vector<std::shared_ptr<ConversionEventsDataItems>>>::fromJsonValue(DataIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ConversionEventsCreate>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ConversionEventsCreate>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ConversionEventsCreate>>>::fromJsonValue(boost::json::parse(json));
}
