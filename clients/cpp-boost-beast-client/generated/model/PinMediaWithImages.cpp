...
PinMediaWithImages::PinMediaWithImages(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string PinMediaWithImages::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value PinMediaWithImages::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PinMediaWithImages::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PinMediaWithImages");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PinMediaWithImages::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ItemsIsSet) {
            object["items"] = JsonValueConverter<std::vector<std::shared_ptr<ImageMetadata>>>::toJsonValue(getItems());
        }
        object["media_type"] = JsonValueConverter<std::string>::toJsonValue(getMediaType());
    return object;
}

void PinMediaWithImages::fromJsonObject_internal(boost::json::object const& object)
{
    m_ItemsIsSet = false;
    {
        const auto ItemsIt = object.find("items");
        if (ItemsIt != object.end()) {
            setItems(JsonValueConverter<std::vector<std::shared_ptr<ImageMetadata>>>::fromJsonValue(ItemsIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PinMediaWithImages>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PinMediaWithImages>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PinMediaWithImages>>>::fromJsonValue(boost::json::parse(json));
}
