...
BasePreferredMediaType::BasePreferredMediaType(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string BasePreferredMediaType::toJsonString(bool prettyJson /* = false */) const
{
...
void BasePreferredMediaType::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value BasePreferredMediaType::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void BasePreferredMediaType::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for BasePreferredMediaType");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object BasePreferredMediaType::toJsonObject_internal() const
{
    boost::json::object object;
    return object;
}

void BasePreferredMediaType::fromJsonObject_internal(boost::json::object const& object)
{
}


std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<BasePreferredMediaType>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<BasePreferredMediaType>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<BasePreferredMediaType>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<BasePreferredMediaType>>>::fromJsonValue(boost::json::parse(json));
}
