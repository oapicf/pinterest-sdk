...
KeywordsMetricsArrayResponse::KeywordsMetricsArrayResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string KeywordsMetricsArrayResponse::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value KeywordsMetricsArrayResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void KeywordsMetricsArrayResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for KeywordsMetricsArrayResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object KeywordsMetricsArrayResponse::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_DataIsSet) {
            object["data"] = JsonValueConverter<std::vector<std::shared_ptr<KeywordMetricsResponse>>>::toJsonValue(getData());
        }
    return object;
}

void KeywordsMetricsArrayResponse::fromJsonObject_internal(boost::json::object const& object)
{
    m_DataIsSet = false;
    {
        const auto DataIt = object.find("data");
        if (DataIt != object.end()) {
            setData(JsonValueConverter<std::vector<std::shared_ptr<KeywordMetricsResponse>>>::fromJsonValue(DataIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<KeywordsMetricsArrayResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<KeywordsMetricsArrayResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<KeywordsMetricsArrayResponse>>>::fromJsonValue(boost::json::parse(json));
}
