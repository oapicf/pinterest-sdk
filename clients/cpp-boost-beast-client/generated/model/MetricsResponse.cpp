...
MetricsResponse::MetricsResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string MetricsResponse::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value MetricsResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void MetricsResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for MetricsResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object MetricsResponse::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_DataIsSet) {
            object["data"] = JsonValueConverter<std::vector<std::shared_ptr<MetricsResponseDataItems>>>::toJsonValue(getData());
        }
    return object;
}

void MetricsResponse::fromJsonObject_internal(boost::json::object const& object)
{
    m_DataIsSet = false;
    {
        const auto DataIt = object.find("data");
        if (DataIt != object.end()) {
            setData(JsonValueConverter<std::vector<std::shared_ptr<MetricsResponseDataItems>>>::fromJsonValue(DataIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<MetricsResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<MetricsResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<MetricsResponse>>>::fromJsonValue(boost::json::parse(json));
}
