...
ScheduleAppTypeMultipliers::ScheduleAppTypeMultipliers(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string ScheduleAppTypeMultipliers::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value ScheduleAppTypeMultipliers::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ScheduleAppTypeMultipliers::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ScheduleAppTypeMultipliers");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ScheduleAppTypeMultipliers::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_APP_TYPEIsSet) {
            object["APP_TYPE"] = JsonValueConverter<std::shared_ptr<TargetingSpecAppType>>::toJsonValue(getAPPTYPE());
        }
    return object;
}

void ScheduleAppTypeMultipliers::fromJsonObject_internal(boost::json::object const& object)
{
    m_APP_TYPEIsSet = false;
    {
        const auto APP_TYPEIt = object.find("APP_TYPE");
        if (APP_TYPEIt != object.end()) {
            setAPPTYPE(JsonValueConverter<std::shared_ptr<TargetingSpecAppType>>::fromJsonValue(APP_TYPEIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ScheduleAppTypeMultipliers>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ScheduleAppTypeMultipliers>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ScheduleAppTypeMultipliers>>>::fromJsonValue(boost::json::parse(json));
}
