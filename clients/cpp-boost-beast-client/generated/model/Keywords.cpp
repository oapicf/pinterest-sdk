...
Keywords::Keywords(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string Keywords::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value Keywords::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void Keywords::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for Keywords");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object Keywords::toJsonObject_internal() const
{
    boost::json::object object;
        object["errors"] = JsonValueConverter<std::vector<std::shared_ptr<KeywordError>>>::toJsonValue(getErrors());
        object["keywords"] = JsonValueConverter<std::vector<std::shared_ptr<Keyword>>>::toJsonValue(getKeywords());
    return object;
}

void Keywords::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto ErrorsIt = object.find("errors");
        if (ErrorsIt != object.end()) {
            setErrors(JsonValueConverter<std::vector<std::shared_ptr<KeywordError>>>::fromJsonValue(ErrorsIt->value()));
        }
    }
    {
        const auto KeywordsIt = object.find("keywords");
        if (KeywordsIt != object.end()) {
            setKeywords(JsonValueConverter<std::vector<std::shared_ptr<Keyword>>>::fromJsonValue(KeywordsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Keywords>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Keywords>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Keywords>>>::fromJsonValue(boost::json::parse(json));
}
