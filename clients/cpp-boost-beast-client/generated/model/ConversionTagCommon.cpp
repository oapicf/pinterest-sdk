/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "ConversionTagCommon.h"

#include <cstddef>
#include <map>
#include <memory>
#include <stdexcept>
#include <sstream>
#include <string>
#include <iterator>
#include <utility>
#include <vector>
#include <algorithm>
#include <array>
#include <boost/json.hpp>

namespace org {
namespace openapitools {
namespace client {
namespace model {

namespace {
template<typename EnumValue, std::size_t Size>
void validateEnumValues(
    const EnumValue& value,
    const std::array<EnumValue, Size>& allowedValues);

template<typename Element, typename EnumValue, std::size_t Size>
void validateEnumValues(
    const std::vector<Element>& values,
    const std::array<EnumValue, Size>& allowedValues);

template<typename MappedValue, typename EnumValue, std::size_t Size>
void validateEnumValues(
    const std::map<std::string, MappedValue>& values,
    const std::array<EnumValue, Size>& allowedValues);

template<typename EnumValue, std::size_t Size>
void validateEnumValues(
    const EnumValue& value,
    const std::array<EnumValue, Size>& allowedValues)
{
    if (std::find(allowedValues.begin(), allowedValues.end(), value) == allowedValues.end()) {
        std::ostringstream errorMessage;
        errorMessage << "Value " << value << " not allowed";
        throw std::runtime_error(errorMessage.str());
    }
}

template<typename Element, typename EnumValue, std::size_t Size>
void validateEnumValues(
    const std::vector<Element>& values,
    const std::array<EnumValue, Size>& allowedValues)
{
    for (const auto& value : values) {
        validateEnumValues(value, allowedValues);
    }
}

template<typename MappedValue, typename EnumValue, std::size_t Size>
void validateEnumValues(
    const std::map<std::string, MappedValue>& values,
    const std::array<EnumValue, Size>& allowedValues)
{
    for (const auto& value : values) {
        validateEnumValues(value.second, allowedValues);
    }
}

template <typename Target>
struct JsonValueConverter
{
    static boost::json::value toJsonValue(const Target& sourceValue)
    {
        return boost::json::value_from(sourceValue);
    }

    static Target fromJsonValue(const boost::json::value& jsonValue)
    {
        return boost::json::value_to<Target>(jsonValue);
    }
};

template <>
struct JsonValueConverter<std::nullptr_t>
{
    static boost::json::value toJsonValue(std::nullptr_t)
    {
        return nullptr;
    }

    static std::nullptr_t fromJsonValue(const boost::json::value& jsonValue)
    {
        if (!jsonValue.is_null()) {
            throw std::invalid_argument("Expected a null JSON value");
        }
        return nullptr;
    }
};

template <typename ModelType>
struct JsonValueConverter<std::shared_ptr<ModelType>>
{
    static boost::json::value toJsonValue(const std::shared_ptr<ModelType>& model)
    {
        return model == nullptr ? boost::json::value(nullptr) : model->toJsonValue();
    }

    static std::shared_ptr<ModelType> fromJsonValue(const boost::json::value& jsonValue)
    {
        if (jsonValue.is_null()) {
            return nullptr;
        }
        return std::make_shared<ModelType>(jsonValue);
    }
};

template <typename Element>
struct JsonValueConverter<std::vector<Element>>
{
    static boost::json::value toJsonValue(const std::vector<Element>& sourceValues)
    {
        boost::json::array jsonValues;
        for (const auto& sourceValue : sourceValues) {
            jsonValues.emplace_back(JsonValueConverter<Element>::toJsonValue(sourceValue));
        }
        return jsonValues;
    }

    static std::vector<Element> fromJsonValue(const boost::json::value& jsonValue)
    {
        const auto& jsonValues = jsonValue.as_array();
        std::vector<Element> convertedValues;
        convertedValues.reserve(jsonValues.size());
        for (const auto& jsonElement : jsonValues) {
            convertedValues.emplace_back(JsonValueConverter<Element>::fromJsonValue(jsonElement));
        }
        return convertedValues;
    }
};

template <typename MappedValue>
struct JsonValueConverter<std::map<std::string, MappedValue>>
{
    static boost::json::value toJsonValue(const std::map<std::string, MappedValue>& sourceValues)
    {
        boost::json::object jsonValues;
        for (const auto& sourceEntry : sourceValues) {
            jsonValues[sourceEntry.first] = JsonValueConverter<MappedValue>::toJsonValue(sourceEntry.second);
        }
        return jsonValues;
    }

    static std::map<std::string, MappedValue> fromJsonValue(const boost::json::value& jsonValue)
    {
        std::map<std::string, MappedValue> convertedValues;
        for (const auto& jsonEntry : jsonValue.as_object()) {
            const std::string entryKey(jsonEntry.key().data(), jsonEntry.key().size());
            convertedValues.emplace(entryKey, JsonValueConverter<MappedValue>::fromJsonValue(jsonEntry.value()));
        }
        return convertedValues;
    }
};

void writePrettyJson(std::ostream& output, boost::json::value const& value, std::string& indent)
{
    if (value.is_object()) {
        const auto& object = value.as_object();
        if (object.empty()) {
            output << "{}";
            return;
        }

        output << "{\n";
        indent.append(4, ' ');
        for (auto iterator = object.begin(); iterator != object.end(); ++iterator) {
            output << indent << boost::json::serialize(iterator->key()) << ": ";
            writePrettyJson(output, iterator->value(), indent);
            if (std::next(iterator) != object.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << '}';
        return;
    }

    if (value.is_array()) {
        const auto& array = value.as_array();
        if (array.empty()) {
            output << "[]";
            return;
        }

        output << "[\n";
        indent.append(4, ' ');
        for (auto iterator = array.begin(); iterator != array.end(); ++iterator) {
            output << indent;
            writePrettyJson(output, *iterator, indent);
            if (std::next(iterator) != array.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << ']';
        return;
    }

    output << boost::json::serialize(value);
}
}

ConversionTagCommon::ConversionTagCommon(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string ConversionTagCommon::toJsonString(bool prettyJson /* = false */) const
{
    const auto value = toJsonValue();
    if (!prettyJson) {
        return boost::json::serialize(value);
    }

    std::ostringstream output;
    std::string indent;
    writePrettyJson(output, value, indent);
    return output.str();
}

void ConversionTagCommon::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value ConversionTagCommon::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ConversionTagCommon::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ConversionTagCommon");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ConversionTagCommon::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Code_snippetIsSet) {
            object["code_snippet"] = JsonValueConverter<std::string>::toJsonValue(getCodeSnippet());
        }
        if (m_ConfigsIsSet) {
            object["configs"] = JsonValueConverter<std::shared_ptr<ConversionTagConfigs>>::toJsonValue(getConfigs());
        }
        if (m_Enhanced_match_statusIsSet) {
            object["enhanced_match_status"] = JsonValueConverter<std::shared_ptr<EnhancedMatchStatusType>>::toJsonValue(getEnhancedMatchStatus());
        }
        if (m_IdIsSet) {
            object["id"] = JsonValueConverter<std::string>::toJsonValue(getId());
        }
        if (m_Last_fired_time_msIsSet) {
            object["last_fired_time_ms"] = JsonValueConverter<double>::toJsonValue(getLastFiredTimeMs());
        }
        object["name"] = JsonValueConverter<std::string>::toJsonValue(getName());
        if (m_VersionIsSet) {
            object["version"] = JsonValueConverter<std::string>::toJsonValue(getVersion());
        }
    return object;
}

void ConversionTagCommon::fromJsonObject_internal(boost::json::object const& object)
{
    m_Code_snippetIsSet = false;
    m_ConfigsIsSet = false;
    m_Enhanced_match_statusIsSet = false;
    m_IdIsSet = false;
    m_Last_fired_time_msIsSet = false;
    m_VersionIsSet = false;
    {
        const auto Code_snippetIt = object.find("code_snippet");
        if (Code_snippetIt != object.end()) {
            setCodeSnippet(JsonValueConverter<std::string>::fromJsonValue(Code_snippetIt->value()));
        }
    }
    {
        const auto ConfigsIt = object.find("configs");
        if (ConfigsIt != object.end()) {
            setConfigs(JsonValueConverter<std::shared_ptr<ConversionTagConfigs>>::fromJsonValue(ConfigsIt->value()));
        }
    }
    {
        const auto Enhanced_match_statusIt = object.find("enhanced_match_status");
        if (Enhanced_match_statusIt != object.end()) {
            setEnhancedMatchStatus(JsonValueConverter<std::shared_ptr<EnhancedMatchStatusType>>::fromJsonValue(Enhanced_match_statusIt->value()));
        }
    }
    {
        const auto IdIt = object.find("id");
        if (IdIt != object.end()) {
            setId(JsonValueConverter<std::string>::fromJsonValue(IdIt->value()));
        }
    }
    {
        const auto Last_fired_time_msIt = object.find("last_fired_time_ms");
        if (Last_fired_time_msIt != object.end()) {
            setLastFiredTimeMs(JsonValueConverter<double>::fromJsonValue(Last_fired_time_msIt->value()));
        }
    }
    {
        const auto NameIt = object.find("name");
        if (NameIt != object.end()) {
            setName(JsonValueConverter<std::string>::fromJsonValue(NameIt->value()));
        }
    }
    {
        const auto VersionIt = object.find("version");
        if (VersionIt != object.end()) {
            setVersion(JsonValueConverter<std::string>::fromJsonValue(VersionIt->value()));
        }
    }
}

std::string ConversionTagCommon::getCodeSnippet() const
{
    return m_Code_snippet;
}

void ConversionTagCommon::setCodeSnippet(std::string value)
{
    
    m_Code_snippet = std::move(value);
    m_Code_snippetIsSet = true;
}
std::shared_ptr<ConversionTagConfigs> ConversionTagCommon::getConfigs() const
{
    return m_Configs;
}

void ConversionTagCommon::setConfigs(std::shared_ptr<ConversionTagConfigs> value)
{
    
    m_Configs = std::move(value);
    m_ConfigsIsSet = true;
}
std::shared_ptr<EnhancedMatchStatusType> ConversionTagCommon::getEnhancedMatchStatus() const
{
    return m_Enhanced_match_status;
}

void ConversionTagCommon::setEnhancedMatchStatus(std::shared_ptr<EnhancedMatchStatusType> value)
{
    
    m_Enhanced_match_status = std::move(value);
    m_Enhanced_match_statusIsSet = true;
}
std::string ConversionTagCommon::getId() const
{
    return m_Id;
}

void ConversionTagCommon::setId(std::string value)
{
    
    m_Id = std::move(value);
    m_IdIsSet = true;
}
double ConversionTagCommon::getLastFiredTimeMs() const
{
    return m_Last_fired_time_ms;
}

void ConversionTagCommon::setLastFiredTimeMs(double value)
{
    
    m_Last_fired_time_ms = std::move(value);
    m_Last_fired_time_msIsSet = true;
}
std::string ConversionTagCommon::getName() const
{
    return m_Name;
}

void ConversionTagCommon::setName(std::string value)
{
    
    m_Name = std::move(value);
}
std::string ConversionTagCommon::getVersion() const
{
    return m_Version;
}

void ConversionTagCommon::setVersion(std::string value)
{
    
    m_Version = std::move(value);
    m_VersionIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<ConversionTagCommon>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ConversionTagCommon>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ConversionTagCommon>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ConversionTagCommon>>>::fromJsonValue(boost::json::parse(json));
}

}
}
}
}

