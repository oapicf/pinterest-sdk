...
TrendingKeywordsResponse::TrendingKeywordsResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value TrendingKeywordsResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void TrendingKeywordsResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for TrendingKeywordsResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object TrendingKeywordsResponse::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_TrendsIsSet) {
            object["trends"] = JsonValueConverter<std::vector<std::shared_ptr<TrendingKeyword>>>::toJsonValue(getTrends());
        }
    return object;
}

void TrendingKeywordsResponse::fromJsonObject_internal(boost::json::object const& object)
{
    m_TrendsIsSet = false;
    {
        const auto TrendsIt = object.find("trends");
        if (TrendsIt != object.end()) {
            setTrends(JsonValueConverter<std::vector<std::shared_ptr<TrendingKeyword>>>::fromJsonValue(TrendsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<TrendingKeywordsResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<TrendingKeywordsResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<TrendingKeywordsResponse>>>::fromJsonValue(boost::json::parse(json));
}
