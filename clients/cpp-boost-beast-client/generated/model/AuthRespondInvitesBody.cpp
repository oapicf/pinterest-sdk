...
AuthRespondInvitesBody::AuthRespondInvitesBody(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string AuthRespondInvitesBody::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value AuthRespondInvitesBody::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void AuthRespondInvitesBody::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for AuthRespondInvitesBody");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object AuthRespondInvitesBody::toJsonObject_internal() const
{
    boost::json::object object;
        object["invites"] = JsonValueConverter<std::vector<std::shared_ptr<AuthRespondInvitesBodyItem>>>::toJsonValue(getInvites());
    return object;
}

void AuthRespondInvitesBody::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto InvitesIt = object.find("invites");
        if (InvitesIt != object.end()) {
            setInvites(JsonValueConverter<std::vector<std::shared_ptr<AuthRespondInvitesBodyItem>>>::fromJsonValue(InvitesIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<AuthRespondInvitesBody>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<AuthRespondInvitesBody>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<AuthRespondInvitesBody>>>::fromJsonValue(boost::json::parse(json));
}
