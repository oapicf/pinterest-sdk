...
BulkReportingJobStatus::BulkReportingJobStatus(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string BulkReportingJobStatus::toJsonString(bool prettyJson /* = false */) const
{
...
void BulkReportingJobStatus::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value BulkReportingJobStatus::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void BulkReportingJobStatus::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for BulkReportingJobStatus");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object BulkReportingJobStatus::toJsonObject_internal() const
{
    boost::json::object object;
    return object;
}

void BulkReportingJobStatus::fromJsonObject_internal(boost::json::object const& object)
{
}


std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<BulkReportingJobStatus>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<BulkReportingJobStatus>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<BulkReportingJobStatus>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<BulkReportingJobStatus>>>::fromJsonValue(boost::json::parse(json));
}
