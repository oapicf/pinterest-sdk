...
BoardMedia::BoardMedia(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value BoardMedia::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void BoardMedia::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for BoardMedia");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object BoardMedia::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Image_cover_urlIsSet) {
            object["image_cover_url"] = JsonValueConverter<std::string>::toJsonValue(getImageCoverUrl());
        }
        if (m_Pin_thumbnail_urlsIsSet) {
            object["pin_thumbnail_urls"] = JsonValueConverter<std::vector<std::string>>::toJsonValue(getPinThumbnailUrls());
        }
    return object;
}

void BoardMedia::fromJsonObject_internal(boost::json::object const& object)
{
    m_Image_cover_urlIsSet = false;
    m_Pin_thumbnail_urlsIsSet = false;
    {
        const auto Image_cover_urlIt = object.find("image_cover_url");
        if (Image_cover_urlIt != object.end()) {
            setImageCoverUrl(JsonValueConverter<std::string>::fromJsonValue(Image_cover_urlIt->value()));
        }
    }
    {
        const auto Pin_thumbnail_urlsIt = object.find("pin_thumbnail_urls");
        if (Pin_thumbnail_urlsIt != object.end()) {
            setPinThumbnailUrls(JsonValueConverter<std::vector<std::string>>::fromJsonValue(Pin_thumbnail_urlsIt->value()));
        }
    }
}
...
void BoardMedia::setImageCoverUrl(std::string value)
{
    ...
    m_Image_cover_url = std::move(value);
...
void BoardMedia::setPinThumbnailUrls(std::vector<std::string> value)
{
    ...
    m_Pin_thumbnail_urls = std::move(value);
    m_Pin_thumbnail_urlsIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<BoardMedia>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<BoardMedia>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<BoardMedia>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<BoardMedia>>>::fromJsonValue(boost::json::parse(json));
}
