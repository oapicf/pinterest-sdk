...
TrendsAgeDistribution::TrendsAgeDistribution(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string TrendsAgeDistribution::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value TrendsAgeDistribution::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void TrendsAgeDistribution::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for TrendsAgeDistribution");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object TrendsAgeDistribution::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Age_distributionIsSet) {
            object["age_distribution"] = JsonValueConverter<std::shared_ptr<TrendsAgeBucket>>::toJsonValue(getAgeDistribution());
        }
    return object;
}

void TrendsAgeDistribution::fromJsonObject_internal(boost::json::object const& object)
{
    m_Age_distributionIsSet = false;
    {
        const auto Age_distributionIt = object.find("age_distribution");
        if (Age_distributionIt != object.end()) {
            setAgeDistribution(JsonValueConverter<std::shared_ptr<TrendsAgeBucket>>::fromJsonValue(Age_distributionIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<TrendsAgeDistribution>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<TrendsAgeDistribution>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<TrendsAgeDistribution>>>::fromJsonValue(boost::json::parse(json));
}
