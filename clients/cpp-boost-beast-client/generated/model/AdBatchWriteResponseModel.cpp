...
AdBatchWriteResponseModel::AdBatchWriteResponseModel(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string AdBatchWriteResponseModel::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value AdBatchWriteResponseModel::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void AdBatchWriteResponseModel::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for AdBatchWriteResponseModel");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object AdBatchWriteResponseModel::toJsonObject_internal() const
{
    boost::json::object object;
        object["items"] = JsonValueConverter<std::vector<std::shared_ptr<AdBatchItem>>>::toJsonValue(getItems());
    return object;
}

void AdBatchWriteResponseModel::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto ItemsIt = object.find("items");
        if (ItemsIt != object.end()) {
            setItems(JsonValueConverter<std::vector<std::shared_ptr<AdBatchItem>>>::fromJsonValue(ItemsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<AdBatchWriteResponseModel>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<AdBatchWriteResponseModel>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<AdBatchWriteResponseModel>>>::fromJsonValue(boost::json::parse(json));
}
