...
PinMediaWithVideos::PinMediaWithVideos(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string PinMediaWithVideos::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value PinMediaWithVideos::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PinMediaWithVideos::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PinMediaWithVideos");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PinMediaWithVideos::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ItemsIsSet) {
            object["items"] = JsonValueConverter<std::vector<std::shared_ptr<VideoMetadataWithItemType>>>::toJsonValue(getItems());
        }
        object["media_type"] = JsonValueConverter<std::string>::toJsonValue(getMediaType());
    return object;
}

void PinMediaWithVideos::fromJsonObject_internal(boost::json::object const& object)
{
    m_ItemsIsSet = false;
    {
        const auto ItemsIt = object.find("items");
        if (ItemsIt != object.end()) {
            setItems(JsonValueConverter<std::vector<std::shared_ptr<VideoMetadataWithItemType>>>::fromJsonValue(ItemsIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PinMediaWithVideos>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PinMediaWithVideos>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PinMediaWithVideos>>>::fromJsonValue(boost::json::parse(json));
}
