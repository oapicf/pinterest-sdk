...
OrderLineMutationResponse::OrderLineMutationResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value OrderLineMutationResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void OrderLineMutationResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for OrderLineMutationResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object OrderLineMutationResponse::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_DataIsSet) {
            object["data"] = JsonValueConverter<std::shared_ptr<OrderLineMutationResult>>::toJsonValue(getData());
        }
    return object;
}

void OrderLineMutationResponse::fromJsonObject_internal(boost::json::object const& object)
{
    m_DataIsSet = false;
    {
        const auto DataIt = object.find("data");
        if (DataIt != object.end()) {
            setData(JsonValueConverter<std::shared_ptr<OrderLineMutationResult>>::fromJsonValue(DataIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<OrderLineMutationResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<OrderLineMutationResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<OrderLineMutationResponse>>>::fromJsonValue(boost::json::parse(json));
}
