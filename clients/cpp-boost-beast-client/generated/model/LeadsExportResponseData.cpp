...
LeadsExportResponseData::LeadsExportResponseData(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string LeadsExportResponseData::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value LeadsExportResponseData::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LeadsExportResponseData::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LeadsExportResponseData");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LeadsExportResponseData::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Download_urlIsSet) {
            object["download_url"] = JsonValueConverter<std::string>::toJsonValue(getDownloadUrl());
        }
        if (m_Export_statusIsSet) {
            object["export_status"] = JsonValueConverter<std::shared_ptr<LeadsExportStatus>>::toJsonValue(getExportStatus());
        }
    return object;
}

void LeadsExportResponseData::fromJsonObject_internal(boost::json::object const& object)
{
    m_Download_urlIsSet = false;
    m_Export_statusIsSet = false;
    {
        const auto Download_urlIt = object.find("download_url");
        if (Download_urlIt != object.end()) {
            setDownloadUrl(JsonValueConverter<std::string>::fromJsonValue(Download_urlIt->value()));
        }
    }
    {
        const auto Export_statusIt = object.find("export_status");
        if (Export_statusIt != object.end()) {
            setExportStatus(JsonValueConverter<std::shared_ptr<LeadsExportStatus>>::fromJsonValue(Export_statusIt->value()));
        }
    }
}
...
void LeadsExportResponseData::setExportStatus(std::shared_ptr<LeadsExportStatus> value)
{
    ...
    m_Export_status = std::move(value);
    m_Export_statusIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<LeadsExportResponseData>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LeadsExportResponseData>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LeadsExportResponseData>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LeadsExportResponseData>>>::fromJsonValue(boost::json::parse(json));
}
