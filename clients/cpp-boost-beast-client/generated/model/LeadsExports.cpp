...
LeadsExports::LeadsExports(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value LeadsExports::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LeadsExports::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LeadsExports");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LeadsExports::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Leads_export_idIsSet) {
            object["leads_export_id"] = JsonValueConverter<std::string>::toJsonValue(getLeadsExportId());
        }
    return object;
}

void LeadsExports::fromJsonObject_internal(boost::json::object const& object)
{
    m_Leads_export_idIsSet = false;
    {
        const auto Leads_export_idIt = object.find("leads_export_id");
        if (Leads_export_idIt != object.end()) {
            setLeadsExportId(JsonValueConverter<std::string>::fromJsonValue(Leads_export_idIt->value()));
        }
    }
}
...
void LeadsExports::setLeadsExportId(std::string value)
{
    ...
    m_Leads_export_id = std::move(value);
    m_Leads_export_idIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<LeadsExports>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LeadsExports>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LeadsExports>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LeadsExports>>>::fromJsonValue(boost::json::parse(json));
}
