...
AdGroupAudienceSizing::AdGroupAudienceSizing(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string AdGroupAudienceSizing::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value AdGroupAudienceSizing::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void AdGroupAudienceSizing::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for AdGroupAudienceSizing");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object AdGroupAudienceSizing::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Audience_size_lower_boundIsSet) {
            object["audience_size_lower_bound"] = JsonValueConverter<double>::toJsonValue(getAudienceSizeLowerBound());
        }
        if (m_Audience_size_upper_boundIsSet) {
            object["audience_size_upper_bound"] = JsonValueConverter<double>::toJsonValue(getAudienceSizeUpperBound());
        }
    return object;
}

void AdGroupAudienceSizing::fromJsonObject_internal(boost::json::object const& object)
{
    m_Audience_size_lower_boundIsSet = false;
    m_Audience_size_upper_boundIsSet = false;
    {
        const auto Audience_size_lower_boundIt = object.find("audience_size_lower_bound");
        if (Audience_size_lower_boundIt != object.end()) {
            setAudienceSizeLowerBound(JsonValueConverter<double>::fromJsonValue(Audience_size_lower_boundIt->value()));
        }
    }
    {
        const auto Audience_size_upper_boundIt = object.find("audience_size_upper_bound");
        if (Audience_size_upper_boundIt != object.end()) {
            setAudienceSizeUpperBound(JsonValueConverter<double>::fromJsonValue(Audience_size_upper_boundIt->value()));
        }
    }
}
...
void AdGroupAudienceSizing::setAudienceSizeLowerBound(double value)
{
    ...
    m_Audience_size_lower_bound = std::move(value);
...
void AdGroupAudienceSizing::setAudienceSizeUpperBound(double value)
{
    ...
    m_Audience_size_upper_bound = std::move(value);
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<AdGroupAudienceSizing>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<AdGroupAudienceSizing>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<AdGroupAudienceSizing>>>::fromJsonValue(boost::json::parse(json));
}
