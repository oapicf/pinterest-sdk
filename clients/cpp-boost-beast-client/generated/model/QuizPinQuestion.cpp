...
QuizPinQuestion::QuizPinQuestion(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value QuizPinQuestion::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void QuizPinQuestion::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for QuizPinQuestion");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object QuizPinQuestion::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_OptionsIsSet) {
            object["options"] = JsonValueConverter<std::vector<std::shared_ptr<QuizPinOption>>>::toJsonValue(getOptions());
        }
        if (m_Question_idIsSet) {
            object["question_id"] = JsonValueConverter<double>::toJsonValue(getQuestionId());
        }
        if (m_Question_textIsSet) {
            object["question_text"] = JsonValueConverter<std::string>::toJsonValue(getQuestionText());
        }
    return object;
}

void QuizPinQuestion::fromJsonObject_internal(boost::json::object const& object)
{
    m_OptionsIsSet = false;
    m_Question_idIsSet = false;
    m_Question_textIsSet = false;
    {
        const auto OptionsIt = object.find("options");
        if (OptionsIt != object.end()) {
            setOptions(JsonValueConverter<std::vector<std::shared_ptr<QuizPinOption>>>::fromJsonValue(OptionsIt->value()));
        }
    }
    {
        const auto Question_idIt = object.find("question_id");
        if (Question_idIt != object.end()) {
            setQuestionId(JsonValueConverter<double>::fromJsonValue(Question_idIt->value()));
        }
    }
    {
        const auto Question_textIt = object.find("question_text");
        if (Question_textIt != object.end()) {
            setQuestionText(JsonValueConverter<std::string>::fromJsonValue(Question_textIt->value()));
        }
    }
}
...
void QuizPinQuestion::setQuestionText(std::string value)
{
    ...
    m_Question_text = std::move(value);
    m_Question_textIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<QuizPinQuestion>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<QuizPinQuestion>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<QuizPinQuestion>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<QuizPinQuestion>>>::fromJsonValue(boost::json::parse(json));
}
