...
LabelUpdateRequest::LabelUpdateRequest(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value LabelUpdateRequest::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LabelUpdateRequest::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LabelUpdateRequest");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LabelUpdateRequest::toJsonObject_internal() const
{
    boost::json::object object;
        object["labels"] = JsonValueConverter<std::vector<std::shared_ptr<LabelUpdateItem>>>::toJsonValue(getLabels());
    return object;
}

void LabelUpdateRequest::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto LabelsIt = object.find("labels");
        if (LabelsIt != object.end()) {
            setLabels(JsonValueConverter<std::vector<std::shared_ptr<LabelUpdateItem>>>::fromJsonValue(LabelsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LabelUpdateRequest>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LabelUpdateRequest>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LabelUpdateRequest>>>::fromJsonValue(boost::json::parse(json));
}
