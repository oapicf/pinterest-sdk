...
S3FilePart::S3FilePart(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value S3FilePart::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void S3FilePart::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for S3FilePart");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object S3FilePart::toJsonObject_internal() const
{
    boost::json::object object;
        object["part_number"] = JsonValueConverter<int32_t>::toJsonValue(getPartNumber());
        object["presigned_url"] = JsonValueConverter<std::string>::toJsonValue(getPresignedUrl());
    return object;
}

void S3FilePart::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Part_numberIt = object.find("part_number");
        if (Part_numberIt != object.end()) {
            setPartNumber(JsonValueConverter<int32_t>::fromJsonValue(Part_numberIt->value()));
        }
    }
    {
        const auto Presigned_urlIt = object.find("presigned_url");
        if (Presigned_urlIt != object.end()) {
            setPresignedUrl(JsonValueConverter<std::string>::fromJsonValue(Presigned_urlIt->value()));
        }
    }
}
...
void S3FilePart::setPresignedUrl(std::string value)
{
    ...
    m_Presigned_url = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<S3FilePart>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<S3FilePart>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<S3FilePart>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<S3FilePart>>>::fromJsonValue(boost::json::parse(json));
}
