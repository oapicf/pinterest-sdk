...
PartnerMetadata::PartnerMetadata(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value PartnerMetadata::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PartnerMetadata::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PartnerMetadata");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PartnerMetadata::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Subscriber_keyIsSet) {
            object["subscriber_key"] = JsonValueConverter<std::string>::toJsonValue(getSubscriberKey());
        }
    return object;
}

void PartnerMetadata::fromJsonObject_internal(boost::json::object const& object)
{
    m_Subscriber_keyIsSet = false;
    {
        const auto Subscriber_keyIt = object.find("subscriber_key");
        if (Subscriber_keyIt != object.end()) {
            setSubscriberKey(JsonValueConverter<std::string>::fromJsonValue(Subscriber_keyIt->value()));
        }
    }
}
...
void PartnerMetadata::setSubscriberKey(std::string value)
{
    ...
    m_Subscriber_key = std::move(value);
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PartnerMetadata>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PartnerMetadata>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PartnerMetadata>>>::fromJsonValue(boost::json::parse(json));
}
