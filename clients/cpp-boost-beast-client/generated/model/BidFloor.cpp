...
BidFloor::BidFloor(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string BidFloor::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value BidFloor::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void BidFloor::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for BidFloor");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object BidFloor::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Bid_floorsIsSet) {
            object["bid_floors"] = JsonValueConverter<std::vector<int32_t>>::toJsonValue(getBidFloors());
        }
        if (m_TypeIsSet) {
...
void BidFloor::fromJsonObject_internal(boost::json::object const& object)
{
    m_Bid_floorsIsSet = false;
    m_TypeIsSet = false;
    {
        const auto Bid_floorsIt = object.find("bid_floors");
        if (Bid_floorsIt != object.end()) {
            setBidFloors(JsonValueConverter<std::vector<int32_t>>::fromJsonValue(Bid_floorsIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<BidFloor>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<BidFloor>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<BidFloor>>>::fromJsonValue(boost::json::parse(json));
}
