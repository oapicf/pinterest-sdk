...
ScheduleAgeBucketMultipliers::ScheduleAgeBucketMultipliers(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value ScheduleAgeBucketMultipliers::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ScheduleAgeBucketMultipliers::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ScheduleAgeBucketMultipliers");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ScheduleAgeBucketMultipliers::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_AGE_BUCKETIsSet) {
            object["AGE_BUCKET"] = JsonValueConverter<std::string>::toJsonValue(getAGEBUCKET());
        }
    return object;
}

void ScheduleAgeBucketMultipliers::fromJsonObject_internal(boost::json::object const& object)
{
    m_AGE_BUCKETIsSet = false;
    {
        const auto AGE_BUCKETIt = object.find("AGE_BUCKET");
        if (AGE_BUCKETIt != object.end()) {
            setAGEBUCKET(JsonValueConverter<std::string>::fromJsonValue(AGE_BUCKETIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ScheduleAgeBucketMultipliers>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ScheduleAgeBucketMultipliers>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ScheduleAgeBucketMultipliers>>>::fromJsonValue(boost::json::parse(json));
}
