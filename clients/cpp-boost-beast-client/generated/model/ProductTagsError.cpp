...
ProductTagsError::ProductTagsError(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value ProductTagsError::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ProductTagsError::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ProductTagsError");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ProductTagsError::toJsonObject_internal() const
{
    boost::json::object object;
        object["code"] = JsonValueConverter<int32_t>::toJsonValue(getCode());
        if (m_DetailsIsSet) {
            object["details"] = JsonValueConverter<std::shared_ptr<IneligibleProductTagsErrorDetails>>::toJsonValue(getDetails());
        }
        object["message"] = JsonValueConverter<std::string>::toJsonValue(getMessage());
    return object;
}

void ProductTagsError::fromJsonObject_internal(boost::json::object const& object)
{
    ...
    {
        const auto DetailsIt = object.find("details");
        if (DetailsIt != object.end()) {
            setDetails(JsonValueConverter<std::shared_ptr<IneligibleProductTagsErrorDetails>>::fromJsonValue(DetailsIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ProductTagsError>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ProductTagsError>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ProductTagsError>>>::fromJsonValue(boost::json::parse(json));
}
