...
UpdateMemberAssetResultItem::UpdateMemberAssetResultItem(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string UpdateMemberAssetResultItem::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value UpdateMemberAssetResultItem::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void UpdateMemberAssetResultItem::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for UpdateMemberAssetResultItem");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object UpdateMemberAssetResultItem::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ResponseIsSet) {
            object["response"] = JsonValueConverter<std::shared_ptr<UsersForIndividualAssetResponse>>::toJsonValue(getResponse());
        }
    return object;
}

void UpdateMemberAssetResultItem::fromJsonObject_internal(boost::json::object const& object)
{
    m_ResponseIsSet = false;
    {
        const auto ResponseIt = object.find("response");
        if (ResponseIt != object.end()) {
            setResponse(JsonValueConverter<std::shared_ptr<UsersForIndividualAssetResponse>>::fromJsonValue(ResponseIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<UpdateMemberAssetResultItem>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<UpdateMemberAssetResultItem>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<UpdateMemberAssetResultItem>>>::fromJsonValue(boost::json::parse(json));
}
