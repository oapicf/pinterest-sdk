...
LocalInventoryItemsGet::LocalInventoryItemsGet(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string LocalInventoryItemsGet::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value LocalInventoryItemsGet::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LocalInventoryItemsGet::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LocalInventoryItemsGet");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LocalInventoryItemsGet::toJsonObject_internal() const
{
    boost::json::object object;
        object["items"] = JsonValueConverter<std::vector<std::shared_ptr<LocalInventoryItemResponse>>>::toJsonValue(getItems());
    return object;
}

void LocalInventoryItemsGet::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto ItemsIt = object.find("items");
        if (ItemsIt != object.end()) {
            setItems(JsonValueConverter<std::vector<std::shared_ptr<LocalInventoryItemResponse>>>::fromJsonValue(ItemsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LocalInventoryItemsGet>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LocalInventoryItemsGet>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LocalInventoryItemsGet>>>::fromJsonValue(boost::json::parse(json));
}
