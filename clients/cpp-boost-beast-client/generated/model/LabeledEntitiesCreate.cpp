...
LabeledEntitiesCreate::LabeledEntitiesCreate(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string LabeledEntitiesCreate::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value LabeledEntitiesCreate::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LabeledEntitiesCreate::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LabeledEntitiesCreate");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LabeledEntitiesCreate::toJsonObject_internal() const
{
    boost::json::object object;
        object["entity_ids"] = JsonValueConverter<std::vector<std::string>>::toJsonValue(getEntityIds());
    return object;
}

void LabeledEntitiesCreate::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Entity_idsIt = object.find("entity_ids");
        if (Entity_idsIt != object.end()) {
            setEntityIds(JsonValueConverter<std::vector<std::string>>::fromJsonValue(Entity_idsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LabeledEntitiesCreate>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LabeledEntitiesCreate>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LabeledEntitiesCreate>>>::fromJsonValue(boost::json::parse(json));
}
