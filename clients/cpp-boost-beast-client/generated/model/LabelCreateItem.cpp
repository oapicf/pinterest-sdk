...
LabelCreateItem::LabelCreateItem(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string LabelCreateItem::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value LabelCreateItem::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LabelCreateItem::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LabelCreateItem");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LabelCreateItem::toJsonObject_internal() const
{
    boost::json::object object;
        object["label_type"] = JsonValueConverter<std::shared_ptr<LabelType>>::toJsonValue(getLabelType());
        object["value"] = JsonValueConverter<std::string>::toJsonValue(getValue());
    return object;
}

void LabelCreateItem::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Label_typeIt = object.find("label_type");
        if (Label_typeIt != object.end()) {
            setLabelType(JsonValueConverter<std::shared_ptr<LabelType>>::fromJsonValue(Label_typeIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LabelCreateItem>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LabelCreateItem>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LabelCreateItem>>>::fromJsonValue(boost::json::parse(json));
}
