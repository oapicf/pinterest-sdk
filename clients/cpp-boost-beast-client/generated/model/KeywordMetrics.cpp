...
KeywordMetrics::KeywordMetrics(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string KeywordMetrics::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value KeywordMetrics::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void KeywordMetrics::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for KeywordMetrics");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object KeywordMetrics::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Keyword_query_volumeIsSet) {
            object["keyword_query_volume"] = JsonValueConverter<std::string>::toJsonValue(getKeywordQueryVolume());
        }
    return object;
}

void KeywordMetrics::fromJsonObject_internal(boost::json::object const& object)
{
    m_Keyword_query_volumeIsSet = false;
    {
        const auto Keyword_query_volumeIt = object.find("keyword_query_volume");
        if (Keyword_query_volumeIt != object.end()) {
            setKeywordQueryVolume(JsonValueConverter<std::string>::fromJsonValue(Keyword_query_volumeIt->value()));
        }
    }
}
...
void KeywordMetrics::setKeywordQueryVolume(std::string value)
{
    ...
    m_Keyword_query_volume = std::move(value);
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<KeywordMetrics>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<KeywordMetrics>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<KeywordMetrics>>>::fromJsonValue(boost::json::parse(json));
}
