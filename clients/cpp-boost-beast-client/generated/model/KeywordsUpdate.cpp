...
KeywordsUpdate::KeywordsUpdate(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value KeywordsUpdate::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void KeywordsUpdate::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for KeywordsUpdate");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object KeywordsUpdate::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_KeywordsIsSet) {
            object["keywords"] = JsonValueConverter<std::vector<std::shared_ptr<KeywordUpdateItem>>>::toJsonValue(getKeywords());
        }
    return object;
}

void KeywordsUpdate::fromJsonObject_internal(boost::json::object const& object)
{
    m_KeywordsIsSet = false;
    {
        const auto KeywordsIt = object.find("keywords");
        if (KeywordsIt != object.end()) {
            setKeywords(JsonValueConverter<std::vector<std::shared_ptr<KeywordUpdateItem>>>::fromJsonValue(KeywordsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<KeywordsUpdate>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<KeywordsUpdate>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<KeywordsUpdate>>>::fromJsonValue(boost::json::parse(json));
}
