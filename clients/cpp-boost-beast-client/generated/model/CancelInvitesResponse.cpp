...
CancelInvitesResponse::CancelInvitesResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string CancelInvitesResponse::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value CancelInvitesResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void CancelInvitesResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for CancelInvitesResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object CancelInvitesResponse::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ItemsIsSet) {
            object["items"] = JsonValueConverter<std::vector<std::shared_ptr<CancelInviteResultItem>>>::toJsonValue(getItems());
        }
    return object;
}

void CancelInvitesResponse::fromJsonObject_internal(boost::json::object const& object)
{
    m_ItemsIsSet = false;
    {
        const auto ItemsIt = object.find("items");
        if (ItemsIt != object.end()) {
            setItems(JsonValueConverter<std::vector<std::shared_ptr<CancelInviteResultItem>>>::fromJsonValue(ItemsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<CancelInvitesResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<CancelInvitesResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<CancelInvitesResponse>>>::fromJsonValue(boost::json::parse(json));
}
