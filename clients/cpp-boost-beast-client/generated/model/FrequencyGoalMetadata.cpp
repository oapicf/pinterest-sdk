...
FrequencyGoalMetadata::FrequencyGoalMetadata(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string FrequencyGoalMetadata::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value FrequencyGoalMetadata::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void FrequencyGoalMetadata::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for FrequencyGoalMetadata");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object FrequencyGoalMetadata::toJsonObject_internal() const
{
    boost::json::object object;
        ...
        if (m_TimerangeIsSet) {
            object["timerange"] = JsonValueConverter<std::shared_ptr<FrequencyGoalMetadataTimerange>>::toJsonValue(getTimerange());
        }
    return object;
}

void FrequencyGoalMetadata::fromJsonObject_internal(boost::json::object const& object)
{
    ...
    {
        const auto TimerangeIt = object.find("timerange");
        if (TimerangeIt != object.end()) {
            setTimerange(JsonValueConverter<std::shared_ptr<FrequencyGoalMetadataTimerange>>::fromJsonValue(TimerangeIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<FrequencyGoalMetadata>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<FrequencyGoalMetadata>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<FrequencyGoalMetadata>>>::fromJsonValue(boost::json::parse(json));
}
