...
BrandAccount::BrandAccount(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string BrandAccount::toJsonString(bool prettyJson /* = false */) const
{
...
void BrandAccount::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value BrandAccount::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void BrandAccount::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for BrandAccount");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object BrandAccount::toJsonObject_internal() const
{
    boost::json::object object;
        object["brand_account_id"] = JsonValueConverter<std::string>::toJsonValue(getBrandAccountId());
    return object;
}

void BrandAccount::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Brand_account_idIt = object.find("brand_account_id");
        if (Brand_account_idIt != object.end()) {
            setBrandAccountId(JsonValueConverter<std::string>::fromJsonValue(Brand_account_idIt->value()));
        }
    }
}
...
void BrandAccount::setBrandAccountId(std::string value)
{
    
    m_Brand_account_id = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<BrandAccount>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<BrandAccount>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<BrandAccount>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<BrandAccount>>>::fromJsonValue(boost::json::parse(json));
}
