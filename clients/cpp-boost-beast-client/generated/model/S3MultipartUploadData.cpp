...
S3MultipartUploadData::S3MultipartUploadData(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value S3MultipartUploadData::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void S3MultipartUploadData::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for S3MultipartUploadData");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object S3MultipartUploadData::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_File_partsIsSet) {
            object["file_parts"] = JsonValueConverter<std::vector<std::shared_ptr<S3FilePart>>>::toJsonValue(getFileParts());
        }
    return object;
}

void S3MultipartUploadData::fromJsonObject_internal(boost::json::object const& object)
{
    m_File_partsIsSet = false;
    {
        const auto File_partsIt = object.find("file_parts");
        if (File_partsIt != object.end()) {
            setFileParts(JsonValueConverter<std::vector<std::shared_ptr<S3FilePart>>>::fromJsonValue(File_partsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<S3MultipartUploadData>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<S3MultipartUploadData>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<S3MultipartUploadData>>>::fromJsonValue(boost::json::parse(json));
}
