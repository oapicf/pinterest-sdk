...
KeywordsCreate::KeywordsCreate(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value KeywordsCreate::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void KeywordsCreate::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for KeywordsCreate");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object KeywordsCreate::toJsonObject_internal() const
{
    boost::json::object object;
        object["keywords"] = JsonValueConverter<std::vector<std::shared_ptr<KeywordCreateItem>>>::toJsonValue(getKeywords());
        object["parent_id"] = JsonValueConverter<std::string>::toJsonValue(getParentId());
    return object;
}

void KeywordsCreate::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto KeywordsIt = object.find("keywords");
        if (KeywordsIt != object.end()) {
            setKeywords(JsonValueConverter<std::vector<std::shared_ptr<KeywordCreateItem>>>::fromJsonValue(KeywordsIt->value()));
        }
    }
    {
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<KeywordsCreate>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<KeywordsCreate>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<KeywordsCreate>>>::fromJsonValue(boost::json::parse(json));
}
