...
CartingRetailer::CartingRetailer(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string CartingRetailer::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value CartingRetailer::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void CartingRetailer::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for CartingRetailer");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object CartingRetailer::toJsonObject_internal() const
{
    boost::json::object object;
        object["retailer_id"] = JsonValueConverter<std::string>::toJsonValue(getRetailerId());
        object["retailer_name"] = JsonValueConverter<std::string>::toJsonValue(getRetailerName());
    return object;
}

void CartingRetailer::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Retailer_idIt = object.find("retailer_id");
        if (Retailer_idIt != object.end()) {
            setRetailerId(JsonValueConverter<std::string>::fromJsonValue(Retailer_idIt->value()));
        }
    }
    {
        const auto Retailer_nameIt = object.find("retailer_name");
        if (Retailer_nameIt != object.end()) {
            setRetailerName(JsonValueConverter<std::string>::fromJsonValue(Retailer_nameIt->value()));
        }
    }
}
...
void CartingRetailer::setRetailerName(std::string value)
{
    ...
    m_Retailer_name = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<CartingRetailer>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<CartingRetailer>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<CartingRetailer>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<CartingRetailer>>>::fromJsonValue(boost::json::parse(json));
}
