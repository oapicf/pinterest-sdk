# coding: utf-8

"""
    Pinterest REST API

    Pinterest's REST API

    The version of the OpenAPI document: 5.28.0
    Contact: blah+oapicf@cliffano.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import json
import pprint
import re  # noqa: F401
from aenum import Enum, no_arg





class GridClickType(str, Enum):
    """
    Where a user is taken after clicking on an ad in grid.  **Note:**  This parameter is read-only and is set to `DIRECT_TO_DESTINATION` by default for direct links supported ads. `grid_click_type` values provided will be ignored.
    """

    """
    allowed enum values
    """
    CLOSEUP = 'CLOSEUP'
    DIRECT_TO_DESTINATION = 'DIRECT_TO_DESTINATION'

    @classmethod
    def from_json(cls, json_str: str) -> GridClickType:
        """Create an instance of GridClickType from a JSON string"""
        return GridClickType(json.loads(json_str))


