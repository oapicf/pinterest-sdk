...
- `POST` to `/ad_accounts/:ad_account_id/reports/brand_category_sku`, OperationId: `analytics/create_conversion_product_report`:
...
- `GET` to `/ad_accounts/:ad_account_id/reports/brand_category_sku`, OperationId: `analytics/get_conversion_product_report`:
...
allowed_methods(Req, #state{operation_id = 'ad_accounts/list'} = State) ->
    {[<<"GET">>], Req, State};
allowed_methods(Req, #state{operation_id = 'analytics/create_conversion_product_report'} = State) ->
    {[<<"POST">>], Req, State};
...
allowed_methods(Req, #state{operation_id = 'analytics/create_template_report'} = State) ->
    {[<<"POST">>], Req, State};
allowed_methods(Req, #state{operation_id = 'analytics/get_conversion_product_report'} = State) ->
    {[<<"GET">>], Req, State};
...
is_authorized(Req0,
              #state{operation_id = 'analytics/create_conversion_product_report' = OperationID,
                     api_key_callback = Handler} = State) ->
    case openapi_auth:authorize_api_key(Handler, OperationID, header, <<"authorization">>, Req0) of
        {true, Context, Req} ->
            {true, Req, State#state{context = Context}};
...
is_authorized(Req0,
              #state{operation_id = 'analytics/get_conversion_product_report' = OperationID,
                     api_key_callback = Handler} = State) ->
    case openapi_auth:authorize_api_key(Handler, OperationID, header, <<"authorization">>, Req0) of
        {true, Context, Req} ->
            {true, Req, State#state{context = Context}};
...
content_types_accepted(Req, #state{operation_id = 'ad_accounts/list'} = State) ->
    {[], Req, State};
content_types_accepted(Req, #state{operation_id = 'analytics/create_conversion_product_report'} = State) ->
    {[
      {<<"application/json">>, handle_type_accepted}
     ], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'analytics/create_template_report'} = State) ->
    {[], Req, State};
content_types_accepted(Req, #state{operation_id = 'analytics/get_conversion_product_report'} = State) ->
    {[], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'ad_accounts/list'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'analytics/create_conversion_product_report'} = State) ->
    {true, Req, State};
...
valid_content_headers(Req, #state{operation_id = 'analytics/create_template_report'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'analytics/get_conversion_product_report'} = State) ->
    {true, Req, State};
...
content_types_provided(Req, #state{operation_id = 'analytics/create_conversion_product_report'} = State) ->
    {[
      {<<"application/json">>, handle_type_provided}
     ], Req, State};
...
content_types_provided(Req, #state{operation_id = 'analytics/get_conversion_product_report'} = State) ->
    {[
      {<<"application/json">>, handle_type_provided}
     ], Req, State};
