...
- `GET` to `/ad_accounts/:ad_account_id/pins/analytics`, OperationId: `ad_pins/analytics`:
Get pins analytics.
...
- `POST` to `/ad_accounts/:ad_account_id/campaigns/delivery_estimates`, OperationId: `get_campaign_delivery_estimates`:
...
allowed_methods(Req, #state{operation_id = 'ad_pins/analytics'} = State) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(Req, #state{operation_id = 'campaigns/update'} = State) ->
    {[<<"PATCH">>], Req, State};
allowed_methods(Req, #state{operation_id = 'get_campaign_delivery_estimates'} = State) ->
    {[<<"POST">>], Req, State};
...
is_authorized(Req0,
              #state{operation_id = 'ad_pins/analytics' = OperationID,
...
is_authorized(Req0,
              #state{operation_id = 'get_campaign_delivery_estimates' = OperationID,
                     api_key_callback = Handler} = State) ->
    case openapi_auth:authorize_api_key(Handler, OperationID, header, <<"authorization">>, Req0) of
        {true, Context, Req} ->
            {true, Req, State#state{context = Context}};
...
content_types_accepted(Req, #state{operation_id = 'ad_pins/analytics'} = State) ->
    {[], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'get_campaign_delivery_estimates'} = State) ->
    {[
      {<<"application/json">>, handle_type_accepted}
     ], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'ad_pins/analytics'} = State) ->
    {true, Req, State};
...
valid_content_headers(Req, #state{operation_id = 'campaigns/update'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'get_campaign_delivery_estimates'} = State) ->
    {true, Req, State};
...
-spec content_types_provided(cowboy_req:req(), state()) ->
    {[{binary(), atom()}], cowboy_req:req(), state()}.
content_types_provided(Req, #state{operation_id = 'ad_pins/analytics'} = State) ->
    {[
...
content_types_provided(Req, #state{operation_id = 'get_campaign_delivery_estimates'} = State) ->
    {[
      {<<"application/json">>, handle_type_provided}
     ], Req, State};
