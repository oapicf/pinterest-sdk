...
open class KeywordsAPI {
    ...
    /**
     ...
     - parameter matchTypes: (query) Keyword &lt;a target&#x3D;\&quot;_blank\&quot; href&#x3D;\&quot;/docs/ads/targeting/#Match%20type%20and%20targeting%20level\&quot;&gt;match type&lt;/a&gt; (optional)
    ...
    open class func trendingKeywordsList(region: TrendsSupportedRegion, trendType: TrendType, interests: [Interests_trendingKeywordsList]? = nil, genders: [Genders_trendingKeywordsList]? = nil, ages: [Ages_trendingKeywordsList]? = nil, normalizeAgainstGroup: Bool? = nil, limit: Int? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: TrendingKeywordsResponse?, _ error: Error?) -> Void)) -> RequestTask {
        return trendingKeywordsListWithRequestBuilder(region: region, trendType: trendType, interests: interests, genders: genders, ages: ages, normalizeAgainstGroup: normalizeAgainstGroup, limit: limit).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
