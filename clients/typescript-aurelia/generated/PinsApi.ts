...
export class PinsApi extends Api {
   ...
   * @param params.adAccountId Unique identifier of an ad account.
   */
  async pinsDelete(params: IPinsDeleteParams): Promise<Pin> {
    // Verify required parameters are set
   ...
   * @param params.pinFilter The filter to apply to the pins
   * @param params.pinMetrics Specify whether to return 90d and lifetime Pin metrics. Total comments and total reactions are only available with lifetime Pin metrics. If Pin was created before &#x60;2023-03-20&#x60; lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then.
   * @param params.includeProtectedPins Whether to include protected pins in the results
   ...
   * @param params.adAccountId Unique identifier of an ad account.
   * @param params.domain Only return pins with links that match the exact domain. Domain should not include \&#39;www.\&#39; prefix. For example, \&#39;pinterest.com\&#39; is a valid domain, but \&#39;www.pinterest.com\&#39; is not (will not match any pins).
   * @param params.domains Only return pins with links whose domain matches any value in the list. Values are joined comma-separated on the wire (e.g. &#x60;?domains&#x3D;instagram.com,jcpenney.com&#x60;).
   * @param params.includeProductTagObj Include product tag objects in the response with their associated links.
  ...
  async pinsList(params: IPinsListParams): Promise<PinsList200Response> {
    ...
    const response = await this.httpClient.createRequest(url)
      ...
      .withParams({ 
        ...
        'include_product_tag_obj': params['includeProductTagObj'],
   ...
   * @param params.pinId Unique identifier of a Pin.
   * @param params.pinsSaveRequestCreate 
   * @param params.adAccountId Unique identifier of an ad account.
   */
  async pinsSave(params: IPinsSaveParams): Promise<Pin> {
    // Verify required parameters are set
    this.ensureParamIsSet('pinsSave', params, 'pinId');
    this.ensureParamIsSet('pinsSave', params, 'pinsSaveRequestCreate');
    ...
    const response = await this.httpClient.createRequest(url)
      ...
      // Encode body parameter
      .withHeader('content-type', 'application/json')
      .withContent(JSON.stringify(params['pinsSaveRequestCreate'] || {}))
