...
from openapi_server.models.targeting_strategy import TargetingStrategy
try:
...
class TargetingSpecOptimal(BaseModel):
    ...
    location: Optional[List[StrictStr]] = Field(default=None, description="Metropolitan codes and/or ISO-Alpha-2, two-letter country codes to include for targeting.  Precede country code values with the `country_codes` key and metro code values with `metro_codes` key. Example:  ``` \"location\": {   \"country_codes\": [\"US\", \"CA\"],   \"metro_codes\": [\"501\", \"602\"] } ```  For each ad group, specify at least one `GEO` or `LOCATION` code.  If you do not specify a `LOCATION` code, only `GEO` values will be targeted (See `GEO` parameter in this targeting spec.).  Learn how to [get a current, complete list of codes](/docs/analytics-and-reports/ads-reporting/#get-all-available-codes-and-zones).", alias="LOCATION")
    location_exclude: Optional[List[StrictStr]] = Field(default=None, description="[Closed beta](/docs/getting-started/using-beta-and-restricted-features/)  Metropolitan codes and/or ISO-Alpha-2, two-letter country codes to exclude from targeting.  See `LOCATION` parameter in this targeting spec for rules, syntax, and other information.", alias="LOCATION_EXCLUDE")
    ...
    targeting_strategy: Optional[List[TargetingStrategy]] = Field(default=None, alias="TARGETING_STRATEGY")
