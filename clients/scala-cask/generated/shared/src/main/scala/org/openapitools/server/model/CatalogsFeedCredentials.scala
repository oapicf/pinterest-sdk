/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 *
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 *
 * https://openapi-generator.tech
 */

// this model was generated using model.mustache
package org.openapitools.server.model

import scala.util.control.NonFatal

// see https://com-lihaoyi.github.io/upickle/
import upickle.default.{ReadWriter => RW, macroRW}
import upickle.default.*


        
case class CatalogsFeedCredentials(
        /* The required password for downloading a feed. */
    password: String,
        /* The required username for downloading a feed. */
    username: String


) {

def asJsonString: String = asData.asJsonString
def asJson: ujson.Value = asData.asJson

def asData : CatalogsFeedCredentialsData = {
CatalogsFeedCredentialsData(
    password = password /* 2 */,
    username = username /* 2 */

)
}
}

object CatalogsFeedCredentials {
given RW[CatalogsFeedCredentials] = summon[RW[ujson.Value]].bimap[CatalogsFeedCredentials](_.asJson, json => read[CatalogsFeedCredentialsData](json).asModel)

enum Fields(val fieldName : String) extends Field(fieldName) {
    case password extends Fields("password")
    case username extends Fields("username")
}


}


