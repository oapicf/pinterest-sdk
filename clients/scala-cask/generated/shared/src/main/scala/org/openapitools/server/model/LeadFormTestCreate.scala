/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 *
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 *
 * https://openapi-generator.tech
 */

// this model was generated using model.mustache
package org.openapitools.server.model

import scala.util.control.NonFatal

// see https://com-lihaoyi.github.io/upickle/
import upickle.default.{ReadWriter => RW, macroRW}
import upickle.default.*


        
case class LeadFormTestCreate(
        /* Test lead answers. Should follow the creation order. */
    answers: Seq[String]


) {

def asJsonString: String = asData.asJsonString
def asJson: ujson.Value = asData.asJson

def asData : LeadFormTestCreateData = {
LeadFormTestCreateData(
    answers = answers /* 2 */

)
}
}

object LeadFormTestCreate {
given RW[LeadFormTestCreate] = summon[RW[ujson.Value]].bimap[LeadFormTestCreate](_.asJson, json => read[LeadFormTestCreateData](json).asModel)

enum Fields(val fieldName : String) extends Field(fieldName) {
    case answers extends Fields("answers")
}


}


