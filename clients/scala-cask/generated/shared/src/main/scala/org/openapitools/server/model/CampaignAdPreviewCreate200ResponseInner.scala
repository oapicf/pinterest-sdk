/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 *
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 *
 * https://openapi-generator.tech
 */

// this model was generated using model.mustache
package org.openapitools.server.model

import scala.util.control.NonFatal

// see https://com-lihaoyi.github.io/upickle/
import upickle.default.{ReadWriter => RW, macroRW}
import upickle.default.*


        
case class CampaignAdPreviewCreate200ResponseInner(
    data: CampaignAdPreviewCreate200ResponseInnerData


) {

def asJsonString: String = asData.asJsonString
def asJson: ujson.Value = asData.asJson

def asData : CampaignAdPreviewCreate200ResponseInnerData = {
CampaignAdPreviewCreate200ResponseInnerData(
    data = data.asData /* 8 */

)
}
}

object CampaignAdPreviewCreate200ResponseInner {
given RW[CampaignAdPreviewCreate200ResponseInner] = summon[RW[ujson.Value]].bimap[CampaignAdPreviewCreate200ResponseInner](_.asJson, json => read[CampaignAdPreviewCreate200ResponseInnerData](json).asModel)

enum Fields(val fieldName : String) extends Field(fieldName) {
    case data extends Fields("data")
}


}


