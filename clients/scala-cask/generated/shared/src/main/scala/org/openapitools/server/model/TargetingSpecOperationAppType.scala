/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 *
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 *
 * https://openapi-generator.tech
 */

// this model was generated using model.mustache
package org.openapitools.server.model

import scala.util.control.NonFatal

// see https://com-lihaoyi.github.io/upickle/
import upickle.default.{ReadWriter => RW, macroRW}
import upickle.default.*


        
case class TargetingSpecOperationAppType(
    field: TargetingSpecOperationAppType.FieldEnum,
    operation: TargetingSpecListOperation,
    values: Seq[TargetingSpecAppType]


) {

def asJsonString: String = asData.asJsonString
def asJson: ujson.Value = asData.asJson

def asData : TargetingSpecOperationAppTypeData = {
TargetingSpecOperationAppTypeData(
    field = field /* 2 */,
    operation = operation /* 2 */,
    values = values /* 5.1 */

)
}
}

object TargetingSpecOperationAppType {
given RW[TargetingSpecOperationAppType] = summon[RW[ujson.Value]].bimap[TargetingSpecOperationAppType](_.asJson, json => read[TargetingSpecOperationAppTypeData](json).asModel)

enum Fields(val fieldName : String) extends Field(fieldName) {
    case field extends Fields("field")
    case operation extends Fields("operation")
    case values extends Fields("values")
}

        // baseName=field
        // nameInCamelCase = field
        enum FieldEnum derives ReadWriter {
            case APPTYPE
        }

}


