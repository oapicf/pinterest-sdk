/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * OpenAPI spec version: 5.28.0
 *
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 *
 * https://openapi-generator.tech
 */

// this model was generated using model.mustache
package org.openapitools.server.model

import scala.util.control.NonFatal

// see https://com-lihaoyi.github.io/upickle/
import upickle.default.{ReadWriter => RW, macroRW}
import upickle.default.*


        
case class AdvertiserDefinedEvent(
        /* Standard type mapped to ADE for optimization */
    mappedConversionType: Option[ConversionTagTypeOptimal] = None ,
        /* Raw string name of the event, usually logged as raw_event_name in our dataset */
    name: Option[String] = None 


) {

def asJsonString: String = asData.asJsonString
def asJson: ujson.Value = asData.asJson

def asData : AdvertiserDefinedEventData = {
AdvertiserDefinedEventData(
    mappedConversionType = mappedConversionType.getOrElse(null) /*  1 */,
    name = name.getOrElse("") /*  1 */

)
}
}

object AdvertiserDefinedEvent {
given RW[AdvertiserDefinedEvent] = summon[RW[ujson.Value]].bimap[AdvertiserDefinedEvent](_.asJson, json => read[AdvertiserDefinedEventData](json).asModel)

enum Fields(val fieldName : String) extends Field(fieldName) {
    case mappedConversionType extends Fields("mappedConversionType")
    case name extends Fields("name")
}


}


