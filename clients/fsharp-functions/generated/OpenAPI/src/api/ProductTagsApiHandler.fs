...
module ProductTagsApiHandlers =
    ...
    /// </summary>
   [<FunctionName("ProductTagsBulkAdd")>]
    let ProductTagsBulkAdd
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/pins/{pin_id}/product_tags")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        ...
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<ProductTagsBulkAddBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = ProductTagsApiService.ProductTagsBulkAdd bodyParams
      match result with
    ...
    /// </summary>
   [<FunctionName("ProductTagsBulkDelete")>]
    let ProductTagsBulkDelete
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/pins/{pin_id}/product_tags/bulk-delete")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        match (contentType.ToLower()) with
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<ProductTagsBulkDeleteBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = ProductTagsApiService.ProductTagsBulkDelete bodyParams
      match result with
    ...
    /// </summary>
   [<FunctionName("ProductTagsList")>]
    let ProductTagsList
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "GET", Route = "/v5/pins/{pin_id}/product_tags")>]
        req:HttpRequest ) =

      let result = ProductTagsApiService.ProductTagsList ()
      match result with
