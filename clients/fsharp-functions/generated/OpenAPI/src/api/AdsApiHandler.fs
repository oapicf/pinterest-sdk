...
module AdsApiHandlers =
   ...
   [<FunctionName("AdsUpdate")>]
    ...
    /// </summary>
   [<FunctionName("CampaignAdPreviewCreate")>]
    let CampaignAdPreviewCreate
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/ad_accounts/{ad_account_id}/campaign_ad_preview")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        match (contentType.ToLower()) with
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<CampaignAdPreviewCreateBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = AdsApiService.CampaignAdPreviewCreate bodyParams
      match result with
    ...
    /// </summary>
   [<FunctionName("CampaignAdPreviewDelete")>]
    let CampaignAdPreviewDelete
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "DELETE", Route = "/v5/ad_accounts/{ad_account_id}/campaign_ad_preview")>]
        req:HttpRequest ) =

      let result = AdsApiService.CampaignAdPreviewDelete ()
      match result with
    ...
    /// </summary>
   [<FunctionName("CampaignAdPreviewRead")>]
    let CampaignAdPreviewRead
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "GET", Route = "/v5/ad_accounts/{ad_account_id}/campaign_ad_preview")>]
        req:HttpRequest ) =

      let result = AdsApiService.CampaignAdPreviewRead ()
      match result with
