...
module LabelsApiHandlers =
    ...
    /// </summary>
   [<FunctionName("LabelsApply")>]
    let LabelsApply
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/ad_accounts/{ad_account_id}/labels/{label_id}/apply")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        ...
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<LabelsApplyBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = LabelsApiService.LabelsApply bodyParams
      match result with
   ...
   [<FunctionName("LabelsList")>]
    ...
    /// </summary>
   [<FunctionName("LabelsRemove")>]
    let LabelsRemove
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/ad_accounts/{ad_account_id}/labels/{label_id}/remove")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        match (contentType.ToLower()) with
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<LabelsRemoveBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = LabelsApiService.LabelsRemove bodyParams
      match result with
