...
module CatalogsApiHandlers =
    ...
    /// </summary>
   [<FunctionName("CatalogsAvailableFilterValues")>]
    let CatalogsAvailableFilterValues
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "GET", Route = "/v5/catalogs/available_filter_values")>]
        req:HttpRequest ) =

      let result = CatalogsApiService.CatalogsAvailableFilterValues ()
      match result with
