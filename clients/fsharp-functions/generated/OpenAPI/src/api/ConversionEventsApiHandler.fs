...
module ConversionEventsApiHandlers =
    ...
    /// Send conversions
    /// </summary>
   [<FunctionName("EventsCreate")>]
    let EventsCreate
        ([<HttpTrigger(Extensions.Http.AuthorizationLevel.Anonymous, "POST", Route = "/v5/ad_accounts/{ad_account_id}/events")>]
        req:HttpRequest ) =
      ...
      let bind (contentType:string) body  =
        ...
        | "application/json" ->
          body |> JsonConvert.DeserializeObject<EventsCreateBodyParams>
        | _ -> failwith (sprintf "TODO - ContentType %s not currently supported" contentType)

      let bodyParams = reader.ReadToEnd() |> bind req.ContentType
      let result = ConversionEventsApiService.EventsCreate bodyParams
      match result with
      ...
      | EventsCreateStatusCode503 resolved ->
          let content = JsonConvert.SerializeObject resolved.content
          let responseContentType = "application/json"
          ContentResult(Content = content, ContentType = responseContentType, StatusCode = System.Nullable(503))
