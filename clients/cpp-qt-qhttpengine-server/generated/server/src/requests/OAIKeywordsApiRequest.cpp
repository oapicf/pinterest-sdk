...
void OAIKeywordsApiRequest::keywords_getRequest(const QString& ad_account_idstr){
    ...
    Q_EMIT keywords_get(ad_account_id, campaign_id, ad_group_id, ad_group_ids, match_types, page_size, bookmark);
}
...
void OAIKeywordsApiRequest::trendingKeywords_listRequest(const QString& regionstr, const QString& trend_typestr){
    ...
    bool include_prediction;
    if(socket->queryString().keys().contains("include_prediction")){
        fromStringValue(socket->queryString().value("include_prediction"), include_prediction);
    }
    ...
    Q_EMIT trendingKeywords_list(region, trend_type, interests, genders, ages, include_keywords, normalize_against_group, limit, include_prediction, include_demographics);
}
