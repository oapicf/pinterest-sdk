...
void OAISearchApiRequest::searchUserBoards_getRequest(){
    ...
    Q_EMIT searchUserBoards_get(ad_account_id, query, bookmark, page_size);
}
...
void OAISearchApiRequest::searchUserBoards_getResponse(const OAIBoards_list_200_response& res){
    setSocketResponseHeaders();
    QJsonDocument resDoc(::OpenAPI::toJsonValue(res).toObject());
    socket->writeJson(resDoc);
    if(socket->isOpen()){
...
void OAISearchApiRequest::searchUserBoards_getError(const OAIBoards_list_200_response& res, QNetworkReply::NetworkError error_type, QString& error_str){
    Q_UNUSED(error_type); // TODO: Remap error_type to QHttpEngine::Socket errors
    setSocketResponseHeaders();
    Q_UNUSED(error_str);  // response will be used instead of error string
    QJsonDocument resDoc(::OpenAPI::toJsonValue(res).toObject());
