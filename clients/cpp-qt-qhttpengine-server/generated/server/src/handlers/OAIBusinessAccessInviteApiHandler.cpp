...
void OAIBusinessAccessInviteApiHandler::cancelInvitesOrRequests(QString business_id, OAICancelInvitesRequest oai_cancel_invites_request) {
    Q_UNUSED(business_id);
    Q_UNUSED(oai_cancel_invites_request);
    auto reqObj = qobject_cast<OAIBusinessAccessInviteApiRequest*>(sender());
    if( reqObj != nullptr )
    {
        OAICancelInvitesResponse res;
...
void OAIBusinessAccessInviteApiHandler::get_invites(QString business_id, bool is_member, QList<OAIInviteFilterStatus> invite_status, OAIInviteType invite_type, QString bookmark, qint32 page_size) {
    Q_UNUSED(business_id);
    Q_UNUSED(is_member);
