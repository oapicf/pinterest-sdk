/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAICreativeAssetsProcessingRecord.h
 *
 * Object describing an item processing record
 */

#ifndef OAICreativeAssetsProcessingRecord_H
#define OAICreativeAssetsProcessingRecord_H

#include <QJsonObject>

#include "OAIItemProcessingStatus.h"
#include "OAIItemValidationEvent.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAICreativeAssetsProcessingRecord : public OAIObject {
public:
    OAICreativeAssetsProcessingRecord();
    OAICreativeAssetsProcessingRecord(QString json);
    ~OAICreativeAssetsProcessingRecord() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getCreativeAssetsId() const;
    void setCreativeAssetsId(const QString &creative_assets_id);
    bool is_creative_assets_id_Set() const;
    bool is_creative_assets_id_Valid() const;

    QList<OAIItemValidationEvent> getErrors() const;
    void setErrors(const QList<OAIItemValidationEvent> &errors);
    bool is_errors_Set() const;
    bool is_errors_Valid() const;

    OAIItemProcessingStatus getStatus() const;
    void setStatus(const OAIItemProcessingStatus &status);
    bool is_status_Set() const;
    bool is_status_Valid() const;

    QList<OAIItemValidationEvent> getWarnings() const;
    void setWarnings(const QList<OAIItemValidationEvent> &warnings);
    bool is_warnings_Set() const;
    bool is_warnings_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString creative_assets_id;
    bool m_creative_assets_id_isSet;
    bool m_creative_assets_id_isValid;

    QList<OAIItemValidationEvent> errors;
    bool m_errors_isSet;
    bool m_errors_isValid;

    OAIItemProcessingStatus status;
    bool m_status_isSet;
    bool m_status_isValid;

    QList<OAIItemValidationEvent> warnings;
    bool m_warnings_isSet;
    bool m_warnings_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAICreativeAssetsProcessingRecord)

#endif // OAICreativeAssetsProcessingRecord_H
