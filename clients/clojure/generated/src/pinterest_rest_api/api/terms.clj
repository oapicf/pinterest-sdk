(ns pinterest-rest-api.api.terms
...
(defn-spec terms-related-list-with-http-info any?
  "List related terms
...
Example: the term 'workout' would list related terms like 'one song workout', 'yoga workout', 'workout motivation', etc."
  [terms (s/coll-of string?)]
  (check-required-params terms)
  (call-api "/terms/related" :get
            {:path-params   {}
             :header-params {}
             :query-params  {"terms" (with-collection-format terms :multi) }
             :form-params   {}
             :content-types []
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec terms-related-list related-terms-spec
  "List related terms
  Get a list of terms logically related to each input term.

Example: the term 'workout' would list related terms like 'one song workout', 'yoga workout', 'workout motivation', etc."
  [terms (s/coll-of string?)]
  (let [res (:data (terms-related-list-with-http-info terms))]
    (if (:decode-models *api-context*)
       (st/decode related-terms-spec res st/string-transformer)
       res)))


(defn-spec terms-suggested-list-with-http-info any?
...
Example: 'sport' would return popular terms like 'sports bar' and 'sportswear', but not 'motor sports' since the phrase does not begin with the given term."
  ([term string?, ] (terms-suggested-list-with-http-info term nil))
  ([term string?, {:keys [limit]} (s/map-of keyword? any?)]
   (check-required-params term)
   (call-api "/terms/suggested" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"term" term "limit" limit }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec terms-suggested-list (s/coll-of string?)
  "List suggested terms
  Get popular search terms that begin with your input term.

Example: 'sport' would return popular terms like 'sports bar' and 'sportswear', but not 'motor sports' since the phrase does not begin with the given term."
  ([term string?, ] (terms-suggested-list term nil))
  ([term string?, optional-params any?]
   (let [res (:data (terms-suggested-list-with-http-info term optional-params))]
     (if (:decode-models *api-context*)
