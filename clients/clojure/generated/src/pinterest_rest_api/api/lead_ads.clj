...
(defn-spec ad-accounts-subscriptions-get-by-id ad-account-get-subscription-response-spec
  "Get lead ads subscription
  Get a specific lead ads subscription record.
- Only requests for the OWNER or ADMIN of the ad_account will be allowed.
...
<strong>This endpoint is currently in beta and not available to all apps. <a href='/docs/getting-started/beta-and-advanced-access/'>Learn more</a>.</strong>"
  ...
  (let [res (:data (ad-accounts-subscriptions-get-by-id-with-http-info ad_account_id subscription_id))]
    (if (:decode-models *api-context*)
       (st/decode ad-account-get-subscription-response-spec res st/string-transformer)
       res)))
...
<strong>This endpoint is currently in beta and not available to all apps. <a href='/docs/getting-started/beta-and-advanced-access/'>Learn more</a>.</strong>"
  ...
  ([ad_account_id string?, {:keys [page_size bookmark]} (s/map-of keyword? any?)]
...
<strong>This endpoint is currently in beta and not available to all apps. <a href='/docs/getting-started/beta-and-advanced-access/'>Learn more</a>.</strong>"
  [ad_account_id string?, ad-account-create-subscription-request ad-account-create-subscription-request]
  (check-required-params ad_account_id ad-account-create-subscription-request)
  (call-api "/ad_accounts/{ad_account_id}/leads/subscriptions" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-account-create-subscription-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-accounts-subscriptions-post ad-account-create-subscription-response-spec
  "Create lead ads subscription
  Create a lead ads webhook subscription.
Subscriptions allow Pinterest to deliver lead data from Ads Manager directly to the subscriber. Subscriptions can exist for a specific lead form or at ad account level.
- Only requests for the OWNER or ADMIN of the ad_account will be allowed.
...
<strong>This endpoint is currently in beta and not available to all apps. <a href='/docs/getting-started/beta-and-advanced-access/'>Learn more</a>.</strong>"
  [ad_account_id string?, ad-account-create-subscription-request ad-account-create-subscription-request]
  (let [res (:data (ad-accounts-subscriptions-post-with-http-info ad_account_id ad-account-create-subscription-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-account-create-subscription-response-spec res st/string-transformer)
       res)))
