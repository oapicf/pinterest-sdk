...
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-targeting-analytics-get-with-http-info ad_account_id ad_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone sort_columns sort_ascending]} (s/map-of keyword? any?)]
...
* If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-targeting-analytics-get ad_account_id ad_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, optional-params any?]
...
(defn-spec ads-analytics-with-http-info any?
  ...
  Get analytics for the specified ads in the specified `ad_account_id`, filtered by the specified options.
    ...
    - If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
    - If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ads-analytics-with-http-info start_date end_date columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [pin_ids ad_ids click_window_days engagement_window_days view_window_days conversion_report_time campaign_ids reporting_timezone]} (s/map-of keyword? any?)]
   ...
   (call-api "/ad_accounts/{ad_account_id}/ads/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"pin_ids" (with-collection-format pin_ids :multi) "start_date" start_date "end_date" end_date "ad_ids" (with-collection-format ad_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "campaign_ids" (with-collection-format campaign_ids :multi) "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ads-analytics (s/coll-of ads-analytics-spec)
  "Get ad analytics
  Get analytics for the specified ads in the specified `ad_account_id`, filtered by the specified options.
    - The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via [Business Access](https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts): Admin, Analyst, Campaign Manager.
    - The request must contain either ad_ids or both campaign_ids and pin_ids.
    - If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
    - If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ads-analytics start_date end_date columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (ads-analytics-with-http-info start_date end_date columns granularity ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ads-analytics-spec) res st/string-transformer)
        res))))
...
(defn-spec ads-create-with-http-info any?
  ...
  [ad_account_id string?, ad-create (s/coll-of ad-create-spec)]
  (check-required-params ad_account_id ad-create)
  (call-api "/ad_accounts/{ad_account_id}/ads" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ads-create ad-batch-write-response-model-spec
  "Create ads
  Create multiple new ads. Request must contain ad_group_id, creative_type, and the source Pin pin_id."
  [ad_account_id string?, ad-create (s/coll-of ad-create-spec)]
  (let [res (:data (ads-create-with-http-info ad_account_id ad-create))]
    (if (:decode-models *api-context*)
       (st/decode ad-batch-write-response-model-spec res st/string-transformer)
       res)))
...
For more information about our policies and rejection reasons see the [Pinterest advertising standards](https://www.pinterest.com/_/_/policy/advertising-guidelines/)."
  [ad_id string?, ad_account_id string?]
  (check-required-params ad_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/ads/{ad_id}" :get
            {:path-params   {"ad_id" ad_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec ads-get ad-spec
  "Get ad
  Get a specific ad given the ad ID. If your pin is rejected, rejected_reasons will
contain additional information from the Ad Review process.
For more information about our policies and rejection reasons see the [Pinterest advertising standards](https://www.pinterest.com/_/_/policy/advertising-guidelines/)."
  [ad_id string?, ad_account_id string?]
  (let [res (:data (ads-get-with-http-info ad_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode ad-spec res st/string-transformer)
       res)))
...
For more, see [Pinterest advertising standards](https://policy.pinterest.com/en/advertising-guidelines)."
  ...
  ([ad_account_id string?, {:keys [bookmark page_size order campaign_ids ad_group_ids ad_ids entity_statuses]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/ads" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "campaign_ids" (with-collection-format campaign_ids :multi) "ad_group_ids" (with-collection-format ad_group_ids :multi) "ad_ids" (with-collection-format ad_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
(defn-spec ads-update-with-http-info any?
  ...
  [ad_account_id string?, ad-batch-update (s/coll-of ad-batch-update-spec)]
  (check-required-params ad_account_id ad-batch-update)
  (call-api "/ad_accounts/{ad_account_id}/ads" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-batch-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ads-update ad-batch-write-response-model-spec
  "Update ads
  Update multiple existing ads"
  [ad_account_id string?, ad-batch-update (s/coll-of ad-batch-update-spec)]
  (let [res (:data (ads-update-with-http-info ad_account_id ad-batch-update))]
    (if (:decode-models *api-context*)
       (st/decode ad-batch-write-response-model-spec res st/string-transformer)
       res)))


(defn-spec campaign-ad-preview-create-with-http-info any?
...
Each ad group is processed independently; individual failures do not block other previews."
  [ad_account_id string?, campaign-ad-preview-create (s/coll-of campaign-ad-preview-create-spec)]
  (check-required-params ad_account_id campaign-ad-preview-create)
  (call-api "/ad_accounts/{ad_account_id}/campaign_ad_preview" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-ad-preview-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaign-ad-preview-create (s/coll-of campaign-ad-preview-create-200-response-inner-spec)
  "Create ad preview records for one or more ad groups
  Create ad preview records for one or more ad groups that can be shared.
Each ad group is processed independently; individual failures do not block other previews."
  [ad_account_id string?, campaign-ad-preview-create (s/coll-of campaign-ad-preview-create-spec)]
  (let [res (:data (campaign-ad-preview-create-with-http-info ad_account_id campaign-ad-preview-create))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of campaign-ad-preview-create-200-response-inner-spec) res st/string-transformer)
       res)))


(defn-spec campaign-ad-preview-delete-with-http-info any?
  "Delete ad preview records for one or more ad groups
  Delete ad preview records for one or more ad groups.
All ad groups are validated before deleting any records."
  [ad_group_ids (s/coll-of string?), ad_account_id string?]
  (check-required-params ad_group_ids ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/campaign_ad_preview" :delete
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {"ad_group_ids" (with-collection-format ad_group_ids :multi) }
             :form-params   {}
             :content-types []
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaign-ad-preview-delete (s/coll-of campaign-ad-preview-delete-200-response-inner-spec)
  "Delete ad preview records for one or more ad groups
  Delete ad preview records for one or more ad groups.
All ad groups are validated before deleting any records."
  [ad_group_ids (s/coll-of string?), ad_account_id string?]
  (let [res (:data (campaign-ad-preview-delete-with-http-info ad_group_ids ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of campaign-ad-preview-delete-200-response-inner-spec) res st/string-transformer)
       res)))


(defn-spec campaign-ad-preview-read-with-http-info any?
  "Fetch ad preview records for one or more ad groups
  Fetch ad preview records for one or more ad groups.
Returns all active previews associated with the provided ad group IDs."
  [ad_group_ids (s/coll-of string?), ad_account_id string?]
  (check-required-params ad_group_ids ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/campaign_ad_preview" :get
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {"ad_group_ids" (with-collection-format ad_group_ids :multi) }
             :form-params   {}
...
(defn-spec campaign-ad-preview-read (s/coll-of campaign-ad-preview-spec)
  "Fetch ad preview records for one or more ad groups
  Fetch ad preview records for one or more ad groups.
Returns all active previews associated with the provided ad group IDs."
  [ad_group_ids (s/coll-of string?), ad_account_id string?]
  (let [res (:data (campaign-ad-preview-read-with-http-info ad_group_ids ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of campaign-ad-preview-spec) res st/string-transformer)
       res)))
