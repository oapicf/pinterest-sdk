...
(defn-spec ad-previews-create-with-http-info any?
...
You can view the returned preview URL on a webpage or iframe for 7 days, after which the URL expires. Collection ads are not currently supported ad preview."
  [ad_account_id string?, ad-preview-request ad-preview-request]
  (check-required-params ad_account_id ad-preview-request)
  (call-api "/ad_accounts/{ad_account_id}/ad_previews" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-preview-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-previews-create ad-preview-url-response-spec
  "Create ad preview with pin or image
  Create an ad preview given an ad account ID and either an existing organic pin ID or the URL for an image to be used to create the Pin and the ad. <p/>
If you are creating a preview from an existing Pin, that Pin must be promotable: that is, it must have a clickthrough link and meet other requirements. (See <a href=\"https://help.pinterest.com/en/business/article/promoted-pins-overview\" target=\"_blank\">Ads Overview</a>.) <p/>
You can view the returned preview URL on a webpage or iframe for 7 days, after which the URL expires. Collection ads are not currently supported ad preview."
  [ad_account_id string?, ad-preview-request ad-preview-request]
  (let [res (:data (ad-previews-create-with-http-info ad_account_id ad-preview-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-preview-url-response-spec res st/string-transformer)
       res)))


(defn-spec ad-targeting-analytics-get-with-http-info any?
...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (ad-targeting-analytics-get-with-http-info ad_account_id ad_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id ad_ids start_date end_date targeting_types columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/ads/targeting_analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"ad_ids" (with-collection-format ad_ids :multi) "start_date" start_date "end_date" end_date "targeting_types" (with-collection-format targeting_types :csv) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "attribution_types" attribution_types }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec ad-targeting-analytics-get metrics-response-spec
  "Get targeting analytics for ads
  Get targeting analytics for one or more ads. For the requested ad(s) and metrics,
the response will include the requested metric information (e.g. SPEND_IN_DOLLAR) for the requested target type
(e.g. \"age_bucket\") for applicable values (e.g. \"45-49\"). <p/>
...
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (ad-targeting-analytics-get ad_account_id ad_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-targeting-type-spec), columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (ad-targeting-analytics-get-with-http-info ad_account_id ad_ids start_date end_date targeting_types columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode metrics-response-spec res st/string-transformer)
        res))))


(defn-spec ads-analytics-with-http-info any?
  "Get ad analytics
...
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, ] (ads-analytics-with-http-info ad_account_id start_date end_date columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, {:keys [ad_ids click_window_days engagement_window_days view_window_days conversion_report_time pin_ids campaign_ids]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id start_date end_date columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/ads/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "ad_ids" (with-collection-format ad_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "pin_ids" (with-collection-format pin_ids :multi) "campaign_ids" (with-collection-format campaign_ids :multi) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec ads-analytics (s/coll-of ads-analytics-response-inner-spec)
  "Get ad analytics
  Get analytics for the specified ads in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
- The request must contain either ad_ids or both campaign_ids and pin_ids.
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, ] (ads-analytics ad_account_id start_date end_date columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (ads-analytics-with-http-info ad_account_id start_date end_date columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ads-analytics-response-inner-spec) res st/string-transformer)
        res))))


(defn-spec ads-create-with-http-info any?
  "Create ads
  Create multiple new ads. Request must contain ad_group_id, creative_type, and the source Pin pin_id."
  [ad_account_id string?, ad-create-request (s/coll-of ad-create-request-spec)]
  (check-required-params ad_account_id ad-create-request)
  (call-api "/ad_accounts/{ad_account_id}/ads" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ads-create ad-array-response-spec
  "Create ads
  Create multiple new ads. Request must contain ad_group_id, creative_type, and the source Pin pin_id."
  [ad_account_id string?, ad-create-request (s/coll-of ad-create-request-spec)]
  (let [res (:data (ads-create-with-http-info ad_account_id ad-create-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-array-response-spec res st/string-transformer)
       res)))


(defn-spec ads-get-with-http-info any?
  "Get ad
...
For more information about our policies and rejection reasons see the <a href=\"https://www.pinterest.com/_/_/policy/advertising-guidelines/\" target=\"_blank\">Pinterest advertising standards</a>."
  [ad_account_id string?, ad_id string?]
  (check-required-params ad_account_id ad_id)
  (call-api "/ad_accounts/{ad_account_id}/ads/{ad_id}" :get
            {:path-params   {"ad_account_id" ad_account_id "ad_id" ad_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec ads-get ad-response-spec
  "Get ad
  Get a specific ad given the ad ID. If your pin is rejected, rejected_reasons will
contain additional information from the Ad Review process.
For more information about our policies and rejection reasons see the <a href=\"https://www.pinterest.com/_/_/policy/advertising-guidelines/\" target=\"_blank\">Pinterest advertising standards</a>."
  [ad_account_id string?, ad_id string?]
  (let [res (:data (ads-get-with-http-info ad_account_id ad_id))]
    (if (:decode-models *api-context*)
       (st/decode ad-response-spec res st/string-transformer)
       res)))


(defn-spec ads-list-with-http-info any?
  "List ads
...
For more, see <a href=\"https://policy.pinterest.com/en/advertising-guidelines\">Pinterest advertising standards</a>."
  ([ad_account_id string?, ] (ads-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [campaign_ids ad_group_ids ad_ids entity_statuses page_size order bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/ads" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "ad_group_ids" (with-collection-format ad_group_ids :multi) "ad_ids" (with-collection-format ad_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "page_size" page_size "order" order "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec ads-list ads-list-200-response-spec
  "List ads
  List ads that meet the filters provided:
  - Listed campaign ids or ad group ids or ad ids
  - Listed entity statuses <p/>
...
Review status is provided for each ad; if review_status is REJECTED, the rejected_reasons field will contain additional information.
For more, see <a href=\"https://policy.pinterest.com/en/advertising-guidelines\">Pinterest advertising standards</a>."
  ([ad_account_id string?, ] (ads-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (ads-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode ads-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec ads-update-with-http-info any?
  "Update ads
  Update multiple existing ads"
  [ad_account_id string?, ad-update-request (s/coll-of ad-update-request-spec)]
  (check-required-params ad_account_id ad-update-request)
  (call-api "/ad_accounts/{ad_account_id}/ads" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ads-update ad-array-response-spec
  "Update ads
  Update multiple existing ads"
  [ad_account_id string?, ad-update-request (s/coll-of ad-update-request-spec)]
  (let [res (:data (ads-update-with-http-info ad_account_id ad-update-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-array-response-spec res st/string-transformer)
       res)))
