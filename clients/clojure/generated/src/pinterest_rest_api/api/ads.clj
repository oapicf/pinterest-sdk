...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ...
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, {:keys [ad_ids click_window_days engagement_window_days view_window_days conversion_report_time pin_ids campaign_ids reporting_timezone]} (s/map-of keyword? any?)]
   ...
   (call-api "/ad_accounts/{ad_account_id}/ads/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "ad_ids" (with-collection-format ad_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "pin_ids" (with-collection-format pin_ids :multi) "campaign_ids" (with-collection-format campaign_ids :multi) "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
