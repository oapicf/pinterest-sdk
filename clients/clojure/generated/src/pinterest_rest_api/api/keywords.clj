(ns pinterest-rest-api.api.keywords
...
(defn-spec country-keywords-metrics-get-with-http-info any?
  ...
  [ad_account_id string?, country_code string?, keywords (s/coll-of string?)]
  (check-required-params ad_account_id country_code keywords)
  (call-api "/ad_accounts/{ad_account_id}/keywords/metrics" :get
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {"country_code" country_code "keywords" (with-collection-format keywords :csv) }
             :form-params   {}
             :content-types []
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))

(defn-spec country-keywords-metrics-get keywords-metrics-array-response-spec
  "Get country's keyword metrics
  See keyword metrics for a specified country, aggregated across all of Pinterest.
  (Definitions are available from the \"Get delivery metrics definitions\"
  [API endpoint](/docs/api/v5/#operation/delivery_metrics/get))."
  [ad_account_id string?, country_code string?, keywords (s/coll-of string?)]
  (let [res (:data (country-keywords-metrics-get-with-http-info ad_account_id country_code keywords))]
    (if (:decode-models *api-context*)
       (st/decode keywords-metrics-array-response-spec res st/string-transformer)
       res)))


(defn-spec keywords-create-with-http-info any?
  "Create keywords
  ...
  **Returns:**
  ...
  [ad_account_id string?, keywords-create keywords-create]
  (check-required-params ad_account_id keywords-create)
  (call-api "/ad_accounts/{ad_account_id}/keywords" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    keywords-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec keywords-create keywords-spec
  "Create keywords
  Create keywords for the following entity types (advertiser, campaign, ad group, or ad). For more information, see [Keyword targeting](https://help.pinterest.com/en/business/article/keyword-targeting).
  ...
  **Returns:**
  ...
  [ad_account_id string?, keywords-create keywords-create]
  (let [res (:data (keywords-create-with-http-info ad_account_id keywords-create))]
    (if (:decode-models *api-context*)
       (st/decode keywords-spec res st/string-transformer)
       res)))


(defn-spec keywords-get-with-http-info any?
  "Get keywords
  ...
  ([ad_account_id string?, ] (keywords-get-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [campaign_id ad_group_id ad_group_ids match_types bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/keywords" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_id" campaign_id "ad_group_id" ad_group_id "ad_group_ids" (with-collection-format ad_group_ids :multi) "match_types" (with-collection-format match_types :multi) "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec keywords-get keywords-get-200-response-spec
  "Get keywords
  Get a list of keywords based on the filters provided. If no filter is provided, it will default to the `ad_account_id` filter, which means it will only return keywords that specifically have `parent_id` set to the `ad_account_id`. Note: Keywords can have `ad_account_ids`, `campaign_ids`, and `ad_group_ids` set as their `parent_ids`. Keywords created through Ads Manager will have their `parent_id` set to an `ad_group_id`, not `ad_account_id`.
  ...
  ([ad_account_id string?, ] (keywords-get ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (keywords-get-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode keywords-get-200-response-spec res st/string-transformer)
        res))))


(defn-spec keywords-update-with-http-info any?
  "Update keywords
  ...
  [ad_account_id string?, keywords-update keywords-update]
  (check-required-params ad_account_id keywords-update)
  (call-api "/ad_accounts/{ad_account_id}/keywords" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    keywords-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec keywords-update keywords-spec
  "Update keywords
  Update one or more keywords' bid and archived fields. Archiving
  a keyword effectively deletes it - keywords no longer receive metrics and
  are no longer visible within the parent entity's keywords list."
  [ad_account_id string?, keywords-update keywords-update]
  (let [res (:data (keywords-update-with-http-info ad_account_id keywords-update))]
    (if (:decode-models *api-context*)
       (st/decode keywords-spec res st/string-transformer)
       res)))


(defn-spec trending-keywords-list-with-http-info any?
...
For an interactive way to explore this data, please visit [trends.pinterest.com](https://trends.pinterest.com)."
  ([region trends-supported-region, trend_type trend-type, ] (trending-keywords-list-with-http-info region trend_type nil))
  ([region trends-supported-region, trend_type trend-type, {:keys [interests genders ages include_keywords normalize_against_group limit include_demographics]} (s/map-of keyword? any?)]
   (check-required-params region trend_type)
   (call-api "/trends/keywords/{region}/top/{trend_type}" :get
             {:path-params   {"region" region "trend_type" trend_type }
              :header-params {}
              :query-params  {"interests" (with-collection-format interests :multi) "genders" (with-collection-format genders :multi) "ages" (with-collection-format ages :multi) "include_keywords" (with-collection-format include_keywords :multi) "normalize_against_group" normalize_against_group "limit" limit "include_demographics" include_demographics }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec trending-keywords-list trending-keywords-response-spec
  "List trending keywords
  Get the top trending search keywords among the Pinterest user audience.
...
For an interactive way to explore this data, please visit [trends.pinterest.com](https://trends.pinterest.com)."
  ([region trends-supported-region, trend_type trend-type, ] (trending-keywords-list region trend_type nil))
  ([region trends-supported-region, trend_type trend-type, optional-params any?]
   (let [res (:data (trending-keywords-list-with-http-info region trend_type optional-params))]
     (if (:decode-models *api-context*)
        (st/decode trending-keywords-response-spec res st/string-transformer)
        res))))
