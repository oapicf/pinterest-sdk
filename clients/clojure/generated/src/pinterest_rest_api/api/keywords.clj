...
<p><b>Returns:</b></p> <ul style=\"list-style-type: square;\"> <li><p>A successful call returns an object containing an array of new keyword objects and an empty &quot;errors&quot; object array.</p></li> <li><p>An unsuccessful call returns an empty keywords array, and, instead, inserts the entire object with nulled/negated properties into the &quot;errors&quot; object array:</p> <pre class=\"last literal-block\"> { \"keywords\": [], \"errors\": [ { \"data\": { \"archived\": null, \"match_type\": \"EXACT\", \"parent_type\": null, \"value\": \"foobar\", \"parent_id\": null, \"type\": \"keyword\", \"id\": null }, \"error_messages\": [ \"Advertisers and Campaigns only accept excluded targeting attributes.\" ] } } </pre></li> </ul>"
  ...
  ([ad_account_id string?, {:keys [campaign_id ad_group_id match_types page_size bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/keywords" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_id" campaign_id "ad_group_id" ad_group_id "match_types" (with-collection-format match_types :multi) "page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
...
(defn-spec trending-keywords-list-with-http-info any?
  ...
  ([region trends-supported-region, trend_type trend-type, {:keys [interests genders ages include_keywords normalize_against_group limit]} (s/map-of keyword? any?)]
   ...
   (call-api "/trends/keywords/{region}/top/{trend_type}" :get
             {:path-params   {"region" region "trend_type" trend_type }
              :header-params {}
              :query-params  {"interests" (with-collection-format interests :multi) "genders" (with-collection-format genders :multi) "ages" (with-collection-format ages :multi) "include_keywords" (with-collection-format include_keywords :multi) "normalize_against_group" normalize_against_group "limit" limit }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
