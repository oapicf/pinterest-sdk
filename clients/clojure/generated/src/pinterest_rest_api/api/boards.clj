...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account."
  ([board board, ] (boards-create-with-http-info board nil))
  ([board board, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params board)
...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account."
  ([board board, ] (boards-create board nil))
  ([board board, optional-params any?]
   (let [res (:data (boards-create-with-http-info board optional-params))]
     (if (:decode-models *api-context*)
...
- If no privacy is specified, all boards that can be returned (based on the scopes of the token and ad_account role if applicable) will be returned."
  ...
  ([{:keys [ad_account_id bookmark page_size privacy]} (s/map-of keyword? any?)]
   (call-api "/boards" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id "bookmark" bookmark "page_size" page_size "privacy" privacy }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
- Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account."
  ([board_id string?, board-update board-update, ] (boards-update-with-http-info board_id board-update nil))
  ([board_id string?, board-update board-update, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params board_id board-update)
   (call-api "/boards/{board_id}" :patch
             {:path-params   {"board_id" board_id }
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    board-update
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec boards-update board-spec
  "Update board
  Update a board owned by the \"operating user_account\".
- Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account."
  ([board_id string?, board-update board-update, ] (boards-update board_id board-update nil))
  ([board_id string?, board-update board-update, optional-params any?]
   (let [res (:data (boards-update-with-http-info board_id board-update optional-params))]
     (if (:decode-models *api-context*)
        (st/decode board-spec res st/string-transformer)
        res))))
