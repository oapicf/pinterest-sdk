(ns pinterest-rest-api.api.pins
...
(defn-spec multi-pins-analytics-with-http-info any?
...
If Pin was created before `2023-03-20` lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then."
  ([pin_ids (s/coll-of string?), start_date inst?, end_date inst?, metric_types (s/coll-of multi-pins-analytics-metric-types-item-spec), ] (multi-pins-analytics-with-http-info pin_ids start_date end_date metric_types nil))
  ([pin_ids (s/coll-of string?), start_date inst?, end_date inst?, metric_types (s/coll-of multi-pins-analytics-metric-types-item-spec), {:keys [app_types ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_ids start_date end_date metric_types)
   (call-api "/pins/analytics" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"pin_ids" (with-collection-format pin_ids :multi) "start_date" start_date "end_date" end_date "app_types" app_types "metric_types" (with-collection-format metric_types :csv) "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec multi-pins-analytics (s/map-of string? map?)
  "Get multiple Pin analytics
  **This endpoint is currently in beta and not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/).**
...
If Pin was created before `2023-03-20` lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then."
  ([pin_ids (s/coll-of string?), start_date inst?, end_date inst?, metric_types (s/coll-of multi-pins-analytics-metric-types-item-spec), ] (multi-pins-analytics pin_ids start_date end_date metric_types nil))
  ([pin_ids (s/coll-of string?), start_date inst?, end_date inst?, metric_types (s/coll-of multi-pins-analytics-metric-types-item-spec), optional-params any?]
...
(defn-spec pins-analytics-with-http-info any?
  "Get Pin analytics
...
If Pin was created before `2023-03-20` lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then."
  ([pin_id string?, start_date inst?, end_date inst?, metric_types (s/coll-of querypinanalyticsmetrictypes-items-spec), ] (pins-analytics-with-http-info pin_id start_date end_date metric_types nil))
  ([pin_id string?, start_date inst?, end_date inst?, metric_types (s/coll-of querypinanalyticsmetrictypes-items-spec), {:keys [app_types split_field ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_id start_date end_date metric_types)
   (call-api "/pins/{pin_id}/analytics" :get
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "app_types" app_types "metric_types" (with-collection-format metric_types :csv) "split_field" split_field "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec pins-analytics (s/map-of string? pin-analytics-metrics-response-spec)
  "Get Pin analytics
  Get analytics for a Pin owned by the \"operation user_account\" - or on a group board that has been shared with this account.
- By default, the \"operation user_account\" is the token user_account.
...
If Pin was created before `2023-03-20` lifetime metrics will only be available for Video and Idea Pin formats. Lifetime metrics are available for all Pin formats since then."
  ([pin_id string?, start_date inst?, end_date inst?, metric_types (s/coll-of querypinanalyticsmetrictypes-items-spec), ] (pins-analytics pin_id start_date end_date metric_types nil))
  ([pin_id string?, start_date inst?, end_date inst?, metric_types (s/coll-of querypinanalyticsmetrictypes-items-spec), optional-params any?]
...
(defn-spec pins-create-with-http-info any?
  "Create Pin
...
**[Learn more](/docs/api-features/creating-boards-and-pins/#creating-image-pins)** about image Pin creation."
  ([pin-create pin-create, ] (pins-create-with-http-info pin-create nil))
  ([pin-create pin-create, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin-create)
   (call-api "/pins" :post
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    pin-create
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec pins-create pin-spec
  "Create Pin
  Create a Pin on a board or board section owned by the \"operation user_account\".
...
**[Learn more](/docs/api-features/creating-boards-and-pins/#creating-image-pins)** about image Pin creation."
  ([pin-create pin-create, ] (pins-create pin-create nil))
  ([pin-create pin-create, optional-params any?]
   (let [res (:data (pins-create-with-http-info pin-create optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pin-spec res st/string-transformer)
        res))))


(defn-spec pins-delete-with-http-info any?
  "Delete Pin
  ...
  ([pin_id string?, ] (pins-delete-with-http-info pin_id nil))
  ([pin_id string?, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_id)
   (call-api "/pins/{pin_id}" :delete
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
...
(defn-spec pins-delete pin-spec
  "Delete Pin
  Delete a Pins owned by the \"operation user_account\" - or on a group board that has been shared with this account.
  - By default, the \"operation user_account\" is the token user_account.
  ...
  - For Pins on public or protected boards: Owner, Admin, Analyst, Campaign Manager.
  - For Pins on secret boards: Owner, Admin."
  ([pin_id string?, ] (pins-delete pin_id nil))
  ([pin_id string?, optional-params any?]
   (let [res (:data (pins-delete-with-http-info pin_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pin-spec res st/string-transformer)
        res))))


(defn-spec pins-get-with-http-info any?
  "Get Pin
  ...
  - For Pins on public or protected boards: Owner, Admin, Analyst, Campaign Manager.
  - For Pins on secret boards: Owner, Admin."
  ([pin_id string?, ] (pins-get-with-http-info pin_id nil))
  ([pin_id string?, {:keys [ad_account_id pin_metrics]} (s/map-of keyword? any?)]
   (check-required-params pin_id)
   (call-api "/pins/{pin_id}" :get
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id "pin_metrics" pin_metrics }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec pins-get pin-spec
  "Get Pin
  Get a Pin owned by the \"operation user_account\" - or on a group board that has been shared with this account.
  - By default, the \"operation user_account\" is the token user_account.
  ...
  - For Pins on public or protected boards: Owner, Admin, Analyst, Campaign Manager.
  - For Pins on secret boards: Owner, Admin."
  ([pin_id string?, ] (pins-get pin_id nil))
  ([pin_id string?, optional-params any?]
   (let [res (:data (pins-get-with-http-info pin_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pin-spec res st/string-transformer)
        res))))


(defn-spec pins-list-with-http-info any?
  "List Pins
  ...
  ([] (pins-list-with-http-info nil))
  ([{:keys [pin_filter pin_metrics include_protected_pins pin_type creative_types ad_account_id domain domains include_product_tag_obj bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/pins" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"pin_filter" pin_filter "pin_metrics" pin_metrics "include_protected_pins" include_protected_pins "pin_type" pin_type "creative_types" (with-collection-format creative_types :multi) "ad_account_id" ad_account_id "domain" domain "domains" (with-collection-format domains :multi) "include_product_tag_obj" include_product_tag_obj "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec pins-list pins-list-200-response-spec
  "List Pins
  Get a list of the Pins owned by the \"operation user_account\".
    - By default, the \"operation user_account\" is the token user_account.
    - All Pins owned by the \"operation user_account\" are included, regardless of who owns the board they are on.
  ...
  ([] (pins-list nil))
  ([optional-params any?]
   (let [res (:data (pins-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pins-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec pins-save-with-http-info any?
  "Save Pin
...
- Any public Pin can be saved given a pin ID."
  ([pin_id string?, pins-save-request-create pins-save-request-create, ] (pins-save-with-http-info pin_id pins-save-request-create nil))
  ([pin_id string?, pins-save-request-create pins-save-request-create, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_id pins-save-request-create)
   (call-api "/pins/{pin_id}/save" :post
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    pins-save-request-create
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec pins-save pin-spec
  "Save Pin
  Save a Pin on a board or board section owned by the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account.
Optional: Business Access: Specify an `ad_account_id` (obtained via [List ad accounts](/docs/api/v5/#operation/ad_accounts/list)) to use the owner of that ad_account as the \"operation user_account\". In order to do this, the token user_account must have one of the following [Business Access](https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts) roles on the ad_account:
...
- Any Pin type can be saved: image Pin, video Pin, Idea Pin, product Pin, etc.
- Any public Pin can be saved given a pin ID."
  ([pin_id string?, pins-save-request-create pins-save-request-create, ] (pins-save pin_id pins-save-request-create nil))
  ([pin_id string?, pins-save-request-create pins-save-request-create, optional-params any?]
   (let [res (:data (pins-save-with-http-info pin_id pins-save-request-create optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pin-spec res st/string-transformer)
        res))))


(defn-spec pins-update-with-http-info any?
  "Update Pin
...
**This endpoint is currently in beta and not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/).**"
  ([pin_id string?, pin-update pin-update, ] (pins-update-with-http-info pin_id pin-update nil))
  ([pin_id string?, pin-update pin-update, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_id pin-update)
   (call-api "/pins/{pin_id}" :patch
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    pin-update
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec pins-update pin-spec
  "Update Pin
  Update a pin owned by the \"operating user_account\".
- By default, the \"operation user_account\" is the token user_account.
...
**This endpoint is currently in beta and not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/).**"
  ([pin_id string?, pin-update pin-update, ] (pins-update pin_id pin-update nil))
  ([pin_id string?, pin-update pin-update, optional-params any?]
   (let [res (:data (pins-update-with-http-info pin_id pin-update optional-params))]
     (if (:decode-models *api-context*)
        (st/decode pin-spec res st/string-transformer)
        res))))
