...
- For Pins on secret boards: Owner, Admin."
  ...
  ([pin_id string?, {:keys [pin_metrics ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params pin_id)
   (call-api "/pins/{pin_id}" :get
             {:path-params   {"pin_id" pin_id }
              :header-params {}
              :query-params  {"pin_metrics" pin_metrics "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
request is timing out in this scenario we encourage you to use <a href='/docs/api/v5/#operation/boards/list_pins'>GET List Pins on Board</a>."
  ...
  ([{:keys [bookmark page_size pin_filter include_protected_pins pin_type creative_types ad_account_id pin_metrics]} (s/map-of keyword? any?)]
   (call-api "/pins" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "pin_filter" pin_filter "include_protected_pins" include_protected_pins "pin_type" pin_type "creative_types" (with-collection-format creative_types :multi) "ad_account_id" ad_account_id "pin_metrics" pin_metrics }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
