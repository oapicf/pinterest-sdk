(ns pinterest-rest-api.api.business-access-invite
...
(defn-spec asset-access-requests-create-with-http-info any?
  "Create a request to access an existing partner's assets.
  Create a request to access an existing partner's assets with the specified permissions. The request will be sent to the partner for approval. The assets that can be requested are ad accounts and profiles."
  [business_id string?, create-asset-access-request-body create-asset-access-request-body]
  (check-required-params business_id create-asset-access-request-body)
  (call-api "/businesses/{business_id}/requests/assets/access" :post
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-asset-access-request-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec asset-access-requests-create create-asset-access-request-response-spec
  "Create a request to access an existing partner's assets.
  Create a request to access an existing partner's assets with the specified permissions. The request will be sent to the partner for approval. The assets that can be requested are ad accounts and profiles."
  [business_id string?, create-asset-access-request-body create-asset-access-request-body]
  (let [res (:data (asset-access-requests-create-with-http-info business_id create-asset-access-request-body))]
    (if (:decode-models *api-context*)
       (st/decode create-asset-access-request-response-spec res st/string-transformer)
       res)))


(defn-spec cancel-invites-or-requests-with-http-info any?
  "Cancel invites/requests
  Cancel membership/partnership invites and/or requests."
  [business_id string?, cancel-invites-request cancel-invites-request]
  (check-required-params business_id cancel-invites-request)
  (call-api "/businesses/{business_id}/invites" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    cancel-invites-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec cancel-invites-or-requests cancel-invites-response-spec
  "Cancel invites/requests
  Cancel membership/partnership invites and/or requests."
  [business_id string?, cancel-invites-request cancel-invites-request]
  (let [res (:data (cancel-invites-or-requests-with-http-info business_id cancel-invites-request))]
    (if (:decode-models *api-context*)
       (st/decode cancel-invites-response-spec res st/string-transformer)
       res)))


(defn-spec create-asset-invites-with-http-info any?
...
To learn more about permission levels, visit https://help.pinterest.com/en/business/article/business-manager-overview."
  [business_id string?, create-asset-invites-request create-asset-invites-request]
  (check-required-params business_id create-asset-invites-request)
  (call-api "/businesses/{business_id}/invites/assets/access" :post
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-asset-invites-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec create-asset-invites update-invites-results-response-array-spec
  "Update invite/request with an asset permission
  Assign asset permissions information to an existing invite/request. Can be used to:
- Request access to a partner's asset. Note: This is only for when no existing partnership exists. If an existing
  partnership exists, use \"Create a request to access an existing partner's assets\" to request access to your
...
To learn more about permission levels, visit https://help.pinterest.com/en/business/article/business-manager-overview."
  [business_id string?, create-asset-invites-request create-asset-invites-request]
  (let [res (:data (create-asset-invites-with-http-info business_id create-asset-invites-request))]
    (if (:decode-models *api-context*)
       (st/decode update-invites-results-response-array-spec res st/string-transformer)
       res)))


(defn-spec create-membership-or-partnership-invites-with-http-info any?
...
- Request to be a partner so you can access their assets. This would require specifying the following:
    ...
    - business_role=\"PARTNER\"
    - partners"
  [business_id string?, create-membership-or-partnership-invites-body create-membership-or-partnership-invites-body]
  (check-required-params business_id create-membership-or-partnership-invites-body)
  (call-api "/businesses/{business_id}/invites" :post
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-membership-or-partnership-invites-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec create-membership-or-partnership-invites create-invites-results-response-array-spec
  "Create invites or requests
  Create batch invites or requests. Can create batch invites or requests as described below.
- Invite members to join the business. This would required specifying the following:
    - invite_type=\"MEMBER_INVITE\"
...
- Request to be a partner so you can access their assets. This would require specifying the following:
    ...
    - partners"
  [business_id string?, create-membership-or-partnership-invites-body create-membership-or-partnership-invites-body]
  (let [res (:data (create-membership-or-partnership-invites-with-http-info business_id create-membership-or-partnership-invites-body))]
    (if (:decode-models *api-context*)
       (st/decode create-invites-results-response-array-spec res st/string-transformer)
       res)))


(defn-spec get-invites-with-http-info any?
  "Get invites/requests
  Get the membership/partnership invites and/or requests for the authorized user."
  ([business_id string?, ] (get-invites-with-http-info business_id nil))
  ([business_id string?, {:keys [is_member invite_status invite_type bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/invites" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"is_member" is_member "invite_status" (with-collection-format invite_status :multi) "invite_type" invite_type "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-invites get-invites-200-response-spec
  "Get invites/requests
  Get the membership/partnership invites and/or requests for the authorized user."
  ([business_id string?, ] (get-invites business_id nil))
  ([business_id string?, optional-params any?]
   (let [res (:data (get-invites-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-invites-200-response-spec res st/string-transformer)
        res))))


(defn-spec respond-business-access-invites-with-http-info any?
  "Accept or decline an invite/request
  Accept or decline invites or requests."
  [auth-respond-invites-body auth-respond-invites-body]
  (check-required-params auth-respond-invites-body)
  (call-api "/businesses/invites" :patch
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    auth-respond-invites-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec respond-business-access-invites respond-to-invites-response-array-spec
  "Accept or decline an invite/request
  Accept or decline invites or requests."
  [auth-respond-invites-body auth-respond-invites-body]
  (let [res (:data (respond-business-access-invites-with-http-info auth-respond-invites-body))]
    (if (:decode-models *api-context*)
       (st/decode respond-to-invites-response-array-spec res st/string-transformer)
       res)))
