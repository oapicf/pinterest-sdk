...
only active entities will return data."
  [ad_account_id string?, bulk-download-request bulk-download-request]
  (check-required-params ad_account_id bulk-download-request)
  (call-api "/ad_accounts/{ad_account_id}/bulk/download" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    bulk-download-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec bulk-download-create bulk-download-response-spec
  "Get advertiser entities in bulk
  Create an asynchronous report that may include information on campaigns, ad groups, product groups, ads,
keywords, and/or labels; can filter by campaigns. Though the entities may be active, archived, or paused,
only active entities will return data."
  [ad_account_id string?, bulk-download-request bulk-download-request]
  (let [res (:data (bulk-download-create-with-http-info ad_account_id bulk-download-request))]
    (if (:decode-models *api-context*)
       (st/decode bulk-download-response-spec res st/string-transformer)
       res)))
...
(defn-spec bulk-request-get bulk-upsert-status-response-spec
  "Download advertiser entities in bulk
  Get the status of a bulk request by <code>request_id</code>, along with a download URL that will allow you to download the
new or updated entity data (campaigns, ad groups, product groups, ads, or keywords)."
  ...
  ([ad_account_id string?, bulk_request_id string?, optional-params any?]
   (let [res (:data (bulk-request-get-with-http-info ad_account_id bulk_request_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode bulk-upsert-status-response-spec res st/string-transformer)
        res))))
