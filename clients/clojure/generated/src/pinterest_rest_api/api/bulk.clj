(ns pinterest-rest-api.api.bulk
...
(defn-spec bulk-download-create-with-http-info any?
...
only active entities will return data."
  [ad_account_id string?, bulk-download-create bulk-download-create]
  (check-required-params ad_account_id bulk-download-create)
  (call-api "/ad_accounts/{ad_account_id}/bulk/download" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    bulk-download-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec bulk-download-create bulk-download-spec
  "Get advertiser entities in bulk
  Create an asynchronous report that may include information on campaigns, ad groups, product groups, ads,
keywords, schedules,and/or labels; can filter by campaigns. Though the entities may be active, archived, or paused,
only active entities will return data."
  [ad_account_id string?, bulk-download-create bulk-download-create]
  (let [res (:data (bulk-download-create-with-http-info ad_account_id bulk-download-create))]
    (if (:decode-models *api-context*)
       (st/decode bulk-download-spec res st/string-transformer)
       res)))


(defn-spec bulk-request-get-with-http-info any?
...
new or updated entity data (campaigns, ad groups, product groups, ads, schedules, or keywords)."
  ([ad_account_id string?, bulk_request_id string?, ] (bulk-request-get-with-http-info ad_account_id bulk_request_id nil))
  ([ad_account_id string?, bulk_request_id string?, {:keys [include_details]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id bulk_request_id)
   (call-api "/ad_accounts/{ad_account_id}/bulk/{bulk_request_id}" :get
             {:path-params   {"ad_account_id" ad_account_id "bulk_request_id" bulk_request_id }
              :header-params {}
              :query-params  {"include_details" include_details }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec bulk-request-get bulk-job-data-spec
  "Download advertiser entities in bulk
  Get the status of a bulk request by `request_id`, along with a download URL that will allow you to download the
new or updated entity data (campaigns, ad groups, product groups, ads, schedules, or keywords)."
  ([ad_account_id string?, bulk_request_id string?, ] (bulk-request-get ad_account_id bulk_request_id nil))
  ([ad_account_id string?, bulk_request_id string?, optional-params any?]
   (let [res (:data (bulk-request-get-with-http-info ad_account_id bulk_request_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode bulk-job-data-spec res st/string-transformer)
        res))))


(defn-spec bulk-upsert-create-with-http-info any?
...
that can be used to obtain the status of the request."
  [ad_account_id string?, bulk-upsert-request bulk-upsert-request]
  (check-required-params ad_account_id bulk-upsert-request)
  (call-api "/ad_accounts/{ad_account_id}/bulk/upsert" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    bulk-upsert-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec bulk-upsert-create bulk-upsert-response-spec
  "Create/update ad entities in bulk
  Either create or update any combination of campaigns, ad groups, product groups, ads, keywords, schedules, or labels.
Note that this request will be processed asynchronously; the response will include a <code>request_id</code>
that can be used to obtain the status of the request."
  [ad_account_id string?, bulk-upsert-request bulk-upsert-request]
  (let [res (:data (bulk-upsert-create-with-http-info ad_account_id bulk-upsert-request))]
    (if (:decode-models *api-context*)
       (st/decode bulk-upsert-response-spec res st/string-transformer)
       res)))
