...
The return response will include the member's business_role and assets they have access to if assets_summary=TRUE"
  ...
  ([business_id string?, {:keys [assets_summary business_roles member_ids start_index bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/members" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"assets_summary" assets_summary "business_roles" (with-collection-format business_roles :multi) "member_ids" member_ids "start_index" start_index "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
