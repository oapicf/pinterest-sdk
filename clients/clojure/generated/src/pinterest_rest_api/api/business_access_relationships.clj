(ns pinterest-rest-api.api.business-access-relationships
...
(defn-spec brand-accounts-create-with-http-info any?
  "Create a Brand Account
  Create a Brand Account that will be a child business of a business hierarchy. Request must contain name, username, and country."
  [business_hierarchy_id string?, brand-account-create brand-account-create]
  (check-required-params business_hierarchy_id brand-account-create)
  (call-api "/business_access/business_hierarchy/{business_hierarchy_id}/brand_accounts" :post
            {:path-params   {"business_hierarchy_id" business_hierarchy_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    brand-account-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec brand-accounts-create brand-account-spec
  "Create a Brand Account
  Create a Brand Account that will be a child business of a business hierarchy. Request must contain name, username, and country."
  [business_hierarchy_id string?, brand-account-create brand-account-create]
  (let [res (:data (brand-accounts-create-with-http-info business_hierarchy_id brand-account-create))]
    (if (:decode-models *api-context*)
       (st/decode brand-account-spec res st/string-transformer)
       res)))


(defn-spec brand-accounts-update-with-http-info any?
  "Update a Brand Account
  Update an existing Brand Account"
  [brand_account_id string?, business_hierarchy_id string?, brand-account-update brand-account-update]
  (check-required-params brand_account_id business_hierarchy_id brand-account-update)
  (call-api "/business_access/business_hierarchy/{business_hierarchy_id}/brand_accounts/{brand_account_id}" :patch
            {:path-params   {"brand_account_id" brand_account_id "business_hierarchy_id" business_hierarchy_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    brand-account-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec brand-accounts-update brand-account-spec
  "Update a Brand Account
  Update an existing Brand Account"
  [brand_account_id string?, business_hierarchy_id string?, brand-account-update brand-account-update]
  (let [res (:data (brand-accounts-update-with-http-info brand_account_id business_hierarchy_id brand-account-update))]
    (if (:decode-models *api-context*)
       (st/decode brand-account-spec res st/string-transformer)
       res)))


(defn-spec delete-business-membership-with-http-info any?
  "Terminate business memberships
  Terminate memberships between the specified members and your business."
  [business_id string?, delete-business-membership-body delete-business-membership-body]
  (check-required-params business_id delete-business-membership-body)
  (call-api "/businesses/{business_id}/members" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    delete-business-membership-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec delete-business-membership delete-business-membership-200-response-spec
  "Terminate business memberships
  Terminate memberships between the specified members and your business."
  [business_id string?, delete-business-membership-body delete-business-membership-body]
  (let [res (:data (delete-business-membership-with-http-info business_id delete-business-membership-body))]
    (if (:decode-models *api-context*)
       (st/decode delete-business-membership-200-response-spec res st/string-transformer)
       res)))


(defn-spec delete-business-partners-with-http-info any?
...
Note: You may only batch terminate partners of the same partner type."
  [business_id string?, delete-business-partners-delete delete-business-partners-delete]
  (check-required-params business_id delete-business-partners-delete)
  (call-api "/businesses/{business_id}/partners" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    delete-business-partners-delete
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec delete-business-partners delete-business-partners-spec
  "Terminate business partnerships
  Terminate partnerships between the specified partners and your business.
Note: You may only batch terminate partners of the same partner type."
  [business_id string?, delete-business-partners-delete delete-business-partners-delete]
  (let [res (:data (delete-business-partners-with-http-info business_id delete-business-partners-delete))]
    (if (:decode-models *api-context*)
       (st/decode delete-business-partners-spec res st/string-transformer)
       res)))


(defn-spec get-business-employers-with-http-info any?
  "List business employers for user
  Get all of the viewing user's business employers."
  ([] (get-business-employers-with-http-info nil))
  ([{:keys [assets_summary bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/businesses/employers" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"assets_summary" assets_summary "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-business-employers get-business-employers-200-response-spec
  "List business employers for user
  Get all of the viewing user's business employers."
  ([] (get-business-employers nil))
  ([optional-params any?]
   (let [res (:data (get-business-employers-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-business-employers-200-response-spec res st/string-transformer)
        res))))


(defn-spec get-business-members-with-http-info any?
  "Get business members
  Get all members of the specified business. The return response will include the member's business_role and assets they have access to if assets_summary=TRUE"
  ([business_id string?, ] (get-business-members-with-http-info business_id nil))
  ([business_id string?, {:keys [fetch_system_users assets_summary business_roles member_ids start_index bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/members" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"fetch_system_users" fetch_system_users "assets_summary" assets_summary "business_roles" (with-collection-format business_roles :multi) "member_ids" member_ids "start_index" start_index "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-business-members get-business-employers-200-response-spec
  "Get business members
  Get all members of the specified business. The return response will include the member's business_role and assets they have access to if assets_summary=TRUE"
  ([business_id string?, ] (get-business-members business_id nil))
  ([business_id string?, optional-params any?]
   (let [res (:data (get-business-members-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-business-employers-200-response-spec res st/string-transformer)
        res))))


(defn-spec get-business-partners-with-http-info any?
...
- partner_type=EXTERNAL, the business assets returned are your partner's business assets the partner has granted you
  access to."
  ([business_id string?, ] (get-business-partners-with-http-info business_id nil))
  ([business_id string?, {:keys [assets_summary partner_type partner_ids start_index sort_ascending bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/partners" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"assets_summary" assets_summary "partner_type" partner_type "partner_ids" partner_ids "start_index" start_index "sort_ascending" sort_ascending "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-business-partners get-business-employers-200-response-spec
  "Get business partners
  Get all partners of the specified business.
...
- partner_type=EXTERNAL, the business assets returned are your partner's business assets the partner has granted you
  access to."
  ([business_id string?, ] (get-business-partners business_id nil))
  ([business_id string?, optional-params any?]
   (let [res (:data (get-business-partners-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-business-employers-200-response-spec res st/string-transformer)
        res))))


(defn-spec system-user-update-with-http-info any?
  "Update a system user information.
  Update a system user information such as name."
  [business_id string?, system_user_id string?, system-user-update-with-required-body system-user-update-with-required-body]
  (check-required-params business_id system_user_id system-user-update-with-required-body)
  (call-api "/businesses/{business_id}/system_users/{system_user_id}" :patch
            {:path-params   {"business_id" business_id "system_user_id" system_user_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    system-user-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec system-user-update any?
  "Update a system user information.
  Update a system user information such as name."
  [business_id string?, system_user_id string?, system-user-update-with-required-body system-user-update-with-required-body]
  (let [res (:data (system-user-update-with-http-info business_id system_user_id system-user-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))


(defn-spec update-business-memberships-with-http-info any?
  "Update member's business role
  Update a member's business role within the business."
  [business_id string?, business-membership-member (s/coll-of business-membership-member-spec)]
  (check-required-params business_id business-membership-member)
  (call-api "/businesses/{business_id}/members" :patch
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    business-membership-member
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-business-memberships update-business-memberships-response-spec
  "Update member's business role
  Update a member's business role within the business."
  [business_id string?, business-membership-member (s/coll-of business-membership-member-spec)]
  (let [res (:data (update-business-memberships-with-http-info business_id business-membership-member))]
    (if (:decode-models *api-context*)
       (st/decode update-business-memberships-response-spec res st/string-transformer)
       res)))
