...
(defn-spec brand-accounts-create-with-http-info any?
  ...
  [business_hierarchy_id string?, brand-accounts-create-request brand-accounts-create-request]
  (check-required-params business_hierarchy_id brand-accounts-create-request)
  (call-api "/business_access/business_hierarchy/{business_hierarchy_id}/brand_accounts" :post
            {:path-params   {"business_hierarchy_id" business_hierarchy_id }
             ...
             :query-params  {}
             :form-params   {}
             :body-param    brand-accounts-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec brand-accounts-create brand-accounts-create-200-response-spec
  "Create a Brand Account
  Create a Brand Account that will be a child business of a business hierarchy. Request must contain name, username, and country."
  [business_hierarchy_id string?, brand-accounts-create-request brand-accounts-create-request]
  (let [res (:data (brand-accounts-create-with-http-info business_hierarchy_id brand-accounts-create-request))]
    (if (:decode-models *api-context*)
       (st/decode brand-accounts-create-200-response-spec res st/string-transformer)
       res)))
...
(defn-spec brand-accounts-update-with-http-info any?
  ...
  [business_hierarchy_id string?, brand_account_id string?, brand-accounts-update-request brand-accounts-update-request]
  (check-required-params business_hierarchy_id brand_account_id brand-accounts-update-request)
  (call-api "/business_access/business_hierarchy/{business_hierarchy_id}/brand_accounts/{brand_account_id}" :patch
            {:path-params   {"business_hierarchy_id" business_hierarchy_id "brand_account_id" brand_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    brand-accounts-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec brand-accounts-update brand-accounts-create-200-response-spec
  "Update a Brand Account
  Update an existing Brand Account"
  [business_hierarchy_id string?, brand_account_id string?, brand-accounts-update-request brand-accounts-update-request]
  (let [res (:data (brand-accounts-update-with-http-info business_hierarchy_id brand_account_id brand-accounts-update-request))]
    (if (:decode-models *api-context*)
       (st/decode brand-accounts-create-200-response-spec res st/string-transformer)
       res)))
...
(defn-spec delete-business-membership-with-http-info any?
  ...
  [business_id string?, members-to-delete-body members-to-delete-body]
  (check-required-params business_id members-to-delete-body)
  (call-api "/businesses/{business_id}/members" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    members-to-delete-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec delete-business-membership deleted-members-response-spec
  "Terminate business memberships
  Terminate memberships between the specified members and your business."
  [business_id string?, members-to-delete-body members-to-delete-body]
  (let [res (:data (delete-business-membership-with-http-info business_id members-to-delete-body))]
    (if (:decode-models *api-context*)
       (st/decode deleted-members-response-spec res st/string-transformer)
       res)))
...
Note: You may only batch terminate partners of the same partner type."
  [business_id string?, delete-partners-request delete-partners-request]
  (check-required-params business_id delete-partners-request)
  (call-api "/businesses/{business_id}/partners" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    delete-partners-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec delete-business-partners delete-partners-response-spec
  "Terminate business partnerships
  Terminate partnerships between the specified partners and your business.
Note: You may only batch terminate partners of the same partner type."
  [business_id string?, delete-partners-request delete-partners-request]
  (let [res (:data (delete-business-partners-with-http-info business_id delete-partners-request))]
    (if (:decode-models *api-context*)
       (st/decode delete-partners-response-spec res st/string-transformer)
       res)))
...
(defn-spec get-business-employers-with-http-info any?
  ...
  ([{:keys [page_size bookmark]} (s/map-of keyword? any?)]
   (call-api "/businesses/employers" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
...
(defn-spec get-business-members get-business-members-200-response-spec
  "Get business members
  Get all members of the specified business.
The return response will include the member's business_role and assets they have access to if assets_summary=TRUE"
  ...
  ([business_id string?, optional-params any?]
   (let [res (:data (get-business-members-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-business-members-200-response-spec res st/string-transformer)
        res))))
...
- partner_type=EXTERNAL, the business assets returned are your partner's business assets the partner has granted you
  ...
  ([business_id string?, {:keys [assets_summary partner_type partner_ids start_index page_size bookmark]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/partners" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"assets_summary" assets_summary "partner_type" partner_type "partner_ids" partner_ids "start_index" start_index "page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-business-partners get-business-partners-200-response-spec
  "Get business partners
  Get all partners of the specified business.
...
- partner_type=EXTERNAL, the business assets returned are your partner's business assets the partner has granted you
  ...
  ([business_id string?, optional-params any?]
   (let [res (:data (get-business-partners-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-business-partners-200-response-spec res st/string-transformer)
        res))))
...
(defn-spec system-user-update-with-http-info any?
  ...
  [business_id string?, system_user_id string?, system-user-update-request system-user-update-request]
  (check-required-params business_id system_user_id system-user-update-request)
  (call-api "/businesses/{business_id}/system_users/{system_user_id}" :patch
            {:path-params   {"business_id" business_id "system_user_id" system_user_id }
             ...
             :query-params  {}
             :form-params   {}
             :body-param    system-user-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))
...
(defn-spec system-user-update any?
  ...
  Update a system user information such as name."
  [business_id string?, system_user_id string?, system-user-update-request system-user-update-request]
  (let [res (:data (system-user-update-with-http-info business_id system_user_id system-user-update-request))]
    (if (:decode-models *api-context*)
...
(defn-spec update-business-memberships-with-http-info any?
  ...
  [business_id string?, update-member-business-role-body (s/coll-of update-member-business-role-body-spec)]
  (check-required-params business_id update-member-business-role-body)
  (call-api "/businesses/{business_id}/members" :patch
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    update-member-business-role-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-business-memberships update-member-results-response-array-spec
  "Update member's business role
  Update a member's business role within the business."
  [business_id string?, update-member-business-role-body (s/coll-of update-member-business-role-body-spec)]
  (let [res (:data (update-business-memberships-with-http-info business_id update-member-business-role-body))]
    (if (:decode-models *api-context*)
       (st/decode update-member-results-response-array-spec res st/string-transformer)
       res)))
