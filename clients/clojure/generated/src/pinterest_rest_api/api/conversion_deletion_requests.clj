(ns pinterest-rest-api.api.conversion-deletion-requests
...
(defn-spec conversion-deletion-request-create-with-http-info any?
...
within a 30 days period, once the request is submitted to the deletion process it cannot be canceled."
  [ad_account_id string?, conversion-deletion-request-create conversion-deletion-request-create]
  (check-required-params ad_account_id conversion-deletion-request-create)
  (call-api "/ad_accounts/{ad_account_id}/conversion_deletion_requests" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    conversion-deletion-request-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))

(defn-spec conversion-deletion-request-create conversion-deletion-request-spec
  "Create a conversion deletion request
  **This endpoint is currently in beta and not available to all apps.**
[Learn more](/docs/key-concepts/using-beta-and-restricted-features/).
Create a request to delete conversion data for a list of user emails and/or EPIKs, limited to the specified `ad_account_id`.
...
within a 30 days period, once the request is submitted to the deletion process it cannot be canceled."
  [ad_account_id string?, conversion-deletion-request-create conversion-deletion-request-create]
  (let [res (:data (conversion-deletion-request-create-with-http-info ad_account_id conversion-deletion-request-create))]
    (if (:decode-models *api-context*)
       (st/decode conversion-deletion-request-spec res st/string-transformer)
       res)))


(defn-spec conversion-deletion-request-delete-with-http-info any?
...
This will cancel the request and prevent it from being processed. This can only be
  ...
  [request_id string?, ad_account_id string?]
  (check-required-params request_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/conversion_deletion_requests/{request_id}" :delete
            {:path-params   {"request_id" request_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec conversion-deletion-request-delete conversion-deletion-request-spec
  "Delete a conversion deletion request
  **This endpoint is currently in beta and not available to all apps.**
[Learn more](/docs/key-concepts/using-beta-and-restricted-features/).
Delete a conversion deletion request from `ad_account_id` with `request_id`.
This will cancel the request and prevent it from being processed. This can only be
  ...
  [request_id string?, ad_account_id string?]
  (let [res (:data (conversion-deletion-request-delete-with-http-info request_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode conversion-deletion-request-spec res st/string-transformer)
       res)))


(defn-spec conversion-deletion-request-get-with-http-info any?
...
Get a single conversion deletion request from `ad_account_id` with `request_id`."
  [request_id string?, ad_account_id string?]
  (check-required-params request_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/conversion_deletion_requests/{request_id}" :get
            {:path-params   {"request_id" request_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec conversion-deletion-request-get conversion-deletion-request-spec
  "Get a single conversion deletion request
  **This endpoint is currently in beta and not available to all apps.**
[Learn more](/docs/key-concepts/using-beta-and-restricted-features/).
Get a single conversion deletion request from `ad_account_id` with `request_id`."
  [request_id string?, ad_account_id string?]
  (let [res (:data (conversion-deletion-request-get-with-http-info request_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode conversion-deletion-request-spec res st/string-transformer)
       res)))


(defn-spec conversion-deletion-request-list-with-http-info any?
  "List conversion deletion requests
  **This endpoint is currently in beta and not available to all apps.**
[Learn more](/docs/key-concepts/using-beta-and-restricted-features/).
Get a list of the conversion deletion requests for the specified `ad_account_id`."
  ([ad_account_id string?, ] (conversion-deletion-request-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/conversion_deletion_requests" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
...
(defn-spec conversion-deletion-request-list conversion-deletion-request-list-200-response-spec
  "List conversion deletion requests
  **This endpoint is currently in beta and not available to all apps.**
[Learn more](/docs/key-concepts/using-beta-and-restricted-features/).
Get a list of the conversion deletion requests for the specified `ad_account_id`."
  ([ad_account_id string?, ] (conversion-deletion-request-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (conversion-deletion-request-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode conversion-deletion-request-list-200-response-spec res st/string-transformer)
        res))))
