...
(defn-spec audience-insights-get audience-insights-response-spec
  "Get audience insights
  Get Audience Insights for an ad account. The response will return insights for 3 types of audiences: the
ad account's engaged audience on Pinterest, the ad account's total audience on Pinterest and Pinterest's
total audience.<p/>
<a href=\"https://help.pinterest.com/en/business/article/audience-insights\" target=\"_blank\">Learn more about Audience Insights</a>."
  ...
  (let [res (:data (audience-insights-get-with-http-info ad_account_id audience_insight_type))]
    (if (:decode-models *api-context*)
       (st/decode audience-insights-response-spec res st/string-transformer)
       res)))
...
(defn-spec audience-insights-scope-and-type-get audience-definition-response-spec
  "Get audience insights scope and type
  Get the scope and type of available audiences, which along with a date, is an audience that has recently had an interaction (referred to here as a type) on pins. Interacted pins can belong to at least the most common **partner** or **Pinterest** scopes. This means that user interactions made on advertiser or partner pins will have the **partner** scope. You can also have user interactions performed in general on Pinterest with the **Pinterest** scope. In that case, you can then use the returned type and scope values together on requests to other endpoints to retrieve insight metrics for a desired audience."
  [ad_account_id string?]
  (let [res (:data (audience-insights-scope-and-type-get-with-http-info ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode audience-definition-response-spec res st/string-transformer)
       res)))
