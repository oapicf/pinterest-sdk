(ns pinterest-rest-api.api.lead-forms
...
(defn-spec lead-form-get-with-http-info any?
  "Get lead form by id
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [lead_form_id string?, ad_account_id string?]
  (check-required-params lead_form_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/lead_forms/{lead_form_id}" :get
            {:path-params   {"lead_form_id" lead_form_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec lead-form-get lead-form-spec
  "Get lead form by id
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [lead_form_id string?, ad_account_id string?]
  (let [res (:data (lead-form-get-with-http-info lead_form_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode lead-form-spec res st/string-transformer)
       res)))


(defn-spec lead-form-test-create-with-http-info any?
...
- List of answers should follow the questions creation order."
  [ad_account_id string?, lead_form_id string?, lead-form-test-create lead-form-test-create]
  (check-required-params ad_account_id lead_form_id lead-form-test-create)
  (call-api "/ad_accounts/{ad_account_id}/lead_forms/{lead_form_id}/test" :post
            {:path-params   {"ad_account_id" ad_account_id "lead_form_id" lead_form_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    lead-form-test-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec lead-form-test-create lead-form-test-spec
  "Create lead form test data
  Create lead form test data based on the list of answers provided as part of the body.
- List of answers should follow the questions creation order."
  [ad_account_id string?, lead_form_id string?, lead-form-test-create lead-form-test-create]
  (let [res (:data (lead-form-test-create-with-http-info ad_account_id lead_form_id lead-form-test-create))]
    (if (:decode-models *api-context*)
       (st/decode lead-form-test-spec res st/string-transformer)
       res)))


(defn-spec lead-forms-create-with-http-info any?
  "Create lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [ad_account_id string?, lead-form-create (s/coll-of lead-form-create-spec)]
  (check-required-params ad_account_id lead-form-create)
  (call-api "/ad_accounts/{ad_account_id}/lead_forms" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    lead-form-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec lead-forms-create lead-forms-create-200-response-spec
  "Create lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [ad_account_id string?, lead-form-create (s/coll-of lead-form-create-spec)]
  (let [res (:data (lead-forms-create-with-http-info ad_account_id lead-form-create))]
    (if (:decode-models *api-context*)
       (st/decode lead-forms-create-200-response-spec res st/string-transformer)
       res)))


(defn-spec lead-forms-list-with-http-info any?
  "List lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  ([ad_account_id string?, ] (lead-forms-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/lead_forms" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
...
(defn-spec lead-forms-list lead-forms-list-200-response-spec
  "List lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  ([ad_account_id string?, ] (lead-forms-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (lead-forms-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode lead-forms-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec lead-forms-update-with-http-info any?
  "Update lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [ad_account_id string?, lead-form-batch-update (s/coll-of lead-form-batch-update-spec)]
  (check-required-params ad_account_id lead-form-batch-update)
  (call-api "/ad_accounts/{ad_account_id}/lead_forms" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    lead-form-batch-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec lead-forms-update lead-forms-create-200-response-spec
  "Update lead forms
  **This feature is currently in beta and not available to all apps, if you're interested in joining the beta, please reach out to your Pinterest account manager.**
...
For more, see [Lead ads](https://help.pinterest.com/en/business/article/lead-ads)."
  [ad_account_id string?, lead-form-batch-update (s/coll-of lead-form-batch-update-spec)]
  (let [res (:data (lead-forms-update-with-http-info ad_account_id lead-form-batch-update))]
    (if (:decode-models *api-context*)
       (st/decode lead-forms-create-200-response-spec res st/string-transformer)
       res)))
