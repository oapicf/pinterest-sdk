(ns pinterest-rest-api.api.customer-lists
...
(defn-spec customer-lists-create-with-http-info any?
...
To use your customer list after creating it, convert it into a customer list audience by passing the `CUSTOMER_LIST` audience type at the [create audience endpoint](https://developer.pinterest.com/docs/api/v5/audiences-create)."
  [ad_account_id string?, customer-list-create customer-list-create]
  (check-required-params ad_account_id customer-list-create)
  (call-api "/ad_accounts/{ad_account_id}/customer_lists" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    customer-list-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec customer-lists-create customer-list-spec
  "Create customer lists
  Create a customer list from your records (hashed or plain-text email addresses, or hashed MAIDs or IDFAs).
...
To use your customer list after creating it, convert it into a customer list audience by passing the `CUSTOMER_LIST` audience type at the [create audience endpoint](https://developer.pinterest.com/docs/api/v5/audiences-create)."
  [ad_account_id string?, customer-list-create customer-list-create]
  (let [res (:data (customer-lists-create-with-http-info ad_account_id customer-list-create))]
    (if (:decode-models *api-context*)
       (st/decode customer-list-spec res st/string-transformer)
       res)))


(defn-spec customer-lists-get-with-http-info any?
  "Get customer list
  Gets a specific customer list given the customer list ID."
  [ad_account_id string?, customer_list_id string?]
  (check-required-params ad_account_id customer_list_id)
  (call-api "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}" :get
            {:path-params   {"ad_account_id" ad_account_id "customer_list_id" customer_list_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec customer-lists-get customer-list-spec
  "Get customer list
  Gets a specific customer list given the customer list ID."
  [ad_account_id string?, customer_list_id string?]
  (let [res (:data (customer-lists-get-with-http-info ad_account_id customer_list_id))]
    (if (:decode-models *api-context*)
       (st/decode customer-list-spec res st/string-transformer)
       res)))


(defn-spec customer-lists-list-with-http-info any?
  "Get customer lists
...
(Customer lists are a type of audience.) For more information, see [Audience targeting](https://help.pinterest.com/en/business/article/audience-targeting) or the [Audiences](/docs/api-features/targeting-overview/) section of the ads management guide."
  ([ad_account_id string?, ] (customer-lists-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order exclude_nca]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/customer_lists" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "exclude_nca" exclude_nca }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec customer-lists-list customer-lists-list-200-response-spec
  "Get customer lists
  Get a set of customer lists including id and name based on the filters provided.

(Customer lists are a type of audience.) For more information, see [Audience targeting](https://help.pinterest.com/en/business/article/audience-targeting) or the [Audiences](/docs/api-features/targeting-overview/) section of the ads management guide."
  ([ad_account_id string?, ] (customer-lists-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (customer-lists-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode customer-lists-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec customer-lists-update-with-http-info any?
...
For more information, see [Audience targeting](https://help.pinterest.com/en/business/article/audience-targeting) or the [Audiences](/docs/api-features/targeting-overview/) section of the ads management guide."
  [ad_account_id string?, customer_list_id string?, customer-list-update-with-required-body customer-list-update-with-required-body]
  (check-required-params ad_account_id customer_list_id customer-list-update-with-required-body)
  (call-api "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}" :patch
            {:path-params   {"ad_account_id" ad_account_id "customer_list_id" customer_list_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    customer-list-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec customer-lists-update customer-list-spec
  "Update customer list
  Append or remove records to/from an existing customer list. (A customer list is one of the four types of Pinterest audiences.)
...
For more information, see [Audience targeting](https://help.pinterest.com/en/business/article/audience-targeting) or the [Audiences](/docs/api-features/targeting-overview/) section of the ads management guide."
  [ad_account_id string?, customer_list_id string?, customer-list-update-with-required-body customer-list-update-with-required-body]
  (let [res (:data (customer-lists-update-with-http-info ad_account_id customer_list_id customer-list-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode customer-list-spec res st/string-transformer)
       res)))
