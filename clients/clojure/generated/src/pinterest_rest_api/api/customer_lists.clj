...
<p>To use your customer list after creating it, convert it into a customer list audience by passing the `CUSTOMER_LIST` audience type at the <a href=\"https://developer.pinterest.com/docs/api/v5/audiences-create\" target=\"blank\">create audience endpoint</a>.</p>"
  [ad_account_id string?, customer-list-request customer-list-request]
  (check-required-params ad_account_id customer-list-request)
  (call-api "/ad_accounts/{ad_account_id}/customer_lists" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    customer-list-request
...
<p>To use your customer list after creating it, convert it into a customer list audience by passing the `CUSTOMER_LIST` audience type at the <a href=\"https://developer.pinterest.com/docs/api/v5/audiences-create\" target=\"blank\">create audience endpoint</a>.</p>"
  [ad_account_id string?, customer-list-request customer-list-request]
  (let [res (:data (customer-lists-create-with-http-info ad_account_id customer-list-request))]
...
or the <a href=\"/docs/api-features/targeting-overview/\" target=\"_blank\">Audiences</a>
section of the ads management guide.</p>"
  [ad_account_id string?, customer_list_id string?, customer-list-update-request customer-list-update-request]
  (check-required-params ad_account_id customer_list_id customer-list-update-request)
  (call-api "/ad_accounts/{ad_account_id}/customer_lists/{customer_list_id}" :patch
            {:path-params   {"ad_account_id" ad_account_id "customer_list_id" customer_list_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    customer-list-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))
...
section of the ads management guide.</p>"
  [ad_account_id string?, customer_list_id string?, customer-list-update-request customer-list-update-request]
  (let [res (:data (customer-lists-update-with-http-info ad_account_id customer_list_id customer-list-update-request))]
    (if (:decode-models *api-context*)
       (st/decode customer-list-spec res st/string-transformer)
       res)))
