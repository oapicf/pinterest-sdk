...
- An [asset group](https://help.pinterest.com/en/business/article/asset-groups) is a custom group of assets based on how you would like to manage your accounts."
  [business_id string?, asset-group-input-create asset-group-input-create]
  (check-required-params business_id asset-group-input-create)
  (call-api "/businesses/{business_id}/asset_groups" :post
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    asset-group-input-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec asset-group-create asset-group-input-spec
  "Create a new asset group.
  Create a new asset group with the specified parameters.
- An [asset group](https://help.pinterest.com/en/business/article/asset-groups) is a custom group of assets based on how you would like to manage your accounts."
  [business_id string?, asset-group-input-create asset-group-input-create]
  (let [res (:data (asset-group-create-with-http-info business_id asset-group-input-create))]
    (if (:decode-models *api-context*)
       (st/decode asset-group-input-spec res st/string-transformer)
       res)))
...
(defn-spec asset-group-delete-with-http-info any?
  ...
  [business_id string?, asset-group-deletion-delete asset-group-deletion-delete]
  (check-required-params business_id asset-group-deletion-delete)
  (call-api "/businesses/{business_id}/asset_groups" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    asset-group-deletion-delete
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec asset-group-delete asset-group-deletion-spec
  "Delete asset groups.
  Delete a batch of asset groups."
  [business_id string?, asset-group-deletion-delete asset-group-deletion-delete]
  (let [res (:data (asset-group-delete-with-http-info business_id asset-group-deletion-delete))]
    (if (:decode-models *api-context*)
       (st/decode asset-group-deletion-spec res st/string-transformer)
       res)))
...
(defn-spec asset-group-update-with-http-info any?
  ...
  [business_id string?, asset-group-modification-read-or-update asset-group-modification-read-or-update]
  (check-required-params business_id asset-group-modification-read-or-update)
  (call-api "/businesses/{business_id}/asset_groups" :patch
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    asset-group-modification-read-or-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec asset-group-update asset-group-modification-spec
  "Update asset groups.
  Update a batch of asset groups with the specified parameters."
  [business_id string?, asset-group-modification-read-or-update asset-group-modification-read-or-update]
  (let [res (:data (asset-group-update-with-http-info business_id asset-group-modification-read-or-update))]
    (if (:decode-models *api-context*)
       (st/decode asset-group-modification-spec res st/string-transformer)
       res)))
...
(defn-spec business-asset-members-get-with-http-info any?
  ...
  ([business_id string?, asset_id string?, {:keys [start_index fetch_system_users bookmark page_size]} (s/map-of keyword? any?)]
   ...
   (call-api "/businesses/{business_id}/assets/{asset_id}/members" :get
             {:path-params   {"business_id" business_id "asset_id" asset_id }
              :header-params {}
              :query-params  {"start_index" start_index "fetch_system_users" fetch_system_users "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
...
(defn-spec business-asset-partners-get business-asset-members-get-200-response-spec
  "Get partners with access to asset
  Get all the partners the requesting business has granted access to on the given asset.
Note: If the asset has been shared with you, an empty array will be returned. This is because an asset shared with
you cannot be shared with a different partner."
...
The return response will include the permissions the member has to that asset and the asset type."
  ...
  ([business_id string?, member_id string?, {:keys [asset_type start_index sort_by sort_ascending search_by search_value asset_permission_type ad_account_statuses bookmark page_size]} (s/map-of keyword? any?)]
   ...
   (call-api "/businesses/{business_id}/members/{member_id}/assets" :get
             {:path-params   {"business_id" business_id "member_id" member_id }
              :header-params {}
              :query-params  {"asset_type" asset_type "start_index" start_index "sort_by" sort_by "sort_ascending" sort_ascending "search_by" search_by "search_value" search_value "asset_permission_type" asset_permission_type "ad_account_statuses" (with-collection-format ad_account_statuses :multi) "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec business-member-assets-get business-member-assets-get-response-spec
  "Get assets assigned to a member
  Get assets on which you assigned asset permissions to the given member. Can be used to:
- get all assets, regardless of asset type or
- get assets of one asset type by using the asset_type query.
The return response will include the permissions the member has to that asset and the asset type."
  ...
  ([business_id string?, member_id string?, optional-params any?]
   (let [res (:data (business-member-assets-get-with-http-info business_id member_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode business-member-assets-get-response-spec res st/string-transformer)
        res))))
...
(defn-spec business-members-asset-access-delete-with-http-info any?
  ...
  [business_id string?, business-members-asset-access-delete-body business-members-asset-access-delete-body]
  (check-required-params business_id business-members-asset-access-delete-body)
  (call-api "/businesses/{business_id}/members/assets/access" :delete
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    business-members-asset-access-delete-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))
...
(defn-spec business-members-asset-access-delete delete-member-access-results-response-array-spec
  ...
  Terminate multiple members' access to an asset."
  [business_id string?, business-members-asset-access-delete-body business-members-asset-access-delete-body]
  (let [res (:data (business-members-asset-access-delete-with-http-info business_id business-members-asset-access-delete-body))]
    (if (:decode-models *api-context*)
...
- partner_type=EXTERNAL, you will retrieve the partner's business assets that the partner has granted you access to."
  ...
  ([business_id string?, partner_id string?, {:keys [partner_type asset_type start_index sort_by sort_ascending search_by search_value bookmark page_size]} (s/map-of keyword? any?)]
   ...
   (call-api "/businesses/{business_id}/partners/{partner_id}/assets" :get
             {:path-params   {"business_id" business_id "partner_id" partner_id }
              :header-params {}
              :query-params  {"partner_type" partner_type "asset_type" asset_type "start_index" start_index "sort_by" sort_by "sort_ascending" sort_ascending "search_by" search_by "search_value" search_value "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
...
(defn-spec delete-partner-asset-access-handler-impl delete-partner-asset-access-results-response-array-spec
  "Delete partner access to asset
  Terminate multiple partners' access to an asset. If
- partner_type=INTERNAL: You will terminate a partner's asset access to your business assets.
- partner_type=EXTERNAL: You will terminate your own access to your partner's business assets."
  ...
  (let [res (:data (delete-partner-asset-access-handler-impl-with-http-info business_id delete-partner-asset-access-body))]
    (if (:decode-models *api-context*)
       (st/decode delete-partner-asset-access-results-response-array-spec res st/string-transformer)
       res)))
