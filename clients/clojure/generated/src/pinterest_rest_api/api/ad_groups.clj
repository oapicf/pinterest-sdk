(ns pinterest-rest-api.api.ad-groups
...
(defn-spec ad-groups-analytics-with-http-info any?
...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-groups-analytics-with-http-info start_date end_date ad_group_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time aggregate_report_rows reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date ad_group_ids columns granularity ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/ad_groups/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "ad_group_ids" (with-collection-format ad_group_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "aggregate_report_rows" aggregate_report_rows "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-groups-analytics (s/coll-of ad-groups-analytics-metrics-spec)
  "Get ad group analytics
  Get analytics for the specified ad groups in the specified `ad_account_id`, filtered by the specified options.
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-groups-analytics start_date end_date ad_group_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (ad-groups-analytics-with-http-info start_date end_date ad_group_ids columns granularity ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ad-groups-analytics-metrics-spec) res st/string-transformer)
        res))))


(defn-spec ad-groups-audience-sizing-with-http-info any?
  "Get audience sizing
...
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  [ad_account_id string?, ad-group-audience-sizing-create ad-group-audience-sizing-create]
  (check-required-params ad_account_id ad-group-audience-sizing-create)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/audience_sizing" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-audience-sizing-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))

(defn-spec ad-groups-audience-sizing ad-group-audience-sizing-spec
  "Get audience sizing
  Get potential audience size for an ad group with given targeting criteria.
Potential audience size estimates the number of people you may be able to reach per month with your campaign.
It is based on historical advertising data and the targeting criteria you select.
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  [ad_account_id string?, ad-group-audience-sizing-create ad-group-audience-sizing-create]
  (let [res (:data (ad-groups-audience-sizing-with-http-info ad_account_id ad-group-audience-sizing-create))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-audience-sizing-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-bid-floor-get-with-http-info any?
  "Get bid floors
...
For more on bid floors see [Set your bid](https://help.pinterest.com/en/business/article/set-your-bid)."
  [ad_account_id string?, bid-floor-create bid-floor-create]
  (check-required-params ad_account_id bid-floor-create)
  (call-api "/ad_accounts/{ad_account_id}/bid_floor" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    bid-floor-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))

(defn-spec ad-groups-bid-floor-get bid-floor-spec
  "Get bid floors
  List bid floors for your campaign configuration. Bid floors are given in microcurrency values based on the currency in the bid floor specification.
...
For more on bid floors see [Set your bid](https://help.pinterest.com/en/business/article/set-your-bid)."
  [ad_account_id string?, bid-floor-create bid-floor-create]
  (let [res (:data (ad-groups-bid-floor-get-with-http-info ad_account_id bid-floor-create))]
    (if (:decode-models *api-context*)
       (st/decode bid-floor-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-create-with-http-info any?
  "Create ad groups
...
- If the parent ad campaign has start and end times set, ad group start and end times must occur within the parent campaign schedule."
  [ad_account_id string?, ad-group-create-create (s/coll-of ad-group-create-create-spec)]
  (check-required-params ad_account_id ad-group-create-create)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-create-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-groups-create ad-groups-create-200-response-spec
  "Create ad groups
  Create multiple new ad groups. All ads in a given ad group will have the same budget, bid, run dates, targeting, and placement (search, browse, other).
...
- If the parent ad campaign has start and end times set, ad group start and end times must occur within the parent campaign schedule."
  [ad_account_id string?, ad-group-create-create (s/coll-of ad-group-create-create-spec)]
  (let [res (:data (ad-groups-create-with-http-info ad_account_id ad-group-create-create))]
    (if (:decode-models *api-context*)
       (st/decode ad-groups-create-200-response-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-dynamic-titles-download-csv-with-http-info any?
  ...
  [ad_account_id string?, ad_group_id string?]
  (check-required-params ad_account_id ad_group_id)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/{ad_group_id}/dynamic_titles/csv" :get
            {:path-params   {"ad_account_id" ad_account_id "ad_group_id" ad_group_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec ad-groups-dynamic-titles-download-csv dynamic-titles-download-csv-spec
  "Get dynamic titles CSV download URL
  Get a presigned S3 download URL for the dynamic titles review CSV. Returns 400 if titles have not been generated yet."
  [ad_account_id string?, ad_group_id string?]
  (let [res (:data (ad-groups-dynamic-titles-download-csv-with-http-info ad_account_id ad_group_id))]
    (if (:decode-models *api-context*)
       (st/decode dynamic-titles-download-csv-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-dynamic-titles-get-status-with-http-info any?
  ...
  [ad_account_id string?, ad_group_id string?]
  (check-required-params ad_account_id ad_group_id)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/{ad_group_id}/dynamic_titles/status" :get
            {:path-params   {"ad_account_id" ad_account_id "ad_group_id" ad_group_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec ad-groups-dynamic-titles-get-status dynamic-titles-get-status-spec
  "Get dynamic titles status
  Get dynamic titles generation status for an ad group, including whether titles are ready for review and counts of generated and reviewed titles."
  [ad_account_id string?, ad_group_id string?]
  (let [res (:data (ad-groups-dynamic-titles-get-status-with-http-info ad_account_id ad_group_id))]
    (if (:decode-models *api-context*)
       (st/decode dynamic-titles-get-status-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-dynamic-titles-get-upload-url-with-http-info any?
  ...
  [ad_account_id string?, ad_group_id string?]
  (check-required-params ad_account_id ad_group_id)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/{ad_group_id}/dynamic_titles/uploads" :get
            {:path-params   {"ad_account_id" ad_account_id "ad_group_id" ad_group_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec ad-groups-dynamic-titles-get-upload-url dynamic-titles-upload-url-spec
  "Get dynamic titles upload URL
  Get a presigned S3 upload URL for the dynamic titles review CSV and a request_id for submission."
  [ad_account_id string?, ad_group_id string?]
  (let [res (:data (ad-groups-dynamic-titles-get-upload-url-with-http-info ad_account_id ad_group_id))]
    (if (:decode-models *api-context*)
       (st/decode dynamic-titles-upload-url-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-dynamic-titles-process-csv-with-http-info any?
  ...
  [ad_account_id string?, ad_group_id string?, dynamic-titles-process-csv-create dynamic-titles-process-csv-create]
  (check-required-params ad_account_id ad_group_id dynamic-titles-process-csv-create)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/{ad_group_id}/dynamic_titles" :post
            {:path-params   {"ad_account_id" ad_account_id "ad_group_id" ad_group_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    dynamic-titles-process-csv-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-groups-dynamic-titles-process-csv dynamic-titles-process-csv-spec
  "Process dynamic titles CSV
  Validate and process the uploaded dynamic titles review CSV. Returns validation errors if the CSV is invalid."
  [ad_account_id string?, ad_group_id string?, dynamic-titles-process-csv-create dynamic-titles-process-csv-create]
  (let [res (:data (ad-groups-dynamic-titles-process-csv-with-http-info ad_account_id ad_group_id dynamic-titles-process-csv-create))]
    (if (:decode-models *api-context*)
       (st/decode dynamic-titles-process-csv-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-get-with-http-info any?
  "Get ad group
  Get a specific ad group given the ad group ID."
  [ad_group_id string?, ad_account_id string?]
  (check-required-params ad_group_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/{ad_group_id}" :get
            {:path-params   {"ad_group_id" ad_group_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec ad-groups-get ad-group-spec
  "Get ad group
  Get a specific ad group given the ad group ID."
  [ad_group_id string?, ad_account_id string?]
  (let [res (:data (ad-groups-get-with-http-info ad_group_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-spec res st/string-transformer)
       res)))


(defn-spec ad-groups-list-with-http-info any?
  "List ad groups
...
**Note:** Provide only campaign_id or ad_group_id. Do not provide both."
  ([ad_account_id string?, ] (ad-groups-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order campaign_ids ad_group_ids entity_statuses translate_interests_to_names]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/ad_groups" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "campaign_ids" (with-collection-format campaign_ids :multi) "ad_group_ids" (with-collection-format ad_group_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "translate_interests_to_names" translate_interests_to_names }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-groups-list ad-groups-list-200-response-spec
  "List ad groups
  List ad groups based on provided campaign IDs or ad group IDs.(campaign_ids or ad_group_ids).
**Note:** Provide only campaign_id or ad_group_id. Do not provide both."
  ([ad_account_id string?, ] (ad-groups-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (ad-groups-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode ad-groups-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec ad-groups-targeting-analytics-get-with-http-info any?
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-groups-targeting-analytics-get-with-http-info ad_account_id ad_group_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone sort_columns sort_ascending]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id ad_group_ids start_date end_date targeting_types columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/ad_groups/targeting_analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"ad_group_ids" (with-collection-format ad_group_ids :multi) "start_date" start_date "end_date" end_date "targeting_types" (with-collection-format targeting_types :csv) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "attribution_types" (with-collection-format attribution_types :csv) "reporting_timezone" reporting_timezone "sort_columns" (with-collection-format sort_columns :multi) "sort_ascending" sort_ascending }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-groups-targeting-analytics-get metrics-response-spec
  "Get targeting analytics for ad groups
  Get targeting analytics for one or more ad groups. For the requested ad group(s) and metrics, the response will include the requested metric information (e.g. SPEND_IN_DOLLAR) for the requested target type (e.g. \"age_bucket\") for applicable values (e.g. \"45-49\").
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-groups-targeting-analytics-get ad_account_id ad_group_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, optional-params any?]
   (let [res (:data (ad-groups-targeting-analytics-get-with-http-info ad_account_id ad_group_ids start_date end_date targeting_types columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode metrics-response-spec res st/string-transformer)
        res))))


(defn-spec ad-groups-update-with-http-info any?
  "Update ad groups
  Update multiple existing ad groups."
  [ad_account_id string?, ad-group-update-batch-update (s/coll-of ad-group-update-batch-update-spec)]
  (check-required-params ad_account_id ad-group-update-batch-update)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-update-batch-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-groups-update ad-groups-create-200-response-spec
  "Update ad groups
  Update multiple existing ad groups."
  [ad_account_id string?, ad-group-update-batch-update (s/coll-of ad-group-update-batch-update-spec)]
  (let [res (:data (ad-groups-update-with-http-info ad_account_id ad-group-update-batch-update))]
    (if (:decode-models *api-context*)
       (st/decode ad-groups-create-200-response-spec res st/string-transformer)
       res)))


(defn-spec get-ad-groups-by-promotion-ids-list-with-http-info any?
  "List of ad groups using promotions IDs.
  Get a list of ad groups that are associated with those promotion ids"
  ([ad_account_id string?, promotion_ids (s/coll-of string?), ] (get-ad-groups-by-promotion-ids-list-with-http-info ad_account_id promotion_ids nil))
  ([ad_account_id string?, promotion_ids (s/coll-of string?), {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id promotion_ids)
   (call-api "/ad_accounts/{ad_account_id}/promotion_applied_entities" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "promotion_ids" (with-collection-format promotion_ids :multi) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec get-ad-groups-by-promotion-ids-list ad-groups-list-200-response-spec
  "List of ad groups using promotions IDs.
  Get a list of ad groups that are associated with those promotion ids"
  ([ad_account_id string?, promotion_ids (s/coll-of string?), ] (get-ad-groups-by-promotion-ids-list ad_account_id promotion_ids nil))
  ([ad_account_id string?, promotion_ids (s/coll-of string?), optional-params any?]
   (let [res (:data (get-ad-groups-by-promotion-ids-list-with-http-info ad_account_id promotion_ids optional-params))]
     (if (:decode-models *api-context*)
        (st/decode ad-groups-list-200-response-spec res st/string-transformer)
        res))))
