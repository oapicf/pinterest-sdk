...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (ad-groups-analytics-with-http-info ad_account_id start_date end_date ad_group_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time aggregate_report_rows reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id start_date end_date ad_group_ids columns granularity)
...
(defn-spec ad-groups-analytics (s/coll-of ad-groups-analytics-response-inner-spec)
  "Get ad group analytics
  Get analytics for the specified ad groups in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (ad-groups-analytics ad_account_id start_date end_date ad_group_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (ad-groups-analytics-with-http-info ad_account_id start_date end_date ad_group_ids columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ad-groups-analytics-response-inner-spec) res st/string-transformer)
        res))))
...
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  [ad_account_id string?, ad-group-audience-sizing-request ad-group-audience-sizing-request]
  (check-required-params ad_account_id ad-group-audience-sizing-request)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups/audience_sizing" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-audience-sizing-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))

(defn-spec ad-groups-audience-sizing ad-group-audience-sizing-response-spec
  "Get audience sizing
  Get potential audience size for an ad group with given targeting criteria. 
Potential audience size estimates the number of people you may be able to reach per month with your campaign. 
It is based on historical advertising data and the targeting criteria you select.
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  [ad_account_id string?, ad-group-audience-sizing-request ad-group-audience-sizing-request]
  (let [res (:data (ad-groups-audience-sizing-with-http-info ad_account_id ad-group-audience-sizing-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-audience-sizing-response-spec res st/string-transformer)
       res)))
...
For more on bid floors see <a class=\"reference external\" href=\"https://help.pinterest.com/en/business/article/set-your-bid\"> Set your bid</a>."
  [ad_account_id string?, bid-floor-request bid-floor-request]
  (check-required-params ad_account_id bid-floor-request)
  (call-api "/ad_accounts/{ad_account_id}/bid_floor" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    bid-floor-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2" "client_credentials"]}))
...
For more on bid floors see <a class=\"reference external\" href=\"https://help.pinterest.com/en/business/article/set-your-bid\"> Set your bid</a>."
  [ad_account_id string?, bid-floor-request bid-floor-request]
  (let [res (:data (ad-groups-bid-floor-get-with-http-info ad_account_id bid-floor-request))]
    (if (:decode-models *api-context*)
...
- If the parent ad campaign has start and end times set, ad group start and end times must occur within the parent campaign schedule."
  [ad_account_id string?, ad-group-create-request (s/coll-of ad-group-create-request-spec)]
  (check-required-params ad_account_id ad-group-create-request)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-groups-create ad-group-array-response-spec
  "Create ad groups
  Create multiple new ad groups. All ads in a given ad group will have the same budget, bid, run dates, targeting, and placement (search, browse, other). For more information, <a href=\"https://help.pinterest.com/en/business/article/campaign-structure\" target=\"_blank\"> click here</a>.
<strong>Notes:</strong>
- `bid_in_micro_currency` and `budget_in_micro_currency` should be expressed in microcurrency amounts based on the currency field set in the advertiser's profile.<p/>
...
- If the parent ad campaign has start and end times set, ad group start and end times must occur within the parent campaign schedule."
  [ad_account_id string?, ad-group-create-request (s/coll-of ad-group-create-request-spec)]
  (let [res (:data (ad-groups-create-with-http-info ad_account_id ad-group-create-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-array-response-spec res st/string-transformer)
       res)))
...
(defn-spec ad-groups-get ad-group-response-spec
  "Get ad group
  Get a specific ad group given the ad group ID."
  [ad_account_id string?, ad_group_id string?]
  (let [res (:data (ad-groups-get-with-http-info ad_account_id ad_group_id))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-response-spec res st/string-transformer)
       res)))
...
Provide only campaign_id or ad_group_id. Do not provide both."
  ...
  ([ad_account_id string?, {:keys [campaign_ids ad_group_ids entity_statuses page_size order bookmark translate_interests_to_names]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/ad_groups" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "ad_group_ids" (with-collection-format ad_group_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "page_size" page_size "order" order "bookmark" bookmark "translate_interests_to_names" translate_interests_to_names }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (ad-groups-targeting-analytics-get-with-http-info ad_account_id ad_group_ids start_date end_date targeting_types columns granularity nil))
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (ad-groups-targeting-analytics-get ad_account_id ad_group_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-ad-group-targeting-type-spec), columns (s/coll-of string?), granularity granularity, optional-params any?]
...
(defn-spec ad-groups-update-with-http-info any?
  ...
  [ad_account_id string?, ad-group-update-request (s/coll-of ad-group-update-request-spec)]
  (check-required-params ad_account_id ad-group-update-request)
  (call-api "/ad_accounts/{ad_account_id}/ad_groups" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-group-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec ad-groups-update ad-group-array-response-spec
  "Update ad groups
  Update multiple existing ad groups."
  [ad_account_id string?, ad-group-update-request (s/coll-of ad-group-update-request-spec)]
  (let [res (:data (ad-groups-update-with-http-info ad_account_id ad-group-update-request))]
    (if (:decode-models *api-context*)
       (st/decode ad-group-array-response-spec res st/string-transformer)
       res)))
