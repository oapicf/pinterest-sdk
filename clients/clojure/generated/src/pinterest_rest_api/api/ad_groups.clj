...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ...
  ([ad_account_id string?, start_date inst?, end_date inst?, ad_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time]} (s/map-of keyword? any?)]
   ...
   (call-api "/ad_accounts/{ad_account_id}/ad_groups/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "ad_group_ids" (with-collection-format ad_group_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time }
              :form-params   {}
              :content-types []
...
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  ([ad_account_id string?, ] (ad-groups-audience-sizing-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [ad-group-audience-sizing-request]} (s/map-of keyword? any?)]
...
It is based on historical advertising data and the targeting criteria you select.
It does not guarantee results or take into account factors such as bid, budget, schedule, seasonality or product experiments."
  ([ad_account_id string?, ] (ad-groups-audience-sizing ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (ad-groups-audience-sizing-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
...
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (ad-groups-targeting-analytics-get-with-http-info ad_account_id ad_group_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-targeting-type-spec), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types]} (s/map-of keyword? any?)]
...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ...
  ([ad_account_id string?, ad_group_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-targeting-type-spec), columns (s/coll-of string?), granularity granularity, optional-params any?]
