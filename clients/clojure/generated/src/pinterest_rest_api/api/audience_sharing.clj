...
(defn-spec ad-accounts-audiences-shared-accounts-list-with-http-info any?
  ...
  ([ad_account_id string?, audience_id string?, account_type audience-account-type, ] (ad-accounts-audiences-shared-accounts-list-with-http-info ad_account_id audience_id account_type nil))
  ([ad_account_id string?, audience_id string?, account_type audience-account-type, {:keys [page_size bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id audience_id account_type)
   (call-api "/ad_accounts/{ad_account_id}/audiences/shared/accounts" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"audience_id" audience_id "account_type" account_type "page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
...
(defn-spec ad-accounts-audiences-shared-accounts-list ad-accounts-audiences-shared-accounts-list-200-response-spec
  "List accounts with access to an audience owned by an ad account
  List all ad accounts and/or businesses that have access to a specific audience. The audience must be owned by the requesting ad account."
  ([ad_account_id string?, audience_id string?, account_type audience-account-type, ] (ad-accounts-audiences-shared-accounts-list ad_account_id audience_id account_type nil))
  ([ad_account_id string?, audience_id string?, account_type audience-account-type, optional-params any?]
   (let [res (:data (ad-accounts-audiences-shared-accounts-list-with-http-info ad_account_id audience_id account_type optional-params))]
...
If the requesting business is not the owner of the audience, only ad accounts owned by the requesting business will be returned."
  ...
  ([business_id string?, audience_id string?, account_type audience-account-type, {:keys [page_size bookmark]} (s/map-of keyword? any?)]
...
(defn-spec shared-audiences-for-business-list-with-http-info any?
  ...
  ([business_id string?, {:keys [bookmark order page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/audiences" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "order" order "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec shared-audiences-for-business-list audiences-list-200-response-spec
  "List received audiences for a business
  Get a list of received audiences for the given business."
...
(defn-spec update-ad-account-to-ad-account-shared-audience-with-http-info any?
  ...
  [ad_account_id string?, shared-audience shared-audience]
  (check-required-params ad_account_id shared-audience)
  (call-api "/ad_accounts/{ad_account_id}/audiences/ad_accounts/shared" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    shared-audience
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-ad-account-to-ad-account-shared-audience shared-audience-response-spec
  "Update audience sharing between ad accounts
  From an ad account, share a specific audience with another ad account, or revoke access to a previously shared audience. Only the audience owner account can share the audience. The recipient ad account(s) must be in the same <a href='https://help.pinterest.com/en/business/article/create-and-manage-accounts'>Pinterest Business Hierarchy</a> as the business owner of the ad account.<br> This endpoint is not available to all apps.<a href='/docs/getting-started/using-beta-and-restricted-features/'>Learn more</a>."
  [ad_account_id string?, shared-audience shared-audience]
  (let [res (:data (update-ad-account-to-ad-account-shared-audience-with-http-info ad_account_id shared-audience))]
    (if (:decode-models *api-context*)
       (st/decode shared-audience-response-spec res st/string-transformer)
       res)))
...
(defn-spec update-ad-account-to-business-shared-audience-with-http-info any?
  ...
  [ad_account_id string?, business-shared-audience business-shared-audience]
  (check-required-params ad_account_id business-shared-audience)
  (call-api "/ad_accounts/{ad_account_id}/audiences/businesses/shared" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    business-shared-audience
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-ad-account-to-business-shared-audience business-shared-audience-response-spec
  "Update audience sharing from an ad account to businesses
  From an ad account, share a specific audience with a business account, or revoke access to a previously shared audience. Only the audience owner account can share the audience. The recipient business account must be in the same business hierarchy as the business owner of the ad account.<br> This endpoint is not available to all apps.<a href='/docs/getting-started/using-beta-and-restricted-features/'>Learn more</a>."
  [ad_account_id string?, business-shared-audience business-shared-audience]
  (let [res (:data (update-ad-account-to-business-shared-audience-with-http-info ad_account_id business-shared-audience))]
    (if (:decode-models *api-context*)
       (st/decode business-shared-audience-response-spec res st/string-transformer)
       res)))
...
(defn-spec update-business-to-ad-account-shared-audience-with-http-info any?
  ...
  [business_id string?, shared-audience shared-audience]
  (check-required-params business_id shared-audience)
...
(defn-spec update-business-to-ad-account-shared-audience shared-audience-response-spec
  "Update audience sharing from a business to ad accounts
  From a business, share a specific audience with other ad account(s), or revoke access to a previously shared audience. <ul> <li>If the business is the owner of the audience, it can share with any ad account within the same business hierarchy.</li> <li>If the business is the recipient of the audience, it can share with any of its owned ad accounts.</li> </ul> This endpoint is not available to all apps.<a href='/docs/getting-started/using-beta-and-restricted-features/'>Learn more</a>."
  [business_id string?, shared-audience shared-audience]
  (let [res (:data (update-business-to-ad-account-shared-audience-with-http-info business_id shared-audience))]
    (if (:decode-models *api-context*)
       (st/decode shared-audience-response-spec res st/string-transformer)
       res)))
...
(defn-spec update-business-to-business-shared-audience-with-http-info any?
  ...
  [business_id string?, business-shared-audience business-shared-audience]
  (check-required-params business_id business-shared-audience)
...
(defn-spec update-business-to-business-shared-audience business-shared-audience-response-spec
  "Update audience sharing between businesses
  From a business, share a specific audience with another business account, or revoke access to a previously shared audience. Only the audience owner can share the audience with other businesses, and the recipient business must be within the same business hierarchy.<br> This endpoint is not available to all apps.<a href='/docs/getting-started/using-beta-and-restricted-features/'>Learn more</a>."
  [business_id string?, business-shared-audience business-shared-audience]
  (let [res (:data (update-business-to-business-shared-audience-with-http-info business_id business-shared-audience))]
    (if (:decode-models *api-context*)
       (st/decode business-shared-audience-response-spec res st/string-transformer)
       res)))
