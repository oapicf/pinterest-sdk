(ns pinterest-rest-api.api.audience-sharing
...
(defn-spec ad-accounts-audiences-shared-accounts-list-with-http-info any?
  "List accounts with access to an audience owned by an ad account
  List all ad accounts and/or businesses that have access to a specific audience. The audience must be owned by the requesting ad account."
  ([audience_id string?, account_type audience-account-type, ad_account_id string?, ] (ad-accounts-audiences-shared-accounts-list-with-http-info audience_id account_type ad_account_id nil))
  ([audience_id string?, account_type audience-account-type, ad_account_id string?, {:keys [bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params audience_id account_type ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/audiences/shared/accounts" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"audience_id" audience_id "account_type" account_type "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-accounts-audiences-shared-accounts-list ad-accounts-audiences-shared-accounts-list-200-response-spec
  "List accounts with access to an audience owned by an ad account
  List all ad accounts and/or businesses that have access to a specific audience. The audience must be owned by the requesting ad account."
  ([audience_id string?, account_type audience-account-type, ad_account_id string?, ] (ad-accounts-audiences-shared-accounts-list audience_id account_type ad_account_id nil))
  ([audience_id string?, account_type audience-account-type, ad_account_id string?, optional-params any?]
   (let [res (:data (ad-accounts-audiences-shared-accounts-list-with-http-info audience_id account_type ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode ad-accounts-audiences-shared-accounts-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec business-account-audiences-shared-accounts-list-with-http-info any?
  "List accounts with access to an audience owned by a business
  List all ad accounts and/or businesses that have access to a specific audience. The audience must either be owned by an ad account in the requesting business, or it must have been shared with the requesting business. If the requesting business is not the owner of the audience, only ad accounts owned by the requesting business will be returned."
  ([business_id string?, audience_id string?, account_type audience-account-type, ] (business-account-audiences-shared-accounts-list-with-http-info business_id audience_id account_type nil))
  ([business_id string?, audience_id string?, account_type audience-account-type, {:keys [bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id audience_id account_type)
   (call-api "/businesses/{business_id}/audiences/shared/accounts" :get
...
(defn-spec business-account-audiences-shared-accounts-list ad-accounts-audiences-shared-accounts-list-200-response-spec
  "List accounts with access to an audience owned by a business
  List all ad accounts and/or businesses that have access to a specific audience. The audience must either be owned by an ad account in the requesting business, or it must have been shared with the requesting business. If the requesting business is not the owner of the audience, only ad accounts owned by the requesting business will be returned."
  ([business_id string?, audience_id string?, account_type audience-account-type, ] (business-account-audiences-shared-accounts-list business_id audience_id account_type nil))
  ([business_id string?, audience_id string?, account_type audience-account-type, optional-params any?]
   (let [res (:data (business-account-audiences-shared-accounts-list-with-http-info business_id audience_id account_type optional-params))]
     (if (:decode-models *api-context*)
        (st/decode ad-accounts-audiences-shared-accounts-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec shared-audiences-for-business-list-with-http-info any?
  "List received audiences for a business
  Get a list of received audiences for the given business."
  ([business_id string?, ] (shared-audiences-for-business-list-with-http-info business_id nil))
  ([business_id string?, {:keys [order bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params business_id)
   (call-api "/businesses/{business_id}/audiences" :get
             {:path-params   {"business_id" business_id }
              :header-params {}
              :query-params  {"order" order "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec shared-audiences-for-business-list shared-audiences-for-business-list-200-response-spec
  "List received audiences for a business
  Get a list of received audiences for the given business."
  ([business_id string?, ] (shared-audiences-for-business-list business_id nil))
  ([business_id string?, optional-params any?]
   (let [res (:data (shared-audiences-for-business-list-with-http-info business_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode shared-audiences-for-business-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec update-ad-account-to-ad-account-shared-audience-with-http-info any?
...
This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [ad_account_id string?, ad-account-to-ad-account-shared-audience-update-with-required-body ad-account-to-ad-account-shared-audience-update-with-required-body]
  (check-required-params ad_account_id ad-account-to-ad-account-shared-audience-update-with-required-body)
  (call-api "/ad_accounts/{ad_account_id}/audiences/ad_accounts/shared" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-account-to-ad-account-shared-audience-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-ad-account-to-ad-account-shared-audience ad-account-to-ad-account-shared-audience-spec
  "Update audience sharing between ad accounts
  From an ad account, share a specific audience with another ad account, or revoke access to a previously shared audience. Only the audience owner account can share the audience. The recipient ad account(s) must be in the same [Pinterest Business Hierarchy](https://help.pinterest.com/en/business/article/create-and-manage-accounts) as the business owner of the ad account.

This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [ad_account_id string?, ad-account-to-ad-account-shared-audience-update-with-required-body ad-account-to-ad-account-shared-audience-update-with-required-body]
  (let [res (:data (update-ad-account-to-ad-account-shared-audience-with-http-info ad_account_id ad-account-to-ad-account-shared-audience-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode ad-account-to-ad-account-shared-audience-spec res st/string-transformer)
       res)))


(defn-spec update-ad-account-to-business-shared-audience-with-http-info any?
...
This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [ad_account_id string?, ad-account-to-business-shared-audience-update-with-required-body ad-account-to-business-shared-audience-update-with-required-body]
  (check-required-params ad_account_id ad-account-to-business-shared-audience-update-with-required-body)
  (call-api "/ad_accounts/{ad_account_id}/audiences/businesses/shared" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-account-to-business-shared-audience-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-ad-account-to-business-shared-audience ad-account-to-business-shared-audience-spec
  "Update audience sharing from an ad account to businesses
  From an ad account, share a specific audience with a business account, or revoke access to a previously shared audience. Only the audience owner account can share the audience. The recipient business account must be in the same business hierarchy as the business owner of the ad account.

This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [ad_account_id string?, ad-account-to-business-shared-audience-update-with-required-body ad-account-to-business-shared-audience-update-with-required-body]
  (let [res (:data (update-ad-account-to-business-shared-audience-with-http-info ad_account_id ad-account-to-business-shared-audience-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode ad-account-to-business-shared-audience-spec res st/string-transformer)
       res)))


(defn-spec update-business-to-ad-account-shared-audience-with-http-info any?
...
This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [business_id string?, business-to-ad-account-shared-audience-update-with-required-body business-to-ad-account-shared-audience-update-with-required-body]
  (check-required-params business_id business-to-ad-account-shared-audience-update-with-required-body)
  (call-api "/businesses/{business_id}/audiences/ad_accounts/shared" :patch
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    business-to-ad-account-shared-audience-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-business-to-ad-account-shared-audience business-to-ad-account-shared-audience-spec
  "Update audience sharing from a business to ad accounts
  From a business, share a specific audience with other ad account(s), or revoke access to a previously shared audience.
...
This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [business_id string?, business-to-ad-account-shared-audience-update-with-required-body business-to-ad-account-shared-audience-update-with-required-body]
  (let [res (:data (update-business-to-ad-account-shared-audience-with-http-info business_id business-to-ad-account-shared-audience-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode business-to-ad-account-shared-audience-spec res st/string-transformer)
       res)))


(defn-spec update-business-to-business-shared-audience-with-http-info any?
...
This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [business_id string?, business-to-business-shared-audience-update-with-required-body business-to-business-shared-audience-update-with-required-body]
  (check-required-params business_id business-to-business-shared-audience-update-with-required-body)
  (call-api "/businesses/{business_id}/audiences/businesses/shared" :patch
            {:path-params   {"business_id" business_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    business-to-business-shared-audience-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec update-business-to-business-shared-audience business-to-business-shared-audience-spec
  "Update audience sharing between businesses
  From a business, share a specific audience with another business account, or revoke access to a previously shared audience. Only the audience owner can share the audience with other businesses, and the recipient business must be within the same business hierarchy.

This endpoint is not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/)."
  [business_id string?, business-to-business-shared-audience-update-with-required-body business-to-business-shared-audience-update-with-required-body]
  (let [res (:data (update-business-to-business-shared-audience-with-http-info business_id business-to-business-shared-audience-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode business-to-business-shared-audience-spec res st/string-transformer)
       res)))
