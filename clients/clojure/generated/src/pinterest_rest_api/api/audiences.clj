...
(defn-spec audiences-create-custom-with-http-info any?
  "Create custom audience
  Create a custom audience and find the audiences you want your ads to reach."
  [ad_account_id string?, audience-create-custom-request audience-create-custom-request]
  (check-required-params ad_account_id audience-create-custom-request)
  (call-api "/ad_accounts/{ad_account_id}/audiences/custom" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    audience-create-custom-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec audiences-create-custom audience-spec
  "Create custom audience
  Create a custom audience and find the audiences you want your ads to reach."
  [ad_account_id string?, audience-create-custom-request audience-create-custom-request]
  (let [res (:data (audiences-create-custom-with-http-info ad_account_id audience-create-custom-request))]
    (if (:decode-models *api-context*)
       (st/decode audience-spec res st/string-transformer)
       res)))
...
(defn-spec audiences-update-with-http-info any?
  ...
  ([ad_account_id string?, audience_id string?, ] (audiences-update-with-http-info ad_account_id audience_id nil))
  ([ad_account_id string?, audience_id string?, {:keys [audience-update-request]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id audience_id)
...
(defn-spec audiences-update audience-spec
  "Update audience
  Update (edit or remove) an existing targeting audience."
  ([ad_account_id string?, audience_id string?, ] (audiences-update ad_account_id audience_id nil))
  ([ad_account_id string?, audience_id string?, optional-params any?]
   (let [res (:data (audiences-update-with-http-info ad_account_id audience_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode audience-spec res st/string-transformer)
        res))))
