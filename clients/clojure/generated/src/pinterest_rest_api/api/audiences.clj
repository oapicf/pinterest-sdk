...
Learn about <a href=\"/docs/work-with-targets-and-audiences/create-audiences/\" target=\"_blank\">creating different kinds of audiences</a>."
  [ad_account_id string?, audience-create-request audience-create-request]
  (check-required-params ad_account_id audience-create-request)
  (call-api "/ad_accounts/{ad_account_id}/audiences" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    audience-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec audiences-create audience-spec
  "Create audience
  Create an audience you can use in targeting for specific ad groups. Targeting combines customer information with
the ways users interact with Pinterest to help you reach specific groups of users; you can include or exclude
specific `audience_ids` when you create an ad group. <p/>
Learn about <a href=\"/docs/work-with-targets-and-audiences/create-audiences/\" target=\"_blank\">creating different kinds of audiences</a>."
  [ad_account_id string?, audience-create-request audience-create-request]
  (let [res (:data (audiences-create-with-http-info ad_account_id audience-create-request))]
    (if (:decode-models *api-context*)
       (st/decode audience-spec res st/string-transformer)
       res)))
...
(defn-spec audiences-get-with-http-info any?
  ...
  [ad_account_id string?, audience_id string?]
  (check-required-params ad_account_id audience_id)
  (call-api "/ad_accounts/{ad_account_id}/audiences/{audience_id}" :get
            {:path-params   {"ad_account_id" ad_account_id "audience_id" audience_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec audiences-get audience-spec
  "Get audience
  Get a specific audience given the audience ID."
  [ad_account_id string?, audience_id string?]
  (let [res (:data (audiences-get-with-http-info ad_account_id audience_id))]
    (if (:decode-models *api-context*)
       (st/decode audience-spec res st/string-transformer)
       res)))
...
(defn-spec audiences-list-with-http-info any?
  ...
  ([ad_account_id string?, {:keys [bookmark order page_size ownership_type]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/audiences" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "order" order "page_size" page_size "ownership_type" ownership_type }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
(defn-spec audiences-update-with-http-info any?
  ...
  [ad_account_id string?, audience_id string?, audience-update-request audience-update-request]
  (check-required-params ad_account_id audience_id audience-update-request)
  (call-api "/ad_accounts/{ad_account_id}/audiences/{audience_id}" :patch
            {:path-params   {"ad_account_id" ad_account_id "audience_id" audience_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    audience-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec audiences-update audience-spec
  "Update audience
  Update (edit or remove) an existing targeting audience."
  [ad_account_id string?, audience_id string?, audience-update-request audience-update-request]
  (let [res (:data (audiences-update-with-http-info ad_account_id audience_id audience-update-request))]
    (if (:decode-models *api-context*)
       (st/decode audience-spec res st/string-transformer)
       res)))
