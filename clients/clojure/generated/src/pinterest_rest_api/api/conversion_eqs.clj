(ns pinterest-rest-api.api.conversion-eqs
...
(defn-spec conversion-eqs-list-with-http-info any?
...
[Event Quality Score](https://help.pinterest.com/en/business/article/eqs) indicates how effective the customer information and event insights (metadata) passed with your web, app and offline conversion events may be at matching to a Pinterest user."
  ([lookback_period lookback-period-options, ad_account_id string?, ] (conversion-eqs-list-with-http-info lookback_period ad_account_id nil))
  ([lookback_period lookback-period-options, ad_account_id string?, {:keys [source_platform ingestion_source]} (s/map-of keyword? any?)]
   (check-required-params lookback_period ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/conversion_eqs" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"lookback_period" lookback_period "source_platform" source_platform "ingestion_source" ingestion_source }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec conversion-eqs-list (s/coll-of event-quality-score-spec)
  "Get event quality score (EQS)
  Get the Event Quality Score (EQS) of your conversion signals.

[Event Quality Score](https://help.pinterest.com/en/business/article/eqs) indicates how effective the customer information and event insights (metadata) passed with your web, app and offline conversion events may be at matching to a Pinterest user."
  ([lookback_period lookback-period-options, ad_account_id string?, ] (conversion-eqs-list lookback_period ad_account_id nil))
  ([lookback_period lookback-period-options, ad_account_id string?, optional-params any?]
   (let [res (:data (conversion-eqs-list-with-http-info lookback_period ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of event-quality-score-spec) res st/string-transformer)
        res))))
