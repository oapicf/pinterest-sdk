...
(defn-spec boards-user-follows-list-with-http-info any?
  ...
  ([{:keys [bookmark page_size explicit_following ad_account_id]} (s/map-of keyword? any?)]
   (call-api "/user_account/following/boards" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "explicit_following" explicit_following "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec boards-user-follows-list boards-user-follows-list-200-response-spec
  "List following boards
  Get a list of the boards a user follows. The request returns a board summary object array."
  ...
  ([optional-params any?]
   (let [res (:data (boards-user-follows-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode boards-user-follows-list-200-response-spec res st/string-transformer)
        res))))
...
Use this request, as a signed-in user, to follow another user."
  [username string?, follow-user-request follow-user-request]
  (check-required-params username follow-user-request)
  (call-api "/user_account/following/{username}" :post
            {:path-params   {"username" username }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    follow-user-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec follow-user-update user-summary-spec
  "Follow user
  <strong>This endpoint is currently in beta and not available to all apps. <a href='/docs/getting-started/using-beta-and-restricted-features/'>Learn more</a>.</strong>

Use this request, as a signed-in user, to follow another user."
  [username string?, follow-user-request follow-user-request]
  (let [res (:data (follow-user-update-with-http-info username follow-user-request))]
    (if (:decode-models *api-context*)
       (st/decode user-summary-spec res st/string-transformer)
       res)))
...
(defn-spec unverify-website-delete any?
  "Unverify website
  Unverifu a website verified by the signed-in user."
  [website string?]
...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by string?, ] (user-account-analytics-top-pins-with-http-info start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by string?, {:keys [from_claimed_content pin_format app_types content_type source metric_types num_of_pins created_in_last_n_days ad_account_id]} (s/map-of keyword? any?)]
...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by string?, ] (user-account-analytics-top-pins start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by string?, optional-params any?]
...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by string?, ] (user-account-analytics-top-video-pins-with-http-info start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by string?, {:keys [from_claimed_content pin_format app_types content_type source metric_types num_of_pins created_in_last_n_days ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date sort_by)
...
Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by string?, ] (user-account-analytics-top-video-pins start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by string?, optional-params any?]
...
(defn-spec user-following-get-with-http-info any?
  ...
  ([{:keys [bookmark page_size feed_type explicit_following ad_account_id]} (s/map-of keyword? any?)]
   (call-api "/user_account/following" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "feed_type" feed_type "explicit_following" explicit_following "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec user-following-get user-following-get-200-response-spec
  "List following
  Get a list of who a certain user follows."
  ...
  ([optional-params any?]
   (let [res (:data (user-following-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-following-get-200-response-spec res st/string-transformer)
        res))))
...
(defn-spec verify-website-update-with-http-info any?
  ...
  ([user-website-verify-request user-website-verify-request, ] (verify-website-update-with-http-info user-website-verify-request nil))
  ([user-website-verify-request user-website-verify-request, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params user-website-verify-request)
   (call-api "/user_account/websites" :post
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    user-website-verify-request
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec verify-website-update user-website-summary-spec
  "Verify website
  Verify a website as a signed-in user."
  ([user-website-verify-request user-website-verify-request, ] (verify-website-update user-website-verify-request nil))
  ([user-website-verify-request user-website-verify-request, optional-params any?]
   (let [res (:data (verify-website-update-with-http-info user-website-verify-request optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-website-summary-spec res st/string-transformer)
        res))))
...
(defn-spec website-verification-get user-website-verification-code-spec
  "Get user verification code for website claiming
  Get verification code for user to install on the website to claim it."
  ...
  ([optional-params any?]
   (let [res (:data (website-verification-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-website-verification-code-spec res st/string-transformer)
        res))))
