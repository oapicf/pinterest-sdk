(ns pinterest-rest-api.api.user-account
...
(defn-spec boards-user-follows-list-with-http-info any?
  "List following boards
  Get a list of the boards a user follows. The request returns a board summary object array."
  ([] (boards-user-follows-list-with-http-info nil))
  ([{:keys [ad_account_id explicit_following bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/user_account/following/boards" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id "explicit_following" explicit_following "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec boards-user-follows-list boards-list-200-response-spec
  "List following boards
  Get a list of the boards a user follows. The request returns a board summary object array."
  ([] (boards-user-follows-list nil))
  ([optional-params any?]
   (let [res (:data (boards-user-follows-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode boards-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec follow-user-update-with-http-info any?
  "Follow user
...
Use this request, as a signed-in user, to follow another user."
  [username string?, follow-user-create follow-user-create]
  (check-required-params username follow-user-create)
  (call-api "/user_account/following/{username}" :post
            {:path-params   {"username" username }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    follow-user-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec follow-user-update follow-user-spec
  "Follow user
  **This endpoint is currently in beta and not available to all apps. [Learn more](/docs/getting-started/using-beta-and-restricted-features/).**

Use this request, as a signed-in user, to follow another user."
  [username string?, follow-user-create follow-user-create]
  (let [res (:data (follow-user-update-with-http-info username follow-user-create))]
    (if (:decode-models *api-context*)
       (st/decode follow-user-spec res st/string-transformer)
       res)))


(defn-spec followers-list-with-http-info any?
  "List followers
  Get a list of your followers."
  ([] (followers-list-with-http-info nil))
  ([{:keys [bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/user_account/followers" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec followers-list followers-list-200-response-spec
  "List followers
  Get a list of your followers."
  ([] (followers-list nil))
  ([optional-params any?]
   (let [res (:data (followers-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode followers-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec linked-business-accounts-get-with-http-info any?
  "List linked businesses
  Get a list of your linked business accounts."
  []
  (call-api "/user_account/businesses" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec linked-business-accounts-get (s/coll-of linked-business-spec)
  "List linked businesses
  Get a list of your linked business accounts."
  []
  (let [res (:data (linked-business-accounts-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of linked-business-spec) res st/string-transformer)
       res)))


(defn-spec unverify-website-delete-with-http-info any?
  "Unverify website
  Unverify a website verified by the signed-in user."
  [website string?]
  (check-required-params website)
  (call-api "/user_account/websites" :delete
            {:path-params   {}
             :header-params {}
             :query-params  {"website" website }
             :form-params   {}
             :content-types []
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec unverify-website-delete user-website-spec
  "Unverify website
  Unverify a website verified by the signed-in user."
  [website string?]
  (let [res (:data (unverify-website-delete-with-http-info website))]
    (if (:decode-models *api-context*)
       (st/decode user-website-spec res st/string-transformer)
       res)))


(defn-spec user-account-analytics-with-http-info any?
  "Get user account analytics
  Get analytics for the \"operation user_account\"
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, ] (user-account-analytics-with-http-info start_date end_date nil))
  ([start_date inst?, end_date inst?, {:keys [from_claimed_content pin_format app_types content_type source metric_types split_field ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date)
   (call-api "/user_account/analytics" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "from_claimed_content" from_claimed_content "pin_format" pin_format "app_types" app_types "content_type" content_type "source" source "metric_types" (with-collection-format metric_types :csv) "split_field" split_field "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec user-account-analytics (s/map-of string? analytics-metrics-response-spec)
  "Get user account analytics
  Get analytics for the \"operation user_account\"
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, ] (user-account-analytics start_date end_date nil))
  ([start_date inst?, end_date inst?, optional-params any?]
   (let [res (:data (user-account-analytics-with-http-info start_date end_date optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/map-of string? analytics-metrics-response-spec) res st/string-transformer)
        res))))


(defn-spec user-account-analytics-top-pins-with-http-info any?
  "Get user account top pins analytics
  Gets analytics data about a user's top pins (limited to the top 50).
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by top-pins-sort-by, ] (user-account-analytics-top-pins-with-http-info start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by top-pins-sort-by, {:keys [from_claimed_content pin_format app_types content_type source metric_types num_of_pins created_in_last_n_days ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date sort_by)
   (call-api "/user_account/analytics/top_pins" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "sort_by" sort_by "from_claimed_content" from_claimed_content "pin_format" pin_format "app_types" app_types "content_type" content_type "source" source "metric_types" (with-collection-format metric_types :csv) "num_of_pins" num_of_pins "created_in_last_n_days" created_in_last_n_days "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec user-account-analytics-top-pins top-pins-analytics-response-spec
  "Get user account top pins analytics
  Gets analytics data about a user's top pins (limited to the top 50).
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by top-pins-sort-by, ] (user-account-analytics-top-pins start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by top-pins-sort-by, optional-params any?]
   (let [res (:data (user-account-analytics-top-pins-with-http-info start_date end_date sort_by optional-params))]
     (if (:decode-models *api-context*)
        (st/decode top-pins-analytics-response-spec res st/string-transformer)
        res))))


(defn-spec user-account-analytics-top-video-pins-with-http-info any?
  "Get user account top video pins analytics
  Gets analytics data about a user's top video pins (limited to the top 50).
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by top-video-pins-sort-by, ] (user-account-analytics-top-video-pins-with-http-info start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by top-video-pins-sort-by, {:keys [from_claimed_content pin_format app_types content_type source metric_types num_of_pins created_in_last_n_days ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date sort_by)
   (call-api "/user_account/analytics/top_video_pins" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "sort_by" sort_by "from_claimed_content" from_claimed_content "pin_format" pin_format "app_types" app_types "content_type" content_type "source" source "metric_types" (with-collection-format metric_types :csv) "num_of_pins" num_of_pins "created_in_last_n_days" created_in_last_n_days "ad_account_id" ad_account_id }
              :form-params   {}
...
(defn-spec user-account-analytics-top-video-pins top-video-pins-analytics-response-spec
  "Get user account top video pins analytics
  Gets analytics data about a user's top video pins (limited to the top 50).
- By default, the \"operation user_account\" is the token user_account.

Optional: Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\"."
  ([start_date inst?, end_date inst?, sort_by top-video-pins-sort-by, ] (user-account-analytics-top-video-pins start_date end_date sort_by nil))
  ([start_date inst?, end_date inst?, sort_by top-video-pins-sort-by, optional-params any?]
   (let [res (:data (user-account-analytics-top-video-pins-with-http-info start_date end_date sort_by optional-params))]
     (if (:decode-models *api-context*)
        (st/decode top-video-pins-analytics-response-spec res st/string-transformer)
        res))))


(defn-spec user-account-followed-interests-with-http-info any?
  "List following interests
  Get a list of a user's following interests in one place."
  ([username string?, ] (user-account-followed-interests-with-http-info username nil))
  ([username string?, {:keys [bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params username)
   (call-api "/users/{username}/interests/follow" :get
             {:path-params   {"username" username }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size }
              :form-params   {}
...
(defn-spec user-account-followed-interests user-account-followed-interests-200-response-spec
  "List following interests
  Get a list of a user's following interests in one place."
  ([username string?, ] (user-account-followed-interests username nil))
  ([username string?, optional-params any?]
   (let [res (:data (user-account-followed-interests-with-http-info username optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-account-followed-interests-200-response-spec res st/string-transformer)
        res))))


(defn-spec user-account-get-with-http-info any?
  "Get user account
...
If using Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\". See [Understanding Business Access] for more information."
  ([] (user-account-get-with-http-info nil))
  ([{:keys [ad_account_id]} (s/map-of keyword? any?)]
   (call-api "/user_account" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec user-account-get account-spec
  "Get user account
  Get account information for the \"operation user_account\"
- By default, the \"operation user_account\" is the token user_account.
...
[Understanding Business Access]: https://developers.pinterest.com/docs/getting-started/using-business-access/ \"Understanding Business Access\"
If using Business Access: Specify an ad_account_id to use the owner of that ad_account as the \"operation user_account\". See [Understanding Business Access] for more information."
  ([] (user-account-get nil))
  ([optional-params any?]
   (let [res (:data (user-account-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode account-spec res st/string-transformer)
        res))))


(defn-spec user-following-get-with-http-info any?
  "List following
  Get a list of who a certain user follows."
  ([] (user-following-get-with-http-info nil))
  ([{:keys [ad_account_id explicit_following feed_type bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/user_account/following" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id "explicit_following" explicit_following "feed_type" feed_type "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec user-following-get followers-list-200-response-spec
  "List following
  Get a list of who a certain user follows."
  ([] (user-following-get nil))
  ([optional-params any?]
   (let [res (:data (user-following-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode followers-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec user-websites-get-with-http-info any?
  "Get user websites
  Get user websites, claimed or not"
  ([] (user-websites-get-with-http-info nil))
  ([{:keys [bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/user_account/websites" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size }
              :form-params   {}
...
(defn-spec user-websites-get user-websites-get-200-response-spec
  "Get user websites
  Get user websites, claimed or not"
  ([] (user-websites-get nil))
  ([optional-params any?]
   (let [res (:data (user-websites-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-websites-get-200-response-spec res st/string-transformer)
        res))))


(defn-spec verify-website-update-with-http-info any?
  "Verify website
  Verify a website as a signed-in user."
  ([user-website-create user-website-create, ] (verify-website-update-with-http-info user-website-create nil))
  ([user-website-create user-website-create, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params user-website-create)
   (call-api "/user_account/websites" :post
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    user-website-create
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec verify-website-update user-website-spec
  "Verify website
  Verify a website as a signed-in user."
  ([user-website-create user-website-create, ] (verify-website-update user-website-create nil))
  ([user-website-create user-website-create, optional-params any?]
   (let [res (:data (verify-website-update-with-http-info user-website-create optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-website-spec res st/string-transformer)
        res))))


(defn-spec website-verification-get-with-http-info any?
  "Get user verification code for website claiming
  Get verification code for user to install on the website to claim it."
  ([] (website-verification-get-with-http-info nil))
  ([{:keys [ad_account_id]} (s/map-of keyword? any?)]
   (call-api "/user_account/websites/verification" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
...
(defn-spec website-verification-get user-website-verification-spec
  "Get user verification code for website claiming
  Get verification code for user to install on the website to claim it."
  ([] (website-verification-get nil))
  ([optional-params any?]
   (let [res (:data (website-verification-get-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-website-verification-spec res st/string-transformer)
        res))))
