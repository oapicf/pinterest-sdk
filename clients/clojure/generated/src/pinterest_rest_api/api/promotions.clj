(ns pinterest-rest-api.api.promotions
...
(defn-spec promotions-create-with-http-info any?
  "Create promotions
  Create multiple new promotions."
  [ad_account_id string?, promotion-create (s/coll-of promotion-create-spec)]
  (check-required-params ad_account_id promotion-create)
  (call-api "/ad_accounts/{ad_account_id}/promotions" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    promotion-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec promotions-create promotions-response-spec
  "Create promotions
  Create multiple new promotions."
  [ad_account_id string?, promotion-create (s/coll-of promotion-create-spec)]
  (let [res (:data (promotions-create-with-http-info ad_account_id promotion-create))]
    (if (:decode-models *api-context*)
       (st/decode promotions-response-spec res st/string-transformer)
       res)))


(defn-spec promotions-delete-with-http-info any?
  "Delete promotion by id
  Delete a promotion within Pinterest."
  [promotion_id string?, ad_account_id string?]
  (check-required-params promotion_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/promotions/{promotion_id}" :delete
            {:path-params   {"promotion_id" promotion_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec promotions-delete promotion-spec
  "Delete promotion by id
  Delete a promotion within Pinterest."
  [promotion_id string?, ad_account_id string?]
  (let [res (:data (promotions-delete-with-http-info promotion_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode promotion-spec res st/string-transformer)
       res)))


(defn-spec promotions-get-with-http-info any?
  "Get promotion by id
  Get a promotion by its Pinterest-specific id. It must be associated with the provided ad account id."
  [promotion_id string?, ad_account_id string?]
  (check-required-params promotion_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/promotions/{promotion_id}" :get
            {:path-params   {"promotion_id" promotion_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec promotions-get promotion-spec
  "Get promotion by id
  Get a promotion by its Pinterest-specific id. It must be associated with the provided ad account id."
  [promotion_id string?, ad_account_id string?]
  (let [res (:data (promotions-get-with-http-info promotion_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode promotion-spec res st/string-transformer)
       res)))


(defn-spec promotions-list-with-http-info any?
  "Get promotions
...
promotions imported from a commerce integration."
  ([ad_account_id string?, ] (promotions-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/promotions" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
...
(defn-spec promotions-list promotions-list-200-response-spec
  "Get promotions
  Gets all promotions associated with an ad account ID that can be
applied to an ad group. Can be either internally-saved promotions or external
promotions imported from a commerce integration."
  ([ad_account_id string?, ] (promotions-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (promotions-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode promotions-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec promotions-update-with-http-info any?
  "Update promotions
  Update multiple promotions."
  [ad_account_id string?, promotion-batch-update (s/coll-of promotion-batch-update-spec)]
  (check-required-params ad_account_id promotion-batch-update)
  (call-api "/ad_accounts/{ad_account_id}/promotions" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    promotion-batch-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec promotions-update promotions-response-spec
  "Update promotions
  Update multiple promotions."
  [ad_account_id string?, promotion-batch-update (s/coll-of promotion-batch-update-spec)]
  (let [res (:data (promotions-update-with-http-info ad_account_id promotion-batch-update))]
    (if (:decode-models *api-context*)
       (st/decode promotions-response-spec res st/string-transformer)
       res)))
