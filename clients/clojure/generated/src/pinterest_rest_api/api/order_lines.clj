(ns pinterest-rest-api.api.order-lines
...
(defn-spec order-lines-get-with-http-info any?
  "Get order line
  Get a specific existing order line associated with an ad account."
  [order_line_id string?, ad_account_id string?]
  (check-required-params order_line_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/order_lines/{order_line_id}" :get
            {:path-params   {"order_line_id" order_line_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec order-lines-get order-line-spec
  "Get order line
  Get a specific existing order line associated with an ad account."
  [order_line_id string?, ad_account_id string?]
  (let [res (:data (order-lines-get-with-http-info order_line_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode order-line-spec res st/string-transformer)
       res)))


(defn-spec order-lines-list-with-http-info any?
  "Get order lines.
  List existing order lines associated with an ad account."
  ([ad_account_id string?, ] (order-lines-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/order_lines" :get
...
(defn-spec order-lines-list order-lines-list-200-response-spec
  "Get order lines.
  List existing order lines associated with an ad account."
  ([ad_account_id string?, ] (order-lines-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (order-lines-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode order-lines-list-200-response-spec res st/string-transformer)
        res))))
