...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days. Data will not be provided for conversion metrics but will be available for non-conversion metrics."
  ([campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-pins-analytics-with-http-info campaign_id pin_ids start_date end_date columns granularity ad_account_id nil))
  ([campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time]} (s/map-of keyword? any?)]
   (check-required-params campaign_id pin_ids start_date end_date columns granularity ad_account_id)
...
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days. Data will not be provided for conversion metrics but will be available for non-conversion metrics."
  ([campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-pins-analytics campaign_id pin_ids start_date end_date columns granularity ad_account_id nil))
  ([campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (ad-pins-analytics-with-http-info campaign_id pin_ids start_date end_date columns granularity ad_account_id optional-params))]
...
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (campaign-targeting-analytics-get-with-http-info ad_account_id campaign_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone]} (s/map-of keyword? any?)]
...
* If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (campaign-targeting-analytics-get ad_account_id campaign_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, optional-params any?]
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (campaigns-analytics-with-http-info start_date end_date campaign_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time aggregate_report_rows reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date campaign_ids columns granularity ad_account_id)
...
(defn-spec campaigns-analytics (s/coll-of campaigns-analytics-metrics-spec)
  "Get campaign analytics
  Get analytics for the specified campaigns in the specified `ad_account_id`, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via [Business Access](https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts): Admin, Analyst, Campaign Manager.
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (campaigns-analytics start_date end_date campaign_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (campaigns-analytics-with-http-info start_date end_date campaign_ids columns granularity ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of campaigns-analytics-metrics-spec) res st/string-transformer)
        res))))
...
- To convert microdollars to dollars, divide microdollars by 1,000,000"
  [ad_account_id string?, campaign-create-item (s/coll-of campaign-create-item-spec)]
  (check-required-params ad_account_id campaign-create-item)
  (call-api "/ad_accounts/{ad_account_id}/campaigns" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-create-item
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaigns-create campaign-batch-write-response-model-spec
  "Create campaigns
  Create multiple new campaigns. Every campaign has its own campaign_id
and houses one or more ad groups, which contain one or more ads.
...
- To convert microdollars to dollars, divide microdollars by 1,000,000"
  [ad_account_id string?, campaign-create-item (s/coll-of campaign-create-item-spec)]
  (let [res (:data (campaigns-create-with-http-info ad_account_id campaign-create-item))]
    (if (:decode-models *api-context*)
       (st/decode campaign-batch-write-response-model-spec res st/string-transformer)
       res)))
...
(defn-spec campaigns-get-with-http-info any?
  ...
  (check-required-params campaign_id ad_account_id)
  (call-api "/ad_accounts/{ad_account_id}/campaigns/{campaign_id}" :get
            {:path-params   {"campaign_id" campaign_id "ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec campaigns-get campaign-spec
  "Get campaign
  Get a specific campaign given the campaign ID."
  [campaign_id string?, ad_account_id string?]
  (let [res (:data (campaigns-get-with-http-info campaign_id ad_account_id))]
    (if (:decode-models *api-context*)
       (st/decode campaign-spec res st/string-transformer)
       res)))
...
roles granted to them via [Business Access](https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts): Admin, Analyst, Campaign Manager."
  ...
  ([ad_account_id string?, {:keys [bookmark page_size order campaign_ids entity_statuses]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/campaigns" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "campaign_ids" (with-collection-format campaign_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
- To convert dollars to microdollars, multiply dollars by 1,000,000
- To convert microdollars to dollars, divide microdollars by 1,000,000"
  [ad_account_id string?, campaign-batch-update-item (s/coll-of campaign-batch-update-item-spec)]
  (check-required-params ad_account_id campaign-batch-update-item)
  (call-api "/ad_accounts/{ad_account_id}/campaigns" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-batch-update-item
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaigns-update campaign-batch-write-response-model-spec
  "Update campaigns
  Update multiple ad campaigns based on campaign_ids.
...
- To convert microdollars to dollars, divide microdollars by 1,000,000"
  [ad_account_id string?, campaign-batch-update-item (s/coll-of campaign-batch-update-item-spec)]
  (let [res (:data (campaigns-update-with-http-info ad_account_id campaign-batch-update-item))]
    (if (:decode-models *api-context*)
       (st/decode campaign-batch-write-response-model-spec res st/string-transformer)
       res)))


(defn-spec get-campaign-delivery-estimates-with-http-info any?
...
**This endpoint is currently in beta and is not available to all apps [Learn more](/docs/new/about-beta-access/).**"
  [ad_account_id string?, campaign-delivery-estimates-campaign (s/coll-of campaign-delivery-estimates-campaign-spec)]
  (check-required-params ad_account_id campaign-delivery-estimates-campaign)
  (call-api "/ad_accounts/{ad_account_id}/campaigns/delivery_estimates" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-delivery-estimates-campaign
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec get-campaign-delivery-estimates campaign-delivery-estimates-response-spec
  "Get campaign delivery estimates
  Get delivery estimates for an ads campaign

**This endpoint is currently in beta and is not available to all apps [Learn more](/docs/new/about-beta-access/).**"
  [ad_account_id string?, campaign-delivery-estimates-campaign (s/coll-of campaign-delivery-estimates-campaign-spec)]
  (let [res (:data (get-campaign-delivery-estimates-with-http-info ad_account_id campaign-delivery-estimates-campaign))]
    (if (:decode-models *api-context*)
       (st/decode campaign-delivery-estimates-response-spec res st/string-transformer)
       res)))
