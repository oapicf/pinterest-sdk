...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days."
  ...
  ([ad_account_id string?, start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time]} (s/map-of keyword? any?)]
   ...
   (call-api "/ad_accounts/{ad_account_id}/campaigns/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "campaign_ids" (with-collection-format campaign_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
