(ns pinterest-rest-api.api.campaigns
...
(defn-spec ad-pins-analytics-with-http-info any?
  "Get pins analytics
...
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days. Data will not be provided for conversion metrics but will be available for non-conversion metrics."
  ([ad_account_id string?, campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, ] (ad-pins-analytics-with-http-info ad_account_id campaign_id pin_ids start_date end_date columns granularity nil))
  ([ad_account_id string?, campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id campaign_id pin_ids start_date end_date columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/pins/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_id" campaign_id "pin_ids" (with-collection-format pin_ids :multi) "start_date" start_date "end_date" end_date "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-pins-analytics (s/coll-of ad-pin-analytics-spec)
  "Get pins analytics
  Get analytics for the pins given a campaign and pins in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
- If granularity is not HOUR, the furthest back you can are allowed to pull data is 90 days before the current date in UTC time and the max time range supported is 90 days.
- If granularity is HOUR, the furthest back you can are allowed to pull data is 8 days before the current date in UTC time and the max time range supported is 3 days. Data will not be provided for conversion metrics but will be available for non-conversion metrics."
  ([ad_account_id string?, campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, ] (ad-pins-analytics ad_account_id campaign_id pin_ids start_date end_date columns granularity nil))
  ([ad_account_id string?, campaign_id string?, pin_ids (s/coll-of string?), start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (ad-pins-analytics-with-http-info ad_account_id campaign_id pin_ids start_date end_date columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ad-pin-analytics-spec) res st/string-transformer)
        res))))


(defn-spec campaign-targeting-analytics-get-with-http-info any?
...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (campaign-targeting-analytics-get-with-http-info ad_account_id campaign_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id campaign_ids start_date end_date targeting_types columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/campaigns/targeting_analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "start_date" start_date "end_date" end_date "targeting_types" (with-collection-format targeting_types :csv) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "attribution_types" (with-collection-format attribution_types :csv) "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec campaign-targeting-analytics-get metrics-response-spec
  "Get targeting analytics for campaigns
  Get targeting analytics for one or more campaigns.
For the requested account and metrics, the response will include the requested metric information
(e.g. SPEND_IN_DOLLAR) for the requested target type (e.g. \"age_bucket\") for applicable values (e.g. \"45-49\"). <p/>
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of string?), granularity granularity, ] (campaign-targeting-analytics-get ad_account_id campaign_ids start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, campaign_ids (s/coll-of string?), start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-campaign-targeting-type-spec), columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (campaign-targeting-analytics-get-with-http-info ad_account_id campaign_ids start_date end_date targeting_types columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode metrics-response-spec res st/string-transformer)
        res))))


(defn-spec campaigns-analytics-with-http-info any?
  "Get campaign analytics
  Get analytics for the specified campaigns in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (campaigns-analytics-with-http-info ad_account_id start_date end_date campaign_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time aggregate_report_rows reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id start_date end_date campaign_ids columns granularity)
   (call-api "/ad_accounts/{ad_account_id}/campaigns/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "campaign_ids" (with-collection-format campaign_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "aggregate_report_rows" aggregate_report_rows "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec campaigns-analytics (s/coll-of campaigns-analytics-response-inner-spec)
  "Get campaign analytics
  Get analytics for the specified campaigns in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (campaigns-analytics ad_account_id start_date end_date campaign_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, campaign_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (campaigns-analytics-with-http-info ad_account_id start_date end_date campaign_ids columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of campaigns-analytics-response-inner-spec) res st/string-transformer)
        res))))


(defn-spec campaigns-create-with-http-info any?
  "Create campaigns
...
<ul>
  ...
  [ad_account_id string?, campaign-create-request (s/coll-of campaign-create-request-spec)]
  (check-required-params ad_account_id campaign-create-request)
  (call-api "/ad_accounts/{ad_account_id}/campaigns" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaigns-create campaign-create-response-spec
  "Create campaigns
  Create multiple new campaigns. Every campaign has its own campaign_id and houses one or more ad groups, which contain one or more ads.
For more, see <a href=\"https://help.pinterest.com/en/business/article/set-up-your-campaign/\">Set up your campaign</a>. <p/>
<strong>Note:</strong>
...
<ul>
  ...
  [ad_account_id string?, campaign-create-request (s/coll-of campaign-create-request-spec)]
  (let [res (:data (campaigns-create-with-http-info ad_account_id campaign-create-request))]
    (if (:decode-models *api-context*)
       (st/decode campaign-create-response-spec res st/string-transformer)
       res)))


(defn-spec campaigns-get-with-http-info any?
  "Get campaign
  Get a specific campaign given the campaign ID."
  [ad_account_id string?, campaign_id string?]
  (check-required-params ad_account_id campaign_id)
  (call-api "/ad_accounts/{ad_account_id}/campaigns/{campaign_id}" :get
            {:path-params   {"ad_account_id" ad_account_id "campaign_id" campaign_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec campaigns-get campaign-response-spec
  "Get campaign
  Get a specific campaign given the campaign ID."
  [ad_account_id string?, campaign_id string?]
  (let [res (:data (campaigns-get-with-http-info ad_account_id campaign_id))]
    (if (:decode-models *api-context*)
       (st/decode campaign-response-spec res st/string-transformer)
       res)))


(defn-spec campaigns-list-with-http-info any?
  "List campaigns
  Get a list of the campaigns in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager."
  ([ad_account_id string?, ] (campaigns-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [campaign_ids entity_statuses page_size order bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/campaigns" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "page_size" page_size "order" order "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec campaigns-list campaigns-list-200-response-spec
  "List campaigns
  Get a list of the campaigns in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager."
  ([ad_account_id string?, ] (campaigns-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (campaigns-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode campaigns-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec campaigns-update-with-http-info any?
  "Update campaigns
...
<ul>
  ...
  [ad_account_id string?, campaign-update-request (s/coll-of campaign-update-request-spec)]
  (check-required-params ad_account_id campaign-update-request)
  (call-api "/ad_accounts/{ad_account_id}/campaigns" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    campaign-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec campaigns-update campaign-update-response-spec
  "Update campaigns
  <p>Update multiple ad campaigns based on campaign_ids. </p>
<p><strong>Note:</strong></p>
- <p>The values for `lifetime_spend_cap` and `daily_spend_cap` are microcurrency amounts based on the currency field set in the advertiser's profile. (e.g. USD) <p/>
...
<ul>
  ...
  [ad_account_id string?, campaign-update-request (s/coll-of campaign-update-request-spec)]
  (let [res (:data (campaigns-update-with-http-info ad_account_id campaign-update-request))]
    (if (:decode-models *api-context*)
       (st/decode campaign-update-response-spec res st/string-transformer)
       res)))
