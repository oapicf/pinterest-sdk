...
(defn-spec product-group-promotions-create-with-http-info any?
  ...
  [ad_account_id string?, product-group-promotion-create-request product-group-promotion-create-request]
  (check-required-params ad_account_id product-group-promotion-create-request)
  (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    product-group-promotion-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec product-group-promotions-create product-group-promotion-response-spec
  "Create product group promotions
  Add one or more product groups from your catalog to an existing ad group. (Product groups added to an ad group are a 'product group promotion.')"
  [ad_account_id string?, product-group-promotion-create-request product-group-promotion-create-request]
  (let [res (:data (product-group-promotions-create-with-http-info ad_account_id product-group-promotion-create-request))]
    (if (:decode-models *api-context*)
       (st/decode product-group-promotion-response-spec res st/string-transformer)
       res)))
...
Only provide one. If multiple options are provided, product_group_promotion_ids takes precedence over ad_group_ids. If none are provided, the endpoint returns an error."
  ...
  ([ad_account_id string?, {:keys [product_group_promotion_ids entity_statuses ad_group_id page_size order bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"product_group_promotion_ids" (with-collection-format product_group_promotion_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "ad_group_id" ad_group_id "page_size" page_size "order" order "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
...
(defn-spec product-group-promotions-update-with-http-info any?
  ...
  [ad_account_id string?, product-group-promotion-update-request product-group-promotion-update-request]
  (check-required-params ad_account_id product-group-promotion-update-request)
  (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    product-group-promotion-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec product-group-promotions-update product-group-promotion-response-spec
  "Update product group promotions
  Update multiple existing Product Group Promotions (by product_group_id)"
  [ad_account_id string?, product-group-promotion-update-request product-group-promotion-update-request]
  (let [res (:data (product-group-promotions-update-with-http-info ad_account_id product-group-promotion-update-request))]
    (if (:decode-models *api-context*)
       (st/decode product-group-promotion-response-spec res st/string-transformer)
       res)))
...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (product-groups-analytics-with-http-info ad_account_id start_date end_date product_group_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id start_date end_date product_group_ids columns granularity)
...
(defn-spec product-groups-analytics (s/coll-of product-group-analytics-response-inner-spec)
  "Get product group analytics
  Get analytics for the specified product groups in the specified <code>ad_account_id</code>, filtered by the specified options.
- The token's user_account must either be the Owner of the specified ad account, or have one of the necessary roles granted to them via <a href=\"https://help.pinterest.com/en/business/article/share-and-manage-access-to-your-ad-accounts\">Business Access</a>: Admin, Analyst, Campaign Manager.
  - If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, ] (product-groups-analytics ad_account_id start_date end_date product_group_ids columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of string?), granularity granularity, optional-params any?]
   (let [res (:data (product-groups-analytics-with-http-info ad_account_id start_date end_date product_group_ids columns granularity optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of product-group-analytics-response-inner-spec) res st/string-transformer)
        res))))
