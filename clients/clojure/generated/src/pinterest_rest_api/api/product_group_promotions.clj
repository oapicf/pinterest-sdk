(ns pinterest-rest-api.api.product-group-promotions
...
(defn-spec product-group-promotions-create-with-http-info any?
  "Create product group promotions
  Add one or more product groups from your catalog to an existing ad group. (Product groups added to an ad group are a 'product group promotion.')"
  [ad_account_id string?, product-group-promotions-create product-group-promotions-create]
  (check-required-params ad_account_id product-group-promotions-create)
  (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    product-group-promotions-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec product-group-promotions-create product-group-promotions-spec
  "Create product group promotions
  Add one or more product groups from your catalog to an existing ad group. (Product groups added to an ad group are a 'product group promotion.')"
  [ad_account_id string?, product-group-promotions-create product-group-promotions-create]
  (let [res (:data (product-group-promotions-create-with-http-info ad_account_id product-group-promotions-create))]
    (if (:decode-models *api-context*)
       (st/decode product-group-promotions-spec res st/string-transformer)
       res)))


(defn-spec product-group-promotions-get-with-http-info any?
  "Get a product group promotion by id
  Get a product group promotion by id"
  [ad_account_id string?, product_group_promotion_id string?]
  (check-required-params ad_account_id product_group_promotion_id)
  (call-api "/ad_accounts/{ad_account_id}/product_group_promotions/{product_group_promotion_id}" :get
            {:path-params   {"ad_account_id" ad_account_id "product_group_promotion_id" product_group_promotion_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec product-group-promotions-get product-group-promotion-spec
  "Get a product group promotion by id
  Get a product group promotion by id"
  [ad_account_id string?, product_group_promotion_id string?]
  (let [res (:data (product-group-promotions-get-with-http-info ad_account_id product_group_promotion_id))]
    (if (:decode-models *api-context*)
       (st/decode product-group-promotion-spec res st/string-transformer)
       res)))


(defn-spec product-group-promotions-list-with-http-info any?
...
Only provide one. If multiple options are provided, product_group_promotion_ids takes precedence over ad_group_ids. If none are provided, the endpoint returns an error."
  ([ad_account_id string?, ] (product-group-promotions-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order product_group_promotion_ids entity_statuses ad_group_id]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "product_group_promotion_ids" (with-collection-format product_group_promotion_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "ad_group_id" ad_group_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec product-group-promotions-list product-group-promotions-list-200-response-spec
  "Get product group promotions
  List existing product group promotions associated with an ad account.
...
**Note:** ad_group_ids and product_group_promotion_ids are mutually exclusive parameters.
Only provide one. If multiple options are provided, product_group_promotion_ids takes precedence over ad_group_ids. If none are provided, the endpoint returns an error."
  ([ad_account_id string?, ] (product-group-promotions-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (product-group-promotions-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode product-group-promotions-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec product-group-promotions-update-with-http-info any?
  "Update product group promotions
  Update multiple existing Product Group Promotions (by product_group_id)"
  [ad_account_id string?, product-group-promotions-update-with-required-body product-group-promotions-update-with-required-body]
  (check-required-params ad_account_id product-group-promotions-update-with-required-body)
  (call-api "/ad_accounts/{ad_account_id}/product_group_promotions" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    product-group-promotions-update-with-required-body
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec product-group-promotions-update product-group-promotions-spec
  "Update product group promotions
  Update multiple existing Product Group Promotions (by product_group_id)"
  [ad_account_id string?, product-group-promotions-update-with-required-body product-group-promotions-update-with-required-body]
  (let [res (:data (product-group-promotions-update-with-http-info ad_account_id product-group-promotions-update-with-required-body))]
    (if (:decode-models *api-context*)
       (st/decode product-group-promotions-spec res st/string-transformer)
       res)))


(defn-spec product-groups-analytics-with-http-info any?
...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (product-groups-analytics-with-http-info start_date end_date product_group_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date product_group_ids columns granularity ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/product_groups/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "product_group_ids" (with-collection-format product_group_ids :multi) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec product-groups-analytics (s/coll-of product-group-analytics-items-spec)
  "Get product group analytics
  Get analytics for the specified product groups in the specified `ad_account_id`, filtered by the specified options.
...
- If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (product-groups-analytics start_date end_date product_group_ids columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, product_group_ids (s/coll-of string?), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (product-groups-analytics-with-http-info start_date end_date product_group_ids columns granularity ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of product-group-analytics-items-spec) res st/string-transformer)
        res))))
