(ns pinterest-rest-api.api.targeting-template
...
(defn-spec targeting-template-create-with-http-info any?
...
Templates can be used to build out basic targeting criteria that you plan to use across campaigns and to reuse performance targeting from prior campaigns for new campaigns."
  [ad_account_id string?, targeting-template-create targeting-template-create]
  (check-required-params ad_account_id targeting-template-create)
  (call-api "/ad_accounts/{ad_account_id}/targeting_templates" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    targeting-template-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec targeting-template-create targeting-template-spec
  "Create targeting templates
  Targeting templates allow advertisers to save a set of targeting details including audience lists, keywords & interest, demographics, and placements to use more than once during the campaign creation process.

Templates can be used to build out basic targeting criteria that you plan to use across campaigns and to reuse performance targeting from prior campaigns for new campaigns."
  [ad_account_id string?, targeting-template-create targeting-template-create]
  (let [res (:data (targeting-template-create-with-http-info ad_account_id targeting-template-create))]
    (if (:decode-models *api-context*)
       (st/decode targeting-template-spec res st/string-transformer)
       res)))


(defn-spec targeting-template-list-with-http-info any?
  "List targeting templates
  Get a list of the targeting templates in the specified `ad_account_id`"
  ([ad_account_id string?, ] (targeting-template-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [bookmark page_size order include_sizing search_query]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/targeting_templates" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "include_sizing" include_sizing "search_query" search_query }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec targeting-template-list targeting-template-list-200-response-spec
  "List targeting templates
  Get a list of the targeting templates in the specified `ad_account_id`"
  ([ad_account_id string?, ] (targeting-template-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (targeting-template-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode targeting-template-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec targeting-template-update-with-http-info any?
  "Update targeting templates
  Update the targeting template given advertiser ID and targeting template ID"
  [ad_account_id string?, targeting-template-update-request-read-or-update targeting-template-update-request-read-or-update]
  (check-required-params ad_account_id targeting-template-update-request-read-or-update)
  (call-api "/ad_accounts/{ad_account_id}/targeting_templates" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    targeting-template-update-request-read-or-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec targeting-template-update any?
  "Update targeting templates
  Update the targeting template given advertiser ID and targeting template ID"
  [ad_account_id string?, targeting-template-update-request-read-or-update targeting-template-update-request-read-or-update]
  (let [res (:data (targeting-template-update-with-http-info ad_account_id targeting-template-update-request-read-or-update))]
    (if (:decode-models *api-context*)
