...
(defn-spec targeting-template-create targeting-template-get-response-data-spec
  "Create targeting templates
  <p>Targeting templates allow advertisers to save a set of targeting details including audience lists,
 keywords & interest, demographics, and placements to use more than once during the campaign creation process.</p>
 <p>Templates can be used to build out basic targeting criteria that you plan to use across campaigns and to reuse
  ...
  (let [res (:data (targeting-template-create-with-http-info ad_account_id targeting-template-create))]
    (if (:decode-models *api-context*)
       (st/decode targeting-template-get-response-data-spec res st/string-transformer)
       res)))
...
(defn-spec targeting-template-list-with-http-info any?
  ...
  ([ad_account_id string?, {:keys [order include_sizing search_query page_size bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/targeting_templates" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"order" order "include_sizing" include_sizing "search_query" search_query "page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
...
(defn-spec targeting-template-update-with-http-info any?
  ...
  [ad_account_id string?, targeting-template-update-request targeting-template-update-request]
  (check-required-params ad_account_id targeting-template-update-request)
  (call-api "/ad_accounts/{ad_account_id}/targeting_templates" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    targeting-template-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))
...
(defn-spec targeting-template-update any?
  ...
  <p>Update the targeting template given advertiser ID and targeting template ID</p>"
  [ad_account_id string?, targeting-template-update-request targeting-template-update-request]
  (let [res (:data (targeting-template-update-with-http-info ad_account_id targeting-template-update-request))]
    (if (:decode-models *api-context*)
