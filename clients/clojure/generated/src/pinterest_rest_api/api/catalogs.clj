...
(defn-spec catalogs-available-filter-values-with-http-info any?
...
[Learn more](/docs/api-features/shopping-overview/)"
  ([catalog_id string?, ] (catalogs-available-filter-values-with-http-info catalog_id nil))
  ([catalog_id string?, {:keys [feed_id country language ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params catalog_id)
   (call-api "/catalogs/available_filter_values" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"catalog_id" catalog_id "feed_id" feed_id "country" country "language" language "ad_account_id" ad_account_id }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec catalogs-available-filter-values catalogs-available-filter-values-spec
  "List available filter values
  Get the available filter attributes and values associated with a given feed or catalog owned by the \"operation user_account\".
- By default, the \"operation user_account\" is the token user_account.
- `country`, `language`, and `feed_id` are only used in retail catalogs.
...
[Learn more](/docs/api-features/shopping-overview/)"
  ([catalog_id string?, ] (catalogs-available-filter-values catalog_id nil))
  ([catalog_id string?, optional-params any?]
   (let [res (:data (catalogs-available-filter-values-with-http-info catalog_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode catalogs-available-filter-values-spec res st/string-transformer)
        res))))
...
Note: Access to the Product and Creative Assets catalog type is restricted to a specific group of users. If you require access, please reach out to your partner manager."
  ([catalog-create catalog-create, ] (catalogs-create-with-http-info catalog-create nil))
  ([catalog-create catalog-create, {:keys [ad_account_id]} (s/map-of keyword? any?)]
   (check-required-params catalog-create)
   (call-api "/catalogs" :post
             {:path-params   {}
              :header-params {}
              :query-params  {"ad_account_id" ad_account_id }
              :form-params   {}
              :body-param    catalog-create
...
Note: Access to the Product and Creative Assets catalog type is restricted to a specific group of users. If you require access, please reach out to your partner manager."
  ([catalog-create catalog-create, ] (catalogs-create catalog-create nil))
  ([catalog-create catalog-create, optional-params any?]
   (let [res (:data (catalogs-create-with-http-info catalog-create optional-params))]
     (if (:decode-models *api-context*)
...
[Learn more](/docs/api-features/shopping-overview/)"
  ...
  ([{:keys [ad_account_id bookmark page_size]} (s/map-of keyword? any?)]
