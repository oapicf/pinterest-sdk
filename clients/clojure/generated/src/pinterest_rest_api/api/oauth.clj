...
(defn-spec oauth-conversion-token conversion-access-token-response-spec
  "Generate OAuth access token for conversion API
  Generate a new and long-lived OAuth access token dedicated for sending conversions using a valid access token."
  []
  (let [res (:data (oauth-conversion-token-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode conversion-access-token-response-spec res st/string-transformer)
       res)))
...
Use <a href='/docs/developer-tools/token-debugger/' target='blank'>Token Debugger</a> to validate and inspect your access token."
  ...
  (call-api "/oauth/token" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {"grant_type" grant_type }
             :content-types ["application/x-www-form-urlencoded"]
             :accepts       ["application/json"]
             :auth-names    ["basic"]}))

(defn-spec oauth-token oauth-access-token-response-spec
  "Generate OAuth access token
  Generate a new OAuth access token using an authorization code; or refresh an existing one using a continuous refresh token.
...
Use <a href='/docs/developer-tools/token-debugger/' target='blank'>Token Debugger</a> to validate and inspect your access token."
  [grant_type string?]
  (let [res (:data (oauth-token-with-http-info grant_type))]
    (if (:decode-models *api-context*)
       (st/decode oauth-access-token-response-spec res st/string-transformer)
       res)))
