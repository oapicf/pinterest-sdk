(ns pinterest-rest-api.api.oauth
...
(defn-spec oauth-conversion-token-with-http-info any?
  "Generate OAuth access token for conversion API
  Generate a new and long-lived OAuth access token dedicated for sending conversions using a valid access token."
  []
  (call-api "/oauth/conversion_token" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec oauth-conversion-token conversion-access-token-spec
  "Generate OAuth access token for conversion API
  Generate a new and long-lived OAuth access token dedicated for sending conversions using a valid access token."
  []
  (let [res (:data (oauth-conversion-token-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode conversion-access-token-spec res st/string-transformer)
       res)))


(defn-spec oauth-token-with-http-info any?
...
Use [Token Debugger](/docs/developer-tools/token-debugger/) to validate and inspect your access token."
  ([grant_type token-grant-type-spec, ] (oauth-token-with-http-info grant_type nil))
  ([grant_type token-grant-type-spec, {:keys [code continuous_refresh redirect_uri refresh_token scope]} (s/map-of keyword? any?)]
   (check-required-params grant_type)
   (call-api "/oauth/token" :post
             {:path-params   {}
              :header-params {}
              :query-params  {}
              :form-params   {"code" code "continuous_refresh" continuous_refresh "grant_type" grant_type "redirect_uri" redirect_uri "refresh_token" refresh_token "scope" scope }
              :content-types ["application/x-www-form-urlencoded"]
              :accepts       ["application/json"]
              :auth-names    ["basic"]})))

(defn-spec oauth-token oauth-access-token-spec
  "Generate OAuth access token
  Generate a new OAuth access token using an authorization code; or refresh an existing one using a continuous refresh token.
...
Use [Token Debugger](/docs/developer-tools/token-debugger/) to validate and inspect your access token."
  ([grant_type token-grant-type-spec, ] (oauth-token grant_type nil))
  ([grant_type token-grant-type-spec, optional-params any?]
   (let [res (:data (oauth-token-with-http-info grant_type optional-params))]
     (if (:decode-models *api-context*)
        (st/decode oauth-access-token-spec res st/string-transformer)
        res))))


(defn-spec token-revoke-with-http-info any?
  "Revoke a token
  Revokes an access or refresh token. Only tokens issued for system users are currently supported. Revoked tokens become immediately invalid and unusable."
  ([token string?, ] (token-revoke-with-http-info token nil))
  ([token string?, {:keys [token_type_hint]} (s/map-of keyword? any?)]
   (check-required-params token)
   (call-api "/oauth/token/revoke" :post
             {:path-params   {}
              :header-params {}
              :query-params  {}
              :form-params   {"token" token "token_type_hint" token_type_hint }
              :content-types ["application/x-www-form-urlencoded"]
              :accepts       ["application/json"]
              :auth-names    ["basic"]})))

(defn-spec token-revoke any?
  "Revoke a token
  Revokes an access or refresh token. Only tokens issued for system users are currently supported. Revoked tokens become immediately invalid and unusable."
  ([token string?, ] (token-revoke token nil))
  ([token string?, optional-params any?]
   (let [res (:data (token-revoke-with-http-info token optional-params))]
     (if (:decode-models *api-context*)
