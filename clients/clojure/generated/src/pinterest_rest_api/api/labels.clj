(ns pinterest-rest-api.api.labels
...
(defn-spec labels-apply-with-http-info any?
  ...
  **Note:** You can only apply one brand label to a campaign. You can apply up to 30 custom labels to a campaign."
  [ad_account_id string?, label_id string?, labeled-entities-create labeled-entities-create]
  (check-required-params ad_account_id label_id labeled-entities-create)
  (call-api "/ad_accounts/{ad_account_id}/labels/{label_id}/apply" :post
            {:path-params   {"ad_account_id" ad_account_id "label_id" label_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    labeled-entities-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec labels-apply labeled-entities-spec
  "Apply label to entity
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)
  ...
  **Note:** You can only apply one brand label to a campaign. You can apply up to 30 custom labels to a campaign."
  [ad_account_id string?, label_id string?, labeled-entities-create labeled-entities-create]
  (let [res (:data (labels-apply-with-http-info ad_account_id label_id labeled-entities-create))]
    (if (:decode-models *api-context*)
       (st/decode labeled-entities-spec res st/string-transformer)
       res)))


(defn-spec labels-create-with-http-info any?
  "Create labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)
...
**Note:** You can only apply one brand label to a campaign. You can apply 30 custom labels to a campaign."
  [ad_account_id string?, label-create-request label-create-request]
  (check-required-params ad_account_id label-create-request)
  (call-api "/ad_accounts/{ad_account_id}/labels" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    label-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec labels-create labels-response-spec
  "Create labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)
...
**Note:** You can only apply one brand label to a campaign. You can apply 30 custom labels to a campaign."
  [ad_account_id string?, label-create-request label-create-request]
  (let [res (:data (labels-create-with-http-info ad_account_id label-create-request))]
    (if (:decode-models *api-context*)
       (st/decode labels-response-spec res st/string-transformer)
       res)))


(defn-spec labels-list-with-http-info any?
  "List labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)
...
If no filter is provided, it will default to labels associated with the ad account id."
  ([ad_account_id string?, ] (labels-list-with-http-info ad_account_id nil))
  ([ad_account_id string?, {:keys [campaign_ids label_ids entity_statuses label_types bookmark page_size]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/labels" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "label_ids" (with-collection-format label_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "label_types" (with-collection-format label_types :multi) "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec labels-list labels-list-200-response-spec
  "List labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)

See a list of labels for assets that your account owns, and filter the list by different criteria.
If no filter is provided, it will default to labels associated with the ad account id."
  ([ad_account_id string?, ] (labels-list ad_account_id nil))
  ([ad_account_id string?, optional-params any?]
   (let [res (:data (labels-list-with-http-info ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode labels-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec labels-remove-with-http-info any?
  ...
  [ad_account_id string?, label_id string?, labeled-entities-create labeled-entities-create]
  (check-required-params ad_account_id label_id labeled-entities-create)
  (call-api "/ad_accounts/{ad_account_id}/labels/{label_id}/remove" :post
            {:path-params   {"ad_account_id" ad_account_id "label_id" label_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec labels-remove labeled-entities-spec
  "Remove label from entities
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)

  Remove a label from one or more entities."
  [ad_account_id string?, label_id string?, labeled-entities-create labeled-entities-create]
  (let [res (:data (labels-remove-with-http-info ad_account_id label_id labeled-entities-create))]
    (if (:decode-models *api-context*)
       (st/decode labeled-entities-spec res st/string-transformer)
       res)))


(defn-spec labels-update-with-http-info any?
  "Update labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)

Change the properties of one or more labels."
  [ad_account_id string?, label-update-request label-update-request]
  (check-required-params ad_account_id label-update-request)
  (call-api "/ad_accounts/{ad_account_id}/labels" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    label-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec labels-update labels-response-spec
  "Update labels
  [Closed beta](/docs/getting-started/using-beta-and-restricted-features/)

Change the properties of one or more labels."
  [ad_account_id string?, label-update-request label-update-request]
  (let [res (:data (labels-update-with-http-info ad_account_id label-update-request))]
    (if (:decode-models *api-context*)
       (st/decode labels-response-spec res st/string-transformer)
       res)))
