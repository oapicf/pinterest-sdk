...
<p>
  ...
  ([ad_account_id string?, {:keys [campaign_ids label_ids entity_statuses label_types page_size bookmark]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/labels" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"campaign_ids" (with-collection-format campaign_ids :multi) "label_ids" (with-collection-format label_ids :multi) "entity_statuses" (with-collection-format entity_statuses :multi) "label_types" (with-collection-format label_types :multi) "page_size" page_size "bookmark" bookmark }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
