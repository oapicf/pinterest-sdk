...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time."
  ...
  ([ad_account_id string?, start_date inst?, end_date inst?, columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time reporting_timezone]} (s/map-of keyword? any?)]
...
- If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ...
  ([ad_account_id string?, start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-targeting-type-spec), columns (s/coll-of string?), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone]} (s/map-of keyword? any?)]
...
This creates an asynchronous brand, category, SKU report based on the given request. This request returns a token that you can use to download the report when it is ready."
  [ad_account_id string?, conversion-product-report-request conversion-product-report-request]
  (check-required-params ad_account_id conversion-product-report-request)
  (call-api "/ad_accounts/{ad_account_id}/reports/brand_category_sku" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    conversion-product-report-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec analytics-create-conversion-product-report ads-analytics-create-async-response-spec
  "Create a request for a brand, category, SKU report
  <a href=\"/docs/getting-started/using-beta-and-restricted-features/\" target=\"blank\" target=\"blank\">Restricted</a>
This creates an asynchronous brand, category, SKU report based on the given request. This request returns a token that you can use to download the report when it is ready."
  [ad_account_id string?, conversion-product-report-request conversion-product-report-request]
  (let [res (:data (analytics-create-conversion-product-report-with-http-info ad_account_id conversion-product-report-request))]
    (if (:decode-models *api-context*)
...
(defn-spec analytics-get-conversion-product-report ads-analytics-get-async-response-spec
  "Get advertiser brand, category, SKU report
  <a href=\"/docs/getting-started/using-beta-and-restricted-features/\" target=\"blank\" target=\"blank\">Restricted</a>
Get a brand, category, SKU report for an ad account. This call returns the URL for the report that matches the token returned in the request to the Create brand, category, SKU report endpoint."
  [ad_account_id string?, token string?]
