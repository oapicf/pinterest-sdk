...
(defn-spec ad-account-analytics-with-http-info any?
  ...
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-account-analytics-with-http-info start_date end_date columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time reporting_timezone]} (s/map-of keyword? any?)]
   (check-required-params start_date end_date columns granularity ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))

(defn-spec ad-account-analytics (s/coll-of ad-account-analytics-items-spec)
  "Get ad account analytics
  Get analytics for the specified `ad_account_id`, filtered by the specified options.
  ...
  - If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time."
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, ] (ad-account-analytics start_date end_date columns granularity ad_account_id nil))
  ([start_date inst?, end_date inst?, columns (s/coll-of reporting-column-sync-spec), granularity granularity, ad_account_id string?, optional-params any?]
   (let [res (:data (ad-account-analytics-with-http-info start_date end_date columns granularity ad_account_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode (s/coll-of ad-account-analytics-items-spec) res st/string-transformer)
        res))))
...
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-account-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-account-targeting-analytics-get-with-http-info ad_account_id start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-account-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, {:keys [click_window_days engagement_window_days view_window_days conversion_report_time attribution_types reporting_timezone]} (s/map-of keyword? any?)]
   ...
   (call-api "/ad_accounts/{ad_account_id}/targeting_analytics" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"start_date" start_date "end_date" end_date "targeting_types" (with-collection-format targeting_types :csv) "columns" (with-collection-format columns :csv) "granularity" granularity "click_window_days" click_window_days "engagement_window_days" engagement_window_days "view_window_days" view_window_days "conversion_report_time" conversion_report_time "attribution_types" (with-collection-format attribution_types :csv) "reporting_timezone" reporting_timezone }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
* If granularity is not HOUR, you can pull data from up to 90 days before the current date in UTC time, with a maximum time range of 90 days.
* If granularity is HOUR, you can pull data from up to 8 days before the current date in UTC time, with a maximum time range of 3 days."
  ([ad_account_id string?, start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-account-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, ] (ad-account-targeting-analytics-get ad_account_id start_date end_date targeting_types columns granularity nil))
  ([ad_account_id string?, start_date inst?, end_date inst?, targeting_types (s/coll-of ads-analytics-account-targeting-type-spec), columns (s/coll-of reporting-column-sync-spec), granularity granularity, optional-params any?]
...
For more, see [Create an advertiser account](https://help.pinterest.com/en/business/article/create-an-advertiser-account)."
  [ad-account-create ad-account-create]
  (check-required-params ad-account-create)
  (call-api "/ad_accounts" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    ad-account-create
...
For more, see [Create an advertiser account](https://help.pinterest.com/en/business/article/create-an-advertiser-account)."
  [ad-account-create ad-account-create]
  (let [res (:data (ad-accounts-create-with-http-info ad-account-create))]
    (if (:decode-models *api-context*)
...
(defn-spec ad-accounts-list-with-http-info any?
  ...
  ([{:keys [include_shared_accounts bookmark page_size]} (s/map-of keyword? any?)]
   (call-api "/ad_accounts" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"include_shared_accounts" include_shared_accounts "bookmark" bookmark "page_size" page_size }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2" "client_credentials"]})))
...
(defn-spec analytics-create-conversion-product-report-with-http-info any?
  ...
  [ad_account_id string?, conversion-product-report-create conversion-product-report-create]
  (check-required-params ad_account_id conversion-product-report-create)
  (call-api "/ad_accounts/{ad_account_id}/reports/brand_category_sku" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    conversion-product-report-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec analytics-create-conversion-product-report conversion-product-report-spec
  "Create a request for a brand, category, SKU report
  [Restricted](/docs/getting-started/using-beta-and-restricted-features/)
  This creates an asynchronous brand, category, SKU report based on the given request. This request returns a token that you can use to download the report when it is ready."
  [ad_account_id string?, conversion-product-report-create conversion-product-report-create]
  (let [res (:data (analytics-create-conversion-product-report-with-http-info ad_account_id conversion-product-report-create))]
    (if (:decode-models *api-context*)
       (st/decode conversion-product-report-spec res st/string-transformer)
       res)))
...
(defn-spec analytics-create-mmm-report-with-http-info any?
  ...
  [ad_account_id string?, mmm-report-create mmm-report-create]
  (check-required-params ad_account_id mmm-report-create)
  (call-api "/ad_accounts/{ad_account_id}/mmm_reports" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    mmm-report-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec analytics-create-mmm-report mmm-report-spec
  "Create a request for a Marketing Mix Modeling (MMM) report
  This creates an asynchronous mmm report based on the given request.
    It returns a token that you can use to download the report when it is
    ready. NOTE: An additional limit of 5 queries per minute per advertiser
    ...
    data becomes available. Production and other non-media fees are excluded."
  [ad_account_id string?, mmm-report-create mmm-report-create]
  (let [res (:data (analytics-create-mmm-report-with-http-info ad_account_id mmm-report-create))]
    (if (:decode-models *api-context*)
       (st/decode mmm-report-spec res st/string-transformer)
       res)))
...
(defn-spec analytics-create-template-report template-based-report-spec
  "Create async request for an analytics report using a template
  This takes a template ID and an optional custom timeframe and
  constructs an asynchronous report based on the template. It returns
  a token that you can use to download the report when it is ready."
  ...
  ([ad_account_id string?, template_id string?, optional-params any?]
   (let [res (:data (analytics-create-template-report-with-http-info ad_account_id template_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode template-based-report-spec res st/string-transformer)
        res))))


(defn-spec analytics-get-conversion-product-report-with-http-info any?
  ...
  (call-api "/ad_accounts/{ad_account_id}/reports/brand_category_sku" :get
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
...
(defn-spec analytics-get-conversion-product-report conversion-product-report-spec
  "Get advertiser brand, category, SKU report
  [Restricted](/docs/getting-started/using-beta-and-restricted-features/)
  Get a brand, category, SKU report for an ad account. This call returns the URL for the report that matches the token returned in the request to the Create brand, category, SKU report endpoint."
  [ad_account_id string?, token string?]
  (let [res (:data (analytics-get-conversion-product-report-with-http-info ad_account_id token))]
    (if (:decode-models *api-context*)
       (st/decode conversion-product-report-spec res st/string-transformer)
       res)))
...
(defn-spec analytics-get-mmm-report mmm-report-spec
  "Get advertiser Marketing Mix Modeling (MMM) report.
  Get an mmm report for an ad account. This returns a URL to an
    mmm metrics report given a token returned from the create mmm report endpoint."
  [ad_account_id string?, token string?]
...
(defn-spec templates-list-with-http-info any?
  ...
  ([ad_account_id string?, {:keys [bookmark page_size order]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id)
   (call-api "/ad_accounts/{ad_account_id}/templates" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))
