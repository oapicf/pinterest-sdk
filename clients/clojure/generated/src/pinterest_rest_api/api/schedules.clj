(ns pinterest-rest-api.api.schedules
...
(defn-spec schedules-create-with-http-info any?
  "Create schedules
  Batch create schedules"
  [ad_account_id string?, schedule-create (s/coll-of schedule-create-spec)]
  (check-required-params ad_account_id schedule-create)
  (call-api "/ad_accounts/{ad_account_id}/schedules" :post
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    schedule-create
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec schedules-create (s/coll-of schedules-create-200-response-inner-spec)
  "Create schedules
  Batch create schedules"
  [ad_account_id string?, schedule-create (s/coll-of schedule-create-spec)]
  (let [res (:data (schedules-create-with-http-info ad_account_id schedule-create))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of schedules-create-200-response-inner-spec) res st/string-transformer)
       res)))


(defn-spec schedules-list-with-http-info any?
  "Get Schedules
  Get schedules for a specific advertiser"
  ([ad_account_id string?, entity_ids (s/coll-of string?), ] (schedules-list-with-http-info ad_account_id entity_ids nil))
  ([ad_account_id string?, entity_ids (s/coll-of string?), {:keys [bookmark page_size order schedule_statuses schedule_type]} (s/map-of keyword? any?)]
   (check-required-params ad_account_id entity_ids)
   (call-api "/ad_accounts/{ad_account_id}/schedules" :get
             {:path-params   {"ad_account_id" ad_account_id }
              :header-params {}
              :query-params  {"bookmark" bookmark "page_size" page_size "order" order "schedule_statuses" (with-collection-format schedule_statuses :multi) "schedule_type" schedule_type "entity_ids" (with-collection-format entity_ids :multi) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["pinterest_oauth2"]})))

(defn-spec schedules-list schedules-list-200-response-spec
  "Get Schedules
  Get schedules for a specific advertiser"
  ([ad_account_id string?, entity_ids (s/coll-of string?), ] (schedules-list ad_account_id entity_ids nil))
  ([ad_account_id string?, entity_ids (s/coll-of string?), optional-params any?]
   (let [res (:data (schedules-list-with-http-info ad_account_id entity_ids optional-params))]
     (if (:decode-models *api-context*)
        (st/decode schedules-list-200-response-spec res st/string-transformer)
        res))))


(defn-spec schedules-update-with-http-info any?
  "Update schedules
  Update one or more schedules"
  [ad_account_id string?, schedule-batch-update (s/coll-of schedule-batch-update-spec)]
  (check-required-params ad_account_id schedule-batch-update)
  (call-api "/ad_accounts/{ad_account_id}/schedules" :patch
            {:path-params   {"ad_account_id" ad_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    schedule-batch-update
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["pinterest_oauth2"]}))

(defn-spec schedules-update (s/coll-of schedules-create-200-response-inner-spec)
  "Update schedules
  Update one or more schedules"
  [ad_account_id string?, schedule-batch-update (s/coll-of schedule-batch-update-spec)]
  (let [res (:data (schedules-update-with-http-info ad_account_id schedule-batch-update))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of schedules-create-200-response-inner-spec) res st/string-transformer)
       res)))
