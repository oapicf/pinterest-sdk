...
export class BusinessAccessInviteApi {
     ...
     */
    public async cancelInvitesOrRequests (businessId: string, cancelInvitesRequest: CancelInvitesRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CancelInvitesResponse;  }> {
        ...
        // verify required parameter 'cancelInvitesRequest' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(cancelInvitesRequest, "CancelInvitesRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CancelInvitesResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CancelInvitesResponse");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     */
    public async getInvites (businessId: string, isMember?: boolean, inviteStatus?: Array<InviteFilterStatus>, inviteType?: InviteType, bookmark?: string, pageSize?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetInvites200Response;  }> {
        const localVarPath = this.basePath + '/businesses/{business_id}/invites'
            .replace('{business_id}', encodeURIComponent(String(businessId)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        if (inviteStatus !== undefined) {
            localVarQueryParameters['invite_status'] = ObjectSerializer.serialize(inviteStatus, "Array<InviteFilterStatus>");
        }
