...
export class BulkApi {
     ...
     */
    public async bulkDownloadCreate (adAccountId: string, bulkDownloadCreate: BulkDownloadCreate, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: BulkDownload;  }> {
        ...
        // verify required parameter 'bulkDownloadCreate' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(bulkDownloadCreate, "BulkDownloadCreate")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: BulkDownload;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "BulkDownload");
     ...
     */
    public async bulkRequestGet (adAccountId: string, bulkRequestId: string, includeDetails?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: BulkJobData;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: BulkJobData;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "BulkJobData");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
