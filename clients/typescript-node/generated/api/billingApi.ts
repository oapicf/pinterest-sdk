...
export class BillingApi {
    ...
    public async billingInvoicesGet (adAccountId: string, bookmark?: string, pageSize?: number, sort?: 'DUE_DATE' | 'BILLING_PERIOD' | 'DOCUMENT_TYPE' | 'TOTAL_AMOUNT' | 'INVOICE_NUMBER', order?: 'ASCENDING' | 'DESCENDING', status?: 'OPEN' | 'CLOSED', documentType?: 'INVOICE' | 'CREDIT_MEMO', startDueDate?: string, endDueDate?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: BillingInvoicesGet200Response;  }> {
        ...
        if (sort !== undefined) {
            localVarQueryParameters['sort'] = ObjectSerializer.serialize(sort, "'DUE_DATE' | 'BILLING_PERIOD' | 'DOCUMENT_TYPE' | 'TOTAL_AMOUNT' | 'INVOICE_NUMBER'");
        }
        ...
        if (status !== undefined) {
            localVarQueryParameters['status'] = ObjectSerializer.serialize(status, "'OPEN' | 'CLOSED'");
        }

        if (documentType !== undefined) {
            localVarQueryParameters['document_type'] = ObjectSerializer.serialize(documentType, "'INVOICE' | 'CREDIT_MEMO'");
        }
