...
//
list_t*
CampaignsAPI_adPinsAnalytics(apiClient_t *apiClient, char *ad_account_id, char *campaign_id, list_t *pin_ids, char start_date, char end_date, list_t *columns, granularity_e granularity, int *click_window_days, int *engagement_window_days, int *view_window_days, pinterest_rest_api_adPinsAnalytics_conversion_report_time_e conversion_report_time)
{
...
//
list_t*
CampaignsAPI_campaignsAnalytics(apiClient_t *apiClient, char *ad_account_id, char start_date, char end_date, list_t *campaign_ids, list_t *columns, granularity_e granularity, int *click_window_days, int *engagement_window_days, int *view_window_days, pinterest_rest_api_campaignsAnalytics_conversion_report_time_e conversion_report_time, int *aggregate_report_rows, reporting_time_zone_e reporting_timezone)
{
    list_t    *localVarQueryParameters = list_createList();
    list_t    *localVarHeaderParameters = NULL;
    list_t    *localVarFormParameters = NULL;
...
CampaignsAPI_campaignsCreate(apiClient_t *apiClient, char *ad_account_id, list_t *campaign_create_request)
{
    ...
    //}
    //nonprimitive not container
    campaign_create_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CampaignsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = campaign_create_response_parseFromJSON(CampaignsAPIlocalVarJSON);
        cJSON_Delete(CampaignsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
CampaignsAPI_campaignsGet(apiClient_t *apiClient, char *ad_account_id, char *campaign_id)
{
    ...
    // Path Params
    long sizeOfPathParams_ad_account_id = strlen(ad_account_id)+3 + strlen(campaign_id)+3 + sizeof("{ ad_account_id }") - 1;
    if(ad_account_id == NULL) {
        goto end;
    }
    char* localVarToReplace_ad_account_id = malloc(sizeOfPathParams_ad_account_id);
    ...
    // Path Params
    long sizeOfPathParams_campaign_id = strlen(ad_account_id)+3 + strlen(campaign_id)+3 + sizeof("{ campaign_id }") - 1;
    if(campaign_id == NULL) {
        goto end;
    }
    ...
    //}
    //nonprimitive not container
    campaign_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CampaignsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = campaign_response_parseFromJSON(CampaignsAPIlocalVarJSON);
        cJSON_Delete(CampaignsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
CampaignsAPI_campaignsList(apiClient_t *apiClient, char *ad_account_id, list_t *campaign_ids, list_t *entity_statuses, int *page_size, pinterest_rest_api_campaignsList_order_e order, char *bookmark)
{
    ...
    if (order)
    {
        ...
        keyPairQuery_order = keyValuePair_create(keyQuery_order, strdup(campaignsList_ORDER_ToString(
        valueQuery_order)));
...
CampaignsAPI_campaignsUpdate(apiClient_t *apiClient, char *ad_account_id, list_t *campaign_update_request)
{
    ...
    //}
    //nonprimitive not container
    campaign_update_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CampaignsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = campaign_update_response_parseFromJSON(CampaignsAPIlocalVarJSON);
        cJSON_Delete(CampaignsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
