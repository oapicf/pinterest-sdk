...
static char* productGroupsAnalytics__ToString(pinterest_rest_api_productGroupsAnalytics_reporting_timezone_e ){
    char *Array[] =  { "NULL", "PINTEREST_TIME_ZONE", "AD_ACCOUNT_TIME_ZONE" };
    return Array[];
}

static pinterest_rest_api_productGroupsAnalytics_reporting_timezone_e productGroupsAnalytics__FromString(char* ){
    int stringToReturn = 0;
    char *Array[] =  { "NULL", "PINTEREST_TIME_ZONE", "AD_ACCOUNT_TIME_ZONE" };
    size_t sizeofArray = sizeof(Array) / sizeof(Array[0]);
    while(stringToReturn < sizeofArray) {
...
// since conversion to JSON passes through the conversion of the model, and ToString. The function is kept for future reference.
//
static cJSON *productGroupsAnalytics__convertToJSON(pinterest_rest_api_productGroupsAnalytics_reporting_timezone_e ) {
    cJSON *item = cJSON_CreateObject();
    return item;
    fail:
    cJSON_Delete(item);
...
// since conversion from JSON passes through the conversion of the model, and FromString. The function is kept for future reference.
//
static pinterest_rest_api_productGroupsAnalytics_reporting_timezone_e productGroupsAnalytics__parseFromJSON(cJSON* JSON) {
    pinterest_rest_api_productGroupsAnalytics_reporting_timezone_e Variable = 0;
    return Variable;
end:
    return 0;
}
...
ProductGroupPromotionsAPI_productGroupPromotionsGet(apiClient_t *apiClient, char *ad_account_id, char *product_group_promotion_id)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *ProductGroupPromotionsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = product_group_promotion_parseFromJSON(ProductGroupPromotionsAPIlocalVarJSON);
        cJSON_Delete(ProductGroupPromotionsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
ProductGroupPromotionsAPI_productGroupsAnalytics(apiClient_t *apiClient, char *ad_account_id, char start_date, char end_date, list_t *product_group_ids, list_t *columns, granularity_e granularity, int *click_window_days, int *engagement_window_days, int *view_window_days, pinterest_rest_api_productGroupsAnalytics_conversion_report_time_e conversion_report_time, reporting_time_zone_e reporting_timezone)
{
    ...
    if (reporting_timezone)
    {
        ...
        keyPairQuery_reporting_timezone = keyValuePair_create(keyQuery_reporting_timezone, strdup(productGroupsAnalytics__ToString(
