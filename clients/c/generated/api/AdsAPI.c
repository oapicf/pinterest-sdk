...
//
metrics_response_t*
AdsAPI_adTargetingAnalyticsGet(apiClient_t *apiClient, char *ad_account_id, list_t *ad_ids, char start_date, char end_date, list_t *targeting_types, list_t *columns, granularity_e granularity, int *click_window_days, int *engagement_window_days, int *view_window_days, pinterest_rest_api_adTargetingAnalyticsGet_conversion_report_time_e conversion_report_time, list_t *attribution_types, reporting_time_zone_e reporting_timezone)
{
    ...
    if (click_window_days)
    {
        ...
        keyPairQuery_click_window_days = keyValuePair_create(keyQuery_click_window_days, strdup(adTargetingAnalyticsGet_CLICKWINDOWDAYS_ToString(
    ...
    if (engagement_window_days)
    {
        ...
        keyPairQuery_engagement_window_days = keyValuePair_create(keyQuery_engagement_window_days, strdup(adTargetingAnalyticsGet_ENGAGEMENTWINDOWDAYS_ToString(
    ...
    if (view_window_days)
    {
        ...
        keyPairQuery_view_window_days = keyValuePair_create(keyQuery_view_window_days, strdup(adTargetingAnalyticsGet_VIEWWINDOWDAYS_ToString(
    ...
    if (conversion_report_time)
    {
        ...
        keyPairQuery_conversion_report_time = keyValuePair_create(keyQuery_conversion_report_time, strdup(adTargetingAnalyticsGet_CONVERSIONREPORTTIME_ToString(
...
//
list_t*
AdsAPI_adsAnalytics(apiClient_t *apiClient, char *ad_account_id, char start_date, char end_date, list_t *columns, granularity_e granularity, list_t *ad_ids, int *click_window_days, int *engagement_window_days, int *view_window_days, pinterest_rest_api_adsAnalytics_conversion_report_time_e conversion_report_time, list_t *pin_ids, list_t *campaign_ids, reporting_time_zone_e reporting_timezone)
{
    list_t    *localVarQueryParameters = list_createList();
    list_t    *localVarHeaderParameters = NULL;
    list_t    *localVarFormParameters = NULL;
...
AdsAPI_adsCreate(apiClient_t *apiClient, char *ad_account_id, list_t *ad_create_request)
{
    ...
    //}
    //nonprimitive not container
    ad_array_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AdsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = ad_array_response_parseFromJSON(AdsAPIlocalVarJSON);
        cJSON_Delete(AdsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
AdsAPI_adsGet(apiClient_t *apiClient, char *ad_account_id, char *ad_id)
{
    ...
    // Path Params
    long sizeOfPathParams_ad_account_id = strlen(ad_account_id)+3 + strlen(ad_id)+3 + sizeof("{ ad_account_id }") - 1;
    if(ad_account_id == NULL) {
        goto end;
    }
    char* localVarToReplace_ad_account_id = malloc(sizeOfPathParams_ad_account_id);
    ...
    // Path Params
    long sizeOfPathParams_ad_id = strlen(ad_account_id)+3 + strlen(ad_id)+3 + sizeof("{ ad_id }") - 1;
    if(ad_id == NULL) {
        goto end;
    }
    ...
    //}
    //nonprimitive not container
    ad_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AdsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = ad_response_parseFromJSON(AdsAPIlocalVarJSON);
        cJSON_Delete(AdsAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
AdsAPI_adsList(apiClient_t *apiClient, char *ad_account_id, list_t *campaign_ids, list_t *ad_group_ids, list_t *ad_ids, list_t *entity_statuses, int *page_size, pinterest_rest_api_adsList_order_e order, char *bookmark)
{
    ...
    if (order)
    {
        ...
        keyPairQuery_order = keyValuePair_create(keyQuery_order, strdup(adsList_ORDER_ToString(
        valueQuery_order)));
