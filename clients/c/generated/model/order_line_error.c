...
static order_line_error_t *order_line_error_create_internal(
    order_line_t *data,
    ...
    order_line_error_local_var->_library_owned = 1;
...
__attribute__((deprecated)) order_line_error_t *order_line_error_create(
    ...
    return order_line_error_create_internal (
        data,
        error_messages
        );
}
...
void order_line_error_free(order_line_error_t *order_line_error) {
    ...
    if(order_line_error->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "order_line_error_free");
        return ;
    }
...
order_line_error_t *order_line_error_parseFromJSON(cJSON *order_line_errorJSON){
    ...
    order_line_error_local_var = order_line_error_create_internal (
