...
catalogs_retail_items_batch_t *catalogs_retail_items_batch_create(
    ...
    catalogs_retail_items_batch_t *catalogs_retail_items_batch_local_var = malloc(sizeof(catalogs_retail_items_batch_t));
...
void catalogs_retail_items_batch_free(catalogs_retail_items_batch_t *catalogs_retail_items_batch) {
    if(NULL == catalogs_retail_items_batch){
        return ;
    }
    listEntry_t *listEntry;
    if (catalogs_retail_items_batch->batch_id) {
        free(catalogs_retail_items_batch->batch_id);
        catalogs_retail_items_batch->batch_id = NULL;
    }
    if (catalogs_retail_items_batch->created_time) {
        free(catalogs_retail_items_batch->created_time);
        catalogs_retail_items_batch->created_time = NULL;
    }
    if (catalogs_retail_items_batch->completed_time) {
        free(catalogs_retail_items_batch->completed_time);
        catalogs_retail_items_batch->completed_time = NULL;
    }
    if (catalogs_retail_items_batch->status) {
        batch_operation_status_free(catalogs_retail_items_batch->status);
        catalogs_retail_items_batch->status = NULL;
    }
    if (catalogs_retail_items_batch->catalog_type) {
        catalogs_type_free(catalogs_retail_items_batch->catalog_type);
        catalogs_retail_items_batch->catalog_type = NULL;
    }
    if (catalogs_retail_items_batch->items) {
        list_ForEach(listEntry, catalogs_retail_items_batch->items) {
            item_processing_record_free(listEntry->data);
        }
        list_freeList(catalogs_retail_items_batch->items);
        catalogs_retail_items_batch->items = NULL;
    }
    free(catalogs_retail_items_batch);
}

cJSON *catalogs_retail_items_batch_convertToJSON(catalogs_retail_items_batch_t *catalogs_retail_items_batch) {
    cJSON *item = cJSON_CreateObject();

    // catalogs_retail_items_batch->batch_id
    if(catalogs_retail_items_batch->batch_id) {
    if(cJSON_AddStringToObject(item, "batch_id", catalogs_retail_items_batch->batch_id) == NULL) {
    goto fail; //String
    }
    }


    // catalogs_retail_items_batch->created_time
    if(catalogs_retail_items_batch->created_time) {
    if(cJSON_AddStringToObject(item, "created_time", catalogs_retail_items_batch->created_time) == NULL) {
    goto fail; //Date-Time
    }
    }
    ...
    if(catalogs_retail_items_batch->completed_time) {
    if(cJSON_AddStringToObject(item, "completed_time", catalogs_retail_items_batch->completed_time) == NULL) {
    goto fail; //Date-Time
    }
    }


    // catalogs_retail_items_batch->status
    if(catalogs_retail_items_batch->status != pinterest_rest_api_catalogs_retail_items_batch__NULL) {
    cJSON *status_local_JSON = batch_operation_status_convertToJSON(catalogs_retail_items_batch->status);
    ...
    if (pinterest_rest_api_catalogs_retail_items_batch__NULL == catalogs_retail_items_batch->catalog_type) {
        goto fail;
    }
    ...
    // catalogs_retail_items_batch->items
    if(catalogs_retail_items_batch->items) {
    ...
    if (catalogs_retail_items_batch->items) {
    list_ForEach(itemsListEntry, catalogs_retail_items_batch->items) {
    cJSON *itemLocal = item_processing_record_convertToJSON(itemsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
catalogs_retail_items_batch_t *catalogs_retail_items_batch_parseFromJSON(cJSON *catalogs_retail_items_batchJSON){
    ...
    // define the local variable for catalogs_retail_items_batch->status
    ...
    // catalogs_retail_items_batch->batch_id
    cJSON *batch_id = cJSON_GetObjectItemCaseSensitive(catalogs_retail_items_batchJSON, "batch_id");
    if (batch_id) { 
    ...
    // catalogs_retail_items_batch->created_time
    cJSON *created_time = cJSON_GetObjectItemCaseSensitive(catalogs_retail_items_batchJSON, "created_time");
    if (created_time) { 
    ...
    // catalogs_retail_items_batch->status
    cJSON *status = cJSON_GetObjectItemCaseSensitive(catalogs_retail_items_batchJSON, "status");
    if (status) { 
    status_local_nonprim = batch_operation_status_parseFromJSON(status); //custom
    }

    // catalogs_retail_items_batch->catalog_type
    cJSON *catalog_type = cJSON_GetObjectItemCaseSensitive(catalogs_retail_items_batchJSON, "catalog_type");
    if (!catalog_type) {
        goto end;
    }
    ...
    // catalogs_retail_items_batch->items
    cJSON *items = cJSON_GetObjectItemCaseSensitive(catalogs_retail_items_batchJSON, "items");
    if (items) { 
    ...
    cJSON_ArrayForEach(items_local_nonprimitive,items )
    {
        ...
        item_processing_record_t *itemsItem = item_processing_record_parseFromJSON(items_local_nonprimitive);
    ...
    catalogs_retail_items_batch_local_var = catalogs_retail_items_batch_create (
