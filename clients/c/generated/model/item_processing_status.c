...
cJSON *item_processing_status_convertToJSON(pinterest_rest_api_item_processing_status__e item_processing_status) {
...
pinterest_rest_api_item_processing_status__e item_processing_status_parseFromJSON(cJSON *item_processing_statusJSON) {
    if(!cJSON_IsString(item_processing_statusJSON) || (item_processing_statusJSON->valuestring == NULL)) {
        return 0;
    }
    return item_processing_status_item_processing_status_FromString(item_processing_statusJSON->valuestring);
}
