...
static catalogs_hotel_items_filter_t *catalogs_hotel_items_filter_create_internal(
    char *catalog_id,
    ...
    catalogs_hotel_items_filter_local_var->_library_owned = 1;
    return catalogs_hotel_items_filter_local_var;
}

__attribute__((deprecated)) catalogs_hotel_items_filter_t *catalogs_hotel_items_filter_create(
    char *catalog_id,
    pinterest_rest_api_catalogs_hotel_items_filter_CATALOGTYPE_e catalog_type,
    list_t *hotel_ids
    ) {
    return catalogs_hotel_items_filter_create_internal (
        catalog_id,
        catalog_type,
        hotel_ids
        );
...
void catalogs_hotel_items_filter_free(catalogs_hotel_items_filter_t *catalogs_hotel_items_filter) {
    ...
    if(catalogs_hotel_items_filter->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "catalogs_hotel_items_filter_free");
        return ;
    }
...
cJSON *catalogs_hotel_items_filter_convertToJSON(catalogs_hotel_items_filter_t *catalogs_hotel_items_filter) {
    ...
    if(cJSON_AddStringToObject(item, "catalog_type", catalogs_hotel_items_filter_catalog_type_ToString(catalogs_hotel_items_filter->catalog_type)) == NULL)
    {
    goto fail; //Enum
    }
...
catalogs_hotel_items_filter_t *catalogs_hotel_items_filter_parseFromJSON(cJSON *catalogs_hotel_items_filterJSON){
    ...
    catalogs_hotel_items_filter_local_var = catalogs_hotel_items_filter_create_internal (
