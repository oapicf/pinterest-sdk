...
static item_delete_batch_record_t *item_delete_batch_record_create_internal(
    char *item_id
    ) {
    item_delete_batch_record_t *item_delete_batch_record_local_var = malloc(sizeof(item_delete_batch_record_t));
...
void item_delete_batch_record_free(item_delete_batch_record_t *item_delete_batch_record) {
    if(NULL == item_delete_batch_record){
        return ;
    }
    if(item_delete_batch_record->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "item_delete_batch_record_free");
        return ;
    }
    listEntry_t *listEntry;
    if (item_delete_batch_record->item_id) {
        free(item_delete_batch_record->item_id);
        item_delete_batch_record->item_id = NULL;
    }
    free(item_delete_batch_record);
}

cJSON *item_delete_batch_record_convertToJSON(item_delete_batch_record_t *item_delete_batch_record) {
    cJSON *item = cJSON_CreateObject();

    // item_delete_batch_record->item_id
    if(item_delete_batch_record->item_id) {
    if(cJSON_AddStringToObject(item, "item_id", item_delete_batch_record->item_id) == NULL) {
    goto fail; //String
    }
    }
...
item_delete_batch_record_t *item_delete_batch_record_parseFromJSON(cJSON *item_delete_batch_recordJSON){
