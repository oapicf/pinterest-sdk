...
static catalogs_update_hotel_item_t *catalogs_update_hotel_item_create_internal(
    ...
    memset(catalogs_update_hotel_item_local_var, 0, sizeof(catalogs_update_hotel_item_t));
    catalogs_update_hotel_item_local_var->_library_owned = 1;
...
__attribute__((deprecated)) catalogs_update_hotel_item_t *catalogs_update_hotel_item_create(
    catalogs_updatable_hotel_attributes_t *attributes,
    char *hotel_id,
    pinterest_rest_api_catalogs_update_hotel_item_OPERATION_e operation
    ) {
    catalogs_update_hotel_item_t *result = catalogs_update_hotel_item_create_internal (
        attributes,
        hotel_id,
        operation
        );
...
void catalogs_update_hotel_item_free(catalogs_update_hotel_item_t *catalogs_update_hotel_item) {
    ...
    if(catalogs_update_hotel_item->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "catalogs_update_hotel_item_free");
        return ;
    }
...
cJSON *catalogs_update_hotel_item_convertToJSON(catalogs_update_hotel_item_t *catalogs_update_hotel_item) {
    ...
    if(cJSON_AddStringToObject(item, "operation", catalogs_update_hotel_item_operation_ToString(catalogs_update_hotel_item->operation)) == NULL)
    {
    goto fail; //Enum
    }
...
catalogs_update_hotel_item_t *catalogs_update_hotel_item_parseFromJSON(cJSON *catalogs_update_hotel_itemJSON){
    ...
    catalogs_update_hotel_item_local_var = catalogs_update_hotel_item_create_internal (
