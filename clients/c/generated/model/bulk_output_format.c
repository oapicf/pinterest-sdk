...
pinterest_rest_api_bulk_output_format__e bulk_output_format_parseFromJSON(cJSON *bulk_output_formatJSON) {
    if(!cJSON_IsString(bulk_output_formatJSON) || (bulk_output_formatJSON->valuestring == NULL)) {
        return 0;
    }
    return bulk_output_format_bulk_output_format_FromString(bulk_output_formatJSON->valuestring);
}
