#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "catalogs_creative_assets_item_response.h"


char* catalogs_creative_assets_item_response_catalog_type_ToString(pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_e catalog_type) {
    char* catalog_typeArray[] =  { "NULL", "CREATIVE_ASSETS" };
    return catalog_typeArray[catalog_type];
}

pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_e catalogs_creative_assets_item_response_catalog_type_FromString(char* catalog_type){
    int stringToReturn = 0;
    char *catalog_typeArray[] =  { "NULL", "CREATIVE_ASSETS" };
    size_t sizeofArray = sizeof(catalog_typeArray) / sizeof(catalog_typeArray[0]);
    while(stringToReturn < sizeofArray) {
        if(strcmp(catalog_type, catalog_typeArray[stringToReturn]) == 0) {
            return stringToReturn;
        }
        stringToReturn++;
    }
    return 0;
}
char* catalogs_creative_assets_item_response_item_response_kind_ToString(pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_e item_response_kind) {
    char* item_response_kindArray[] =  { "NULL", "creative_assets_item" };
    return item_response_kindArray[item_response_kind];
}

pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_e catalogs_creative_assets_item_response_item_response_kind_FromString(char* item_response_kind){
    int stringToReturn = 0;
    char *item_response_kindArray[] =  { "NULL", "creative_assets_item" };
    size_t sizeofArray = sizeof(item_response_kindArray) / sizeof(item_response_kindArray[0]);
    while(stringToReturn < sizeofArray) {
        if(strcmp(item_response_kind, item_response_kindArray[stringToReturn]) == 0) {
            return stringToReturn;
        }
        stringToReturn++;
    }
    return 0;
}

static catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response_create_internal(
    catalogs_creative_assets_attributes_t *attributes,
    pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_e catalog_type,
    char *creative_assets_id,
    pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_e item_response_kind,
    list_t *pins
    ) {
    catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response_local_var = malloc(sizeof(catalogs_creative_assets_item_response_t));
    if (!catalogs_creative_assets_item_response_local_var) {
        return NULL;
    }
    memset(catalogs_creative_assets_item_response_local_var, 0, sizeof(catalogs_creative_assets_item_response_t));
    catalogs_creative_assets_item_response_local_var->_library_owned = 1;
    catalogs_creative_assets_item_response_local_var->attributes = attributes;
    catalogs_creative_assets_item_response_local_var->catalog_type = catalog_type;
    catalogs_creative_assets_item_response_local_var->creative_assets_id = creative_assets_id;
    catalogs_creative_assets_item_response_local_var->item_response_kind = item_response_kind;
    catalogs_creative_assets_item_response_local_var->pins = pins;
    return catalogs_creative_assets_item_response_local_var;
}

__attribute__((deprecated)) catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response_create(
    catalogs_creative_assets_attributes_t *attributes,
    pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_e catalog_type,
    char *creative_assets_id,
    pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_e item_response_kind,
    list_t *pins
    ) {
    catalogs_creative_assets_item_response_t *result = catalogs_creative_assets_item_response_create_internal (
        attributes,
        catalog_type,
        creative_assets_id,
        item_response_kind,
        pins
        );
    if (!result) {
    }
    return result;
}

void catalogs_creative_assets_item_response_free(catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response) {
    if(NULL == catalogs_creative_assets_item_response){
        return ;
    }
    if(catalogs_creative_assets_item_response->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "catalogs_creative_assets_item_response_free");
        return ;
    }
    listEntry_t *listEntry;
    if (catalogs_creative_assets_item_response->attributes) {
        catalogs_creative_assets_attributes_free(catalogs_creative_assets_item_response->attributes);
        catalogs_creative_assets_item_response->attributes = NULL;
    }
    if (catalogs_creative_assets_item_response->creative_assets_id) {
        free(catalogs_creative_assets_item_response->creative_assets_id);
        catalogs_creative_assets_item_response->creative_assets_id = NULL;
    }
    if (catalogs_creative_assets_item_response->pins) {
        list_ForEach(listEntry, catalogs_creative_assets_item_response->pins) {
            pin_free(listEntry->data);
        }
        list_freeList(catalogs_creative_assets_item_response->pins);
        catalogs_creative_assets_item_response->pins = NULL;
    }
    free(catalogs_creative_assets_item_response);
}

cJSON *catalogs_creative_assets_item_response_convertToJSON(catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response) {
    cJSON *item = cJSON_CreateObject();

    // catalogs_creative_assets_item_response->attributes
    if(catalogs_creative_assets_item_response->attributes) {
    cJSON *attributes_local_JSON = catalogs_creative_assets_attributes_convertToJSON(catalogs_creative_assets_item_response->attributes);
    if(attributes_local_JSON == NULL) {
    goto fail; //model
    }
    cJSON_AddItemToObject(item, "attributes", attributes_local_JSON);
    if(item->child == NULL) {
    goto fail;
    }
    }


    // catalogs_creative_assets_item_response->catalog_type
    if (pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_NULL == catalogs_creative_assets_item_response->catalog_type) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "catalog_type", catalogs_creative_assets_item_response_catalog_type_ToString(catalogs_creative_assets_item_response->catalog_type)) == NULL)
    {
    goto fail; //Enum
    }


    // catalogs_creative_assets_item_response->creative_assets_id
    if(catalogs_creative_assets_item_response->creative_assets_id) {
    if(cJSON_AddStringToObject(item, "creative_assets_id", catalogs_creative_assets_item_response->creative_assets_id) == NULL) {
    goto fail; //String
    }
    }


    // catalogs_creative_assets_item_response->item_response_kind
    if (pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_NULL == catalogs_creative_assets_item_response->item_response_kind) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "item_response_kind", catalogs_creative_assets_item_response_item_response_kind_ToString(catalogs_creative_assets_item_response->item_response_kind)) == NULL)
    {
    goto fail; //Enum
    }


    // catalogs_creative_assets_item_response->pins
    if(catalogs_creative_assets_item_response->pins) {
    cJSON *pins = cJSON_AddArrayToObject(item, "pins");
    if(pins == NULL) {
    goto fail; //nonprimitive container
    }

    listEntry_t *pinsListEntry;
    if (catalogs_creative_assets_item_response->pins) {
    list_ForEach(pinsListEntry, catalogs_creative_assets_item_response->pins) {
    cJSON *itemLocal = pin_convertToJSON(pinsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    cJSON_AddItemToArray(pins, itemLocal);
    }
    }
    }

    return item;
fail:
    if (item) {
        cJSON_Delete(item);
    }
    return NULL;
}

catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response_parseFromJSON(cJSON *catalogs_creative_assets_item_responseJSON){

    catalogs_creative_assets_item_response_t *catalogs_creative_assets_item_response_local_var = NULL;

    // define the local variable for catalogs_creative_assets_item_response->attributes
    catalogs_creative_assets_attributes_t *attributes_local_nonprim = NULL;

    char *creative_assets_id_local_str = NULL;

    // define the local list for catalogs_creative_assets_item_response->pins
    list_t *pinsList = NULL;

    // catalogs_creative_assets_item_response->attributes
    cJSON *attributes = cJSON_GetObjectItemCaseSensitive(catalogs_creative_assets_item_responseJSON, "attributes");
    if (cJSON_IsNull(attributes)) {
        attributes = NULL;
    }
    if (attributes) { 
    attributes_local_nonprim = catalogs_creative_assets_attributes_parseFromJSON(attributes); //nonprimitive
    }

    // catalogs_creative_assets_item_response->catalog_type
    cJSON *catalog_type = cJSON_GetObjectItemCaseSensitive(catalogs_creative_assets_item_responseJSON, "catalog_type");
    if (cJSON_IsNull(catalog_type)) {
        catalog_type = NULL;
    }
    if (!catalog_type) {
        goto end;
    }

    pinterest_rest_api_catalogs_creative_assets_item_response_CATALOGTYPE_e catalog_typeVariable;
    
    if(!cJSON_IsString(catalog_type))
    {
    goto end; //Enum
    }
    catalog_typeVariable = catalogs_creative_assets_item_response_catalog_type_FromString(catalog_type->valuestring);

    // catalogs_creative_assets_item_response->creative_assets_id
    cJSON *creative_assets_id = cJSON_GetObjectItemCaseSensitive(catalogs_creative_assets_item_responseJSON, "creative_assets_id");
    if (cJSON_IsNull(creative_assets_id)) {
        creative_assets_id = NULL;
    }
    if (creative_assets_id) { 
    if(!cJSON_IsString(creative_assets_id) && !cJSON_IsNull(creative_assets_id))
    {
    goto end; //String
    }
    }

    // catalogs_creative_assets_item_response->item_response_kind
    cJSON *item_response_kind = cJSON_GetObjectItemCaseSensitive(catalogs_creative_assets_item_responseJSON, "item_response_kind");
    if (cJSON_IsNull(item_response_kind)) {
        item_response_kind = NULL;
    }
    if (!item_response_kind) {
        goto end;
    }

    pinterest_rest_api_catalogs_creative_assets_item_response_ITEMRESPONSEKIND_e item_response_kindVariable;
    
    if(!cJSON_IsString(item_response_kind))
    {
    goto end; //Enum
    }
    item_response_kindVariable = catalogs_creative_assets_item_response_item_response_kind_FromString(item_response_kind->valuestring);

    // catalogs_creative_assets_item_response->pins
    cJSON *pins = cJSON_GetObjectItemCaseSensitive(catalogs_creative_assets_item_responseJSON, "pins");
    if (cJSON_IsNull(pins)) {
        pins = NULL;
    }
    if (pins) { 
    cJSON *pins_local_nonprimitive = NULL;
    if(!cJSON_IsArray(pins)){
        goto end; //nonprimitive container
    }

    pinsList = list_createList();

    cJSON_ArrayForEach(pins_local_nonprimitive,pins )
    {
        if(!cJSON_IsObject(pins_local_nonprimitive)){
            goto end;
        }
        pin_t *pinsItem = pin_parseFromJSON(pins_local_nonprimitive);

        list_addElement(pinsList, pinsItem);
    }
    }


    if (creative_assets_id && !cJSON_IsNull(creative_assets_id)) creative_assets_id_local_str = strdup(creative_assets_id->valuestring);

    catalogs_creative_assets_item_response_local_var = catalogs_creative_assets_item_response_create_internal (
        attributes ? attributes_local_nonprim : NULL,
        catalog_typeVariable,
        creative_assets_id_local_str,
        item_response_kindVariable,
        pins ? pinsList : NULL
        );

    if (!catalogs_creative_assets_item_response_local_var) {
        goto end;
    }

    return catalogs_creative_assets_item_response_local_var;
end:
    if (attributes_local_nonprim) {
        catalogs_creative_assets_attributes_free(attributes_local_nonprim);
        attributes_local_nonprim = NULL;
    }
    if (creative_assets_id_local_str) {
        free(creative_assets_id_local_str);
        creative_assets_id_local_str = NULL;
    }
    if (pinsList) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, pinsList) {
            pin_free(listEntry->data);
            listEntry->data = NULL;
        }
        list_freeList(pinsList);
        pinsList = NULL;
    }
    return NULL;

}
