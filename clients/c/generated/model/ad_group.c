...
static ad_group_t *ad_group_create_internal(
    char *ad_account_id,
    ...
    ad_group_t *ad_group_local_var = malloc(sizeof(ad_group_t));
    if (!ad_group_local_var) {
        return NULL;
    }
    memset(ad_group_local_var, 0, sizeof(ad_group_t));
    ad_group_local_var->_library_owned = 1;
    ad_group_local_var->ad_account_id = ad_account_id;
    ad_group_local_var->bid_in_micro_currency = bid_in_micro_currency;
    ad_group_local_var->bid_strategy_type = bid_strategy_type;
    ad_group_local_var->billable_event = billable_event;
    ad_group_local_var->budget_in_micro_currency = budget_in_micro_currency;
    ad_group_local_var->campaign_id = campaign_id;
    ad_group_local_var->conversion_learning_mode_type = conversion_learning_mode_type;
    ad_group_local_var->created_time = created_time;
    ad_group_local_var->customer_segment_id = customer_segment_id;
    ad_group_local_var->dca_assets = dca_assets;
    ad_group_local_var->end_time = end_time;
    ad_group_local_var->ext_features = ext_features;
    ad_group_local_var->feed_profile_id = feed_profile_id;
    ad_group_local_var->id = id;
    ad_group_local_var->is_creative_optimization = is_creative_optimization;
    ad_group_local_var->is_local_inventory = is_local_inventory;
    ad_group_local_var->lifetime_frequency_cap = lifetime_frequency_cap;
    ad_group_local_var->local_inventory_radius_in_miles = local_inventory_radius_in_miles;
    ad_group_local_var->name = name;
    ad_group_local_var->optimization_goal_metadata = optimization_goal_metadata;
    ad_group_local_var->performance_plus_campaign_settings = performance_plus_campaign_settings;
    ad_group_local_var->placement_group = placement_group;
    ad_group_local_var->placement_traffic_type = placement_traffic_type;
    ad_group_local_var->promotion_application_level = promotion_application_level;
    ad_group_local_var->promotion_id = promotion_id;
    ad_group_local_var->promotion_ids = promotion_ids;
    ad_group_local_var->start_time = start_time;
    ad_group_local_var->status = status;
    ad_group_local_var->summary_status = summary_status;
    ad_group_local_var->targeting_spec = targeting_spec;
    ad_group_local_var->targeting_template_ids = targeting_template_ids;
    ad_group_local_var->tracking_urls = tracking_urls;
    ad_group_local_var->type = type;
    ad_group_local_var->updated_time = updated_time;
    ad_group_local_var->auto_targeting_enabled = auto_targeting_enabled;
    ad_group_local_var->bid_multiplier = bid_multiplier;
    ad_group_local_var->budget_type = budget_type;
    ad_group_local_var->pacing_delivery_type = pacing_delivery_type;
    return ad_group_local_var;
}
...
__attribute__((deprecated)) ad_group_t *ad_group_create(
    ...
    ad_group_t *result = ad_group_create_internal (
        ad_account_id,
...
void ad_group_free(ad_group_t *ad_group) {
    ...
    if(ad_group->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "ad_group_free");
        return ;
    }
    listEntry_t *listEntry;
    if (ad_group->ad_account_id) {
        free(ad_group->ad_account_id);
        ad_group->ad_account_id = NULL;
    }
    if (ad_group->bid_in_micro_currency) {
        free(ad_group->bid_in_micro_currency);
        ad_group->bid_in_micro_currency = NULL;
    }
    if (ad_group->budget_in_micro_currency) {
        free(ad_group->budget_in_micro_currency);
        ad_group->budget_in_micro_currency = NULL;
    }
    if (ad_group->campaign_id) {
        free(ad_group->campaign_id);
        ad_group->campaign_id = NULL;
    }
    if (ad_group->conversion_learning_mode_type) {
        conversion_learning_mode_type_free(ad_group->conversion_learning_mode_type);
        ad_group->conversion_learning_mode_type = NULL;
    }
    if (ad_group->created_time) {
        free(ad_group->created_time);
        ad_group->created_time = NULL;
    }
    if (ad_group->customer_segment_id) {
        free(ad_group->customer_segment_id);
        ad_group->customer_segment_id = NULL;
    }
    if (ad_group->dca_assets) {
        _free(ad_group->dca_assets);
        ad_group->dca_assets = NULL;
    }
    if (ad_group->end_time) {
        free(ad_group->end_time);
        ad_group->end_time = NULL;
    }
    if (ad_group->ext_features) {
        adgroup_tracking_features_free(ad_group->ext_features);
        ad_group->ext_features = NULL;
    }
    if (ad_group->feed_profile_id) {
        free(ad_group->feed_profile_id);
        ad_group->feed_profile_id = NULL;
    }
    if (ad_group->id) {
        free(ad_group->id);
        ad_group->id = NULL;
    }
    if (ad_group->is_creative_optimization) {
        free(ad_group->is_creative_optimization);
        ad_group->is_creative_optimization = NULL;
    }
    if (ad_group->is_local_inventory) {
        free(ad_group->is_local_inventory);
        ad_group->is_local_inventory = NULL;
    }
    if (ad_group->lifetime_frequency_cap) {
        free(ad_group->lifetime_frequency_cap);
        ad_group->lifetime_frequency_cap = NULL;
    }
    if (ad_group->local_inventory_radius_in_miles) {
        free(ad_group->local_inventory_radius_in_miles);
        ad_group->local_inventory_radius_in_miles = NULL;
    }
    if (ad_group->name) {
        free(ad_group->name);
        ad_group->name = NULL;
    }
    if (ad_group->optimization_goal_metadata) {
        nullable_optimization_goal_metadata_free(ad_group->optimization_goal_metadata);
        ad_group->optimization_goal_metadata = NULL;
    }
    if (ad_group->performance_plus_campaign_settings) {
        performance_plus_campaign_settings_free(ad_group->performance_plus_campaign_settings);
        ad_group->performance_plus_campaign_settings = NULL;
    }
    if (ad_group->placement_group) {
        placement_group_type_free(ad_group->placement_group);
        ad_group->placement_group = NULL;
    }
    if (ad_group->placement_traffic_type) {
        placement_traffic_type_free(ad_group->placement_traffic_type);
        ad_group->placement_traffic_type = NULL;
    }
    if (ad_group->promotion_application_level) {
        promotion_application_level_free(ad_group->promotion_application_level);
        ad_group->promotion_application_level = NULL;
    }
    if (ad_group->promotion_id) {
        free(ad_group->promotion_id);
        ad_group->promotion_id = NULL;
    }
    if (ad_group->promotion_ids) {
        list_ForEach(listEntry, ad_group->promotion_ids) {
            free(listEntry->data);
        }
        list_freeList(ad_group->promotion_ids);
        ad_group->promotion_ids = NULL;
    }
    if (ad_group->start_time) {
        free(ad_group->start_time);
        ad_group->start_time = NULL;
    }
    if (ad_group->status) {
        entity_status_free(ad_group->status);
        ad_group->status = NULL;
    }
    if (ad_group->summary_status) {
        summary_status_free(ad_group->summary_status);
        ad_group->summary_status = NULL;
    }
    if (ad_group->targeting_spec) {
        targeting_spec_optimal_free(ad_group->targeting_spec);
        ad_group->targeting_spec = NULL;
    }
    if (ad_group->targeting_template_ids) {
        list_ForEach(listEntry, ad_group->targeting_template_ids) {
            free(listEntry->data);
        }
        list_freeList(ad_group->targeting_template_ids);
        ad_group->targeting_template_ids = NULL;
    }
    if (ad_group->tracking_urls) {
        ad_group_tracking_urls_free(ad_group->tracking_urls);
        ad_group->tracking_urls = NULL;
    }
    if (ad_group->type) {
        free(ad_group->type);
        ad_group->type = NULL;
    }
    if (ad_group->updated_time) {
        free(ad_group->updated_time);
        ad_group->updated_time = NULL;
    }
    if (ad_group->auto_targeting_enabled) {
        free(ad_group->auto_targeting_enabled);
        ad_group->auto_targeting_enabled = NULL;
    }
    if (ad_group->bid_multiplier) {
        free(ad_group->bid_multiplier);
        ad_group->bid_multiplier = NULL;
    }
    free(ad_group);
}
...
cJSON *ad_group_convertToJSON(ad_group_t *ad_group) {
    ...
    // ad_group->ad_account_id
    if(ad_group->ad_account_id) {
    if(cJSON_AddStringToObject(item, "ad_account_id", ad_group->ad_account_id) == NULL) {
    goto fail; //String
    }
    }


    // ad_group->bid_in_micro_currency
    if(ad_group->bid_in_micro_currency) {
    if(cJSON_AddNumberToObject(item, "bid_in_micro_currency", *ad_group->bid_in_micro_currency) == NULL) {
    goto fail; //Numeric
    }
    }


    // ad_group->bid_strategy_type
    if(ad_group->bid_strategy_type != pinterest_rest_api_bid_strategy_type__NULL) {
    cJSON *bid_strategy_type_local_JSON = bid_strategy_type_convertToJSON(ad_group->bid_strategy_type);
    ...
    // ad_group->billable_event
    if (pinterest_rest_api_action_type__NULL == ad_group->billable_event) {
        goto fail;
    }
    cJSON *billable_event_local_JSON = action_type_convertToJSON(ad_group->billable_event);
    ...
    // ad_group->budget_in_micro_currency
    if(ad_group->budget_in_micro_currency) {
    if(cJSON_AddNumberToObject(item, "budget_in_micro_currency", *ad_group->budget_in_micro_currency) == NULL) {
    goto fail; //Numeric
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "campaign_id", ad_group->campaign_id) == NULL) {
    goto fail; //String
    }


    // ad_group->conversion_learning_mode_type
    if (!ad_group->conversion_learning_mode_type) {
        goto fail;
    }
    cJSON *conversion_learning_mode_type_local_JSON = conversion_learning_mode_type_convertToJSON(ad_group->conversion_learning_mode_type);
    ...
    // ad_group->created_time
    if (!ad_group->created_time) {
        goto fail;
    }
    if(cJSON_AddNumberToObject(item, "created_time", *ad_group->created_time) == NULL) {
    goto fail; //Numeric
    }


    // ad_group->customer_segment_id
    if(ad_group->customer_segment_id) {
    if(cJSON_AddStringToObject(item, "customer_segment_id", ad_group->customer_segment_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    // ad_group->dca_assets
    if(ad_group->dca_assets) {
    cJSON *dca_assets_local_JSON = _convertToJSON(ad_group->dca_assets);
    ...
    // ad_group->end_time
    if(ad_group->end_time) {
    if(cJSON_AddNumberToObject(item, "end_time", *ad_group->end_time) == NULL) {
    goto fail; //Numeric
    }
    }


    // ad_group->ext_features
    if(ad_group->ext_features) {
    cJSON *ext_features_local_JSON = adgroup_tracking_features_convertToJSON(ad_group->ext_features);
    ...
    // ad_group->feed_profile_id
    if(ad_group->feed_profile_id) {
    if(cJSON_AddStringToObject(item, "feed_profile_id", ad_group->feed_profile_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "id", ad_group->id) == NULL) {
    goto fail; //String
    }


    // ad_group->is_creative_optimization
    if(ad_group->is_creative_optimization) {
    if(cJSON_AddBoolToObject(item, "is_creative_optimization", *ad_group->is_creative_optimization) == NULL) {
    goto fail; //Bool
    }
    }


    // ad_group->is_local_inventory
    if(ad_group->is_local_inventory) {
    if(cJSON_AddBoolToObject(item, "is_local_inventory", *ad_group->is_local_inventory) == NULL) {
    goto fail; //Bool
    }
    }


    // ad_group->lifetime_frequency_cap
    if(ad_group->lifetime_frequency_cap) {
    if(cJSON_AddNumberToObject(item, "lifetime_frequency_cap", *ad_group->lifetime_frequency_cap) == NULL) {
    goto fail; //Numeric
    }
    }


    // ad_group->local_inventory_radius_in_miles
    if(ad_group->local_inventory_radius_in_miles) {
    if(cJSON_AddNumberToObject(item, "local_inventory_radius_in_miles", *ad_group->local_inventory_radius_in_miles) == NULL) {
    goto fail; //Numeric
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "name", ad_group->name) == NULL) {
    goto fail; //String
    }


    // ad_group->optimization_goal_metadata
    if(ad_group->optimization_goal_metadata) {
    cJSON *optimization_goal_metadata_local_JSON = nullable_optimization_goal_metadata_convertToJSON(ad_group->optimization_goal_metadata);
    ...
    // ad_group->performance_plus_campaign_settings
    if(ad_group->performance_plus_campaign_settings) {
    cJSON *performance_plus_campaign_settings_local_JSON = performance_plus_campaign_settings_convertToJSON(ad_group->performance_plus_campaign_settings);
    ...
    // ad_group->placement_group
    if(ad_group->placement_group) {
    cJSON *placement_group_local_JSON = placement_group_type_convertToJSON(ad_group->placement_group);
    ...
    // ad_group->placement_traffic_type
    if(ad_group->placement_traffic_type) {
    cJSON *placement_traffic_type_local_JSON = placement_traffic_type_convertToJSON(ad_group->placement_traffic_type);
    ...
    // ad_group->promotion_application_level
    if(ad_group->promotion_application_level) {
    cJSON *promotion_application_level_local_JSON = promotion_application_level_convertToJSON(ad_group->promotion_application_level);
    ...
    // ad_group->promotion_id
    if(ad_group->promotion_id) {
    if(cJSON_AddStringToObject(item, "promotion_id", ad_group->promotion_id) == NULL) {
    goto fail; //String
    }
    }


    // ad_group->promotion_ids
    if(ad_group->promotion_ids) {
    ...
    list_ForEach(promotion_idsListEntry, ad_group->promotion_ids) {
    ...
    // ad_group->start_time
    if(ad_group->start_time) {
    if(cJSON_AddNumberToObject(item, "start_time", *ad_group->start_time) == NULL) {
    goto fail; //Numeric
    }
    }
    ...
    // ad_group->status
    if(ad_group->status) {
    cJSON *status_local_JSON = entity_status_convertToJSON(ad_group->status);
    ...
    // ad_group->summary_status
    if (!ad_group->summary_status) {
        goto fail;
    }
    cJSON *summary_status_local_JSON = summary_status_convertToJSON(ad_group->summary_status);
    ...
    // ad_group->targeting_spec
    if(ad_group->targeting_spec) {
    cJSON *targeting_spec_local_JSON = targeting_spec_optimal_convertToJSON(ad_group->targeting_spec);
    ...
    // ad_group->targeting_template_ids
    if(ad_group->targeting_template_ids) {
    ...
    list_ForEach(targeting_template_idsListEntry, ad_group->targeting_template_ids) {
    ...
    // ad_group->tracking_urls
    if(ad_group->tracking_urls) {
    cJSON *tracking_urls_local_JSON = ad_group_tracking_urls_convertToJSON(ad_group->tracking_urls);
    ...
    if(cJSON_AddStringToObject(item, "type", ad_group->type) == NULL) {
    goto fail; //String
    }


    // ad_group->updated_time
    if (!ad_group->updated_time) {
        goto fail;
    }
    if(cJSON_AddNumberToObject(item, "updated_time", *ad_group->updated_time) == NULL) {
    goto fail; //Numeric
    }


    // ad_group->auto_targeting_enabled
    if(ad_group->auto_targeting_enabled) {
    if(cJSON_AddBoolToObject(item, "auto_targeting_enabled", *ad_group->auto_targeting_enabled) == NULL) {
    goto fail; //Bool
    }
    }


    // ad_group->bid_multiplier
    if(ad_group->bid_multiplier) {
    if(cJSON_AddNumberToObject(item, "bid_multiplier", *ad_group->bid_multiplier) == NULL) {
    goto fail; //Numeric
    }
    }


    // ad_group->budget_type
    if(ad_group->budget_type != pinterest_rest_api_budget_type__NULL) {
    cJSON *budget_type_local_JSON = budget_type_convertToJSON(ad_group->budget_type);
    ...
    // ad_group->pacing_delivery_type
    if(ad_group->pacing_delivery_type != pinterest_rest_api_pacing_delivery_type__NULL) {
    cJSON *pacing_delivery_type_local_JSON = pacing_delivery_type_convertToJSON(ad_group->pacing_delivery_type);
...
ad_group_t *ad_group_parseFromJSON(cJSON *ad_groupJSON){
    ...
    ad_group_t *ad_group_local_var = NULL;

    char *ad_account_id_local_str = NULL;

    // define the local variable for ad_group->bid_in_micro_currency
    int *bid_in_micro_currency_local_var = NULL;

    // define the local variable for ad_group->bid_strategy_type
    pinterest_rest_api_bid_strategy_type__e bid_strategy_type_local_nonprim = 0;

    // define the local variable for ad_group->billable_event
    pinterest_rest_api_action_type__e billable_event_local_nonprim = 0;

    // define the local variable for ad_group->budget_in_micro_currency
    int *budget_in_micro_currency_local_var = NULL;

    char *campaign_id_local_str = NULL;

    // define the local variable for ad_group->conversion_learning_mode_type
    conversion_learning_mode_type_t *conversion_learning_mode_type_local_nonprim = NULL;

    // define the local variable for ad_group->created_time
    int *created_time_local_var = NULL;

    char *customer_segment_id_local_str = NULL;

    // define the local variable for ad_group->dca_assets
    _t *dca_assets_local_nonprim = NULL;

    // define the local variable for ad_group->end_time
    int *end_time_local_var = NULL;

    // define the local variable for ad_group->ext_features
    ...
    // define the local variable for ad_group->is_creative_optimization
    int *is_creative_optimization_local_var = NULL;

    // define the local variable for ad_group->is_local_inventory
    int *is_local_inventory_local_var = NULL;

    // define the local variable for ad_group->lifetime_frequency_cap
    int *lifetime_frequency_cap_local_var = NULL;

    // define the local variable for ad_group->local_inventory_radius_in_miles
    double *local_inventory_radius_in_miles_local_var = NULL;

    char *name_local_str = NULL;

    // define the local variable for ad_group->optimization_goal_metadata
    nullable_optimization_goal_metadata_t *optimization_goal_metadata_local_nonprim = NULL;

    // define the local variable for ad_group->performance_plus_campaign_settings
    performance_plus_campaign_settings_t *performance_plus_campaign_settings_local_nonprim = NULL;

    // define the local variable for ad_group->placement_group
    placement_group_type_t *placement_group_local_nonprim = NULL;

    // define the local variable for ad_group->placement_traffic_type
    placement_traffic_type_t *placement_traffic_type_local_nonprim = NULL;

    // define the local variable for ad_group->promotion_application_level
    promotion_application_level_t *promotion_application_level_local_nonprim = NULL;

    char *promotion_id_local_str = NULL;

    // define the local list for ad_group->promotion_ids
    list_t *promotion_idsList = NULL;

    // define the local variable for ad_group->start_time
    int *start_time_local_var = NULL;

    // define the local variable for ad_group->status
    entity_status_t *status_local_nonprim = NULL;

    // define the local variable for ad_group->summary_status
    summary_status_t *summary_status_local_nonprim = NULL;

    // define the local variable for ad_group->targeting_spec
    targeting_spec_optimal_t *targeting_spec_local_nonprim = NULL;

    // define the local list for ad_group->targeting_template_ids
    list_t *targeting_template_idsList = NULL;

    // define the local variable for ad_group->tracking_urls
    ad_group_tracking_urls_t *tracking_urls_local_nonprim = NULL;

    char *type_local_str = NULL;

    // define the local variable for ad_group->updated_time
    int *updated_time_local_var = NULL;

    // define the local variable for ad_group->auto_targeting_enabled
    int *auto_targeting_enabled_local_var = NULL;

    // define the local variable for ad_group->bid_multiplier
    double *bid_multiplier_local_var = NULL;

    // define the local variable for ad_group->budget_type
    pinterest_rest_api_budget_type__e budget_type_local_nonprim = 0;

    // define the local variable for ad_group->pacing_delivery_type
    pinterest_rest_api_pacing_delivery_type__e pacing_delivery_type_local_nonprim = 0;

    // ad_group->ad_account_id
    cJSON *ad_account_id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "ad_account_id");
    ...
    // ad_group->bid_in_micro_currency
    cJSON *bid_in_micro_currency = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "bid_in_micro_currency");
    ...
    *bid_in_micro_currency_local_var = bid_in_micro_currency->valuedouble;
    ...
    // ad_group->bid_strategy_type
    cJSON *bid_strategy_type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "bid_strategy_type");
    ...
    // ad_group->billable_event
    cJSON *billable_event = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "billable_event");
    ...
    // ad_group->budget_in_micro_currency
    cJSON *budget_in_micro_currency = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "budget_in_micro_currency");
    ...
    *budget_in_micro_currency_local_var = budget_in_micro_currency->valuedouble;
    ...
    // ad_group->campaign_id
    cJSON *campaign_id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "campaign_id");
    ...
    // ad_group->conversion_learning_mode_type
    cJSON *conversion_learning_mode_type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "conversion_learning_mode_type");
    ...
    // ad_group->created_time
    cJSON *created_time = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "created_time");
    ...
    *created_time_local_var = created_time->valuedouble;
    ...
    // ad_group->customer_segment_id
    cJSON *customer_segment_id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "customer_segment_id");
    ...
    // ad_group->dca_assets
    cJSON *dca_assets = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "dca_assets");
    ...
    // ad_group->end_time
    cJSON *end_time = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "end_time");
    ...
    *end_time_local_var = end_time->valuedouble;
    ...
    // ad_group->ext_features
    cJSON *ext_features = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "ext_features");
    ...
    // ad_group->feed_profile_id
    cJSON *feed_profile_id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "feed_profile_id");
    ...
    // ad_group->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "id");
    if (cJSON_IsNull(id)) {
        id = NULL;
    }
    if (!id) {
    ...
    // ad_group->is_creative_optimization
    cJSON *is_creative_optimization = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "is_creative_optimization");
    ...
    *is_creative_optimization_local_var = is_creative_optimization->valueint;
    ...
    // ad_group->is_local_inventory
    cJSON *is_local_inventory = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "is_local_inventory");
    ...
    *is_local_inventory_local_var = is_local_inventory->valueint;
    ...
    // ad_group->lifetime_frequency_cap
    cJSON *lifetime_frequency_cap = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "lifetime_frequency_cap");
    ...
    *lifetime_frequency_cap_local_var = lifetime_frequency_cap->valuedouble;
    ...
    // ad_group->local_inventory_radius_in_miles
    cJSON *local_inventory_radius_in_miles = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "local_inventory_radius_in_miles");
    ...
    *local_inventory_radius_in_miles_local_var = local_inventory_radius_in_miles->valuedouble;
    ...
    // ad_group->name
    cJSON *name = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "name");
    ...
    // ad_group->optimization_goal_metadata
    cJSON *optimization_goal_metadata = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "optimization_goal_metadata");
    ...
    // ad_group->performance_plus_campaign_settings
    cJSON *performance_plus_campaign_settings = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "performance_plus_campaign_settings");
    ...
    // ad_group->placement_group
    cJSON *placement_group = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "placement_group");
    ...
    // ad_group->placement_traffic_type
    cJSON *placement_traffic_type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "placement_traffic_type");
    ...
    // ad_group->promotion_application_level
    cJSON *promotion_application_level = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "promotion_application_level");
    ...
    // ad_group->promotion_id
    cJSON *promotion_id = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "promotion_id");
    ...
    // ad_group->promotion_ids
    cJSON *promotion_ids = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "promotion_ids");
    ...
    // ad_group->start_time
    cJSON *start_time = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "start_time");
    ...
    *start_time_local_var = start_time->valuedouble;
    ...
    // ad_group->status
    cJSON *status = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "status");
    ...
    // ad_group->summary_status
    cJSON *summary_status = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "summary_status");
    ...
    // ad_group->targeting_spec
    cJSON *targeting_spec = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "targeting_spec");
    ...
    // ad_group->targeting_template_ids
    cJSON *targeting_template_ids = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "targeting_template_ids");
    ...
    // ad_group->tracking_urls
    cJSON *tracking_urls = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "tracking_urls");
    ...
    // ad_group->type
    cJSON *type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "type");
    ...
    // ad_group->updated_time
    cJSON *updated_time = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "updated_time");
    ...
    *updated_time_local_var = updated_time->valuedouble;
    ...
    // ad_group->auto_targeting_enabled
    cJSON *auto_targeting_enabled = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "auto_targeting_enabled");
    ...
    *auto_targeting_enabled_local_var = auto_targeting_enabled->valueint;
    ...
    // ad_group->bid_multiplier
    cJSON *bid_multiplier = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "bid_multiplier");
    ...
    *bid_multiplier_local_var = bid_multiplier->valuedouble;
    ...
    // ad_group->budget_type
    cJSON *budget_type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "budget_type");
    ...
    // ad_group->pacing_delivery_type
    cJSON *pacing_delivery_type = cJSON_GetObjectItemCaseSensitive(ad_groupJSON, "pacing_delivery_type");
    ...
    ad_group_local_var = ad_group_create_internal (
