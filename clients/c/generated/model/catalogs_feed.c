...
static catalogs_feed_t *catalogs_feed_create_internal(
    ...
    memset(catalogs_feed_local_var, 0, sizeof(catalogs_feed_t));
...
__attribute__((deprecated)) catalogs_feed_t *catalogs_feed_create(
    ...
    catalogs_feed_t *result = catalogs_feed_create_internal (
        catalog_type,
        created_at,
        credentials,
...
cJSON *catalogs_feed_convertToJSON(catalogs_feed_t *catalogs_feed) {
    ...
    if (pinterest_rest_api_catalogs_feed_CATALOGTYPE_NULL == catalogs_feed->catalog_type) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "catalog_type", catalogs_feed_catalog_type_ToString(catalogs_feed->catalog_type)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(catalogs_feed->default_availability != pinterest_rest_api_product_availability_type__NULL) {
    ...
    if(catalogs_feed->default_currency != pinterest_rest_api_nullable_currency__NULL) {
...
catalogs_feed_t *catalogs_feed_parseFromJSON(cJSON *catalogs_feedJSON){
    ...
    pinterest_rest_api_catalogs_feed_CATALOGTYPE_e catalog_typeVariable;
    
    if(!cJSON_IsString(catalog_type))
    {
    goto end; //Enum
    }
    catalog_typeVariable = catalogs_feed_catalog_type_FromString(catalog_type->valuestring);
