...
ad_group_common_tracking_urls_t *ad_group_common_tracking_urls_create(
    ...
    ad_group_common_tracking_urls_t *ad_group_common_tracking_urls_local_var = malloc(sizeof(ad_group_common_tracking_urls_t));
    if (!ad_group_common_tracking_urls_local_var) {
        return NULL;
    }
    ad_group_common_tracking_urls_local_var->impression = impression;
    ad_group_common_tracking_urls_local_var->click = click;
    ad_group_common_tracking_urls_local_var->engagement = engagement;
    ad_group_common_tracking_urls_local_var->buyable_button = buyable_button;
    ad_group_common_tracking_urls_local_var->audience_verification = audience_verification;

    return ad_group_common_tracking_urls_local_var;
}


void ad_group_common_tracking_urls_free(ad_group_common_tracking_urls_t *ad_group_common_tracking_urls) {
    if(NULL == ad_group_common_tracking_urls){
        return ;
    }
    listEntry_t *listEntry;
    if (ad_group_common_tracking_urls->impression) {
        list_ForEach(listEntry, ad_group_common_tracking_urls->impression) {
            free(listEntry->data);
        }
        list_freeList(ad_group_common_tracking_urls->impression);
        ad_group_common_tracking_urls->impression = NULL;
    }
    if (ad_group_common_tracking_urls->click) {
        list_ForEach(listEntry, ad_group_common_tracking_urls->click) {
            free(listEntry->data);
        }
        list_freeList(ad_group_common_tracking_urls->click);
        ad_group_common_tracking_urls->click = NULL;
    }
    if (ad_group_common_tracking_urls->engagement) {
        list_ForEach(listEntry, ad_group_common_tracking_urls->engagement) {
            free(listEntry->data);
        }
        list_freeList(ad_group_common_tracking_urls->engagement);
        ad_group_common_tracking_urls->engagement = NULL;
    }
    if (ad_group_common_tracking_urls->buyable_button) {
        list_ForEach(listEntry, ad_group_common_tracking_urls->buyable_button) {
            free(listEntry->data);
        }
        list_freeList(ad_group_common_tracking_urls->buyable_button);
        ad_group_common_tracking_urls->buyable_button = NULL;
    }
    if (ad_group_common_tracking_urls->audience_verification) {
        list_ForEach(listEntry, ad_group_common_tracking_urls->audience_verification) {
            free(listEntry->data);
        }
        list_freeList(ad_group_common_tracking_urls->audience_verification);
        ad_group_common_tracking_urls->audience_verification = NULL;
    }
    free(ad_group_common_tracking_urls);
}

cJSON *ad_group_common_tracking_urls_convertToJSON(ad_group_common_tracking_urls_t *ad_group_common_tracking_urls) {
    cJSON *item = cJSON_CreateObject();

    // ad_group_common_tracking_urls->impression
    if(ad_group_common_tracking_urls->impression) {
    ...
    list_ForEach(impressionListEntry, ad_group_common_tracking_urls->impression) {
    ...
    // ad_group_common_tracking_urls->click
    if(ad_group_common_tracking_urls->click) {
    ...
    list_ForEach(clickListEntry, ad_group_common_tracking_urls->click) {
    ...
    // ad_group_common_tracking_urls->engagement
    if(ad_group_common_tracking_urls->engagement) {
    ...
    list_ForEach(engagementListEntry, ad_group_common_tracking_urls->engagement) {
    ...
    // ad_group_common_tracking_urls->buyable_button
    if(ad_group_common_tracking_urls->buyable_button) {
    ...
    list_ForEach(buyable_buttonListEntry, ad_group_common_tracking_urls->buyable_button) {
    ...
    // ad_group_common_tracking_urls->audience_verification
    if(ad_group_common_tracking_urls->audience_verification) {
    ...
    list_ForEach(audience_verificationListEntry, ad_group_common_tracking_urls->audience_verification) {
...
ad_group_common_tracking_urls_t *ad_group_common_tracking_urls_parseFromJSON(cJSON *ad_group_common_tracking_urlsJSON){
    ...
    ad_group_common_tracking_urls_t *ad_group_common_tracking_urls_local_var = NULL;

    // define the local list for ad_group_common_tracking_urls->impression
    list_t *impressionList = NULL;

    // define the local list for ad_group_common_tracking_urls->click
    list_t *clickList = NULL;

    // define the local list for ad_group_common_tracking_urls->engagement
    list_t *engagementList = NULL;

    // define the local list for ad_group_common_tracking_urls->buyable_button
    list_t *buyable_buttonList = NULL;

    // define the local list for ad_group_common_tracking_urls->audience_verification
    list_t *audience_verificationList = NULL;

    // ad_group_common_tracking_urls->impression
    cJSON *impression = cJSON_GetObjectItemCaseSensitive(ad_group_common_tracking_urlsJSON, "impression");
    if (impression) { 
    ...
    // ad_group_common_tracking_urls->click
    cJSON *click = cJSON_GetObjectItemCaseSensitive(ad_group_common_tracking_urlsJSON, "click");
    if (click) { 
    ...
    // ad_group_common_tracking_urls->engagement
    cJSON *engagement = cJSON_GetObjectItemCaseSensitive(ad_group_common_tracking_urlsJSON, "engagement");
    if (engagement) { 
    ...
    // ad_group_common_tracking_urls->buyable_button
    cJSON *buyable_button = cJSON_GetObjectItemCaseSensitive(ad_group_common_tracking_urlsJSON, "buyable_button");
    if (buyable_button) { 
    ...
    // ad_group_common_tracking_urls->audience_verification
    cJSON *audience_verification = cJSON_GetObjectItemCaseSensitive(ad_group_common_tracking_urlsJSON, "audience_verification");
    ...
    ad_group_common_tracking_urls_local_var = ad_group_common_tracking_urls_create (
