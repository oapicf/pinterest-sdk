...
#include <string.h>
#include <stdio.h>
#include "advanced_auction_items_submit_request.h"



static advanced_auction_items_submit_request_t *advanced_auction_items_submit_request_create_internal(
    char *catalog_id,
    list_t *items
    ) {
    advanced_auction_items_submit_request_t *advanced_auction_items_submit_request_local_var = malloc(sizeof(advanced_auction_items_submit_request_t));
...
void advanced_auction_items_submit_request_free(advanced_auction_items_submit_request_t *advanced_auction_items_submit_request) {
    if(NULL == advanced_auction_items_submit_request){
        return ;
    }
    if(advanced_auction_items_submit_request->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "advanced_auction_items_submit_request_free");
        return ;
    }
    listEntry_t *listEntry;
    if (advanced_auction_items_submit_request->catalog_id) {
        free(advanced_auction_items_submit_request->catalog_id);
        advanced_auction_items_submit_request->catalog_id = NULL;
    }
    if (advanced_auction_items_submit_request->items) {
        list_ForEach(listEntry, advanced_auction_items_submit_request->items) {
            advanced_auction_items_submit_record_free(listEntry->data);
        }
        list_freeList(advanced_auction_items_submit_request->items);
        advanced_auction_items_submit_request->items = NULL;
    }
    free(advanced_auction_items_submit_request);
}
...
cJSON *advanced_auction_items_submit_request_convertToJSON(advanced_auction_items_submit_request_t *advanced_auction_items_submit_request) {
    ...
    // advanced_auction_items_submit_request->catalog_id
    if (!advanced_auction_items_submit_request->catalog_id) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "catalog_id", advanced_auction_items_submit_request->catalog_id) == NULL) {
    goto fail; //String
    }


    // advanced_auction_items_submit_request->items
    if (!advanced_auction_items_submit_request->items) {
        goto fail;
    }
    ...
    if (advanced_auction_items_submit_request->items) {
    list_ForEach(itemsListEntry, advanced_auction_items_submit_request->items) {
...
advanced_auction_items_submit_request_t *advanced_auction_items_submit_request_parseFromJSON(cJSON *advanced_auction_items_submit_requestJSON){
    ...
    // advanced_auction_items_submit_request->catalog_id
    cJSON *catalog_id = cJSON_GetObjectItemCaseSensitive(advanced_auction_items_submit_requestJSON, "catalog_id");
    ...
    // advanced_auction_items_submit_request->items
    cJSON *items = cJSON_GetObjectItemCaseSensitive(advanced_auction_items_submit_requestJSON, "items");
