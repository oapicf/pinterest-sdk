...
pinterest_rest_api_pacing_delivery_type__e pacing_delivery_type_parseFromJSON(cJSON *pacing_delivery_typeJSON) {
    if(!cJSON_IsString(pacing_delivery_typeJSON) || (pacing_delivery_typeJSON->valuestring == NULL)) {
        return 0;
    }
    return pacing_delivery_type_pacing_delivery_type_FromString(pacing_delivery_typeJSON->valuestring);
}
