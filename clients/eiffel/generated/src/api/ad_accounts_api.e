...
feature -- API Access
	...
	ad_account_analytics (start_date: DATE; end_date: DATE; columns: LIST [REPORTING_COLUMN_SYNC]; granularity: GRANULARITY; ad_account_id: STRING_32; click_window_days: REAL_32; engagement_window_days: REAL_32; view_window_days: REAL_32; conversion_report_time: STRING_32; reporting_timezone: detachable REPORTING_TIME_ZONE): detachable LIST [AD_ACCOUNT_ANALYTICS_ITEMS]
			...
			-- 
			-- 
			-- Result LIST [AD_ACCOUNT_ANALYTICS_ITEMS]
		require
		local
  			l_path: STRING
		...
		do
			...
			elseif attached { LIST [AD_ACCOUNT_ANALYTICS_ITEMS] } l_response.data ({ LIST [AD_ACCOUNT_ANALYTICS_ITEMS] }) as l_data then
				Result := l_data
			else
	...
	ad_accounts_create (ad_account_create: AD_ACCOUNT_CREATE): detachable AD_ACCOUNT
			...
			-- 
			-- argument: ad_account_create  (required)
			-- 
			-- 
			-- Result AD_ACCOUNT
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(ad_account_create)
			l_path := "/ad_accounts"
	...
	ad_accounts_list (include_shared_accounts: BOOLEAN; bookmark: STRING_32; page_size: INTEGER_32): detachable AD_ACCOUNTS_LIST_200_RESPONSE
			-- List ad accounts
	...
	analytics_create_conversion_product_report (ad_account_id: STRING_32; conversion_product_report_create: CONVERSION_PRODUCT_REPORT_CREATE): detachable CONVERSION_PRODUCT_REPORT
			...
			-- argument: ad_account_id Unique identifier of an ad account. (required)
			-- 
			-- argument: conversion_product_report_create  (required)
			-- 
			-- 
			-- Result CONVERSION_PRODUCT_REPORT
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			reset_error
			create l_request
			l_request.set_body(conversion_product_report_create)
			l_path := "/ad_accounts/{ad_account_id}/reports/brand_category_sku"
			l_path.replace_substring_all ("{"+"ad_account_id"+"}", api_client.url_encode (ad_account_id.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { CONVERSION_PRODUCT_REPORT } l_response.data ({ CONVERSION_PRODUCT_REPORT }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	analytics_create_mmm_report (ad_account_id: STRING_32; mmm_report_create: MMM_REPORT_CREATE): detachable MMM_REPORT
			...
			-- argument: ad_account_id  (required)
			-- 
			-- argument: mmm_report_create  (required)
			-- 
			-- 
			-- Result MMM_REPORT
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(mmm_report_create)
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { MMM_REPORT } l_response.data ({ MMM_REPORT }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	analytics_create_template_report (ad_account_id: STRING_32; template_id: STRING_32; start_date: DATE; end_date: DATE; granularity: detachable GRANULARITY): detachable TEMPLATE_BASED_REPORT
			...
			-- 
			-- argument: granularity   TOTAL - metrics are aggregated over the specified date range.    DAY - metrics are broken down daily.    HOUR - metrics are broken down hourly.    WEEK - metrics are broken down weekly.    MONTH - metrics are broken down monthly (optional, default to null)
			-- 
			-- 
			-- Result TEMPLATE_BASED_REPORT
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { TEMPLATE_BASED_REPORT } l_response.data ({ TEMPLATE_BASED_REPORT }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	templates_list (ad_account_id: STRING_32; bookmark: STRING_32; page_size: INTEGER_32; order: detachable PINTEREST_LIB_PAGINATION_ORDER): detachable TEMPLATES_LIST_200_RESPONSE
			-- List templates
