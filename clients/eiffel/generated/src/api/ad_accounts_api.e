...
feature -- API Access
	...
	analytics_create_conversion_product_report (ad_account_id: STRING_32; conversion_product_report_request: CONVERSION_PRODUCT_REPORT_REQUEST): detachable ADS_ANALYTICS_CREATE_ASYNC_RESPONSE
			...
			-- argument: ad_account_id Unique identifier of an ad account. (required)
			-- 
			-- argument: conversion_product_report_request  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(conversion_product_report_request)
	...
	analytics_create_template_report (ad_account_id: STRING_32; template_id: STRING_32; start_date: DATE; end_date: DATE; granularity: detachable GRANULARITY): detachable TEMPLATE_BASED_REPORT
			...
			-- 
			-- argument: granularity    TOTAL - metrics are aggregated over the specified date range.    DAY - metrics are broken down daily.    HOUR - metrics are broken down hourly.    WEEKLY - metrics are broken down weekly.    MONTHLY - metrics are broken down monthly (optional, default to null)
			-- 
			-- 
