...
feature -- API Access
	...
	search_user_pins_list (query: STRING_32; ad_account_id: STRING_32; bookmark: STRING_32): detachable SEARCH_USER_PINS_LIST_200_RESPONSE
			-- Search user&#39;s Pins
			...
			-- 
			-- 
			-- Result SEARCH_USER_PINS_LIST_200_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { SEARCH_USER_PINS_LIST_200_RESPONSE } l_response.data ({ SEARCH_USER_PINS_LIST_200_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
