...
feature -- API Access
	...
	audience_insights_get (ad_account_id: STRING_32; audience_insight_type: AUDIENCE_INSIGHT_TYPE): detachable AUDIENCE_INSIGHTS
		...
		do
			...
			elseif attached { AUDIENCE_INSIGHTS } l_response.data ({ AUDIENCE_INSIGHTS }) as l_data then
				Result := l_data
			else
	...
	audience_insights_scope_and_type_get (ad_account_id: STRING_32): detachable AUDIENCE_INSIGHTS_SCOPE_AND_TYPE_GET_200_RESPONSE
			...
			-- 
			-- 
			-- Result AUDIENCE_INSIGHTS_SCOPE_AND_TYPE_GET_200_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { AUDIENCE_INSIGHTS_SCOPE_AND_TYPE_GET_200_RESPONSE } l_response.data ({ AUDIENCE_INSIGHTS_SCOPE_AND_TYPE_GET_200_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
