...
feature -- API Access
	...
	cancel_invites_or_requests (business_id: STRING_32; cancel_invites_request: CANCEL_INVITES_REQUEST): detachable CANCEL_INVITES_RESPONSE
			...
			-- argument: business_id Unique identifier of the requesting business. (required)
			-- 
			-- argument: cancel_invites_request  (required)
			-- 
			-- 
			-- Result CANCEL_INVITES_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			reset_error
			create l_request
			l_request.set_body(cancel_invites_request)
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { CANCEL_INVITES_RESPONSE } l_response.data ({ CANCEL_INVITES_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	create_asset_invites (business_id: STRING_32; create_asset_invites_request: CREATE_ASSET_INVITES_REQUEST): detachable UPDATE_INVITES_RESULTS_RESPONSE_ARRAY
			...
			-- argument: business_id Unique identifier of the requesting business. (required)
			-- 
			-- argument: create_asset_invites_request  (required)
			-- 
			-- 
	...
	create_membership_or_partnership_invites (business_id: STRING_32; create_membership_or_partnership_invites_body: CREATE_MEMBERSHIP_OR_PARTNERSHIP_INVITES_BODY): detachable CREATE_INVITES_RESULTS_RESPONSE_ARRAY
			...
			-- argument: business_id Unique identifier of the requesting business. (required)
			-- 
			-- argument: create_membership_or_partnership_invites_body  (required)
			-- 
			-- 
	...
	invites (business_id: STRING_32; is_member: BOOLEAN; invite_status: detachable LIST [INVITE_FILTER_STATUS]; invite_type: detachable INVITE_TYPE; bookmark: STRING_32; page_size: INTEGER_32): detachable GET_INVITES_200_RESPONSE
