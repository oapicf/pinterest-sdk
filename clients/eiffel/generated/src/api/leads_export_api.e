...
feature -- API Access
	...
	leads_export_create (ad_account_id: STRING_32; leads_exports_create: LEADS_EXPORTS_CREATE): detachable LEADS_EXPORTS
			...
			-- 
			-- argument: leads_exports_create  (required)
			-- 
			-- 
			-- Result LEADS_EXPORTS
		require
		local
  			l_path: STRING
		...
		do
			reset_error
			create l_request
			l_request.set_body(leads_exports_create)
			...
			elseif attached { LEADS_EXPORTS } l_response.data ({ LEADS_EXPORTS }) as l_data then
				Result := l_data
			else
