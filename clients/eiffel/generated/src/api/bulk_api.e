...
feature -- API Access
	...
	bulk_download_create (ad_account_id: STRING_32; bulk_download_create: BULK_DOWNLOAD_CREATE): detachable BULK_DOWNLOAD
			...
			-- 
			-- argument: bulk_download_create  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(bulk_download_create)
			...
			elseif attached { BULK_DOWNLOAD } l_response.data ({ BULK_DOWNLOAD }) as l_data then
				Result := l_data
			else
	...
	bulk_request_get (ad_account_id: STRING_32; bulk_request_id: STRING_32; include_details: BOOLEAN): detachable BULK_JOB_DATA
			...
			-- argument: ad_account_id Unique identifier of an ad account. (required)
			-- 
			-- argument: bulk_request_id Bulk request ID that is from one of the entities bulk endpoints (required)
			-- 
			-- argument: include_details If set to True then attach the errors/details to all the requests (optional, default to false)
			-- 
			-- 
			-- Result BULK_JOB_DATA
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { BULK_JOB_DATA } l_response.data ({ BULK_JOB_DATA }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
