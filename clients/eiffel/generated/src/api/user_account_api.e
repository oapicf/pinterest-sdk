...
feature -- API Access
	...
	boards_user_follows_list (ad_account_id: STRING_32; explicit_following: BOOLEAN; bookmark: STRING_32; page_size: INTEGER_32): detachable BOARDS_LIST_200_RESPONSE
	...
	follow_user_update (username: STRING_32; follow_user_create: FOLLOW_USER_CREATE): detachable FOLLOW_USER
			...
			-- 
			-- argument: follow_user_create  (required)
			-- 
			-- 
			-- Result FOLLOW_USER
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(follow_user_create)
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { FOLLOW_USER } l_response.data ({ FOLLOW_USER }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	unverify_website_delete (website: STRING_32): detachable USER_WEBSITE
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { USER_WEBSITE } l_response.data ({ USER_WEBSITE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	user_account_analytics (start_date: DATE; end_date: DATE; from_claimed_content: STRING_32; pin_format: STRING_32; app_types: STRING_32; content_type: STRING_32; source: STRING_32; metric_types: detachable LIST [QUERYMETRICTYPES_ITEMS]; split_field: STRING_32; ad_account_id: STRING_32): detachable STRING_TABLE [ANALYTICS_METRICS_RESPONSE]
	...
	user_account_analytics_top_pins (start_date: DATE; end_date: DATE; sort_by: TOP_PINS_SORT_BY; from_claimed_content: STRING_32; pin_format: STRING_32; app_types: STRING_32; content_type: STRING_32; source: STRING_32; metric_types: detachable LIST [QUERYMETRICTYPES_ITEMS]; num_of_pins: INTEGER_32; created_in_last_n_days: REAL_32; ad_account_id: STRING_32): detachable TOP_PINS_ANALYTICS_RESPONSE
	...
	user_account_analytics_top_video_pins (start_date: DATE; end_date: DATE; sort_by: TOP_VIDEO_PINS_SORT_BY; from_claimed_content: STRING_32; pin_format: STRING_32; app_types: STRING_32; content_type: STRING_32; source: STRING_32; metric_types: detachable LIST [QUERYVIDEOPINMETRICTYPES_ITEMS]; num_of_pins: INTEGER_32; created_in_last_n_days: REAL_32; ad_account_id: STRING_32): detachable TOP_VIDEO_PINS_ANALYTICS_RESPONSE
	...
	user_following_get (ad_account_id: STRING_32; explicit_following: BOOLEAN; feed_type: detachable USER_FOLLOWING_FEED_TYPE; bookmark: STRING_32; page_size: INTEGER_32): detachable FOLLOWERS_LIST_200_RESPONSE
			-- List following
	...
	verify_website_update (user_website_create: USER_WEBSITE_CREATE; ad_account_id: STRING_32): detachable USER_WEBSITE
			-- Verify website
			-- Verify a website as a signed-in user.
			-- 
			-- argument: user_website_create  (required)
			-- 
			-- argument: ad_account_id Unique identifier of an ad account. (optional, default to null)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(user_website_create)
			l_path := "/user_account/websites"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "ad_account_id", ad_account_id));
	...
	website_verification_get (ad_account_id: STRING_32): detachable USER_WEBSITE_VERIFICATION
			...
			-- 
			-- 
			-- Result USER_WEBSITE_VERIFICATION
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { USER_WEBSITE_VERIFICATION } l_response.data ({ USER_WEBSITE_VERIFICATION }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
