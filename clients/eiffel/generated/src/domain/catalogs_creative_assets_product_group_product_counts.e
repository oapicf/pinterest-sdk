note
 description:"[
		Pinterest REST API
 		Pinterest's REST API
  		The version of the OpenAPI document: 5.28.0
 	    Contact: blah+oapicf@cliffano.com

  	NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

 		 Do not edit the class manually.
 	]"
	date: "$Date$"
	revision: "$Revision$"
	EIS:"Eiffel openapi generator", "src=https://openapi-generator.tech", "protocol=uri"
class CATALOGS_CREATIVE_ASSETS_PRODUCT_GROUP_PRODUCT_COUNTS




feature --Access

    app_links: REAL_32
      
    catalog_type: detachable STRING_32
      
    images: REAL_32
      
    total: REAL_32
      
    videos: REAL_32
      

feature -- Change Element

    set_app_links (a_name: like app_links)
        -- Set 'app_links' with 'a_name'.
      do
        app_links := a_name
      ensure
        app_links_set: app_links = a_name
      end

    set_catalog_type (a_name: like catalog_type)
        -- Set 'catalog_type' with 'a_name'.
      do
        catalog_type := a_name
      ensure
        catalog_type_set: catalog_type = a_name
      end

    set_images (a_name: like images)
        -- Set 'images' with 'a_name'.
      do
        images := a_name
      ensure
        images_set: images = a_name
      end

    set_total (a_name: like total)
        -- Set 'total' with 'a_name'.
      do
        total := a_name
      ensure
        total_set: total = a_name
      end

    set_videos (a_name: like videos)
        -- Set 'videos' with 'a_name'.
      do
        videos := a_name
      ensure
        videos_set: videos = a_name
      end


 feature -- Status Report

    output: STRING
          -- <Precursor>
      do
        create Result.make_empty
        Result.append("%Nclass CATALOGS_CREATIVE_ASSETS_PRODUCT_GROUP_PRODUCT_COUNTS%N")
        if attached app_links as l_app_links then
          Result.append ("%Napp_links:")
          Result.append (l_app_links.out)
          Result.append ("%N")
        end
        if attached catalog_type as l_catalog_type then
          Result.append ("%Ncatalog_type:")
          Result.append (l_catalog_type.out)
          Result.append ("%N")
        end
        if attached images as l_images then
          Result.append ("%Nimages:")
          Result.append (l_images.out)
          Result.append ("%N")
        end
        if attached total as l_total then
          Result.append ("%Ntotal:")
          Result.append (l_total.out)
          Result.append ("%N")
        end
        if attached videos as l_videos then
          Result.append ("%Nvideos:")
          Result.append (l_videos.out)
          Result.append ("%N")
        end
      end
end

