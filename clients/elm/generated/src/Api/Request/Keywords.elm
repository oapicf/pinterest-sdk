...
-}
keywordsGet : String -> Maybe String -> Maybe String -> Maybe (List String) -> Maybe (List MatchType) -> Maybe Int -> Maybe String -> Api.Request Api.Data.KeywordsGet200Response
keywordsGet adAccountId_path campaignId_query adGroupId_query adGroupIds_query matchTypes_query pageSize_query bookmark_query =
    Api.request
        "GET"
        "/ad_accounts/{ad_account_id}/keywords"
        [ ( "ad_account_id", identity adAccountId_path ) ]
        [ ( "campaign_id", Maybe.map identity campaignId_query ), ( "ad_group_id", Maybe.map identity adGroupId_query ), ( "ad_group_ids", Maybe.map (String.join "," << List.map identity) adGroupIds_query ), ( "match_types", Maybe.map (String.join "," << List.map Api.Data.stringFromMatchType) matchTypes_query ), ( "page_size", Maybe.map String.fromInt pageSize_query ), ( "bookmark", Maybe.map identity bookmark_query ) ]
        []
        Nothing
...
-}
trendingKeywordsList : TrendsSupportedRegion -> TrendType -> Maybe (List Interests) -> Maybe (List Genders) -> Maybe (List Ages) -> Maybe (List String) -> Maybe Bool -> Maybe Int -> Maybe Bool -> Maybe Bool -> Api.Request Api.Data.TrendingKeywordsResponse
trendingKeywordsList region_path trendType_path interests_query genders_query ages_query includeKeywords_query normalizeAgainstGroup_query limit_query includePrediction_query includeDemographics_query =
    Api.request
        "GET"
