...
-}
businessAssetMembersGet : String -> String -> Maybe Bool -> Maybe String -> Maybe Int -> Maybe Int -> Api.Request Api.Data.BusinessAssetMembersGet200Response
businessAssetMembersGet businessId_path assetId_path fetchSystemUsers_query bookmark_query pageSize_query startIndex_query =
    Api.request
        "GET"
        "/businesses/{business_id}/assets/{asset_id}/members"
        [ ( "business_id", identity businessId_path ), ( "asset_id", identity assetId_path ) ]
        [ ( "fetch_system_users", Maybe.map (\val -> if val then "true" else "false") fetchSystemUsers_query ), ( "bookmark", Maybe.map identity bookmark_query ), ( "page_size", Maybe.map String.fromInt pageSize_query ), ( "start_index", Maybe.map String.fromInt startIndex_query ) ]
        []
        Nothing
