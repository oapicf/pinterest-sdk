...
# **itemsBatchPost**
```swift
    open class func itemsBatchPost(catalogsItemsBatchPostRequest: CatalogsItemsBatchPostRequest, adAccountId: String? = nil, completion: @escaping (_ data: CatalogsItemsBatch?, _ error: Error?) -> Void)
```
...
let catalogsItemsBatchPostRequest = CatalogsItemsBatchPostRequest(catalogId: "catalogId_example", catalogType: "catalogType_example", country: Country(), items: [ItemDeleteBatchRecord(itemId: "itemId_example")], language: "language_example", operation: "operation_example") // CatalogsItemsBatchPostRequest | 
let adAccountId = "adAccountId_example" // String | Unique identifier of an ad account. (optional)

// Operate on item batch
CatalogItemsAPI.itemsBatchPost(catalogsItemsBatchPostRequest: catalogsItemsBatchPostRequest, adAccountId: adAccountId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **itemsPost**
```swift
    open class func itemsPost(catalogsItemsRequest: CatalogsItemsRequest, adAccountId: String? = nil, completion: @escaping (_ data: ItemsPost200Response?, _ error: Error?) -> Void)
```
...
let catalogsItemsRequest = CatalogsItemsRequest(country: Country(), filters: CatalogsItemsPostFilters(catalogId: "catalogId_example", catalogType: "catalogType_example", itemIds: ["itemIds_example"], hotelIds: ["hotelIds_example"], creativeAssetsIds: ["creativeAssetsIds_example"]), language: "language_example") // CatalogsItemsRequest | 
let adAccountId = "adAccountId_example" // String | Unique identifier of an ad account. (optional)
