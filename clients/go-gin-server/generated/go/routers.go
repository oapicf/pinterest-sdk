/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * API version: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi

import (
	"net/http"

	"github.com/gin-gonic/gin"
)

// Route is the information for every URI.
type Route struct {
	// Name is the name of this Route.
	Name		string
	// Method is the string for the HTTP method. ex) GET, POST etc..
	Method		string
	// Pattern is the pattern of the URI.
	Pattern	 	string
	// HandlerFunc is the handler function of this route.
	HandlerFunc	gin.HandlerFunc
}

// NewRouter returns a new router.
func NewRouter(handleFunctions ApiHandleFunctions) *gin.Engine {
	return NewRouterWithGinEngine(gin.Default(), handleFunctions)
}

// NewRouter add routes to existing gin engine.
func NewRouterWithGinEngine(router *gin.Engine, handleFunctions ApiHandleFunctions) *gin.Engine {
	for _, route := range getRoutes(handleFunctions) {
		if route.HandlerFunc == nil {
			route.HandlerFunc = DefaultHandleFunc
		}
		switch route.Method {
		case http.MethodGet:
			router.GET(route.Pattern, route.HandlerFunc)
		case http.MethodPost:
			router.POST(route.Pattern, route.HandlerFunc)
		case http.MethodPut:
			router.PUT(route.Pattern, route.HandlerFunc)
		case http.MethodPatch:
			router.PATCH(route.Pattern, route.HandlerFunc)
		case http.MethodDelete:
			router.DELETE(route.Pattern, route.HandlerFunc)
		}
	}

	return router
}

// Default handler for not yet implemented routes
func DefaultHandleFunc(c *gin.Context) {
	c.String(http.StatusNotImplemented, "501 not implemented")
}

type ApiHandleFunctions struct {

	// Routes for the AdAccountsAPI part of the API
	AdAccountsAPI AdAccountsAPI
	// Routes for the AdGroupsAPI part of the API
	AdGroupsAPI AdGroupsAPI
	// Routes for the AdsAPI part of the API
	AdsAPI AdsAPI
	// Routes for the AdvancedAuctionAPI part of the API
	AdvancedAuctionAPI AdvancedAuctionAPI
	// Routes for the AudienceInsightsAPI part of the API
	AudienceInsightsAPI AudienceInsightsAPI
	// Routes for the AudienceSharingAPI part of the API
	AudienceSharingAPI AudienceSharingAPI
	// Routes for the AudiencesAPI part of the API
	AudiencesAPI AudiencesAPI
	// Routes for the BillingAPI part of the API
	BillingAPI BillingAPI
	// Routes for the BoardsAPI part of the API
	BoardsAPI BoardsAPI
	// Routes for the BulkAPI part of the API
	BulkAPI BulkAPI
	// Routes for the BusinessAccessAssetsAPI part of the API
	BusinessAccessAssetsAPI BusinessAccessAssetsAPI
	// Routes for the BusinessAccessInviteAPI part of the API
	BusinessAccessInviteAPI BusinessAccessInviteAPI
	// Routes for the BusinessAccessRelationshipsAPI part of the API
	BusinessAccessRelationshipsAPI BusinessAccessRelationshipsAPI
	// Routes for the CampaignsAPI part of the API
	CampaignsAPI CampaignsAPI
	// Routes for the CatalogFeedsAPI part of the API
	CatalogFeedsAPI CatalogFeedsAPI
	// Routes for the CatalogItemsAPI part of the API
	CatalogItemsAPI CatalogItemsAPI
	// Routes for the CatalogProductGroupsAPI part of the API
	CatalogProductGroupsAPI CatalogProductGroupsAPI
	// Routes for the CatalogReportsAPI part of the API
	CatalogReportsAPI CatalogReportsAPI
	// Routes for the CatalogSupplementalAPI part of the API
	CatalogSupplementalAPI CatalogSupplementalAPI
	// Routes for the CatalogsAPI part of the API
	CatalogsAPI CatalogsAPI
	// Routes for the ConversionDeletionRequestsAPI part of the API
	ConversionDeletionRequestsAPI ConversionDeletionRequestsAPI
	// Routes for the ConversionEqsAPI part of the API
	ConversionEqsAPI ConversionEqsAPI
	// Routes for the ConversionEventsAPI part of the API
	ConversionEventsAPI ConversionEventsAPI
	// Routes for the ConversionTagsAPI part of the API
	ConversionTagsAPI ConversionTagsAPI
	// Routes for the ConversionsAPI part of the API
	ConversionsAPI ConversionsAPI
	// Routes for the CustomerListUploadsAPI part of the API
	CustomerListUploadsAPI CustomerListUploadsAPI
	// Routes for the CustomerListsAPI part of the API
	CustomerListsAPI CustomerListsAPI
	// Routes for the CustomerSegmentAPI part of the API
	CustomerSegmentAPI CustomerSegmentAPI
	// Routes for the IntegrationsAPI part of the API
	IntegrationsAPI IntegrationsAPI
	// Routes for the KeywordsAPI part of the API
	KeywordsAPI KeywordsAPI
	// Routes for the LabelsAPI part of the API
	LabelsAPI LabelsAPI
	// Routes for the LeadAdsAPI part of the API
	LeadAdsAPI LeadAdsAPI
	// Routes for the LeadFormsAPI part of the API
	LeadFormsAPI LeadFormsAPI
	// Routes for the LeadsExportAPI part of the API
	LeadsExportAPI LeadsExportAPI
	// Routes for the MediaAPI part of the API
	MediaAPI MediaAPI
	// Routes for the MsotEventsAPI part of the API
	MsotEventsAPI MsotEventsAPI
	// Routes for the NotificationAPI part of the API
	NotificationAPI NotificationAPI
	// Routes for the OauthAPI part of the API
	OauthAPI OauthAPI
	// Routes for the OrderLinesAPI part of the API
	OrderLinesAPI OrderLinesAPI
	// Routes for the PinsAPI part of the API
	PinsAPI PinsAPI
	// Routes for the ProductGroupPromotionsAPI part of the API
	ProductGroupPromotionsAPI ProductGroupPromotionsAPI
	// Routes for the ProductTagsAPI part of the API
	ProductTagsAPI ProductTagsAPI
	// Routes for the PromotionsAPI part of the API
	PromotionsAPI PromotionsAPI
	// Routes for the ResourcesAPI part of the API
	ResourcesAPI ResourcesAPI
	// Routes for the SchedulesAPI part of the API
	SchedulesAPI SchedulesAPI
	// Routes for the SearchAPI part of the API
	SearchAPI SearchAPI
	// Routes for the TargetingTemplateAPI part of the API
	TargetingTemplateAPI TargetingTemplateAPI
	// Routes for the TermsAPI part of the API
	TermsAPI TermsAPI
	// Routes for the TermsOfServiceAPI part of the API
	TermsOfServiceAPI TermsOfServiceAPI
	// Routes for the TrendsAPI part of the API
	TrendsAPI TrendsAPI
	// Routes for the UserAccountAPI part of the API
	UserAccountAPI UserAccountAPI
}

func getRoutes(handleFunctions ApiHandleFunctions) []Route {
	return []Route{ 
		{
			"AdAccountAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/analytics",
			handleFunctions.AdAccountsAPI.AdAccountAnalytics,
		},
		{
			"AdAccountTargetingAnalyticsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/targeting_analytics",
			handleFunctions.AdAccountsAPI.AdAccountTargetingAnalyticsGet,
		},
		{
			"AdAccountsCreate",
			http.MethodPost,
			"/v5/ad_accounts",
			handleFunctions.AdAccountsAPI.AdAccountsCreate,
		},
		{
			"AdAccountsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id",
			handleFunctions.AdAccountsAPI.AdAccountsGet,
		},
		{
			"AdAccountsList",
			http.MethodGet,
			"/v5/ad_accounts",
			handleFunctions.AdAccountsAPI.AdAccountsList,
		},
		{
			"AnalyticsCreateConversionProductReport",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/reports/brand_category_sku",
			handleFunctions.AdAccountsAPI.AnalyticsCreateConversionProductReport,
		},
		{
			"AnalyticsCreateMmmReport",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/mmm_reports",
			handleFunctions.AdAccountsAPI.AnalyticsCreateMmmReport,
		},
		{
			"AnalyticsCreateReport",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/reports",
			handleFunctions.AdAccountsAPI.AnalyticsCreateReport,
		},
		{
			"AnalyticsCreateTemplateReport",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/templates/:template_id/reports",
			handleFunctions.AdAccountsAPI.AnalyticsCreateTemplateReport,
		},
		{
			"AnalyticsGetConversionProductReport",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/reports/brand_category_sku",
			handleFunctions.AdAccountsAPI.AnalyticsGetConversionProductReport,
		},
		{
			"AnalyticsGetMmmReport",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/mmm_reports",
			handleFunctions.AdAccountsAPI.AnalyticsGetMmmReport,
		},
		{
			"AnalyticsGetReport",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/reports",
			handleFunctions.AdAccountsAPI.AnalyticsGetReport,
		},
		{
			"SandboxDelete",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/sandbox",
			handleFunctions.AdAccountsAPI.SandboxDelete,
		},
		{
			"TemplatesList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/templates",
			handleFunctions.AdAccountsAPI.TemplatesList,
		},
		{
			"AdGroupsAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/analytics",
			handleFunctions.AdGroupsAPI.AdGroupsAnalytics,
		},
		{
			"AdGroupsAudienceSizing",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ad_groups/audience_sizing",
			handleFunctions.AdGroupsAPI.AdGroupsAudienceSizing,
		},
		{
			"AdGroupsBidFloorGet",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/bid_floor",
			handleFunctions.AdGroupsAPI.AdGroupsBidFloorGet,
		},
		{
			"AdGroupsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ad_groups",
			handleFunctions.AdGroupsAPI.AdGroupsCreate,
		},
		{
			"AdGroupsDynamicTitlesDownloadCsv",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/:ad_group_id/dynamic_titles/csv",
			handleFunctions.AdGroupsAPI.AdGroupsDynamicTitlesDownloadCsv,
		},
		{
			"AdGroupsDynamicTitlesGetStatus",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/:ad_group_id/dynamic_titles/status",
			handleFunctions.AdGroupsAPI.AdGroupsDynamicTitlesGetStatus,
		},
		{
			"AdGroupsDynamicTitlesGetUploadUrl",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/:ad_group_id/dynamic_titles/uploads",
			handleFunctions.AdGroupsAPI.AdGroupsDynamicTitlesGetUploadUrl,
		},
		{
			"AdGroupsDynamicTitlesProcessCsv",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ad_groups/:ad_group_id/dynamic_titles",
			handleFunctions.AdGroupsAPI.AdGroupsDynamicTitlesProcessCsv,
		},
		{
			"AdGroupsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/:ad_group_id",
			handleFunctions.AdGroupsAPI.AdGroupsGet,
		},
		{
			"AdGroupsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups",
			handleFunctions.AdGroupsAPI.AdGroupsList,
		},
		{
			"AdGroupsTargetingAnalyticsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ad_groups/targeting_analytics",
			handleFunctions.AdGroupsAPI.AdGroupsTargetingAnalyticsGet,
		},
		{
			"AdGroupsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/ad_groups",
			handleFunctions.AdGroupsAPI.AdGroupsUpdate,
		},
		{
			"GetAdGroupsByPromotionIdsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/promotion_applied_entities",
			handleFunctions.AdGroupsAPI.GetAdGroupsByPromotionIdsList,
		},
		{
			"AdPreviewsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ad_previews",
			handleFunctions.AdsAPI.AdPreviewsCreate,
		},
		{
			"AdTargetingAnalyticsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ads/targeting_analytics",
			handleFunctions.AdsAPI.AdTargetingAnalyticsGet,
		},
		{
			"AdsAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ads/analytics",
			handleFunctions.AdsAPI.AdsAnalytics,
		},
		{
			"AdsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ads",
			handleFunctions.AdsAPI.AdsCreate,
		},
		{
			"AdsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ads/:ad_id",
			handleFunctions.AdsAPI.AdsGet,
		},
		{
			"AdsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ads",
			handleFunctions.AdsAPI.AdsList,
		},
		{
			"AdsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/ads",
			handleFunctions.AdsAPI.AdsUpdate,
		},
		{
			"CampaignAdPreviewCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/campaign_ad_preview",
			handleFunctions.AdsAPI.CampaignAdPreviewCreate,
		},
		{
			"CampaignAdPreviewDelete",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/campaign_ad_preview",
			handleFunctions.AdsAPI.CampaignAdPreviewDelete,
		},
		{
			"CampaignAdPreviewRead",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/campaign_ad_preview",
			handleFunctions.AdsAPI.CampaignAdPreviewRead,
		},
		{
			"AdvancedAuctionItemsGetPost",
			http.MethodPost,
			"/v5/advanced_auction/items/get",
			handleFunctions.AdvancedAuctionAPI.AdvancedAuctionItemsGetPost,
		},
		{
			"AdvancedAuctionItemsSubmitPost",
			http.MethodPost,
			"/v5/advanced_auction/items/submit",
			handleFunctions.AdvancedAuctionAPI.AdvancedAuctionItemsSubmitPost,
		},
		{
			"AudienceInsightsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/audience_insights",
			handleFunctions.AudienceInsightsAPI.AudienceInsightsGet,
		},
		{
			"AudienceInsightsScopeAndTypeGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/insights/audiences",
			handleFunctions.AudienceInsightsAPI.AudienceInsightsScopeAndTypeGet,
		},
		{
			"AdAccountsAudiencesSharedAccountsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/audiences/shared/accounts",
			handleFunctions.AudienceSharingAPI.AdAccountsAudiencesSharedAccountsList,
		},
		{
			"BusinessAccountAudiencesSharedAccountsList",
			http.MethodGet,
			"/v5/businesses/:business_id/audiences/shared/accounts",
			handleFunctions.AudienceSharingAPI.BusinessAccountAudiencesSharedAccountsList,
		},
		{
			"SharedAudiencesForBusinessList",
			http.MethodGet,
			"/v5/businesses/:business_id/audiences",
			handleFunctions.AudienceSharingAPI.SharedAudiencesForBusinessList,
		},
		{
			"UpdateAdAccountToAdAccountSharedAudience",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/audiences/ad_accounts/shared",
			handleFunctions.AudienceSharingAPI.UpdateAdAccountToAdAccountSharedAudience,
		},
		{
			"UpdateAdAccountToBusinessSharedAudience",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/audiences/businesses/shared",
			handleFunctions.AudienceSharingAPI.UpdateAdAccountToBusinessSharedAudience,
		},
		{
			"UpdateBusinessToAdAccountSharedAudience",
			http.MethodPatch,
			"/v5/businesses/:business_id/audiences/ad_accounts/shared",
			handleFunctions.AudienceSharingAPI.UpdateBusinessToAdAccountSharedAudience,
		},
		{
			"UpdateBusinessToBusinessSharedAudience",
			http.MethodPatch,
			"/v5/businesses/:business_id/audiences/businesses/shared",
			handleFunctions.AudienceSharingAPI.UpdateBusinessToBusinessSharedAudience,
		},
		{
			"AudiencesCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/audiences",
			handleFunctions.AudiencesAPI.AudiencesCreate,
		},
		{
			"AudiencesGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/audiences/:audience_id",
			handleFunctions.AudiencesAPI.AudiencesGet,
		},
		{
			"AudiencesList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/audiences",
			handleFunctions.AudiencesAPI.AudiencesList,
		},
		{
			"AudiencesUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/audiences/:audience_id",
			handleFunctions.AudiencesAPI.AudiencesUpdate,
		},
		{
			"AdsCreditRedeem",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ads_credit/redeem",
			handleFunctions.BillingAPI.AdsCreditRedeem,
		},
		{
			"AdsCreditsDiscountsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ads_credit/discounts",
			handleFunctions.BillingAPI.AdsCreditsDiscountsGet,
		},
		{
			"BillingInvoiceDownloadGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/billing_invoice/:billing_invoice_id/download",
			handleFunctions.BillingAPI.BillingInvoiceDownloadGet,
		},
		{
			"BillingInvoicesGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/billing_invoices",
			handleFunctions.BillingAPI.BillingInvoicesGet,
		},
		{
			"BillingProfilesGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/billing_profiles",
			handleFunctions.BillingAPI.BillingProfilesGet,
		},
		{
			"SsioAccountsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ssio/accounts",
			handleFunctions.BillingAPI.SsioAccountsGet,
		},
		{
			"SsioInsertionOrderCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/ssio/insertion_orders",
			handleFunctions.BillingAPI.SsioInsertionOrderCreate,
		},
		{
			"SsioInsertionOrderEdit",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/ssio/insertion_orders",
			handleFunctions.BillingAPI.SsioInsertionOrderEdit,
		},
		{
			"SsioInsertionOrdersStatusGetByAdAccount",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ssio/insertion_orders/status",
			handleFunctions.BillingAPI.SsioInsertionOrdersStatusGetByAdAccount,
		},
		{
			"SsioInsertionOrdersStatusGetByPinOrderId",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ssio/insertion_orders/:pin_order_id/status",
			handleFunctions.BillingAPI.SsioInsertionOrdersStatusGetByPinOrderId,
		},
		{
			"SsioOrderLinesGetByAdAccount",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/ssio/order_lines",
			handleFunctions.BillingAPI.SsioOrderLinesGetByAdAccount,
		},
		{
			"BoardSectionsCreate",
			http.MethodPost,
			"/v5/boards/:board_id/sections",
			handleFunctions.BoardsAPI.BoardSectionsCreate,
		},
		{
			"BoardSectionsDelete",
			http.MethodDelete,
			"/v5/boards/:board_id/sections/:section_id",
			handleFunctions.BoardsAPI.BoardSectionsDelete,
		},
		{
			"BoardSectionsList",
			http.MethodGet,
			"/v5/boards/:board_id/sections",
			handleFunctions.BoardsAPI.BoardSectionsList,
		},
		{
			"BoardSectionsListPins",
			http.MethodGet,
			"/v5/boards/:board_id/sections/:section_id/pins",
			handleFunctions.BoardsAPI.BoardSectionsListPins,
		},
		{
			"BoardSectionsUpdate",
			http.MethodPatch,
			"/v5/boards/:board_id/sections/:section_id",
			handleFunctions.BoardsAPI.BoardSectionsUpdate,
		},
		{
			"BoardsCreate",
			http.MethodPost,
			"/v5/boards",
			handleFunctions.BoardsAPI.BoardsCreate,
		},
		{
			"BoardsDelete",
			http.MethodDelete,
			"/v5/boards/:board_id",
			handleFunctions.BoardsAPI.BoardsDelete,
		},
		{
			"BoardsGet",
			http.MethodGet,
			"/v5/boards/:board_id",
			handleFunctions.BoardsAPI.BoardsGet,
		},
		{
			"BoardsList",
			http.MethodGet,
			"/v5/boards",
			handleFunctions.BoardsAPI.BoardsList,
		},
		{
			"BoardsListPins",
			http.MethodGet,
			"/v5/boards/:board_id/pins",
			handleFunctions.BoardsAPI.BoardsListPins,
		},
		{
			"BoardsUpdate",
			http.MethodPatch,
			"/v5/boards/:board_id",
			handleFunctions.BoardsAPI.BoardsUpdate,
		},
		{
			"BulkDownloadCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/bulk/download",
			handleFunctions.BulkAPI.BulkDownloadCreate,
		},
		{
			"BulkRequestGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/bulk/:bulk_request_id",
			handleFunctions.BulkAPI.BulkRequestGet,
		},
		{
			"BulkUpsertCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/bulk/upsert",
			handleFunctions.BulkAPI.BulkUpsertCreate,
		},
		{
			"AssetGroupCreate",
			http.MethodPost,
			"/v5/businesses/:business_id/asset_groups",
			handleFunctions.BusinessAccessAssetsAPI.AssetGroupCreate,
		},
		{
			"AssetGroupDelete",
			http.MethodDelete,
			"/v5/businesses/:business_id/asset_groups",
			handleFunctions.BusinessAccessAssetsAPI.AssetGroupDelete,
		},
		{
			"AssetGroupUpdate",
			http.MethodPatch,
			"/v5/businesses/:business_id/asset_groups",
			handleFunctions.BusinessAccessAssetsAPI.AssetGroupUpdate,
		},
		{
			"BusinessAssetMembersGet",
			http.MethodGet,
			"/v5/businesses/:business_id/assets/:asset_id/members",
			handleFunctions.BusinessAccessAssetsAPI.BusinessAssetMembersGet,
		},
		{
			"BusinessAssetPartnersGet",
			http.MethodGet,
			"/v5/businesses/:business_id/assets/:asset_id/partners",
			handleFunctions.BusinessAccessAssetsAPI.BusinessAssetPartnersGet,
		},
		{
			"BusinessAssetsGet",
			http.MethodGet,
			"/v5/businesses/:business_id/assets",
			handleFunctions.BusinessAccessAssetsAPI.BusinessAssetsGet,
		},
		{
			"BusinessMemberAssetsGet",
			http.MethodGet,
			"/v5/businesses/:business_id/members/:member_id/assets",
			handleFunctions.BusinessAccessAssetsAPI.BusinessMemberAssetsGet,
		},
		{
			"BusinessMembersAssetAccessDelete",
			http.MethodDelete,
			"/v5/businesses/:business_id/members/assets/access",
			handleFunctions.BusinessAccessAssetsAPI.BusinessMembersAssetAccessDelete,
		},
		{
			"BusinessMembersAssetAccessUpdate",
			http.MethodPatch,
			"/v5/businesses/:business_id/members/assets/access",
			handleFunctions.BusinessAccessAssetsAPI.BusinessMembersAssetAccessUpdate,
		},
		{
			"BusinessPartnerAssetAccessGet",
			http.MethodGet,
			"/v5/businesses/:business_id/partners/:partner_id/assets",
			handleFunctions.BusinessAccessAssetsAPI.BusinessPartnerAssetAccessGet,
		},
		{
			"DeletePartnerAssetAccessHandlerImpl",
			http.MethodDelete,
			"/v5/businesses/:business_id/partners/assets",
			handleFunctions.BusinessAccessAssetsAPI.DeletePartnerAssetAccessHandlerImpl,
		},
		{
			"UpdatePartnerAssetAccessHandlerImpl",
			http.MethodPatch,
			"/v5/businesses/:business_id/partners/assets",
			handleFunctions.BusinessAccessAssetsAPI.UpdatePartnerAssetAccessHandlerImpl,
		},
		{
			"AssetAccessRequestsCreate",
			http.MethodPost,
			"/v5/businesses/:business_id/requests/assets/access",
			handleFunctions.BusinessAccessInviteAPI.AssetAccessRequestsCreate,
		},
		{
			"CancelInvitesOrRequests",
			http.MethodDelete,
			"/v5/businesses/:business_id/invites",
			handleFunctions.BusinessAccessInviteAPI.CancelInvitesOrRequests,
		},
		{
			"CreateAssetInvites",
			http.MethodPost,
			"/v5/businesses/:business_id/invites/assets/access",
			handleFunctions.BusinessAccessInviteAPI.CreateAssetInvites,
		},
		{
			"CreateMembershipOrPartnershipInvites",
			http.MethodPost,
			"/v5/businesses/:business_id/invites",
			handleFunctions.BusinessAccessInviteAPI.CreateMembershipOrPartnershipInvites,
		},
		{
			"GetInvites",
			http.MethodGet,
			"/v5/businesses/:business_id/invites",
			handleFunctions.BusinessAccessInviteAPI.GetInvites,
		},
		{
			"RespondBusinessAccessInvites",
			http.MethodPatch,
			"/v5/businesses/invites",
			handleFunctions.BusinessAccessInviteAPI.RespondBusinessAccessInvites,
		},
		{
			"BrandAccountsCreate",
			http.MethodPost,
			"/v5/business_access/business_hierarchy/:business_hierarchy_id/brand_accounts",
			handleFunctions.BusinessAccessRelationshipsAPI.BrandAccountsCreate,
		},
		{
			"BrandAccountsUpdate",
			http.MethodPatch,
			"/v5/business_access/business_hierarchy/:business_hierarchy_id/brand_accounts/:brand_account_id",
			handleFunctions.BusinessAccessRelationshipsAPI.BrandAccountsUpdate,
		},
		{
			"DeleteBusinessMembership",
			http.MethodDelete,
			"/v5/businesses/:business_id/members",
			handleFunctions.BusinessAccessRelationshipsAPI.DeleteBusinessMembership,
		},
		{
			"DeleteBusinessPartners",
			http.MethodDelete,
			"/v5/businesses/:business_id/partners",
			handleFunctions.BusinessAccessRelationshipsAPI.DeleteBusinessPartners,
		},
		{
			"GetBusinessEmployers",
			http.MethodGet,
			"/v5/businesses/employers",
			handleFunctions.BusinessAccessRelationshipsAPI.GetBusinessEmployers,
		},
		{
			"GetBusinessMembers",
			http.MethodGet,
			"/v5/businesses/:business_id/members",
			handleFunctions.BusinessAccessRelationshipsAPI.GetBusinessMembers,
		},
		{
			"GetBusinessPartners",
			http.MethodGet,
			"/v5/businesses/:business_id/partners",
			handleFunctions.BusinessAccessRelationshipsAPI.GetBusinessPartners,
		},
		{
			"SystemUserUpdate",
			http.MethodPatch,
			"/v5/businesses/:business_id/system_users/:system_user_id",
			handleFunctions.BusinessAccessRelationshipsAPI.SystemUserUpdate,
		},
		{
			"UpdateBusinessMemberships",
			http.MethodPatch,
			"/v5/businesses/:business_id/members",
			handleFunctions.BusinessAccessRelationshipsAPI.UpdateBusinessMemberships,
		},
		{
			"AdPinsAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/pins/analytics",
			handleFunctions.CampaignsAPI.AdPinsAnalytics,
		},
		{
			"CampaignTargetingAnalyticsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/campaigns/targeting_analytics",
			handleFunctions.CampaignsAPI.CampaignTargetingAnalyticsGet,
		},
		{
			"CampaignsAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/campaigns/analytics",
			handleFunctions.CampaignsAPI.CampaignsAnalytics,
		},
		{
			"CampaignsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/campaigns",
			handleFunctions.CampaignsAPI.CampaignsCreate,
		},
		{
			"CampaignsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/campaigns/:campaign_id",
			handleFunctions.CampaignsAPI.CampaignsGet,
		},
		{
			"CampaignsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/campaigns",
			handleFunctions.CampaignsAPI.CampaignsList,
		},
		{
			"CampaignsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/campaigns",
			handleFunctions.CampaignsAPI.CampaignsUpdate,
		},
		{
			"GetCampaignDeliveryEstimates",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/campaigns/delivery_estimates",
			handleFunctions.CampaignsAPI.GetCampaignDeliveryEstimates,
		},
		{
			"FeedProcessingResultsList",
			http.MethodGet,
			"/v5/catalogs/feeds/:feed_id/processing_results",
			handleFunctions.CatalogFeedsAPI.FeedProcessingResultsList,
		},
		{
			"FeedsCreate",
			http.MethodPost,
			"/v5/catalogs/feeds",
			handleFunctions.CatalogFeedsAPI.FeedsCreate,
		},
		{
			"FeedsDelete",
			http.MethodDelete,
			"/v5/catalogs/feeds/:feed_id",
			handleFunctions.CatalogFeedsAPI.FeedsDelete,
		},
		{
			"FeedsGet",
			http.MethodGet,
			"/v5/catalogs/feeds/:feed_id",
			handleFunctions.CatalogFeedsAPI.FeedsGet,
		},
		{
			"FeedsIngest",
			http.MethodPost,
			"/v5/catalogs/feeds/:feed_id/ingest",
			handleFunctions.CatalogFeedsAPI.FeedsIngest,
		},
		{
			"FeedsList",
			http.MethodGet,
			"/v5/catalogs/feeds",
			handleFunctions.CatalogFeedsAPI.FeedsList,
		},
		{
			"FeedsUpdate",
			http.MethodPatch,
			"/v5/catalogs/feeds/:feed_id",
			handleFunctions.CatalogFeedsAPI.FeedsUpdate,
		},
		{
			"ItemsIssuesList",
			http.MethodGet,
			"/v5/catalogs/processing_results/:processing_result_id/item_issues",
			handleFunctions.CatalogFeedsAPI.ItemsIssuesList,
		},
		{
			"ItemsBatchGet",
			http.MethodGet,
			"/v5/catalogs/items/batch/:batch_id",
			handleFunctions.CatalogItemsAPI.ItemsBatchGet,
		},
		{
			"ItemsBatchPost",
			http.MethodPost,
			"/v5/catalogs/items/batch",
			handleFunctions.CatalogItemsAPI.ItemsBatchPost,
		},
		{
			"ItemsPost",
			http.MethodPost,
			"/v5/catalogs/items",
			handleFunctions.CatalogItemsAPI.ItemsPost,
		},
		{
			"CatalogsProductGroupPinsList",
			http.MethodGet,
			"/v5/catalogs/product_groups/:product_group_id/products",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupPinsList,
		},
		{
			"CatalogsProductGroupsCreate",
			http.MethodPost,
			"/v5/catalogs/product_groups",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsCreate,
		},
		{
			"CatalogsProductGroupsCreateMany",
			http.MethodPost,
			"/v5/catalogs/product_groups/multiple",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsCreateMany,
		},
		{
			"CatalogsProductGroupsDelete",
			http.MethodDelete,
			"/v5/catalogs/product_groups/:product_group_id",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsDelete,
		},
		{
			"CatalogsProductGroupsDeleteMany",
			http.MethodDelete,
			"/v5/catalogs/product_groups/multiple",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsDeleteMany,
		},
		{
			"CatalogsProductGroupsGet",
			http.MethodGet,
			"/v5/catalogs/product_groups/:product_group_id",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsGet,
		},
		{
			"CatalogsProductGroupsList",
			http.MethodGet,
			"/v5/catalogs/product_groups",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsList,
		},
		{
			"CatalogsProductGroupsProductCountsGet",
			http.MethodGet,
			"/v5/catalogs/product_groups/:product_group_id/product_counts",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsProductCountsGet,
		},
		{
			"CatalogsProductGroupsUpdate",
			http.MethodPatch,
			"/v5/catalogs/product_groups/:product_group_id",
			handleFunctions.CatalogProductGroupsAPI.CatalogsProductGroupsUpdate,
		},
		{
			"ProductsByProductGroupFilterList",
			http.MethodPost,
			"/v5/catalogs/products/get_by_product_group_filters",
			handleFunctions.CatalogProductGroupsAPI.ProductsByProductGroupFilterList,
		},
		{
			"ReportsCreate",
			http.MethodPost,
			"/v5/catalogs/reports",
			handleFunctions.CatalogReportsAPI.ReportsCreate,
		},
		{
			"ReportsGet",
			http.MethodGet,
			"/v5/catalogs/reports",
			handleFunctions.CatalogReportsAPI.ReportsGet,
		},
		{
			"ReportsStats",
			http.MethodGet,
			"/v5/catalogs/reports/stats",
			handleFunctions.CatalogReportsAPI.ReportsStats,
		},
		{
			"CatalogsLocalInventoryItemsBatchOperate",
			http.MethodPost,
			"/v5/catalogs/:catalog_id/local_inventory_items/batch",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalInventoryItemsBatchOperate,
		},
		{
			"CatalogsLocalInventoryItemsPost",
			http.MethodPost,
			"/v5/catalogs/:catalog_id/local_inventory_items/query",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalInventoryItemsPost,
		},
		{
			"CatalogsLocalStoresCreate",
			http.MethodPost,
			"/v5/catalogs/:catalog_id/local_stores",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalStoresCreate,
		},
		{
			"CatalogsLocalStoresDelete",
			http.MethodDelete,
			"/v5/catalogs/:catalog_id/local_stores",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalStoresDelete,
		},
		{
			"CatalogsLocalStoresList",
			http.MethodGet,
			"/v5/catalogs/:catalog_id/local_stores",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalStoresList,
		},
		{
			"CatalogsLocalStoresUpdate",
			http.MethodPatch,
			"/v5/catalogs/:catalog_id/local_stores",
			handleFunctions.CatalogSupplementalAPI.CatalogsLocalStoresUpdate,
		},
		{
			"CatalogsSupplementalItemsBatchGet",
			http.MethodGet,
			"/v5/catalogs/:catalog_id/supplemental_items/batch/:batch_id",
			handleFunctions.CatalogSupplementalAPI.CatalogsSupplementalItemsBatchGet,
		},
		{
			"CatalogsAvailableFilterValues",
			http.MethodGet,
			"/v5/catalogs/available_filter_values",
			handleFunctions.CatalogsAPI.CatalogsAvailableFilterValues,
		},
		{
			"CatalogsCreate",
			http.MethodPost,
			"/v5/catalogs",
			handleFunctions.CatalogsAPI.CatalogsCreate,
		},
		{
			"CatalogsList",
			http.MethodGet,
			"/v5/catalogs",
			handleFunctions.CatalogsAPI.CatalogsList,
		},
		{
			"ConversionDeletionRequestCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/conversion_deletion_requests",
			handleFunctions.ConversionDeletionRequestsAPI.ConversionDeletionRequestCreate,
		},
		{
			"ConversionDeletionRequestDelete",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/conversion_deletion_requests/:request_id",
			handleFunctions.ConversionDeletionRequestsAPI.ConversionDeletionRequestDelete,
		},
		{
			"ConversionDeletionRequestGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_deletion_requests/:request_id",
			handleFunctions.ConversionDeletionRequestsAPI.ConversionDeletionRequestGet,
		},
		{
			"ConversionDeletionRequestList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_deletion_requests",
			handleFunctions.ConversionDeletionRequestsAPI.ConversionDeletionRequestList,
		},
		{
			"ConversionEqsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_eqs",
			handleFunctions.ConversionEqsAPI.ConversionEqsList,
		},
		{
			"EventsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/events",
			handleFunctions.ConversionEventsAPI.EventsCreate,
		},
		{
			"ConversionTagsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/conversion_tags",
			handleFunctions.ConversionTagsAPI.ConversionTagsCreate,
		},
		{
			"ConversionTagsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_tags/:conversion_tag_id",
			handleFunctions.ConversionTagsAPI.ConversionTagsGet,
		},
		{
			"ConversionTagsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_tags",
			handleFunctions.ConversionTagsAPI.ConversionTagsList,
		},
		{
			"OcpmEligibleConversionTagsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_tags/ocpm_eligible",
			handleFunctions.ConversionTagsAPI.OcpmEligibleConversionTagsGet,
		},
		{
			"PageVisitConversionTagsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/conversion_tags/page_visit",
			handleFunctions.ConversionTagsAPI.PageVisitConversionTagsGet,
		},
		{
			"AdvertiserDefinedEventsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/advertiser_defined_events",
			handleFunctions.ConversionsAPI.AdvertiserDefinedEventsCreate,
		},
		{
			"AdvertiserDefinedEventsDelete",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/advertiser_defined_events",
			handleFunctions.ConversionsAPI.AdvertiserDefinedEventsDelete,
		},
		{
			"AdvertiserDefinedEventsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/advertiser_defined_events",
			handleFunctions.ConversionsAPI.AdvertiserDefinedEventsGet,
		},
		{
			"AdvertiserDefinedEventsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/advertiser_defined_events",
			handleFunctions.ConversionsAPI.AdvertiserDefinedEventsUpdate,
		},
		{
			"CustomerListUploadsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/customer_lists/:customer_list_id/uploads",
			handleFunctions.CustomerListUploadsAPI.CustomerListUploadsCreate,
		},
		{
			"CustomerListUploadsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/customer_lists/:customer_list_id/uploads/:customer_list_upload_id",
			handleFunctions.CustomerListUploadsAPI.CustomerListUploadsGet,
		},
		{
			"CustomerListUploadsRun",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/customer_lists/:customer_list_id/uploads/:customer_list_upload_id/run",
			handleFunctions.CustomerListUploadsAPI.CustomerListUploadsRun,
		},
		{
			"CustomerListsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/customer_lists",
			handleFunctions.CustomerListsAPI.CustomerListsCreate,
		},
		{
			"CustomerListsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/customer_lists/:customer_list_id",
			handleFunctions.CustomerListsAPI.CustomerListsGet,
		},
		{
			"CustomerListsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/customer_lists",
			handleFunctions.CustomerListsAPI.CustomerListsList,
		},
		{
			"CustomerListsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/customer_lists/:customer_list_id",
			handleFunctions.CustomerListsAPI.CustomerListsUpdate,
		},
		{
			"CustomerSegmentCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/customer_segments",
			handleFunctions.CustomerSegmentAPI.CustomerSegmentCreate,
		},
		{
			"CustomerSegmentList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/customer_segments",
			handleFunctions.CustomerSegmentAPI.CustomerSegmentList,
		},
		{
			"CustomerSegmentUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/customer_segments",
			handleFunctions.CustomerSegmentAPI.CustomerSegmentUpdate,
		},
		{
			"IntegrationsCommerceDel",
			http.MethodDelete,
			"/v5/integrations/commerce/:external_business_id",
			handleFunctions.IntegrationsAPI.IntegrationsCommerceDel,
		},
		{
			"IntegrationsCommerceGet",
			http.MethodGet,
			"/v5/integrations/commerce/:external_business_id",
			handleFunctions.IntegrationsAPI.IntegrationsCommerceGet,
		},
		{
			"IntegrationsCommercePatch",
			http.MethodPatch,
			"/v5/integrations/commerce/:external_business_id",
			handleFunctions.IntegrationsAPI.IntegrationsCommercePatch,
		},
		{
			"IntegrationsCommercePost",
			http.MethodPost,
			"/v5/integrations/commerce",
			handleFunctions.IntegrationsAPI.IntegrationsCommercePost,
		},
		{
			"IntegrationsGetById",
			http.MethodGet,
			"/v5/integrations/:id",
			handleFunctions.IntegrationsAPI.IntegrationsGetById,
		},
		{
			"IntegrationsGetList",
			http.MethodGet,
			"/v5/integrations",
			handleFunctions.IntegrationsAPI.IntegrationsGetList,
		},
		{
			"IntegrationsLogsPost",
			http.MethodPost,
			"/v5/integrations/logs",
			handleFunctions.IntegrationsAPI.IntegrationsLogsPost,
		},
		{
			"CountryKeywordsMetricsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/keywords/metrics",
			handleFunctions.KeywordsAPI.CountryKeywordsMetricsGet,
		},
		{
			"KeywordsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/keywords",
			handleFunctions.KeywordsAPI.KeywordsCreate,
		},
		{
			"KeywordsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/keywords",
			handleFunctions.KeywordsAPI.KeywordsGet,
		},
		{
			"KeywordsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/keywords",
			handleFunctions.KeywordsAPI.KeywordsUpdate,
		},
		{
			"TrendingKeywordsList",
			http.MethodGet,
			"/v5/trends/keywords/:region/top/:trend_type",
			handleFunctions.KeywordsAPI.TrendingKeywordsList,
		},
		{
			"LabelsApply",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/labels/:label_id/apply",
			handleFunctions.LabelsAPI.LabelsApply,
		},
		{
			"LabelsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/labels",
			handleFunctions.LabelsAPI.LabelsCreate,
		},
		{
			"LabelsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/labels",
			handleFunctions.LabelsAPI.LabelsList,
		},
		{
			"LabelsRemove",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/labels/:label_id/remove",
			handleFunctions.LabelsAPI.LabelsRemove,
		},
		{
			"LabelsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/labels",
			handleFunctions.LabelsAPI.LabelsUpdate,
		},
		{
			"AdAccountsSubscriptionsDelById",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/leads/subscriptions/:subscription_id",
			handleFunctions.LeadAdsAPI.AdAccountsSubscriptionsDelById,
		},
		{
			"AdAccountsSubscriptionsGetById",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/leads/subscriptions/:subscription_id",
			handleFunctions.LeadAdsAPI.AdAccountsSubscriptionsGetById,
		},
		{
			"AdAccountsSubscriptionsGetList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/leads/subscriptions",
			handleFunctions.LeadAdsAPI.AdAccountsSubscriptionsGetList,
		},
		{
			"AdAccountsSubscriptionsPost",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/leads/subscriptions",
			handleFunctions.LeadAdsAPI.AdAccountsSubscriptionsPost,
		},
		{
			"LeadFormGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/lead_forms/:lead_form_id",
			handleFunctions.LeadFormsAPI.LeadFormGet,
		},
		{
			"LeadFormTestCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/lead_forms/:lead_form_id/test",
			handleFunctions.LeadFormsAPI.LeadFormTestCreate,
		},
		{
			"LeadFormsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/lead_forms",
			handleFunctions.LeadFormsAPI.LeadFormsCreate,
		},
		{
			"LeadFormsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/lead_forms",
			handleFunctions.LeadFormsAPI.LeadFormsList,
		},
		{
			"LeadFormsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/lead_forms",
			handleFunctions.LeadFormsAPI.LeadFormsUpdate,
		},
		{
			"LeadsExportCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/leads_export",
			handleFunctions.LeadsExportAPI.LeadsExportCreate,
		},
		{
			"LeadsExportGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/leads_export/:leads_export_id",
			handleFunctions.LeadsExportAPI.LeadsExportGet,
		},
		{
			"MediaCreate",
			http.MethodPost,
			"/v5/media",
			handleFunctions.MediaAPI.MediaCreate,
		},
		{
			"MediaGet",
			http.MethodGet,
			"/v5/media/:media_id",
			handleFunctions.MediaAPI.MediaGet,
		},
		{
			"MediaList",
			http.MethodGet,
			"/v5/media",
			handleFunctions.MediaAPI.MediaList,
		},
		{
			"MsotEventsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/msot/events",
			handleFunctions.MsotEventsAPI.MsotEventsCreate,
		},
		{
			"NotificationPost",
			http.MethodPost,
			"/v5/notifications",
			handleFunctions.NotificationAPI.NotificationPost,
		},
		{
			"OauthConversionToken",
			http.MethodPost,
			"/v5/oauth/conversion_token",
			handleFunctions.OauthAPI.OauthConversionToken,
		},
		{
			"OauthToken",
			http.MethodPost,
			"/v5/oauth/token",
			handleFunctions.OauthAPI.OauthToken,
		},
		{
			"TokenRevoke",
			http.MethodPost,
			"/v5/oauth/token/revoke",
			handleFunctions.OauthAPI.TokenRevoke,
		},
		{
			"OrderLinesGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/order_lines/:order_line_id",
			handleFunctions.OrderLinesAPI.OrderLinesGet,
		},
		{
			"OrderLinesList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/order_lines",
			handleFunctions.OrderLinesAPI.OrderLinesList,
		},
		{
			"MultiPinsAnalytics",
			http.MethodGet,
			"/v5/pins/analytics",
			handleFunctions.PinsAPI.MultiPinsAnalytics,
		},
		{
			"PinsAnalytics",
			http.MethodGet,
			"/v5/pins/:pin_id/analytics",
			handleFunctions.PinsAPI.PinsAnalytics,
		},
		{
			"PinsCreate",
			http.MethodPost,
			"/v5/pins",
			handleFunctions.PinsAPI.PinsCreate,
		},
		{
			"PinsDelete",
			http.MethodDelete,
			"/v5/pins/:pin_id",
			handleFunctions.PinsAPI.PinsDelete,
		},
		{
			"PinsGet",
			http.MethodGet,
			"/v5/pins/:pin_id",
			handleFunctions.PinsAPI.PinsGet,
		},
		{
			"PinsList",
			http.MethodGet,
			"/v5/pins",
			handleFunctions.PinsAPI.PinsList,
		},
		{
			"PinsSave",
			http.MethodPost,
			"/v5/pins/:pin_id/save",
			handleFunctions.PinsAPI.PinsSave,
		},
		{
			"PinsUpdate",
			http.MethodPatch,
			"/v5/pins/:pin_id",
			handleFunctions.PinsAPI.PinsUpdate,
		},
		{
			"ProductGroupPromotionsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/product_group_promotions",
			handleFunctions.ProductGroupPromotionsAPI.ProductGroupPromotionsCreate,
		},
		{
			"ProductGroupPromotionsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/product_group_promotions/:product_group_promotion_id",
			handleFunctions.ProductGroupPromotionsAPI.ProductGroupPromotionsGet,
		},
		{
			"ProductGroupPromotionsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/product_group_promotions",
			handleFunctions.ProductGroupPromotionsAPI.ProductGroupPromotionsList,
		},
		{
			"ProductGroupPromotionsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/product_group_promotions",
			handleFunctions.ProductGroupPromotionsAPI.ProductGroupPromotionsUpdate,
		},
		{
			"ProductGroupsAnalytics",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/product_groups/analytics",
			handleFunctions.ProductGroupPromotionsAPI.ProductGroupsAnalytics,
		},
		{
			"ProductTagsBulkAdd",
			http.MethodPost,
			"/v5/pins/:pin_id/product_tags",
			handleFunctions.ProductTagsAPI.ProductTagsBulkAdd,
		},
		{
			"ProductTagsBulkDelete",
			http.MethodPost,
			"/v5/pins/:pin_id/product_tags/bulk-delete",
			handleFunctions.ProductTagsAPI.ProductTagsBulkDelete,
		},
		{
			"ProductTagsList",
			http.MethodGet,
			"/v5/pins/:pin_id/product_tags",
			handleFunctions.ProductTagsAPI.ProductTagsList,
		},
		{
			"PromotionsCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/promotions",
			handleFunctions.PromotionsAPI.PromotionsCreate,
		},
		{
			"PromotionsDelete",
			http.MethodDelete,
			"/v5/ad_accounts/:ad_account_id/promotions/:promotion_id",
			handleFunctions.PromotionsAPI.PromotionsDelete,
		},
		{
			"PromotionsGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/promotions/:promotion_id",
			handleFunctions.PromotionsAPI.PromotionsGet,
		},
		{
			"PromotionsList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/promotions",
			handleFunctions.PromotionsAPI.PromotionsList,
		},
		{
			"PromotionsUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/promotions",
			handleFunctions.PromotionsAPI.PromotionsUpdate,
		},
		{
			"AdAccountCountriesGet",
			http.MethodGet,
			"/v5/resources/ad_account_countries",
			handleFunctions.ResourcesAPI.AdAccountCountriesGet,
		},
		{
			"DeliveryMetricsGet",
			http.MethodGet,
			"/v5/resources/delivery_metrics",
			handleFunctions.ResourcesAPI.DeliveryMetricsGet,
		},
		{
			"InterestTargetingOptionsGet",
			http.MethodGet,
			"/v5/resources/targeting/interests/:interest_id",
			handleFunctions.ResourcesAPI.InterestTargetingOptionsGet,
		},
		{
			"LeadFormQuestionsGet",
			http.MethodGet,
			"/v5/resources/lead_form_questions",
			handleFunctions.ResourcesAPI.LeadFormQuestionsGet,
		},
		{
			"MetricsReadyStateGet",
			http.MethodGet,
			"/v5/resources/metrics_ready_state",
			handleFunctions.ResourcesAPI.MetricsReadyStateGet,
		},
		{
			"TargetingOptionsGet",
			http.MethodGet,
			"/v5/resources/targeting/:targeting_type",
			handleFunctions.ResourcesAPI.TargetingOptionsGet,
		},
		{
			"SchedulesCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/schedules",
			handleFunctions.SchedulesAPI.SchedulesCreate,
		},
		{
			"SchedulesList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/schedules",
			handleFunctions.SchedulesAPI.SchedulesList,
		},
		{
			"SchedulesUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/schedules",
			handleFunctions.SchedulesAPI.SchedulesUpdate,
		},
		{
			"SearchPartnerPins",
			http.MethodGet,
			"/v5/search/partner/pins",
			handleFunctions.SearchAPI.SearchPartnerPins,
		},
		{
			"SearchUserBoardsGet",
			http.MethodGet,
			"/v5/search/boards",
			handleFunctions.SearchAPI.SearchUserBoardsGet,
		},
		{
			"SearchUserPinsList",
			http.MethodGet,
			"/v5/search/pins",
			handleFunctions.SearchAPI.SearchUserPinsList,
		},
		{
			"TargetingTemplateCreate",
			http.MethodPost,
			"/v5/ad_accounts/:ad_account_id/targeting_templates",
			handleFunctions.TargetingTemplateAPI.TargetingTemplateCreate,
		},
		{
			"TargetingTemplateList",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/targeting_templates",
			handleFunctions.TargetingTemplateAPI.TargetingTemplateList,
		},
		{
			"TargetingTemplateUpdate",
			http.MethodPatch,
			"/v5/ad_accounts/:ad_account_id/targeting_templates",
			handleFunctions.TargetingTemplateAPI.TargetingTemplateUpdate,
		},
		{
			"TermsRelatedList",
			http.MethodGet,
			"/v5/terms/related",
			handleFunctions.TermsAPI.TermsRelatedList,
		},
		{
			"TermsSuggestedList",
			http.MethodGet,
			"/v5/terms/suggested",
			handleFunctions.TermsAPI.TermsSuggestedList,
		},
		{
			"TermsOfServiceGet",
			http.MethodGet,
			"/v5/ad_accounts/:ad_account_id/terms_of_service",
			handleFunctions.TermsOfServiceAPI.TermsOfServiceGet,
		},
		{
			"TrendsEditorialArticlesList",
			http.MethodGet,
			"/v5/trends/editorial_articles",
			handleFunctions.TrendsAPI.TrendsEditorialArticlesList,
		},
		{
			"TrendsFeaturedTopicsList",
			http.MethodGet,
			"/v5/trends/topics/featured",
			handleFunctions.TrendsAPI.TrendsFeaturedTopicsList,
		},
		{
			"TrendsProductCategoriesDetailsList",
			http.MethodGet,
			"/v5/trends/product_categories/details",
			handleFunctions.TrendsAPI.TrendsProductCategoriesDetailsList,
		},
		{
			"TrendsProductCategoriesTrendingList",
			http.MethodGet,
			"/v5/trends/product_categories/trending",
			handleFunctions.TrendsAPI.TrendsProductCategoriesTrendingList,
		},
		{
			"BoardsUserFollowsList",
			http.MethodGet,
			"/v5/user_account/following/boards",
			handleFunctions.UserAccountAPI.BoardsUserFollowsList,
		},
		{
			"FollowUserUpdate",
			http.MethodPost,
			"/v5/user_account/following/:username",
			handleFunctions.UserAccountAPI.FollowUserUpdate,
		},
		{
			"FollowersList",
			http.MethodGet,
			"/v5/user_account/followers",
			handleFunctions.UserAccountAPI.FollowersList,
		},
		{
			"LinkedBusinessAccountsGet",
			http.MethodGet,
			"/v5/user_account/businesses",
			handleFunctions.UserAccountAPI.LinkedBusinessAccountsGet,
		},
		{
			"UnverifyWebsiteDelete",
			http.MethodDelete,
			"/v5/user_account/websites",
			handleFunctions.UserAccountAPI.UnverifyWebsiteDelete,
		},
		{
			"UserAccountAnalytics",
			http.MethodGet,
			"/v5/user_account/analytics",
			handleFunctions.UserAccountAPI.UserAccountAnalytics,
		},
		{
			"UserAccountAnalyticsTopPins",
			http.MethodGet,
			"/v5/user_account/analytics/top_pins",
			handleFunctions.UserAccountAPI.UserAccountAnalyticsTopPins,
		},
		{
			"UserAccountAnalyticsTopVideoPins",
			http.MethodGet,
			"/v5/user_account/analytics/top_video_pins",
			handleFunctions.UserAccountAPI.UserAccountAnalyticsTopVideoPins,
		},
		{
			"UserAccountFollowedInterests",
			http.MethodGet,
			"/v5/users/:username/interests/follow",
			handleFunctions.UserAccountAPI.UserAccountFollowedInterests,
		},
		{
			"UserAccountGet",
			http.MethodGet,
			"/v5/user_account",
			handleFunctions.UserAccountAPI.UserAccountGet,
		},
		{
			"UserFollowingGet",
			http.MethodGet,
			"/v5/user_account/following",
			handleFunctions.UserAccountAPI.UserFollowingGet,
		},
		{
			"UserWebsitesGet",
			http.MethodGet,
			"/v5/user_account/websites",
			handleFunctions.UserAccountAPI.UserWebsitesGet,
		},
		{
			"VerifyWebsiteUpdate",
			http.MethodPost,
			"/v5/user_account/websites",
			handleFunctions.UserAccountAPI.VerifyWebsiteUpdate,
		},
		{
			"WebsiteVerificationGet",
			http.MethodGet,
			"/v5/user_account/websites/verification",
			handleFunctions.UserAccountAPI.WebsiteVerificationGet,
		},
	}
}
