...
class BusinessAccessInviteController extends Controller
{
    ...
    public function cancelInvitesOrRequests(Request $request, string $businessId): JsonResponse
    {
        ...
        $cancelInvitesRequest = $this->serde->deserialize($request->getContent(), from: 'json', to: \OpenAPI\Server\Model\CancelInvitesRequest::class);


        $apiResult = $this->api->cancelInvitesOrRequests($businessId, $cancelInvitesRequest);

        if ($apiResult instanceof \OpenAPI\Server\Model\CancelInvitesResponse) {
            return response()->json($this->serde->serialize($apiResult, format: 'array'), 200);
        }
