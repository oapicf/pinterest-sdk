...
let adPreviewRequest: AdPreviewRequest; //Create ad preview with pin or image.
...
let targetingTypes: Array<AdsAnalyticsAdTargetingType>; //Targeting type breakdowns for the report. The reporting per targeting type <br> is independent from each other. [\"AGE_BUCKET_AND_GENDER\"] is in BETA and not yet available to all users. (default to undefined)
...
let adAccountId: string; //Unique identifier of an ad account. (default to undefined)
let adCreateRequest: Array<AdCreateRequest>; //List of ads to create, size limit [1, 30].
...
let adAccountId: string; //Unique identifier of an ad account. (default to undefined)
let adId: string; //Unique identifier of an ad. (default to undefined)
...
let adAccountId: string; //Unique identifier of an ad account. (default to undefined)
let adUpdateRequest: Array<AdUpdateRequest>; //List of ads to update, size limit [1, 30]
