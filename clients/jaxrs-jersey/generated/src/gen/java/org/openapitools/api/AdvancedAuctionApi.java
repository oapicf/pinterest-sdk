...
public class AdvancedAuctionApi  {
   ...
   public AdvancedAuctionApi(@Context ServletConfig servletContext) {
      AdvancedAuctionApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("AdvancedAuctionApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (AdvancedAuctionApiService) Class.forName(implClass).getDeclaredConstructor().newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = AdvancedAuctionApiServiceFactory.getAdvancedAuctionApi();
      }
    ...
    public Response advancedAuctionItemsGetPost(@ApiParam(value = "Request object used to get bid options values for a batch of retail catalog items", required = true) @NotNull @Valid  AdvancedAuctionItemsGetRequest advancedAuctionItemsGetRequest,@ApiParam(value = "Unique identifier of an ad account.") @QueryParam("ad_account_id")  @Pattern(regexp="^\\d+$") @Size(max=18) String adAccountId,@Context SecurityContext securityContext)
    ...
    public Response advancedAuctionItemsSubmitPost(@ApiParam(value = "Request object used to upsert or delete bid options for a batch of retail catalog items", required = true) @NotNull @Valid  AdvancedAuctionItemsSubmitRequest advancedAuctionItemsSubmitRequest,@ApiParam(value = "Unique identifier of an ad account.") @QueryParam("ad_account_id")  @Pattern(regexp="^\\d+$") @Size(max=18) String adAccountId,@Context SecurityContext securityContext)
    throws NotFoundException {
