...
module OpenapiClient
  class Configuration
    attr_accessor :base_url, :timeout, :logger, :debugging, :query_array_encoding, :access_token, :username, :password

    def initialize(base_url: nil, **options)
      @base_url = base_url || 'https://api.pinterest.com/v5'
      @timeout = 60
    ...
    def apply_auth(headers, query, auth_names)
      headers['Authorization'] = "Bearer #{access_token}" if access_token && auth_names.include?('conversion_token')
      if username && auth_names.include?('basic')
