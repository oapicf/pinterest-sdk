...
import org.openapitools.models.ConversionMSOTEventsCreate
...
final case class MsotEventsApiRoutes[
  F[_]: JsonDecoder: Monad, pinterest_oauth2
](delegate: MsotEventsApiDelegate[F, pinterest_oauth2]) extends Http4sDsl[F] {
  object msotEvents/create {
    import MsotEventsApiDelegate.msotEvents/createResponses
    ...
    val routepinterest_oauth2 = AuthedRoutes.of[pinterest_oauth2, F] {
      case (req @ POST -> Root / "ad_accounts" / ad_account_idVarr(ad_account_id) / "msot" / "events") as auth =>
          delegate.msotEvents/create.handle_pinterest_oauth2(auth, req, req.asJsonDecode[ConversionMSOTEventsCreate] , ad_account_id, responses)

    }

    val responses: msotEvents/createResponses[F] = new msotEvents/createResponses[F] {
      def resp200(): F[Response[F]] = Ok()
      def resp201(value: Object): F[Response[F]] = Created(value)
  ...
  val routespinterest_oauth2 =
    msotEvents/create.routepinterest_oauth2
}

object MsotEventsApiDelegate {
  trait msotEvents/createResponses[F[_]] {
    def resp200(): F[Response[F]]
    def resp201(value: Object): F[Response[F]]
...
trait MsotEventsApiDelegate[F[_], pinterest_oauth2] {

  trait msotEvents/create {
    import MsotEventsApiDelegate.msotEvents/createResponses
    ...
    def handle_pinterest_oauth2(
      ...
      req: Request[F],
      msotEvents/create: F[ConversionMSOTEventsCreate],
      adAccountId: Refined[String, MaxSize[18] And MatchesRegex["^\\d+$"]],
      responses: msotEvents/createResponses[F]
    ): F[Response[F]]


  }
  def msotEvents/create: msotEvents/create
