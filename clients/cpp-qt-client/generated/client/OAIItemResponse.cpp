/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIItemResponse.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIItemResponse::OAIItemResponse(const QString &json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIItemResponse::OAIItemResponse() {
    this->initializeModel();
}

OAIItemResponse::~OAIItemResponse() {}

void OAIItemResponse::initializeModel() {

    m_attributes_isSet = false;
    m_attributes_isValid = false;

    m_catalog_type_isSet = false;
    m_catalog_type_isValid = false;

    m_item_id_isSet = false;
    m_item_id_isValid = false;

    m_item_response_kind_isSet = false;
    m_item_response_kind_isValid = false;

    m_pins_isSet = false;
    m_pins_isValid = false;

    m_hotel_id_isSet = false;
    m_hotel_id_isValid = false;

    m_creative_assets_id_isSet = false;
    m_creative_assets_id_isValid = false;

    m_errors_isSet = false;
    m_errors_isValid = false;
}

void OAIItemResponse::fromJson(const QString &jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIItemResponse::fromJsonObject(QJsonObject json) {

    m_attributes_isValid = ::OpenAPI::fromJsonValue(m_attributes, json[QString("attributes")]);
    m_attributes_isSet = !json[QString("attributes")].isNull() && m_attributes_isValid;

    m_catalog_type_isValid = ::OpenAPI::fromJsonValue(m_catalog_type, json[QString("catalog_type")]);
    m_catalog_type_isSet = !json[QString("catalog_type")].isNull() && m_catalog_type_isValid;

    m_item_id_isValid = ::OpenAPI::fromJsonValue(m_item_id, json[QString("item_id")]);
    m_item_id_isSet = !json[QString("item_id")].isNull() && m_item_id_isValid;

    m_item_response_kind_isValid = ::OpenAPI::fromJsonValue(m_item_response_kind, json[QString("item_response_kind")]);
    m_item_response_kind_isSet = !json[QString("item_response_kind")].isNull() && m_item_response_kind_isValid;

    m_pins_isValid = ::OpenAPI::fromJsonValue(m_pins, json[QString("pins")]);
    m_pins_isSet = !json[QString("pins")].isNull() && m_pins_isValid;

    m_hotel_id_isValid = ::OpenAPI::fromJsonValue(m_hotel_id, json[QString("hotel_id")]);
    m_hotel_id_isSet = !json[QString("hotel_id")].isNull() && m_hotel_id_isValid;

    m_creative_assets_id_isValid = ::OpenAPI::fromJsonValue(m_creative_assets_id, json[QString("creative_assets_id")]);
    m_creative_assets_id_isSet = !json[QString("creative_assets_id")].isNull() && m_creative_assets_id_isValid;

    m_errors_isValid = ::OpenAPI::fromJsonValue(m_errors, json[QString("errors")]);
    m_errors_isSet = !json[QString("errors")].isNull() && m_errors_isValid;
}

QString OAIItemResponse::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIItemResponse::asJsonObject() const {
    QJsonObject obj;
    if (m_attributes.isSet()) {
        obj.insert(QString("attributes"), ::OpenAPI::toJsonValue(m_attributes));
    }
    if (m_catalog_type_isSet) {
        obj.insert(QString("catalog_type"), ::OpenAPI::toJsonValue(m_catalog_type));
    }
    if (m_item_id_isSet) {
        obj.insert(QString("item_id"), ::OpenAPI::toJsonValue(m_item_id));
    }
    if (m_item_response_kind_isSet) {
        obj.insert(QString("item_response_kind"), ::OpenAPI::toJsonValue(m_item_response_kind));
    }
    if (m_pins.size() > 0) {
        obj.insert(QString("pins"), ::OpenAPI::toJsonValue(m_pins));
    }
    if (m_hotel_id_isSet) {
        obj.insert(QString("hotel_id"), ::OpenAPI::toJsonValue(m_hotel_id));
    }
    if (m_creative_assets_id_isSet) {
        obj.insert(QString("creative_assets_id"), ::OpenAPI::toJsonValue(m_creative_assets_id));
    }
    if (m_errors.size() > 0) {
        obj.insert(QString("errors"), ::OpenAPI::toJsonValue(m_errors));
    }
    return obj;
}

OAICatalogsCreativeAssetsAttributes OAIItemResponse::getAttributes() const {
    return m_attributes;
}
void OAIItemResponse::setAttributes(const OAICatalogsCreativeAssetsAttributes &attributes) {
    m_attributes = attributes;
    m_attributes_isSet = true;
}

bool OAIItemResponse::is_attributes_Set() const{
    return m_attributes_isSet;
}

bool OAIItemResponse::is_attributes_Valid() const{
    return m_attributes_isValid;
}

QString OAIItemResponse::getCatalogType() const {
    return m_catalog_type;
}
void OAIItemResponse::setCatalogType(const QString &catalog_type) {
    m_catalog_type = catalog_type;
    m_catalog_type_isSet = true;
}

bool OAIItemResponse::is_catalog_type_Set() const{
    return m_catalog_type_isSet;
}

bool OAIItemResponse::is_catalog_type_Valid() const{
    return m_catalog_type_isValid;
}

QString OAIItemResponse::getItemId() const {
    return m_item_id;
}
void OAIItemResponse::setItemId(const QString &item_id) {
    m_item_id = item_id;
    m_item_id_isSet = true;
}

bool OAIItemResponse::is_item_id_Set() const{
    return m_item_id_isSet;
}

bool OAIItemResponse::is_item_id_Valid() const{
    return m_item_id_isValid;
}

QString OAIItemResponse::getItemResponseKind() const {
    return m_item_response_kind;
}
void OAIItemResponse::setItemResponseKind(const QString &item_response_kind) {
    m_item_response_kind = item_response_kind;
    m_item_response_kind_isSet = true;
}

bool OAIItemResponse::is_item_response_kind_Set() const{
    return m_item_response_kind_isSet;
}

bool OAIItemResponse::is_item_response_kind_Valid() const{
    return m_item_response_kind_isValid;
}

QList<OAIPin> OAIItemResponse::getPins() const {
    return m_pins;
}
void OAIItemResponse::setPins(const QList<OAIPin> &pins) {
    m_pins = pins;
    m_pins_isSet = true;
}

bool OAIItemResponse::is_pins_Set() const{
    return m_pins_isSet;
}

bool OAIItemResponse::is_pins_Valid() const{
    return m_pins_isValid;
}

QString OAIItemResponse::getHotelId() const {
    return m_hotel_id;
}
void OAIItemResponse::setHotelId(const QString &hotel_id) {
    m_hotel_id = hotel_id;
    m_hotel_id_isSet = true;
}

bool OAIItemResponse::is_hotel_id_Set() const{
    return m_hotel_id_isSet;
}

bool OAIItemResponse::is_hotel_id_Valid() const{
    return m_hotel_id_isValid;
}

QString OAIItemResponse::getCreativeAssetsId() const {
    return m_creative_assets_id;
}
void OAIItemResponse::setCreativeAssetsId(const QString &creative_assets_id) {
    m_creative_assets_id = creative_assets_id;
    m_creative_assets_id_isSet = true;
}

bool OAIItemResponse::is_creative_assets_id_Set() const{
    return m_creative_assets_id_isSet;
}

bool OAIItemResponse::is_creative_assets_id_Valid() const{
    return m_creative_assets_id_isValid;
}

QList<OAIItemValidationEvent> OAIItemResponse::getErrors() const {
    return m_errors;
}
void OAIItemResponse::setErrors(const QList<OAIItemValidationEvent> &errors) {
    m_errors = errors;
    m_errors_isSet = true;
}

bool OAIItemResponse::is_errors_Set() const{
    return m_errors_isSet;
}

bool OAIItemResponse::is_errors_Valid() const{
    return m_errors_isValid;
}

bool OAIItemResponse::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_attributes.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_catalog_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_item_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_item_response_kind_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_pins.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_hotel_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_creative_assets_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_errors.size() > 0) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIItemResponse::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_catalog_type_isValid && m_item_response_kind_isValid && m_errors_isValid && true;
}

} // namespace OpenAPI
