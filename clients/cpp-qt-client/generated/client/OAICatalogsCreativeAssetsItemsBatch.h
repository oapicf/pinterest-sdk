/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAICatalogsCreativeAssetsItemsBatch.h
 *
 * Object describing the catalogs creative assets items batch
 */

#ifndef OAICatalogsCreativeAssetsItemsBatch_H
#define OAICatalogsCreativeAssetsItemsBatch_H

#include <QJsonObject>

#include "OAIBatchOperationStatus.h"
#include "OAICreativeAssetsProcessingRecord.h"
#include <QDateTime>
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAICreativeAssetsProcessingRecord;

class OAICatalogsCreativeAssetsItemsBatch : public OAIObject {
public:
    OAICatalogsCreativeAssetsItemsBatch();
    OAICatalogsCreativeAssetsItemsBatch(const QString &json);
    ~OAICatalogsCreativeAssetsItemsBatch() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(const QString &jsonString) override;

    QString getBatchId() const;
    void setBatchId(const QString &batch_id);
    bool is_batch_id_Set() const;
    bool is_batch_id_Valid() const;

    QString getCatalogType() const;
    void setCatalogType(const QString &catalog_type);
    bool is_catalog_type_Set() const;
    bool is_catalog_type_Valid() const;

    QDateTime getCompletedTime() const;
    void setCompletedTime(const QDateTime &completed_time);
    bool is_completed_time_Set() const;
    bool is_completed_time_Valid() const;

    QDateTime getCreatedTime() const;
    void setCreatedTime(const QDateTime &created_time);
    bool is_created_time_Set() const;
    bool is_created_time_Valid() const;

    QList<OAICreativeAssetsProcessingRecord> getItems() const;
    void setItems(const QList<OAICreativeAssetsProcessingRecord> &items);
    bool is_items_Set() const;
    bool is_items_Valid() const;

    OAIBatchOperationStatus getStatus() const;
    void setStatus(const OAIBatchOperationStatus &status);
    bool is_status_Set() const;
    bool is_status_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_batch_id;
    bool m_batch_id_isSet;
    bool m_batch_id_isValid;

    QString m_catalog_type;
    bool m_catalog_type_isSet;
    bool m_catalog_type_isValid;

    QDateTime m_completed_time;
    bool m_completed_time_isSet;
    bool m_completed_time_isValid;

    QDateTime m_created_time;
    bool m_created_time_isSet;
    bool m_created_time_isValid;

    QList<OAICreativeAssetsProcessingRecord> m_items;
    bool m_items_isSet;
    bool m_items_isValid;

    OAIBatchOperationStatus m_status;
    bool m_status_isSet;
    bool m_status_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAICatalogsCreativeAssetsItemsBatch)

#endif // OAICatalogsCreativeAssetsItemsBatch_H
