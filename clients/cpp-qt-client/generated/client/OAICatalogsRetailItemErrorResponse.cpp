/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICatalogsRetailItemErrorResponse.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICatalogsRetailItemErrorResponse::OAICatalogsRetailItemErrorResponse(const QString &json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICatalogsRetailItemErrorResponse::OAICatalogsRetailItemErrorResponse() {
    this->initializeModel();
}

OAICatalogsRetailItemErrorResponse::~OAICatalogsRetailItemErrorResponse() {}

void OAICatalogsRetailItemErrorResponse::initializeModel() {

    m_catalog_type_isSet = false;
    m_catalog_type_isValid = false;

    m_errors_isSet = false;
    m_errors_isValid = false;

    m_item_id_isSet = false;
    m_item_id_isValid = false;

    m_item_response_kind_isSet = false;
    m_item_response_kind_isValid = false;
}

void OAICatalogsRetailItemErrorResponse::fromJson(const QString &jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICatalogsRetailItemErrorResponse::fromJsonObject(QJsonObject json) {

    m_catalog_type_isValid = ::OpenAPI::fromJsonValue(m_catalog_type, json[QString("catalog_type")]);
    m_catalog_type_isSet = !json[QString("catalog_type")].isNull() && m_catalog_type_isValid;

    m_errors_isValid = ::OpenAPI::fromJsonValue(m_errors, json[QString("errors")]);
    m_errors_isSet = !json[QString("errors")].isNull() && m_errors_isValid;

    m_item_id_isValid = ::OpenAPI::fromJsonValue(m_item_id, json[QString("item_id")]);
    m_item_id_isSet = !json[QString("item_id")].isNull() && m_item_id_isValid;

    m_item_response_kind_isValid = ::OpenAPI::fromJsonValue(m_item_response_kind, json[QString("item_response_kind")]);
    m_item_response_kind_isSet = !json[QString("item_response_kind")].isNull() && m_item_response_kind_isValid;
}

QString OAICatalogsRetailItemErrorResponse::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICatalogsRetailItemErrorResponse::asJsonObject() const {
    QJsonObject obj;
    if (m_catalog_type_isSet) {
        obj.insert(QString("catalog_type"), ::OpenAPI::toJsonValue(m_catalog_type));
    }
    if (m_errors.size() > 0) {
        obj.insert(QString("errors"), ::OpenAPI::toJsonValue(m_errors));
    }
    if (m_item_id_isSet) {
        obj.insert(QString("item_id"), ::OpenAPI::toJsonValue(m_item_id));
    }
    if (m_item_response_kind_isSet) {
        obj.insert(QString("item_response_kind"), ::OpenAPI::toJsonValue(m_item_response_kind));
    }
    return obj;
}

QString OAICatalogsRetailItemErrorResponse::getCatalogType() const {
    return m_catalog_type;
}
void OAICatalogsRetailItemErrorResponse::setCatalogType(const QString &catalog_type) {
    m_catalog_type = catalog_type;
    m_catalog_type_isSet = true;
}

bool OAICatalogsRetailItemErrorResponse::is_catalog_type_Set() const{
    return m_catalog_type_isSet;
}

bool OAICatalogsRetailItemErrorResponse::is_catalog_type_Valid() const{
    return m_catalog_type_isValid;
}

QList<OAIItemValidationEvent> OAICatalogsRetailItemErrorResponse::getErrors() const {
    return m_errors;
}
void OAICatalogsRetailItemErrorResponse::setErrors(const QList<OAIItemValidationEvent> &errors) {
    m_errors = errors;
    m_errors_isSet = true;
}

bool OAICatalogsRetailItemErrorResponse::is_errors_Set() const{
    return m_errors_isSet;
}

bool OAICatalogsRetailItemErrorResponse::is_errors_Valid() const{
    return m_errors_isValid;
}

QString OAICatalogsRetailItemErrorResponse::getItemId() const {
    return m_item_id;
}
void OAICatalogsRetailItemErrorResponse::setItemId(const QString &item_id) {
    m_item_id = item_id;
    m_item_id_isSet = true;
}

bool OAICatalogsRetailItemErrorResponse::is_item_id_Set() const{
    return m_item_id_isSet;
}

bool OAICatalogsRetailItemErrorResponse::is_item_id_Valid() const{
    return m_item_id_isValid;
}

QString OAICatalogsRetailItemErrorResponse::getItemResponseKind() const {
    return m_item_response_kind;
}
void OAICatalogsRetailItemErrorResponse::setItemResponseKind(const QString &item_response_kind) {
    m_item_response_kind = item_response_kind;
    m_item_response_kind_isSet = true;
}

bool OAICatalogsRetailItemErrorResponse::is_item_response_kind_Set() const{
    return m_item_response_kind_isSet;
}

bool OAICatalogsRetailItemErrorResponse::is_item_response_kind_Valid() const{
    return m_item_response_kind_isValid;
}

bool OAICatalogsRetailItemErrorResponse::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_catalog_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_errors.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_item_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_item_response_kind_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICatalogsRetailItemErrorResponse::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_catalog_type_isValid && m_errors_isValid && m_item_response_kind_isValid && true;
}

} // namespace OpenAPI
