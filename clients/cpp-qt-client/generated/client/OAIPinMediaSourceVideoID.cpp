/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIPinMediaSourceVideoID.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIPinMediaSourceVideoID::OAIPinMediaSourceVideoID(const QString &json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIPinMediaSourceVideoID::OAIPinMediaSourceVideoID() {
    this->initializeModel();
}

OAIPinMediaSourceVideoID::~OAIPinMediaSourceVideoID() {}

void OAIPinMediaSourceVideoID::initializeModel() {

    m_cover_image_content_type_isSet = false;
    m_cover_image_content_type_isValid = false;

    m_cover_image_data_isSet = false;
    m_cover_image_data_isValid = false;

    m_cover_image_key_frame_time_isSet = false;
    m_cover_image_key_frame_time_isValid = false;

    m_cover_image_url_isSet = false;
    m_cover_image_url_isValid = false;

    m_is_standard_isSet = false;
    m_is_standard_isValid = false;

    m_media_id_isSet = false;
    m_media_id_isValid = false;

    m_source_type_isSet = false;
    m_source_type_isValid = false;
}

void OAIPinMediaSourceVideoID::fromJson(const QString &jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIPinMediaSourceVideoID::fromJsonObject(QJsonObject json) {

    m_cover_image_content_type_isValid = ::OpenAPI::fromJsonValue(m_cover_image_content_type, json[QString("cover_image_content_type")]);
    m_cover_image_content_type_isSet = !json[QString("cover_image_content_type")].isNull() && m_cover_image_content_type_isValid;

    m_cover_image_data_isValid = ::OpenAPI::fromJsonValue(m_cover_image_data, json[QString("cover_image_data")]);
    m_cover_image_data_isSet = !json[QString("cover_image_data")].isNull() && m_cover_image_data_isValid;

    m_cover_image_key_frame_time_isValid = ::OpenAPI::fromJsonValue(m_cover_image_key_frame_time, json[QString("cover_image_key_frame_time")]);
    m_cover_image_key_frame_time_isSet = !json[QString("cover_image_key_frame_time")].isNull() && m_cover_image_key_frame_time_isValid;

    m_cover_image_url_isValid = ::OpenAPI::fromJsonValue(m_cover_image_url, json[QString("cover_image_url")]);
    m_cover_image_url_isSet = !json[QString("cover_image_url")].isNull() && m_cover_image_url_isValid;

    m_is_standard_isValid = ::OpenAPI::fromJsonValue(m_is_standard, json[QString("is_standard")]);
    m_is_standard_isSet = !json[QString("is_standard")].isNull() && m_is_standard_isValid;

    m_media_id_isValid = ::OpenAPI::fromJsonValue(m_media_id, json[QString("media_id")]);
    m_media_id_isSet = !json[QString("media_id")].isNull() && m_media_id_isValid;

    m_source_type_isValid = ::OpenAPI::fromJsonValue(m_source_type, json[QString("source_type")]);
    m_source_type_isSet = !json[QString("source_type")].isNull() && m_source_type_isValid;
}

QString OAIPinMediaSourceVideoID::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIPinMediaSourceVideoID::asJsonObject() const {
    QJsonObject obj;
    if (m_cover_image_content_type.isSet()) {
        obj.insert(QString("cover_image_content_type"), ::OpenAPI::toJsonValue(m_cover_image_content_type));
    }
    if (m_cover_image_data_isSet) {
        obj.insert(QString("cover_image_data"), ::OpenAPI::toJsonValue(m_cover_image_data));
    }
    if (m_cover_image_key_frame_time_isSet) {
        obj.insert(QString("cover_image_key_frame_time"), ::OpenAPI::toJsonValue(m_cover_image_key_frame_time));
    }
    if (m_cover_image_url_isSet) {
        obj.insert(QString("cover_image_url"), ::OpenAPI::toJsonValue(m_cover_image_url));
    }
    if (m_is_standard_isSet) {
        obj.insert(QString("is_standard"), ::OpenAPI::toJsonValue(m_is_standard));
    }
    if (m_media_id_isSet) {
        obj.insert(QString("media_id"), ::OpenAPI::toJsonValue(m_media_id));
    }
    if (m_source_type_isSet) {
        obj.insert(QString("source_type"), ::OpenAPI::toJsonValue(m_source_type));
    }
    return obj;
}

OAIContentType OAIPinMediaSourceVideoID::getCoverImageContentType() const {
    return m_cover_image_content_type;
}
void OAIPinMediaSourceVideoID::setCoverImageContentType(const OAIContentType &cover_image_content_type) {
    m_cover_image_content_type = cover_image_content_type;
    m_cover_image_content_type_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_cover_image_content_type_Set() const{
    return m_cover_image_content_type_isSet;
}

bool OAIPinMediaSourceVideoID::is_cover_image_content_type_Valid() const{
    return m_cover_image_content_type_isValid;
}

QString OAIPinMediaSourceVideoID::getCoverImageData() const {
    return m_cover_image_data;
}
void OAIPinMediaSourceVideoID::setCoverImageData(const QString &cover_image_data) {
    m_cover_image_data = cover_image_data;
    m_cover_image_data_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_cover_image_data_Set() const{
    return m_cover_image_data_isSet;
}

bool OAIPinMediaSourceVideoID::is_cover_image_data_Valid() const{
    return m_cover_image_data_isValid;
}

qint32 OAIPinMediaSourceVideoID::getCoverImageKeyFrameTime() const {
    return m_cover_image_key_frame_time;
}
void OAIPinMediaSourceVideoID::setCoverImageKeyFrameTime(const qint32 &cover_image_key_frame_time) {
    m_cover_image_key_frame_time = cover_image_key_frame_time;
    m_cover_image_key_frame_time_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_cover_image_key_frame_time_Set() const{
    return m_cover_image_key_frame_time_isSet;
}

bool OAIPinMediaSourceVideoID::is_cover_image_key_frame_time_Valid() const{
    return m_cover_image_key_frame_time_isValid;
}

QString OAIPinMediaSourceVideoID::getCoverImageUrl() const {
    return m_cover_image_url;
}
void OAIPinMediaSourceVideoID::setCoverImageUrl(const QString &cover_image_url) {
    m_cover_image_url = cover_image_url;
    m_cover_image_url_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_cover_image_url_Set() const{
    return m_cover_image_url_isSet;
}

bool OAIPinMediaSourceVideoID::is_cover_image_url_Valid() const{
    return m_cover_image_url_isValid;
}

bool OAIPinMediaSourceVideoID::isIsStandard() const {
    return m_is_standard;
}
void OAIPinMediaSourceVideoID::setIsStandard(const bool &is_standard) {
    m_is_standard = is_standard;
    m_is_standard_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_is_standard_Set() const{
    return m_is_standard_isSet;
}

bool OAIPinMediaSourceVideoID::is_is_standard_Valid() const{
    return m_is_standard_isValid;
}

QString OAIPinMediaSourceVideoID::getMediaId() const {
    return m_media_id;
}
void OAIPinMediaSourceVideoID::setMediaId(const QString &media_id) {
    m_media_id = media_id;
    m_media_id_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_media_id_Set() const{
    return m_media_id_isSet;
}

bool OAIPinMediaSourceVideoID::is_media_id_Valid() const{
    return m_media_id_isValid;
}

QString OAIPinMediaSourceVideoID::getSourceType() const {
    return m_source_type;
}
void OAIPinMediaSourceVideoID::setSourceType(const QString &source_type) {
    m_source_type = source_type;
    m_source_type_isSet = true;
}

bool OAIPinMediaSourceVideoID::is_source_type_Set() const{
    return m_source_type_isSet;
}

bool OAIPinMediaSourceVideoID::is_source_type_Valid() const{
    return m_source_type_isValid;
}

bool OAIPinMediaSourceVideoID::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_cover_image_content_type.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_cover_image_data_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_cover_image_key_frame_time_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_cover_image_url_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_is_standard_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_media_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_source_type_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIPinMediaSourceVideoID::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_media_id_isValid && m_source_type_isValid && true;
}

} // namespace OpenAPI
