/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIUpdatePartnerAssetsResult.h
 *
 * An object containing the permissions a business partner has on the asset.
 */

#ifndef OAIUpdatePartnerAssetsResult_H
#define OAIUpdatePartnerAssetsResult_H

#include <QJsonObject>

#include "OAIAssetTypeResponse.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIUpdatePartnerAssetsResult : public OAIObject {
public:
    OAIUpdatePartnerAssetsResult();
    OAIUpdatePartnerAssetsResult(const QString &json);
    ~OAIUpdatePartnerAssetsResult() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(const QString &jsonString) override;

    QString getAssetId() const;
    void setAssetId(const QString &asset_id);
    bool is_asset_id_Set() const;
    bool is_asset_id_Valid() const;

    OAIAssetTypeResponse getAssetType() const;
    void setAssetType(const OAIAssetTypeResponse &asset_type);
    bool is_asset_type_Set() const;
    bool is_asset_type_Valid() const;

    QString getPartnerId() const;
    void setPartnerId(const QString &partner_id);
    bool is_partner_id_Set() const;
    bool is_partner_id_Valid() const;

    QList<QString> getPermissions() const;
    void setPermissions(const QList<QString> &permissions);
    bool is_permissions_Set() const;
    bool is_permissions_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_asset_id;
    bool m_asset_id_isSet;
    bool m_asset_id_isValid;

    OAIAssetTypeResponse m_asset_type;
    bool m_asset_type_isSet;
    bool m_asset_type_isValid;

    QString m_partner_id;
    bool m_partner_id_isSet;
    bool m_partner_id_isValid;

    QList<QString> m_permissions;
    bool m_permissions_isSet;
    bool m_permissions_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIUpdatePartnerAssetsResult)

#endif // OAIUpdatePartnerAssetsResult_H
