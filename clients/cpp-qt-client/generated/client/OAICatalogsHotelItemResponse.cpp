/**
 * Pinterest REST API
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICatalogsHotelItemResponse.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICatalogsHotelItemResponse::OAICatalogsHotelItemResponse(const QString &json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICatalogsHotelItemResponse::OAICatalogsHotelItemResponse() {
    this->initializeModel();
}

OAICatalogsHotelItemResponse::~OAICatalogsHotelItemResponse() {}

void OAICatalogsHotelItemResponse::initializeModel() {

    m_attributes_isSet = false;
    m_attributes_isValid = false;

    m_catalog_type_isSet = false;
    m_catalog_type_isValid = false;

    m_hotel_id_isSet = false;
    m_hotel_id_isValid = false;

    m_item_response_kind_isSet = false;
    m_item_response_kind_isValid = false;

    m_pins_isSet = false;
    m_pins_isValid = false;
}

void OAICatalogsHotelItemResponse::fromJson(const QString &jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICatalogsHotelItemResponse::fromJsonObject(QJsonObject json) {

    m_attributes_isValid = ::OpenAPI::fromJsonValue(m_attributes, json[QString("attributes")]);
    m_attributes_isSet = !json[QString("attributes")].isNull() && m_attributes_isValid;

    m_catalog_type_isValid = ::OpenAPI::fromJsonValue(m_catalog_type, json[QString("catalog_type")]);
    m_catalog_type_isSet = !json[QString("catalog_type")].isNull() && m_catalog_type_isValid;

    m_hotel_id_isValid = ::OpenAPI::fromJsonValue(m_hotel_id, json[QString("hotel_id")]);
    m_hotel_id_isSet = !json[QString("hotel_id")].isNull() && m_hotel_id_isValid;

    m_item_response_kind_isValid = ::OpenAPI::fromJsonValue(m_item_response_kind, json[QString("item_response_kind")]);
    m_item_response_kind_isSet = !json[QString("item_response_kind")].isNull() && m_item_response_kind_isValid;

    m_pins_isValid = ::OpenAPI::fromJsonValue(m_pins, json[QString("pins")]);
    m_pins_isSet = !json[QString("pins")].isNull() && m_pins_isValid;
}

QString OAICatalogsHotelItemResponse::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICatalogsHotelItemResponse::asJsonObject() const {
    QJsonObject obj;
    if (m_attributes.isSet()) {
        obj.insert(QString("attributes"), ::OpenAPI::toJsonValue(m_attributes));
    }
    if (m_catalog_type_isSet) {
        obj.insert(QString("catalog_type"), ::OpenAPI::toJsonValue(m_catalog_type));
    }
    if (m_hotel_id_isSet) {
        obj.insert(QString("hotel_id"), ::OpenAPI::toJsonValue(m_hotel_id));
    }
    if (m_item_response_kind_isSet) {
        obj.insert(QString("item_response_kind"), ::OpenAPI::toJsonValue(m_item_response_kind));
    }
    if (m_pins.size() > 0) {
        obj.insert(QString("pins"), ::OpenAPI::toJsonValue(m_pins));
    }
    return obj;
}

OAICatalogsHotelAttributes OAICatalogsHotelItemResponse::getAttributes() const {
    return m_attributes;
}
void OAICatalogsHotelItemResponse::setAttributes(const OAICatalogsHotelAttributes &attributes) {
    m_attributes = attributes;
    m_attributes_isSet = true;
}

bool OAICatalogsHotelItemResponse::is_attributes_Set() const{
    return m_attributes_isSet;
}

bool OAICatalogsHotelItemResponse::is_attributes_Valid() const{
    return m_attributes_isValid;
}

QString OAICatalogsHotelItemResponse::getCatalogType() const {
    return m_catalog_type;
}
void OAICatalogsHotelItemResponse::setCatalogType(const QString &catalog_type) {
    m_catalog_type = catalog_type;
    m_catalog_type_isSet = true;
}

bool OAICatalogsHotelItemResponse::is_catalog_type_Set() const{
    return m_catalog_type_isSet;
}

bool OAICatalogsHotelItemResponse::is_catalog_type_Valid() const{
    return m_catalog_type_isValid;
}

QString OAICatalogsHotelItemResponse::getHotelId() const {
    return m_hotel_id;
}
void OAICatalogsHotelItemResponse::setHotelId(const QString &hotel_id) {
    m_hotel_id = hotel_id;
    m_hotel_id_isSet = true;
}

bool OAICatalogsHotelItemResponse::is_hotel_id_Set() const{
    return m_hotel_id_isSet;
}

bool OAICatalogsHotelItemResponse::is_hotel_id_Valid() const{
    return m_hotel_id_isValid;
}

QString OAICatalogsHotelItemResponse::getItemResponseKind() const {
    return m_item_response_kind;
}
void OAICatalogsHotelItemResponse::setItemResponseKind(const QString &item_response_kind) {
    m_item_response_kind = item_response_kind;
    m_item_response_kind_isSet = true;
}

bool OAICatalogsHotelItemResponse::is_item_response_kind_Set() const{
    return m_item_response_kind_isSet;
}

bool OAICatalogsHotelItemResponse::is_item_response_kind_Valid() const{
    return m_item_response_kind_isValid;
}

QList<OAIPin> OAICatalogsHotelItemResponse::getPins() const {
    return m_pins;
}
void OAICatalogsHotelItemResponse::setPins(const QList<OAIPin> &pins) {
    m_pins = pins;
    m_pins_isSet = true;
}

bool OAICatalogsHotelItemResponse::is_pins_Set() const{
    return m_pins_isSet;
}

bool OAICatalogsHotelItemResponse::is_pins_Valid() const{
    return m_pins_isValid;
}

bool OAICatalogsHotelItemResponse::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_attributes.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_catalog_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_hotel_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_item_response_kind_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_pins.size() > 0) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICatalogsHotelItemResponse::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_catalog_type_isValid && m_item_response_kind_isValid && true;
}

} // namespace OpenAPI
