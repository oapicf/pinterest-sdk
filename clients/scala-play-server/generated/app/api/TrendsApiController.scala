...
@Singleton
class TrendsApiController @Inject()(cc: ControllerComponents, api: TrendsApi) extends AbstractController(cc) {
  /**
    * GET /v5/trends/editorial_articles?region=[value]
    */
  def trendsEditorialArticlesList(): Action[AnyContent] = Action { request =>
    def executeApi(): List[TrendsEditorial] = {
