...
public:
    ...
    [[nodiscard]] std::vector<std::vector<std::string>> getValues() const;
    void setValues(const std::vector<std::vector<std::string>>& values);


    // JSON serialization using NLOHMANN INTRUSIVE macro (must be inside class to access private members)
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(CatalogsProductGroupMultipleStringListCriteria,
         negated, values)
