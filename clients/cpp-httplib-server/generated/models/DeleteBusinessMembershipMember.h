/**
* This file is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/

#pragma once
// System headers
#include <nlohmann/json.hpp>
#include <string>
#include "BusinessRoleForMembers.h"



namespace models {


class DeleteBusinessMembershipMember
{
public:

    DeleteBusinessMembershipMember();
    virtual ~DeleteBusinessMembershipMember() = default;

    // Getters and setters
    [[nodiscard]] BusinessRoleForMembers getBusinessRole() const;
    void setBusinessRole(const BusinessRoleForMembers& businessRole);
    [[nodiscard]] std::string getMemberId() const;
    void setMemberId(const std::string& memberId);


    // JSON serialization using NLOHMANN INTRUSIVE macro (must be inside class to access private members)
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(DeleteBusinessMembershipMember,
         businessRole, memberId)

private:
    BusinessRoleForMembers businessRole;
    std::string memberId;
};


} // namespace models

