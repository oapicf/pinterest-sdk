/**
* This file is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/

#pragma once
// System headers
#include <nlohmann/json.hpp>
#include "CatalogsProductGroupMultipleCountriesCriteria.h"



namespace models {


class Country
{
public:

    Country();
    virtual ~Country() = default;

    // Getters and setters
    [[nodiscard]] CatalogsProductGroupMultipleCountriesCriteria getCOUNTRY() const;
    void setCOUNTRY(const CatalogsProductGroupMultipleCountriesCriteria& cOUNTRY);


    // JSON serialization using NLOHMANN INTRUSIVE macro (must be inside class to access private members)
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(Country,
         cOUNTRY)

private:
    CatalogsProductGroupMultipleCountriesCriteria cOUNTRY;
};


} // namespace models

