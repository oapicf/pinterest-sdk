...
import type { OrderLinesGetRequest } from '';
...
async function example() {
  ...
  const api = new OrderLinesApi(config);
  ...
  } satisfies OrderLinesGetRequest;

  try {
    const data = await api.orderLinesGet(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { OrderLinesListRequest } from '';
...
async function example() {
  ...
  } satisfies OrderLinesListRequest;

  try {
    const data = await api.orderLinesList(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
