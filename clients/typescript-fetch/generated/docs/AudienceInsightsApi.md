...
import type { AudienceInsightsGetRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    audienceInsightType: ...,
  } satisfies AudienceInsightsGetRequest;

  try {
    const data = await api.audienceInsightsGet(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **audienceInsightType** | `AudienceInsightType` | Type of audience insights. | [Defaults to `undefined`] [Enum: YOUR_TOTAL_AUDIENCE, YOUR_ENGAGED_AUDIENCE, PINTEREST_TOTAL_AUDIENCE] |
...
import type { AudienceInsightsScopeAndTypeGetRequest } from '';
...
async function example() {
  ...
  const body = {
    // string | Unique identifier of an ad account.
    adAccountId: adAccountId_example,
  } satisfies AudienceInsightsScopeAndTypeGetRequest;

  try {
    const data = await api.audienceInsightsScopeAndTypeGet(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
