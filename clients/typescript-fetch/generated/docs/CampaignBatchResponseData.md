...
`startTime` | number
`status` | [NullableEntityStatus](NullableEntityStatus.md)
...
```typescript
import type { CampaignBatchResponseData } from ''
...
} satisfies CampaignBatchResponseData
...
const exampleParsed = JSON.parse(exampleJSON) as CampaignBatchResponseData
