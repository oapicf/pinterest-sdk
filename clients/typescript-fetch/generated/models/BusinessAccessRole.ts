/* tslint:disable */
/* eslint-disable */
/**
 * Pinterest REST API
 * Pinterest\'s REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


/**
 * Permission role for business access.
 * @export
 */
export const BusinessAccessRole = {
    Owner: 'OWNER',
    Admin: 'ADMIN',
    Analyst: 'ANALYST',
    SosReader: 'SOS_READER',
    FinanceManager: 'FINANCE_MANAGER',
    FinanceView: 'FINANCE_VIEW',
    FinanceEdit: 'FINANCE_EDIT',
    AudienceManager: 'AUDIENCE_MANAGER',
    CampaignManager: 'CAMPAIGN_MANAGER',
    CatalogsManager: 'CATALOGS_MANAGER',
    RestrictedOwner: 'RESTRICTED_OWNER',
    ProfileManager: 'PROFILE_MANAGER',
    ProfilePublisher: 'PROFILE_PUBLISHER',
    ResourcePinnerListOwner: 'RESOURCE_PINNER_LIST_OWNER',
    ResourcePinnerListReader: 'RESOURCE_PINNER_LIST_READER',
    BizPinnerListSharer: 'BIZ_PINNER_LIST_SHARER',
    ResourceConversionTagsReader: 'RESOURCE_CONVERSION_TAGS_READER'
} as const;
export type BusinessAccessRole = typeof BusinessAccessRole[keyof typeof BusinessAccessRole];


export function instanceOfBusinessAccessRole(value: any): boolean {
    for (const key in BusinessAccessRole) {
        if (Object.prototype.hasOwnProperty.call(BusinessAccessRole, key)) {
            if (BusinessAccessRole[key as keyof typeof BusinessAccessRole] === value) {
                return true;
            }
        }
    }
    return false;
}

export function BusinessAccessRoleFromJSON(json: any): BusinessAccessRole {
    return BusinessAccessRoleFromJSONTyped(json, false);
}

export function BusinessAccessRoleFromJSONTyped(json: any, ignoreDiscriminator: boolean): BusinessAccessRole {
    return json as BusinessAccessRole;
}

export function BusinessAccessRoleToJSON(value?: BusinessAccessRole | null): any {
    return value as any;
}

export function BusinessAccessRoleToJSONTyped(value: any, ignoreDiscriminator: boolean): BusinessAccessRole {
    return value as BusinessAccessRole;
}

