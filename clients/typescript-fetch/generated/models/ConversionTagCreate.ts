/* tslint:disable */
/* eslint-disable */
/**
 * Pinterest REST API
 * Pinterest\'s REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * Resource create operation model.
 * @export
 * @interface ConversionTagCreate
 */
export interface ConversionTagCreate {
    /**
     * Whether Automatic Enhanced Match birthdate is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemDbEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match email is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match location is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemExternalIdEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match name is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemFnlnEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match gender is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemGeEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match location is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemLocEnabled?: boolean | null;
    /**
     * Whether Automatic Enhanced Match phone is enabled. See [Enhanced match](https://help.pinterest.com/en/business/article/enhanced-match) for more information.
     * @type {boolean}
     * @memberof ConversionTagCreate
     */
    aemPhEnabled?: boolean | null;
    /**
     * Metadata ingestion frequency.
     * @type {number}
     * @memberof ConversionTagCreate
     */
    mdFrequency?: number | null;
    /**
     * Conversion tag name.
     * @type {string}
     * @memberof ConversionTagCreate
     */
    name: string;
}

/**
 * Check if a given object implements the ConversionTagCreate interface.
 */
export function instanceOfConversionTagCreate(value: object): value is ConversionTagCreate {
    if (!('name' in value) || value['name'] === undefined) return false;
    return true;
}

export function ConversionTagCreateFromJSON(json: any): ConversionTagCreate {
    return ConversionTagCreateFromJSONTyped(json, false);
}

export function ConversionTagCreateFromJSONTyped(json: any, ignoreDiscriminator: boolean): ConversionTagCreate {
    if (json == null) {
        return json;
    }
    return {
        
        'aemDbEnabled': json['aem_db_enabled'] === undefined ? undefined : json['aem_db_enabled'] === null ? null : json['aem_db_enabled'],
        'aemEnabled': json['aem_enabled'] === undefined ? undefined : json['aem_enabled'] === null ? null : json['aem_enabled'],
        'aemExternalIdEnabled': json['aem_external_id_enabled'] === undefined ? undefined : json['aem_external_id_enabled'] === null ? null : json['aem_external_id_enabled'],
        'aemFnlnEnabled': json['aem_fnln_enabled'] === undefined ? undefined : json['aem_fnln_enabled'] === null ? null : json['aem_fnln_enabled'],
        'aemGeEnabled': json['aem_ge_enabled'] === undefined ? undefined : json['aem_ge_enabled'] === null ? null : json['aem_ge_enabled'],
        'aemLocEnabled': json['aem_loc_enabled'] === undefined ? undefined : json['aem_loc_enabled'] === null ? null : json['aem_loc_enabled'],
        'aemPhEnabled': json['aem_ph_enabled'] === undefined ? undefined : json['aem_ph_enabled'] === null ? null : json['aem_ph_enabled'],
        'mdFrequency': json['md_frequency'] === undefined ? undefined : json['md_frequency'] === null ? null : json['md_frequency'],
        'name': json['name'],
    };
}

export function ConversionTagCreateToJSON(json: any): ConversionTagCreate {
    return ConversionTagCreateToJSONTyped(json, false);
}

export function ConversionTagCreateToJSONTyped(value?: ConversionTagCreate | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'aem_db_enabled': value['aemDbEnabled'],
        'aem_enabled': value['aemEnabled'],
        'aem_external_id_enabled': value['aemExternalIdEnabled'],
        'aem_fnln_enabled': value['aemFnlnEnabled'],
        'aem_ge_enabled': value['aemGeEnabled'],
        'aem_loc_enabled': value['aemLocEnabled'],
        'aem_ph_enabled': value['aemPhEnabled'],
        'md_frequency': value['mdFrequency'],
        'name': value['name'],
    };
}

