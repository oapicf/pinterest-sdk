...
-export([openapi_catalogs_retail_item_error_response/0]).

-export([openapi_catalogs_retail_item_error_response/1]).
...
openapi_catalogs_retail_item_error_response() ->
    openapi_catalogs_retail_item_error_response([]).
...
openapi_catalogs_retail_item_error_response(Fields) ->
  Default = [ {'catalog_type', elements([<<"RETAIL">>]) }
            , {'errors', list(openapi_item_validation_event:openapi_item_validation_event()) }
            , {'item_id', binary() }
            , {'item_response_kind', elements([<<"retail_item_error">>]) }
            ],
