...
import com.github.oapicf.pinterestsdk.model.IntegrationLogsInvalidLogResponseRejectedLogsItems;
...
public class IntegrationLogsInvalidLogResponse {
  ...
  public IntegrationLogsInvalidLogResponse rejectedLogs(@javax.annotation.Nullable List<IntegrationLogsInvalidLogResponseRejectedLogsItems> rejectedLogs) {
  ...
  public void setRejectedLogs(@javax.annotation.Nullable List<IntegrationLogsInvalidLogResponseRejectedLogsItems> rejectedLogs) {
    this.rejectedLogs = rejectedLogs;
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("rejected_logs"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IntegrationLogsInvalidLogResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in IntegrationLogsInvalidLogResponse is not found in the empty JSON string", IntegrationLogsInvalidLogResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IntegrationLogsInvalidLogResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `IntegrationLogsInvalidLogResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("rejected_logs") != null && !jsonObj.get("rejected_logs").isJsonNull()) {
        ...
        if (jsonArrayrejectedLogs != null) {
          ...
          if (!jsonObj.get("rejected_logs").isJsonArray()) {
            throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `rejected_logs` to be an array in the JSON string but got `%s`", jsonObj.get("rejected_logs").toString()));
          }

          // validate the optional field `rejected_logs` (array)
          for (int i = 0; i < jsonArrayrejectedLogs.size(); i++) {
            IntegrationLogsInvalidLogResponseRejectedLogsItems.validateJsonElement(jsonArrayrejectedLogs.get(i));
          };
        }
      }
  }
