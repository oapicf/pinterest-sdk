...
import com.github.oapicf.pinterestsdk.api.TrendsApi;
...
public class Example {
  public static void main(String[] args) {
    ...
    ProductCategoryRegion region = ProductCategoryRegion.fromValue("US"); // ProductCategoryRegion |      The geographic region of interest. Only top product categories within the specified region will be returned.      The `region` parameter is formatted as ISO 3166-2 country codes delimited by `+`.      - `US` - United States     - `GB+IE` - Great Britain & Ireland     - `CA` - Canada
    try {
      List<TrendsEditorial> result = apiInstance.trendsEditorialArticlesList(region);
...
public class Example {
  public static void main(String[] args) {
    ...
    TrendsApi apiInstance = new TrendsApi(defaultClient);
    ProductCategoryRegion region = ProductCategoryRegion.fromValue("US"); // ProductCategoryRegion |      The geographic region of interest. Only top product categories within the specified region will be returned.      The `region` parameter is formatted as ISO 3166-2 country codes delimited by `+`.      - `US` - United States     - `GB+IE` - Great Britain & Ireland     - `CA` - Canada
    InterestsEnum interest = InterestsEnum.fromValue("ALL"); // InterestsEnum | Interest to filter by
    try {
...
public class Example {
  public static void main(String[] args) {
    ...
    TrendsApi apiInstance = new TrendsApi(defaultClient);
    List<ProductCategoryEnum> productCategories = Arrays.asList(); // List<ProductCategoryEnum> | List of product categories
    ProductCategoryRegion region = ProductCategoryRegion.fromValue("US"); // ProductCategoryRegion |      The geographic region of interest. Only top product categories within the specified region will be returned.      The `region` parameter is formatted as ISO 3166-2 country codes delimited by `+`.      - `US` - United States     - `GB+IE` - Great Britain & Ireland     - `CA` - Canada
    ProductCategoryDetailLookbackWindow lookbackWindow = ProductCategoryDetailLookbackWindow.fromValue("90"); // ProductCategoryDetailLookbackWindow |   Time period for historical data analysis in days. The lookback window defines how far back in time the API will analyze data to compute trend metrics.   - `90` - Last 90 days (3 months)   - `180` - Last 180 days (6 months)   - `365` - Last 365 days (1 year)   - `730` - Last 730 days (2 years)
    ProductCategoriesEngagementType engagementType = ProductCategoriesEngagementType.fromValue("ENGAGEMENT"); // ProductCategoriesEngagementType |     Type of engagement metric to analyze. - `ENGAGEMENT` - Overall engagement metric - `OUTBOUND_CLICK` - Number of outbound clicks - `SAVE` - Number of pin saves
    try {
...
public class Example {
  public static void main(String[] args) {
    ...
    TrendsApi apiInstance = new TrendsApi(defaultClient);
    ProductCategoryRegion region = ProductCategoryRegion.fromValue("US"); // ProductCategoryRegion |      The geographic region of interest. Only top product categories within the specified region will be returned.      The `region` parameter is formatted as ISO 3166-2 country codes delimited by `+`.      - `US` - United States     - `GB+IE` - Great Britain & Ireland     - `CA` - Canada
    List<VerticalProductCategory> verticals = Arrays.asList(); // List<VerticalProductCategory> | List of verticals to filter by
    List<AgeTrendsBucket> ages = Arrays.asList(); // List<AgeTrendsBucket> | Age to filter by. If not provided, the results will be filtered by all ages.
    List<GenderBucket> genders = Arrays.asList(); // List<GenderBucket> | Gender to filter by, If not provided, the results will be filtered by all genders.
    ProductCategoriesEngagementType engagementType = ProductCategoriesEngagementType.fromValue("ENGAGEMENT"); // ProductCategoriesEngagementType |     Type of engagement metric to analyze. - `ENGAGEMENT` - Overall engagement metric - `OUTBOUND_CLICK` - Number of outbound clicks - `SAVE` - Number of pin saves
    try {
