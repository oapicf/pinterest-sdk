...
void
Schedules_list_200_response::__init()
{
	//bookmark = std::string();
...
void
Schedules_list_200_response::__cleanup()
{
	//if(bookmark != NULL) {
	//
	//delete bookmark;
...
void
Schedules_list_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<Schedule> new_list;
			Schedule inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("Schedule")) {
					jsonToValue(&inst, temp_json, "Schedule", "");
				} else {
...
Schedules_list_200_response::Schedules_list_200_response(char* json)
{
	this->fromJson(json);
}
...
Schedules_list_200_response::toJson()
{
	...
	if (isprimitive("Schedule")) {
		list<Schedule> new_list = static_cast<list <Schedule> > (getItems());
		node = converttoJson(&new_list, "Schedule", "array");
	} else {
		...
		for (list<Schedule>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Schedule obj = *it;
