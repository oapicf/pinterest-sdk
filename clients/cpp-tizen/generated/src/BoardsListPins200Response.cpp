...
void
Boards_list_pins_200_response::__init()
{
	//bookmark = std::string();
...
void
Boards_list_pins_200_response::__cleanup()
{
	//if(bookmark != NULL) {
	//
	//delete bookmark;
...
void
Boards_list_pins_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<PinRead> new_list;
			PinRead inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("PinRead")) {
					jsonToValue(&inst, temp_json, "PinRead", "");
				} else {
...
Boards_list_pins_200_response::Boards_list_pins_200_response(char* json)
{
	this->fromJson(json);
}
...
Boards_list_pins_200_response::toJson()
{
	...
	if (isprimitive("PinRead")) {
		list<PinRead> new_list = static_cast<list <PinRead> > (getItems());
		node = converttoJson(&new_list, "PinRead", "array");
	} else {
		...
		for (list<PinRead>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			PinRead obj = *it;
