...
User_websites_get_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<UserWebsite> new_list;
			UserWebsite inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("UserWebsite")) {
					jsonToValue(&inst, temp_json, "UserWebsite", "");
				} else {
...
User_websites_get_200_response::toJson()
{
	...
	if (isprimitive("UserWebsite")) {
		list<UserWebsite> new_list = static_cast<list <UserWebsite> > (getItems());
		node = converttoJson(&new_list, "UserWebsite", "array");
	} else {
		...
		for (list<UserWebsite>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			UserWebsite obj = *it;
