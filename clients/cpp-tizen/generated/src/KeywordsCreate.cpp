#include <map>
#include <cstdlib>
#include <glib-object.h>
#include <json-glib/json-glib.h>
#include "Helpers.h"


#include "KeywordsCreate.h"

using namespace std;
using namespace Tizen::ArtikCloud;

KeywordsCreate::KeywordsCreate()
{
	//__init();
}

KeywordsCreate::~KeywordsCreate()
{
	//__cleanup();
}

void
KeywordsCreate::__init()
{
	//new std::list()std::list> keywords;
	//parent_id = std::string();
}

void
KeywordsCreate::__cleanup()
{
	//if(keywords != NULL) {
	//keywords.RemoveAll(true);
	//delete keywords;
	//keywords = NULL;
	//}
	//if(parent_id != NULL) {
	//
	//delete parent_id;
	//parent_id = NULL;
	//}
	//
}

void
KeywordsCreate::fromJson(char* jsonStr)
{
	JsonObject *pJsonObject = json_node_get_object(json_from_string(jsonStr,NULL));
	JsonNode *node;
	const gchar *keywordsKey = "keywords";
	node = json_object_get_member(pJsonObject, keywordsKey);
	if (node !=NULL) {
	
		{
			JsonArray* arr = json_node_get_array(node);
			JsonNode*  temp_json;
			list<KeywordCreateItem> new_list;
			KeywordCreateItem inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("KeywordCreateItem")) {
					jsonToValue(&inst, temp_json, "KeywordCreateItem", "");
				} else {
					
					inst.fromJson(json_to_string(temp_json, false));
					
				}
				new_list.push_back(inst);
			}
			keywords = new_list;
		}
		
	}
	const gchar *parent_idKey = "parent_id";
	node = json_object_get_member(pJsonObject, parent_idKey);
	if (node !=NULL) {
	

		if (isprimitive("std::string")) {
			jsonToValue(&parent_id, node, "std::string", "");
		} else {
			
		}
	}
}

KeywordsCreate::KeywordsCreate(char* json)
{
	this->fromJson(json);
}

char*
KeywordsCreate::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("KeywordCreateItem")) {
		list<KeywordCreateItem> new_list = static_cast<list <KeywordCreateItem> > (getKeywords());
		node = converttoJson(&new_list, "KeywordCreateItem", "array");
	} else {
		node = json_node_alloc();
		list<KeywordCreateItem> new_list = static_cast<list <KeywordCreateItem> > (getKeywords());
		JsonArray* json_array = json_array_new();
		GError *mygerror;
		
		for (list<KeywordCreateItem>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			KeywordCreateItem obj = *it;
			JsonNode *node_temp = json_from_string(obj.toJson(), &mygerror);
			json_array_add_element(json_array, node_temp);
			g_clear_error(&mygerror);
		}
		json_node_init_array(node, json_array);
		json_array_unref(json_array);
		
	}


	
	const gchar *keywordsKey = "keywords";
	json_object_set_member(pJsonObject, keywordsKey, node);
	if (isprimitive("std::string")) {
		std::string obj = getParentId();
		node = converttoJson(&obj, "std::string", "");
	}
	else {
		
	}
	const gchar *parent_idKey = "parent_id";
	json_object_set_member(pJsonObject, parent_idKey, node);
	node = json_node_alloc();
	json_node_init(node, JSON_NODE_OBJECT);
	json_node_take_object(node, pJsonObject);
	char * ret = json_to_string(node, false);
	json_node_free(node);
	return ret;
}

std::list<KeywordCreateItem>
KeywordsCreate::getKeywords()
{
	return keywords;
}

void
KeywordsCreate::setKeywords(std::list <KeywordCreateItem> keywords)
{
	this->keywords = keywords;
}

std::string
KeywordsCreate::getParentId()
{
	return parent_id;
}

void
KeywordsCreate::setParentId(std::string  parent_id)
{
	this->parent_id = parent_id;
}


