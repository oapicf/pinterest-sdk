...
void
Customer_segment_list_200_response::__init()
{
	//bookmark = std::string();
...
void
Customer_segment_list_200_response::__cleanup()
{
	//if(bookmark != NULL) {
	//
	//delete bookmark;
...
void
Customer_segment_list_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<CustomerSegment> new_list;
			CustomerSegment inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("CustomerSegment")) {
					jsonToValue(&inst, temp_json, "CustomerSegment", "");
				} else {
...
Customer_segment_list_200_response::Customer_segment_list_200_response(char* json)
{
	this->fromJson(json);
}
...
Customer_segment_list_200_response::toJson()
{
	...
	if (isprimitive("CustomerSegment")) {
		list<CustomerSegment> new_list = static_cast<list <CustomerSegment> > (getItems());
		node = converttoJson(&new_list, "CustomerSegment", "array");
	} else {
		...
		for (list<CustomerSegment>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			CustomerSegment obj = *it;
