...
void
LeadFormBatchUpdate::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<LeadFormPolicyLink> new_list;
			LeadFormPolicyLink inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("LeadFormPolicyLink")) {
					jsonToValue(&inst, temp_json, "LeadFormPolicyLink", "");
				} else {
...
LeadFormBatchUpdate::LeadFormBatchUpdate(char* json)
{
	this->fromJson(json);
}

char*
LeadFormBatchUpdate::toJson()
{
	...
	if (isprimitive("LeadFormPolicyLink")) {
		list<LeadFormPolicyLink> new_list = static_cast<list <LeadFormPolicyLink> > (getPolicyLinks());
		node = converttoJson(&new_list, "LeadFormPolicyLink", "array");
	} else {
		...
		for (list<LeadFormPolicyLink>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			LeadFormPolicyLink obj = *it;
...
std::string
LeadFormBatchUpdate::getCompletionMessage()
{
	return completion_message;
}

void
LeadFormBatchUpdate::setCompletionMessage(std::string  completion_message)
{
	this->completion_message = completion_message;
}

std::string
LeadFormBatchUpdate::getDisclosureLanguage()
{
	return disclosure_language;
}

void
LeadFormBatchUpdate::setDisclosureLanguage(std::string  disclosure_language)
{
	this->disclosure_language = disclosure_language;
}

bool
LeadFormBatchUpdate::getHasAcceptedTerms()
{
	return has_accepted_terms;
}

void
LeadFormBatchUpdate::setHasAcceptedTerms(bool  has_accepted_terms)
{
	this->has_accepted_terms = has_accepted_terms;
}

std::string
LeadFormBatchUpdate::getId()
{
	return id;
}

void
LeadFormBatchUpdate::setId(std::string  id)
{
	this->id = id;
}

std::string
LeadFormBatchUpdate::getName()
{
	return name;
}

void
LeadFormBatchUpdate::setName(std::string  name)
{
	this->name = name;
}

std::list<LeadFormPolicyLink>
LeadFormBatchUpdate::getPolicyLinks()
{
	return policy_links;
}

void
LeadFormBatchUpdate::setPolicyLinks(std::list <LeadFormPolicyLink> policy_links)
{
	this->policy_links = policy_links;
}

std::string
LeadFormBatchUpdate::getPrivacyPolicyLink()
{
	return privacy_policy_link;
}

void
LeadFormBatchUpdate::setPrivacyPolicyLink(std::string  privacy_policy_link)
{
	this->privacy_policy_link = privacy_policy_link;
}

std::list<LeadFormQuestion>
LeadFormBatchUpdate::getQuestions()
{
	return questions;
}

void
LeadFormBatchUpdate::setQuestions(std::list <LeadFormQuestion> questions)
{
	this->questions = questions;
}

LeadFormStatus
LeadFormBatchUpdate::getStatus()
{
	return status;
}

void
LeadFormBatchUpdate::setStatus(LeadFormStatus  status)
{
	this->status = status;
}
