...
void
Page_visit_conversion_tags_get_200_response::__init()
{
...
void
Page_visit_conversion_tags_get_200_response::__cleanup()
{
	//if(items != NULL) {
...
void
Page_visit_conversion_tags_get_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ConversionEventResponse> new_list;
			ConversionEventResponse inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ConversionEventResponse")) {
					jsonToValue(&inst, temp_json, "ConversionEventResponse", "");
				} else {
...
Page_visit_conversion_tags_get_200_response::Page_visit_conversion_tags_get_200_response(char* json)
{
	this->fromJson(json);
}
...
Page_visit_conversion_tags_get_200_response::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("ConversionEventResponse")) {
		list<ConversionEventResponse> new_list = static_cast<list <ConversionEventResponse> > (getItems());
		node = converttoJson(&new_list, "ConversionEventResponse", "array");
	} else {
		...
		for (list<ConversionEventResponse>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ConversionEventResponse obj = *it;
