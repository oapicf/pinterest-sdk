...
void
Get_invites_200_response::__init()
{
...
void
Get_invites_200_response::__cleanup()
{
	//if(items != NULL) {
...
void
Get_invites_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<InviteResponse> new_list;
			InviteResponse inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("InviteResponse")) {
					jsonToValue(&inst, temp_json, "InviteResponse", "");
				} else {
...
Get_invites_200_response::Get_invites_200_response(char* json)
{
	this->fromJson(json);
}
...
Get_invites_200_response::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("InviteResponse")) {
		list<InviteResponse> new_list = static_cast<list <InviteResponse> > (getItems());
		node = converttoJson(&new_list, "InviteResponse", "array");
	} else {
		...
		for (list<InviteResponse>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			InviteResponse obj = *it;
