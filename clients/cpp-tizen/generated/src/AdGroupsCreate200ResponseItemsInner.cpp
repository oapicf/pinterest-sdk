...
void
Ad_groups_create_200_response_items_inner::__init()
{
	//data = new AdGroup();
...
void
Ad_groups_create_200_response_items_inner::__cleanup()
{
	//if(data != NULL) {
	//
	//delete data;
...
void
Ad_groups_create_200_response_items_inner::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("AdGroup")) {
			jsonToValue(&data, node, "AdGroup", "AdGroup");
		} else {
			
			AdGroup* obj = static_cast<AdGroup*> (&data);
...
Ad_groups_create_200_response_items_inner::Ad_groups_create_200_response_items_inner(char* json)
{
	this->fromJson(json);
}
...
Ad_groups_create_200_response_items_inner::toJson()
{
	...
	if (isprimitive("AdGroup")) {
		AdGroup obj = getData();
		node = converttoJson(&obj, "AdGroup", "");
	}
	else {
		
		AdGroup obj = static_cast<AdGroup> (getData());
		GError *mygerror;
		mygerror = NULL;
