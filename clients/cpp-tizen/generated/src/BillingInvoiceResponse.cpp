...
void
BillingInvoiceResponse::__cleanup()
{
...
void
BillingInvoiceResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("std::string")) {
			jsonToValue(&document_type, node, "std::string", "");
		} else {
...
BillingInvoiceResponse::BillingInvoiceResponse(char* json)
{
	this->fromJson(json);
}
