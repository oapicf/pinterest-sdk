...
Ad::__init()
{
	...
	//new std::list()std::list> carting_products;
...
Ad::__cleanup()
{
	...
	//}
	//if(carting_platform_type != NULL) {
	//
	//delete carting_platform_type;
	//carting_platform_type = NULL;
	//}
	//if(carting_products != NULL) {
	//carting_products.RemoveAll(true);
	//delete carting_products;
...
Ad::fromJson(char* jsonStr)
{
	...
	const gchar *carting_platform_typeKey = "carting_platform_type";
	node = json_object_get_member(pJsonObject, carting_platform_typeKey);
	if (node !=NULL) {
	

		if (isprimitive("int")) {
			jsonToValue(&carting_platform_type, node, "int", "");
		} else {
			
		}
	}
	const gchar *carting_productsKey = "carting_products";
	node = json_object_get_member(pJsonObject, carting_productsKey);
	if (node !=NULL) {
	
		{
			JsonArray* arr = json_node_get_array(node);
			JsonNode*  temp_json;
			list<CartingProduct> new_list;
			CartingProduct inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("CartingProduct")) {
					jsonToValue(&inst, temp_json, "CartingProduct", "");
				} else {
	...
	if (node !=NULL) {
	
		{
			JsonArray* arr = json_node_get_array(node);
			JsonNode*  temp_json;
			list<AdDisapprovalReasons> new_list;
			AdDisapprovalReasons inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("AdDisapprovalReasons")) {
					jsonToValue(&inst, temp_json, "AdDisapprovalReasons", "");
				} else {
	...
	if (node !=NULL) {
		...
		if (isprimitive("AdReviewStatus")) {
			jsonToValue(&review_status, node, "AdReviewStatus", "AdReviewStatus");
		} else {
			
			AdReviewStatus* obj = static_cast<AdReviewStatus*> (&review_status);
			obj->fromJson(json_to_string(node, false));
...
Ad::toJson()
{
	...
	if (isprimitive("int")) {
		int obj = getCartingPlatformType();
	...
	const gchar *carting_platform_typeKey = "carting_platform_type";
	json_object_set_member(pJsonObject, carting_platform_typeKey, node);
	if (isprimitive("CartingProduct")) {
		list<CartingProduct> new_list = static_cast<list <CartingProduct> > (getCartingProducts());
		node = converttoJson(&new_list, "CartingProduct", "array");
	} else {
		node = json_node_alloc();
		list<CartingProduct> new_list = static_cast<list <CartingProduct> > (getCartingProducts());
		JsonArray* json_array = json_array_new();
		...
		for (list<CartingProduct>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			CartingProduct obj = *it;
	...
	const gchar *carting_productsKey = "carting_products";
	json_object_set_member(pJsonObject, carting_productsKey, node);
	if (isprimitive("std::string")) {
	...
	if (isprimitive("AdDisapprovalReasons")) {
		list<AdDisapprovalReasons> new_list = static_cast<list <AdDisapprovalReasons> > (getRejectedReasons());
		node = converttoJson(&new_list, "AdDisapprovalReasons", "array");
	} else {
		node = json_node_alloc();
		list<AdDisapprovalReasons> new_list = static_cast<list <AdDisapprovalReasons> > (getRejectedReasons());
		JsonArray* json_array = json_array_new();
		...
		for (list<AdDisapprovalReasons>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			AdDisapprovalReasons obj = *it;
			JsonNode *node_temp = json_from_string(obj.toJson(), &mygerror);
			json_array_add_element(json_array, node_temp);
			g_clear_error(&mygerror);
		}
	...
	if (isprimitive("AdReviewStatus")) {
		AdReviewStatus obj = getReviewStatus();
		node = converttoJson(&obj, "AdReviewStatus", "");
	}
	else {
		
		AdReviewStatus obj = static_cast<AdReviewStatus> (getReviewStatus());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
...
bool
Ad::getIsCollageSingleDestination()
{
