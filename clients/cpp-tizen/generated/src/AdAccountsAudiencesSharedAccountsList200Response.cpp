...
void
Ad_accounts_audiences_shared_accounts_list_200_response::__init()
{
...
void
Ad_accounts_audiences_shared_accounts_list_200_response::__cleanup()
{
	//if(items != NULL) {
...
void
Ad_accounts_audiences_shared_accounts_list_200_response::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<SharedAudienceAccount> new_list;
			SharedAudienceAccount inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("SharedAudienceAccount")) {
					jsonToValue(&inst, temp_json, "SharedAudienceAccount", "");
				} else {
...
Ad_accounts_audiences_shared_accounts_list_200_response::Ad_accounts_audiences_shared_accounts_list_200_response(char* json)
{
	this->fromJson(json);
}
...
Ad_accounts_audiences_shared_accounts_list_200_response::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("SharedAudienceAccount")) {
		list<SharedAudienceAccount> new_list = static_cast<list <SharedAudienceAccount> > (getItems());
		node = converttoJson(&new_list, "SharedAudienceAccount", "array");
	} else {
		...
		for (list<SharedAudienceAccount>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			SharedAudienceAccount obj = *it;
