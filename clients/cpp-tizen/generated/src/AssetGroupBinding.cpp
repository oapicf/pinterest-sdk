...
AssetGroupBinding::__init()
{
	...
	//new std::list()std::list> catalogs_ids;
	//created_by = null;
	//created_time = int(0);
	//id = std::string();
	//owner = null;
...
AssetGroupBinding::fromJson(char* jsonStr)
{
	...
	const gchar *catalogs_idsKey = "catalogs_ids";
	node = json_object_get_member(pJsonObject, catalogs_idsKey);
	if (node !=NULL) {
...
AssetGroupBinding::toJson()
{
	...
	if (isprimitive("std::string")) {
		list<std::string> new_list = static_cast<list <std::string> > (getCatalogsIds());
		node = converttoJson(&new_list, "std::string", "array");
	} else {
		node = json_node_alloc();
		list<std::string> new_list = static_cast<list <std::string> > (getCatalogsIds());
	...
	const gchar *catalogs_idsKey = "catalogs_ids";
	json_object_set_member(pJsonObject, catalogs_idsKey, node);
	if (isprimitive("BusinessAccessUserSummary")) {
