...
void
Media_list_200_response::__init()
{
	//bookmark = std::string();
...
void
Media_list_200_response::__cleanup()
{
	//if(bookmark != NULL) {
	//
	//delete bookmark;
...
void
Media_list_200_response::fromJson(char* jsonStr)
{
	...
	
		{
			...
			for (guint i=0;i<json_array_get_length(arr);i++) {
				...
				if (isprimitive("Media")) {
					jsonToValue(&inst, temp_json, "Media", "");
				} else {
...
Media_list_200_response::Media_list_200_response(char* json)
{
	this->fromJson(json);
}
...
Media_list_200_response::toJson()
{
	...
	if (isprimitive("Media")) {
		list<Media> new_list = static_cast<list <Media> > (getItems());
		node = converttoJson(&new_list, "Media", "array");
	} else {
		...
		for (list<Media>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Media obj = *it;
