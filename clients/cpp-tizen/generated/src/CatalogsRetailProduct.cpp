...
void
CatalogsRetailProduct::__init()
{
	//catalog_type = std::string();
	//metadata = new CatalogsRetailProductMetadata();
	//pin = new Pin();
}

void
CatalogsRetailProduct::__cleanup()
{
...
void
CatalogsRetailProduct::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("CatalogsRetailProductMetadata")) {
			jsonToValue(&metadata, node, "CatalogsRetailProductMetadata", "CatalogsRetailProductMetadata");
		} else {
			
			CatalogsRetailProductMetadata* obj = static_cast<CatalogsRetailProductMetadata*> (&metadata);
...
CatalogsRetailProduct::CatalogsRetailProduct(char* json)
{
	this->fromJson(json);
}
...
CatalogsRetailProduct::toJson()
{
	...
	if (isprimitive("CatalogsRetailProductMetadata")) {
		CatalogsRetailProductMetadata obj = getMetadata();
		node = converttoJson(&obj, "CatalogsRetailProductMetadata", "");
	}
	else {
		
		CatalogsRetailProductMetadata obj = static_cast<CatalogsRetailProductMetadata> (getMetadata());
		GError *mygerror;
		mygerror = NULL;
