...
void
CampaignBatchItem::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("CampaignBatchResponseData")) {
			jsonToValue(&data, node, "CampaignBatchResponseData", "CampaignBatchResponseData");
		} else {
			
			CampaignBatchResponseData* obj = static_cast<CampaignBatchResponseData*> (&data);
...
CampaignBatchItem::CampaignBatchItem(char* json)
{
	this->fromJson(json);
}
...
CampaignBatchItem::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("CampaignBatchResponseData")) {
		CampaignBatchResponseData obj = getData();
		node = converttoJson(&obj, "CampaignBatchResponseData", "");
	}
	else {
		
		CampaignBatchResponseData obj = static_cast<CampaignBatchResponseData> (getData());
		GError *mygerror;
		mygerror = NULL;
