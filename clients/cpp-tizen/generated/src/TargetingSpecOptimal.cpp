...
void
TargetingSpecOptimal::__cleanup()
{
	//if(aGE_BUCKET != NULL) {
...
void
TargetingSpecOptimal::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			JsonArray* arr = json_node_get_array(node);
			JsonNode*  temp_json;
			list<TargetingStrategy> new_list;
			TargetingStrategy inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("TargetingStrategy")) {
					jsonToValue(&inst, temp_json, "TargetingStrategy", "");
				} else {
...
TargetingSpecOptimal::TargetingSpecOptimal(char* json)
{
	this->fromJson(json);
}
...
TargetingSpecOptimal::toJson()
{
	...
	if (isprimitive("TargetingStrategy")) {
		list<TargetingStrategy> new_list = static_cast<list <TargetingStrategy> > (getTARGETINGSTRATEGY());
		node = converttoJson(&new_list, "TargetingStrategy", "array");
	} else {
		node = json_node_alloc();
		list<TargetingStrategy> new_list = static_cast<list <TargetingStrategy> > (getTARGETINGSTRATEGY());
		JsonArray* json_array = json_array_new();
		...
		for (list<TargetingStrategy>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			TargetingStrategy obj = *it;
			JsonNode *node_temp = json_from_string(obj.toJson(), &mygerror);
			json_array_add_element(json_array, node_temp);
			g_clear_error(&mygerror);
		}
