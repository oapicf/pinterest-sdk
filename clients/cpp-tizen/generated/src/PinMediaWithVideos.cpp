...
PinMediaWithVideos::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<VideoMetadataWithItemType> new_list;
			VideoMetadataWithItemType inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("VideoMetadataWithItemType")) {
					jsonToValue(&inst, temp_json, "VideoMetadataWithItemType", "");
				} else {
...
PinMediaWithVideos::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("VideoMetadataWithItemType")) {
		list<VideoMetadataWithItemType> new_list = static_cast<list <VideoMetadataWithItemType> > (getItems());
		node = converttoJson(&new_list, "VideoMetadataWithItemType", "array");
	} else {
		...
		for (list<VideoMetadataWithItemType>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			VideoMetadataWithItemType obj = *it;
