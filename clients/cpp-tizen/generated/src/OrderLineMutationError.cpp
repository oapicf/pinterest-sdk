...
void
OrderLineMutationError::__cleanup()
{
	//if(data != NULL) {
	//
	//delete data;
...
void
OrderLineMutationError::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("OrderLine")) {
			jsonToValue(&data, node, "OrderLine", "OrderLine");
		} else {
...
OrderLineMutationError::OrderLineMutationError(char* json)
{
	this->fromJson(json);
}
