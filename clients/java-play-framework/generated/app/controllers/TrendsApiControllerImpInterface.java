...
public abstract class TrendsApiControllerImpInterface {
    @Inject private Config configuration;
    @Inject private SecurityAPIUtils securityAPIUtils;
    private ObjectMapper mapper = new ObjectMapper();

    public Result trendsEditorialArticlesListHttp(Http.Request request, @NotNull ProductCategoryRegion region) throws Exception {
        if (!securityAPIUtils.isRequestTokenValid(request, "pinterest_oauth2")) {
            return unauthorized();
        }

        List<TrendsEditorial> obj = trendsEditorialArticlesList(request, region);

        if (configuration.getBoolean("useOutputBeanValidation")) {
            for (TrendsEditorial curItem : obj) {
    ...
    public abstract List<TrendsEditorial> trendsEditorialArticlesList(Http.Request request, @NotNull ProductCategoryRegion region) throws Exception;
