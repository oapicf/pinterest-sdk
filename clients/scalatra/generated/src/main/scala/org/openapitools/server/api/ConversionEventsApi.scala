...
class ConversionEventsApi(implicit val swagger: Swagger) extends ScalatraServlet
  ...
  val eventsCreateOperation = (apiOperation[ConversionEvents]("eventsCreate")
    summary "Send conversions"
    parameters(pathParam[String]("adAccountId").description(""), bodyParam[ConversionEventsCreate]("conversionEventsCreate").description(""), queryParam[Boolean]("test").description("").optional)
  )
  ...
  post("/ad_accounts/:ad_account_id/events", operation(eventsCreateOperation)) {
    ...
    //println("conversionEventsCreate: " + conversionEventsCreate)
