...
class KeywordsApi(implicit val swagger: Swagger) extends ScalatraServlet
  ...
  val keywordsCreateOperation = (apiOperation[Keywords]("keywordsCreate")
    summary "Create keywords"
    parameters(pathParam[String]("adAccountId").description(""), bodyParam[KeywordsCreate]("keywordsCreate").description(""))
  )

  post("/ad_accounts/:ad_account_id/keywords", operation(keywordsCreateOperation)) {
    val adAccountId = params.getOrElse("adAccountId", halt(400))
    //println("adAccountId: " + adAccountId)
    //println("keywordsCreate: " + keywordsCreate)
  }
  ...
  val keywordsUpdateOperation = (apiOperation[Keywords]("keywordsUpdate")
    summary "Update keywords"
    parameters(pathParam[String]("adAccountId").description(""), bodyParam[KeywordsUpdate]("keywordsUpdate").description(""))
  )

  patch("/ad_accounts/:ad_account_id/keywords", operation(keywordsUpdateOperation)) {
    val adAccountId = params.getOrElse("adAccountId", halt(400))
    //println("adAccountId: " + adAccountId)
    //println("keywordsUpdate: " + keywordsUpdate)
  }
  ...
  val trendingKeywordsListOperation = (apiOperation[TrendingKeywordsResponse]("trendingKeywordsList")
    ...
    parameters(pathParam[TrendsSupportedRegion]("region").description(""), pathParam[TrendType]("trendType").description(""), queryParam[List[TrendsL1Interest]]("interests").description("").optional.defaultValue(List.empty[TrendsL1Interest] ), queryParam[List[TrendsGenderFilter]]("genders").description("").optional.defaultValue(List.empty[TrendsGenderFilter] ), queryParam[List[TrendsAgeBucket]]("ages").description("").optional.defaultValue(List.empty[TrendsAgeBucket] ), queryParam[List[String]]("includeKeywords").description("").optional.defaultValue(List.empty[String] ), queryParam[Boolean]("normalizeAgainstGroup").description("").optional.defaultValue(false), queryParam[Int]("limit").description("").optional.defaultValue(50), queryParam[Boolean]("includeDemographics").description("").optional.defaultValue(false))
  )
  ...
  get("/trends/keywords/:region/top/:trend_type", operation(trendingKeywordsListOperation)) {
    ...
    //println("limit: " + limit)
            val includeDemographics = params.getAs[Boolean]("includeDemographics")

    //println("includeDemographics: " + includeDemographics)
  }
