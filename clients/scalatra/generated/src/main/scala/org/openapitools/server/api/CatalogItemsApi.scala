...
class CatalogItemsApi(implicit val swagger: Swagger) extends ScalatraServlet
  ...
  val itemsBatchPostOperation = (apiOperation[CatalogsItemsBatch]("itemsBatchPost")
    ...
    parameters(bodyParam[CatalogsItemsBatchPostRequest]("catalogsItemsBatchPostRequest").description(""), queryParam[String]("adAccountId").description("").optional)
  )

  post("/catalogs/items/batch", operation(itemsBatchPostOperation)) {
    //println("catalogsItemsBatchPostRequest: " + catalogsItemsBatchPostRequest)
            val adAccountId = params.getAs[String]("adAccountId")
  ...
  val itemsPostOperation = (apiOperation[ItemsPost200Response]("itemsPost")
