...
class BulkApi(implicit val swagger: Swagger) extends ScalatraServlet
  ...
  val bulkDownloadCreateOperation = (apiOperation[BulkDownload]("bulkDownloadCreate")
    summary "Get advertiser entities in bulk"
    parameters(pathParam[String]("adAccountId").description(""), bodyParam[BulkDownloadCreate]("bulkDownloadCreate").description(""))
  )
  ...
  post("/ad_accounts/:ad_account_id/bulk/download", operation(bulkDownloadCreateOperation)) {
    ...
    //println("bulkDownloadCreate: " + bulkDownloadCreate)
  }

  

  val bulkRequestGetOperation = (apiOperation[BulkJobData]("bulkRequestGet")
