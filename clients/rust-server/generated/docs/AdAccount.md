# AdAccount

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**country** | [***models::Country**](Country.md) |  | [optional] [default to None]
**created_time** | **i32** |  Creation time. Unix timestamp in seconds. | [optional] [readonly] [default to None]
**currency** | [***models::Currency**](Currency.md) |  | [optional] [default to None]
**id** | **String** |  | 
**name** | **String** | Ad account name. | [optional] [default to None]
**owner** | [***models::AdAccountOwner**](AdAccountOwner.md) | Ad account owner | [optional] [readonly] [default to None]
**permissions** | [**Vec<models::BusinessAccessRole>**](BusinessAccessRole.md) |  | [optional] [readonly] [default to None]
**time_zone** | **String** | The time zone of the ad account, in IANA format (e.g., \"America/Los_Angeles\"). Adding your local time zone lets you view your campaigns and ad reporting in your preferred time zone. Future reports will be available in both your local time zone and default UTC time zone. Historical data takes 1-2 months to backfill. Your billing and order lines will remain in UTC. | [optional] [default to None]
**updated_time** | **i32** |  | [optional] [readonly] [default to None]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


