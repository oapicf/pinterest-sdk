    ...
    override fun handle(msg: Message<JsonObject>) {
        try {
            ...
            when (action) {
                ...
                "mediaGet" -> {
                    val params = context.params
                    val mediaId = ApiHandlerUtils.searchStringInJson(params,"media_id")
                    if(mediaId == null){
                        throw IllegalArgumentException("mediaId is required")
                    }
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.mediaGet(mediaId,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "mediaList" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.mediaList(bookmark,pageSize,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
