...
import com.prokarma.pkmst.model.Keywords;
import com.prokarma.pkmst.model.KeywordsCreate;
import com.prokarma.pkmst.model.KeywordsGet200Response;
import com.prokarma.pkmst.model.KeywordsMetricsArrayResponse;
import com.prokarma.pkmst.model.KeywordsUpdate;
...
import com.prokarma.pkmst.model.TrendsGenderFilter;
import com.prokarma.pkmst.model.TrendsL1Interest;
...
@Autowired
    ...
    public ResponseEntity<Keywords> keywordsCreate(@ApiParam(value = "Unique identifier of an ad account.",required=true ) @PathVariable("ad_account_id") String adAccountId,
        @ApiParam(value = "" ,required=true )   @RequestBody KeywordsCreate keywordsCreate,
        @RequestHeader(value = "Accept", required = false) String accept) throws Exception {
        // do some magic!

        if (accept != null && accept.contains("application/json")) {
            return new ResponseEntity<Keywords>(objectMapper.readValue("", Keywords.class), HttpStatus.OK);
        }
    ...
    public ResponseEntity<KeywordsGet200Response> keywordsGet(@ApiParam(value = "Unique identifier of an ad account.",required=true ) @PathVariable("ad_account_id") String adAccountId,
        ...
        @ApiParam(value = "Keyword [match type](/docs/api-features/targeting-overview/)")  @RequestParam(value = "match_types", required = false) List<MatchType> matchTypes,
    ...
    public ResponseEntity<Keywords> keywordsUpdate(@ApiParam(value = "Unique identifier of an ad account.",required=true ) @PathVariable("ad_account_id") String adAccountId,
        @ApiParam(value = "" ,required=true )   @RequestBody KeywordsUpdate keywordsUpdate,
        @RequestHeader(value = "Accept", required = false) String accept) throws Exception {
        // do some magic!
    ...
    public ResponseEntity<TrendingKeywordsResponse> trendingKeywordsList(@ApiParam(value = "  The geographic region of interest. Only top trends within the specified region will be returned.    The `region` parameter is formatted as ISO 3166-2 country codes delimited by `+`, corresponding to the following geographic areas:   - `US` - United States   - `CA` - Canada   - `DE` - Germany   - `FR` - France   - `ES` - Spain   - `IT` - Italy   - `DE+AT+CH` - Germanic countries   - `GB+IE` - Great Britain & Ireland   - `IT+ES+PT+GR+MT` - Southern Europe   - `PL+RO+HU+SK+CZ` - Eastern Europe   - `SE+DK+FI+NO` - Nordic countries   - `NL+BE+LU` - Benelux   - `AR` - Argentina   - `BR` - Brazil   - `CO` - Colombia   - `MX` - Mexico   - `MX+AR+CO+CL` - Hispanic LatAm   - `AU+NZ` - Australasia",required=true, allowableValues="{values=[US, CA, DE, FR, ES, IT, DE+AT+CH, GB+IE, IT+ES+PT+GR+MT, PL+RO+HU+SK+CZ, SE+DK+FI+NO, NL+BE+LU, AR, BR, CO, MX, MX+AR+CO+CL, AU+NZ], enumVars=[{name=US, isString=true, value="US"}, {name=CA, isString=true, value="CA"}, {name=DE, isString=true, value="DE"}, {name=FR, isString=true, value="FR"}, {name=ES, isString=true, value="ES"}, {name=IT, isString=true, value="IT"}, {name=DE_AT_CH, isString=true, value="DE+AT+CH"}, {name=GB_IE, isString=true, value="GB+IE"}, {name=IT_ES_PT_GR_MT, isString=true, value="IT+ES+PT+GR+MT"}, {name=PL_RO_HU_SK_CZ, isString=true, value="PL+RO+HU+SK+CZ"}, {name=SE_DK_FI_NO, isString=true, value="SE+DK+FI+NO"}, {name=NL_BE_LU, isString=true, value="NL+BE+LU"}, {name=AR, isString=true, value="AR"}, {name=BR, isString=true, value="BR"}, {name=CO, isString=true, value="CO"}, {name=MX, isString=true, value="MX"}, {name=MX_AR_CO_CL, isString=true, value="MX+AR+CO+CL"}, {name=AU_NZ, isString=true, value="AU+NZ"}]}" ) @PathVariable("region") TrendsSupportedRegion region,
        ...
        @ApiParam(value = "  The list of supported interests is:   - `animals` - Animals   - `architecture` - Architecture   - `art` - Art   - `beauty` - Beauty   - `childrens_fashion` - Children's Fashion   - `design` - Design   - `diy_and_crafts` - DIY & Crafts   - `education` - Education   - `electronics` - Electronics   - `entertainment` - Entertainment   - `event_planning` - Event Planning   - `finance` - Finance   - `food_and_drinks` - Food & Drink   - `gardening` - Gardening   - `health` - Health   - `home_decor` - Home Decor   - `mens_fashion` - Men's Fashion   - `parenting` - Parenting   - `quotes` - Quotes   - `sport` - Sports   - `travel` - Travel   - `vehicles` - Vehicles   - `wedding` - Wedding   - `womens_fashion` - Women's Fashion")  @RequestParam(value = "interests", required = false) List<TrendsL1Interest> interests,
        @ApiParam(value = "If set, filters the results to trends among users who identify with the specified gender(s). If unset, trends among all genders will be returned. The `unknown` group includes users with unspecified or customized gender profile settings.")  @RequestParam(value = "genders", required = false) List<TrendsGenderFilter> genders,
        @ApiParam(value = "If set, filters the results to trends among users in the specified age range(s). If unset, trends among all age groups will be returned.")  @RequestParam(value = "ages", required = false) List<TrendsAgeBucket> ages,
        @ApiParam(value = "If set, filters the results to top trends which include at least one of the specified keywords. If unset, no keyword filtering logic is applied.")  @RequestParam(value = "include_keywords", required = false) List<String> includeKeywords,
        ...
        @ApiParam(value = "Including the age and gender distribution for each keyword. By default (`false`), the response will not include demographics data.", defaultValue = "false")  @RequestParam(value = "include_demographics", required = false, defaultValue="false") Boolean includeDemographics,
        @RequestHeader(value = "Accept", required = false) String accept) throws Exception {
        // do some magic!
