// <auto-generated>
/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

#nullable enable

using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Text.Json;
using System.Text.Json.Serialization;
using System.ComponentModel.DataAnnotations;
using Org.OpenAPITools.Client;

namespace Org.OpenAPITools.Model
{
    /// <summary>
    /// Currency Codes from ISO 4217.
    /// </summary>
    /// <value>Currency Codes from ISO 4217.</value>
    public enum NonNullableCatalogsCurrency
    {
        /// <summary>
        /// Enum AED for value: AED
        /// </summary>
        AED = 1,

        /// <summary>
        /// Enum AFN for value: AFN
        /// </summary>
        AFN = 2,

        /// <summary>
        /// Enum ALL for value: ALL
        /// </summary>
        ALL = 3,

        /// <summary>
        /// Enum AMD for value: AMD
        /// </summary>
        AMD = 4,

        /// <summary>
        /// Enum ANG for value: ANG
        /// </summary>
        ANG = 5,

        /// <summary>
        /// Enum AOA for value: AOA
        /// </summary>
        AOA = 6,

        /// <summary>
        /// Enum ARS for value: ARS
        /// </summary>
        ARS = 7,

        /// <summary>
        /// Enum AUD for value: AUD
        /// </summary>
        AUD = 8,

        /// <summary>
        /// Enum AWG for value: AWG
        /// </summary>
        AWG = 9,

        /// <summary>
        /// Enum AZN for value: AZN
        /// </summary>
        AZN = 10,

        /// <summary>
        /// Enum BAM for value: BAM
        /// </summary>
        BAM = 11,

        /// <summary>
        /// Enum BBD for value: BBD
        /// </summary>
        BBD = 12,

        /// <summary>
        /// Enum BDT for value: BDT
        /// </summary>
        BDT = 13,

        /// <summary>
        /// Enum BGN for value: BGN
        /// </summary>
        BGN = 14,

        /// <summary>
        /// Enum BHD for value: BHD
        /// </summary>
        BHD = 15,

        /// <summary>
        /// Enum BIF for value: BIF
        /// </summary>
        BIF = 16,

        /// <summary>
        /// Enum BMD for value: BMD
        /// </summary>
        BMD = 17,

        /// <summary>
        /// Enum BND for value: BND
        /// </summary>
        BND = 18,

        /// <summary>
        /// Enum BOB for value: BOB
        /// </summary>
        BOB = 19,

        /// <summary>
        /// Enum BRL for value: BRL
        /// </summary>
        BRL = 20,

        /// <summary>
        /// Enum BSD for value: BSD
        /// </summary>
        BSD = 21,

        /// <summary>
        /// Enum BTN for value: BTN
        /// </summary>
        BTN = 22,

        /// <summary>
        /// Enum BWP for value: BWP
        /// </summary>
        BWP = 23,

        /// <summary>
        /// Enum BYN for value: BYN
        /// </summary>
        BYN = 24,

        /// <summary>
        /// Enum BYR for value: BYR
        /// </summary>
        BYR = 25,

        /// <summary>
        /// Enum BZD for value: BZD
        /// </summary>
        BZD = 26,

        /// <summary>
        /// Enum CAD for value: CAD
        /// </summary>
        CAD = 27,

        /// <summary>
        /// Enum CDF for value: CDF
        /// </summary>
        CDF = 28,

        /// <summary>
        /// Enum CHF for value: CHF
        /// </summary>
        CHF = 29,

        /// <summary>
        /// Enum CLP for value: CLP
        /// </summary>
        CLP = 30,

        /// <summary>
        /// Enum CNY for value: CNY
        /// </summary>
        CNY = 31,

        /// <summary>
        /// Enum COP for value: COP
        /// </summary>
        COP = 32,

        /// <summary>
        /// Enum CRC for value: CRC
        /// </summary>
        CRC = 33,

        /// <summary>
        /// Enum CUC for value: CUC
        /// </summary>
        CUC = 34,

        /// <summary>
        /// Enum CUP for value: CUP
        /// </summary>
        CUP = 35,

        /// <summary>
        /// Enum CVE for value: CVE
        /// </summary>
        CVE = 36,

        /// <summary>
        /// Enum CZK for value: CZK
        /// </summary>
        CZK = 37,

        /// <summary>
        /// Enum DJF for value: DJF
        /// </summary>
        DJF = 38,

        /// <summary>
        /// Enum DKK for value: DKK
        /// </summary>
        DKK = 39,

        /// <summary>
        /// Enum DOP for value: DOP
        /// </summary>
        DOP = 40,

        /// <summary>
        /// Enum DZD for value: DZD
        /// </summary>
        DZD = 41,

        /// <summary>
        /// Enum EGP for value: EGP
        /// </summary>
        EGP = 42,

        /// <summary>
        /// Enum ERN for value: ERN
        /// </summary>
        ERN = 43,

        /// <summary>
        /// Enum ETB for value: ETB
        /// </summary>
        ETB = 44,

        /// <summary>
        /// Enum EUR for value: EUR
        /// </summary>
        EUR = 45,

        /// <summary>
        /// Enum FJD for value: FJD
        /// </summary>
        FJD = 46,

        /// <summary>
        /// Enum FKP for value: FKP
        /// </summary>
        FKP = 47,

        /// <summary>
        /// Enum GBP for value: GBP
        /// </summary>
        GBP = 48,

        /// <summary>
        /// Enum GEL for value: GEL
        /// </summary>
        GEL = 49,

        /// <summary>
        /// Enum GGP for value: GGP
        /// </summary>
        GGP = 50,

        /// <summary>
        /// Enum GHS for value: GHS
        /// </summary>
        GHS = 51,

        /// <summary>
        /// Enum GIP for value: GIP
        /// </summary>
        GIP = 52,

        /// <summary>
        /// Enum GMD for value: GMD
        /// </summary>
        GMD = 53,

        /// <summary>
        /// Enum GNF for value: GNF
        /// </summary>
        GNF = 54,

        /// <summary>
        /// Enum GTQ for value: GTQ
        /// </summary>
        GTQ = 55,

        /// <summary>
        /// Enum GYD for value: GYD
        /// </summary>
        GYD = 56,

        /// <summary>
        /// Enum HKD for value: HKD
        /// </summary>
        HKD = 57,

        /// <summary>
        /// Enum HNL for value: HNL
        /// </summary>
        HNL = 58,

        /// <summary>
        /// Enum HRK for value: HRK
        /// </summary>
        HRK = 59,

        /// <summary>
        /// Enum HTG for value: HTG
        /// </summary>
        HTG = 60,

        /// <summary>
        /// Enum HUF for value: HUF
        /// </summary>
        HUF = 61,

        /// <summary>
        /// Enum IDR for value: IDR
        /// </summary>
        IDR = 62,

        /// <summary>
        /// Enum ILS for value: ILS
        /// </summary>
        ILS = 63,

        /// <summary>
        /// Enum IMP for value: IMP
        /// </summary>
        IMP = 64,

        /// <summary>
        /// Enum INR for value: INR
        /// </summary>
        INR = 65,

        /// <summary>
        /// Enum IQD for value: IQD
        /// </summary>
        IQD = 66,

        /// <summary>
        /// Enum IRR for value: IRR
        /// </summary>
        IRR = 67,

        /// <summary>
        /// Enum ISK for value: ISK
        /// </summary>
        ISK = 68,

        /// <summary>
        /// Enum JEP for value: JEP
        /// </summary>
        JEP = 69,

        /// <summary>
        /// Enum JMD for value: JMD
        /// </summary>
        JMD = 70,

        /// <summary>
        /// Enum JOD for value: JOD
        /// </summary>
        JOD = 71,

        /// <summary>
        /// Enum JPY for value: JPY
        /// </summary>
        JPY = 72,

        /// <summary>
        /// Enum KES for value: KES
        /// </summary>
        KES = 73,

        /// <summary>
        /// Enum KGS for value: KGS
        /// </summary>
        KGS = 74,

        /// <summary>
        /// Enum KHR for value: KHR
        /// </summary>
        KHR = 75,

        /// <summary>
        /// Enum KMF for value: KMF
        /// </summary>
        KMF = 76,

        /// <summary>
        /// Enum KPW for value: KPW
        /// </summary>
        KPW = 77,

        /// <summary>
        /// Enum KRW for value: KRW
        /// </summary>
        KRW = 78,

        /// <summary>
        /// Enum KWD for value: KWD
        /// </summary>
        KWD = 79,

        /// <summary>
        /// Enum KYD for value: KYD
        /// </summary>
        KYD = 80,

        /// <summary>
        /// Enum KZT for value: KZT
        /// </summary>
        KZT = 81,

        /// <summary>
        /// Enum LAK for value: LAK
        /// </summary>
        LAK = 82,

        /// <summary>
        /// Enum LBP for value: LBP
        /// </summary>
        LBP = 83,

        /// <summary>
        /// Enum LKR for value: LKR
        /// </summary>
        LKR = 84,

        /// <summary>
        /// Enum LRD for value: LRD
        /// </summary>
        LRD = 85,

        /// <summary>
        /// Enum LSL for value: LSL
        /// </summary>
        LSL = 86,

        /// <summary>
        /// Enum LYD for value: LYD
        /// </summary>
        LYD = 87,

        /// <summary>
        /// Enum MAD for value: MAD
        /// </summary>
        MAD = 88,

        /// <summary>
        /// Enum MDL for value: MDL
        /// </summary>
        MDL = 89,

        /// <summary>
        /// Enum MGA for value: MGA
        /// </summary>
        MGA = 90,

        /// <summary>
        /// Enum MKD for value: MKD
        /// </summary>
        MKD = 91,

        /// <summary>
        /// Enum MMK for value: MMK
        /// </summary>
        MMK = 92,

        /// <summary>
        /// Enum MNT for value: MNT
        /// </summary>
        MNT = 93,

        /// <summary>
        /// Enum MOP for value: MOP
        /// </summary>
        MOP = 94,

        /// <summary>
        /// Enum MRO for value: MRO
        /// </summary>
        MRO = 95,

        /// <summary>
        /// Enum MUR for value: MUR
        /// </summary>
        MUR = 96,

        /// <summary>
        /// Enum MVR for value: MVR
        /// </summary>
        MVR = 97,

        /// <summary>
        /// Enum MWK for value: MWK
        /// </summary>
        MWK = 98,

        /// <summary>
        /// Enum MXN for value: MXN
        /// </summary>
        MXN = 99,

        /// <summary>
        /// Enum MYR for value: MYR
        /// </summary>
        MYR = 100,

        /// <summary>
        /// Enum MZN for value: MZN
        /// </summary>
        MZN = 101,

        /// <summary>
        /// Enum NAD for value: NAD
        /// </summary>
        NAD = 102,

        /// <summary>
        /// Enum NGN for value: NGN
        /// </summary>
        NGN = 103,

        /// <summary>
        /// Enum NIO for value: NIO
        /// </summary>
        NIO = 104,

        /// <summary>
        /// Enum NOK for value: NOK
        /// </summary>
        NOK = 105,

        /// <summary>
        /// Enum NPR for value: NPR
        /// </summary>
        NPR = 106,

        /// <summary>
        /// Enum NZD for value: NZD
        /// </summary>
        NZD = 107,

        /// <summary>
        /// Enum OMR for value: OMR
        /// </summary>
        OMR = 108,

        /// <summary>
        /// Enum PAB for value: PAB
        /// </summary>
        PAB = 109,

        /// <summary>
        /// Enum PEN for value: PEN
        /// </summary>
        PEN = 110,

        /// <summary>
        /// Enum PGK for value: PGK
        /// </summary>
        PGK = 111,

        /// <summary>
        /// Enum PHP for value: PHP
        /// </summary>
        PHP = 112,

        /// <summary>
        /// Enum PKR for value: PKR
        /// </summary>
        PKR = 113,

        /// <summary>
        /// Enum PLN for value: PLN
        /// </summary>
        PLN = 114,

        /// <summary>
        /// Enum PYG for value: PYG
        /// </summary>
        PYG = 115,

        /// <summary>
        /// Enum QAR for value: QAR
        /// </summary>
        QAR = 116,

        /// <summary>
        /// Enum RON for value: RON
        /// </summary>
        RON = 117,

        /// <summary>
        /// Enum RSD for value: RSD
        /// </summary>
        RSD = 118,

        /// <summary>
        /// Enum RUB for value: RUB
        /// </summary>
        RUB = 119,

        /// <summary>
        /// Enum RWF for value: RWF
        /// </summary>
        RWF = 120,

        /// <summary>
        /// Enum SAR for value: SAR
        /// </summary>
        SAR = 121,

        /// <summary>
        /// Enum SBD for value: SBD
        /// </summary>
        SBD = 122,

        /// <summary>
        /// Enum SCR for value: SCR
        /// </summary>
        SCR = 123,

        /// <summary>
        /// Enum SDG for value: SDG
        /// </summary>
        SDG = 124,

        /// <summary>
        /// Enum SEK for value: SEK
        /// </summary>
        SEK = 125,

        /// <summary>
        /// Enum SGD for value: SGD
        /// </summary>
        SGD = 126,

        /// <summary>
        /// Enum SHP for value: SHP
        /// </summary>
        SHP = 127,

        /// <summary>
        /// Enum SLL for value: SLL
        /// </summary>
        SLL = 128,

        /// <summary>
        /// Enum SOS for value: SOS
        /// </summary>
        SOS = 129,

        /// <summary>
        /// Enum SPL for value: SPL
        /// </summary>
        SPL = 130,

        /// <summary>
        /// Enum SRD for value: SRD
        /// </summary>
        SRD = 131,

        /// <summary>
        /// Enum STD for value: STD
        /// </summary>
        STD = 132,

        /// <summary>
        /// Enum SVC for value: SVC
        /// </summary>
        SVC = 133,

        /// <summary>
        /// Enum SYP for value: SYP
        /// </summary>
        SYP = 134,

        /// <summary>
        /// Enum SZL for value: SZL
        /// </summary>
        SZL = 135,

        /// <summary>
        /// Enum THB for value: THB
        /// </summary>
        THB = 136,

        /// <summary>
        /// Enum TJS for value: TJS
        /// </summary>
        TJS = 137,

        /// <summary>
        /// Enum TMT for value: TMT
        /// </summary>
        TMT = 138,

        /// <summary>
        /// Enum TND for value: TND
        /// </summary>
        TND = 139,

        /// <summary>
        /// Enum TOP for value: TOP
        /// </summary>
        TOP = 140,

        /// <summary>
        /// Enum TRY for value: TRY
        /// </summary>
        TRY = 141,

        /// <summary>
        /// Enum TTD for value: TTD
        /// </summary>
        TTD = 142,

        /// <summary>
        /// Enum TVD for value: TVD
        /// </summary>
        TVD = 143,

        /// <summary>
        /// Enum TWD for value: TWD
        /// </summary>
        TWD = 144,

        /// <summary>
        /// Enum TZS for value: TZS
        /// </summary>
        TZS = 145,

        /// <summary>
        /// Enum UAH for value: UAH
        /// </summary>
        UAH = 146,

        /// <summary>
        /// Enum UGX for value: UGX
        /// </summary>
        UGX = 147,

        /// <summary>
        /// Enum USD for value: USD
        /// </summary>
        USD = 148,

        /// <summary>
        /// Enum UYU for value: UYU
        /// </summary>
        UYU = 149,

        /// <summary>
        /// Enum UZS for value: UZS
        /// </summary>
        UZS = 150,

        /// <summary>
        /// Enum VEF for value: VEF
        /// </summary>
        VEF = 151,

        /// <summary>
        /// Enum VND for value: VND
        /// </summary>
        VND = 152,

        /// <summary>
        /// Enum VUV for value: VUV
        /// </summary>
        VUV = 153,

        /// <summary>
        /// Enum WST for value: WST
        /// </summary>
        WST = 154,

        /// <summary>
        /// Enum XAF for value: XAF
        /// </summary>
        XAF = 155,

        /// <summary>
        /// Enum XCD for value: XCD
        /// </summary>
        XCD = 156,

        /// <summary>
        /// Enum XDR for value: XDR
        /// </summary>
        XDR = 157,

        /// <summary>
        /// Enum XOF for value: XOF
        /// </summary>
        XOF = 158,

        /// <summary>
        /// Enum XPF for value: XPF
        /// </summary>
        XPF = 159,

        /// <summary>
        /// Enum YER for value: YER
        /// </summary>
        YER = 160,

        /// <summary>
        /// Enum ZAR for value: ZAR
        /// </summary>
        ZAR = 161,

        /// <summary>
        /// Enum ZMW for value: ZMW
        /// </summary>
        ZMW = 162,

        /// <summary>
        /// Enum ZWD for value: ZWD
        /// </summary>
        ZWD = 163
    }

    /// <summary>
    /// Converts <see cref="NonNullableCatalogsCurrency"/> to and from the JSON value
    /// </summary>
    public static class NonNullableCatalogsCurrencyValueConverter
    {
        /// <summary>
        /// Parses a given value to <see cref="NonNullableCatalogsCurrency"/>
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public static NonNullableCatalogsCurrency FromString(string value)
        {
            if (value.Equals("AED"))
                return NonNullableCatalogsCurrency.AED;

            if (value.Equals("AFN"))
                return NonNullableCatalogsCurrency.AFN;

            if (value.Equals("ALL"))
                return NonNullableCatalogsCurrency.ALL;

            if (value.Equals("AMD"))
                return NonNullableCatalogsCurrency.AMD;

            if (value.Equals("ANG"))
                return NonNullableCatalogsCurrency.ANG;

            if (value.Equals("AOA"))
                return NonNullableCatalogsCurrency.AOA;

            if (value.Equals("ARS"))
                return NonNullableCatalogsCurrency.ARS;

            if (value.Equals("AUD"))
                return NonNullableCatalogsCurrency.AUD;

            if (value.Equals("AWG"))
                return NonNullableCatalogsCurrency.AWG;

            if (value.Equals("AZN"))
                return NonNullableCatalogsCurrency.AZN;

            if (value.Equals("BAM"))
                return NonNullableCatalogsCurrency.BAM;

            if (value.Equals("BBD"))
                return NonNullableCatalogsCurrency.BBD;

            if (value.Equals("BDT"))
                return NonNullableCatalogsCurrency.BDT;

            if (value.Equals("BGN"))
                return NonNullableCatalogsCurrency.BGN;

            if (value.Equals("BHD"))
                return NonNullableCatalogsCurrency.BHD;

            if (value.Equals("BIF"))
                return NonNullableCatalogsCurrency.BIF;

            if (value.Equals("BMD"))
                return NonNullableCatalogsCurrency.BMD;

            if (value.Equals("BND"))
                return NonNullableCatalogsCurrency.BND;

            if (value.Equals("BOB"))
                return NonNullableCatalogsCurrency.BOB;

            if (value.Equals("BRL"))
                return NonNullableCatalogsCurrency.BRL;

            if (value.Equals("BSD"))
                return NonNullableCatalogsCurrency.BSD;

            if (value.Equals("BTN"))
                return NonNullableCatalogsCurrency.BTN;

            if (value.Equals("BWP"))
                return NonNullableCatalogsCurrency.BWP;

            if (value.Equals("BYN"))
                return NonNullableCatalogsCurrency.BYN;

            if (value.Equals("BYR"))
                return NonNullableCatalogsCurrency.BYR;

            if (value.Equals("BZD"))
                return NonNullableCatalogsCurrency.BZD;

            if (value.Equals("CAD"))
                return NonNullableCatalogsCurrency.CAD;

            if (value.Equals("CDF"))
                return NonNullableCatalogsCurrency.CDF;

            if (value.Equals("CHF"))
                return NonNullableCatalogsCurrency.CHF;

            if (value.Equals("CLP"))
                return NonNullableCatalogsCurrency.CLP;

            if (value.Equals("CNY"))
                return NonNullableCatalogsCurrency.CNY;

            if (value.Equals("COP"))
                return NonNullableCatalogsCurrency.COP;

            if (value.Equals("CRC"))
                return NonNullableCatalogsCurrency.CRC;

            if (value.Equals("CUC"))
                return NonNullableCatalogsCurrency.CUC;

            if (value.Equals("CUP"))
                return NonNullableCatalogsCurrency.CUP;

            if (value.Equals("CVE"))
                return NonNullableCatalogsCurrency.CVE;

            if (value.Equals("CZK"))
                return NonNullableCatalogsCurrency.CZK;

            if (value.Equals("DJF"))
                return NonNullableCatalogsCurrency.DJF;

            if (value.Equals("DKK"))
                return NonNullableCatalogsCurrency.DKK;

            if (value.Equals("DOP"))
                return NonNullableCatalogsCurrency.DOP;

            if (value.Equals("DZD"))
                return NonNullableCatalogsCurrency.DZD;

            if (value.Equals("EGP"))
                return NonNullableCatalogsCurrency.EGP;

            if (value.Equals("ERN"))
                return NonNullableCatalogsCurrency.ERN;

            if (value.Equals("ETB"))
                return NonNullableCatalogsCurrency.ETB;

            if (value.Equals("EUR"))
                return NonNullableCatalogsCurrency.EUR;

            if (value.Equals("FJD"))
                return NonNullableCatalogsCurrency.FJD;

            if (value.Equals("FKP"))
                return NonNullableCatalogsCurrency.FKP;

            if (value.Equals("GBP"))
                return NonNullableCatalogsCurrency.GBP;

            if (value.Equals("GEL"))
                return NonNullableCatalogsCurrency.GEL;

            if (value.Equals("GGP"))
                return NonNullableCatalogsCurrency.GGP;

            if (value.Equals("GHS"))
                return NonNullableCatalogsCurrency.GHS;

            if (value.Equals("GIP"))
                return NonNullableCatalogsCurrency.GIP;

            if (value.Equals("GMD"))
                return NonNullableCatalogsCurrency.GMD;

            if (value.Equals("GNF"))
                return NonNullableCatalogsCurrency.GNF;

            if (value.Equals("GTQ"))
                return NonNullableCatalogsCurrency.GTQ;

            if (value.Equals("GYD"))
                return NonNullableCatalogsCurrency.GYD;

            if (value.Equals("HKD"))
                return NonNullableCatalogsCurrency.HKD;

            if (value.Equals("HNL"))
                return NonNullableCatalogsCurrency.HNL;

            if (value.Equals("HRK"))
                return NonNullableCatalogsCurrency.HRK;

            if (value.Equals("HTG"))
                return NonNullableCatalogsCurrency.HTG;

            if (value.Equals("HUF"))
                return NonNullableCatalogsCurrency.HUF;

            if (value.Equals("IDR"))
                return NonNullableCatalogsCurrency.IDR;

            if (value.Equals("ILS"))
                return NonNullableCatalogsCurrency.ILS;

            if (value.Equals("IMP"))
                return NonNullableCatalogsCurrency.IMP;

            if (value.Equals("INR"))
                return NonNullableCatalogsCurrency.INR;

            if (value.Equals("IQD"))
                return NonNullableCatalogsCurrency.IQD;

            if (value.Equals("IRR"))
                return NonNullableCatalogsCurrency.IRR;

            if (value.Equals("ISK"))
                return NonNullableCatalogsCurrency.ISK;

            if (value.Equals("JEP"))
                return NonNullableCatalogsCurrency.JEP;

            if (value.Equals("JMD"))
                return NonNullableCatalogsCurrency.JMD;

            if (value.Equals("JOD"))
                return NonNullableCatalogsCurrency.JOD;

            if (value.Equals("JPY"))
                return NonNullableCatalogsCurrency.JPY;

            if (value.Equals("KES"))
                return NonNullableCatalogsCurrency.KES;

            if (value.Equals("KGS"))
                return NonNullableCatalogsCurrency.KGS;

            if (value.Equals("KHR"))
                return NonNullableCatalogsCurrency.KHR;

            if (value.Equals("KMF"))
                return NonNullableCatalogsCurrency.KMF;

            if (value.Equals("KPW"))
                return NonNullableCatalogsCurrency.KPW;

            if (value.Equals("KRW"))
                return NonNullableCatalogsCurrency.KRW;

            if (value.Equals("KWD"))
                return NonNullableCatalogsCurrency.KWD;

            if (value.Equals("KYD"))
                return NonNullableCatalogsCurrency.KYD;

            if (value.Equals("KZT"))
                return NonNullableCatalogsCurrency.KZT;

            if (value.Equals("LAK"))
                return NonNullableCatalogsCurrency.LAK;

            if (value.Equals("LBP"))
                return NonNullableCatalogsCurrency.LBP;

            if (value.Equals("LKR"))
                return NonNullableCatalogsCurrency.LKR;

            if (value.Equals("LRD"))
                return NonNullableCatalogsCurrency.LRD;

            if (value.Equals("LSL"))
                return NonNullableCatalogsCurrency.LSL;

            if (value.Equals("LYD"))
                return NonNullableCatalogsCurrency.LYD;

            if (value.Equals("MAD"))
                return NonNullableCatalogsCurrency.MAD;

            if (value.Equals("MDL"))
                return NonNullableCatalogsCurrency.MDL;

            if (value.Equals("MGA"))
                return NonNullableCatalogsCurrency.MGA;

            if (value.Equals("MKD"))
                return NonNullableCatalogsCurrency.MKD;

            if (value.Equals("MMK"))
                return NonNullableCatalogsCurrency.MMK;

            if (value.Equals("MNT"))
                return NonNullableCatalogsCurrency.MNT;

            if (value.Equals("MOP"))
                return NonNullableCatalogsCurrency.MOP;

            if (value.Equals("MRO"))
                return NonNullableCatalogsCurrency.MRO;

            if (value.Equals("MUR"))
                return NonNullableCatalogsCurrency.MUR;

            if (value.Equals("MVR"))
                return NonNullableCatalogsCurrency.MVR;

            if (value.Equals("MWK"))
                return NonNullableCatalogsCurrency.MWK;

            if (value.Equals("MXN"))
                return NonNullableCatalogsCurrency.MXN;

            if (value.Equals("MYR"))
                return NonNullableCatalogsCurrency.MYR;

            if (value.Equals("MZN"))
                return NonNullableCatalogsCurrency.MZN;

            if (value.Equals("NAD"))
                return NonNullableCatalogsCurrency.NAD;

            if (value.Equals("NGN"))
                return NonNullableCatalogsCurrency.NGN;

            if (value.Equals("NIO"))
                return NonNullableCatalogsCurrency.NIO;

            if (value.Equals("NOK"))
                return NonNullableCatalogsCurrency.NOK;

            if (value.Equals("NPR"))
                return NonNullableCatalogsCurrency.NPR;

            if (value.Equals("NZD"))
                return NonNullableCatalogsCurrency.NZD;

            if (value.Equals("OMR"))
                return NonNullableCatalogsCurrency.OMR;

            if (value.Equals("PAB"))
                return NonNullableCatalogsCurrency.PAB;

            if (value.Equals("PEN"))
                return NonNullableCatalogsCurrency.PEN;

            if (value.Equals("PGK"))
                return NonNullableCatalogsCurrency.PGK;

            if (value.Equals("PHP"))
                return NonNullableCatalogsCurrency.PHP;

            if (value.Equals("PKR"))
                return NonNullableCatalogsCurrency.PKR;

            if (value.Equals("PLN"))
                return NonNullableCatalogsCurrency.PLN;

            if (value.Equals("PYG"))
                return NonNullableCatalogsCurrency.PYG;

            if (value.Equals("QAR"))
                return NonNullableCatalogsCurrency.QAR;

            if (value.Equals("RON"))
                return NonNullableCatalogsCurrency.RON;

            if (value.Equals("RSD"))
                return NonNullableCatalogsCurrency.RSD;

            if (value.Equals("RUB"))
                return NonNullableCatalogsCurrency.RUB;

            if (value.Equals("RWF"))
                return NonNullableCatalogsCurrency.RWF;

            if (value.Equals("SAR"))
                return NonNullableCatalogsCurrency.SAR;

            if (value.Equals("SBD"))
                return NonNullableCatalogsCurrency.SBD;

            if (value.Equals("SCR"))
                return NonNullableCatalogsCurrency.SCR;

            if (value.Equals("SDG"))
                return NonNullableCatalogsCurrency.SDG;

            if (value.Equals("SEK"))
                return NonNullableCatalogsCurrency.SEK;

            if (value.Equals("SGD"))
                return NonNullableCatalogsCurrency.SGD;

            if (value.Equals("SHP"))
                return NonNullableCatalogsCurrency.SHP;

            if (value.Equals("SLL"))
                return NonNullableCatalogsCurrency.SLL;

            if (value.Equals("SOS"))
                return NonNullableCatalogsCurrency.SOS;

            if (value.Equals("SPL"))
                return NonNullableCatalogsCurrency.SPL;

            if (value.Equals("SRD"))
                return NonNullableCatalogsCurrency.SRD;

            if (value.Equals("STD"))
                return NonNullableCatalogsCurrency.STD;

            if (value.Equals("SVC"))
                return NonNullableCatalogsCurrency.SVC;

            if (value.Equals("SYP"))
                return NonNullableCatalogsCurrency.SYP;

            if (value.Equals("SZL"))
                return NonNullableCatalogsCurrency.SZL;

            if (value.Equals("THB"))
                return NonNullableCatalogsCurrency.THB;

            if (value.Equals("TJS"))
                return NonNullableCatalogsCurrency.TJS;

            if (value.Equals("TMT"))
                return NonNullableCatalogsCurrency.TMT;

            if (value.Equals("TND"))
                return NonNullableCatalogsCurrency.TND;

            if (value.Equals("TOP"))
                return NonNullableCatalogsCurrency.TOP;

            if (value.Equals("TRY"))
                return NonNullableCatalogsCurrency.TRY;

            if (value.Equals("TTD"))
                return NonNullableCatalogsCurrency.TTD;

            if (value.Equals("TVD"))
                return NonNullableCatalogsCurrency.TVD;

            if (value.Equals("TWD"))
                return NonNullableCatalogsCurrency.TWD;

            if (value.Equals("TZS"))
                return NonNullableCatalogsCurrency.TZS;

            if (value.Equals("UAH"))
                return NonNullableCatalogsCurrency.UAH;

            if (value.Equals("UGX"))
                return NonNullableCatalogsCurrency.UGX;

            if (value.Equals("USD"))
                return NonNullableCatalogsCurrency.USD;

            if (value.Equals("UYU"))
                return NonNullableCatalogsCurrency.UYU;

            if (value.Equals("UZS"))
                return NonNullableCatalogsCurrency.UZS;

            if (value.Equals("VEF"))
                return NonNullableCatalogsCurrency.VEF;

            if (value.Equals("VND"))
                return NonNullableCatalogsCurrency.VND;

            if (value.Equals("VUV"))
                return NonNullableCatalogsCurrency.VUV;

            if (value.Equals("WST"))
                return NonNullableCatalogsCurrency.WST;

            if (value.Equals("XAF"))
                return NonNullableCatalogsCurrency.XAF;

            if (value.Equals("XCD"))
                return NonNullableCatalogsCurrency.XCD;

            if (value.Equals("XDR"))
                return NonNullableCatalogsCurrency.XDR;

            if (value.Equals("XOF"))
                return NonNullableCatalogsCurrency.XOF;

            if (value.Equals("XPF"))
                return NonNullableCatalogsCurrency.XPF;

            if (value.Equals("YER"))
                return NonNullableCatalogsCurrency.YER;

            if (value.Equals("ZAR"))
                return NonNullableCatalogsCurrency.ZAR;

            if (value.Equals("ZMW"))
                return NonNullableCatalogsCurrency.ZMW;

            if (value.Equals("ZWD"))
                return NonNullableCatalogsCurrency.ZWD;

            throw new NotImplementedException($"Could not convert value to type NonNullableCatalogsCurrency: '{value}'");
        }

        /// <summary>
        /// Parses a given value to <see cref="NonNullableCatalogsCurrency"/>
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public static NonNullableCatalogsCurrency? FromStringOrDefault(string value)
        {
            if (value.Equals("AED"))
                return NonNullableCatalogsCurrency.AED;

            if (value.Equals("AFN"))
                return NonNullableCatalogsCurrency.AFN;

            if (value.Equals("ALL"))
                return NonNullableCatalogsCurrency.ALL;

            if (value.Equals("AMD"))
                return NonNullableCatalogsCurrency.AMD;

            if (value.Equals("ANG"))
                return NonNullableCatalogsCurrency.ANG;

            if (value.Equals("AOA"))
                return NonNullableCatalogsCurrency.AOA;

            if (value.Equals("ARS"))
                return NonNullableCatalogsCurrency.ARS;

            if (value.Equals("AUD"))
                return NonNullableCatalogsCurrency.AUD;

            if (value.Equals("AWG"))
                return NonNullableCatalogsCurrency.AWG;

            if (value.Equals("AZN"))
                return NonNullableCatalogsCurrency.AZN;

            if (value.Equals("BAM"))
                return NonNullableCatalogsCurrency.BAM;

            if (value.Equals("BBD"))
                return NonNullableCatalogsCurrency.BBD;

            if (value.Equals("BDT"))
                return NonNullableCatalogsCurrency.BDT;

            if (value.Equals("BGN"))
                return NonNullableCatalogsCurrency.BGN;

            if (value.Equals("BHD"))
                return NonNullableCatalogsCurrency.BHD;

            if (value.Equals("BIF"))
                return NonNullableCatalogsCurrency.BIF;

            if (value.Equals("BMD"))
                return NonNullableCatalogsCurrency.BMD;

            if (value.Equals("BND"))
                return NonNullableCatalogsCurrency.BND;

            if (value.Equals("BOB"))
                return NonNullableCatalogsCurrency.BOB;

            if (value.Equals("BRL"))
                return NonNullableCatalogsCurrency.BRL;

            if (value.Equals("BSD"))
                return NonNullableCatalogsCurrency.BSD;

            if (value.Equals("BTN"))
                return NonNullableCatalogsCurrency.BTN;

            if (value.Equals("BWP"))
                return NonNullableCatalogsCurrency.BWP;

            if (value.Equals("BYN"))
                return NonNullableCatalogsCurrency.BYN;

            if (value.Equals("BYR"))
                return NonNullableCatalogsCurrency.BYR;

            if (value.Equals("BZD"))
                return NonNullableCatalogsCurrency.BZD;

            if (value.Equals("CAD"))
                return NonNullableCatalogsCurrency.CAD;

            if (value.Equals("CDF"))
                return NonNullableCatalogsCurrency.CDF;

            if (value.Equals("CHF"))
                return NonNullableCatalogsCurrency.CHF;

            if (value.Equals("CLP"))
                return NonNullableCatalogsCurrency.CLP;

            if (value.Equals("CNY"))
                return NonNullableCatalogsCurrency.CNY;

            if (value.Equals("COP"))
                return NonNullableCatalogsCurrency.COP;

            if (value.Equals("CRC"))
                return NonNullableCatalogsCurrency.CRC;

            if (value.Equals("CUC"))
                return NonNullableCatalogsCurrency.CUC;

            if (value.Equals("CUP"))
                return NonNullableCatalogsCurrency.CUP;

            if (value.Equals("CVE"))
                return NonNullableCatalogsCurrency.CVE;

            if (value.Equals("CZK"))
                return NonNullableCatalogsCurrency.CZK;

            if (value.Equals("DJF"))
                return NonNullableCatalogsCurrency.DJF;

            if (value.Equals("DKK"))
                return NonNullableCatalogsCurrency.DKK;

            if (value.Equals("DOP"))
                return NonNullableCatalogsCurrency.DOP;

            if (value.Equals("DZD"))
                return NonNullableCatalogsCurrency.DZD;

            if (value.Equals("EGP"))
                return NonNullableCatalogsCurrency.EGP;

            if (value.Equals("ERN"))
                return NonNullableCatalogsCurrency.ERN;

            if (value.Equals("ETB"))
                return NonNullableCatalogsCurrency.ETB;

            if (value.Equals("EUR"))
                return NonNullableCatalogsCurrency.EUR;

            if (value.Equals("FJD"))
                return NonNullableCatalogsCurrency.FJD;

            if (value.Equals("FKP"))
                return NonNullableCatalogsCurrency.FKP;

            if (value.Equals("GBP"))
                return NonNullableCatalogsCurrency.GBP;

            if (value.Equals("GEL"))
                return NonNullableCatalogsCurrency.GEL;

            if (value.Equals("GGP"))
                return NonNullableCatalogsCurrency.GGP;

            if (value.Equals("GHS"))
                return NonNullableCatalogsCurrency.GHS;

            if (value.Equals("GIP"))
                return NonNullableCatalogsCurrency.GIP;

            if (value.Equals("GMD"))
                return NonNullableCatalogsCurrency.GMD;

            if (value.Equals("GNF"))
                return NonNullableCatalogsCurrency.GNF;

            if (value.Equals("GTQ"))
                return NonNullableCatalogsCurrency.GTQ;

            if (value.Equals("GYD"))
                return NonNullableCatalogsCurrency.GYD;

            if (value.Equals("HKD"))
                return NonNullableCatalogsCurrency.HKD;

            if (value.Equals("HNL"))
                return NonNullableCatalogsCurrency.HNL;

            if (value.Equals("HRK"))
                return NonNullableCatalogsCurrency.HRK;

            if (value.Equals("HTG"))
                return NonNullableCatalogsCurrency.HTG;

            if (value.Equals("HUF"))
                return NonNullableCatalogsCurrency.HUF;

            if (value.Equals("IDR"))
                return NonNullableCatalogsCurrency.IDR;

            if (value.Equals("ILS"))
                return NonNullableCatalogsCurrency.ILS;

            if (value.Equals("IMP"))
                return NonNullableCatalogsCurrency.IMP;

            if (value.Equals("INR"))
                return NonNullableCatalogsCurrency.INR;

            if (value.Equals("IQD"))
                return NonNullableCatalogsCurrency.IQD;

            if (value.Equals("IRR"))
                return NonNullableCatalogsCurrency.IRR;

            if (value.Equals("ISK"))
                return NonNullableCatalogsCurrency.ISK;

            if (value.Equals("JEP"))
                return NonNullableCatalogsCurrency.JEP;

            if (value.Equals("JMD"))
                return NonNullableCatalogsCurrency.JMD;

            if (value.Equals("JOD"))
                return NonNullableCatalogsCurrency.JOD;

            if (value.Equals("JPY"))
                return NonNullableCatalogsCurrency.JPY;

            if (value.Equals("KES"))
                return NonNullableCatalogsCurrency.KES;

            if (value.Equals("KGS"))
                return NonNullableCatalogsCurrency.KGS;

            if (value.Equals("KHR"))
                return NonNullableCatalogsCurrency.KHR;

            if (value.Equals("KMF"))
                return NonNullableCatalogsCurrency.KMF;

            if (value.Equals("KPW"))
                return NonNullableCatalogsCurrency.KPW;

            if (value.Equals("KRW"))
                return NonNullableCatalogsCurrency.KRW;

            if (value.Equals("KWD"))
                return NonNullableCatalogsCurrency.KWD;

            if (value.Equals("KYD"))
                return NonNullableCatalogsCurrency.KYD;

            if (value.Equals("KZT"))
                return NonNullableCatalogsCurrency.KZT;

            if (value.Equals("LAK"))
                return NonNullableCatalogsCurrency.LAK;

            if (value.Equals("LBP"))
                return NonNullableCatalogsCurrency.LBP;

            if (value.Equals("LKR"))
                return NonNullableCatalogsCurrency.LKR;

            if (value.Equals("LRD"))
                return NonNullableCatalogsCurrency.LRD;

            if (value.Equals("LSL"))
                return NonNullableCatalogsCurrency.LSL;

            if (value.Equals("LYD"))
                return NonNullableCatalogsCurrency.LYD;

            if (value.Equals("MAD"))
                return NonNullableCatalogsCurrency.MAD;

            if (value.Equals("MDL"))
                return NonNullableCatalogsCurrency.MDL;

            if (value.Equals("MGA"))
                return NonNullableCatalogsCurrency.MGA;

            if (value.Equals("MKD"))
                return NonNullableCatalogsCurrency.MKD;

            if (value.Equals("MMK"))
                return NonNullableCatalogsCurrency.MMK;

            if (value.Equals("MNT"))
                return NonNullableCatalogsCurrency.MNT;

            if (value.Equals("MOP"))
                return NonNullableCatalogsCurrency.MOP;

            if (value.Equals("MRO"))
                return NonNullableCatalogsCurrency.MRO;

            if (value.Equals("MUR"))
                return NonNullableCatalogsCurrency.MUR;

            if (value.Equals("MVR"))
                return NonNullableCatalogsCurrency.MVR;

            if (value.Equals("MWK"))
                return NonNullableCatalogsCurrency.MWK;

            if (value.Equals("MXN"))
                return NonNullableCatalogsCurrency.MXN;

            if (value.Equals("MYR"))
                return NonNullableCatalogsCurrency.MYR;

            if (value.Equals("MZN"))
                return NonNullableCatalogsCurrency.MZN;

            if (value.Equals("NAD"))
                return NonNullableCatalogsCurrency.NAD;

            if (value.Equals("NGN"))
                return NonNullableCatalogsCurrency.NGN;

            if (value.Equals("NIO"))
                return NonNullableCatalogsCurrency.NIO;

            if (value.Equals("NOK"))
                return NonNullableCatalogsCurrency.NOK;

            if (value.Equals("NPR"))
                return NonNullableCatalogsCurrency.NPR;

            if (value.Equals("NZD"))
                return NonNullableCatalogsCurrency.NZD;

            if (value.Equals("OMR"))
                return NonNullableCatalogsCurrency.OMR;

            if (value.Equals("PAB"))
                return NonNullableCatalogsCurrency.PAB;

            if (value.Equals("PEN"))
                return NonNullableCatalogsCurrency.PEN;

            if (value.Equals("PGK"))
                return NonNullableCatalogsCurrency.PGK;

            if (value.Equals("PHP"))
                return NonNullableCatalogsCurrency.PHP;

            if (value.Equals("PKR"))
                return NonNullableCatalogsCurrency.PKR;

            if (value.Equals("PLN"))
                return NonNullableCatalogsCurrency.PLN;

            if (value.Equals("PYG"))
                return NonNullableCatalogsCurrency.PYG;

            if (value.Equals("QAR"))
                return NonNullableCatalogsCurrency.QAR;

            if (value.Equals("RON"))
                return NonNullableCatalogsCurrency.RON;

            if (value.Equals("RSD"))
                return NonNullableCatalogsCurrency.RSD;

            if (value.Equals("RUB"))
                return NonNullableCatalogsCurrency.RUB;

            if (value.Equals("RWF"))
                return NonNullableCatalogsCurrency.RWF;

            if (value.Equals("SAR"))
                return NonNullableCatalogsCurrency.SAR;

            if (value.Equals("SBD"))
                return NonNullableCatalogsCurrency.SBD;

            if (value.Equals("SCR"))
                return NonNullableCatalogsCurrency.SCR;

            if (value.Equals("SDG"))
                return NonNullableCatalogsCurrency.SDG;

            if (value.Equals("SEK"))
                return NonNullableCatalogsCurrency.SEK;

            if (value.Equals("SGD"))
                return NonNullableCatalogsCurrency.SGD;

            if (value.Equals("SHP"))
                return NonNullableCatalogsCurrency.SHP;

            if (value.Equals("SLL"))
                return NonNullableCatalogsCurrency.SLL;

            if (value.Equals("SOS"))
                return NonNullableCatalogsCurrency.SOS;

            if (value.Equals("SPL"))
                return NonNullableCatalogsCurrency.SPL;

            if (value.Equals("SRD"))
                return NonNullableCatalogsCurrency.SRD;

            if (value.Equals("STD"))
                return NonNullableCatalogsCurrency.STD;

            if (value.Equals("SVC"))
                return NonNullableCatalogsCurrency.SVC;

            if (value.Equals("SYP"))
                return NonNullableCatalogsCurrency.SYP;

            if (value.Equals("SZL"))
                return NonNullableCatalogsCurrency.SZL;

            if (value.Equals("THB"))
                return NonNullableCatalogsCurrency.THB;

            if (value.Equals("TJS"))
                return NonNullableCatalogsCurrency.TJS;

            if (value.Equals("TMT"))
                return NonNullableCatalogsCurrency.TMT;

            if (value.Equals("TND"))
                return NonNullableCatalogsCurrency.TND;

            if (value.Equals("TOP"))
                return NonNullableCatalogsCurrency.TOP;

            if (value.Equals("TRY"))
                return NonNullableCatalogsCurrency.TRY;

            if (value.Equals("TTD"))
                return NonNullableCatalogsCurrency.TTD;

            if (value.Equals("TVD"))
                return NonNullableCatalogsCurrency.TVD;

            if (value.Equals("TWD"))
                return NonNullableCatalogsCurrency.TWD;

            if (value.Equals("TZS"))
                return NonNullableCatalogsCurrency.TZS;

            if (value.Equals("UAH"))
                return NonNullableCatalogsCurrency.UAH;

            if (value.Equals("UGX"))
                return NonNullableCatalogsCurrency.UGX;

            if (value.Equals("USD"))
                return NonNullableCatalogsCurrency.USD;

            if (value.Equals("UYU"))
                return NonNullableCatalogsCurrency.UYU;

            if (value.Equals("UZS"))
                return NonNullableCatalogsCurrency.UZS;

            if (value.Equals("VEF"))
                return NonNullableCatalogsCurrency.VEF;

            if (value.Equals("VND"))
                return NonNullableCatalogsCurrency.VND;

            if (value.Equals("VUV"))
                return NonNullableCatalogsCurrency.VUV;

            if (value.Equals("WST"))
                return NonNullableCatalogsCurrency.WST;

            if (value.Equals("XAF"))
                return NonNullableCatalogsCurrency.XAF;

            if (value.Equals("XCD"))
                return NonNullableCatalogsCurrency.XCD;

            if (value.Equals("XDR"))
                return NonNullableCatalogsCurrency.XDR;

            if (value.Equals("XOF"))
                return NonNullableCatalogsCurrency.XOF;

            if (value.Equals("XPF"))
                return NonNullableCatalogsCurrency.XPF;

            if (value.Equals("YER"))
                return NonNullableCatalogsCurrency.YER;

            if (value.Equals("ZAR"))
                return NonNullableCatalogsCurrency.ZAR;

            if (value.Equals("ZMW"))
                return NonNullableCatalogsCurrency.ZMW;

            if (value.Equals("ZWD"))
                return NonNullableCatalogsCurrency.ZWD;

            return null;
        }

        /// <summary>
        /// Converts the <see cref="NonNullableCatalogsCurrency"/> to the json value
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public static string ToJsonValue(NonNullableCatalogsCurrency value)
        {
            if (value == NonNullableCatalogsCurrency.AED)
                return "AED";

            if (value == NonNullableCatalogsCurrency.AFN)
                return "AFN";

            if (value == NonNullableCatalogsCurrency.ALL)
                return "ALL";

            if (value == NonNullableCatalogsCurrency.AMD)
                return "AMD";

            if (value == NonNullableCatalogsCurrency.ANG)
                return "ANG";

            if (value == NonNullableCatalogsCurrency.AOA)
                return "AOA";

            if (value == NonNullableCatalogsCurrency.ARS)
                return "ARS";

            if (value == NonNullableCatalogsCurrency.AUD)
                return "AUD";

            if (value == NonNullableCatalogsCurrency.AWG)
                return "AWG";

            if (value == NonNullableCatalogsCurrency.AZN)
                return "AZN";

            if (value == NonNullableCatalogsCurrency.BAM)
                return "BAM";

            if (value == NonNullableCatalogsCurrency.BBD)
                return "BBD";

            if (value == NonNullableCatalogsCurrency.BDT)
                return "BDT";

            if (value == NonNullableCatalogsCurrency.BGN)
                return "BGN";

            if (value == NonNullableCatalogsCurrency.BHD)
                return "BHD";

            if (value == NonNullableCatalogsCurrency.BIF)
                return "BIF";

            if (value == NonNullableCatalogsCurrency.BMD)
                return "BMD";

            if (value == NonNullableCatalogsCurrency.BND)
                return "BND";

            if (value == NonNullableCatalogsCurrency.BOB)
                return "BOB";

            if (value == NonNullableCatalogsCurrency.BRL)
                return "BRL";

            if (value == NonNullableCatalogsCurrency.BSD)
                return "BSD";

            if (value == NonNullableCatalogsCurrency.BTN)
                return "BTN";

            if (value == NonNullableCatalogsCurrency.BWP)
                return "BWP";

            if (value == NonNullableCatalogsCurrency.BYN)
                return "BYN";

            if (value == NonNullableCatalogsCurrency.BYR)
                return "BYR";

            if (value == NonNullableCatalogsCurrency.BZD)
                return "BZD";

            if (value == NonNullableCatalogsCurrency.CAD)
                return "CAD";

            if (value == NonNullableCatalogsCurrency.CDF)
                return "CDF";

            if (value == NonNullableCatalogsCurrency.CHF)
                return "CHF";

            if (value == NonNullableCatalogsCurrency.CLP)
                return "CLP";

            if (value == NonNullableCatalogsCurrency.CNY)
                return "CNY";

            if (value == NonNullableCatalogsCurrency.COP)
                return "COP";

            if (value == NonNullableCatalogsCurrency.CRC)
                return "CRC";

            if (value == NonNullableCatalogsCurrency.CUC)
                return "CUC";

            if (value == NonNullableCatalogsCurrency.CUP)
                return "CUP";

            if (value == NonNullableCatalogsCurrency.CVE)
                return "CVE";

            if (value == NonNullableCatalogsCurrency.CZK)
                return "CZK";

            if (value == NonNullableCatalogsCurrency.DJF)
                return "DJF";

            if (value == NonNullableCatalogsCurrency.DKK)
                return "DKK";

            if (value == NonNullableCatalogsCurrency.DOP)
                return "DOP";

            if (value == NonNullableCatalogsCurrency.DZD)
                return "DZD";

            if (value == NonNullableCatalogsCurrency.EGP)
                return "EGP";

            if (value == NonNullableCatalogsCurrency.ERN)
                return "ERN";

            if (value == NonNullableCatalogsCurrency.ETB)
                return "ETB";

            if (value == NonNullableCatalogsCurrency.EUR)
                return "EUR";

            if (value == NonNullableCatalogsCurrency.FJD)
                return "FJD";

            if (value == NonNullableCatalogsCurrency.FKP)
                return "FKP";

            if (value == NonNullableCatalogsCurrency.GBP)
                return "GBP";

            if (value == NonNullableCatalogsCurrency.GEL)
                return "GEL";

            if (value == NonNullableCatalogsCurrency.GGP)
                return "GGP";

            if (value == NonNullableCatalogsCurrency.GHS)
                return "GHS";

            if (value == NonNullableCatalogsCurrency.GIP)
                return "GIP";

            if (value == NonNullableCatalogsCurrency.GMD)
                return "GMD";

            if (value == NonNullableCatalogsCurrency.GNF)
                return "GNF";

            if (value == NonNullableCatalogsCurrency.GTQ)
                return "GTQ";

            if (value == NonNullableCatalogsCurrency.GYD)
                return "GYD";

            if (value == NonNullableCatalogsCurrency.HKD)
                return "HKD";

            if (value == NonNullableCatalogsCurrency.HNL)
                return "HNL";

            if (value == NonNullableCatalogsCurrency.HRK)
                return "HRK";

            if (value == NonNullableCatalogsCurrency.HTG)
                return "HTG";

            if (value == NonNullableCatalogsCurrency.HUF)
                return "HUF";

            if (value == NonNullableCatalogsCurrency.IDR)
                return "IDR";

            if (value == NonNullableCatalogsCurrency.ILS)
                return "ILS";

            if (value == NonNullableCatalogsCurrency.IMP)
                return "IMP";

            if (value == NonNullableCatalogsCurrency.INR)
                return "INR";

            if (value == NonNullableCatalogsCurrency.IQD)
                return "IQD";

            if (value == NonNullableCatalogsCurrency.IRR)
                return "IRR";

            if (value == NonNullableCatalogsCurrency.ISK)
                return "ISK";

            if (value == NonNullableCatalogsCurrency.JEP)
                return "JEP";

            if (value == NonNullableCatalogsCurrency.JMD)
                return "JMD";

            if (value == NonNullableCatalogsCurrency.JOD)
                return "JOD";

            if (value == NonNullableCatalogsCurrency.JPY)
                return "JPY";

            if (value == NonNullableCatalogsCurrency.KES)
                return "KES";

            if (value == NonNullableCatalogsCurrency.KGS)
                return "KGS";

            if (value == NonNullableCatalogsCurrency.KHR)
                return "KHR";

            if (value == NonNullableCatalogsCurrency.KMF)
                return "KMF";

            if (value == NonNullableCatalogsCurrency.KPW)
                return "KPW";

            if (value == NonNullableCatalogsCurrency.KRW)
                return "KRW";

            if (value == NonNullableCatalogsCurrency.KWD)
                return "KWD";

            if (value == NonNullableCatalogsCurrency.KYD)
                return "KYD";

            if (value == NonNullableCatalogsCurrency.KZT)
                return "KZT";

            if (value == NonNullableCatalogsCurrency.LAK)
                return "LAK";

            if (value == NonNullableCatalogsCurrency.LBP)
                return "LBP";

            if (value == NonNullableCatalogsCurrency.LKR)
                return "LKR";

            if (value == NonNullableCatalogsCurrency.LRD)
                return "LRD";

            if (value == NonNullableCatalogsCurrency.LSL)
                return "LSL";

            if (value == NonNullableCatalogsCurrency.LYD)
                return "LYD";

            if (value == NonNullableCatalogsCurrency.MAD)
                return "MAD";

            if (value == NonNullableCatalogsCurrency.MDL)
                return "MDL";

            if (value == NonNullableCatalogsCurrency.MGA)
                return "MGA";

            if (value == NonNullableCatalogsCurrency.MKD)
                return "MKD";

            if (value == NonNullableCatalogsCurrency.MMK)
                return "MMK";

            if (value == NonNullableCatalogsCurrency.MNT)
                return "MNT";

            if (value == NonNullableCatalogsCurrency.MOP)
                return "MOP";

            if (value == NonNullableCatalogsCurrency.MRO)
                return "MRO";

            if (value == NonNullableCatalogsCurrency.MUR)
                return "MUR";

            if (value == NonNullableCatalogsCurrency.MVR)
                return "MVR";

            if (value == NonNullableCatalogsCurrency.MWK)
                return "MWK";

            if (value == NonNullableCatalogsCurrency.MXN)
                return "MXN";

            if (value == NonNullableCatalogsCurrency.MYR)
                return "MYR";

            if (value == NonNullableCatalogsCurrency.MZN)
                return "MZN";

            if (value == NonNullableCatalogsCurrency.NAD)
                return "NAD";

            if (value == NonNullableCatalogsCurrency.NGN)
                return "NGN";

            if (value == NonNullableCatalogsCurrency.NIO)
                return "NIO";

            if (value == NonNullableCatalogsCurrency.NOK)
                return "NOK";

            if (value == NonNullableCatalogsCurrency.NPR)
                return "NPR";

            if (value == NonNullableCatalogsCurrency.NZD)
                return "NZD";

            if (value == NonNullableCatalogsCurrency.OMR)
                return "OMR";

            if (value == NonNullableCatalogsCurrency.PAB)
                return "PAB";

            if (value == NonNullableCatalogsCurrency.PEN)
                return "PEN";

            if (value == NonNullableCatalogsCurrency.PGK)
                return "PGK";

            if (value == NonNullableCatalogsCurrency.PHP)
                return "PHP";

            if (value == NonNullableCatalogsCurrency.PKR)
                return "PKR";

            if (value == NonNullableCatalogsCurrency.PLN)
                return "PLN";

            if (value == NonNullableCatalogsCurrency.PYG)
                return "PYG";

            if (value == NonNullableCatalogsCurrency.QAR)
                return "QAR";

            if (value == NonNullableCatalogsCurrency.RON)
                return "RON";

            if (value == NonNullableCatalogsCurrency.RSD)
                return "RSD";

            if (value == NonNullableCatalogsCurrency.RUB)
                return "RUB";

            if (value == NonNullableCatalogsCurrency.RWF)
                return "RWF";

            if (value == NonNullableCatalogsCurrency.SAR)
                return "SAR";

            if (value == NonNullableCatalogsCurrency.SBD)
                return "SBD";

            if (value == NonNullableCatalogsCurrency.SCR)
                return "SCR";

            if (value == NonNullableCatalogsCurrency.SDG)
                return "SDG";

            if (value == NonNullableCatalogsCurrency.SEK)
                return "SEK";

            if (value == NonNullableCatalogsCurrency.SGD)
                return "SGD";

            if (value == NonNullableCatalogsCurrency.SHP)
                return "SHP";

            if (value == NonNullableCatalogsCurrency.SLL)
                return "SLL";

            if (value == NonNullableCatalogsCurrency.SOS)
                return "SOS";

            if (value == NonNullableCatalogsCurrency.SPL)
                return "SPL";

            if (value == NonNullableCatalogsCurrency.SRD)
                return "SRD";

            if (value == NonNullableCatalogsCurrency.STD)
                return "STD";

            if (value == NonNullableCatalogsCurrency.SVC)
                return "SVC";

            if (value == NonNullableCatalogsCurrency.SYP)
                return "SYP";

            if (value == NonNullableCatalogsCurrency.SZL)
                return "SZL";

            if (value == NonNullableCatalogsCurrency.THB)
                return "THB";

            if (value == NonNullableCatalogsCurrency.TJS)
                return "TJS";

            if (value == NonNullableCatalogsCurrency.TMT)
                return "TMT";

            if (value == NonNullableCatalogsCurrency.TND)
                return "TND";

            if (value == NonNullableCatalogsCurrency.TOP)
                return "TOP";

            if (value == NonNullableCatalogsCurrency.TRY)
                return "TRY";

            if (value == NonNullableCatalogsCurrency.TTD)
                return "TTD";

            if (value == NonNullableCatalogsCurrency.TVD)
                return "TVD";

            if (value == NonNullableCatalogsCurrency.TWD)
                return "TWD";

            if (value == NonNullableCatalogsCurrency.TZS)
                return "TZS";

            if (value == NonNullableCatalogsCurrency.UAH)
                return "UAH";

            if (value == NonNullableCatalogsCurrency.UGX)
                return "UGX";

            if (value == NonNullableCatalogsCurrency.USD)
                return "USD";

            if (value == NonNullableCatalogsCurrency.UYU)
                return "UYU";

            if (value == NonNullableCatalogsCurrency.UZS)
                return "UZS";

            if (value == NonNullableCatalogsCurrency.VEF)
                return "VEF";

            if (value == NonNullableCatalogsCurrency.VND)
                return "VND";

            if (value == NonNullableCatalogsCurrency.VUV)
                return "VUV";

            if (value == NonNullableCatalogsCurrency.WST)
                return "WST";

            if (value == NonNullableCatalogsCurrency.XAF)
                return "XAF";

            if (value == NonNullableCatalogsCurrency.XCD)
                return "XCD";

            if (value == NonNullableCatalogsCurrency.XDR)
                return "XDR";

            if (value == NonNullableCatalogsCurrency.XOF)
                return "XOF";

            if (value == NonNullableCatalogsCurrency.XPF)
                return "XPF";

            if (value == NonNullableCatalogsCurrency.YER)
                return "YER";

            if (value == NonNullableCatalogsCurrency.ZAR)
                return "ZAR";

            if (value == NonNullableCatalogsCurrency.ZMW)
                return "ZMW";

            if (value == NonNullableCatalogsCurrency.ZWD)
                return "ZWD";

            throw new NotImplementedException($"Value could not be handled: '{value}'");
        }
    }

    /// <summary>
    /// A Json converter for type <see cref="NonNullableCatalogsCurrency"/>
    /// </summary>
    /// <exception cref="NotImplementedException"></exception>
    public class NonNullableCatalogsCurrencyJsonConverter : JsonConverter<NonNullableCatalogsCurrency>
    {
        /// <summary>
        /// Returns a  from the Json object
        /// </summary>
        /// <param name="reader"></param>
        /// <param name="typeToConvert"></param>
        /// <param name="options"></param>
        /// <returns></returns>
        public override NonNullableCatalogsCurrency Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            string? rawValue = reader.GetString();

            NonNullableCatalogsCurrency? result = rawValue == null
                ? null
                : NonNullableCatalogsCurrencyValueConverter.FromStringOrDefault(rawValue);

            if (result != null)
                return result.Value;

            throw new JsonException();
        }

        /// <summary>
        /// Writes the NonNullableCatalogsCurrency to the json writer
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="nonNullableCatalogsCurrency"></param>
        /// <param name="options"></param>
        public override void Write(Utf8JsonWriter writer, NonNullableCatalogsCurrency nonNullableCatalogsCurrency, JsonSerializerOptions options)
        {
            writer.WriteStringValue(NonNullableCatalogsCurrencyValueConverter.ToJsonValue(nonNullableCatalogsCurrency).ToString());
        }
    }

    /// <summary>
    /// A Json converter for type <see cref="NonNullableCatalogsCurrency"/>
    /// </summary>
    public class NonNullableCatalogsCurrencyNullableJsonConverter : JsonConverter<NonNullableCatalogsCurrency?>
    {
        /// <summary>
        /// Returns a NonNullableCatalogsCurrency from the Json object
        /// </summary>
        /// <param name="reader"></param>
        /// <param name="typeToConvert"></param>
        /// <param name="options"></param>
        /// <returns></returns>
        public override NonNullableCatalogsCurrency? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (reader.TokenType == JsonTokenType.Null)
                return null;

            string? rawValue = reader.GetString();

            NonNullableCatalogsCurrency? result = rawValue == null
                ? null
                : NonNullableCatalogsCurrencyValueConverter.FromStringOrDefault(rawValue);

            if (result != null)
                return result.Value;

            throw new JsonException();
        }

        /// <summary>
        /// Writes the NonNullableCatalogsCurrency to the json writer
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="nonNullableCatalogsCurrency"></param>
        /// <param name="options"></param>
        public override void Write(Utf8JsonWriter writer, NonNullableCatalogsCurrency? nonNullableCatalogsCurrency, JsonSerializerOptions options)
        {
            if (nonNullableCatalogsCurrency.HasValue)
                writer.WriteStringValue(NonNullableCatalogsCurrencyValueConverter.ToJsonValue(nonNullableCatalogsCurrency.Value).ToString());
            else
                writer.WriteNullValue();
        }
    }
}
