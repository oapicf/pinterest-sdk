import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for SSIOEditInsertionOrderRequest
void main() {
  final instance = SSIOEditInsertionOrderRequestBuilder();
  // TODO add properties to the builder and call build()

  group(SSIOEditInsertionOrderRequest, () {
    // Starting date of time period. Format: YYYY-MM-DD
    // String startDate
    test('to test the property `startDate`', () async {
      // TODO
    });

    // End date of time period. Format: YYYY-MM-DD
    // String endDate
    test('to test the property `endDate`', () async {
      // TODO
    });

    // The po number
    // String poNumber
    test('to test the property `poNumber`', () async {
      // TODO
    });

    // If Budget order line, the budget amount.
    // num budgetAmount
    test('to test the property `budgetAmount`', () async {
      // TODO
    });

    // The billing contact first name
    // String billingContactFirstname
    test('to test the property `billingContactFirstname`', () async {
      // TODO
    });

    // The billing contact last name
    // String billingContactLastname
    test('to test the property `billingContactLastname`', () async {
      // TODO
    });

    // The billing contact email
    // String billingContactEmail
    test('to test the property `billingContactEmail`', () async {
      // TODO
    });

    // The media contact first name
    // String mediaContactFirstname
    test('to test the property `mediaContactFirstname`', () async {
      // TODO
    });

    // The media contact last name
    // String mediaContactLastname
    test('to test the property `mediaContactLastname`', () async {
      // TODO
    });

    // The media contact email
    // String mediaContactEmail
    test('to test the property `mediaContactEmail`', () async {
      // TODO
    });

    // URL link for agency
    // String agencyLink
    test('to test the property `agencyLink`', () async {
      // TODO
    });

    // The email of user submitting the insertion order
    // String userEmail
    test('to test the property `userEmail`', () async {
      // TODO
    });

    // LineId in the Oracle DB
    // String oracleLineId
    test('to test the property `oracleLineId`', () async {
      // TODO
    });

    // OrderId in SFDC
    // String salesforceOrderId
    test('to test the property `salesforceOrderId`', () async {
      // TODO
    });

    // OrderLineId in SFDC
    // String salesforceOrderLineId
    test('to test the property `salesforceOrderLineId`', () async {
      // TODO
    });

    // Ads manager OrderLineId
    // String adsManagerOrderLineId
    test('to test the property `adsManagerOrderLineId`', () async {
      // TODO
    });

  });
}
