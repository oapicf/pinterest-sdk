import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for ConversionEventDeviceInfo
void main() {
  final instance = ConversionEventDeviceInfoBuilder();
  // TODO add properties to the builder and call build()

  group(ConversionEventDeviceInfo, () {
    // Battery charge level percentage
    // int batteryLevel
    test('to test the property `batteryLevel`', () async {
      // TODO
    });

    // Device brand
    // String brand
    test('to test the property `brand`', () async {
      // TODO
    });

    // User device's mobile carrier.
    // String carrier
    test('to test the property `carrier`', () async {
      // TODO
    });

    // Number of CPU cores
    // int cpuCores
    test('to test the property `cpuCores`', () async {
      // TODO
    });

    // External storage size in GB
    // int externalStorageFreeSpace
    test('to test the property `externalStorageFreeSpace`', () async {
      // TODO
    });

    // External storage size in GB
    // int externalStorageSize
    test('to test the property `externalStorageSize`', () async {
      // TODO
    });

    // Device form factor
    // String formFactor
    test('to test the property `formFactor`', () async {
      // TODO
    });

    // Kernel version. Examples: Linux: 6.15. Obtain by running: uname -r MacOS: 24.3.0. Obtain by running: sysctl kern.version Android: 6.6. Obtain from OS.uname().release
    // String kernelVersion
    test('to test the property `kernelVersion`', () async {
      // TODO
    });

    // List of user installed languages. ISO 639-1 format
    // BuiltList<String> languages
    test('to test the property `languages`', () async {
      // TODO
    });

    // Device locale BCP-47 format
    // String locale
    test('to test the property `locale`', () async {
      // TODO
    });

    // Device model name
    // String model
    test('to test the property `model`', () async {
      // TODO
    });

    // Network type: 4G, 5G, ethernet, wifi In Android: NetworkCapabilities.getNetworkCapabilities()
    // String networkType
    test('to test the property `networkType`', () async {
      // TODO
    });

    // OS Family
    // String osFamily
    test('to test the property `osFamily`', () async {
      // TODO
    });

    // Short name of the OS. This value if specific to os family. Examples: Windows: 10, 11; Android: 16; iOS: 18; MacOS: 15; Linux: Debian, Ubuntu, Arch
    // String osName
    test('to test the property `osName`', () async {
      // TODO
    });

    // Marketing name for the release version iOS: Dawn Android: Baklava MacOS: Sequoia Ubuntu Linux: Plucky Puffin
    // String osReleaseName
    test('to test the property `osReleaseName`', () async {
      // TODO
    });

    // Full name of the version. Examples: iOS: 18.3 Android: 16.1 MacOS: 15.5 Windows: 24H2 Ubuntu Linux: 25.04
    // String osVersion
    test('to test the property `osVersion`', () async {
      // TODO
    });

    // Screen density, PPI
    // int screenDensity
    test('to test the property `screenDensity`', () async {
      // TODO
    });

    // Screen height in pixels
    // int screenHeight
    test('to test the property `screenHeight`', () async {
      // TODO
    });

    // Screen width in pixels
    // int screenWidth
    test('to test the property `screenWidth`', () async {
      // TODO
    });

    // Internal storage size in GB
    // int storageFreeSpace
    test('to test the property `storageFreeSpace`', () async {
      // TODO
    });

    // Internal storage size in GB
    // int storageSize
    test('to test the property `storageSize`', () async {
      // TODO
    });

    // Device timezone
    // String timezone
    test('to test the property `timezone`', () async {
      // TODO
    });

    // Timezone abbreviation
    // String timezoneAbbr
    test('to test the property `timezoneAbbr`', () async {
      // TODO
    });

    // Device type
    // String type
    test('to test the property `type`', () async {
      // TODO
    });

  });
}
