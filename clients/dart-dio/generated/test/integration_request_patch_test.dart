import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for IntegrationRequestPatch
void main() {
  final instance = IntegrationRequestPatchBuilder();
  // TODO add properties to the builder and call build()

  group(IntegrationRequestPatch, () {
    // String connectedMerchantId
    test('to test the property `connectedMerchantId`', () async {
      // TODO
    });

    // String connectedAdvertiserId
    test('to test the property `connectedAdvertiserId`', () async {
      // TODO
    });

    // String connectedLbaId
    test('to test the property `connectedLbaId`', () async {
      // TODO
    });

    // String connectedTagId
    test('to test the property `connectedTagId`', () async {
      // TODO
    });

    // String partnerAccessToken
    test('to test the property `partnerAccessToken`', () async {
      // TODO
    });

    // String partnerRefreshToken
    test('to test the property `partnerRefreshToken`', () async {
      // TODO
    });

    // String partnerPrimaryEmail
    test('to test the property `partnerPrimaryEmail`', () async {
      // TODO
    });

    // num partnerAccessTokenExpiry
    test('to test the property `partnerAccessTokenExpiry`', () async {
      // TODO
    });

    // num partnerRefreshTokenExpiry
    test('to test the property `partnerRefreshTokenExpiry`', () async {
      // TODO
    });

    // String scopes
    test('to test the property `scopes`', () async {
      // TODO
    });

    // String additionalId1
    test('to test the property `additionalId1`', () async {
      // TODO
    });

    // String partnerMetadata
    test('to test the property `partnerMetadata`', () async {
      // TODO
    });

  });
}
