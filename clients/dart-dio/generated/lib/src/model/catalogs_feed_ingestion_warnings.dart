//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_element
import 'package:built_value/built_value.dart';
import 'package:built_value/serializer.dart';

part 'catalogs_feed_ingestion_warnings.g.dart';

/// CatalogsFeedIngestionWarnings
///
/// Properties:
/// * [AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS400] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS403] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS404] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS405] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS410] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS429] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS500] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS502] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS503] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS504] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS507] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS508] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS520] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS521] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS522] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [aDIMAGEDOWNLOADHTTPSTATUS525] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_ERROR] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_IMAGE_DOWNLOAD_INVALID_FILE] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_SITE_ERROR] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_SITE_TIMEOUT] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_SSL_ERROR] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_PROCESSING_EMPTY_FILE] - Ad image files are unreadable. Please check your link and upload new files to continue.
/// * [AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL] - Ad images must have a height larger than 75 pixels
/// * [AD_IMAGE_PROCESSING_TOO_MANY_PIXELS] - Ad images must have a maximum area (width x height) of less than 89478485 pixels
/// * [AD_IMAGE_PROCESSING_TYPE_MISMATCH] - Some ad images could not be processed due to a file type mismatch.
/// * [AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL] - Ad images must have a width larger than 75 pixels
/// * [AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS400] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS403] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS404] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS405] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS410] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS429] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS500] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS502] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS503] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS504] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS507] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS508] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS520] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS521] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS522] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [aDVIDEODOWNLOADHTTPSTATUS525] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_ERROR] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [AD_VIDEO_DOWNLOAD_INVALID_FILE] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_SITE_ERROR] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_SITE_TIMEOUT] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_SSL_ERROR] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_LENGTH_TOO_SHORT] - Ad videos length is too short. Please ensure that all ad videos are at least 4 seconds long.
/// * [AD_VIDEO_PROCESSING_EMPTY_FILE] - Ad video files are unreadable. Please check your link and upload new files to continue.
/// * [AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL] - Ad videos must have a height larger than 75 pixels
/// * [AD_VIDEO_PROCESSING_TOO_MANY_PIXELS] - Ad videos must have a maximum area (width x height) of less than 89478485 pixels
/// * [AD_VIDEO_PROCESSING_TYPE_MISMATCH] - Some ad videos could not be processed due to a file type mismatch.
/// * [AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL] - Ad videos must have a width larger than 75 pixels
/// * [ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE] - Additional image files are unreadable. Please upload new files to continue.
/// * [ADDITIONAL_IMAGE_FILE_NOT_FOUND] - Additional image files are unreadable. Please upload new files to continue.
/// * [ADDITIONAL_IMAGE_INVALID_FILE] - Additional image files are unreadable. Please upload new files to continue.
/// * [ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR] - We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
/// * [ADDITIONAL_IMAGE_MALFORMED_URL] - Additional image files are unreadable. Please check your link and upload new files to continue.
/// * [FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT] - Update your Google Sheets sharing settings from 'Editor' to 'Viewer'.
/// * [HOTEL_PRICE_HEADER_IS_PRESENT] - Price is not a supported column. Use base_price and sale_price instead.
/// * [VIDEO_DOWNLOAD_VIDEO_TOO_SHORT] - Video length is too short. Please ensure that the main video is at least 4 seconds long.
/// * [VIDEO_FILE_NOT_ACCESSIBLE] - Video files are unreadable. Please upload new files to continue.
/// * [VIDEO_FILE_NOT_FOUND] - Video files are unreadable. Please upload new files to continue.
/// * [VIDEO_INVALID_FILE] - Video files are unreadable. Please upload new files to continue.
/// * [VIDEO_LEVEL_INTERNAL_ERROR] - We experienced a technical difficulty and were unable to download some additional videos. The next download attempt will happen in 24 hours.
/// * [VIDEO_MALFORMED_URL] - Video files are unreadable. Please check your link and upload new files to continue.
@BuiltValue()
abstract class CatalogsFeedIngestionWarnings implements Built<CatalogsFeedIngestionWarnings, CatalogsFeedIngestionWarningsBuilder> {
  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR')
  int? get AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR')
  int? get AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE')
  int? get AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND')
  int? get AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_400')
  int? get aDIMAGEDOWNLOADHTTPSTATUS400;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_403')
  int? get aDIMAGEDOWNLOADHTTPSTATUS403;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_404')
  int? get aDIMAGEDOWNLOADHTTPSTATUS404;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_405')
  int? get aDIMAGEDOWNLOADHTTPSTATUS405;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_410')
  int? get aDIMAGEDOWNLOADHTTPSTATUS410;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_429')
  int? get aDIMAGEDOWNLOADHTTPSTATUS429;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_500')
  int? get aDIMAGEDOWNLOADHTTPSTATUS500;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_502')
  int? get aDIMAGEDOWNLOADHTTPSTATUS502;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_503')
  int? get aDIMAGEDOWNLOADHTTPSTATUS503;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_504')
  int? get aDIMAGEDOWNLOADHTTPSTATUS504;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_507')
  int? get aDIMAGEDOWNLOADHTTPSTATUS507;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_508')
  int? get aDIMAGEDOWNLOADHTTPSTATUS508;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_520')
  int? get aDIMAGEDOWNLOADHTTPSTATUS520;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_521')
  int? get aDIMAGEDOWNLOADHTTPSTATUS521;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_522')
  int? get aDIMAGEDOWNLOADHTTPSTATUS522;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_525')
  int? get aDIMAGEDOWNLOADHTTPSTATUS525;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_ERROR')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_ERROR;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED')
  int? get AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_INVALID_FILE')
  int? get AD_IMAGE_DOWNLOAD_INVALID_FILE;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_SITE_ERROR')
  int? get AD_IMAGE_DOWNLOAD_SITE_ERROR;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_SITE_TIMEOUT')
  int? get AD_IMAGE_DOWNLOAD_SITE_TIMEOUT;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_SSL_ERROR')
  int? get AD_IMAGE_DOWNLOAD_SSL_ERROR;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR')
  int? get AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR;

  /// Ad image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_IMAGE_PROCESSING_EMPTY_FILE')
  int? get AD_IMAGE_PROCESSING_EMPTY_FILE;

  /// Ad images must have a height larger than 75 pixels
  @BuiltValueField(wireName: r'AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL')
  int? get AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL;

  /// Ad images must have a maximum area (width x height) of less than 89478485 pixels
  @BuiltValueField(wireName: r'AD_IMAGE_PROCESSING_TOO_MANY_PIXELS')
  int? get AD_IMAGE_PROCESSING_TOO_MANY_PIXELS;

  /// Some ad images could not be processed due to a file type mismatch.
  @BuiltValueField(wireName: r'AD_IMAGE_PROCESSING_TYPE_MISMATCH')
  int? get AD_IMAGE_PROCESSING_TYPE_MISMATCH;

  /// Ad images must have a width larger than 75 pixels
  @BuiltValueField(wireName: r'AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL')
  int? get AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR')
  int? get AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR')
  int? get AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE')
  int? get AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND')
  int? get AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_400')
  int? get aDVIDEODOWNLOADHTTPSTATUS400;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_403')
  int? get aDVIDEODOWNLOADHTTPSTATUS403;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_404')
  int? get aDVIDEODOWNLOADHTTPSTATUS404;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_405')
  int? get aDVIDEODOWNLOADHTTPSTATUS405;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_410')
  int? get aDVIDEODOWNLOADHTTPSTATUS410;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_429')
  int? get aDVIDEODOWNLOADHTTPSTATUS429;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_500')
  int? get aDVIDEODOWNLOADHTTPSTATUS500;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_502')
  int? get aDVIDEODOWNLOADHTTPSTATUS502;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_503')
  int? get aDVIDEODOWNLOADHTTPSTATUS503;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_504')
  int? get aDVIDEODOWNLOADHTTPSTATUS504;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_507')
  int? get aDVIDEODOWNLOADHTTPSTATUS507;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_508')
  int? get aDVIDEODOWNLOADHTTPSTATUS508;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_520')
  int? get aDVIDEODOWNLOADHTTPSTATUS520;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_521')
  int? get aDVIDEODOWNLOADHTTPSTATUS521;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_522')
  int? get aDVIDEODOWNLOADHTTPSTATUS522;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_525')
  int? get aDVIDEODOWNLOADHTTPSTATUS525;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_ERROR')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_ERROR;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED')
  int? get AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_INVALID_FILE')
  int? get AD_VIDEO_DOWNLOAD_INVALID_FILE;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_SITE_ERROR')
  int? get AD_VIDEO_DOWNLOAD_SITE_ERROR;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_SITE_TIMEOUT')
  int? get AD_VIDEO_DOWNLOAD_SITE_TIMEOUT;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_SSL_ERROR')
  int? get AD_VIDEO_DOWNLOAD_SSL_ERROR;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR')
  int? get AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR;

  /// Ad videos length is too short. Please ensure that all ad videos are at least 4 seconds long.
  @BuiltValueField(wireName: r'AD_VIDEO_LENGTH_TOO_SHORT')
  int? get AD_VIDEO_LENGTH_TOO_SHORT;

  /// Ad video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'AD_VIDEO_PROCESSING_EMPTY_FILE')
  int? get AD_VIDEO_PROCESSING_EMPTY_FILE;

  /// Ad videos must have a height larger than 75 pixels
  @BuiltValueField(wireName: r'AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL')
  int? get AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL;

  /// Ad videos must have a maximum area (width x height) of less than 89478485 pixels
  @BuiltValueField(wireName: r'AD_VIDEO_PROCESSING_TOO_MANY_PIXELS')
  int? get AD_VIDEO_PROCESSING_TOO_MANY_PIXELS;

  /// Some ad videos could not be processed due to a file type mismatch.
  @BuiltValueField(wireName: r'AD_VIDEO_PROCESSING_TYPE_MISMATCH')
  int? get AD_VIDEO_PROCESSING_TYPE_MISMATCH;

  /// Ad videos must have a width larger than 75 pixels
  @BuiltValueField(wireName: r'AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL')
  int? get AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL;

  /// Additional image files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE')
  int? get ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE;

  /// Additional image files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'ADDITIONAL_IMAGE_FILE_NOT_FOUND')
  int? get ADDITIONAL_IMAGE_FILE_NOT_FOUND;

  /// Additional image files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'ADDITIONAL_IMAGE_INVALID_FILE')
  int? get ADDITIONAL_IMAGE_INVALID_FILE;

  /// We experienced a technical difficulty and were unable to download some additional images. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR')
  int? get ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR;

  /// Additional image files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'ADDITIONAL_IMAGE_MALFORMED_URL')
  int? get ADDITIONAL_IMAGE_MALFORMED_URL;

  /// Update your Google Sheets sharing settings from 'Editor' to 'Viewer'.
  @BuiltValueField(wireName: r'FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT')
  int? get FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT;

  /// Price is not a supported column. Use base_price and sale_price instead.
  @BuiltValueField(wireName: r'HOTEL_PRICE_HEADER_IS_PRESENT')
  int? get HOTEL_PRICE_HEADER_IS_PRESENT;

  /// Video length is too short. Please ensure that the main video is at least 4 seconds long.
  @BuiltValueField(wireName: r'VIDEO_DOWNLOAD_VIDEO_TOO_SHORT')
  int? get VIDEO_DOWNLOAD_VIDEO_TOO_SHORT;

  /// Video files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'VIDEO_FILE_NOT_ACCESSIBLE')
  int? get VIDEO_FILE_NOT_ACCESSIBLE;

  /// Video files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'VIDEO_FILE_NOT_FOUND')
  int? get VIDEO_FILE_NOT_FOUND;

  /// Video files are unreadable. Please upload new files to continue.
  @BuiltValueField(wireName: r'VIDEO_INVALID_FILE')
  int? get VIDEO_INVALID_FILE;

  /// We experienced a technical difficulty and were unable to download some additional videos. The next download attempt will happen in 24 hours.
  @BuiltValueField(wireName: r'VIDEO_LEVEL_INTERNAL_ERROR')
  int? get VIDEO_LEVEL_INTERNAL_ERROR;

  /// Video files are unreadable. Please check your link and upload new files to continue.
  @BuiltValueField(wireName: r'VIDEO_MALFORMED_URL')
  int? get VIDEO_MALFORMED_URL;

  CatalogsFeedIngestionWarnings._();

  factory CatalogsFeedIngestionWarnings([void updates(CatalogsFeedIngestionWarningsBuilder b)]) = _$CatalogsFeedIngestionWarnings;

  @BuiltValueHook(initializeBuilder: true)
  static void _defaults(CatalogsFeedIngestionWarningsBuilder b) => b;

  @BuiltValueSerializer(custom: true)
  static Serializer<CatalogsFeedIngestionWarnings> get serializer => _$CatalogsFeedIngestionWarningsSerializer();
}

class _$CatalogsFeedIngestionWarningsSerializer implements PrimitiveSerializer<CatalogsFeedIngestionWarnings> {
  @override
  final Iterable<Type> types = const [CatalogsFeedIngestionWarnings, _$CatalogsFeedIngestionWarnings];

  @override
  final String wireName = r'CatalogsFeedIngestionWarnings';

  Iterable<Object?> _serializeProperties(
    Serializers serializers,
    CatalogsFeedIngestionWarnings object, {
    FullType specifiedType = FullType.unspecified,
  }) sync* {
    if (object.AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE != null) {
      yield r'AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND != null) {
      yield r'AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS400 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_400';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS400,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS403 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_403';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS403,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS404 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_404';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS404,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS405 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_405';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS405,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS410 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_410';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS410,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS429 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_429';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS429,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS500 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_500';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS500,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS502 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_502';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS502,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS503 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_503';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS503,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS504 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_504';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS504,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS507 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_507';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS507,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS508 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_508';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS508,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS520 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_520';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS520,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS521 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_521';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS521,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS522 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_522';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS522,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDIMAGEDOWNLOADHTTPSTATUS525 != null) {
      yield r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_525';
      yield serializers.serialize(
        object.aDIMAGEDOWNLOADHTTPSTATUS525,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED != null) {
      yield r'AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_INVALID_FILE != null) {
      yield r'AD_IMAGE_DOWNLOAD_INVALID_FILE';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_INVALID_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_SITE_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_SITE_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_SITE_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_SITE_TIMEOUT != null) {
      yield r'AD_IMAGE_DOWNLOAD_SITE_TIMEOUT';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_SITE_TIMEOUT,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_SSL_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_SSL_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_SSL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR != null) {
      yield r'AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR';
      yield serializers.serialize(
        object.AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_PROCESSING_EMPTY_FILE != null) {
      yield r'AD_IMAGE_PROCESSING_EMPTY_FILE';
      yield serializers.serialize(
        object.AD_IMAGE_PROCESSING_EMPTY_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL != null) {
      yield r'AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL';
      yield serializers.serialize(
        object.AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_PROCESSING_TOO_MANY_PIXELS != null) {
      yield r'AD_IMAGE_PROCESSING_TOO_MANY_PIXELS';
      yield serializers.serialize(
        object.AD_IMAGE_PROCESSING_TOO_MANY_PIXELS,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_PROCESSING_TYPE_MISMATCH != null) {
      yield r'AD_IMAGE_PROCESSING_TYPE_MISMATCH';
      yield serializers.serialize(
        object.AD_IMAGE_PROCESSING_TYPE_MISMATCH,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL != null) {
      yield r'AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL';
      yield serializers.serialize(
        object.AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE != null) {
      yield r'AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND != null) {
      yield r'AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS400 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_400';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS400,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS403 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_403';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS403,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS404 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_404';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS404,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS405 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_405';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS405,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS410 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_410';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS410,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS429 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_429';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS429,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS500 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_500';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS500,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS502 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_502';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS502,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS503 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_503';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS503,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS504 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_504';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS504,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS507 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_507';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS507,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS508 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_508';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS508,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS520 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_520';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS520,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS521 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_521';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS521,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS522 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_522';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS522,
        specifiedType: const FullType(int),
      );
    }
    if (object.aDVIDEODOWNLOADHTTPSTATUS525 != null) {
      yield r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_525';
      yield serializers.serialize(
        object.aDVIDEODOWNLOADHTTPSTATUS525,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED != null) {
      yield r'AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_INVALID_FILE != null) {
      yield r'AD_VIDEO_DOWNLOAD_INVALID_FILE';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_INVALID_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_SITE_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_SITE_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_SITE_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_SITE_TIMEOUT != null) {
      yield r'AD_VIDEO_DOWNLOAD_SITE_TIMEOUT';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_SITE_TIMEOUT,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_SSL_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_SSL_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_SSL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR != null) {
      yield r'AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR';
      yield serializers.serialize(
        object.AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_LENGTH_TOO_SHORT != null) {
      yield r'AD_VIDEO_LENGTH_TOO_SHORT';
      yield serializers.serialize(
        object.AD_VIDEO_LENGTH_TOO_SHORT,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_PROCESSING_EMPTY_FILE != null) {
      yield r'AD_VIDEO_PROCESSING_EMPTY_FILE';
      yield serializers.serialize(
        object.AD_VIDEO_PROCESSING_EMPTY_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL != null) {
      yield r'AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL';
      yield serializers.serialize(
        object.AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_PROCESSING_TOO_MANY_PIXELS != null) {
      yield r'AD_VIDEO_PROCESSING_TOO_MANY_PIXELS';
      yield serializers.serialize(
        object.AD_VIDEO_PROCESSING_TOO_MANY_PIXELS,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_PROCESSING_TYPE_MISMATCH != null) {
      yield r'AD_VIDEO_PROCESSING_TYPE_MISMATCH';
      yield serializers.serialize(
        object.AD_VIDEO_PROCESSING_TYPE_MISMATCH,
        specifiedType: const FullType(int),
      );
    }
    if (object.AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL != null) {
      yield r'AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL';
      yield serializers.serialize(
        object.AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL,
        specifiedType: const FullType(int),
      );
    }
    if (object.ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE != null) {
      yield r'ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE';
      yield serializers.serialize(
        object.ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE,
        specifiedType: const FullType(int),
      );
    }
    if (object.ADDITIONAL_IMAGE_FILE_NOT_FOUND != null) {
      yield r'ADDITIONAL_IMAGE_FILE_NOT_FOUND';
      yield serializers.serialize(
        object.ADDITIONAL_IMAGE_FILE_NOT_FOUND,
        specifiedType: const FullType(int),
      );
    }
    if (object.ADDITIONAL_IMAGE_INVALID_FILE != null) {
      yield r'ADDITIONAL_IMAGE_INVALID_FILE';
      yield serializers.serialize(
        object.ADDITIONAL_IMAGE_INVALID_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR != null) {
      yield r'ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR';
      yield serializers.serialize(
        object.ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.ADDITIONAL_IMAGE_MALFORMED_URL != null) {
      yield r'ADDITIONAL_IMAGE_MALFORMED_URL';
      yield serializers.serialize(
        object.ADDITIONAL_IMAGE_MALFORMED_URL,
        specifiedType: const FullType(int),
      );
    }
    if (object.FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT != null) {
      yield r'FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT';
      yield serializers.serialize(
        object.FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT,
        specifiedType: const FullType(int),
      );
    }
    if (object.HOTEL_PRICE_HEADER_IS_PRESENT != null) {
      yield r'HOTEL_PRICE_HEADER_IS_PRESENT';
      yield serializers.serialize(
        object.HOTEL_PRICE_HEADER_IS_PRESENT,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_DOWNLOAD_VIDEO_TOO_SHORT != null) {
      yield r'VIDEO_DOWNLOAD_VIDEO_TOO_SHORT';
      yield serializers.serialize(
        object.VIDEO_DOWNLOAD_VIDEO_TOO_SHORT,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_FILE_NOT_ACCESSIBLE != null) {
      yield r'VIDEO_FILE_NOT_ACCESSIBLE';
      yield serializers.serialize(
        object.VIDEO_FILE_NOT_ACCESSIBLE,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_FILE_NOT_FOUND != null) {
      yield r'VIDEO_FILE_NOT_FOUND';
      yield serializers.serialize(
        object.VIDEO_FILE_NOT_FOUND,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_INVALID_FILE != null) {
      yield r'VIDEO_INVALID_FILE';
      yield serializers.serialize(
        object.VIDEO_INVALID_FILE,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_LEVEL_INTERNAL_ERROR != null) {
      yield r'VIDEO_LEVEL_INTERNAL_ERROR';
      yield serializers.serialize(
        object.VIDEO_LEVEL_INTERNAL_ERROR,
        specifiedType: const FullType(int),
      );
    }
    if (object.VIDEO_MALFORMED_URL != null) {
      yield r'VIDEO_MALFORMED_URL';
      yield serializers.serialize(
        object.VIDEO_MALFORMED_URL,
        specifiedType: const FullType(int),
      );
    }
  }

  @override
  Object serialize(
    Serializers serializers,
    CatalogsFeedIngestionWarnings object, {
    FullType specifiedType = FullType.unspecified,
  }) {
    return _serializeProperties(serializers, object, specifiedType: specifiedType).toList();
  }

  void _deserializeProperties(
    Serializers serializers,
    Object serialized, {
    FullType specifiedType = FullType.unspecified,
    required List<Object?> serializedList,
    required CatalogsFeedIngestionWarningsBuilder result,
    required List<Object?> unhandled,
  }) {
    for (var i = 0; i < serializedList.length; i += 2) {
      final key = serializedList[i] as String;
      final value = serializedList[i + 1];
      switch (key) {
        case r'AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_CONTENT_READ_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_DNS_LOOKUP_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_FILE_NOT_ACCESSIBLE = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_FILE_NOT_FOUND = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_400':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS400 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_403':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS403 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_404':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS404 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_405':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS405 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_410':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS410 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_429':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS429 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_500':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS500 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_502':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS502 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_503':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS503 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_504':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS504 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_507':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS507 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_508':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS508 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_520':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS520 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_521':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS521 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_522':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS522 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_HTTP_STATUS_525':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDIMAGEDOWNLOADHTTPSTATUS525 = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_MALFORMED_URL = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_RATE_LIMITED = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INTERNAL_REQUEST_EXPIRED = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_INVALID_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_INVALID_FILE = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_SITE_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_SITE_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_SITE_TIMEOUT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_SITE_TIMEOUT = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_SSL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_SSL_ERROR = valueDes;
          break;
        case r'AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_DOWNLOAD_SSL_HANDSHAKE_ERROR = valueDes;
          break;
        case r'AD_IMAGE_PROCESSING_EMPTY_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_PROCESSING_EMPTY_FILE = valueDes;
          break;
        case r'AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_PROCESSING_HEIGHT_TOO_SMALL = valueDes;
          break;
        case r'AD_IMAGE_PROCESSING_TOO_MANY_PIXELS':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_PROCESSING_TOO_MANY_PIXELS = valueDes;
          break;
        case r'AD_IMAGE_PROCESSING_TYPE_MISMATCH':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_PROCESSING_TYPE_MISMATCH = valueDes;
          break;
        case r'AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_IMAGE_PROCESSING_WIDTH_TOO_SMALL = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_CONTENT_READ_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_DNS_LOOKUP_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_FILE_NOT_ACCESSIBLE = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_FILE_NOT_FOUND = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_400':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS400 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_403':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS403 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_404':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS404 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_405':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS405 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_410':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS410 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_429':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS429 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_500':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS500 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_502':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS502 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_503':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS503 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_504':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS504 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_507':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS507 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_508':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS508 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_520':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS520 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_521':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS521 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_522':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS522 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_HTTP_STATUS_525':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.aDVIDEODOWNLOADHTTPSTATUS525 = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_CONFIGURATION_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_FAILED_TO_DOWNLOAD = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_MALFORMED_URL = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_RATE_LIMITED = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INTERNAL_REQUEST_EXPIRED = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_INVALID_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_INVALID_FILE = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_SITE_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_SITE_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_SITE_TIMEOUT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_SITE_TIMEOUT = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_SSL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_SSL_ERROR = valueDes;
          break;
        case r'AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_DOWNLOAD_SSL_HANDSHAKE_ERROR = valueDes;
          break;
        case r'AD_VIDEO_LENGTH_TOO_SHORT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_LENGTH_TOO_SHORT = valueDes;
          break;
        case r'AD_VIDEO_PROCESSING_EMPTY_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_PROCESSING_EMPTY_FILE = valueDes;
          break;
        case r'AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_PROCESSING_HEIGHT_TOO_SMALL = valueDes;
          break;
        case r'AD_VIDEO_PROCESSING_TOO_MANY_PIXELS':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_PROCESSING_TOO_MANY_PIXELS = valueDes;
          break;
        case r'AD_VIDEO_PROCESSING_TYPE_MISMATCH':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_PROCESSING_TYPE_MISMATCH = valueDes;
          break;
        case r'AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.AD_VIDEO_PROCESSING_WIDTH_TOO_SMALL = valueDes;
          break;
        case r'ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.ADDITIONAL_IMAGE_FILE_NOT_ACCESSIBLE = valueDes;
          break;
        case r'ADDITIONAL_IMAGE_FILE_NOT_FOUND':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.ADDITIONAL_IMAGE_FILE_NOT_FOUND = valueDes;
          break;
        case r'ADDITIONAL_IMAGE_INVALID_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.ADDITIONAL_IMAGE_INVALID_FILE = valueDes;
          break;
        case r'ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.ADDITIONAL_IMAGE_LEVEL_INTERNAL_ERROR = valueDes;
          break;
        case r'ADDITIONAL_IMAGE_MALFORMED_URL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.ADDITIONAL_IMAGE_MALFORMED_URL = valueDes;
          break;
        case r'FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.FETCH_GOOGLE_SHEET_PUBLIC_CAN_EDIT = valueDes;
          break;
        case r'HOTEL_PRICE_HEADER_IS_PRESENT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.HOTEL_PRICE_HEADER_IS_PRESENT = valueDes;
          break;
        case r'VIDEO_DOWNLOAD_VIDEO_TOO_SHORT':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_DOWNLOAD_VIDEO_TOO_SHORT = valueDes;
          break;
        case r'VIDEO_FILE_NOT_ACCESSIBLE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_FILE_NOT_ACCESSIBLE = valueDes;
          break;
        case r'VIDEO_FILE_NOT_FOUND':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_FILE_NOT_FOUND = valueDes;
          break;
        case r'VIDEO_INVALID_FILE':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_INVALID_FILE = valueDes;
          break;
        case r'VIDEO_LEVEL_INTERNAL_ERROR':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_LEVEL_INTERNAL_ERROR = valueDes;
          break;
        case r'VIDEO_MALFORMED_URL':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(int),
          ) as int;
          result.VIDEO_MALFORMED_URL = valueDes;
          break;
        default:
          unhandled.add(key);
          unhandled.add(value);
          break;
      }
    }
  }

  @override
  CatalogsFeedIngestionWarnings deserialize(
    Serializers serializers,
    Object serialized, {
    FullType specifiedType = FullType.unspecified,
  }) {
    final result = CatalogsFeedIngestionWarningsBuilder();
    final serializedList = (serialized as Iterable<Object?>).toList();
    final unhandled = <Object?>[];
    _deserializeProperties(
      serializers,
      serialized,
      specifiedType: specifiedType,
      serializedList: serializedList,
      unhandled: unhandled,
      result: result,
    );
    return result.build();
  }
}

