/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.28.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: https://openapi-generator.tech
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Org.OpenAPITools.Converters;

namespace Org.OpenAPITools.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class BusinessAssets : IEquatable<BusinessAssets>
    {
        /// <summary>
        /// An object containing all the information specific to the provided asset group. This field will be populated only if asset_type equals &#39;ASSET_GROUP&#39;.
        /// </summary>
        /// <value>An object containing all the information specific to the provided asset group. This field will be populated only if asset_type equals &#39;ASSET_GROUP&#39;.</value>
        [DataMember(Name="asset_group_info", EmitDefaultValue=false)]
        public AssetGroupBinding AssetGroupInfo { get; set; }

        /// <summary>
        /// Unique identifier of a business asset.
        /// </summary>
        /// <value>Unique identifier of a business asset.</value>
        /* <example>549755885175</example> */
        [RegularExpression("^\\d+$")]
        [StringLength(20, MinimumLength=1)]
        [DataMember(Name="asset_id", EmitDefaultValue=false)]
        public string AssetId { get; set; }

        /// <summary>
        /// Gets or Sets AssetType
        /// </summary>
        [DataMember(Name="asset_type", EmitDefaultValue=true)]
        public AssetTypeResponse AssetType { get; set; }

        /// <summary>
        /// Permission levels the requesting business has on an asset.
        /// </summary>
        /// <value>Permission levels the requesting business has on an asset.</value>
        /* <example>[FINANCE_MANAGER, CATALOGS_MANAGER, AUDIENCE_MANAGER]</example> */
        [DataMember(Name="permissions", EmitDefaultValue=false)]
        public List<string> Permissions { get; set; }

        /// <summary>
        /// An object containing all the information specific to the provided catalog. This field will be populated only if asset_type equals &#39;CATALOG&#39;.
        /// </summary>
        /// <value>An object containing all the information specific to the provided catalog. This field will be populated only if asset_type equals &#39;CATALOG&#39;.</value>
        [DataMember(Name="catalog_info", EmitDefaultValue=false)]
        public CatalogBinding CatalogInfo { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class BusinessAssets {\n");
            sb.Append("  AssetGroupInfo: ").Append(AssetGroupInfo).Append("\n");
            sb.Append("  AssetId: ").Append(AssetId).Append("\n");
            sb.Append("  AssetType: ").Append(AssetType).Append("\n");
            sb.Append("  Permissions: ").Append(Permissions).Append("\n");
            sb.Append("  CatalogInfo: ").Append(CatalogInfo).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (obj is null) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((BusinessAssets)obj);
        }

        /// <summary>
        /// Returns true if BusinessAssets instances are equal
        /// </summary>
        /// <param name="other">Instance of BusinessAssets to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(BusinessAssets other)
        {
            if (other is null) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    AssetGroupInfo == other.AssetGroupInfo ||
                    AssetGroupInfo != null &&
                    AssetGroupInfo.Equals(other.AssetGroupInfo)
                ) && 
                (
                    AssetId == other.AssetId ||
                    AssetId != null &&
                    AssetId.Equals(other.AssetId)
                ) && 
                (
                    AssetType == other.AssetType ||
                    
                    AssetType.Equals(other.AssetType)
                ) && 
                (
                    Permissions == other.Permissions ||
                    Permissions != null &&
                    other.Permissions != null &&
                    Permissions.SequenceEqual(other.Permissions)
                ) && 
                (
                    CatalogInfo == other.CatalogInfo ||
                    CatalogInfo != null &&
                    CatalogInfo.Equals(other.CatalogInfo)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (AssetGroupInfo != null)
                    hashCode = hashCode * 59 + AssetGroupInfo.GetHashCode();
                    if (AssetId != null)
                    hashCode = hashCode * 59 + AssetId.GetHashCode();
                    
                    hashCode = hashCode * 59 + AssetType.GetHashCode();
                    if (Permissions != null)
                    hashCode = hashCode * 59 + Permissions.GetHashCode();
                    if (CatalogInfo != null)
                    hashCode = hashCode * 59 + CatalogInfo.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(BusinessAssets left, BusinessAssets right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(BusinessAssets left, BusinessAssets right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
