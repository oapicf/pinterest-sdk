
# ScheduleCreateRequest

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**entityId** | **kotlin.String** |  | 
**entityType** | **kotlin.String** | Entity type | 
**deltaValue** | [**ScheduleCommonDeltaValue**](ScheduleCommonDeltaValue.md) |  | 
**endTimestamp** | **kotlin.Int** | Schedule end time. Unix timestamp in seconds. | 
**name** | **kotlin.String** |  | 
**scheduleAction** | [**ScheduleAction**](ScheduleAction.md) |  | 
**scheduleStatus** | [**ScheduleStatus**](ScheduleStatus.md) |  | 
**scheduleType** | [**ScheduleType**](ScheduleType.md) |  | 
**startTimestamp** | **kotlin.Int** | Schedule start time. Unix timestamp in seconds. | 



