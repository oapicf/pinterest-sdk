 ...
 * @param date 
 */
case class TimeSeries(date: Option[LocalDateTime]
                )

object TimeSeries {
    /**
     * Creates the codec for converting TimeSeries from and to JSON.
     */
    implicit val decoder: Decoder[TimeSeries] = deriveDecoder
    implicit val encoder: ObjectEncoder[TimeSeries] = deriveEncoder
}
