 ...
 * @param date 
 */
case class PredictedTimeSeries(date: Option[LocalDateTime]
                )

object PredictedTimeSeries {
    /**
     * Creates the codec for converting PredictedTimeSeries from and to JSON.
     */
    implicit val decoder: Decoder[PredictedTimeSeries] = deriveDecoder
    implicit val encoder: ObjectEncoder[PredictedTimeSeries] = deriveEncoder
}
