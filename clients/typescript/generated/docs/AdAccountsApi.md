...
> AdsAnalyticsCreateAsyncResponse analyticsCreateConversionProductReport(conversionProductReportRequest)
...
```typescript
import { createConfiguration, AdAccountsApi } from '';
import type { AdAccountsApiAnalyticsCreateConversionProductReportRequest } from '';

const configuration = createConfiguration();
const apiInstance = new AdAccountsApi(configuration);

const request: AdAccountsApiAnalyticsCreateConversionProductReportRequest = {
    // Unique identifier of an ad account.
  adAccountId: "4",
  ...
  conversionProductReportRequest: {
    ...
    conversionProductBreakdown: "PRODUCT_BRAND",
...
const data = await apiInstance.analyticsCreateConversionProductReport(request);
console.log('API called successfully. Returned data:', data);
```
...
const request: AdAccountsApiAnalyticsCreateTemplateReportRequest = {
  ...
  endDate: new Date('1970-01-01').toISOString().split('T')[0];,
    //    TOTAL - metrics are aggregated over the specified date range.    DAY - metrics are broken down daily.    HOUR - metrics are broken down hourly.    WEEKLY - metrics are broken down weekly.    MONTHLY - metrics are broken down monthly (optional)
  granularity: "TOTAL",
};
 ...
 **granularity** | **Granularity** |    TOTAL - metrics are aggregated over the specified date range.    DAY - metrics are broken down daily.    HOUR - metrics are broken down hourly.    WEEKLY - metrics are broken down weekly.    MONTHLY - metrics are broken down monthly | (optional) defaults to undefined
...
```typescript
import { createConfiguration, AdAccountsApi } from '';
import type { AdAccountsApiAnalyticsGetConversionProductReportRequest } from '';

const configuration = createConfiguration();
const apiInstance = new AdAccountsApi(configuration);

const request: AdAccountsApiAnalyticsGetConversionProductReportRequest = {
    // Unique identifier of an ad account.
  adAccountId: "4",
...
const data = await apiInstance.analyticsGetConversionProductReport(request);
console.log('API called successfully. Returned data:', data);
```
