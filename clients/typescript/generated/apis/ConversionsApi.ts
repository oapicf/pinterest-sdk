...
export class ConversionsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async advertiserDefinedEventsGet(adAccountId: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'adAccountId' is not null or undefined
        if (adAccountId === null || adAccountId === undefined) {
            throw new RequiredError("ConversionsApi", "advertiserDefinedEventsGet", "adAccountId");
        }


        // Path Params
        const localVarPath = '/ad_accounts/{ad_account_id}/advertiser_defined_events'
...
export class ConversionsApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to advertiserDefinedEventsGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async advertiserDefinedEventsGetWithHttpInfo(response: ResponseContext): Promise<HttpInfo<AdvertiserDefinedEventsResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AdvertiserDefinedEventsResponse = ObjectSerializer.deserialize(
