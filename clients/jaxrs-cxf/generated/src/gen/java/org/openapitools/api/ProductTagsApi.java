...
public interface ProductTagsApi  {
    ...
    @ApiOperation(value = "Add product tags to pin", tags={ "product_tags" })
    @ApiResponses(value = { 
    ...
    public ProductTagsResponse productTagsBulkAdd(@PathParam("pin_id") @Pattern(regexp="^\\d+$") String pinId, @Valid @NotNull ProductTagsBulkAddRequest productTagsBulkAddRequest);
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Delete product tags from pin", tags={ "product_tags" })
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 429, message = "The user has sent too many requests in a given amount of time and is being rate limited.", response = PinterestLibError.class),
        @ApiResponse(code = 200, message = "An unexpected error response.", response = PinterestLibError.class) })
    public void productTagsBulkDelete(@PathParam("pin_id") @Pattern(regexp="^\\d+$") String pinId, @Valid @NotNull ProductTagsBulkDeleteRequest productTagsBulkDeleteRequest);
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Get product tags for pin", tags={ "product_tags" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "The request has succeeded.", response = ProductTagsResponse.class),
        @ApiResponse(code = 400, message = "The request could not be understood by the server due to unexpected data.", response = PinterestLibError.class),
        @ApiResponse(code = 401, message = "Authentication is required and has either failed or not been provided.", response = PinterestLibError.class),
        ...
        @ApiResponse(code = 429, message = "The user has sent too many requests in a given amount of time and is being rate limited.", response = PinterestLibError.class),
        @ApiResponse(code = 200, message = "An unexpected error response.", response = PinterestLibError.class) })
    public ProductTagsResponse productTagsList(@PathParam("pin_id") @Pattern(regexp="^\\d+$") String pinId);
}
