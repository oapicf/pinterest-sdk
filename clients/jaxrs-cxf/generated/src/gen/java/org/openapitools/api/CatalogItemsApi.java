...
public interface CatalogItemsApi  {
    ...
    @ApiOperation(value = "Get item batch status", tags={ "catalog_items" })
    @ApiResponses(value = { 
    ...
    public CatalogsItemsBatch itemsBatchGet(@PathParam("batch_id") @Pattern(regexp="^\\d+$") String batchId, @QueryParam("ad_account_id") @Pattern(regexp="^\\d+$") @Size(max=18) String adAccountId);
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Operate on item batch", tags={ "catalog_items" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "The request has succeeded.", response = CatalogsItemsBatch.class),
        @ApiResponse(code = 400, message = "The request could not be understood by the server due to unexpected data.", response = PinterestLibError.class),
        @ApiResponse(code = 401, message = "Authentication is required and has either failed or not been provided.", response = PinterestLibError.class),
        ...
        @ApiResponse(code = 429, message = "The user has sent too many requests in a given amount of time and is being rate limited.", response = PinterestLibError.class),
        @ApiResponse(code = 200, message = "An unexpected error response.", response = PinterestLibError.class) })
    public CatalogsItemsBatch itemsBatchPost(@Valid @NotNull CatalogsItemsBatchPostRequest catalogsItemsBatchPostRequest, @QueryParam("ad_account_id") @Pattern(regexp="^\\d+$") @Size(max=18) String adAccountId);
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get catalogs items (POST)", tags={ "catalog_items" })
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 429, message = "The user has sent too many requests in a given amount of time and is being rate limited.", response = PinterestLibError.class),
        @ApiResponse(code = 200, message = "An unexpected error response.", response = PinterestLibError.class) })
    public ItemsPost200Response itemsPost(@Valid @NotNull CatalogsItemsRequest catalogsItemsRequest, @QueryParam("ad_account_id") @Pattern(regexp="^\\d+$") @Size(max=18) String adAccountId);
}
