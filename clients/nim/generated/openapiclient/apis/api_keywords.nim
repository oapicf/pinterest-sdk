...
proc keywordsGet*(httpClient: HttpClient, adAccountId: string, campaignId: string, adGroupId: string, adGroupIds: seq[string], matchTypes: seq[MatchType], pageSize: int, bookmark: string): (Option[keywords_get_200_response], Response) =
  ## Get keywords
  var query_params_list: seq[(string, string)] = @[]
  if $campaignId != "":
...
proc trendingKeywordsList*(httpClient: HttpClient, region: TrendsSupportedRegion, trendType: TrendType, interests: seq[Interests], genders: seq[Genders], ages: seq[Ages], includeKeywords: seq[string], normalizeAgainstGroup: bool, limit: int, includePrediction: bool, includeDemographics: bool): (Option[TrendingKeywordsResponse], Response) =
  ...
  if $includePrediction != "":
    query_params_list.add(("include_prediction", $includePrediction))
