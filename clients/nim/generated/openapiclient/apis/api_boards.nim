...
proc boardsListPins*(httpClient: HttpClient, boardId: string, bookmark: string, pageSize: int, creativeTypes: seq[CreativeType], adAccountId: string, pinMetrics: bool): (Option[boards_list_pins_200_response], Response) =
  ## List Pins on board
  var query_params_list: seq[(string, string)] = @[]
  if $bookmark != "":
    query_params_list.add(("bookmark", $bookmark))
