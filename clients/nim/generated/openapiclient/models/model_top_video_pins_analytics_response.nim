...
import model_top_video_pins_analytics_response_date_availability
import model_top_video_pins_analytics_response_pins_items
import model_top_video_pins_sort_by

type TopVideoPinsAnalyticsResponse* = object
  ## 
  dateAvailability*: Option[TopVideoPinsAnalyticsResponseDateAvailability]
  pins*: Option[seq[TopVideoPinsAnalyticsResponsePinsItems]]
  ...
  if node.kind == JObject:
    ...
    if node.hasKey("pins") and node["pins"].kind != JNull:
      ...
      if arrayNode.kind == JArray:
        var arr: seq[TopVideoPinsAnalyticsResponsePinsItems] = @[]
        for item in arrayNode.items:
          arr.add(to(item, TopVideoPinsAnalyticsResponsePinsItems))
