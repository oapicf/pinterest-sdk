...
import model_pinner_list_type

type Audience* = object
  ## 
  adAccountId*: Option[string] ## Ad account ID.
  audienceType*: Option[PinnerListType] ## [Audience types](/docs/reference/glossary/#Audience Types): ACTALIKE, ENGAGEMENT, CUSTOMER_LIST and VISITOR
  ...
  isNca*: Option[bool] ## Whether the audience derives from a new customer acquisition (expanded matching) customer list. Read-only.
  name*: Option[string] ## Audience name.
  rule*: Option[AudienceRule]
  size*: Option[int] ## Audience size.
  status*: Option[AudienceStatus] ## Audience status. READY, INITIALIZING, TOO_SMALL - Each audience list needs to have at least 100 people with Pinterest accounts before you can start using it.
