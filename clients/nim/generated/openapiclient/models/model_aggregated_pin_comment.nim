...
type AggregatedPinComment* = object
  ## 
  ...
  isProduct*: bool ## Whether the Pin is a product Pin.
...
# Custom JSON deserialization for AggregatedPinComment with custom field names
proc to*(node: JsonNode, T: typedesc[AggregatedPinComment]): AggregatedPinComment =
  result = AggregatedPinComment()
  if node.kind == JObject:
    ...
    if node.hasKey("is_product"):
      result.isProduct = to(node["is_product"], bool)
...
# Custom JSON serialization for AggregatedPinComment with custom field names
proc `%`*(obj: AggregatedPinComment): JsonNode =
  result = newJObject()
  ...
  result["is_product"] = %obj.isProduct
