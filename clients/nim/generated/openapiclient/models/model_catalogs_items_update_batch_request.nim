...
# Custom JSON deserialization for CatalogsItemsUpdateBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsItemsUpdateBatchRequest]): CatalogsItemsUpdateBatchRequest =
  result = CatalogsItemsUpdateBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, ItemUpdateBatchRecord))
...
# Custom JSON serialization for CatalogsItemsUpdateBatchRequest with custom field names
proc `%`*(obj: CatalogsItemsUpdateBatchRequest): JsonNode =
  result = newJObject()
