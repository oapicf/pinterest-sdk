...
# Custom JSON deserialization for LeadFormUpdateRequest with custom field names
proc to*(node: JsonNode, T: typedesc[LeadFormUpdateRequest]): LeadFormUpdateRequest =
  result = LeadFormUpdateRequest()
...
# Custom JSON serialization for LeadFormUpdateRequest with custom field names
proc `%`*(obj: LeadFormUpdateRequest): JsonNode =
  result = newJObject()
  if obj.name.isSome():
