...
# Custom JSON deserialization for Campaign with custom field names
proc to*(node: JsonNode, T: typedesc[Campaign]): Campaign =
  result = Campaign()
  if node.kind == JObject:
    ...
    if node.hasKey("objective_type"):
      result.objectiveType = to(node["objective_type"], CampaignObjectiveType)
...
# Custom JSON serialization for Campaign with custom field names
proc `%`*(obj: Campaign): JsonNode =
  result = newJObject()
