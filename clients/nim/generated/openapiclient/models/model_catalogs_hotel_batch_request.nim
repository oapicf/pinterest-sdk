...
# Custom JSON deserialization for CatalogsHotelBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsHotelBatchRequest]): CatalogsHotelBatchRequest =
  result = CatalogsHotelBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, CatalogsHotelBatchItem))
    if node.hasKey("language"):
      result.language = to(node["language"], Language)

# Custom JSON serialization for CatalogsHotelBatchRequest with custom field names
proc `%`*(obj: CatalogsHotelBatchRequest): JsonNode =
  result = newJObject()
