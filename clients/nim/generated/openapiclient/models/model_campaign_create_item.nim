...
type CampaignCreateItem* = object
...
# Custom JSON deserialization for CampaignCreateItem with custom field names
proc to*(node: JsonNode, T: typedesc[CampaignCreateItem]): CampaignCreateItem =
  result = CampaignCreateItem()
  if node.kind == JObject:
    ...
    if node.hasKey("objective_type"):
      result.objectiveType = to(node["objective_type"], ConversionObjectiveType)
...
# Custom JSON serialization for CampaignCreateItem with custom field names
proc `%`*(obj: CampaignCreateItem): JsonNode =
  result = newJObject()
