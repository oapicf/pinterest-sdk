...
type TargetingTemplateGetResponseData* = object
  ## 
  ...
  valid*: Option[bool] ## Inform if the targeting template is valid (ex. would be false if has revoked audience)
...
# Custom JSON deserialization for TargetingTemplateGetResponseData with custom field names
proc to*(node: JsonNode, T: typedesc[TargetingTemplateGetResponseData]): TargetingTemplateGetResponseData =
  result = TargetingTemplateGetResponseData()
  if node.kind == JObject:
    ...
    if node.hasKey("valid") and node["valid"].kind != JNull:
      result.valid = some(to(node["valid"], typeof(result.valid.get())))

# Custom JSON serialization for TargetingTemplateGetResponseData with custom field names
proc `%`*(obj: TargetingTemplateGetResponseData): JsonNode =
  result = newJObject()
