...
type AdArrayResponseElement* = object
  ## 
  data*: Option[AdResponse]
  exceptions*: Option[Exception]


# Custom JSON deserialization for AdArrayResponseElement with custom field names
proc to*(node: JsonNode, T: typedesc[AdArrayResponseElement]): AdArrayResponseElement =
  result = AdArrayResponseElement()
...
# Custom JSON serialization for AdArrayResponseElement with custom field names
proc `%`*(obj: AdArrayResponseElement): JsonNode =
  result = newJObject()
  if obj.data.isSome():
