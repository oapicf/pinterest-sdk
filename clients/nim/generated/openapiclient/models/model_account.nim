...
import model_user_account_type

type Account* = object
  ## User account model containing properties related to a user's account.
  about*: Option[string] ## Profile about description.
  accountType*: Option[UserAccountType] ## Type of account
  boardCount*: int ##   User account board count.   **Note**: Board count on user account level may differ from counts found elsewhere due to attribution of collaborative Boards.
...
proc to*(node: JsonNode, T: typedesc[Account]): Account =
  ...
  if node.kind == JObject:
    ...
    if node.hasKey("account_type") and node["account_type"].kind != JNull:
      result.accountType = some(to(node["account_type"], typeof(result.accountType.get())))
