...
# Custom JSON deserialization for CatalogsItemsRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsItemsRequest]): CatalogsItemsRequest =
  result = CatalogsItemsRequest()
  if node.kind == JObject:
    if node.hasKey("country"):
      result.country = to(node["country"], Country)
    if node.hasKey("filters"):
      result.filters = to(node["filters"], CatalogsItemsPostFilters)
    if node.hasKey("language"):
      result.language = to(node["language"], Language)

# Custom JSON serialization for CatalogsItemsRequest with custom field names
proc `%`*(obj: CatalogsItemsRequest): JsonNode =
  result = newJObject()
