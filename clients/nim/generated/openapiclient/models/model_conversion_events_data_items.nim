...
import model_conversion_events_data_items_custom_data
import model_conversion_events_user_data_properties

type ConversionEventsDataItems* = object
  ## 
  ...
  userData*: ConversionEventsUserDataProperties ## Object containing customer information data. Note, It is required at least one of 1) em, 2) hashed_maids or 3) pair client_ip_address + client_user_agent.
  wifi*: Option[bool] ## Whether the event occurred when the user device was connected to wifi.


# Custom JSON deserialization for ConversionEventsDataItems with custom field names
proc to*(node: JsonNode, T: typedesc[ConversionEventsDataItems]): ConversionEventsDataItems =
  result = ConversionEventsDataItems()
  if node.kind == JObject:
    ...
    if node.hasKey("user_data"):
      result.userData = to(node["user_data"], ConversionEventsUserDataProperties)
    if node.hasKey("wifi") and node["wifi"].kind != JNull:
      result.wifi = some(to(node["wifi"], typeof(result.wifi.get())))

# Custom JSON serialization for ConversionEventsDataItems with custom field names
proc `%`*(obj: ConversionEventsDataItems): JsonNode =
  result = newJObject()
