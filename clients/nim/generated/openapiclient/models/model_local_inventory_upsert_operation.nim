...
type LocalInventoryUpsertOperation* = object
...
# Custom JSON deserialization for LocalInventoryUpsertOperation with custom field names
proc to*(node: JsonNode, T: typedesc[LocalInventoryUpsertOperation]): LocalInventoryUpsertOperation =
  result = LocalInventoryUpsertOperation()
  if node.kind == JObject:
    if node.hasKey("attributes"):
      result.attributes = to(node["attributes"], RetailLocalInventoryItemAttributes)
...
# Custom JSON serialization for LocalInventoryUpsertOperation with custom field names
proc `%`*(obj: LocalInventoryUpsertOperation): JsonNode =
  result = newJObject()
