...
import model_catalogs_retail_batch_request_items_items
...
type CatalogsRetailBatchRequest* = object
  ...
  country*: Country
  items*: seq[CatalogsRetailBatchRequestItemsItems] ## Array with catalogs item operations
...
# Custom JSON deserialization for CatalogsRetailBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsRetailBatchRequest]): CatalogsRetailBatchRequest =
  result = CatalogsRetailBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, CatalogsRetailBatchRequestItemsItems))
    if node.hasKey("language"):
      result.language = to(node["language"], Language)

# Custom JSON serialization for CatalogsRetailBatchRequest with custom field names
proc `%`*(obj: CatalogsRetailBatchRequest): JsonNode =
  result = newJObject()
