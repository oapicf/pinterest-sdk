...
# Custom JSON deserialization for CatalogsCreativeAssetsBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsCreativeAssetsBatchRequest]): CatalogsCreativeAssetsBatchRequest =
  result = CatalogsCreativeAssetsBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, CatalogsCreativeAssetsBatchItem))
    if node.hasKey("language"):
      result.language = to(node["language"], Language)

# Custom JSON serialization for CatalogsCreativeAssetsBatchRequest with custom field names
proc `%`*(obj: CatalogsCreativeAssetsBatchRequest): JsonNode =
  result = newJObject()
