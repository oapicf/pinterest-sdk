...
type LeadFormResponse* = object
  ...
  createdTime*: Option[int] ## Lead form creation time. Unix timestamp in seconds.
  id*: Option[string] ## The ID of this lead form
  updatedTime*: Option[int] ## Last update time. Unix timestamp in seconds.


# Custom JSON deserialization for LeadFormResponse with custom field names
proc to*(node: JsonNode, T: typedesc[LeadFormResponse]): LeadFormResponse =
  result = LeadFormResponse()
...
# Custom JSON serialization for LeadFormResponse with custom field names
proc `%`*(obj: LeadFormResponse): JsonNode =
  result = newJObject()
