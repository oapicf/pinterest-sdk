...
type LeadFormCreateRequest* = object
  ## 
...
# Custom JSON deserialization for LeadFormCreateRequest with custom field names
proc to*(node: JsonNode, T: typedesc[LeadFormCreateRequest]): LeadFormCreateRequest =
  result = LeadFormCreateRequest()
...
# Custom JSON serialization for LeadFormCreateRequest with custom field names
proc `%`*(obj: LeadFormCreateRequest): JsonNode =
  result = newJObject()
  if obj.name.isSome():
