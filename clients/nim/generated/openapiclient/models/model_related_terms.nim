...
import model_related_terms_related_terms_list_items
...
type RelatedTerms* = object
  ...
  relatedTermsList*: Option[seq[RelatedTermsRelatedTermsListItems]] ## The id of the advertiser.
...
proc to*(node: JsonNode, T: typedesc[RelatedTerms]): RelatedTerms =
  ...
  if node.kind == JObject:
    ...
    if node.hasKey("related_terms_list") and node["related_terms_list"].kind != JNull:
      ...
      let arrayNode = node["related_terms_list"]
      if arrayNode.kind == JArray:
        var arr: seq[RelatedTermsRelatedTermsListItems] = @[]
        for item in arrayNode.items:
          arr.add(to(item, RelatedTermsRelatedTermsListItems))
        result.relatedTermsList = some(arr)
