...
# Custom JSON deserialization for CatalogsItemsDeleteBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsItemsDeleteBatchRequest]): CatalogsItemsDeleteBatchRequest =
  result = CatalogsItemsDeleteBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, ItemDeleteBatchRecord))
...
# Custom JSON serialization for CatalogsItemsDeleteBatchRequest with custom field names
proc `%`*(obj: CatalogsItemsDeleteBatchRequest): JsonNode =
  result = newJObject()
