...
import model_conversion_events_data_items_custom_data_contents_items

type ConversionEventsDataItemsCustomData* = object
  ...
  contentIds*: Option[seq[string]] ## List of products IDs. We recommend using this if you are a merchant for PageVisit, AddToCart and Checkouts. For detail, please check [here](https://help.pinterest.com/en/business/article/before-you-get-started-with-catalogs) (Install the Pinterest tag section).
  contentName*: Option[string] ## The name of the page or product associated with the event.
  contents*: Option[seq[ConversionEventsDataItemsCustomDataContentsItems]] ## A list of objects containing information about products, such as price and quantity. We recommend using this if you are a merchant for PageVisit, AddToCart and Checkouts. For detail, please check [here](https://help.pinterest.com/en/business/article/before-you-get-started-with-catalogs) (Install the Pinterest tag section).
  ...
  externalMeasurementId*: Option[string] ## Only use when instructed.
  externalMeasurementVendorId*: Option[int] ## Only use when instructed.
  ...
  optOutType*: Option[string] ## Flags for different privacy rights laws to opt out users of sharing personal information. Separate values with commas. See the Help Center article about [limited data processing](https://help.pinterest.com/en/business/article/limited-data-processing) and the developer's guide for [tracking conversion events](/docs/track-conversions/track-conversions-in-the-api/#whether-the-user-has-opted-out-of-web-or-offline-conversion-events) for help with using this parameter.
  ...
  predictedLtv*: Option[string] ## Predicted lifetime value of user associated with the event. Accepted as a string in the request; it will be parsed into a double.
...
# Custom JSON deserialization for ConversionEventsDataItemsCustomData with custom field names
proc to*(node: JsonNode, T: typedesc[ConversionEventsDataItemsCustomData]): ConversionEventsDataItemsCustomData =
  result = ConversionEventsDataItemsCustomData()
  if node.kind == JObject:
    ...
    if node.hasKey("contents") and node["contents"].kind != JNull:
      ...
      let arrayNode = node["contents"]
      if arrayNode.kind == JArray:
        var arr: seq[ConversionEventsDataItemsCustomDataContentsItems] = @[]
        for item in arrayNode.items:
          arr.add(to(item, ConversionEventsDataItemsCustomDataContentsItems))
        result.contents = some(arr)
    if node.hasKey("currency") and node["currency"].kind != JNull:
      result.currency = some(to(node["currency"], typeof(result.currency.get())))
    if node.hasKey("external_measurement_id") and node["external_measurement_id"].kind != JNull:
      result.externalMeasurementId = some(to(node["external_measurement_id"], typeof(result.externalMeasurementId.get())))
    if node.hasKey("external_measurement_vendor_id") and node["external_measurement_vendor_id"].kind != JNull:
      result.externalMeasurementVendorId = some(to(node["external_measurement_vendor_id"], typeof(result.externalMeasurementVendorId.get())))
    ...
    if node.hasKey("predicted_ltv") and node["predicted_ltv"].kind != JNull:
      result.predictedLtv = some(to(node["predicted_ltv"], typeof(result.predictedLtv.get())))
...
# Custom JSON serialization for ConversionEventsDataItemsCustomData with custom field names
proc `%`*(obj: ConversionEventsDataItemsCustomData): JsonNode =
  result = newJObject()
  ...
  if obj.externalMeasurementId.isSome():
    result["external_measurement_id"] = %obj.externalMeasurementId.get()
  if obj.externalMeasurementVendorId.isSome():
    result["external_measurement_vendor_id"] = %obj.externalMeasurementVendorId.get()
  if obj.np.isSome():
  ...
  if obj.predictedLtv.isSome():
    result["predicted_ltv"] = %obj.predictedLtv.get()
