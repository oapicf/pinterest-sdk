...
import model_catalogs_hotel_main_image

type CatalogsHotelAttributes* = object
  ## 
  address*: Option[CatalogsHotelAddress] ## Hotel address
  ...
  guestRatings*: Option[CatalogsHotelGuestRatings] ## If specified, you must provide all properties
  ...
  additionalImageLink*: Option[seq[string]] ## <= 2000 characters. The links to additional images for your hotel. Up to ten additional images can be used to show a hotel from different angles. Must begin with http:// or https://.
  aiDisclosures*: Option[seq[CatalogsAiContentDisclosure]] ## AI content disclosures for individual assets (main_image.link or additional_image_link) on this hotel item. Each entry declares which disclosure types apply to a single asset URL.
  mainImage*: Option[CatalogsHotelMainImage] ## The main hotel image
