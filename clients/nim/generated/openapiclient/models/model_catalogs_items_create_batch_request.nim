...
# Custom JSON deserialization for CatalogsItemsCreateBatchRequest with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsItemsCreateBatchRequest]): CatalogsItemsCreateBatchRequest =
  result = CatalogsItemsCreateBatchRequest()
    ...
    if node.hasKey("items"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.items.add(to(item, ItemCreateBatchRecord))
...
# Custom JSON serialization for CatalogsItemsCreateBatchRequest with custom field names
proc `%`*(obj: CatalogsItemsCreateBatchRequest): JsonNode =
  result = newJObject()
