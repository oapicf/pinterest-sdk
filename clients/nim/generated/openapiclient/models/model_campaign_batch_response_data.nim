...
import model_nullable_entity_status
import model_performance_plus_campaign_settings
import model_summary_status
import model_tracking_urls

type CampaignBatchResponseData* = object
  ## Campaign data in batch response, with all fields optional to support error cases where data may be empty.
  adAccountId*: Option[string] ## Campaign's Advertiser ID.
  bidOptions*: Option[CampaignBidOptions] ## [Restricted](/docs/getting-started/using-beta-and-restricted-features/)  Object describing the campaign level bid multipliers.
  ...
  isCarting*: bool ## Whether the campaign contains a carting(where-to-buy link) ad.
...
# Custom JSON deserialization for CampaignBatchResponseData with custom field names
proc to*(node: JsonNode, T: typedesc[CampaignBatchResponseData]): CampaignBatchResponseData =
  result = CampaignBatchResponseData()
  if node.kind == JObject:
    ...
    if node.hasKey("is_carting"):
      result.isCarting = to(node["is_carting"], bool)
...
# Custom JSON serialization for CampaignBatchResponseData with custom field names
proc `%`*(obj: CampaignBatchResponseData): JsonNode =
  result = newJObject()
  ...
  result["is_carting"] = %obj.isCarting
