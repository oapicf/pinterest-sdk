#
# Pinterest REST API
# 
# Pinterest's REST API
# The version of the OpenAPI document: 5.28.0
# Contact: blah+oapicf@cliffano.com
# Generated by: https://openapi-generator.tech
#

import json
import tables
import marshal
import options

import model_catalogs_hotel_report_parameters_report

type CatalogType* {.pure.} = enum
  HOTEL

type CatalogsHotelReportParameters* = object
  ## Parameters for hotel report
  catalogType*: CatalogType
  report*: CatalogsHotelReportParametersReport

func `%`*(v: CatalogType): JsonNode =
  result = case v:
    of CatalogType.HOTEL: %"HOTEL"
func `$`*(v: CatalogType): string =
  result = case v:
    of CatalogType.HOTEL: $("HOTEL")

proc to*(node: JsonNode, T: typedesc[CatalogType]): CatalogType =
  if node.kind != JString:
    raise newException(ValueError, "Expected string for enum CatalogType, got " & $node.kind)
  let strVal = node.getStr()
  case strVal:
  of $("HOTEL"):
    return CatalogType.HOTEL
  else:
    raise newException(ValueError, "Invalid enum value for CatalogType: " & strVal)


# Custom JSON deserialization for CatalogsHotelReportParameters with custom field names
proc to*(node: JsonNode, T: typedesc[CatalogsHotelReportParameters]): CatalogsHotelReportParameters =
  result = CatalogsHotelReportParameters()
  if node.kind == JObject:
    if node.hasKey("catalog_type"):
      result.catalogType = to(node["catalog_type"], CatalogType)
    if node.hasKey("report"):
      result.report = to(node["report"], CatalogsHotelReportParametersReport)

# Custom JSON serialization for CatalogsHotelReportParameters with custom field names
proc `%`*(obj: CatalogsHotelReportParameters): JsonNode =
  result = newJObject()
  result["catalog_type"] = %obj.catalogType
  result["report"] = %obj.report

