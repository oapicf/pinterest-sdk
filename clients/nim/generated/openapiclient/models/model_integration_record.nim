...
proc to*(node: JsonNode, T: typedesc[IntegrationRecord]): IntegrationRecord =
  ...
  if node.kind == JObject:
    ...
    if node.hasKey("connected_user_id") and node["connected_user_id"].kind != JNull:
      result.connectedUserId = some(to(node["connected_user_id"], typeof(result.connectedUserId.get())))
...
proc `%`*(obj: IntegrationRecord): JsonNode =
  ...
  if obj.connectedUserId.isSome():
    result["connected_user_id"] = %obj.connectedUserId.get()
