=begin comment

Pinterest REST API

Pinterest's REST API

The version of the OpenAPI document: 5.28.0
Contact: blah+oapicf@cliffano.com
Generated by: https://openapi-generator.tech

=end comment

=cut

#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# Do not edit the class manually.
# Ref: https://openapi-generator.tech
#
package WWW::OpenAPIClient::Object::Ad;

require 5.6.0;
use strict;
use warnings;
use utf8;
use JSON qw(decode_json);
use Data::Dumper;
use Module::Runtime qw(use_module);
use Log::Any qw($log);
use Date::Parse;
use DateTime;

use WWW::OpenAPIClient::Object::AdCollectionsHeaderType;
use WWW::OpenAPIClient::Object::AdDisapprovalReasons;
use WWW::OpenAPIClient::Object::AdReviewStatus;
use WWW::OpenAPIClient::Object::CartingProduct;
use WWW::OpenAPIClient::Object::CreativeType;
use WWW::OpenAPIClient::Object::CustomizableCTAType;
use WWW::OpenAPIClient::Object::DisclosureType;
use WWW::OpenAPIClient::Object::EntityStatus;
use WWW::OpenAPIClient::Object::GridClickType;
use WWW::OpenAPIClient::Object::PinPromotionSummaryStatus;
use WWW::OpenAPIClient::Object::QuizPinData;
use WWW::OpenAPIClient::Object::TrackingUrls;

use base ("Class::Accessor", "Class::Data::Inheritable");

#
#
#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech). Do not edit the class manually.
# REF: https://openapi-generator.tech
#

=begin comment

Pinterest REST API

Pinterest's REST API

The version of the OpenAPI document: 5.28.0
Contact: blah+oapicf@cliffano.com
Generated by: https://openapi-generator.tech

=end comment

=cut

#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# Do not edit the class manually.
# Ref: https://openapi-generator.tech
#
__PACKAGE__->mk_classdata('attribute_map' => {});
__PACKAGE__->mk_classdata('openapi_types' => {});
__PACKAGE__->mk_classdata('method_documentation' => {});
__PACKAGE__->mk_classdata('class_documentation' => {});

# new plain object
sub new {
    my ($class, %args) = @_;

    my $self = bless {}, $class;

    $self->init(%args);

    return $self;
}

# initialize the object
sub init
{
    my ($self, %args) = @_;

    foreach my $attribute (keys %{$self->attribute_map}) {
        my $args_key = $self->attribute_map->{$attribute};
        $self->$attribute( $args{ $args_key } );
    }
}

# return perl hash
sub to_hash {
    my $self = shift;
    my $_hash = decode_json(JSON->new->convert_blessed->encode($self));

    return $_hash;
}

# used by JSON for serialization
sub TO_JSON {
    my $self = shift;
    my $_data = {};
    foreach my $_key (keys %{$self->attribute_map}) {
        if (defined $self->{$_key}) {
            my $_json_attribute = $self->attribute_map->{$_key};
            my $_type = $self->openapi_types->{$_key};
            my $_value = $self->{$_key};
            if ($_type =~ /^array\[(.+)\]$/i) { # array
                my $_subclass = $1;
                $_data->{$_json_attribute} =  [ map { $self->_to_json_primitives($_subclass, $_) } @$_value ];
            } elsif ($_type =~ /^hash\[string,(.+)\]$/i) { # hash
                my $_subclass = $1;
                my %_hash = ();
                while (my($_key, $_element) = each %{$_value}) {
                    $_hash{$_key} = $self->_to_json_primitives($_subclass, $_element);
                }
                $_data->{$_json_attribute} = \%_hash;
            } elsif ( grep( /^$_type$/, ('int', 'double', 'string', 'boolean', 'DATE', 'DATE_TIME'))) {
                $_data->{$_json_attribute} = $self->_to_json_primitives($_type, $_value);
            } else {
                $_data->{$_json_attribute} = $_value;
            }
        }
    }

    return $_data;
}

# to_json non-array data
sub _to_json_primitives {
    my ($self, $type, $data) = @_;
    if ( grep( /^$type$/, ('int', 'double'))) {
        # https://metacpan.org/pod/JSON#simple-scalars
        # numify it, ensuring it will be dumped as a number
        return undef unless defined $data;
        return $data + 0;
    } elsif ($type eq 'string') {
        # https://metacpan.org/pod/JSON#simple-scalars
        # stringified
        return undef unless defined $data;
        return $data . q();
    } elsif ($type eq 'boolean') {
        # https://metacpan.org/pod/JSON#JSON::true,-JSON::false,-JSON::null
        return $data ? \1 : \0;
    } elsif ($type eq 'DATE') {
        return undef unless defined $data;
        if (ref($data) eq 'DateTime') {
            # https://metacpan.org/pod/DateTime#$dt-%3Eymd($optional_separator),-$dt-%3Emdy(...),-$dt-%3Edmy(...)
            return $data->ymd;
        }
        return $data .q();
    } elsif ($type eq 'DATE_TIME') {
        return undef unless defined $data;
        # the date-time notation as defined by RFC 3339, section 5.6, for example, 2017-07-21T17:32:28Z
        if (ref($data) eq 'DateTime') {
            # https://metacpan.org/pod/DateTime#$dt-%3Erfc3339
            return $data->rfc3339;
        }
        return $data .q();
    } else { # hash (model),  In this case, the TO_JSON of the $data object is executed
        return $data;
    }
}

# from Perl hashref
sub from_hash {
    my ($self, $hash) = @_;

    # loop through attributes and use openapi_types to deserialize the data
    while ( my ($_key, $_type) = each %{$self->openapi_types} ) {
        my $_json_attribute = $self->attribute_map->{$_key};
        if ($_type =~ /^array\[(.+)\]$/i) { # array
            my $_subclass = $1;
            my @_array = ();
            foreach my $_element (@{$hash->{$_json_attribute}}) {
                push @_array, $self->_deserialize($_subclass, $_element);
            }
            $self->{$_key} = \@_array;
        } elsif ($_type =~ /^hash\[string,(.+)\]$/i) { # hash
            my $_subclass = $1;
            my %_hash = ();
            while (my($_key, $_element) = each %{$hash->{$_json_attribute}}) {
                $_hash{$_key} = $self->_deserialize($_subclass, $_element);
            }
            $self->{$_key} = \%_hash;
        } elsif (exists $hash->{$_json_attribute}) { #hash(model), primitive, datetime
            $self->{$_key} = $self->_deserialize($_type, $hash->{$_json_attribute});
        } else {
            $log->debugf("Warning: %s (%s) does not exist in input hash\n", $_key, $_json_attribute);
        }
    }

    return $self;
}

# deserialize non-array data
sub _deserialize {
    my ($self, $type, $data) = @_;
    $log->debugf("deserializing %s with %s",Dumper($data), $type);

    if (grep( /^$type$/ , ('DATE_TIME', 'DATE'))) {
        return DateTime->from_epoch(epoch => str2time($data));
    } elsif ( grep( /^$type$/, ('int', 'double'))) {
        return undef unless defined $data;
        return $data + 0;
    } elsif ($type eq 'string') {
        return undef unless defined $data;
        return $data . q();
    } elsif ($type eq 'boolean') {
        return !!$data;
    } else { # hash(model)
        my $_instance = eval "WWW::OpenAPIClient::Object::$type->new()";
        return $_instance->from_hash($data);
    }
}


__PACKAGE__->class_documentation({description => '',
                                  class => 'Ad',
                                  required => [], # TODO
}                                 );

__PACKAGE__->method_documentation({
    'ad_account_id' => {
        datatype => 'string',
        base_name => 'ad_account_id',
        description => 'The ID of the advertiser that this ad belongs to.',
        format => '',
        read_only => '',
            },
    'ad_group_id' => {
        datatype => 'string',
        base_name => 'ad_group_id',
        description => 'ID of the ad group that contains the ad.',
        format => '',
        read_only => '',
            },
    'android_deep_link' => {
        datatype => 'string',
        base_name => 'android_deep_link',
        description => 'Deep link URL for Android devices.',
        format => '',
        read_only => '',
            },
    'campaign_id' => {
        datatype => 'string',
        base_name => 'campaign_id',
        description => 'ID of the ad campaign that contains this ad.',
        format => '',
        read_only => '',
            },
    'carousel_android_deep_links' => {
        datatype => 'ARRAY[string]',
        base_name => 'carousel_android_deep_links',
        description => 'Comma-separated deep links for the carousel pin on Android.',
        format => '',
        read_only => '',
            },
    'carousel_destination_urls' => {
        datatype => 'ARRAY[string]',
        base_name => 'carousel_destination_urls',
        description => 'Comma-separated destination URLs for the carousel pin to promote.',
        format => '',
        read_only => '',
            },
    'carousel_ios_deep_links' => {
        datatype => 'ARRAY[string]',
        base_name => 'carousel_ios_deep_links',
        description => 'Comma-separated deep links for the carousel pin on iOS.',
        format => '',
        read_only => '',
            },
    'carting_platform_type' => {
        datatype => 'int',
        base_name => 'carting_platform_type',
        description => 'The vendor platform type of the carting/WTB ad.',
        format => '',
        read_only => '',
            },
    'carting_products' => {
        datatype => 'ARRAY[CartingProduct]',
        base_name => 'carting_products',
        description => 'Array of carting/WTB products for the ad.',
        format => '',
        read_only => '',
            },
    'click_tracking_url' => {
        datatype => 'string',
        base_name => 'click_tracking_url',
        description => 'Tracking url for the ad clicks.',
        format => '',
        read_only => '',
            },
    'collection_items_destination_url_template' => {
        datatype => 'string',
        base_name => 'collection_items_destination_url_template',
        description => 'Destination URL template for all items within a collections drawer.',
        format => '',
        read_only => '',
            },
    'collections_header_type' => {
        datatype => 'AdCollectionsHeaderType',
        base_name => 'collections_header_type',
        description => '',
        format => '',
        read_only => '',
            },
    'created_time' => {
        datatype => 'int',
        base_name => 'created_time',
        description => 'Pin creation time. Unix timestamp in seconds.',
        format => '',
        read_only => '',
            },
    'creative_type' => {
        datatype => 'CreativeType',
        base_name => 'creative_type',
        description => '',
        format => '',
        read_only => '',
            },
    'customizable_cta_type' => {
        datatype => 'CustomizableCTAType',
        base_name => 'customizable_cta_type',
        description => '',
        format => '',
        read_only => '',
            },
    'destination_url' => {
        datatype => 'string',
        base_name => 'destination_url',
        description => 'Destination URL.',
        format => '',
        read_only => '',
            },
    'disclosure_type' => {
        datatype => 'DisclosureType',
        base_name => 'disclosure_type',
        description => '',
        format => '',
        read_only => '',
            },
    'disclosure_url' => {
        datatype => 'string',
        base_name => 'disclosure_url',
        description => 'URL for a page that provides disclosures about a pharmaceutical product, such as potential side effects. Make sure the URL takes the user directly to the disclosure content and the referenced site is secure.',
        format => '',
        read_only => '',
            },
    'grid_click_type' => {
        datatype => 'GridClickType',
        base_name => 'grid_click_type',
        description => '',
        format => '',
        read_only => '',
            },
    'id' => {
        datatype => 'string',
        base_name => 'id',
        description => 'The ID of this ad.',
        format => '',
        read_only => '',
            },
    'ios_deep_link' => {
        datatype => 'string',
        base_name => 'ios_deep_link',
        description => 'Deep link URL for iOS devices.',
        format => '',
        read_only => '',
            },
    'is_carting' => {
        datatype => 'boolean',
        base_name => 'is_carting',
        description => 'Is the ad a carting/WTB ad?',
        format => '',
        read_only => '',
            },
    'is_collage_accepted_terms' => {
        datatype => 'boolean',
        base_name => 'is_collage_accepted_terms',
        description => 'Whether the advertiser has accepted the terms and conditions for collage ad.',
        format => '',
        read_only => '',
            },
    'is_collage_single_destination' => {
        datatype => 'boolean',
        base_name => 'is_collage_single_destination',
        description => 'Whether the collage ad has a single destination url override.',
        format => '',
        read_only => '',
            },
    'is_pin_deleted' => {
        datatype => 'boolean',
        base_name => 'is_pin_deleted',
        description => 'Is original pin deleted?',
        format => '',
        read_only => '',
            },
    'is_removable' => {
        datatype => 'boolean',
        base_name => 'is_removable',
        description => 'Is pin repinnable?',
        format => '',
        read_only => '',
            },
    'lead_form_id' => {
        datatype => 'string',
        base_name => 'lead_form_id',
        description => 'Lead form ID for lead ad generation.',
        format => '',
        read_only => '',
            },
    'name' => {
        datatype => 'string',
        base_name => 'name',
        description => 'Name of the ad - 255 chars max.',
        format => '',
        read_only => '',
            },
    'pin_id' => {
        datatype => 'string',
        base_name => 'pin_id',
        description => 'Pin ID. This field may only be updated for draft ads.',
        format => '',
        read_only => '',
            },
    'quiz_pin_data' => {
        datatype => 'QuizPinData',
        base_name => 'quiz_pin_data',
        description => 'Before creating a quiz ad, you must create an organic Pin using POST/Create Pin for each result in the quiz. Quiz ads cannot be saved by a Pinner. Quiz ad results can be saved.',
        format => '',
        read_only => '',
            },
    'rejected_reasons' => {
        datatype => 'ARRAY[AdDisapprovalReasons]',
        base_name => 'rejected_reasons',
        description => 'Enum reason why the pin was rejected. Returned if &#x60;review_status&#x60; is \&quot;REJECTED\&quot;.',
        format => '',
        read_only => '',
            },
    'rejection_labels' => {
        datatype => 'ARRAY[string]',
        base_name => 'rejection_labels',
        description => 'Text reason why the pin was rejected. Returned if &#x60;review_status&#x60; is \&quot;REJECTED\&quot;.',
        format => '',
        read_only => '',
            },
    'review_status' => {
        datatype => 'AdReviewStatus',
        base_name => 'review_status',
        description => 'Ad review status',
        format => '',
        read_only => '',
            },
    'status' => {
        datatype => 'EntityStatus',
        base_name => 'status',
        description => '',
        format => '',
        read_only => '',
            },
    'summary_status' => {
        datatype => 'PinPromotionSummaryStatus',
        base_name => 'summary_status',
        description => 'Ad summary status',
        format => '',
        read_only => '',
            },
    'tracking_urls' => {
        datatype => 'TrackingUrls',
        base_name => 'tracking_urls',
        description => '',
        format => '',
        read_only => '',
            },
    'type' => {
        datatype => 'string',
        base_name => 'type',
        description => 'Always \&quot;ad\&quot;.',
        format => '',
        read_only => '',
            },
    'updated_time' => {
        datatype => 'int',
        base_name => 'updated_time',
        description => 'Last update time. Unix timestamp in seconds.',
        format => '',
        read_only => '',
            },
    'view_tracking_url' => {
        datatype => 'string',
        base_name => 'view_tracking_url',
        description => 'Tracking URL for ad impressions.',
        format => '',
        read_only => '',
            },
});

__PACKAGE__->openapi_types( {
    'ad_account_id' => 'string',
    'ad_group_id' => 'string',
    'android_deep_link' => 'string',
    'campaign_id' => 'string',
    'carousel_android_deep_links' => 'ARRAY[string]',
    'carousel_destination_urls' => 'ARRAY[string]',
    'carousel_ios_deep_links' => 'ARRAY[string]',
    'carting_platform_type' => 'int',
    'carting_products' => 'ARRAY[CartingProduct]',
    'click_tracking_url' => 'string',
    'collection_items_destination_url_template' => 'string',
    'collections_header_type' => 'AdCollectionsHeaderType',
    'created_time' => 'int',
    'creative_type' => 'CreativeType',
    'customizable_cta_type' => 'CustomizableCTAType',
    'destination_url' => 'string',
    'disclosure_type' => 'DisclosureType',
    'disclosure_url' => 'string',
    'grid_click_type' => 'GridClickType',
    'id' => 'string',
    'ios_deep_link' => 'string',
    'is_carting' => 'boolean',
    'is_collage_accepted_terms' => 'boolean',
    'is_collage_single_destination' => 'boolean',
    'is_pin_deleted' => 'boolean',
    'is_removable' => 'boolean',
    'lead_form_id' => 'string',
    'name' => 'string',
    'pin_id' => 'string',
    'quiz_pin_data' => 'QuizPinData',
    'rejected_reasons' => 'ARRAY[AdDisapprovalReasons]',
    'rejection_labels' => 'ARRAY[string]',
    'review_status' => 'AdReviewStatus',
    'status' => 'EntityStatus',
    'summary_status' => 'PinPromotionSummaryStatus',
    'tracking_urls' => 'TrackingUrls',
    'type' => 'string',
    'updated_time' => 'int',
    'view_tracking_url' => 'string'
} );

__PACKAGE__->attribute_map( {
    'ad_account_id' => 'ad_account_id',
    'ad_group_id' => 'ad_group_id',
    'android_deep_link' => 'android_deep_link',
    'campaign_id' => 'campaign_id',
    'carousel_android_deep_links' => 'carousel_android_deep_links',
    'carousel_destination_urls' => 'carousel_destination_urls',
    'carousel_ios_deep_links' => 'carousel_ios_deep_links',
    'carting_platform_type' => 'carting_platform_type',
    'carting_products' => 'carting_products',
    'click_tracking_url' => 'click_tracking_url',
    'collection_items_destination_url_template' => 'collection_items_destination_url_template',
    'collections_header_type' => 'collections_header_type',
    'created_time' => 'created_time',
    'creative_type' => 'creative_type',
    'customizable_cta_type' => 'customizable_cta_type',
    'destination_url' => 'destination_url',
    'disclosure_type' => 'disclosure_type',
    'disclosure_url' => 'disclosure_url',
    'grid_click_type' => 'grid_click_type',
    'id' => 'id',
    'ios_deep_link' => 'ios_deep_link',
    'is_carting' => 'is_carting',
    'is_collage_accepted_terms' => 'is_collage_accepted_terms',
    'is_collage_single_destination' => 'is_collage_single_destination',
    'is_pin_deleted' => 'is_pin_deleted',
    'is_removable' => 'is_removable',
    'lead_form_id' => 'lead_form_id',
    'name' => 'name',
    'pin_id' => 'pin_id',
    'quiz_pin_data' => 'quiz_pin_data',
    'rejected_reasons' => 'rejected_reasons',
    'rejection_labels' => 'rejection_labels',
    'review_status' => 'review_status',
    'status' => 'status',
    'summary_status' => 'summary_status',
    'tracking_urls' => 'tracking_urls',
    'type' => 'type',
    'updated_time' => 'updated_time',
    'view_tracking_url' => 'view_tracking_url'
} );

__PACKAGE__->mk_accessors(keys %{__PACKAGE__->attribute_map});


1;
