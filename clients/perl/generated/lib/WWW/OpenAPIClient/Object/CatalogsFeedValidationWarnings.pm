=begin comment

Pinterest REST API

Pinterest's REST API

The version of the OpenAPI document: 5.28.0
Contact: blah+oapicf@cliffano.com
Generated by: https://openapi-generator.tech

=end comment

=cut

#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# Do not edit the class manually.
# Ref: https://openapi-generator.tech
#
package WWW::OpenAPIClient::Object::CatalogsFeedValidationWarnings;

require 5.6.0;
use strict;
use warnings;
use utf8;
use JSON qw(decode_json);
use Data::Dumper;
use Module::Runtime qw(use_module);
use Log::Any qw($log);
use Date::Parse;
use DateTime;


use base ("Class::Accessor", "Class::Data::Inheritable");

#
#
#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech). Do not edit the class manually.
# REF: https://openapi-generator.tech
#

=begin comment

Pinterest REST API

Pinterest's REST API

The version of the OpenAPI document: 5.28.0
Contact: blah+oapicf@cliffano.com
Generated by: https://openapi-generator.tech

=end comment

=cut

#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# Do not edit the class manually.
# Ref: https://openapi-generator.tech
#
__PACKAGE__->mk_classdata('attribute_map' => {});
__PACKAGE__->mk_classdata('openapi_types' => {});
__PACKAGE__->mk_classdata('method_documentation' => {});
__PACKAGE__->mk_classdata('class_documentation' => {});

# new plain object
sub new {
    my ($class, %args) = @_;

    my $self = bless {}, $class;

    $self->init(%args);

    return $self;
}

# initialize the object
sub init
{
    my ($self, %args) = @_;

    foreach my $attribute (keys %{$self->attribute_map}) {
        my $args_key = $self->attribute_map->{$attribute};
        $self->$attribute( $args{ $args_key } );
    }
}

# return perl hash
sub to_hash {
    my $self = shift;
    my $_hash = decode_json(JSON->new->convert_blessed->encode($self));

    return $_hash;
}

# used by JSON for serialization
sub TO_JSON {
    my $self = shift;
    my $_data = {};
    foreach my $_key (keys %{$self->attribute_map}) {
        if (defined $self->{$_key}) {
            my $_json_attribute = $self->attribute_map->{$_key};
            my $_type = $self->openapi_types->{$_key};
            my $_value = $self->{$_key};
            if ($_type =~ /^array\[(.+)\]$/i) { # array
                my $_subclass = $1;
                $_data->{$_json_attribute} =  [ map { $self->_to_json_primitives($_subclass, $_) } @$_value ];
            } elsif ($_type =~ /^hash\[string,(.+)\]$/i) { # hash
                my $_subclass = $1;
                my %_hash = ();
                while (my($_key, $_element) = each %{$_value}) {
                    $_hash{$_key} = $self->_to_json_primitives($_subclass, $_element);
                }
                $_data->{$_json_attribute} = \%_hash;
            } elsif ( grep( /^$_type$/, ('int', 'double', 'string', 'boolean', 'DATE', 'DATE_TIME'))) {
                $_data->{$_json_attribute} = $self->_to_json_primitives($_type, $_value);
            } else {
                $_data->{$_json_attribute} = $_value;
            }
        }
    }

    return $_data;
}

# to_json non-array data
sub _to_json_primitives {
    my ($self, $type, $data) = @_;
    if ( grep( /^$type$/, ('int', 'double'))) {
        # https://metacpan.org/pod/JSON#simple-scalars
        # numify it, ensuring it will be dumped as a number
        return undef unless defined $data;
        return $data + 0;
    } elsif ($type eq 'string') {
        # https://metacpan.org/pod/JSON#simple-scalars
        # stringified
        return undef unless defined $data;
        return $data . q();
    } elsif ($type eq 'boolean') {
        # https://metacpan.org/pod/JSON#JSON::true,-JSON::false,-JSON::null
        return $data ? \1 : \0;
    } elsif ($type eq 'DATE') {
        return undef unless defined $data;
        if (ref($data) eq 'DateTime') {
            # https://metacpan.org/pod/DateTime#$dt-%3Eymd($optional_separator),-$dt-%3Emdy(...),-$dt-%3Edmy(...)
            return $data->ymd;
        }
        return $data .q();
    } elsif ($type eq 'DATE_TIME') {
        return undef unless defined $data;
        # the date-time notation as defined by RFC 3339, section 5.6, for example, 2017-07-21T17:32:28Z
        if (ref($data) eq 'DateTime') {
            # https://metacpan.org/pod/DateTime#$dt-%3Erfc3339
            return $data->rfc3339;
        }
        return $data .q();
    } else { # hash (model),  In this case, the TO_JSON of the $data object is executed
        return $data;
    }
}

# from Perl hashref
sub from_hash {
    my ($self, $hash) = @_;

    # loop through attributes and use openapi_types to deserialize the data
    while ( my ($_key, $_type) = each %{$self->openapi_types} ) {
        my $_json_attribute = $self->attribute_map->{$_key};
        if ($_type =~ /^array\[(.+)\]$/i) { # array
            my $_subclass = $1;
            my @_array = ();
            foreach my $_element (@{$hash->{$_json_attribute}}) {
                push @_array, $self->_deserialize($_subclass, $_element);
            }
            $self->{$_key} = \@_array;
        } elsif ($_type =~ /^hash\[string,(.+)\]$/i) { # hash
            my $_subclass = $1;
            my %_hash = ();
            while (my($_key, $_element) = each %{$hash->{$_json_attribute}}) {
                $_hash{$_key} = $self->_deserialize($_subclass, $_element);
            }
            $self->{$_key} = \%_hash;
        } elsif (exists $hash->{$_json_attribute}) { #hash(model), primitive, datetime
            $self->{$_key} = $self->_deserialize($_type, $hash->{$_json_attribute});
        } else {
            $log->debugf("Warning: %s (%s) does not exist in input hash\n", $_key, $_json_attribute);
        }
    }

    return $self;
}

# deserialize non-array data
sub _deserialize {
    my ($self, $type, $data) = @_;
    $log->debugf("deserializing %s with %s",Dumper($data), $type);

    if (grep( /^$type$/ , ('DATE_TIME', 'DATE'))) {
        return DateTime->from_epoch(epoch => str2time($data));
    } elsif ( grep( /^$type$/, ('int', 'double'))) {
        return undef unless defined $data;
        return $data + 0;
    } elsif ($type eq 'string') {
        return undef unless defined $data;
        return $data . q();
    } elsif ($type eq 'boolean') {
        return !!$data;
    } else { # hash(model)
        my $_instance = eval "WWW::OpenAPIClient::Object::$type->new()";
        return $_instance->from_hash($data);
    }
}


__PACKAGE__->class_documentation({description => '',
                                  class => 'CatalogsFeedValidationWarnings',
                                  required => [], # TODO
}                                 );

__PACKAGE__->method_documentation({
    'ad_image_0_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_LINK_DUPLICATED',
        description => 'ad_image_0_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_0_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 0 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_0_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_LINK_REQUIRED',
        description => 'Ad image link 0 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_0_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_LINK_WARNING',
        description => 'Ad image link 0 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_0_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_TAG_DUPLICATED',
        description => 'ad_image_0_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_0_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 0 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_0_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_0_TAG_REQUIRED',
        description => 'Ad image tag 0 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_10_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_LINK_DUPLICATED',
        description => 'ad_image_10_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_10_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 10 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_10_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_LINK_REQUIRED',
        description => 'Ad image link 10 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_10_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_LINK_WARNING',
        description => 'Ad image link 10 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_10_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_TAG_DUPLICATED',
        description => 'ad_image_10_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_10_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 10 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_10_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_10_TAG_REQUIRED',
        description => 'Ad image tag 10 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_11_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_LINK_DUPLICATED',
        description => 'ad_image_11_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_11_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 11 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_11_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_LINK_REQUIRED',
        description => 'Ad image link 11 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_11_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_LINK_WARNING',
        description => 'Ad image link 11 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_11_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_TAG_DUPLICATED',
        description => 'ad_image_11_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_11_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 11 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_11_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_11_TAG_REQUIRED',
        description => 'Ad image tag 11 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_12_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_LINK_DUPLICATED',
        description => 'ad_image_12_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_12_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 12 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_12_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_LINK_REQUIRED',
        description => 'Ad image link 12 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_12_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_LINK_WARNING',
        description => 'Ad image link 12 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_12_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_TAG_DUPLICATED',
        description => 'ad_image_12_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_12_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 12 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_12_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_12_TAG_REQUIRED',
        description => 'Ad image tag 12 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_13_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_LINK_DUPLICATED',
        description => 'ad_image_13_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_13_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 13 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_13_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_LINK_REQUIRED',
        description => 'Ad image link 13 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_13_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_LINK_WARNING',
        description => 'Ad image link 13 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_13_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_TAG_DUPLICATED',
        description => 'ad_image_13_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_13_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 13 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_13_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_13_TAG_REQUIRED',
        description => 'Ad image tag 13 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_14_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_LINK_DUPLICATED',
        description => 'ad_image_14_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_14_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 14 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_14_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_LINK_REQUIRED',
        description => 'Ad image link 14 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_14_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_LINK_WARNING',
        description => 'Ad image link 14 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_14_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_TAG_DUPLICATED',
        description => 'ad_image_14_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_14_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 14 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_14_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_14_TAG_REQUIRED',
        description => 'Ad image tag 14 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_15_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_LINK_DUPLICATED',
        description => 'ad_image_15_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_15_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 15 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_15_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_LINK_REQUIRED',
        description => 'Ad image link 15 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_15_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_LINK_WARNING',
        description => 'Ad image link 15 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_15_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_TAG_DUPLICATED',
        description => 'ad_image_15_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_15_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 15 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_15_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_15_TAG_REQUIRED',
        description => 'Ad image tag 15 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_16_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_LINK_DUPLICATED',
        description => 'ad_image_16_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_16_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 16 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_16_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_LINK_REQUIRED',
        description => 'Ad image link 16 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_16_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_LINK_WARNING',
        description => 'Ad image link 16 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_16_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_TAG_DUPLICATED',
        description => 'ad_image_16_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_16_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 16 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_16_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_16_TAG_REQUIRED',
        description => 'Ad image tag 16 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_17_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_LINK_DUPLICATED',
        description => 'ad_image_17_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_17_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 17 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_17_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_LINK_REQUIRED',
        description => 'Ad image link 17 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_17_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_LINK_WARNING',
        description => 'Ad image link 17 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_17_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_TAG_DUPLICATED',
        description => 'ad_image_17_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_17_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 17 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_17_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_17_TAG_REQUIRED',
        description => 'Ad image tag 17 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_18_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_LINK_DUPLICATED',
        description => 'ad_image_18_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_18_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 18 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_18_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_LINK_REQUIRED',
        description => 'Ad image link 18 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_18_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_LINK_WARNING',
        description => 'Ad image link 18 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_18_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_TAG_DUPLICATED',
        description => 'ad_image_18_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_18_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 18 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_18_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_18_TAG_REQUIRED',
        description => 'Ad image tag 18 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_19_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_LINK_DUPLICATED',
        description => 'ad_image_19_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_19_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 19 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_19_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_LINK_REQUIRED',
        description => 'Ad image link 19 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_19_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_LINK_WARNING',
        description => 'Ad image link 19 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_19_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_TAG_DUPLICATED',
        description => 'ad_image_19_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_19_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 19 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_19_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_19_TAG_REQUIRED',
        description => 'Ad image tag 19 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_1_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_LINK_DUPLICATED',
        description => 'ad_image_1_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_1_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 1 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_1_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_LINK_REQUIRED',
        description => 'Ad image link 1 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_1_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_LINK_WARNING',
        description => 'Ad image link 1 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_1_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_TAG_DUPLICATED',
        description => 'ad_image_1_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_1_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 1 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_1_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_1_TAG_REQUIRED',
        description => 'Ad image tag 1 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_2_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_LINK_DUPLICATED',
        description => 'ad_image_2_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_2_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 2 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_2_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_LINK_REQUIRED',
        description => 'Ad image link 2 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_2_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_LINK_WARNING',
        description => 'Ad image link 2 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_2_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_TAG_DUPLICATED',
        description => 'ad_image_2_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_2_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 2 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_2_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_2_TAG_REQUIRED',
        description => 'Ad image tag 2 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_3_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_LINK_DUPLICATED',
        description => 'ad_image_3_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_3_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 3 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_3_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_LINK_REQUIRED',
        description => 'Ad image link 3 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_3_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_LINK_WARNING',
        description => 'Ad image link 3 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_3_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_TAG_DUPLICATED',
        description => 'ad_image_3_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_3_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 3 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_3_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_3_TAG_REQUIRED',
        description => 'Ad image tag 3 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_4_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_LINK_DUPLICATED',
        description => 'ad_image_4_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_4_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 4 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_4_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_LINK_REQUIRED',
        description => 'Ad image link 4 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_4_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_LINK_WARNING',
        description => 'Ad image link 4 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_4_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_TAG_DUPLICATED',
        description => 'ad_image_4_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_4_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 4 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_4_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_4_TAG_REQUIRED',
        description => 'Ad image tag 4 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_5_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_LINK_DUPLICATED',
        description => 'ad_image_5_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_5_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 5 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_5_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_LINK_REQUIRED',
        description => 'Ad image link 5 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_5_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_LINK_WARNING',
        description => 'Ad image link 5 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_5_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_TAG_DUPLICATED',
        description => 'ad_image_5_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_5_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 5 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_5_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_5_TAG_REQUIRED',
        description => 'Ad image tag 5 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_6_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_LINK_DUPLICATED',
        description => 'ad_image_6_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_6_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 6 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_6_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_LINK_REQUIRED',
        description => 'Ad image link 6 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_6_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_LINK_WARNING',
        description => 'Ad image link 6 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_6_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_TAG_DUPLICATED',
        description => 'ad_image_6_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_6_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 6 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_6_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_6_TAG_REQUIRED',
        description => 'Ad image tag 6 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_7_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_LINK_DUPLICATED',
        description => 'ad_image_7_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_7_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 7 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_7_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_LINK_REQUIRED',
        description => 'Ad image link 7 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_7_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_LINK_WARNING',
        description => 'Ad image link 7 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_7_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_TAG_DUPLICATED',
        description => 'ad_image_7_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_7_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 7 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_7_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_7_TAG_REQUIRED',
        description => 'Ad image tag 7 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_8_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_LINK_DUPLICATED',
        description => 'ad_image_8_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_8_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 8 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_8_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_LINK_REQUIRED',
        description => 'Ad image link 8 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_8_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_LINK_WARNING',
        description => 'Ad image link 8 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_8_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_TAG_DUPLICATED',
        description => 'ad_image_8_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_8_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 8 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_8_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_8_TAG_REQUIRED',
        description => 'Ad image tag 8 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_9_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_LINK_DUPLICATED',
        description => 'ad_image_9_link is duplicated with another ad image link.',
        format => '',
        read_only => '',
            },
    'ad_image_9_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_LINK_LENGTH_TOO_LONG',
        description => 'Ad image link 9 length is too long. The maximum length is 2047 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_9_link_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_LINK_REQUIRED',
        description => 'Ad image link 9 is required because an image tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_image_9_link_warning' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_LINK_WARNING',
        description => 'Ad image link 9 format is unsupported.',
        format => '',
        read_only => '',
            },
    'ad_image_9_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_TAG_DUPLICATED',
        description => 'ad_image_9_tag is duplicated with another ad image tag.',
        format => '',
        read_only => '',
            },
    'ad_image_9_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_TAG_LENGTH_TOO_LONG',
        description => 'Ad image tag 9 length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_image_9_tag_required' => {
        datatype => 'int',
        base_name => 'AD_IMAGE_9_TAG_REQUIRED',
        description => 'Ad image tag 9 is required because an image link was provided.',
        format => '',
        read_only => '',
            },
    'ad_link_format_warning' => {
        datatype => 'int',
        base_name => 'AD_LINK_FORMAT_WARNING',
        description => 'Some items have ad links that are formatted incorrectly.',
        format => '',
        read_only => '',
            },
    'ad_link_same_as_link' => {
        datatype => 'int',
        base_name => 'AD_LINK_SAME_AS_LINK',
        description => 'Some items have ad link URLs that are duplicates of the link URLs for those items.',
        format => '',
        read_only => '',
            },
    'ad_video_0_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_LINK_DUPLICATED',
        description => 'ad_video_0_link is duplicated with another ad video link.',
        format => '',
        read_only => '',
            },
    'ad_video_0_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_LINK_LENGTH_TOO_LONG',
        description => 'ad_video_0_link length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_0_link_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_LINK_REQUIRED',
        description => 'ad_video_0_link is required for this item because ad_video_0_tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_video_0_link_warning' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_LINK_WARNING',
        description => 'ad_video_0_link is formatted incorrectly and will not be published with your items.',
        format => '',
        read_only => '',
            },
    'ad_video_0_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_TAG_DUPLICATED',
        description => 'ad_video_0_tag is duplicated with another ad video tag.',
        format => '',
        read_only => '',
            },
    'ad_video_0_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_TAG_LENGTH_TOO_LONG',
        description => 'ad_video_0_tag length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_0_tag_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_0_TAG_REQUIRED',
        description => 'ad_video_0_tag is required because ad_video_0_link was provided.',
        format => '',
        read_only => '',
            },
    'ad_video_1_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_LINK_DUPLICATED',
        description => 'ad_video_1_link is duplicated with another ad video link.',
        format => '',
        read_only => '',
            },
    'ad_video_1_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_LINK_LENGTH_TOO_LONG',
        description => 'ad_video_1_link length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_1_link_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_LINK_REQUIRED',
        description => 'ad_video_1_link is required for this item because ad_video_1_tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_video_1_link_warning' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_LINK_WARNING',
        description => 'ad_video_1_link is formatted incorrectly and will not be published with your items.',
        format => '',
        read_only => '',
            },
    'ad_video_1_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_TAG_DUPLICATED',
        description => 'ad_video_1_tag is duplicated with another ad video tag.',
        format => '',
        read_only => '',
            },
    'ad_video_1_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_TAG_LENGTH_TOO_LONG',
        description => 'ad_video_1_tag length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_1_tag_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_1_TAG_REQUIRED',
        description => 'ad_video_1_tag is required because ad_video_1_link was provided.',
        format => '',
        read_only => '',
            },
    'ad_video_2_link_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_LINK_DUPLICATED',
        description => 'ad_video_2_link is duplicated with another ad video link.',
        format => '',
        read_only => '',
            },
    'ad_video_2_link_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_LINK_LENGTH_TOO_LONG',
        description => 'ad_video_2_link length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_2_link_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_LINK_REQUIRED',
        description => 'ad_video_2_link is required for this item because ad_video_2_tag was provided.',
        format => '',
        read_only => '',
            },
    'ad_video_2_link_warning' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_LINK_WARNING',
        description => 'ad_video_2_link is formatted incorrectly and will not be published with your items.',
        format => '',
        read_only => '',
            },
    'ad_video_2_tag_duplicated' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_TAG_DUPLICATED',
        description => 'ad_video_2_tag is duplicated with another ad video tag.',
        format => '',
        read_only => '',
            },
    'ad_video_2_tag_length_too_long' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_TAG_LENGTH_TOO_LONG',
        description => 'ad_video_2_tag length is too long. The maximum length is 511 characters.',
        format => '',
        read_only => '',
            },
    'ad_video_2_tag_required' => {
        datatype => 'int',
        base_name => 'AD_VIDEO_2_TAG_REQUIRED',
        description => 'ad_video_2_tag is required because ad_video_2_link was provided.',
        format => '',
        read_only => '',
            },
    'additional_image_link_length_too_long' => {
        datatype => 'int',
        base_name => 'ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG',
        description => 'Some items have additional_image_link URLs that contain too many characters, so those items will not be published.',
        format => '',
        read_only => '',
            },
    'additional_image_link_warning' => {
        datatype => 'int',
        base_name => 'ADDITIONAL_IMAGE_LINK_WARNING',
        description => 'Some items have additional_image_link URLs that are formatted incorrectly and will not be published with your items.',
        format => '',
        read_only => '',
            },
    'adwords_format_warning' => {
        datatype => 'int',
        base_name => 'ADWORDS_FORMAT_WARNING',
        description => 'Some items have adwords_redirect links that are formatted incorrectly.',
        format => '',
        read_only => '',
            },
    'adwords_same_as_link' => {
        datatype => 'int',
        base_name => 'ADWORDS_SAME_AS_LINK',
        description => 'Some items have adwords_redirect URLs that are duplicates of the link URLs for those items.',
        format => '',
        read_only => '',
            },
    'age_group_invalid' => {
        datatype => 'int',
        base_name => 'AGE_GROUP_INVALID',
        description => 'Some items have age group values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'android_deep_link_invalid' => {
        datatype => 'int',
        base_name => 'ANDROID_DEEP_LINK_INVALID',
        description => 'Some items include invalid android_deep_link.',
        format => '',
        read_only => '',
            },
    'availability_date_invalid' => {
        datatype => 'int',
        base_name => 'AVAILABILITY_DATE_INVALID',
        description => 'Some items have availability_date values that are formatted incorrectly, those items will be published without an availability date.',
        format => '',
        read_only => '',
            },
    'country_does_not_map_to_currency' => {
        datatype => 'int',
        base_name => 'COUNTRY_DOES_NOT_MAP_TO_CURRENCY',
        description => 'Some items include a currency that doesn&#39;t match the usual currency for the location where that product is sold or shipped.',
        format => '',
        read_only => '',
            },
    'custom_label_length_too_long' => {
        datatype => 'int',
        base_name => 'CUSTOM_LABEL_LENGTH_TOO_LONG',
        description => 'Some items have custom_label values that are too long, those items will be published without that custom label.',
        format => '',
        read_only => '',
            },
    'description_length_too_long' => {
        datatype => 'int',
        base_name => 'DESCRIPTION_LENGTH_TOO_LONG',
        description => 'The description for some items were truncated because they contain too many characters.',
        format => '',
        read_only => '',
            },
    'duplicate_headers' => {
        datatype => 'int',
        base_name => 'DUPLICATE_HEADERS',
        description => 'Your feed contains duplicate headers.',
        format => '',
        read_only => '',
            },
    'expiration_date_invalid' => {
        datatype => 'int',
        base_name => 'EXPIRATION_DATE_INVALID',
        description => 'Some items have expiration_date values that are formatted incorrectly, those items will be published without an expiration date.',
        format => '',
        read_only => '',
            },
    'fetch_same_signature' => {
        datatype => 'int',
        base_name => 'FETCH_SAME_SIGNATURE',
        description => 'Ingestion completed early because there are no changes to your feed since the last successful update.',
        format => '',
        read_only => '',
            },
    'gender_invalid' => {
        datatype => 'int',
        base_name => 'GENDER_INVALID',
        description => 'Some items have gender values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'gtin_invalid' => {
        datatype => 'int',
        base_name => 'GTIN_INVALID',
        description => 'Some items include incorrectly formatted GTINs.',
        format => '',
        read_only => '',
            },
    'image_link_warning' => {
        datatype => 'int',
        base_name => 'IMAGE_LINK_WARNING',
        description => 'Some items have image_link URLs that are formatted incorrectly and will not be published with those items.',
        format => '',
        read_only => '',
            },
    'inconsistent_currency_values' => {
        datatype => 'int',
        base_name => 'INCONSISTENT_CURRENCY_VALUES',
        description => 'Some items include inconsistent currencies in price fields.',
        format => '',
        read_only => '',
            },
    'indexed_product_count_large_delta' => {
        datatype => 'int',
        base_name => 'INDEXED_PRODUCT_COUNT_LARGE_DELTA',
        description => 'The product count has increased or decreased significantly compared to the last successful ingestion.',
        format => '',
        read_only => '',
            },
    'ios_deep_link_invalid' => {
        datatype => 'int',
        base_name => 'IOS_DEEP_LINK_INVALID',
        description => 'Some items include invalid ios_deep_link values.',
        format => '',
        read_only => '',
            },
    'is_bundle_invalid' => {
        datatype => 'int',
        base_name => 'IS_BUNDLE_INVALID',
        description => 'Some items have is_bundle values that are formatted incorrectly, those items will be published without being bundled with other products.',
        format => '',
        read_only => '',
            },
    'item_additional_image_download_failure' => {
        datatype => 'int',
        base_name => 'ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE',
        description => 'Some items include additional_image_links that can&#39;t be found.',
        format => '',
        read_only => '',
            },
    'link_format_warning' => {
        datatype => 'int',
        base_name => 'LINK_FORMAT_WARNING',
        description => 'Some items have an invalid product link which contains invalid UTM tracking paramaters.',
        format => '',
        read_only => '',
            },
    'min_ad_price_invalid' => {
        datatype => 'int',
        base_name => 'MIN_AD_PRICE_INVALID',
        description => 'Some items include min_ad_price values that are formatted incorrectly.',
        format => '',
        read_only => '',
            },
    'mpn_invalid' => {
        datatype => 'int',
        base_name => 'MPN_INVALID',
        description => 'Some items include incorrectly formatted MPNs.',
        format => '',
        read_only => '',
            },
    'multipack_invalid' => {
        datatype => 'int',
        base_name => 'MULTIPACK_INVALID',
        description => 'Some items have invalid multipack values.',
        format => '',
        read_only => '',
            },
    'optional_condition_invalid' => {
        datatype => 'int',
        base_name => 'OPTIONAL_CONDITION_INVALID',
        description => 'Some items include condition values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'optional_condition_missing' => {
        datatype => 'int',
        base_name => 'OPTIONAL_CONDITION_MISSING',
        description => 'Some items are missing a condition value, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'optional_product_category_invalid' => {
        datatype => 'int',
        base_name => 'OPTIONAL_PRODUCT_CATEGORY_INVALID',
        description => 'Some items include google_product_category values that are not formatted correctly according to the GPC taxonomy.',
        format => '',
        read_only => '',
            },
    'optional_product_category_missing' => {
        datatype => 'int',
        base_name => 'OPTIONAL_PRODUCT_CATEGORY_MISSING',
        description => 'Some items are missing a google_product_category.',
        format => '',
        read_only => '',
            },
    'product_category_depth_warning' => {
        datatype => 'int',
        base_name => 'PRODUCT_CATEGORY_DEPTH_WARNING',
        description => 'Some items only have 1 or 2 levels of google_product_category values, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'product_type_length_too_long' => {
        datatype => 'int',
        base_name => 'PRODUCT_TYPE_LENGTH_TOO_LONG',
        description => 'Some items have product_type values that are too long, those items will be published without that product type.',
        format => '',
        read_only => '',
            },
    'sale_date_invalid' => {
        datatype => 'int',
        base_name => 'SALE_DATE_INVALID',
        description => 'Some items have sale_price_effective_date values that are formatted incorrectly, those items will be published without a sale date.',
        format => '',
        read_only => '',
            },
    'sales_price_invalid' => {
        datatype => 'int',
        base_name => 'SALES_PRICE_INVALID',
        description => 'Some items have sale price values that are higher than the original price of the item.',
        format => '',
        read_only => '',
            },
    'sales_price_too_high' => {
        datatype => 'int',
        base_name => 'SALES_PRICE_TOO_HIGH',
        description => 'Some items include a sales price that is higher than the list price. The sales price has been defaulted to the list price.',
        format => '',
        read_only => '',
            },
    'sales_price_too_low' => {
        datatype => 'int',
        base_name => 'SALES_PRICE_TOO_LOW',
        description => 'Some items include sales price that is much lower than the list price.',
        format => '',
        read_only => '',
            },
    'shipping_height_invalid' => {
        datatype => 'int',
        base_name => 'SHIPPING_HEIGHT_INVALID',
        description => 'Some items include incorrectly formatted shipping_height.',
        format => '',
        read_only => '',
            },
    'shipping_invalid' => {
        datatype => 'int',
        base_name => 'SHIPPING_INVALID',
        description => 'Some items have shipping values that are formatted incorrectly.',
        format => '',
        read_only => '',
            },
    'shipping_weight_invalid' => {
        datatype => 'int',
        base_name => 'SHIPPING_WEIGHT_INVALID',
        description => 'Some items have invalid shipping_weight values.',
        format => '',
        read_only => '',
            },
    'shipping_width_invalid' => {
        datatype => 'int',
        base_name => 'SHIPPING_WIDTH_INVALID',
        description => 'Some items include incorrectly formatted shipping_width.',
        format => '',
        read_only => '',
            },
    'size_system_invalid' => {
        datatype => 'int',
        base_name => 'SIZE_SYSTEM_INVALID',
        description => 'Some items have size system values which are not one of the supported size systems.',
        format => '',
        read_only => '',
            },
    'size_type_invalid' => {
        datatype => 'int',
        base_name => 'SIZE_TYPE_INVALID',
        description => 'Some items have size type values that are formatted incorrectly, which may limit visibility in recommendations, search results and shopping experiences.',
        format => '',
        read_only => '',
            },
    'tax_invalid' => {
        datatype => 'int',
        base_name => 'TAX_INVALID',
        description => 'Some items have tax values that are formatted incorrectly.',
        format => '',
        read_only => '',
            },
    'title_length_too_long' => {
        datatype => 'int',
        base_name => 'TITLE_LENGTH_TOO_LONG',
        description => 'The title for some items were truncated because they contain too many characters.',
        format => '',
        read_only => '',
            },
    'too_many_additional_image_links' => {
        datatype => 'int',
        base_name => 'TOO_MANY_ADDITIONAL_IMAGE_LINKS',
        description => 'Some items have additional_image_link values that exceed the limit for additional images, those items will be published without some of your images.',
        format => '',
        read_only => '',
            },
    'updated_time_invalid' => {
        datatype => 'int',
        base_name => 'UPDATED_TIME_INVALID',
        description => 'Some items have updated_time values thate are formatted incorrectly, those items will be published without an updated time.',
        format => '',
        read_only => '',
            },
    'utm_source_auto_corrected' => {
        datatype => 'int',
        base_name => 'UTM_SOURCE_AUTO_CORRECTED',
        description => 'Some items include utm_source values that are formatted incorrectly and have been automatically corrected.',
        format => '',
        read_only => '',
            },
    'video_required_when_ad_video_provided' => {
        datatype => 'int',
        base_name => 'VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED',
        description => 'A video is required in the item when ad_video fields are provided.',
        format => '',
        read_only => '',
            },
    'weight_unit_invalid' => {
        datatype => 'int',
        base_name => 'WEIGHT_UNIT_INVALID',
        description => 'Some items have weight_unit values that are formatted incorrectly, those items will be published without a weight unit.',
        format => '',
        read_only => '',
            },
});

__PACKAGE__->openapi_types( {
    'ad_image_0_link_duplicated' => 'int',
    'ad_image_0_link_length_too_long' => 'int',
    'ad_image_0_link_required' => 'int',
    'ad_image_0_link_warning' => 'int',
    'ad_image_0_tag_duplicated' => 'int',
    'ad_image_0_tag_length_too_long' => 'int',
    'ad_image_0_tag_required' => 'int',
    'ad_image_10_link_duplicated' => 'int',
    'ad_image_10_link_length_too_long' => 'int',
    'ad_image_10_link_required' => 'int',
    'ad_image_10_link_warning' => 'int',
    'ad_image_10_tag_duplicated' => 'int',
    'ad_image_10_tag_length_too_long' => 'int',
    'ad_image_10_tag_required' => 'int',
    'ad_image_11_link_duplicated' => 'int',
    'ad_image_11_link_length_too_long' => 'int',
    'ad_image_11_link_required' => 'int',
    'ad_image_11_link_warning' => 'int',
    'ad_image_11_tag_duplicated' => 'int',
    'ad_image_11_tag_length_too_long' => 'int',
    'ad_image_11_tag_required' => 'int',
    'ad_image_12_link_duplicated' => 'int',
    'ad_image_12_link_length_too_long' => 'int',
    'ad_image_12_link_required' => 'int',
    'ad_image_12_link_warning' => 'int',
    'ad_image_12_tag_duplicated' => 'int',
    'ad_image_12_tag_length_too_long' => 'int',
    'ad_image_12_tag_required' => 'int',
    'ad_image_13_link_duplicated' => 'int',
    'ad_image_13_link_length_too_long' => 'int',
    'ad_image_13_link_required' => 'int',
    'ad_image_13_link_warning' => 'int',
    'ad_image_13_tag_duplicated' => 'int',
    'ad_image_13_tag_length_too_long' => 'int',
    'ad_image_13_tag_required' => 'int',
    'ad_image_14_link_duplicated' => 'int',
    'ad_image_14_link_length_too_long' => 'int',
    'ad_image_14_link_required' => 'int',
    'ad_image_14_link_warning' => 'int',
    'ad_image_14_tag_duplicated' => 'int',
    'ad_image_14_tag_length_too_long' => 'int',
    'ad_image_14_tag_required' => 'int',
    'ad_image_15_link_duplicated' => 'int',
    'ad_image_15_link_length_too_long' => 'int',
    'ad_image_15_link_required' => 'int',
    'ad_image_15_link_warning' => 'int',
    'ad_image_15_tag_duplicated' => 'int',
    'ad_image_15_tag_length_too_long' => 'int',
    'ad_image_15_tag_required' => 'int',
    'ad_image_16_link_duplicated' => 'int',
    'ad_image_16_link_length_too_long' => 'int',
    'ad_image_16_link_required' => 'int',
    'ad_image_16_link_warning' => 'int',
    'ad_image_16_tag_duplicated' => 'int',
    'ad_image_16_tag_length_too_long' => 'int',
    'ad_image_16_tag_required' => 'int',
    'ad_image_17_link_duplicated' => 'int',
    'ad_image_17_link_length_too_long' => 'int',
    'ad_image_17_link_required' => 'int',
    'ad_image_17_link_warning' => 'int',
    'ad_image_17_tag_duplicated' => 'int',
    'ad_image_17_tag_length_too_long' => 'int',
    'ad_image_17_tag_required' => 'int',
    'ad_image_18_link_duplicated' => 'int',
    'ad_image_18_link_length_too_long' => 'int',
    'ad_image_18_link_required' => 'int',
    'ad_image_18_link_warning' => 'int',
    'ad_image_18_tag_duplicated' => 'int',
    'ad_image_18_tag_length_too_long' => 'int',
    'ad_image_18_tag_required' => 'int',
    'ad_image_19_link_duplicated' => 'int',
    'ad_image_19_link_length_too_long' => 'int',
    'ad_image_19_link_required' => 'int',
    'ad_image_19_link_warning' => 'int',
    'ad_image_19_tag_duplicated' => 'int',
    'ad_image_19_tag_length_too_long' => 'int',
    'ad_image_19_tag_required' => 'int',
    'ad_image_1_link_duplicated' => 'int',
    'ad_image_1_link_length_too_long' => 'int',
    'ad_image_1_link_required' => 'int',
    'ad_image_1_link_warning' => 'int',
    'ad_image_1_tag_duplicated' => 'int',
    'ad_image_1_tag_length_too_long' => 'int',
    'ad_image_1_tag_required' => 'int',
    'ad_image_2_link_duplicated' => 'int',
    'ad_image_2_link_length_too_long' => 'int',
    'ad_image_2_link_required' => 'int',
    'ad_image_2_link_warning' => 'int',
    'ad_image_2_tag_duplicated' => 'int',
    'ad_image_2_tag_length_too_long' => 'int',
    'ad_image_2_tag_required' => 'int',
    'ad_image_3_link_duplicated' => 'int',
    'ad_image_3_link_length_too_long' => 'int',
    'ad_image_3_link_required' => 'int',
    'ad_image_3_link_warning' => 'int',
    'ad_image_3_tag_duplicated' => 'int',
    'ad_image_3_tag_length_too_long' => 'int',
    'ad_image_3_tag_required' => 'int',
    'ad_image_4_link_duplicated' => 'int',
    'ad_image_4_link_length_too_long' => 'int',
    'ad_image_4_link_required' => 'int',
    'ad_image_4_link_warning' => 'int',
    'ad_image_4_tag_duplicated' => 'int',
    'ad_image_4_tag_length_too_long' => 'int',
    'ad_image_4_tag_required' => 'int',
    'ad_image_5_link_duplicated' => 'int',
    'ad_image_5_link_length_too_long' => 'int',
    'ad_image_5_link_required' => 'int',
    'ad_image_5_link_warning' => 'int',
    'ad_image_5_tag_duplicated' => 'int',
    'ad_image_5_tag_length_too_long' => 'int',
    'ad_image_5_tag_required' => 'int',
    'ad_image_6_link_duplicated' => 'int',
    'ad_image_6_link_length_too_long' => 'int',
    'ad_image_6_link_required' => 'int',
    'ad_image_6_link_warning' => 'int',
    'ad_image_6_tag_duplicated' => 'int',
    'ad_image_6_tag_length_too_long' => 'int',
    'ad_image_6_tag_required' => 'int',
    'ad_image_7_link_duplicated' => 'int',
    'ad_image_7_link_length_too_long' => 'int',
    'ad_image_7_link_required' => 'int',
    'ad_image_7_link_warning' => 'int',
    'ad_image_7_tag_duplicated' => 'int',
    'ad_image_7_tag_length_too_long' => 'int',
    'ad_image_7_tag_required' => 'int',
    'ad_image_8_link_duplicated' => 'int',
    'ad_image_8_link_length_too_long' => 'int',
    'ad_image_8_link_required' => 'int',
    'ad_image_8_link_warning' => 'int',
    'ad_image_8_tag_duplicated' => 'int',
    'ad_image_8_tag_length_too_long' => 'int',
    'ad_image_8_tag_required' => 'int',
    'ad_image_9_link_duplicated' => 'int',
    'ad_image_9_link_length_too_long' => 'int',
    'ad_image_9_link_required' => 'int',
    'ad_image_9_link_warning' => 'int',
    'ad_image_9_tag_duplicated' => 'int',
    'ad_image_9_tag_length_too_long' => 'int',
    'ad_image_9_tag_required' => 'int',
    'ad_link_format_warning' => 'int',
    'ad_link_same_as_link' => 'int',
    'ad_video_0_link_duplicated' => 'int',
    'ad_video_0_link_length_too_long' => 'int',
    'ad_video_0_link_required' => 'int',
    'ad_video_0_link_warning' => 'int',
    'ad_video_0_tag_duplicated' => 'int',
    'ad_video_0_tag_length_too_long' => 'int',
    'ad_video_0_tag_required' => 'int',
    'ad_video_1_link_duplicated' => 'int',
    'ad_video_1_link_length_too_long' => 'int',
    'ad_video_1_link_required' => 'int',
    'ad_video_1_link_warning' => 'int',
    'ad_video_1_tag_duplicated' => 'int',
    'ad_video_1_tag_length_too_long' => 'int',
    'ad_video_1_tag_required' => 'int',
    'ad_video_2_link_duplicated' => 'int',
    'ad_video_2_link_length_too_long' => 'int',
    'ad_video_2_link_required' => 'int',
    'ad_video_2_link_warning' => 'int',
    'ad_video_2_tag_duplicated' => 'int',
    'ad_video_2_tag_length_too_long' => 'int',
    'ad_video_2_tag_required' => 'int',
    'additional_image_link_length_too_long' => 'int',
    'additional_image_link_warning' => 'int',
    'adwords_format_warning' => 'int',
    'adwords_same_as_link' => 'int',
    'age_group_invalid' => 'int',
    'android_deep_link_invalid' => 'int',
    'availability_date_invalid' => 'int',
    'country_does_not_map_to_currency' => 'int',
    'custom_label_length_too_long' => 'int',
    'description_length_too_long' => 'int',
    'duplicate_headers' => 'int',
    'expiration_date_invalid' => 'int',
    'fetch_same_signature' => 'int',
    'gender_invalid' => 'int',
    'gtin_invalid' => 'int',
    'image_link_warning' => 'int',
    'inconsistent_currency_values' => 'int',
    'indexed_product_count_large_delta' => 'int',
    'ios_deep_link_invalid' => 'int',
    'is_bundle_invalid' => 'int',
    'item_additional_image_download_failure' => 'int',
    'link_format_warning' => 'int',
    'min_ad_price_invalid' => 'int',
    'mpn_invalid' => 'int',
    'multipack_invalid' => 'int',
    'optional_condition_invalid' => 'int',
    'optional_condition_missing' => 'int',
    'optional_product_category_invalid' => 'int',
    'optional_product_category_missing' => 'int',
    'product_category_depth_warning' => 'int',
    'product_type_length_too_long' => 'int',
    'sale_date_invalid' => 'int',
    'sales_price_invalid' => 'int',
    'sales_price_too_high' => 'int',
    'sales_price_too_low' => 'int',
    'shipping_height_invalid' => 'int',
    'shipping_invalid' => 'int',
    'shipping_weight_invalid' => 'int',
    'shipping_width_invalid' => 'int',
    'size_system_invalid' => 'int',
    'size_type_invalid' => 'int',
    'tax_invalid' => 'int',
    'title_length_too_long' => 'int',
    'too_many_additional_image_links' => 'int',
    'updated_time_invalid' => 'int',
    'utm_source_auto_corrected' => 'int',
    'video_required_when_ad_video_provided' => 'int',
    'weight_unit_invalid' => 'int'
} );

__PACKAGE__->attribute_map( {
    'ad_image_0_link_duplicated' => 'AD_IMAGE_0_LINK_DUPLICATED',
    'ad_image_0_link_length_too_long' => 'AD_IMAGE_0_LINK_LENGTH_TOO_LONG',
    'ad_image_0_link_required' => 'AD_IMAGE_0_LINK_REQUIRED',
    'ad_image_0_link_warning' => 'AD_IMAGE_0_LINK_WARNING',
    'ad_image_0_tag_duplicated' => 'AD_IMAGE_0_TAG_DUPLICATED',
    'ad_image_0_tag_length_too_long' => 'AD_IMAGE_0_TAG_LENGTH_TOO_LONG',
    'ad_image_0_tag_required' => 'AD_IMAGE_0_TAG_REQUIRED',
    'ad_image_10_link_duplicated' => 'AD_IMAGE_10_LINK_DUPLICATED',
    'ad_image_10_link_length_too_long' => 'AD_IMAGE_10_LINK_LENGTH_TOO_LONG',
    'ad_image_10_link_required' => 'AD_IMAGE_10_LINK_REQUIRED',
    'ad_image_10_link_warning' => 'AD_IMAGE_10_LINK_WARNING',
    'ad_image_10_tag_duplicated' => 'AD_IMAGE_10_TAG_DUPLICATED',
    'ad_image_10_tag_length_too_long' => 'AD_IMAGE_10_TAG_LENGTH_TOO_LONG',
    'ad_image_10_tag_required' => 'AD_IMAGE_10_TAG_REQUIRED',
    'ad_image_11_link_duplicated' => 'AD_IMAGE_11_LINK_DUPLICATED',
    'ad_image_11_link_length_too_long' => 'AD_IMAGE_11_LINK_LENGTH_TOO_LONG',
    'ad_image_11_link_required' => 'AD_IMAGE_11_LINK_REQUIRED',
    'ad_image_11_link_warning' => 'AD_IMAGE_11_LINK_WARNING',
    'ad_image_11_tag_duplicated' => 'AD_IMAGE_11_TAG_DUPLICATED',
    'ad_image_11_tag_length_too_long' => 'AD_IMAGE_11_TAG_LENGTH_TOO_LONG',
    'ad_image_11_tag_required' => 'AD_IMAGE_11_TAG_REQUIRED',
    'ad_image_12_link_duplicated' => 'AD_IMAGE_12_LINK_DUPLICATED',
    'ad_image_12_link_length_too_long' => 'AD_IMAGE_12_LINK_LENGTH_TOO_LONG',
    'ad_image_12_link_required' => 'AD_IMAGE_12_LINK_REQUIRED',
    'ad_image_12_link_warning' => 'AD_IMAGE_12_LINK_WARNING',
    'ad_image_12_tag_duplicated' => 'AD_IMAGE_12_TAG_DUPLICATED',
    'ad_image_12_tag_length_too_long' => 'AD_IMAGE_12_TAG_LENGTH_TOO_LONG',
    'ad_image_12_tag_required' => 'AD_IMAGE_12_TAG_REQUIRED',
    'ad_image_13_link_duplicated' => 'AD_IMAGE_13_LINK_DUPLICATED',
    'ad_image_13_link_length_too_long' => 'AD_IMAGE_13_LINK_LENGTH_TOO_LONG',
    'ad_image_13_link_required' => 'AD_IMAGE_13_LINK_REQUIRED',
    'ad_image_13_link_warning' => 'AD_IMAGE_13_LINK_WARNING',
    'ad_image_13_tag_duplicated' => 'AD_IMAGE_13_TAG_DUPLICATED',
    'ad_image_13_tag_length_too_long' => 'AD_IMAGE_13_TAG_LENGTH_TOO_LONG',
    'ad_image_13_tag_required' => 'AD_IMAGE_13_TAG_REQUIRED',
    'ad_image_14_link_duplicated' => 'AD_IMAGE_14_LINK_DUPLICATED',
    'ad_image_14_link_length_too_long' => 'AD_IMAGE_14_LINK_LENGTH_TOO_LONG',
    'ad_image_14_link_required' => 'AD_IMAGE_14_LINK_REQUIRED',
    'ad_image_14_link_warning' => 'AD_IMAGE_14_LINK_WARNING',
    'ad_image_14_tag_duplicated' => 'AD_IMAGE_14_TAG_DUPLICATED',
    'ad_image_14_tag_length_too_long' => 'AD_IMAGE_14_TAG_LENGTH_TOO_LONG',
    'ad_image_14_tag_required' => 'AD_IMAGE_14_TAG_REQUIRED',
    'ad_image_15_link_duplicated' => 'AD_IMAGE_15_LINK_DUPLICATED',
    'ad_image_15_link_length_too_long' => 'AD_IMAGE_15_LINK_LENGTH_TOO_LONG',
    'ad_image_15_link_required' => 'AD_IMAGE_15_LINK_REQUIRED',
    'ad_image_15_link_warning' => 'AD_IMAGE_15_LINK_WARNING',
    'ad_image_15_tag_duplicated' => 'AD_IMAGE_15_TAG_DUPLICATED',
    'ad_image_15_tag_length_too_long' => 'AD_IMAGE_15_TAG_LENGTH_TOO_LONG',
    'ad_image_15_tag_required' => 'AD_IMAGE_15_TAG_REQUIRED',
    'ad_image_16_link_duplicated' => 'AD_IMAGE_16_LINK_DUPLICATED',
    'ad_image_16_link_length_too_long' => 'AD_IMAGE_16_LINK_LENGTH_TOO_LONG',
    'ad_image_16_link_required' => 'AD_IMAGE_16_LINK_REQUIRED',
    'ad_image_16_link_warning' => 'AD_IMAGE_16_LINK_WARNING',
    'ad_image_16_tag_duplicated' => 'AD_IMAGE_16_TAG_DUPLICATED',
    'ad_image_16_tag_length_too_long' => 'AD_IMAGE_16_TAG_LENGTH_TOO_LONG',
    'ad_image_16_tag_required' => 'AD_IMAGE_16_TAG_REQUIRED',
    'ad_image_17_link_duplicated' => 'AD_IMAGE_17_LINK_DUPLICATED',
    'ad_image_17_link_length_too_long' => 'AD_IMAGE_17_LINK_LENGTH_TOO_LONG',
    'ad_image_17_link_required' => 'AD_IMAGE_17_LINK_REQUIRED',
    'ad_image_17_link_warning' => 'AD_IMAGE_17_LINK_WARNING',
    'ad_image_17_tag_duplicated' => 'AD_IMAGE_17_TAG_DUPLICATED',
    'ad_image_17_tag_length_too_long' => 'AD_IMAGE_17_TAG_LENGTH_TOO_LONG',
    'ad_image_17_tag_required' => 'AD_IMAGE_17_TAG_REQUIRED',
    'ad_image_18_link_duplicated' => 'AD_IMAGE_18_LINK_DUPLICATED',
    'ad_image_18_link_length_too_long' => 'AD_IMAGE_18_LINK_LENGTH_TOO_LONG',
    'ad_image_18_link_required' => 'AD_IMAGE_18_LINK_REQUIRED',
    'ad_image_18_link_warning' => 'AD_IMAGE_18_LINK_WARNING',
    'ad_image_18_tag_duplicated' => 'AD_IMAGE_18_TAG_DUPLICATED',
    'ad_image_18_tag_length_too_long' => 'AD_IMAGE_18_TAG_LENGTH_TOO_LONG',
    'ad_image_18_tag_required' => 'AD_IMAGE_18_TAG_REQUIRED',
    'ad_image_19_link_duplicated' => 'AD_IMAGE_19_LINK_DUPLICATED',
    'ad_image_19_link_length_too_long' => 'AD_IMAGE_19_LINK_LENGTH_TOO_LONG',
    'ad_image_19_link_required' => 'AD_IMAGE_19_LINK_REQUIRED',
    'ad_image_19_link_warning' => 'AD_IMAGE_19_LINK_WARNING',
    'ad_image_19_tag_duplicated' => 'AD_IMAGE_19_TAG_DUPLICATED',
    'ad_image_19_tag_length_too_long' => 'AD_IMAGE_19_TAG_LENGTH_TOO_LONG',
    'ad_image_19_tag_required' => 'AD_IMAGE_19_TAG_REQUIRED',
    'ad_image_1_link_duplicated' => 'AD_IMAGE_1_LINK_DUPLICATED',
    'ad_image_1_link_length_too_long' => 'AD_IMAGE_1_LINK_LENGTH_TOO_LONG',
    'ad_image_1_link_required' => 'AD_IMAGE_1_LINK_REQUIRED',
    'ad_image_1_link_warning' => 'AD_IMAGE_1_LINK_WARNING',
    'ad_image_1_tag_duplicated' => 'AD_IMAGE_1_TAG_DUPLICATED',
    'ad_image_1_tag_length_too_long' => 'AD_IMAGE_1_TAG_LENGTH_TOO_LONG',
    'ad_image_1_tag_required' => 'AD_IMAGE_1_TAG_REQUIRED',
    'ad_image_2_link_duplicated' => 'AD_IMAGE_2_LINK_DUPLICATED',
    'ad_image_2_link_length_too_long' => 'AD_IMAGE_2_LINK_LENGTH_TOO_LONG',
    'ad_image_2_link_required' => 'AD_IMAGE_2_LINK_REQUIRED',
    'ad_image_2_link_warning' => 'AD_IMAGE_2_LINK_WARNING',
    'ad_image_2_tag_duplicated' => 'AD_IMAGE_2_TAG_DUPLICATED',
    'ad_image_2_tag_length_too_long' => 'AD_IMAGE_2_TAG_LENGTH_TOO_LONG',
    'ad_image_2_tag_required' => 'AD_IMAGE_2_TAG_REQUIRED',
    'ad_image_3_link_duplicated' => 'AD_IMAGE_3_LINK_DUPLICATED',
    'ad_image_3_link_length_too_long' => 'AD_IMAGE_3_LINK_LENGTH_TOO_LONG',
    'ad_image_3_link_required' => 'AD_IMAGE_3_LINK_REQUIRED',
    'ad_image_3_link_warning' => 'AD_IMAGE_3_LINK_WARNING',
    'ad_image_3_tag_duplicated' => 'AD_IMAGE_3_TAG_DUPLICATED',
    'ad_image_3_tag_length_too_long' => 'AD_IMAGE_3_TAG_LENGTH_TOO_LONG',
    'ad_image_3_tag_required' => 'AD_IMAGE_3_TAG_REQUIRED',
    'ad_image_4_link_duplicated' => 'AD_IMAGE_4_LINK_DUPLICATED',
    'ad_image_4_link_length_too_long' => 'AD_IMAGE_4_LINK_LENGTH_TOO_LONG',
    'ad_image_4_link_required' => 'AD_IMAGE_4_LINK_REQUIRED',
    'ad_image_4_link_warning' => 'AD_IMAGE_4_LINK_WARNING',
    'ad_image_4_tag_duplicated' => 'AD_IMAGE_4_TAG_DUPLICATED',
    'ad_image_4_tag_length_too_long' => 'AD_IMAGE_4_TAG_LENGTH_TOO_LONG',
    'ad_image_4_tag_required' => 'AD_IMAGE_4_TAG_REQUIRED',
    'ad_image_5_link_duplicated' => 'AD_IMAGE_5_LINK_DUPLICATED',
    'ad_image_5_link_length_too_long' => 'AD_IMAGE_5_LINK_LENGTH_TOO_LONG',
    'ad_image_5_link_required' => 'AD_IMAGE_5_LINK_REQUIRED',
    'ad_image_5_link_warning' => 'AD_IMAGE_5_LINK_WARNING',
    'ad_image_5_tag_duplicated' => 'AD_IMAGE_5_TAG_DUPLICATED',
    'ad_image_5_tag_length_too_long' => 'AD_IMAGE_5_TAG_LENGTH_TOO_LONG',
    'ad_image_5_tag_required' => 'AD_IMAGE_5_TAG_REQUIRED',
    'ad_image_6_link_duplicated' => 'AD_IMAGE_6_LINK_DUPLICATED',
    'ad_image_6_link_length_too_long' => 'AD_IMAGE_6_LINK_LENGTH_TOO_LONG',
    'ad_image_6_link_required' => 'AD_IMAGE_6_LINK_REQUIRED',
    'ad_image_6_link_warning' => 'AD_IMAGE_6_LINK_WARNING',
    'ad_image_6_tag_duplicated' => 'AD_IMAGE_6_TAG_DUPLICATED',
    'ad_image_6_tag_length_too_long' => 'AD_IMAGE_6_TAG_LENGTH_TOO_LONG',
    'ad_image_6_tag_required' => 'AD_IMAGE_6_TAG_REQUIRED',
    'ad_image_7_link_duplicated' => 'AD_IMAGE_7_LINK_DUPLICATED',
    'ad_image_7_link_length_too_long' => 'AD_IMAGE_7_LINK_LENGTH_TOO_LONG',
    'ad_image_7_link_required' => 'AD_IMAGE_7_LINK_REQUIRED',
    'ad_image_7_link_warning' => 'AD_IMAGE_7_LINK_WARNING',
    'ad_image_7_tag_duplicated' => 'AD_IMAGE_7_TAG_DUPLICATED',
    'ad_image_7_tag_length_too_long' => 'AD_IMAGE_7_TAG_LENGTH_TOO_LONG',
    'ad_image_7_tag_required' => 'AD_IMAGE_7_TAG_REQUIRED',
    'ad_image_8_link_duplicated' => 'AD_IMAGE_8_LINK_DUPLICATED',
    'ad_image_8_link_length_too_long' => 'AD_IMAGE_8_LINK_LENGTH_TOO_LONG',
    'ad_image_8_link_required' => 'AD_IMAGE_8_LINK_REQUIRED',
    'ad_image_8_link_warning' => 'AD_IMAGE_8_LINK_WARNING',
    'ad_image_8_tag_duplicated' => 'AD_IMAGE_8_TAG_DUPLICATED',
    'ad_image_8_tag_length_too_long' => 'AD_IMAGE_8_TAG_LENGTH_TOO_LONG',
    'ad_image_8_tag_required' => 'AD_IMAGE_8_TAG_REQUIRED',
    'ad_image_9_link_duplicated' => 'AD_IMAGE_9_LINK_DUPLICATED',
    'ad_image_9_link_length_too_long' => 'AD_IMAGE_9_LINK_LENGTH_TOO_LONG',
    'ad_image_9_link_required' => 'AD_IMAGE_9_LINK_REQUIRED',
    'ad_image_9_link_warning' => 'AD_IMAGE_9_LINK_WARNING',
    'ad_image_9_tag_duplicated' => 'AD_IMAGE_9_TAG_DUPLICATED',
    'ad_image_9_tag_length_too_long' => 'AD_IMAGE_9_TAG_LENGTH_TOO_LONG',
    'ad_image_9_tag_required' => 'AD_IMAGE_9_TAG_REQUIRED',
    'ad_link_format_warning' => 'AD_LINK_FORMAT_WARNING',
    'ad_link_same_as_link' => 'AD_LINK_SAME_AS_LINK',
    'ad_video_0_link_duplicated' => 'AD_VIDEO_0_LINK_DUPLICATED',
    'ad_video_0_link_length_too_long' => 'AD_VIDEO_0_LINK_LENGTH_TOO_LONG',
    'ad_video_0_link_required' => 'AD_VIDEO_0_LINK_REQUIRED',
    'ad_video_0_link_warning' => 'AD_VIDEO_0_LINK_WARNING',
    'ad_video_0_tag_duplicated' => 'AD_VIDEO_0_TAG_DUPLICATED',
    'ad_video_0_tag_length_too_long' => 'AD_VIDEO_0_TAG_LENGTH_TOO_LONG',
    'ad_video_0_tag_required' => 'AD_VIDEO_0_TAG_REQUIRED',
    'ad_video_1_link_duplicated' => 'AD_VIDEO_1_LINK_DUPLICATED',
    'ad_video_1_link_length_too_long' => 'AD_VIDEO_1_LINK_LENGTH_TOO_LONG',
    'ad_video_1_link_required' => 'AD_VIDEO_1_LINK_REQUIRED',
    'ad_video_1_link_warning' => 'AD_VIDEO_1_LINK_WARNING',
    'ad_video_1_tag_duplicated' => 'AD_VIDEO_1_TAG_DUPLICATED',
    'ad_video_1_tag_length_too_long' => 'AD_VIDEO_1_TAG_LENGTH_TOO_LONG',
    'ad_video_1_tag_required' => 'AD_VIDEO_1_TAG_REQUIRED',
    'ad_video_2_link_duplicated' => 'AD_VIDEO_2_LINK_DUPLICATED',
    'ad_video_2_link_length_too_long' => 'AD_VIDEO_2_LINK_LENGTH_TOO_LONG',
    'ad_video_2_link_required' => 'AD_VIDEO_2_LINK_REQUIRED',
    'ad_video_2_link_warning' => 'AD_VIDEO_2_LINK_WARNING',
    'ad_video_2_tag_duplicated' => 'AD_VIDEO_2_TAG_DUPLICATED',
    'ad_video_2_tag_length_too_long' => 'AD_VIDEO_2_TAG_LENGTH_TOO_LONG',
    'ad_video_2_tag_required' => 'AD_VIDEO_2_TAG_REQUIRED',
    'additional_image_link_length_too_long' => 'ADDITIONAL_IMAGE_LINK_LENGTH_TOO_LONG',
    'additional_image_link_warning' => 'ADDITIONAL_IMAGE_LINK_WARNING',
    'adwords_format_warning' => 'ADWORDS_FORMAT_WARNING',
    'adwords_same_as_link' => 'ADWORDS_SAME_AS_LINK',
    'age_group_invalid' => 'AGE_GROUP_INVALID',
    'android_deep_link_invalid' => 'ANDROID_DEEP_LINK_INVALID',
    'availability_date_invalid' => 'AVAILABILITY_DATE_INVALID',
    'country_does_not_map_to_currency' => 'COUNTRY_DOES_NOT_MAP_TO_CURRENCY',
    'custom_label_length_too_long' => 'CUSTOM_LABEL_LENGTH_TOO_LONG',
    'description_length_too_long' => 'DESCRIPTION_LENGTH_TOO_LONG',
    'duplicate_headers' => 'DUPLICATE_HEADERS',
    'expiration_date_invalid' => 'EXPIRATION_DATE_INVALID',
    'fetch_same_signature' => 'FETCH_SAME_SIGNATURE',
    'gender_invalid' => 'GENDER_INVALID',
    'gtin_invalid' => 'GTIN_INVALID',
    'image_link_warning' => 'IMAGE_LINK_WARNING',
    'inconsistent_currency_values' => 'INCONSISTENT_CURRENCY_VALUES',
    'indexed_product_count_large_delta' => 'INDEXED_PRODUCT_COUNT_LARGE_DELTA',
    'ios_deep_link_invalid' => 'IOS_DEEP_LINK_INVALID',
    'is_bundle_invalid' => 'IS_BUNDLE_INVALID',
    'item_additional_image_download_failure' => 'ITEM_ADDITIONAL_IMAGE_DOWNLOAD_FAILURE',
    'link_format_warning' => 'LINK_FORMAT_WARNING',
    'min_ad_price_invalid' => 'MIN_AD_PRICE_INVALID',
    'mpn_invalid' => 'MPN_INVALID',
    'multipack_invalid' => 'MULTIPACK_INVALID',
    'optional_condition_invalid' => 'OPTIONAL_CONDITION_INVALID',
    'optional_condition_missing' => 'OPTIONAL_CONDITION_MISSING',
    'optional_product_category_invalid' => 'OPTIONAL_PRODUCT_CATEGORY_INVALID',
    'optional_product_category_missing' => 'OPTIONAL_PRODUCT_CATEGORY_MISSING',
    'product_category_depth_warning' => 'PRODUCT_CATEGORY_DEPTH_WARNING',
    'product_type_length_too_long' => 'PRODUCT_TYPE_LENGTH_TOO_LONG',
    'sale_date_invalid' => 'SALE_DATE_INVALID',
    'sales_price_invalid' => 'SALES_PRICE_INVALID',
    'sales_price_too_high' => 'SALES_PRICE_TOO_HIGH',
    'sales_price_too_low' => 'SALES_PRICE_TOO_LOW',
    'shipping_height_invalid' => 'SHIPPING_HEIGHT_INVALID',
    'shipping_invalid' => 'SHIPPING_INVALID',
    'shipping_weight_invalid' => 'SHIPPING_WEIGHT_INVALID',
    'shipping_width_invalid' => 'SHIPPING_WIDTH_INVALID',
    'size_system_invalid' => 'SIZE_SYSTEM_INVALID',
    'size_type_invalid' => 'SIZE_TYPE_INVALID',
    'tax_invalid' => 'TAX_INVALID',
    'title_length_too_long' => 'TITLE_LENGTH_TOO_LONG',
    'too_many_additional_image_links' => 'TOO_MANY_ADDITIONAL_IMAGE_LINKS',
    'updated_time_invalid' => 'UPDATED_TIME_INVALID',
    'utm_source_auto_corrected' => 'UTM_SOURCE_AUTO_CORRECTED',
    'video_required_when_ad_video_provided' => 'VIDEO_REQUIRED_WHEN_AD_VIDEO_PROVIDED',
    'weight_unit_invalid' => 'WEIGHT_UNIT_INVALID'
} );

__PACKAGE__->mk_accessors(keys %{__PACKAGE__->attribute_map});


1;
