# WWW::OpenAPIClient::TrendsApi

## Load the API package
```perl
use WWW::OpenAPIClient::Object::TrendsApi;
```
...
> ARRAY[TrendsEditorial] trends_editorial_articles_list(region => $region)
...
my $api_instance = WWW::OpenAPIClient::TrendsApi->new(
...
eval {
    my $result = $api_instance->trends_editorial_articles_list(region => $region);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling TrendsApi->trends_editorial_articles_list: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling TrendsApi->trends_featured_topics_list: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling TrendsApi->trends_product_categories_details_list: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling TrendsApi->trends_product_categories_trending_list: $@\n";
}
```
