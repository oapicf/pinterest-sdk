...
module OpenAPIClient
  ...
  # ScheduleDeltaValue (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class ScheduleDeltaValue
    ...
    def initialize(@value : ScheduleBidMultipliers)
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(ScheduleBidMultipliers.from_json(json))
      ...
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in ScheduleDeltaValue (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(ScheduleBidMultipliers.new(ctx, node))
      ...
      raise YAML::ParseException.new("doesn't match any schema listed in ScheduleDeltaValue (anyOf)", 0, 0)
    end
