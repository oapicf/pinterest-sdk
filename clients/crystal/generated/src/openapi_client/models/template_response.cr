# #Pinterest REST API
#
##Pinterest's REST API
#
#The version of the OpenAPI document: 5.23.0
#Contact: blah+oapicf@cliffano.com
#Generated by: https://openapi-generator.tech
#Generator version: 7.18.0
#

require "big"
require "json"
require "yaml"
require "time"

module OpenAPIClient
  # Template fields
  class TemplateResponse
    include JSON::Serializable
    include YAML::Serializable

    # Optional properties
    # ID of the Ad Account that owns the template
    @[JSON::Field(key: "ad_account_id", type: String?, nillable: true, emit_null: false)]
    property ad_account_id : String?

    # IDs of the Ad Accounts that have access to this template
    @[JSON::Field(key: "ad_account_ids", type: Array(String)?, nillable: true, emit_null: false)]
    property ad_account_ids : Array(String)?

    # The length of the sliding window over which click conversions will be attributed
    @[JSON::Field(key: "click_window_days", type: Float64?, nillable: true, emit_null: false)]
    property click_window_days : Float64?

    # A list of columns to be included in the report
    @[JSON::Field(key: "columns", type: Array(String)?, nillable: true, emit_null: false)]
    property columns : Array(String)?

    # Conversion report time type
    @[JSON::Field(key: "conversion_report_time_type", type: String?, nillable: true, emit_null: false)]
    property conversion_report_time_type : String?

    # The surface used to create this template
    @[JSON::Field(key: "creation_source", type: String?, nillable: true, emit_null: false)]
    property creation_source : String?

    # A list of custom column IDs
    @[JSON::Field(key: "custom_column_ids", type: Array(String)?, nillable: true, emit_null: false)]
    property custom_column_ids : Array(String)?

    @[JSON::Field(key: "date_range", type: TemplateResponseDateRange?, nillable: true, emit_null: false)]
    property date_range : TemplateResponseDateRange?

    # The length of the sliding window over which engagement conversions will be attributed
    @[JSON::Field(key: "engagement_window_days", type: Float64?, nillable: true, emit_null: false)]
    property engagement_window_days : Float64?

    # A JSON representation of any filters to be applied before returning report data. Each filter object should contain all of the following fields:<br> \"field\": The column name<br> \"operator\": The operator. Allowed operators: [\"=\", \"!=\", \"in\", \"not_in\", \"~\", \">\", \"<\", \"contains_substring\"]<br> \"value\": A single value or a list of values
    @[JSON::Field(key: "filters_json", type: String?, nillable: true, emit_null: false)]
    property filters_json : String?

    @[JSON::Field(key: "granularity", type: Granularity?, nillable: true, emit_null: false)]
    property granularity : Granularity?

    # Template ID
    @[JSON::Field(key: "id", type: String?, nillable: true, emit_null: false)]
    property id : String?

    # The filter on the conversion ingestion source method for conversion metrics
    @[JSON::Field(key: "ingestion_sources", type: Array(String)?, nillable: true, emit_null: false)]
    property ingestion_sources : Array(String)?

    # A boolean that indicates if the template has been deleted
    @[JSON::Field(key: "is_deleted", type: Bool?, nillable: true, emit_null: false)]
    property is_deleted : Bool?

    # A boolean value that indicates if the user owns the template
    @[JSON::Field(key: "is_owned_by_user", type: Bool?, nillable: true, emit_null: false)]
    property is_owned_by_user : Bool?

    # A boolean value that indicates if this template has been used to create a scheduled report
    @[JSON::Field(key: "is_scheduled", type: Bool?, nillable: true, emit_null: false)]
    property is_scheduled : Bool?

    # Template Name
    @[JSON::Field(key: "name", type: String?, nillable: true, emit_null: false)]
    property name : String?

    # The number of days prior to the day the report will be delivered at which the report will end
    @[JSON::Field(key: "report_end_relative_days_in_past", type: Float64?, nillable: true, emit_null: false)]
    property report_end_relative_days_in_past : Float64?

    @[JSON::Field(key: "report_format", type: DataOutputFormat?, nillable: true, emit_null: false)]
    property report_format : DataOutputFormat?

    @[JSON::Field(key: "report_level", type: MetricsReportingLevel?, nillable: true, emit_null: false)]
    property report_level : MetricsReportingLevel?

    # The number of days prior to the day the report will be delivered at which the report will start
    @[JSON::Field(key: "report_start_relative_days_in_past", type: Float64?, nillable: true, emit_null: false)]
    property report_start_relative_days_in_past : Float64?

    # Reporting template type
    @[JSON::Field(key: "type", type: String?, nillable: true, emit_null: false)]
    property _type : String?

    # Time of last update in seconds since Unix epoch
    @[JSON::Field(key: "updated_time", type: Float64?, nillable: true, emit_null: false)]
    property updated_time : Float64?

    # ID of the user who created the template
    @[JSON::Field(key: "user_id", type: String?, nillable: true, emit_null: false)]
    property user_id : String?

    # The length of the sliding window over which view conversions will be attributed
    @[JSON::Field(key: "view_window_days", type: Float64?, nillable: true, emit_null: false)]
    property view_window_days : Float64?

    abstract class EnumAttributeValidator
      def valid?(value)
        !value || @allowable_values.includes?(value)
      end

      def message
        "invalid value for \"#{@attribute}\", must be one of #{@allowable_values}."
      end

      def to(_type, value)
        case _type
        when Int32
          value.to_i32
        when Int64
          value.to_i64
        when Float32
          value.to_f32
        when Float64
          value.to_f64
        else
          value.to_s
        end
      end
    end

    class EnumAttributeValidatorForConversion_report_time_type < EnumAttributeValidator
      @attribute : String
      @allowable_values : Array(Int32 | Int64 | Float32 | Float64 | String)

      def initialize
        @attribute = "conversion_report_time_type"
        @allowable_values = ["TIME_OF_AD_ACTION", "TIME_OF_CONVERSION"].map { |value| to(String, value)}
      end
    end

    class EnumAttributeValidatorForCreation_source < EnumAttributeValidator
      @attribute : String
      @allowable_values : Array(Int32 | Int64 | Float32 | Float64 | String)

      def initialize
        @attribute = "creation_source"
        @allowable_values = ["ADS_API", "ADS_MANAGER_REPORTING_PAGE", "ADS_MANAGER_REPORT_BUILDER"].map { |value| to(String, value)}
      end
    end

    class EnumAttributeValidatorFor_type < EnumAttributeValidator
      @attribute : String
      @allowable_values : Array(Int32 | Int64 | Float32 | Float64 | String)

      def initialize
        @attribute = "_type"
        @allowable_values = ["UNSPECIFIED", "BULK", "OVERVIEW", "TABLE"].map { |value| to(String, value)}
      end
    end


    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(@ad_account_id : String? = nil, @ad_account_ids : Array(String)? = nil, @click_window_days : Float64? = nil, @columns : Array(String)? = nil, @conversion_report_time_type : String? = nil, @creation_source : String? = nil, @custom_column_ids : Array(String)? = nil, @date_range : TemplateResponseDateRange? = nil, @engagement_window_days : Float64? = nil, @filters_json : String? = nil, @granularity : Granularity? = nil, @id : String? = nil, @ingestion_sources : Array(String)? = nil, @is_deleted : Bool? = nil, @is_owned_by_user : Bool? = nil, @is_scheduled : Bool? = nil, @name : String? = nil, @report_end_relative_days_in_past : Float64? = nil, @report_format : DataOutputFormat? = nil, @report_level : MetricsReportingLevel? = nil, @report_start_relative_days_in_past : Float64? = nil, @_type : String? = nil, @updated_time : Float64? = nil, @user_id : String? = nil, @view_window_days : Float64? = nil)
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array(String).new
      conversion_report_time_type_validator = EnumAttributeValidatorForConversion_report_time_type.new
      if !conversion_report_time_type_validator.valid?(@conversion_report_time_type)
        message = conversion_report_time_type_validator.message
        invalid_properties.push(message)
      end

      creation_source_validator = EnumAttributeValidatorForCreation_source.new
      if !creation_source_validator.valid?(@creation_source)
        message = creation_source_validator.message
        invalid_properties.push(message)
      end

      _type_validator = EnumAttributeValidatorFor_type.new
      if !_type_validator.valid?(@_type)
        message = _type_validator.message
        invalid_properties.push(message)
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      conversion_report_time_type_validator = EnumAttributeValidatorForConversion_report_time_type.new
      return false unless conversion_report_time_type_validator.valid?(@conversion_report_time_type)
      creation_source_validator = EnumAttributeValidatorForCreation_source.new
      return false unless creation_source_validator.valid?(@creation_source)
      _type_validator = EnumAttributeValidatorFor_type.new
      return false unless _type_validator.valid?(@_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] conversion_report_time_type Object to be assigned
    def conversion_report_time_type=(conversion_report_time_type)
      validator = EnumAttributeValidatorForConversion_report_time_type.new
      unless validator.valid?(conversion_report_time_type)
        raise ArgumentError.new(validator.message)
      end
      @conversion_report_time_type = conversion_report_time_type
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] creation_source Object to be assigned
    def creation_source=(creation_source)
      validator = EnumAttributeValidatorForCreation_source.new
      unless validator.valid?(creation_source)
        raise ArgumentError.new(validator.message)
      end
      @creation_source = creation_source
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] _type Object to be assigned
    def _type=(_type)
      validator = EnumAttributeValidatorFor_type.new
      unless validator.valid?(_type)
        raise ArgumentError.new(validator.message)
      end
      @_type = _type
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(other)
      return true if self.same?(other)
      self.class == other.class &&
          ad_account_id == other.ad_account_id &&
          ad_account_ids == other.ad_account_ids &&
          click_window_days == other.click_window_days &&
          columns == other.columns &&
          conversion_report_time_type == other.conversion_report_time_type &&
          creation_source == other.creation_source &&
          custom_column_ids == other.custom_column_ids &&
          date_range == other.date_range &&
          engagement_window_days == other.engagement_window_days &&
          filters_json == other.filters_json &&
          granularity == other.granularity &&
          id == other.id &&
          ingestion_sources == other.ingestion_sources &&
          is_deleted == other.is_deleted &&
          is_owned_by_user == other.is_owned_by_user &&
          is_scheduled == other.is_scheduled &&
          name == other.name &&
          report_end_relative_days_in_past == other.report_end_relative_days_in_past &&
          report_format == other.report_format &&
          report_level == other.report_level &&
          report_start_relative_days_in_past == other.report_start_relative_days_in_past &&
          _type == other._type &&
          updated_time == other.updated_time &&
          user_id == other.user_id &&
          view_window_days == other.view_window_days
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(other)
      self == other
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [ad_account_id, ad_account_ids, click_window_days, columns, conversion_report_time_type, creation_source, custom_column_ids, date_range, engagement_window_days, filters_json, granularity, id, ingestion_sources, is_deleted, is_owned_by_user, is_scheduled, name, report_end_relative_days_in_past, report_format, report_level, report_start_relative_days_in_past, _type, updated_time, user_id, view_window_days].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      new.build_from_hash(attributes)
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.openapi_types.each_pair do |key, type|
        if !attributes[self.class.attribute_map[key]]? && self.class.openapi_nullable.includes?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :Time
        Time.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        ({} of Symbol => String).tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        # models (e.g. Pet) or oneOf
        klass = OpenAPIClient.const_get(type)
        klass.respond_to?(:openapi_one_of) ? klass.build(value) : klass.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_h.to_s
    end

    # to_body is an alias to to_h (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_h
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_h
      hash = NetboxClient::RecursiveHash.new
      hash["ad_account_id"] = _to_h(ad_account_id)
      hash["ad_account_ids"] = _to_h(ad_account_ids)
      hash["click_window_days"] = _to_h(click_window_days)
      hash["columns"] = _to_h(columns)
      hash["conversion_report_time_type"] = _to_h(conversion_report_time_type)
      hash["creation_source"] = _to_h(creation_source)
      hash["custom_column_ids"] = _to_h(custom_column_ids)
      hash["date_range"] = _to_h(date_range)
      hash["engagement_window_days"] = _to_h(engagement_window_days)
      hash["filters_json"] = _to_h(filters_json)
      hash["granularity"] = _to_h(granularity)
      hash["id"] = _to_h(id)
      hash["ingestion_sources"] = _to_h(ingestion_sources)
      hash["is_deleted"] = _to_h(is_deleted)
      hash["is_owned_by_user"] = _to_h(is_owned_by_user)
      hash["is_scheduled"] = _to_h(is_scheduled)
      hash["name"] = _to_h(name)
      hash["report_end_relative_days_in_past"] = _to_h(report_end_relative_days_in_past)
      hash["report_format"] = _to_h(report_format)
      hash["report_level"] = _to_h(report_level)
      hash["report_start_relative_days_in_past"] = _to_h(report_start_relative_days_in_past)
      hash["type"] = _to_h(_type)
      hash["updated_time"] = _to_h(updated_time)
      hash["user_id"] = _to_h(user_id)
      hash["view_window_days"] = _to_h(view_window_days)
      hash.to_h
    end

    # Outputs non-array value in the form of hash
    # For object, use to_h. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    private def _to_h(value)
      if value.is_a?(Hash)
        hash = NetboxClient::RecursiveHash.new
        value.each { |k, v| hash[k] = _to_h(v) }
        hash
      elsif value.is_a?(Array)
        value.compact.map { |v| _to_h(v) }
      elsif value.responds_to?(:to_h)
        value.to_h
      else
        value
      end
    end

  end

end
