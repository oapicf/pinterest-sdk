# #Pinterest REST API
#
#The version of the OpenAPI document: 5.28.0
#Contact: blah+oapicf@cliffano.com
#Generated by: https://openapi-generator.tech
#Generator version: 7.24.0
#

module OpenAPIClient
  # Metadata of the member/partner that has access to the asset.
  class BusinessAccessUserSummary
    include JSON::Serializable
    include YAML::Serializable
    include OpenAPIClient::Serializable
    include OpenAPIClient::Validation

    # Optional properties
    # Email of the business member/partner.
    @[JSON::Field(key: "email", emit_null: false)]
    property email : String?

    # Unique identifier of the business member/partner.
    @[JSON::Field(key: "id", emit_null: false)]
    property id : String?

    # Username of the business member/partner.
    @[JSON::Field(key: "username", emit_null: false)]
    property username : String?

    validates(id, String, true, max_length: 20, min_length: 1)

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(@email : String? = nil, @id : String? = nil, @username : String? = nil)
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array(String).new
      if (msg = id_validation_error(@id))
        invalid_properties.push(msg)
      end
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      list_invalid_properties.empty?
    end

    def_equals_and_hash(email, id, username)
  end

end
