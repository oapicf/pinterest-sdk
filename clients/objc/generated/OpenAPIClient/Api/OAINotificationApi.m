...
-(NSURLSessionTask*) notificationPostWithNotificationPostRequest: (OAINotificationPostRequest*) notificationPostRequest
    ...
    if (notificationPostRequest == nil) {
        NSParameterAssert(notificationPostRequest);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"notificationPostRequest"] };
    ...
    bodyParam = notificationPostRequest;
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAINotificationResponse*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAINotificationResponse*)data, error);
                                }
                            }];
}
