...
-(NSURLSessionTask*) keywordsCreateWithAdAccountId: (NSString*) adAccountId
    ...
    if (keywordsCreate == nil) {
        NSParameterAssert(keywordsCreate);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"keywordsCreate"] };
            NSError* error = [NSError errorWithDomain:kOAIKeywordsApiErrorDomain code:kOAIKeywordsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"OAIKeywords*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIKeywords*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) keywordsUpdateWithAdAccountId: (NSString*) adAccountId
    ...
    if (keywordsUpdate == nil) {
        NSParameterAssert(keywordsUpdate);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"keywordsUpdate"] };
            NSError* error = [NSError errorWithDomain:kOAIKeywordsApiErrorDomain code:kOAIKeywordsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
...
///
///  @param includeDemographics Including the age and gender distribution for each keyword. By default (`false`), the response will not include demographics data. (optional, default to @(NO))
///
...
-(NSURLSessionTask*) trendingKeywordsListWithRegion: (OAITrendsSupportedRegion) region
    ...
    if (includeKeywords != nil) {
        queryParams[@"include_keywords"] = [[OAIQueryParamCollection alloc] initWithValuesAndFormat: includeKeywords format: @"multi"];
    }
