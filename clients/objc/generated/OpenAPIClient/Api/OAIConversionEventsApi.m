...
-(NSURLSessionTask*) eventsCreateWithAdAccountId: (NSString*) adAccountId
    ...
    if (conversionEventsCreate == nil) {
        NSParameterAssert(conversionEventsCreate);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"conversionEventsCreate"] };
            NSError* error = [NSError errorWithDomain:kOAIConversionEventsApiErrorDomain code:kOAIConversionEventsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    bodyParam = conversionEventsCreate;
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIConversionEvents*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIConversionEvents*)data, error);
                                }
                            }];
}
