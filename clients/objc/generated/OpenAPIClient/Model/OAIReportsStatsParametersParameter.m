...
@implementation OAIReportsStatsParametersParameter
...
- (id)initWithDictionary:(NSDictionary *)dict error:(NSError *__autoreleasing *)err {
    ...
    if ([discriminatedClassName isEqualToString:@"HOTEL"]) {
        class = NSClassFromString(@"OAICatalogsHotelReportStatsParameters");
    }
    else
    if ([discriminatedClassName isEqualToString:@"RETAIL"]) {
        class = NSClassFromString(@"OAICatalogsRetailReportStatsParameters");
    }
    else
    {
...
+ (JSONKeyMapper *)keyMapper {
  return [[JSONKeyMapper alloc] initWithModelToJSONDictionary:@{ @"catalogType": @"catalog_type", @"report": @"report" }];
}
