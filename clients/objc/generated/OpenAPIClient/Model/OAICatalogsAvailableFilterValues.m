...
@implementation OAICatalogsAvailableFilterValues
...
- (id)initWithDictionary:(NSDictionary *)dict error:(NSError *__autoreleasing *)err {
    ...
    if ([discriminatedClassName isEqualToString:@"CREATIVE_ASSETS"]) {
        class = NSClassFromString(@"OAICatalogsCreativeAssetsAvailableFilterValues");
    }
    else
    if ([discriminatedClassName isEqualToString:@"HOTEL"]) {
        class = NSClassFromString(@"OAICatalogsHotelAvailableFilterValues");
    }
    else
    if ([discriminatedClassName isEqualToString:@"RETAIL"]) {
        class = NSClassFromString(@"OAICatalogsRetailAvailableFilterValues");
    }
    else
    {
...
+ (JSONKeyMapper *)keyMapper {
  return [[JSONKeyMapper alloc] initWithModelToJSONDictionary:@{ @"catalogType": @"catalog_type", @"filterValues": @"filter_values" }];
}
