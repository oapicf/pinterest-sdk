...
@implementation OAICatalogsItemsPostFilters
...
- (id)initWithDictionary:(NSDictionary *)dict error:(NSError *__autoreleasing *)err {
    ...
    if ([discriminatedClassName isEqualToString:@"CREATIVE_ASSETS"]) {
        class = NSClassFromString(@"OAICatalogsCreativeAssetsItemsPostFilter");
    }
    else
    if ([discriminatedClassName isEqualToString:@"HOTEL"]) {
        class = NSClassFromString(@"OAICatalogsHotelItemsPostFilter");
    }
    else
    if ([discriminatedClassName isEqualToString:@"RETAIL"]) {
        class = NSClassFromString(@"OAICatalogsRetailItemsPostFilter");
    }
    else
    {
