...
@implementation OAIOauthAccessTokenRequest
...
- (id)initWithDictionary:(NSDictionary *)dict error:(NSError *__autoreleasing *)err {
    NSString * discriminatedClassName = [dict valueForKey:@"grantType"];
    ...
    if ([discriminatedClassName isEqualToString:@"authorization_code"]) {
        class = NSClassFromString(@"OAIOauthAccessTokenRequestCode");
    }
    else
    if ([discriminatedClassName isEqualToString:@"refresh_token"]) {
        class = NSClassFromString(@"OAIOauthAccessTokenRequestRefresh");
    }
    else
    {
...
+ (JSONKeyMapper *)keyMapper {
  return [[JSONKeyMapper alloc] initWithModelToJSONDictionary:@{ @"grantType": @"grant_type" }];
}
