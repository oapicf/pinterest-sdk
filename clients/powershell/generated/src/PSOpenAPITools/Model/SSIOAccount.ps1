#
# Pinterest REST API
# Pinterest's REST API
# Version: 5.28.0
# Contact: blah+oapicf@cliffano.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

Salesforce account details including bill-to information.

.PARAMETER BilltoInfos
An array of Salesforce account information that includes address, io terms, etc.
.PARAMETER CanEdit
Advertiser eligible to update order lines
.PARAMETER Currency
No description available.
.PARAMETER Eligible
Advertiser eligible to create order lines
.PARAMETER VarError
Error indicator from Salesforce which could be ""No Error""
.PARAMETER PmpNames
No description available.
.OUTPUTS

SSIOAccount<PSCustomObject>
#>

function Initialize-SSIOAccount {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject[]]
        ${BilltoInfos},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [System.Nullable[Boolean]]
        ${CanEdit},
        [Parameter(Position = 2, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${Currency},
        [Parameter(Position = 3, ValueFromPipelineByPropertyName = $true)]
        [System.Nullable[Boolean]]
        ${Eligible},
        [Parameter(Position = 4, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${VarError},
        [Parameter(Position = 5, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject[]]
        ${PmpNames}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => SSIOAccount' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug


        $PSO = [PSCustomObject]@{
            'billto_infos' = ${BilltoInfos}
            'can_edit' = ${CanEdit}
            'currency' = ${Currency}
            'eligible' = ${Eligible}
            'error' = ${VarError}
            'pmp_names' = ${PmpNames}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to SSIOAccount<PSCustomObject>

.DESCRIPTION

Convert from JSON to SSIOAccount<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

SSIOAccount<PSCustomObject>
#>
function ConvertFrom-JsonToSSIOAccount {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => SSIOAccount' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in SSIOAccount
        $AllProperties = ('billto_infos', 'can_edit', 'currency', 'eligible', 'error', 'pmp_names')
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'billto_infos'))) { #optional property not found
            $BilltoInfos = $null
        } else {
            $BilltoInfos = $JsonParameters.PSobject.Properties['billto_infos'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'can_edit'))) { #optional property not found
            $CanEdit = $null
        } else {
            $CanEdit = $JsonParameters.PSobject.Properties['can_edit'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'currency'))) { #optional property not found
            $Currency = $null
        } else {
            $Currency = $JsonParameters.PSobject.Properties['currency'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'eligible'))) { #optional property not found
            $Eligible = $null
        } else {
            $Eligible = $JsonParameters.PSobject.Properties['eligible'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'error'))) { #optional property not found
            $VarError = $null
        } else {
            $VarError = $JsonParameters.PSobject.Properties['error'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'pmp_names'))) { #optional property not found
            $PmpNames = $null
        } else {
            $PmpNames = $JsonParameters.PSobject.Properties['pmp_names'].value
        }

        $PSO = [PSCustomObject]@{
            'billto_infos' = ${BilltoInfos}
            'can_edit' = ${CanEdit}
            'currency' = ${Currency}
            'eligible' = ${Eligible}
            'error' = ${VarError}
            'pmp_names' = ${PmpNames}
        }

        return $PSO
    }

}

