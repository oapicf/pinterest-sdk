...
CatalogsCreativeAssetsItemsFilter<PSCustomObject>
#>

function Initialize-CatalogsCreativeAssetsItemsFilter {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => CatalogsCreativeAssetsItemsFilter' | Write-Debug
        ...
        if ($CreativeAssetsIds.length -gt 100) {
            throw "invalid value for 'CreativeAssetsIds', number of items must be less than or equal to 100."
        }
...
Convert from JSON to CatalogsCreativeAssetsItemsFilter<PSCustomObject>
...
CatalogsCreativeAssetsItemsFilter<PSCustomObject>
#>
function ConvertFrom-JsonToCatalogsCreativeAssetsItemsFilter {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => CatalogsCreativeAssetsItemsFilter' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in CatalogsCreativeAssetsItemsFilter
        $AllProperties = ("catalog_id", "catalog_type", "creative_assets_ids")
