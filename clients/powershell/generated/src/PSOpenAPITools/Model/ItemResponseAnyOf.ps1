...
function ConvertFrom-JsonToItemResponseAnyOf {
    ...
    Process {
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([CatalogsHotelItemResponse, CatalogsRetailItemResponse]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("CatalogsHotelItemResponse", "CatalogsRetailItemResponse")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([CatalogsHotelItemResponse, CatalogsRetailItemResponse]). JSON Payload: $($Json)"
        }
    }
}
