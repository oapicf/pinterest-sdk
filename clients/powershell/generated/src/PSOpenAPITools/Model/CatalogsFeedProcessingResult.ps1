...
function ConvertFrom-JsonToCatalogsFeedProcessingResult {
    ...
    Process {
        ...
        $AllProperties = ('created_at', 'id', 'ingestion_details', 'product_counts', 'status', 'updated_at', 'validation_details', 'video_counts')
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'video_counts'))) { #optional property not found
            $VideoCounts = $null
        } else {
            $VideoCounts = $JsonParameters.PSobject.Properties['video_counts'].value
        }
