...
function ConvertFrom-JsonToCatalogsVerticalFeedsCreateRequest {
    ...
    Process {
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([CatalogsHotelFeedsCreateRequest, CatalogsRetailFeedsCreateRequest]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("CatalogsHotelFeedsCreateRequest", "CatalogsRetailFeedsCreateRequest")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([CatalogsHotelFeedsCreateRequest, CatalogsRetailFeedsCreateRequest]). JSON Payload: $($Json)"
        }
    }
}
