#
# Pinterest REST API
# Pinterest's REST API
# Version: 5.28.0
# Contact: blah+oapicf@cliffano.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

Base64-encoded image media source

.PARAMETER ContentType
No description available.
.PARAMETER VarData
No description available.
.OUTPUTS

BrandAccountProfileImage<PSCustomObject>
#>

function Initialize-BrandAccountProfileImage {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [ValidateSet("image/jpeg", "image/png")]
        [String]
        ${ContentType},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [ValidatePattern("[a-zA-Z0-9+/=]+")]
        [String]
        ${VarData}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => BrandAccountProfileImage' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $ContentType) {
            throw "invalid value for 'ContentType', 'ContentType' cannot be null."
        }

        if ($null -eq $VarData) {
            throw "invalid value for 'VarData', 'VarData' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            'content_type' = ${ContentType}
            'data' = ${VarData}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to BrandAccountProfileImage<PSCustomObject>

.DESCRIPTION

Convert from JSON to BrandAccountProfileImage<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

BrandAccountProfileImage<PSCustomObject>
#>
function ConvertFrom-JsonToBrandAccountProfileImage {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => BrandAccountProfileImage' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in BrandAccountProfileImage
        $AllProperties = ('content_type', 'data')
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'content_type' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'content_type'))) {
            throw "Error! JSON cannot be serialized due to the required property 'content_type' missing."
        } else {
            $ContentType = $JsonParameters.PSobject.Properties['content_type'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'data'))) {
            throw "Error! JSON cannot be serialized due to the required property 'data' missing."
        } else {
            $VarData = $JsonParameters.PSobject.Properties['data'].value
        }

        $PSO = [PSCustomObject]@{
            'content_type' = ${ContentType}
            'data' = ${VarData}
        }

        return $PSO
    }

}

