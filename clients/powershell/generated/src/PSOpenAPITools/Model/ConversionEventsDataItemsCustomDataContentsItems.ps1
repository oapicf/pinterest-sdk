...
ConversionEventsDataItemsCustomDataContentsItems<PSCustomObject>
#>

function Initialize-ConversionEventsDataItemsCustomDataContentsItems {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => ConversionEventsDataItemsCustomDataContentsItems' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if (!$ItemBrandId -and $ItemBrandId.length -gt 64) {
            throw "invalid value for 'ItemBrandId', the character length must be smaller than or equal to 64."
        }
...
Convert from JSON to ConversionEventsDataItemsCustomDataContentsItems<PSCustomObject>
...
ConversionEventsDataItemsCustomDataContentsItems<PSCustomObject>
#>
function ConvertFrom-JsonToConversionEventsDataItemsCustomDataContentsItems {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => ConversionEventsDataItemsCustomDataContentsItems' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ConversionEventsDataItemsCustomDataContentsItems
        $AllProperties = ('id', 'item_brand', 'item_brand_id', 'item_category', 'item_name', 'item_price', 'quantity')
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'item_brand_id'))) { #optional property not found
            $ItemBrandId = $null
        } else {
            $ItemBrandId = $JsonParameters.PSobject.Properties['item_brand_id'].value
        }
