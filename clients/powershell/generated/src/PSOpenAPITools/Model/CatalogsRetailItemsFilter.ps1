...
CatalogsRetailItemsFilter<PSCustomObject>
#>

function Initialize-CatalogsRetailItemsFilter {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => CatalogsRetailItemsFilter' | Write-Debug
        ...
        if ($ItemIds.length -gt 100) {
            throw "invalid value for 'ItemIds', number of items must be less than or equal to 100."
        }
...
Convert from JSON to CatalogsRetailItemsFilter<PSCustomObject>
...
CatalogsRetailItemsFilter<PSCustomObject>
#>
function ConvertFrom-JsonToCatalogsRetailItemsFilter {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => CatalogsRetailItemsFilter' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in CatalogsRetailItemsFilter
        $AllProperties = ("catalog_id", "catalog_type", "item_ids")
