
/*
 * CatalogsBaseFiltersAnyOf.h
 *
 * 
 */

#ifndef TINY_CPP_CLIENT_CatalogsBaseFiltersAnyOf_H_
#define TINY_CPP_CLIENT_CatalogsBaseFiltersAnyOf_H_


#include <string>
#include "bourne/json.hpp"
#include "Helpers.h"
#include "CatalogsBaseFilterKeys.h"
#include <list>

namespace Tiny {


/*! \brief 
 *
 *  \ingroup Models
 *
 */

class CatalogsBaseFiltersAnyOf{
public:

    /*! \brief Constructor.
	 */
    CatalogsBaseFiltersAnyOf();
    CatalogsBaseFiltersAnyOf(std::string jsonString);


    /*! \brief Destructor.
	 */
    virtual ~CatalogsBaseFiltersAnyOf();


    /*! \brief Retrieve a bourne JSON representation of this class.
	 */
    bourne::json toJson();


    /*! \brief Fills in members of this class from bourne JSON object representing it.
	 */
    void fromJson(std::string jsonObj);

	/*! \brief Get 
	 */
	std::list<CatalogsBaseFilterKeys> getAnyOf();

	/*! \brief Set 
	 */
	void setAnyOf(std::list<CatalogsBaseFilterKeys> any_of);


    private:
    std::list<CatalogsBaseFilterKeys> any_of;
};
}

#endif /* TINY_CPP_CLIENT_CatalogsBaseFiltersAnyOf_H_ */
