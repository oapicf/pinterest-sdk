# LeadSubscriptionPostParams

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**ad_account_id** | Option<**String**> | The Ad Account ID that this lead form belongs to. | [optional]
**api_version** | Option<**String**> | API version. | [optional]
**created_time** | Option<**i32**> | Subscription creation time. Unix timestamp in milliseconds. | [optional]
**cryptographic_algorithm** | Option<**String**> | Lead data encryption algorithm. | [optional]
**cryptographic_key** | Option<**String**> | Base64 encoded key for client to decrypt lead data. | [optional]
**id** | Option<**String**> | Subscription ID. | [optional]
**lead_form_id** | Option<**String**> | Lead form ID. | [optional]
**user_account_id** | Option<**String**> | User account used to subscribe lead data. | [optional]
**webhook_url** | Option<**String**> | Standard HTTPS webhook URL. | [optional]
**partner_access_token** | Option<**String**> | Partner access token. Only for clients that requires authentication. We recommend to avoid this param. | [optional]
**partner_metadata** | Option<[**models::PartnerMetadata**](PartnerMetadata.md)> | Partner metadata. Only for clients that requires special handling. We recommend to avoid this param. | [optional]
**partner_refresh_token** | Option<**String**> | Partner refresh token. Only for clients that requires authentication. We recommend to avoid this param. | [optional]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


