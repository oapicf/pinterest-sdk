/*
 * Pinterest REST API
 *
 * Pinterest's REST API
 *
 * The version of the OpenAPI document: 5.14.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CreateInvitesResultsResponseArrayItemsInner {
    #[serde(rename = "exception", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub exception: Option<Option<Box<models::InviteExceptionResponse>>>,
    #[serde(rename = "invite", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub invite: Option<Option<Box<models::CreateInvitesResultsResponseArrayItemsInnerInvite>>>,
}

impl CreateInvitesResultsResponseArrayItemsInner {
    pub fn new() -> CreateInvitesResultsResponseArrayItemsInner {
        CreateInvitesResultsResponseArrayItemsInner {
            exception: None,
            invite: None,
        }
    }
}

