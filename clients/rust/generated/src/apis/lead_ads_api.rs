...
pub async fn ad_accounts_subscriptions_slash_get_by_id(configuration: &configuration::Configuration, ad_account_id: &str, subscription_id: &str) -> Result<models::AdAccountGetSubscriptionResponse, Error<AdAccountsSubscriptionsSlashGetByIdError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AdAccountGetSubscriptionResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AdAccountGetSubscriptionResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn ad_accounts_subscriptions_slash_post(configuration: &configuration::Configuration, ad_account_id: &str, ad_account_create_subscription_request: models::AdAccountCreateSubscriptionRequest) -> Result<models::AdAccountCreateSubscriptionResponse, Error<AdAccountsSubscriptionsSlashPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AdAccountCreateSubscriptionResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AdAccountCreateSubscriptionResponse`")))),
        }
    } else {
        let content = resp.text().await?;
