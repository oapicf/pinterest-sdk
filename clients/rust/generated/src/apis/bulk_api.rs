...
pub async fn bulk_download_slash_create(configuration: &configuration::Configuration, ad_account_id: &str, bulk_download_create: models::BulkDownloadCreate) -> Result<models::BulkDownload, Error<BulkDownloadSlashCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ad_account_id = ad_account_id;
    let p_body_bulk_download_create = bulk_download_create;
    ...
    req_builder = req_builder.json(&p_body_bulk_download_create);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::BulkDownload`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::BulkDownload`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn bulk_request_slash_get(configuration: &configuration::Configuration, ad_account_id: &str, bulk_request_id: &str, include_details: Option<bool>) -> Result<models::BulkJobData, Error<BulkRequestSlashGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ad_account_id = ad_account_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::BulkJobData`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::BulkJobData`")))),
        }
    } else {
        let content = resp.text().await?;
