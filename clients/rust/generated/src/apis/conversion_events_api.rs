...
pub async fn events_slash_create(configuration: &configuration::Configuration, ad_account_id: &str, conversion_events_create: models::ConversionEventsCreate, test: Option<bool>) -> Result<models::ConversionEvents, Error<EventsSlashCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ad_account_id = ad_account_id;
    let p_body_conversion_events_create = conversion_events_create;
    ...
    req_builder = req_builder.json(&p_body_conversion_events_create);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ConversionEvents`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ConversionEvents`")))),
        }
    } else {
        let content = resp.text().await?;
