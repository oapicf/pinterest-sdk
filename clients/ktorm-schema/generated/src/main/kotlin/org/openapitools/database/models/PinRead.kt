/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
package org.openapitools.database.models

import org.ktorm.dsl.*
import org.ktorm.schema.*
import org.ktorm.database.Database
import .*


/**
 * 
 * @param id 
 * @param aiDisclosures AI disclosure declarations the creator has made about this Pin.
 * @param boardId The board to which this Pin belongs.
 * @param boardOwner 
 * @param boardSectionId The board section to which this Pin belongs.
 * @param createdAt 
 * @param creativeType 
 * @param dominantColor Dominant pin color. Hex number, e.g. `#6E7874`.
 * @param hasBeenPromoted Whether the Pin has been promoted or not.
 * @param isOwner Whether the \"operation user_account\" is the Pin owner.
 * @param isProduct Whether the Pin is a product Pin.
 * @param isStandard Whether the Pin is standard or not. See documentation on [Changes to Pin creation](/docs/api-features/content-overview/) for more information.
 * @param media 
 * @param parentPinId The source pin id if this pin was saved from another pin. [Learn more](https://help.pinterest.com/article/save-pins-on-pinterest).
 * @param pinMetrics Pin metrics with associated time intervals if any.
 * @param altText 
 * @param description 
 * @param link 
 * @param title 
 */
object PinReads : BaseTable<PinRead>("PinRead") {
    val id = text("id")
    val aiDisclosures = long("ai_disclosures") /* null */ /* AI disclosure declarations the creator has made about this Pin. */
    val boardId = text("board_id") /* null */ /* The board to which this Pin belongs. */
    val boardOwner = long("board_owner") /* null */
    val boardSectionId = text("board_section_id") /* null */ /* The board section to which this Pin belongs. */
    val createdAt = datetime("created_at") /* null */
    val creativeType = long("creative_type") /* null */
    val dominantColor = text("dominant_color") /* null */ /* Dominant pin color. Hex number, e.g. `#6E7874`. */
    val hasBeenPromoted = boolean("has_been_promoted") /* null */ /* Whether the Pin has been promoted or not. */
    val isOwner = boolean("is_owner") /* null */ /* Whether the \"operation user_account\" is the Pin owner. */
    val isProduct = boolean("is_product") /* null */ /* Whether the Pin is a product Pin. */
    val isStandard = boolean("is_standard") /* null */ /* Whether the Pin is standard or not. See documentation on [Changes to Pin creation](/docs/api-features/content-overview/) for more information. */
    val media = long("media") /* null */
    val parentPinId = text("parent_pin_id") /* null */ /* The source pin id if this pin was saved from another pin. [Learn more](https://help.pinterest.com/article/save-pins-on-pinterest). */
    val pinMetrics = blob("pin_metrics") /* null */ /* Pin metrics with associated time intervals if any. */
    val altText = text("alt_text") /* null */
    val description = text("description") /* null */
    val link = text("link") /* null */
    val title = text("title") /* null */

    /**
     * Create an entity of type PinRead from the model
     */
    override fun doCreateEntity(row: QueryRowSet, withReferences: Boolean) = PinRead(
        id = row[id] ?: "" /* kotlin.String */,
        aiDisclosures = AiDisclosuress.createEntity(row, withReferences) /* AiDisclosures? */ /* AI disclosure declarations the creator has made about this Pin. */,
        boardId = row[boardId]  /* kotlin.String? */ /* The board to which this Pin belongs. */,
        boardOwner = BoardOwners.createEntity(row, withReferences) /* BoardOwner? */,
        boardSectionId = row[boardSectionId]  /* kotlin.String? */ /* The board section to which this Pin belongs. */,
        createdAt = row[createdAt]  /* java.time.LocalDateTime? */,
        creativeType = CreativeTypes.createEntity(row, withReferences) /* CreativeType? */,
        dominantColor = row[dominantColor]  /* kotlin.String? */ /* Dominant pin color. Hex number, e.g. `#6E7874`. */,
        hasBeenPromoted = row[hasBeenPromoted]  /* kotlin.Boolean? */ /* Whether the Pin has been promoted or not. */,
        isOwner = row[isOwner]  /* kotlin.Boolean? */ /* Whether the \"operation user_account\" is the Pin owner. */,
        isProduct = row[isProduct]  /* kotlin.Boolean? */ /* Whether the Pin is a product Pin. */,
        isStandard = row[isStandard]  /* kotlin.Boolean? */ /* Whether the Pin is standard or not. See documentation on [Changes to Pin creation](/docs/api-features/content-overview/) for more information. */,
        media = PinMedias.createEntity(row, withReferences) /* PinMedia? */,
        parentPinId = row[parentPinId]  /* kotlin.String? */ /* The source pin id if this pin was saved from another pin. [Learn more](https://help.pinterest.com/article/save-pins-on-pinterest). */,
        pinMetrics = row[pinMetrics]  /* kotlin.Any? */ /* Pin metrics with associated time intervals if any. */,
        altText = row[altText]  /* kotlin.String? */,
        description = row[description]  /* kotlin.String? */,
        link = row[link]  /* kotlin.String? */,
        title = row[title]  /* kotlin.String? */
    )

    /**
    * Assign all the columns from the entity of type PinRead to the DML expression.
    *
    * Usage:
    *
    * ```kotlin
    * let entity = PinRead()
    * database.update(PinReads, {
    *     assignFrom(entity)
    * })
    * ```
    * @return the builder with the columns for the update or insert.
    */
    fun AssignmentsBuilder.assignFrom(entity: PinRead) {
        this.apply {
            set(PinReads.id, entity.id)
            set(PinReads.aiDisclosures, entity.aiDisclosures)
            set(PinReads.boardId, entity.boardId)
            set(PinReads.boardOwner, entity.boardOwner)
            set(PinReads.boardSectionId, entity.boardSectionId)
            set(PinReads.createdAt, entity.createdAt)
            set(PinReads.creativeType, entity.creativeType)
            set(PinReads.dominantColor, entity.dominantColor)
            set(PinReads.hasBeenPromoted, entity.hasBeenPromoted)
            set(PinReads.isOwner, entity.isOwner)
            set(PinReads.isProduct, entity.isProduct)
            set(PinReads.isStandard, entity.isStandard)
            set(PinReads.media, entity.media)
            set(PinReads.parentPinId, entity.parentPinId)
            set(PinReads.pinMetrics, entity.pinMetrics)
            set(PinReads.altText, entity.altText)
            set(PinReads.description, entity.description)
            set(PinReads.link, entity.link)
            set(PinReads.title, entity.title)
        }
    }

}


