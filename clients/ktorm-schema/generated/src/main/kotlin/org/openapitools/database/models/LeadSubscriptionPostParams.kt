/**
* Pinterest REST API
* Pinterest's REST API
*
* The version of the OpenAPI document: 5.28.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
package org.openapitools.database.models

import org.ktorm.dsl.*
import org.ktorm.schema.*
import org.ktorm.database.Database
import .*


/**
 * 
 * @param adAccountId The Ad Account ID that this lead form belongs to.
 * @param apiVersion API version.
 * @param createdTime Subscription creation time. Unix timestamp in milliseconds.
 * @param cryptographicAlgorithm Lead data encryption algorithm.
 * @param cryptographicKey Base64 encoded key for client to decrypt lead data.
 * @param id Subscription ID.
 * @param leadFormId Lead form ID.
 * @param userAccountId User account used to subscribe lead data.
 * @param webhookUrl Standard HTTPS webhook URL.
 * @param partnerAccessToken Partner access token. Only for clients that requires authentication. We recommend to avoid this param.
 * @param partnerMetadata Partner metadata. Only for clients that requires special handling. We recommend to avoid this param.
 * @param partnerRefreshToken Partner refresh token. Only for clients that requires authentication. We recommend to avoid this param.
 */
object LeadSubscriptionPostParamss : BaseTable<LeadSubscriptionPostParams>("LeadSubscriptionPostParams") {
    val adAccountId = text("ad_account_id") /* null */ /* The Ad Account ID that this lead form belongs to. */
    val apiVersion = text("api_version") /* null */ /* API version. */
    val createdTime = int("created_time") /* null */ /* Subscription creation time. Unix timestamp in milliseconds. */
    val cryptographicAlgorithm = text("cryptographic_algorithm") /* null */ /* Lead data encryption algorithm. */
    val cryptographicKey = text("cryptographic_key") /* null */ /* Base64 encoded key for client to decrypt lead data. */
    val id = text("id") /* null */ /* Subscription ID. */
    val leadFormId = text("lead_form_id") /* null */ /* Lead form ID. */
    val userAccountId = text("user_account_id") /* null */ /* User account used to subscribe lead data. */
    val webhookUrl = text("webhook_url") /* null */ /* Standard HTTPS webhook URL. */
    val partnerAccessToken = text("partner_access_token") /* null */ /* Partner access token. Only for clients that requires authentication. We recommend to avoid this param. */
    val partnerMetadata = long("partner_metadata") /* null */ /* Partner metadata. Only for clients that requires special handling. We recommend to avoid this param. */
    val partnerRefreshToken = text("partner_refresh_token") /* null */ /* Partner refresh token. Only for clients that requires authentication. We recommend to avoid this param. */

    /**
     * Create an entity of type LeadSubscriptionPostParams from the model
     */
    override fun doCreateEntity(row: QueryRowSet, withReferences: Boolean) = LeadSubscriptionPostParams(
        adAccountId = row[adAccountId]  /* kotlin.String? */ /* The Ad Account ID that this lead form belongs to. */,
        apiVersion = row[apiVersion]  /* kotlin.String? */ /* API version. */,
        createdTime = row[createdTime]  /* kotlin.Int? */ /* Subscription creation time. Unix timestamp in milliseconds. */,
        cryptographicAlgorithm = row[cryptographicAlgorithm]  /* kotlin.String? */ /* Lead data encryption algorithm. */,
        cryptographicKey = row[cryptographicKey]  /* kotlin.String? */ /* Base64 encoded key for client to decrypt lead data. */,
        id = row[id]  /* kotlin.String? */ /* Subscription ID. */,
        leadFormId = row[leadFormId]  /* kotlin.String? */ /* Lead form ID. */,
        userAccountId = row[userAccountId]  /* kotlin.String? */ /* User account used to subscribe lead data. */,
        webhookUrl = row[webhookUrl]  /* kotlin.String? */ /* Standard HTTPS webhook URL. */,
        partnerAccessToken = row[partnerAccessToken]  /* kotlin.String? */ /* Partner access token. Only for clients that requires authentication. We recommend to avoid this param. */,
        partnerMetadata = PartnerMetadatas.createEntity(row, withReferences) /* PartnerMetadata? */ /* Partner metadata. Only for clients that requires special handling. We recommend to avoid this param. */,
        partnerRefreshToken = row[partnerRefreshToken]  /* kotlin.String? */ /* Partner refresh token. Only for clients that requires authentication. We recommend to avoid this param. */
    )

    /**
    * Assign all the columns from the entity of type LeadSubscriptionPostParams to the DML expression.
    *
    * Usage:
    *
    * ```kotlin
    * let entity = LeadSubscriptionPostParams()
    * database.update(LeadSubscriptionPostParamss, {
    *     assignFrom(entity)
    * })
    * ```
    * @return the builder with the columns for the update or insert.
    */
    fun AssignmentsBuilder.assignFrom(entity: LeadSubscriptionPostParams) {
        this.apply {
            set(LeadSubscriptionPostParamss.adAccountId, entity.adAccountId)
            set(LeadSubscriptionPostParamss.apiVersion, entity.apiVersion)
            set(LeadSubscriptionPostParamss.createdTime, entity.createdTime)
            set(LeadSubscriptionPostParamss.cryptographicAlgorithm, entity.cryptographicAlgorithm)
            set(LeadSubscriptionPostParamss.cryptographicKey, entity.cryptographicKey)
            set(LeadSubscriptionPostParamss.id, entity.id)
            set(LeadSubscriptionPostParamss.leadFormId, entity.leadFormId)
            set(LeadSubscriptionPostParamss.userAccountId, entity.userAccountId)
            set(LeadSubscriptionPostParamss.webhookUrl, entity.webhookUrl)
            set(LeadSubscriptionPostParamss.partnerAccessToken, entity.partnerAccessToken)
            set(LeadSubscriptionPostParamss.partnerMetadata, entity.partnerMetadata)
            set(LeadSubscriptionPostParamss.partnerRefreshToken, entity.partnerRefreshToken)
        }
    }

}


